 ...
 *
 * ■ 왜
 *   28칸을 다 읽으라고 하면 아무도 안 읽는다. 사람 눈이 값진 자리는 셋뿐이다:
 *     ① 정답지와 어긋난 줄  ② 두 모델이 서로 갈린 줄  ③ 답을 못 읽은 줄(오류·판정불능)
...
function 죽는다(말) { console.error('🔴 확인 불가 — ' + 말); process.exit(1); }
...
if (!Array.isArray(결과들) || !결과들.length) 죽는다('결과 JSON 안에서 칸(results)을 못 찾았다 — promptfoo 판이 바뀌었을 수 있다');
...
for (const [, f] of 문항) {
  for (const c of f.칸) {
    if (!점수.has(c.모델)) 점수.set(c.모델, { 맞음: 0, 틀림: 0, 못잼: 0 });
    const s = 점수.get(c.모델);
    if (c.판정 === null) s.못잼++;
...
console.log(`  문항 ${문항.size} · 칸 ${결과들.length} · 의심 줄 ${올릴것.length}`);
console.log(`  (맞은 줄은 여기 안 올라온다 — 결과 전량은 ${경로})\n`);
console.log('■ 점수판 (맞음 + 틀림 + 못잼 = 문항 수)');
...
if (!올릴것.length) {
  console.log('✅ 정답지와 어긋난 줄도, 두 모델이 갈린 줄도 없다.');
