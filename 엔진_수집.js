...
function 사람이봤나_(발행경로, 상태, 확인자, 확인지문, 지금지문) {
...
/* [v9.316] 카드 지문 — 학생이 «실제로 읽는 세 자리»(고친문장·칭찬·다음미션)만 잰다.
 *   상태·포인트처럼 사람이 확인한 내용과 무관한 칸이 바뀌었다고 서명을 죽이지 않기 위해서다.
 *   ⚠ `Utilities` 는 Apps Script 층이라 회귀가 이 함수를 못 잰다 — 재는 것은 판정(`사람이봤나_`)뿐이다. */
function 카드지문_(고친문장, 칭찬, 다음미션) {
  /* 구분자 대신 «길이»를 앞에 붙인다 — 어떤 구분 문자를 골라도 본문에 그 글자가 들어오면
     경계가 흐려지고, 쓰는 문자는 한글·몽골어·영어 셋뿐이라 별난 기호를 넣을 수도 없다(v9.191). */
  const 원문 = [고친문장, 칭찬, 다음미션].map(function (s) {
