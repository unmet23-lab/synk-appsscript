...
/* ⚠ 이 배열 안에는 주석을 넣지 않는다 — `tests/safety.test.js` 가 리터럴을 **JSON.parse** 로 읽어
 *   골격 대조를 한다(09-07 실측: 안에 넣은 주석 한 줄이 시험 셋을 한꺼번에 빨갛게 만들었다).
 *
 * [v9.207] schema_ver — A-8(유호 확정) · 끝에만 붙인다
 * [v9.315] 서명 두 칸 — 「이 답장을 사람이 봤나」. 명품 습관 ㉡(트랙 §0-명품 · 유호 지시 09-07).
 *   🔴 소급 불가다: 이 행이 «만들어질 때» 어느 길로 나갔는지는 그 순간에만 참이고, `상태` 열은
 *   나중에 덮이므로 사후에 못 되짚는다. 지금 안 남기면 1기 첫 첨삭부터 영영 빈칸이다.
 *   ⚠ 왜 `상태` 하나로 안 되나 — 무인 발행('노출')과 사람이 격리를 푼 '노출'이 **같은 글자**다.
 *   그 둘을 가르는 것이 이 칸이고, 그래야 학생에게 「선생님이 보았습니다」를 거짓 없이 말할 수 있다. */
...
/* [v9.315] 발행경로 어휘 — **닫혀 있다.** 자유 문자열이면 같은 길이 여러 이름으로 쌓여
 *   「사람이 봤나」가 다시 모호해진다(오류태그를 enum 으로 못박은 것과 같은 사유). */
const 발행경로_ = { 무인: '무인', 격리: '격리', 대기: '대기', 오류: '오류' };

/* [v9.315] 「이 답장을 사람이 봤나」 — 학생에게 «선생님이 보았습니다»를 말해도 되는가의 **단일 정본**.
 *
 * ■ 왜 `상태` 하나로 못 가르나 — 무인 발행이 낸 '노출'과 사람이 격리를 풀어 만든 '노출'이
 *   시트에서 **같은 글자**다. 지금 `AI_FEEDBACK_AUTOPUBLISH = true` 라 대부분이 앞쪽인데,
 *   그 둘을 안 가르면 학생이 받는 모든 카드에 「선생님이 보았습니다」가 붙어 **매일 거짓말**이 된다.
 *
 * ■ 판정 — 사람 손이 실제로 닿는 자리는 하나뿐이다: 만들 때 '격리:'·'대기' 로 멈춘 카드를
 *   사람이 확인해 '노출'로 바꾸는 것(그 안내가 야간 메일 문구에 이미 서 있다).
 *   ⇒ 「멈췄던 길 + 지금 노출」 = 사람이 봤다.
 *
 * ■ 🔴 빈 발행경로는 «안 봤다»가 아니라 «모른다»인데, **모를 때는 말하지 않는다** —
 *   v9.315 전에 쌓인 행은 그 값이 없고, 없는 것을 참으로 읽으면 옛 카드 전량에 거짓 서명이 붙는다.
 *   철학 Ⅲ-2 「아직 모르는 것에 0을 넣지 않는다」의 이 자리 판이다.
 *
 * ■ 🔴 [v9.316] **「멈췄던 길 + 노출」만으로는 모자란다** — 09-07 심문(아스트라 P0-2)이 실제 입력을
 *   넣어 잡았다: 사람이 격리를 푼 «뒤» 문장이 바뀌어도 그 두 값은 그대로라 서명이 계속 살아 있다.
 *   그러면 서명이 가리키는 것이 **그 사람이 본 적 없는 문장**이 된다 — 없느니만 못한 거짓 증거다.
 *   ⇒ 확인 «기록»(누가·언제·무엇을 보고)까지 있어야 참으로 친다. 넷 중 하나라도 없으면 거짓이다.
 * ■ ⚠ 그래서 지금은 **언제나 거짓**이다 — 확인자를 채우는 통로가 아직 0줄이라서고, 그것이 사실이다
 *   (실제로 아무도 안 본다). 통로가 서는 날 이 자가 저절로 참을 내기 시작한다.
 * ■ [v9.317] 「언제」도 실제로 잰다 — 09-07 배포 검수(코덱스 P3)가 잡았다: 주석은 넷을 말하는데 함수는
 *   확인시각을 안 받았다. 인자 순서는 시트 칸 순서(확인자·확인시각·확인지문) 그대로다. */
function 사람이봤나_(발행경로, 상태, 확인자, 확인시각, 확인지문, 지금지문) {
  const 길 = String(발행경로 == null ? '' : 발행경로).trim();
  if (길 !== 발행경로_.격리 && 길 !== 발행경로_.대기) return false; // 무인·빈칸·낯선 값 전부 false
  if (!노출카드_(상태)) return false;                                // 멈춘 채면 아직 안 풀린 것
  if (!String(확인자 == null ? '' : 확인자).trim()) return false;    // 이름이 없으면 「사람」이 없다
  if (!String(확인시각 == null ? '' : 확인시각).trim()) return false; // 「언제」가 없으면 기록이 아니다
  /* 본 «그 문장»에 서명을 묶는다 — 지문이 없거나 지금 것과 다르면 그 서명은 죽은 것이다. */
  const 봤던 = String(확인지문 == null ? '' : 확인지문).trim();
  const 지금 = String(지금지문 == null ? '' : 지금지문).trim();
  if (!봤던 || !지금) return false;
  return 봤던 === 지금;
}

/* [v9.316] 카드 지문 — 학생이 «실제로 읽는 자리»만 잰다.
 *   상태·포인트지급처럼 사람이 확인한 내용과 무관한 칸이 바뀌었다고 서명을 죽이지 않기 위해서다.
 *   [v9.317] 「오늘의포인트」를 넣었다 — 09-07 배포 검수(코덱스 P1)가 잡았다: 첫 판은 셋(고친문장·칭찬·다음미션)만
 *   쟀는데, 오늘의포인트는 보상 수치가 아니라 학생이 읽는 몽골어 교정 설명이다(callClaudeFeedback_ 의 point_mn).
 *   그것만 바꿔도 지문이 같아 「본 적 없는 설명」에 서명이 남았다. 이제 학생이 읽는 네 자리 전부를 잰다.
 *   ⚠ `Utilities` 는 Apps Script 층이라 회귀가 이 함수를 못 잰다 — 재는 것은 판정(`사람이봤나_`)과
 *   이 함수의 «인자 넷»(tests/첨삭서명.test.js 가 소스에서 읽는다)뿐이다. */
function 카드지문_(고친문장, 오늘의포인트, 칭찬, 다음미션) {
  /* 구분자 대신 «길이»를 앞에 붙인다 — 어떤 구분 문자를 골라도 본문에 그 글자가 들어오면
     경계가 흐려지고, 쓰는 문자는 한글·몽골어·영어 셋뿐이라 별난 기호를 넣을 수도 없다(v9.191). */
  const 원문 = [고친문장, 오늘의포인트, 칭찬, 다음미션].map(function (s) {
    const t = String(s == null ? '' : s); return t.length + ':' + t;
  }).join('|');
...
/* 🔴 [09-08 검수 P1 7d7e5ecc43e7 · 2회차 ea4027be9310] 헤더 치유 «보류판» — 폭은 늘리되,
 *   **아래에 값이 있는 열은 이름을 안 덮는다**(이름이 남의 것이든, 아예 비었든 똑같이).
 *   보류한 자리를 배열로 돌려준다. 🔴 그 배열에 1행 원문은 안 담는다(아래 P1 02ab57bcd477).
 *
...
function 헤더보정보류_(sh, HEADERS) {
  ...
  HEADERS.forEach((h, i) => {
    ...
    if (지금 === h) return;                                  // 제자리
    /* 🔴 [09-08 검수 2회차 ea4027be9310] **이름이 비었어도 아래에 값이 있으면 안 덮는다.**
     *   첫 판은 「빈 이름은 덮는다 — 아침 자가 그 갈래를 따로 멈춘다」로 두었는데, 그것이 틀렸다:
     *   아침 자가 멈추는 것은 «아침 자의 일»이지 이 밤 통로가 아니다. 그래서 아침이 지키던 것
     *   (검수 86417b4327af 채택 · 「이름은 없는데 아래에 값이 있는 열」)을 밤이 그대로 우회했다.
     *   ⇒ 판정을 하나로 맞춘다: **아래에 값이 있으면 이름을 안 건드린다.**
     * 🔑 그래도 배치는 안 멈춘다 — 그 칸만 건너뛰고 나머지 칸은 이름을 세운다. */
    if (열에값있나_(sh, i + 1)) {                            // 엔진_셋업확장.js · 런타임 호출이라 로드 순서 무관
      /* 🔴 [09-08 검수 2회차 P1 02ab57bcd477] **1행 원문을 여기 담지 않는다.**
       *   머리줄이 지워지거나 잘못 붙여 넣어지면 학생 이름이 1행으로 올라오는데, 그 원문을 담으면
       *   부르는 쪽 로그가 그것을 그대로 복사한다(학생 식별 데이터의 출구 · CLAUDE.md 안전 넷).
       *   사람이 어디를 볼지 알기에는 «몇째 칸이고 정본 이름이 무엇인지»면 충분하다. */
      보류.push((i + 1) + '열(정본 「' + h + '」) — ' + (지금 ? '다른 이름' : '이름 없음') + ' · 아래에 값 있음');
      return;
    }
 ...
 *   (현재 경계 = 상담AI.js doGet), Glide는 월 500 update 상한이 있어 왕복 대화를 감당하지 못한다. — 구 Glide(08-05 폐기 · 이관 = docs/글라이드_이관대장.md)
 ...
 *   ② 이름·번호를 제거해도 개발·공개 사용에 대한 목적별 명시적 동의를 대신하지 않는다.
 ...
 */
function 골든픽스처_() {
  /* [2026-09-11] 학생 원문을 개발 평가자료로 만드는 출구는 현재 중지한다.
   * 학습용 일반 동의 yes·예전 동의 버전·운영자 클릭·공개 출구 설정은 해당 사용에 대한 학생 동의가 아니다.
   * 대상 자료와 목적·제공 범위에 대한 기록된 명시적 동의 및 철회 상태를 확인하는 구현이 없으므로,
   * 원문을 읽기 전에 차단한다. 설정값만으로 해제하는 통로는 두지 않는다. */
  const 목적별반출동의확인 = false;
  if (!목적별반출동의확인) return '학생 원문을 사용하는 평가자료 반출은 중지되어 있습니다. 해당 개발·공개 사용에 대한 기록된 명시적 동의와 철회 상태를 확인하는 절차가 구현되기 전에는 내 드라이브·저장소로 내보내지 않습니다. 학습용 일반 동의나 이름 제거만으로 허용되지 않습니다.';
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const gd = ss.getSheetByName('teacher_gold');
 ...
 *      이름 살균(`명단이름_`·`이름살균_`)이 이 출구의 선행이 됐다. 2026-09-11 현재는 목적별 동의 확인이 없어 두 출구 모두 중지한다.
 *   ④ **자동 배치에 넣지 않는다** — 운영자 클릭은 실행 의사만 나타내며 학생의 목적별 명시적 동의를 대신하지 않는다.
...
function 골든전송점검_() {
  ...
  return { ok: true, msg: '연결 OK — ' + GH_OWNER + '/' + GH_REPO + ' 쓰기 권한 확인. 연결 확인은 반출 허용이 아닙니다. 학생 원문의 개발·공개 사용에 대한 목적별 동의 확인이 없어 내 드라이브·저장소 반출은 중지되어 있습니다.' };
}
...
function pushGoldenFixture_() {
   ...
   *   GOLDEN_PUBLIC_EXIT=on은 운영자 출구 설정일 뿐 학생의 동의가 아니다. 골든픽스처_의 목적별 동의 차단을 해제하지 않는다. */
  if (String(PropertiesService.getScriptProperties().getProperty('GOLDEN_PUBLIC_EXIT') || '').trim() !== 'on') {
    return '공개 저장소 출구는 꺼져 있습니다(유호 위임 2026-09-06 · 결정 원장 안전 행).\n'
      + '「📤 강사 정답 모음 → 픽스처 파일로」(내 드라이브) 경로도 목적별 명시적 동의 확인이 없어 중지되어 있습니다.\n'
      + '출구 설정이나 학습용 일반 동의만으로 학생 원문 반출을 허용하지 않습니다.';
  }
  const r = 골든픽스처_();
  if (typeof r === 'string') return r;
  const token = PropertiesService.getScriptProperties().getProperty(GH_TOKEN_KEY);
  ...
  if (!r.건수) {
    const c = 골든전송점검_();
    return '채점에 쓸 항목이 0건이라 올리지 않았습니다(' + r.요약 + ').'
      + '\n\n' + (c.ok ? '✅ ' : '⚠️ ') + c.msg;
  }
...
function 증언비식별_(이름들, 글) {
  ...
  if (/\d{4}\s*년\s*생|\d{4}\s*년(?:에)?\s*(?:태어|입학|졸업)|төрсөн|born in/i.test(s)) return null;   // 「2020년생」·「2023년에 입학」·「төрсөн」(태어났다) — 연도가 최근이어도 생년·입학년 문맥이면 식별([v9.328])
  ...
  if (/сарын\s*\d{1,2}/i.test(s)) return null;                                                // 3 сарын 5 · Тавдугаар сарын 3 · 3 дугаар сарын 5 — «달 + 날 숫자»면 날짜([v9.328] 글자 달도)
  if (/academ(?:y|ies)\b|institutes?\b|kindergartens?\b/i.test(s)) return null;               // 기관 낱말(복수·붙여 쓴 꼴도 · academic 은 아니다)
  if (/[가-힣]+(?:대로|로|길)\s*\d+\s*(?:번길|번지|번|호|층|동|-|,|에|입니다|이에요|예요|\.|$)/.test(s)) return null;  // 테헤란로  12에 · 종로 1에 · 가상로 1입니다 — 「한국어로 3문장」은 아니다
  return s;
}
...
/** 학생·보호자의 말로 외부 사용 판을 만드는 출구. 비식별 검사는 목적별 동의를 대신하지 않는다.
 *  현재는 개발·공개 사용에 대한 기록된 명시적 동의와 철회 상태를 확인할 구현이 없어 중지(null).
 *  내부 학습 맥락에서 읽는 증언맵_과 이 반출 출구는 별개다. */
function 증언반출_(ss) {
  const 목적별반출동의확인 = false;
  if (!목적별반출동의확인) return null;
  const 이름들 = 명단이름_(ss);
  if (!이름들) return null;
  const sh = ss.getSheetByName(TESTIMONY_TAB_);
