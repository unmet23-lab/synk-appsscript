...
/* [v9.211] 「닫힘」 판정 — 커서·재시도 로직이 이 행을 **지나가도 되는가**. 노출카드_(노출 여부)와 축이 다르다:
 * 격리·오류는 미노출이면서 다시 열리지 않아 지나가도 되고, 대기·빈칸은 미노출이면서 **판정 전**이라
 * 지나가면 검토자가 나중에 승인해도 커서 뒤에 남아 영구 누락된다(재검수 P1 — 오류뱅크 커서가 그 모양이었다).
 * 낯선 값·앞뒤 공백은 닫힘이 아니다 — 틀릴 때 방향이 「유실」이 아니라 「지연」이어야 한다. */
function 닫힌카드_(상태) {
  const s = String(상태 == null ? '' : 상태).trim();
  return s.indexOf('격리') === 0 || s.indexOf('오류') === 0;
}
...
function dataCoverageMonthly_() {
  ...
  if (s.골든응답 && !PropertiesService.getScriptProperties().getProperty(GH_TOKEN_KEY)) 처방.push(
    '④ 골든셋 응답 ' + s.골든응답 + '건이 쌓였는데 회화 앱으로 보낼 통로가 아직 안 열려 있습니다.\n' +
...
function 골든픽스처_() {
  ...
  rows.forEach(r => {
     ...
     *   AI교정이 정답으로 실린다.** Glide 「골든셋」 조립 중 실측된 두 경로:
     ...
     *     (조립가이드 v947 §8-5). 모르는 문자열은 정답으로 치지 않고 미응답으로 센다. */
    let 교정, 출처;
