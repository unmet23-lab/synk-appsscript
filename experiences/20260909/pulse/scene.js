...
export async function createWorld({ container, hotspots, onInspect, onStatus }) {
  ...
  renderer.domElement.setAttribute('aria-label', '실시간 3D 도시. 드래그 또는 방향키로 둘러보고 휠로 확대할 수 있습니다.');
  container.append(renderer.domElement);

  const scene = new THREE.Scene();
  scene.background = new THREE.Color(0x233961);
  scene.fog = new THREE.FogExp2(0x263c62, .011);
  ...
  const lights = [];
  const glowSprites = [];
  const fiberPositions = [], fiberColors = [];
  const sun = new THREE.DirectionalLight(0xc5d6f5, 1.15);
  ...
  scene.add(new THREE.HemisphereLight(0xc5d6f5, 0x575046, .43));
  const warmFill = new THREE.DirectionalLight(C.butterSoft, .12);
  warmFill.position.set(12, 8, 16);
  scene.add(warmFill);
  ...
  function textile() {
    ...
    for (let i = 0; i < data.data.length; i += 4) {
      const v = rnd(120, 210); data.data[i] = data.data[i + 1] = data.data[i + 2] = v; data.data[i + 3] = 255;
    }
    ctx.putImageData(data, 0, 0);
    ctx.lineCap = 'round';
    for (let i = 0; i < 54000; i++) {
      const x = rnd(0, 512), y = rnd(0, 512), a = rnd(0, Math.PI * 2), l = rnd(2, 15);
      const v = Math.floor(rnd(95, 245));
      ctx.strokeStyle = `rgba(${v},${v},${v},${rnd(.1,.42)})`; ctx.lineWidth = rnd(.4, 1.3);
      ctx.beginPath(); ctx.moveTo(x, y); ctx.quadraticCurveTo(x + Math.cos(a + .4) * l * .6, y + Math.sin(a + .4) * l * .6, x + Math.cos(a) * l, y + Math.sin(a) * l); ctx.stroke();
    }
  ...
  let woolTexture = textile(), officialTextile = false;
  try {
    const officialFelt = await new THREE.TextureLoader().loadAsync(new URL('./assets/felt-neutral.avif', import.meta.url).href);
    officialFelt.wrapS = officialFelt.wrapT = THREE.RepeatWrapping;
    officialFelt.anisotropy = Math.min(renderer.capabilities.getMaxAnisotropy(), 8);
    woolTexture.dispose(); woolTexture = officialFelt; officialTextile = true;
  } catch { /* A locally generated fibre texture keeps the scene usable if the art tile is unavailable. */ }
  const woolColor = woolTexture.clone(); woolColor.colorSpace = THREE.SRGBColorSpace;
  function felt(color, roughness = .97, bump = .105) {
    return new THREE.MeshPhysicalMaterial({ color, map: woolColor, roughness, metalness: 0, bumpMap: woolTexture, bumpScale: bump * 1.5, sheen: .6, sheenRoughness: .88, sheenColor: new THREE.Color(color).lerp(new THREE.Color(C.oat), .16) });
  }
  const m = {
    paving: felt(0x777979, .84, .07), pavingLight: felt(C.ash, .87, .08), curb: felt(C.stone), masonry: felt(0x85817b), cream: felt(C.stone), paper: felt(C.oat), wallDark: felt(0x59606b), roof: felt(C.indigo), blue: felt(C.lapisDeep), trim: felt(C.oat), dark: felt(0x30323b), trunk: felt(C.wool), leaf: felt(0x495652), leafLight: felt(0x636e60), rope: felt(C.stone), coral: felt(C.coralDeep), brass: felt(0xb7a37d, .5, .06), ocean: felt(0x243b56, .39, .17), rock: felt(0x515c67), hill: felt(0x3d4d62), wood: felt(0x71665a), stitch: felt(C.oat), windowFrame: felt(C.ink), rubber: felt(0x23242b)
  };
  m.stone = m.cream;
  m.window = new THREE.MeshStandardMaterial({ color: 0x8e7851, emissive: C.butter, emissiveIntensity: .7, roughness: .85 });
  m.windowDim = new THREE.MeshStandardMaterial({ color: 0x8e8564, emissive: C.butterSoft, emissiveIntensity: .15, roughness: .85 });
  ...
  function boxGeo(w, h, d, r = .07) {
    ...
    for (let i = 0; i < p.count; i++) {
      ...
      uv.setXY(i, (ay > ax && ay > az ? p.getX(i) : ax > az ? p.getZ(i) : p.getX(i)) * .43, (ay > ax && ay > az ? p.getZ(i) : p.getY(i)) * .43);
    }
  ...
  function seam(a, b, parent = root, color = m.stitch, step = .22) {
    ...
    for (let s = 0; s < length; s += step) rod(av.clone().lerp(bv, s / length).toArray(), av.clone().lerp(bv, Math.min(1, (s + step * .48) / length)).toArray(), .016, color, parent);
  }
  function fuzz(o, count = 260, length = .05) {
    o.updateWorldMatrix(true, false);
    const g = o.geometry, p = g.attributes.position, n = g.attributes.normal, index = g.index;
    const c = new THREE.Color(o.material.color || C.stone).lerp(new THREE.Color(C.oat), .18);
    ...
    count=Math.floor(count*.72);length*=.34;
    for (let i = 0; i < count; i++) {
      ...
      normal.fromBufferAttribute(n, ia).applyMatrix3(normalMatrix).normalize();
      const end = point.clone().addScaledVector(normal, rnd(length * .45, length * 1.4)); end.x += rnd(-.018, .018); end.z += rnd(-.018, .018);
      fiberPositions.push(...point.toArray(), ...end.toArray());
      const cc = c.clone().multiplyScalar(rnd(.25, .62)); fiberColors.push(cc.r, cc.g, cc.b, cc.r, cc.g, cc.b);
    }
  }
  function softDisc() {
  ...
  function shadow(x, z, sx, sz, opacity = .55) {
    ...
    const o = mesh(new THREE.PlaneGeometry(sx, sz), mat, x, .025, z); o.rotation.x = -Math.PI / 2; o.castShadow = false; return o;
  }
  ...
  // The night sky is a painted procedural cloud field on a surrounding 3D dome.
  const skyCanvas = document.createElement('canvas'); skyCanvas.width = 1024; skyCanvas.height = 512;
  const skyCtx = skyCanvas.getContext('2d');
  const skyGradient = skyCtx.createLinearGradient(0, 0, 0, 512); skyGradient.addColorStop(0, '#142b53'); skyGradient.addColorStop(.45, '#355886'); skyGradient.addColorStop(.64, '#647183'); skyGradient.addColorStop(.74, '#68717b'); skyGradient.addColorStop(1, '#263951'); skyCtx.fillStyle = skyGradient; skyCtx.fillRect(0, 0, 1024, 512);
  for (let i = 0; i < 700; i++) { const x = rnd(-40, 1064), y = rnd(30, 310), rx = rnd(10, 75), ry = rnd(2, 14); skyCtx.fillStyle = `rgba(15,29,51,${rnd(.012,.052)})`; skyCtx.beginPath(); skyCtx.ellipse(x, y, rx, ry, rnd(-.08,.08), 0, Math.PI * 2); skyCtx.fill(); }
  const skyTex = new THREE.CanvasTexture(skyCanvas); skyTex.colorSpace = THREE.SRGBColorSpace;
  const sky = new THREE.Mesh(new THREE.SphereGeometry(190, 48, 32), new THREE.MeshBasicMaterial({ map: skyTex, side: THREE.BackSide, depthWrite: false, fog: false })); sky.position.y = 8; scene.add(sky);
  ...
  const waterGeo = new THREE.PlaneGeometry(230, 240, 80, 100); waterGeo.rotateX(-Math.PI / 2);
  ...
  function building({ x, z, w = 6, d = 6, h = 10, material = m.cream, style = 0, floors = 3 }) {
    ...
    for (let y = 1.85; y < h - .7; y += h / floors) {
      for (let xx = -w / 2 + 1.15; xx < w / 2 - .6; xx += 1.75) {
        ...
        box(1.0, 1.56, .14, m.windowFrame, xx, y, d / 2 + .05, g, .07);
        box(.78, 1.35, .1, lit, xx, y, d / 2 + .14, g, .05);
        box(.055, 1.4, .12, m.wood, xx, y, d / 2 + .23, g, .01);
        box(.88, .055, .12, m.wood, xx, y + .1, d / 2 + .23, g, .01);
        box(1.14, .14, .35, m.stone, xx, y - .83, d / 2 + .11, g, .025);
        if (lit === m.window && z > -15 && y < 4) glow(xx, y, d / 2 + .28, 2.5, C.butter, .075, g);
      }
      for (let zz = -d / 2 + 1.15; zz < d / 2 - .4; zz += 1.75) {
        box(.14, 1.56, 1, m.windowFrame, -w / 2 - .03, y, zz, g, .05);
        box(.1, 1.3, .78, rnd() > .43 ? m.window : m.windowOff, -w / 2 - .12, y, zz, g, .04);
        box(.12, 1.32, .055, m.wood, -w / 2 - .21, y, zz, g, .01);
      }
    }
    for (let q = 0; q < 2; q++) {
  ...
  const chassis = box(2.7, 2.55, 5.8, m.blue, 0, 1.68, 0, tram, .28); fuzz(chassis, 7200, .047);
  ...
  for (const x of [-1.05, 0, 1.05]) {
    box(.83, 1.15, .12, m.windowFrame, x, 2.26, 2.88, tram, .05);
    box(.66, .98, .12, m.window, x, 2.26, 2.96, tram, .04);
    box(.04, 1.02, .06, m.wood, x, 2.26, 3.035, tram, .008);
  }
  for (const x of [-1.39, 1.39]) for (let z = -2.1; z < 2.4; z += 1.12) {
    box(.12, 1.22, .97, m.windowFrame, x, 2.25, z, tram, .05);
    box(.1, 1.04, .77, m.window, x * 1.02, 2.25, z, tram, .04);
  ...
  const tramLight = new THREE.PointLight(C.butter, 14, 10, 2); tramLight.position.set(0, 2.5, 1.5); scene.add(tramLight);
  shadow(.3, -2, 5.5, 9, .75);
  ...
  function cypress(x, z, h) {
    ...
    for (let i = 0; i < 5; i++) { const leaf = sphere(h * (.16 - i * .018), i % 2 ? m.leaf : m.leafLight, x + rnd(-.1, .1), h * .42 + i * h * .13, z, root, .7, 1.75, .7); if (z > -20) fuzz(leaf, 1000, .075); }
  }
  ...
  root.traverse(o => {
    if(!o.isMesh || o===beacon || o.material.transparent || Array.isArray(o.material))return;
  ...
  for(const {material,geometries:gs} of buckets.values()) {
    ...
    if(combined){const o=new THREE.Mesh(combined,material);o.castShadow=!material.emissive?.getHex()&&![m.paving,m.pavingLight,m.ocean].includes(material);o.receiveShadow=true;scene.add(o);}
  }
  const fiberGeo=new THREE.BufferGeometry();fiberGeo.setAttribute('position',new THREE.Float32BufferAttribute(fiberPositions,3));fiberGeo.setAttribute('color',new THREE.Float32BufferAttribute(fiberColors,3));
  const fibers=new THREE.LineSegments(fiberGeo,new THREE.LineBasicMaterial({vertexColors:true,transparent:true,opacity:.48}));scene.add(fibers);

  const composer=new EffectComposer(renderer);
  composer.addPass(new RenderPass(scene,camera));
  const bloom=new UnrealBloomPass(new THREE.Vector2(container.clientWidth,container.clientHeight),.2,.45,1.1);composer.addPass(bloom);composer.addPass(new OutputPass());
  const poses={
    street:{position:vec(5.3,3.9,18.6),target:vec(-1.7,3.7,-13)},
    overview:{position:vec(30,29,32),target:vec(-3,2,-12)},
    tram:{position:vec(-5.9,3.7,7.7),target:vec(.1,2.6,-2.3)},
    radio:{position:vec(1.5,4.0,7.4),target:vec(10.8,4,-4.2)},
    buoy:{position:vec(-5.8,3.5,2.4),target:vec(-18,1.4,-12)},
    postbox:{position:vec(-2.4,3.1,17.3),target:vec(-7.3,1.9,11.3)},
  ...
  const focusTarget=poses.street.target.clone();camera.position.copy(poses.street.position);camera.lookAt(focusTarget);
  let desiredPosition=camera.position.clone(),desiredTarget=focusTarget.clone();
  const objectInfo=[
    {id:'tram',title:'전차 정류장',point:vec(.1,4.6,-1),role:'signal'},
    {id:'radio',title:'방송국',point:vec(9.3,5.2,-4),role:'archive'},
    {id:'buoy',title:'항구의 부표',point:vec(-13.5,2.3,-6.5),role:'coast'},
    {id:'postbox',title:'빨간 우체통',point:vec(-7.2,3.3,12.7),role:'courier'}
  ];
  const hitVolumes=[];
  ...
  let pointer=null,dragged=false,endingChoice=null;
  const pointerVec=new THREE.Vector2(),raycaster=new THREE.Raycaster();
  function focus(id){const pose=poses[id]||poses.street;desiredPosition.copy(pose.position);desiredTarget.copy(pose.target);if(coarse && !['street','overview'].includes(id)){desiredPosition.y+=1;desiredTarget.y-=.2;}if(reduced){camera.position.copy(desiredPosition);focusTarget.copy(desiredTarget);}onStatus?.({place:objectInfo.find(o=>o.id===id)?.title||'서쪽 해안, 오래된 전차 광장'});}
  function orbit(dx,dy){
    const offset=desiredPosition.clone().sub(desiredTarget);const sph=new THREE.Spherical().setFromVector3(offset);sph.theta-=dx*.004;sph.phi=clamp(sph.phi+dy*.003, .24,1.49);offset.setFromSpherical(sph);desiredPosition.copy(desiredTarget).add(offset);
  }
  renderer.domElement.addEventListener('pointerdown',e=>{if(e.button!==0)return;pointer={x:e.clientX,y:e.clientY,startX:e.clientX,startY:e.clientY};dragged=false;renderer.domElement.setPointerCapture(e.pointerId);});
  renderer.domElement.addEventListener('pointermove',e=>{if(!pointer)return;const dx=e.clientX-pointer.x,dy=e.clientY-pointer.y;if(Math.hypot(e.clientX-pointer.startX,e.clientY-pointer.startY)>5)dragged=true;orbit(dx,dy);pointer.x=e.clientX;pointer.y=e.clientY;});
  renderer.domElement.addEventListener('pointerup',e=>{if(pointer&&!dragged&&phase!=='entry'){
    ...
    pointer=null;renderer.domElement.releasePointerCapture(e.pointerId);
  });
  renderer.domElement.addEventListener('pointercancel',()=>{pointer=null;});
  renderer.domElement.addEventListener('wheel',e=>{e.preventDefault();const o=desiredPosition.clone().sub(desiredTarget);o.setLength(clamp(o.length()*Math.exp(e.deltaY*.0008),4,85));desiredPosition.copy(desiredTarget).add(o);},{passive:false});
  renderer.domElement.addEventListener('keydown',e=>{if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','+','-','Home'].includes(e.key)){e.preventDefault();if(e.key==='Home')focus('street');else if(e.key==='+'||e.key==='-'){const o=desiredPosition.clone().sub(desiredTarget);o.setLength(clamp(o.length()*(e.key==='+'?.9:1.1),4,85));desiredPosition.copy(desiredTarget).add(o);}else orbit(e.key==='ArrowLeft'?-45:e.key==='ArrowRight'?45:0,e.key==='ArrowUp'?-35:e.key==='ArrowDown'?35:0);}});
  ...
  function frame(now){
    ...
    const smooth=reduced?1:1-Math.exp(-dt*5.3);camera.position.lerp(desiredPosition,smooth);focusTarget.lerp(desiredTarget,smooth);camera.lookAt(focusTarget);
    if(!reduced){
    ...
    if(phase!=='entry')for(const item of objectInfo){
      ...
      item.button.style.visibility=visible?'visible':'hidden';item.button.style.transform=`translate3d(${x.toFixed(1)}px,${y.toFixed(1)}px,0) translate(-50%,-100%)`;
    }
  ...
  setQuality('auto');resize();requestAnimationFrame(frame);
  return {
    focus,
    setQuality,
    setReducedMotion(value){reduced=Boolean(value);if(reduced){camera.position.copy(desiredPosition);focusTarget.copy(desiredTarget);}},
    setPhase(next){phase=next;hotspots.inert=next==='entry';},
    ...
    setEnding(choice){
      ...
      else if(choice==='homes'){m.window.emissiveIntensity=1.65;for(const l of lights)l.intensity=24;}
      else {m.window.emissiveIntensity=.7;for(const l of lights)l.intensity=35;tramSignal.intensity=5;}
      if(choice)focus(choice);
    },
    getMetrics(){return {renderer:'Three.js r180 / WebGL2',geometries:renderer.info.memory.geometries,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,pixelRatio:renderer.getPixelRatio(),quality,autoReduced,reflections:puddles.visible,silhouetteFibres:fibers.visible,reducedMotion:reduced,textile:officialTextile?'SYNK felt tile':'procedural fallback'};},
    dispose(){active=false;observer.disconnect();composer.dispose();puddles.dispose();renderer.dispose();}
  };
}
