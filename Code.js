...
const AI_FEEDBACK_MODEL = 'claude-opus-5'; // 라우팅 정본 = tools/모델정책.js(유호 확정 2026-08-10 「올려」 — 옛 「콘텐츠=Sonnet급」 조항은 v8.8 에서 폐기). ⚠ 이 리터럴의 소비자가 셋 더 있다 — 엔진_두뇌.js 폴백 · 상담AI.js 폴백(상담AI_모델_) · 상담AI.js 상담AI_단가 표의 키. 여기만 고치면 폴백 둘은 조용히 옛 모델로 돌고, 단가 표는 새 모델 키가 없어 '' 옛 단가로 조용히 읽힌다(상담AI.js 상담AI_단가 의 '' 항목 규약) — 모델 교체는 네 자리 한 벌이다. API 키는 Script Properties 'CLAUDE_API_KEY' — 없으면 배치 전체 스킵(NOTION_TOKEN 패턴, 배포만으로는 아무 일도 안 일어남)
