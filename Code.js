...
function createConsultForm() { // [v8.4] 상담지 v2 · 시트 v18.1 완전 정합 — 문항 제목=시트 헤더명, 선택지=시트 드롭다운 값
  ...
  txt('영어수준').setHelpText('예: TOEIC 800 / 회화 중');
  ...
  txt('목표기한').setHelpText('예: 2027-06');
  txt('대학진학시기').setHelpText('예: 2027년 3월');

  form.addSectionHeaderItem().setTitle('PART 3 — 한국 경험과 비자');
  mc('방문경험', ['없음', '관광', '단기연수', '장기체류']);
  mc('비자이력', ['없음', '승인', '거절'], true);
  mc('거절비자', ['C-3', 'D-4', 'D-2', '기타']).setHelpText('비자 거절 경험이 있을 때만');
  mc('가족체류', ['없음', '현재합법체류', '특이사항'], true);

  form.addSectionHeaderItem().setTitle('PART 4 — 학업 계획과 재정');
  mc('진학과정', ['어학연수', '전문대', '학사', '편입', '석사', '박사', '취업비자준비']);
  mc('진학지역', ['서울', '경기·인천', '지방국공립', '상관없음']);
  mc('현재직업', ['학생', '직장인', '구직', '기타']);
  mc('학습시간', ['1시간 미만', '1~2시간', '2~3시간', '3시간 이상']).setHelpText('하루 평균 공부 시간');
  mc('달성시기', ['6개월(집중)', '1년(표준)', '2년(장기)']);
  mc('졸업후진로', ['한국취업', '몽골귀국', '미정']);
  mc('핵심가치관', ['안정성', '고수입', '보람성취', '워라밸']);
  txt('지망대학1'); txt('지망대학2'); txt('지망대학3');
  txt('지망전공1'); txt('지망전공2'); txt('지망전공3');
  mc('유학예산', ['여유있음', '보통', '빠듯함'], true);
  mc('학비조달', ['부모님지원', '본인저축', '정부·기관장학금', '한국내근로', '기타'], true);
  ...
  mc('영어수업', ['매우희망', '보통', '없음']).setHelpText('SYNK 영어 수업 (GLOBAL ENGLISH) 참여 희망');
  para('영어 학습 목표 (선택)');
...
function importFormResponses() {
  ...
  try {
    ...
    responses.forEach(resp => {
      ...
      if (narrative.length && colOf['📝노션이관'] && colOf['📝노션이관'] <= 59) {
        rowArr[colOf['📝노션이관'] - 1] = narrative.join('\n\n');
      }
