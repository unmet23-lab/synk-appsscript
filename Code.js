...
function calcAll() {
   ...
   *
   * 🔑 갈래가 «셋»인 까닭(08-27~28 · 검수 3회전이 하나씩 드러냈다):
   *   ①**모자람** — 「행이 0개」만 보던 때는 그림이 영영 안 들어갔다. 라이브엔 이미 guide 3행이 서 있고
   *     E열만 비어서, 아바타를 씨앗에 넣어 배포해도 조건이 거짓이라 손 클릭을 기다렸다
   *     (「수동 실행 0」이라 적어 놓고 수동을 요구하던 자리다). 개수가 아니라 «이름»으로 센다.
   *   ②**군더더기** — 정본 셋이 다 있어도 낡은 행이 붙어 있으면 화이트리스트가 그 이름까지 인정한다.
   *   ③**그림전멸** — 씨앗엔 그림이 있는데 시트엔 «렌더 가능한» 것이 한 장도 없을 때.
   *   셋 다 setupGuides 는 보존 병합이라 사람이 채운 URL 은 그대로 살고, 한 번 정리되면 다음 회차부터
   *   조건이 거짓이 된다(멱등). */
  ...
  /* 씨앗에 «없는» 낡은 행(G99 같은 것)이 붙은 경우도 정리한다(codex c3ba45f1). 안 그러면 정본 셋이
   * 다 있다는 이유로 복구가 안 돌고, 아래 화이트리스트가 그 낡은 이름까지 유효한 픽으로 인정한다. */
  const 군더더기9 = 가이드행9.some(r => !씨앗이름9[이름9(r)]);
  ...
  if (모자람9 || 군더더기9 || 그림전멸9) {
    try {
      ...
      for (let i9 = ctData.length - 1; i9 >= 0; i9--) {
        if (ctData[i9][1] === 'guide' && !씨앗이름9[이름9(ctData[i9])]) ctData.splice(i9, 1);
      }
    } catch (e9) {
