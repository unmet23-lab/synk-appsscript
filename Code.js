/**********************************************************
 * ═══════════════════════════════════════════════════════════
 * 🆘 SYNK 운영 엔진 복구 안내 — 현재 저장소의 배포 집합을 사용합니다
 * ═══════════════════════════════════════════════════════════
 * Code.js는 엔진의 일부입니다. 엔진_*.js·contents_*.js·manifest 등
 * .claspignore와 .clasp.json이 지정하는 파일/순서를 함께 복원합니다.
 * 데이터 흐름과 외부 연결의 현행 안내는 docs/SYNK_동기화_구조.md입니다.
 ...
 *   2) 현재 승인된 전체 배포 집합을 기존 배포 도구로 복원(이 파일 단독 붙여넣기 금지)
 *   3) bootstrapSynk() 1회 실행 (권한 승인 2회) — 세계가 재림합니다
 *   4) resetAllTriggers() 1회 실행 — 기대 예약은 triggerManifest_의 현재 목록으로 확인
 ...
 *      · 현행 연결: CLAUDE_API_KEY·ROSTER_INGEST_*·해당 Meta 상담 채널 설정
 *        설정 존재는 호출 성공이 아니다. 실제 연결/작업 결과를 별도로 확인한다.
 *        Notion은 사용 종료했다. NOTION_TOKEN을 복원하거나 새로 발급하지 않는다.
...
const PARENT_Q = [
  ...
  '«Өнөөдөр хичээл дээр юу хамгийн сонирхолтой байсан бэ?» (오늘 수업에서 제일 재미있었던 게 뭐야?)',
