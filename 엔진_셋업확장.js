...
function updateBizDashboard(asText, kpiData) { // 계기판 시트 갱신 + 요약 텍스트 반환 (주간 통합 리포트 섹션 규약)
  ...
  if (sh.pay.getLastRow() >= 2) {
    ...
    sh.pay.getRange(2, 1, sh.pay.getLastRow() - 1, 6).getValues().forEach(r => {
      ...
      /* 🔴 [검수 P1 94aab534] 「환불」이라는 «글자»만 보면 비고의 **「환불 없음」도 환불**이 된다.
       *   재현: 양수 납부 한 줄 + 비고 「환불 없음」 ⇒ 1/1명 = 100%.
       *   ⇒ 돈이 «들어온» 줄(금액 양수)은 환불 줄이 아니다. 글자 신호는 금액이 없는 줄에서만 읽는다. */
      if (amt < 0 || (amt === 0 && 말.indexOf('환불') >= 0)) 돌려받은사람[sid] = 1;
    });
  ...
  const rows = [
    ...
    ['⑦ 환불 비율(누적)', payerN === 0 ? '— (결제 기입 전)' : refundN + '/' + payerN + '명 = ' + refundPct + '%', '⚪',
      '분모는 payments 에서 «납부일이 찍힌» 사람이다 — leads 「등록」도 정원 16도 아니다. 환불 = 금액이 음수이거나, 금액 없이 방법·비고에 「환불」'],
