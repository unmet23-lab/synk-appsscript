...
function 수집도달_() {
  return {
    ...
    /* 🔴 [09-04] **읽는 곳이 없어졌다** — 유호 확정 09-04 「연습 노트는 걷어줘」로 유일한 소비자였던
     *   `buildFocusNotes_`(교재연동.js)가 걷혔다. 이 시트는 이제 «쌓이기만 하고 아무도 안 읽는다».
     * 🔑 그래도 계속 쌓는 까닭: 무엇을 못 넘었는지는 **그날에만 알 수 있다**(소급 불가). 나중에
     *   읽는 곳을 새로 지으면 그 기록이 이미 있어야 값이 산다 — 지금 멈추면 그 창이 닫힌다.
     *   ⇒ 「도달 0」이 결함이 아니라 **알고 남긴 상태**다. 되살릴 자리는 git 이력에 그대로 있다. */
    'mastery_log': { 소비자: null, 층: '제품', 사유: '09-04 연습 노트 폐지로 소비자 0 — 수집은 계속(소급 불가)' },
...
function 시트도달상한_() {
   ...
   *   📈 6 (2026-09-04 · **분모는 그대로 · 소비자가 사라졌다** — `mastery_log`. 유호 확정 09-04
   *      「연습 노트는 걷어줘」로 그 시트의 **유일한 소비자**였던 `buildFocusNotes_`(교재연동.js)가 걷혔다.
   *      🔑 앞의 다섯과 결이 다르다 — 그것들은 «새 원장을 열어» 분모가 는 것이었고, 이번은 **읽던 곳이
   *      없어진 것**이다. 빚의 종류로는 이쪽이 더 나쁘다(쌓기만 하고 안 읽는다).
   *      ⚠ 그래도 **수집은 멈추지 않는다** — 무엇을 못 넘었는지는 그날에만 알 수 있어 소급이 안 된다.
   *      지금 멈추면 나중에 읽는 곳을 지어도 그날 기록이 없다(v9.197 자기선언 이력이 그은 선 그대로).
   *      ▶ 되갚는 조건: `mastery_log` 의 '연습'(미도달) 상태를 읽어 학생에게 되돌리는 자리가 새로 서는 날.
   *         교재를 안 쓰는 재료(급수 요목·문법 뱅크)로 지으면 09-03 확정과도 안 부딪힌다.
   *      ✅ 재고 올렸다 — 장부 등재(소비자 null) → 실측 6 → 상한 순서를 지켰다.) */
  return { 도달0: 6, 손: 1 };
}
