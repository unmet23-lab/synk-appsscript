...
function sheetSkeleton_() {
  return [
    ...
    ['접점밀도', ['#','접점','점수','근거','정본','판정일','판정자']], // [㉠-3 · 09-07] 접점 열둘 × 0~3 — 최저점이 원장 오늘판 첫 줄 · 사람이 고치는 표(수집 아님 · 헤더 정본은 이 줄 하나)
    ...
    [TESTIMONY_TAB_, TESTIMONY_HEADERS, 수집표식_], // [㉡-2 · 09-07] 학생이 스스로 한 말 — 번호와 함께 · 소급 불가(그날 그 말은 그날에만) · 상수는 엔진_콘텐츠AI.js(소비자 파일)
...
function 시트칸정본맞추기_(ss, 옵션) {
  ...
  /* 🔴 [09-08 검수 P1 a8832d0e0226 · P2 08dca71aeab1] 결과를 «밖에서 준 그릇»에 담을 수 있게 한다.
   *   안 그러면 도중에 예외가 났을 때 그때까지의 실적(늘린 칸 · 민 열 · 스위치 상태)이 통째로 사라지고,
   *   부르는 쪽은 「0칸 · 밀기 꺼짐」이라는 **거짓 기록**을 남긴다(실제로는 늘리고 옮긴 뒤였는데도).
   *   그릇을 안 주면 옛날처럼 새로 만든다 — 부르는 쪽 한 곳만 바뀐다. */
  const 결과 = (옵션 && 옵션.그릇) || { 맞춘표: [], 민열: [], 건너뛴표: [], 보류표: [], 더한칸: 0, 밀기: 밀기허용 };
  결과.밀기 = 밀기허용;
  sheetSkeleton_().forEach(function (k) {
    ...
    /* 🔴 [09-08 검수 P1 bddf9a7aad1d] 늘린 칸은 **아무것도 만지기 «전»** 폭을 기준으로 센다.
     *   옛 판은 `헤더보정_` 직전에 폭을 잡아, 그 «앞»에서 늘린 것(정본 폭 맞추기 · 미는 자리 만들기)이
     *   기록에서 통째로 빠졌다 — 실제로 여러 칸을 늘리고도 「더한칸 0」으로 남았다. */
    const 시작폭 = sh.getMaxColumns();
    /* 🔴 [09-08 검수 P1 d308df9a6738] 표 하나를 **통째로** 감싼다 — 그래야 «그 표 안에서» 죽어도
     *   이미 늘어난 폭이 세어진다. 앞 판은 합산 줄이 갈래 둘(정상 끝·막힘)에만 있어서,
     *   증설 뒤 헤더 보정에서 던지면 실제 두 칸이 기록에 0 으로 남았다. */
    try {
    ...
    for (let i = 볼폭 - 1; i >= 0; i--) {              // 뒤에서부터 — 앞쪽 인덱스가 안 밀린다
      ...
      try {
        /* 🔴 09-07 실행층 실측 — 이 두 줄은 «한 번도 성공한 적이 없었다».
         *   ① `moveColumns` 의 목적지가 폭을 «넘으면» 던진다: `해당 열이 범위를 벗어납니다.`
         *      옛 판은 자리 하나를 만들고(폭 M→M+1) 목적지로 M+2 를 줘서 늘 여기서 죽었다.
         *      폭과 무관하다(폭 24 에서도 같은 예외) ⇒ 남의 이름이 선 표는 전부 조용히 건너뛰어졌다.
         *   ② 옮겨 간 열은 목적지 «한 칸 앞»에 선다(목적지는 옮기기 «전» 좌표라서다).
         *   ⇒ 그래서 자리를 «둘» 만들고 목적지로 폭을 준다: M → M+2, 목적지 M+2, 착지 M+1.
         *      자리 하나 + 목적지 폭 이면 폭이 정본과 같을 때 착지가 정본 «안»이라,
         *      바로 뒤 `헤더보정_` 가 그 열의 값 위에 정본 이름을 씌운다(그게 이 자가 막으려던 손실이다). */
        sh.insertColumnsAfter(sh.getMaxColumns(), 2);
        sh.moveColumns(sh.getRange(1, i + 1, 1, 1), sh.getMaxColumns());
        /* 🔴 [09-08 검수 P1 df3b1e78·47ca570f] **옮기고 나면 빈 자리를 도로 채워야 한다.**
         *   `moveColumns` 는 열을 «빼서» 옮기므로 뒤에 있던 열들이 한 칸씩 왼쪽으로 당겨진다.
         *   그대로 두고 아래 `헤더보정_` 가 이름만 정본으로 덮으면 **값이 남의 이름을 뒤집어쓴다** —
         *   정본 [A,B,C,D] · 라이브 [A,X,C,D] · 값 [a,x,c,d] 일 때 c 가 B 의 값으로, d 가 C 의 값으로 읽힌다.
         *   이 자가 애초에 막으려던 손실이 바로 그것이다(위 「덮으면 그 열의 값이 남의 이름을 뒤집어쓴다」).
         *   ⇒ 뺀 자리에 빈 열 하나를 도로 넣어 **다른 열의 위치를 그대로 되돌린다.**
         *     그러면 그 자리는 「정본 이름 + 빈 값」이 되고, 남의 열은 값과 함께 정본 폭 밖에 남는다. */
        sh.insertColumnsBefore(i + 1, 1);
        결과.민열.push(이름 + ' ' + (i + 1) + '열 「' + nm + '」');
        민것++;
      } catch (e) { 막힘 = (i + 1) + '열 「' + nm + '」 — ' + e; break; }
    }
    /* 🔑 막혀서 되돌아설 때도 이미 늘린 칸은 «샌 것»이라 센다 — 안 세면 「막혔는데 폭이 왜 늘었나」를
     *   나중에 아무도 못 잇는다(09-08 검수 P1 bddf9a7aad1d 의 같은 뿌리). */
    if (막힘) { 결과.건너뛴표.push(이름 + ': ' + 막힘); return; }   // 증설량은 아래 finally 가 센다

    헤더보정_(sh, 정본칸);                              // 엔진_수집.js · 런타임 호출이라 로드 순서 무관
    결과.맞춘표.push(이름 + '(' + 정본칸.length + '칸' + (민것 ? ' · 민 열 ' + 민것 : '') + ')');

    } finally {
      /* 🔑 어느 갈래로 끝나든(정상·막힘·예외) «만지기 전» 폭과의 차이를 센다. */
      결과.더한칸 += Math.max(0, sh.getMaxColumns() - 시작폭);
    }
  });
  return 결과;
}
...
/** 아침 배치가 부르는 «한 판» — 예외로 끝나도 **그때까지의 실적을 그대로** 남긴다.
 *
 * 🔴 왜 함수로 떼어냈나(09-08 검수 P1 a8832d0e0226 · 33079b2b2699):
 *   ① 옛 판은 예외가 나면 빈 기본값을 기록해 「늘린 칸 0 · 밀기 꺼짐」이라는 **거짓 기록**을 남겼다.
 *      실제로는 앞 표를 늘리고 옮긴 뒤였는데도 그렇다. ⇒ 그릇을 «먼저» 만들어 넘기고, 예외가 나도
 *      그 그릇을 그대로 적는다. 스위치 상태(밀기)도 함수가 그릇에 먼저 적으므로 보존된다.
 *   ② `morningJobs` 안에 인라인으로 두면 **시험이 그 자리를 못 부른다** — 예외 경로를 재려면
 *      배치 전체를 돌려야 하는데 그건 못 한다. 함수로 서면 가짜 문서로 예외를 던져 볼 수 있다.
...
function 시트칸맞추기한판_(ss, 옵션) {
  ...
  // 보호 때문에 안 만진 표는 성공 0건과 다르다. 상세는 기존 장부에 두고 배치는 부분 실패로 남긴다.
  if (조각 && !보류 && (그릇.보류표.length || 그릇.건너뛴표.length)) throw new Error('시트 칸 점검 미해결 — 기존 열과 값 보존');
...
function 수집도달_() {
  return {
     ...
     * **두 곳**의 «내용»을 바꾼다(v9.209). 철학 §5 세 층 중 ㉠ 실력의 기록이 학생에게 되돌아가는 배선(구 v1.24 Ⅰ-1).
     * ([09-04] 셋째였던 «연습 노트»가 걷혀 세 곳 → 두 곳이 됐다.)
     * [09-11 실측 · 발전안 20260911] 곁소비자 — 장부 지목은 «출력이 바뀌는 자리» 하나(aiWeakMap_)지만 같은 원천을 읽는 자리가
     *   다섯 더 있다: `masteryFromFeedback_`(교재연동.js · mastery_log) · `재작성준비_`(엔진_수집.js) · `goldenSampleWeekly_`(teacher_gold) ·
     *   `회고_알게된것_`(엔진_운영배치.js · 시즌 회고 편지) · 서클 문서(엔진_셋업확장.js). 그리고 «오류사전»(aiStudioBatch_ ② · error_bank)은
     *   이 원천을 읽어 쌓기만 하고 **읽는 코드가 0** 이다 — 도달이 아니다(엔진_콘텐츠AI.js 오류사전꺼짐_ 주석). */
    'hw_feedback': { 소비자: '엔진_콘텐츠AI.js:aiWeakMap_', 층: '제품', 회수: '밤마다', 대장칸: '㉠', 수신자: '학생·강사' },
    /* [진단] 진단 세션 — 결과 화면(약한 자리 카드)이 읽고, 12-21 판정·무료 연장 판정의 자다(docs/급수진단_설계_v1.md §⑤·§⑥). */
    '진단세션': { 소비자: '엔진_진단.js:진단결과_', 층: '제품', 회수: '요청 때', 대장칸: '㉠', 수신자: '학생' },
    /* [자율일] 묶음 — 월요일 판정이 읽고, 다음 주 묶음이 이월(자리 차지)로 되읽는다(docs/자율일_설계_v1.md §④·§③-㉡). */
    '자율일배정': { 소비자: '엔진_자율일.js:sundayBundleJudge_', 층: '제품', 회수: '주마다', 대장칸: '㉡', 수신자: '학생·운영자' },
    ...
    'mastery_log': { 소비자: 'Code.js:calcAll', 층: '제품', 회수: '매일', 대장칸: '㉠', 수신자: '학생' },
    ...
    'teacher_gold': { 소비자: '엔진_수집.js:골든픽스처_', 층: '자산', 손: true, 회수: '사람이 누를 때', 대장칸: '엔진', 수신자: '내부' },
    ...
    [OUTCOME_TAB_]: { 소비자: '엔진_궤적.js:궤적_최신관측_', 층: '자산', 회수: '매일', 대장칸: '엔진', 수신자: '내부' },
    ...
    'point_logs': { 소비자: 'Code.js:calcAll', 층: '제품', 회수: '매일', 대장칸: '㉡', 수신자: '학생' },
    'attendance': { 소비자: 'Code.js:calcAll', 층: '제품', 회수: '매일', 대장칸: '㉡', 수신자: '학생' },
    ...
    [PORTFOLIO_MOMENTS_TAB_]: { 소비자: 'Code.js:calcAll', 층: '제품', 회수: '매일', 대장칸: '㉡', 수신자: '학생' },
    'attendance_batch': { 소비자: '엔진_운영배치.js:expandAttendanceBatch_', 층: '제품', 회수: '10분마다', 대장칸: '㉡', 수신자: '학생' },
    'hw_batch': { 소비자: '엔진_운영배치.js:expandHwBatch', 층: '제품', 회수: '밤마다', 대장칸: '㉡', 수신자: '학생' },
    'weekly_topics': { 소비자: '엔진_운영배치.js:expandMasteryLog_', 층: '제품', 회수: '밤마다', 대장칸: '㉠', 수신자: '학생' },
    /* [09-11] 수신자 = 강사 — quietScoreMap_ 은 조 편성(assignGroups → groups 시트)의 입력이고 강사 화면이 읽는다. 학생이 좌석·역할을 보는 경로는 09-11 에 확인하지 못해 «학생»을 안 적었다(확인되면 더한다). */
    'lesson_close': { 소비자: '엔진_셋업확장.js:quietScoreMap_', 층: '제품', 회수: '10분마다', 대장칸: '㉡', 수신자: '강사' },
    /* [09-11] 수신자 = 운영자 — lectureRatesOf_ 의 호출자 둘(lectureJoinDiag 메뉴 · lectureWeeklyText_ → weeklyJobs)은 둘 다 ADMIN_EMAIL·운영 화면으로만 간다(독립 검토 09-11 이 잡음 · 강사 접점 0). */
    'lecture_views': { 소비자: '엔진_셋업확장.js:lectureRatesOf_', 층: '제품', 회수: '주마다', 대장칸: '㉡', 수신자: '운영자' },
    'student_errors': { 소비자: '엔진_콘텐츠AI.js:aiWeakMap_', 층: '제품', 회수: '밤마다', 대장칸: '㉠', 수신자: '학생·강사' },
    /* [09-11] 수신자 = 운영자·강사 — checkNoShow 는 ADMIN_EMAIL 과 담당 강사에게 보내고, 학부모에게는 «연락을 권장»할 뿐 직접 보내지 않는다(실측). */
    'absence_notice': { 소비자: '엔진_운영배치.js:checkNoShow', 층: '제품', 회수: '10분마다', 대장칸: '㉡', 수신자: '운영자·강사' },
    'absence_followup': { 소비자: '엔진_운영배치.js:calcTeacherStats', 층: '자산', 회수: '주마다', 대장칸: '운영', 수신자: '운영자' },
    'academic_log': { 소비자: '엔진_운영배치.js:calcTeacherStats', 층: '자산', 회수: '주마다', 대장칸: '운영', 수신자: '운영자' },
    ...
    'achievements': { 소비자: '엔진_콘텐츠AI.js:aiStudioBatch_', 층: '제품', 회수: '밤마다', 대장칸: '㉡', 수신자: '학생' },
     ...
     *   지목은 **출력이 바뀌는 자리**로 한다(설계 §5 「읽힌 것이지 보이는 것이 아니다」). */
    'exit_log': { 소비자: '엔진_콘텐츠AI.js:aiStudioBatch_', 층: '제품', 회수: '밤마다', 대장칸: '㉡', 수신자: '학생' },
    ...
    'quiz_log': { 소비자: '엔진_콘텐츠AI.js:aiStudioBatch_', 층: '제품', 회수: '밤마다', 대장칸: '㉠', 수신자: '학생' },
    ...
    'talk_log': { 소비자: '교재연동.js:masteryFromTalk_', 층: '제품', 회수: '밤마다', 대장칸: '㉠', 수신자: '학생' },
    ...
    'voice_log': { 소비자: '교재연동.js:masteryFromVoice_', 층: '제품', 회수: '밤마다', 대장칸: '㉠', 수신자: '학생' },
    ...
    /* [09-11] 수신자 = 강사 — 조 편성 결과(groups 시트)를 읽는 접점은 강사 화면이다. 학생 좌석·역할 표시 경로는 미확인이라 «학생»을 안 적었다(lesson_close 와 같은 판정). */
    [TALK_INDEX_LOG_SHEET]: { 소비자: '엔진_셋업확장.js:assignGroups', 층: '제품', 손: true, 회수: '사람이 누를 때', 대장칸: '㉡', 수신자: '강사' },
    ...
    /* [㉡-2 · 09-07] 학생이 스스로 한 말 → aiStudioBatch_ ① 「오늘의 한 문장」이 그 말을 되받는다(엔진_수집.js 증언맵_).
     *   ㉢ 삶 이해의 첫 소비자 — 사람 화면이 아니라 산출(응원·미니미션)이 바뀌는 자리다. 생산·소비 같은 커밋(설계 §2-c ⚠). */
    [TESTIMONY_TAB_]: { 소비자: '엔진_콘텐츠AI.js:aiStudioBatch_', 층: '제품', 회수: '밤마다', 대장칸: '㉢', 수신자: '학생' },
    ...
    'scene_log': { 소비자: 'Code.js:calcAll', 층: '제품', 회수: '매일', 대장칸: '㉡', 수신자: '학생' },
  };
}
...
function 시트도달상한_() {
  ...
  /* 📈 손 1 → 2 (2026-09-07 · ㉢-2 연결표가 소비자마다 «언제 도나»를 묻자 `talk_index_log` → `assignGroups` 가
   *    **메뉴로만 돈다**(자동 경로 0 · `menuAssignGroups` 뿐)는 것이 드러났다 — 도달은 맞되 «손» 표기가 빠져 있었다.
   *    분모 확대가 아니라 **표기 정정**이라 도달0 은 그대로다. ▶ 되갚는 조건 = 조 편성이 배치(주간)에서 저절로 도는 날.
   *    ✅ 재고 올렸다 — 장부에 손을 적은 뒤 실측 2 를 보고 상한을 맞췄다(09-01 의 「늘겠지」 순서를 되풀이하지 않았다). */
  return { 도달0: 5, 손: 2 };
}
...
function weeklyJobs() {    // 매주 월 07시
  ...
  safeRun('seasonRetro', 시즌회고발송_); // [㉡-1 · 09-07] 시즌이 끝난 다음 월요일 — 「우리가 알게 된 것」 편지 + 물음 하나(엔진_운영배치.js · 한 시즌 한 번)
...
function triggerManifest_(tbOn) {
  const 목록 = [
    ...
    'onHwFeedbackEdit',                   // [㉡-1 부품 · 09-07] hw_feedback 에서 사람이 고친 자리에 첨삭 서명 셋(엔진_수집.js) — 설치는 아침 배치가 보장
...
function resetAllTriggers(force) {
  ...
  // [㉡-1 부품 · 09-07] 첨삭 서명 onEdit(이 스프레드시트) — 위 전체 삭제 루프가 지우므로 여기서 재설치. 아침 배치(첨삭서명트리거보장_)도 같은 이름을 보장한다.
  ScriptApp.newTrigger('onHwFeedbackEdit').forSpreadsheet(SpreadsheetApp.getActiveSpreadsheet()).onEdit().create(); // 이름을 글자로 — safety 회귀가 매니페스트와 이 목록을 글자로 대조한다
...
function updateBizDashboard(asText, kpiData) { // 계기판 시트 갱신 + 요약 텍스트 반환 (주간 통합 리포트 섹션 규약)
   ...
   *   새 칸은 안 만든다(라이브 칸 늘리기는 성공한 적이 없다). 사람 단위라 student_id 로 중복을 제거한다.
  ...
  if (sh.pay.getLastRow() >= 2) {
    ...
    sh.pay.getRange(2, 1, sh.pay.getLastRow() - 1, 6).getValues().forEach(r => {
      ...
      /* 🔴 [검수 P1 84f88c9a · P3 53a19d32] 결제자는 «납부일이 찍힌» 사람이다 —
       *   확정 문서 `docs/명품마케팅_회사별_v1.md` §㉣ 가 그렇게 정의한다. 금액만 먼저 적어 둔 행
       *   (약속·견적)까지 세면 분모가 부풀어 환불 비율이 «좋아 보이는» 쪽으로 조용히 틀어진다.
       *   재현: 기입 8명 · 실제 결제 2명 · 그중 환불 1명 ⇒ 1/2 = 50% 여야 하는데 1/8 = 12.5% 가 나왔다.
       *   🔑 「찍혔나」만 본다 — 「언제 찍혔나」는 기수가 하나뿐이라 아직 가를 것이 없다(위 창 주석). */
      const 납부일찍혔나 = String(r[3] == null ? '' : r[3]).trim() !== '';
      if (amt > 0 && 납부일찍혔나) 낸사람[sid] = 1;
      /* 🔴 [검수 P1 94aab534 → 2회차 57941bc17fea → 3회차 d8cc32449898] **글자를 아예 안 센다.**
       *   ■ 세 판이 같은 자리에서 무너졌다. 「환불」이라는 글자를 세는 한 목록이 계속 구멍이 됐다:
       *     ① 비고 「환불 없음」 → 1/1명 = 100%   ② 금액을 비운 줄의 같은 말 → 그대로 열려 있었다
       *     ③ 방법 「환불 취소」·「환불 예정」 → 아직 안 나간 돈인데 환불로 셌다.
       *     막을 낱말을 셀 때마다 다음 낱말이 나왔다. 목록을 늘리는 것은 처방이 아니다.
       *   ⇒ **환불은 「돈이 되돌아간 것」이고, 돈이 움직인 증거는 «금액» 하나다.**
       *     금액을 «빼기»로 적은 줄만 환불로 센다. 방법도 비고도 판정에서 뺀다 ⇒ 세는 낱말 0 개.
       *   🔑 그러면 «유호님이 빼기로 안 적으신 진짜 환불»을 놓친다. 그 구멍을 «확인 필요» 줄로
       *     메우려다 4회차 검수에 또 걸렸다(P1 · 엔진_셋업확장.js:2277) — 확인 대상을 고르는 데서
       *     **다시 글자를 세고 있었다.** 그래서 비고에 「환불 없음」이라 적힌 «정상 양수 납부» 줄이
       *     확인 대상이 됐고, 안내대로 금액을 빼기로 고치면 **정상 납부가 환불로 바뀌고 그 사람이
       *     분모에서도 사라진다.** 자가 못 잡는 것을 잡는 척하다 돈을 틀 뻔했다.
       *   ⇒ 확인 줄도 걷는다. 자가 못 재는 것은 **못 잰다고 말한다** — 계기판·주간 리포트 설명에
       *     「환불은 금액을 빼기로 적어 주세요」라는 **적는 법 한 줄**을 세워 그 한계를 드러낸다.
       *     이것은 자유 서술을 제한하는 부탁이 아니라 숫자 칸의 규약이고, 한 줄로 끝난다. */
      if (amt < 0) 돌려받은사람[sid] = 1;
    });
  ...
  const rows = [
    ...
    ['⑦ 환불 비율(누적)', payerN === 0 ? '— (결제 기입 전)' : refundN + '/' + payerN + '명 = ' + refundPct + '%', '⚪',
      '분모는 payments 에서 «납부일이 찍힌» 사람이다 — leads 「등록」도 정원 16도 아니다. 🔴 적는 법: **환불은 금액 칸에 「빼기」로 적어 주세요**(예: -30). 이 자는 그 빼기 줄만 환불로 세고 방법·비고 «글자는 안 봅니다» — 「환불 예정」·「환불 취소」·「환불 없음」이 다 환불로 세어지기 때문입니다. 그래서 빼기로 안 적힌 환불은 이 자가 «못 잡습니다»'],
  ...
  const t = [
    ...
    '⑦ 환불 비율: ' + (payerN === 0 ? '— (결제 기입 전)' : refundN + '/' + payerN + '명 = ' + refundPct + '%')
      + ' ⚪ (환불은 payments 금액 칸에 「빼기」로 적힌 줄만 셉니다 — 글자는 안 봅니다)' // [v9.331]
  ];
...
function silentRosterAlert_(ss, tz) {
  ...
  return '🔇 4주차 침묵 학생 ' + ids.length + '명 — 5주차 지명 발화 배정 대상(상세는 기획자 메일)';
}
