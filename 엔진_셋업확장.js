...
function morningJobs() {   // 매일 07시
  ...
  safeRun('시트칸맞추기', function () {
    ...
    /* 🔴 [09-08 검수 P1 a0a278d4cc1f · P2 97689851540f] 기록을 «인수 자리»에 두면 안 된다.
     *   옛 판은 `시트칸맞추기기록_(ss, 시트칸정본맞추기_(ss))` 였다 — 맞추기가 도중에 던지면
     *   기록 함수가 **아예 안 불려** 「안 돌았다」와 「돌다 죽었다」가 같은 얼굴이 됐다.
     *   ⇒ finally 로 옮겨 **어느 쪽으로 끝나든 한 줄은 남긴다.** 죽은 판은 그 사실을 건너뛴표에 적는다. */
    let 잰것 = null;
    try {
      잰것 = 시트칸정본맞추기_(ssCol);
    } finally {
      시트칸맞추기기록_(ssCol, 잰것 || {
        맞춘표: [], 민열: [], 보류표: [], 더한칸: 0, 밀기: false,
        건너뛴표: ['(전부) — 칸 맞추기가 예외로 끝났다. 까닭은 safeRun 실패 메일과 실행 이력에 있다'],
      });
    }
  });
