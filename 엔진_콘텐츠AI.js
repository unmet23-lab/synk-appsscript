...
/* [함께한날 막7] 구 setupMonsters(7단계 씨앗) 소각 — 씨앗 원문은 git 이력에 있다. contents 시트의
 * type='monster' 7행 실물은 남아 있되(운영 삭제 = 유호님 승인 자리) 이제 어떤 계산·화면도 안 읽는다.
...
function setupOnboarding() {
  ...
  const rows = [
    ['student', 'SYNK에 온 걸 환영해! 🧶', '가이드를 하나 고르면, 함께한 날이 쌓일수록 네 곁으로 다가와. 네 손으로 맞힌 말이 늘수록 새 장면이 열리고, 한국어도 쑥쑥 자라!', 'Хөтчөө сонговол хамт өнгөрүүлсэн өдөр бүрт чам руу ойртоно. Өөрийн гараар зөв хэлсэн үг чинь олшрох тусам шинэ дүр зураг нээгдэж, солонгос хэл ч өснө!', '🐣'], // [함께한날 막7] 진화·도감 세계관 → 함께한 날(몽골어는 검수 큐 Q — 초벌)
