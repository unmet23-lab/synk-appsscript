...
/* [㉡-2 · 2026-09-07] 🗣 증언 그릇 — 학생이 스스로 한 말(testimony_log). 상수가 **이 파일**에 사는 이유 = 이 탭의 소비자가
  ...
  for (let i = 0; i < q.length; i++) {
    ...
    try {
      ...
      fb.appendRow([fbId, sid,
        ...
        // [v9.316] 확인 3칸(확인자·확인시각·확인지문)은 만들 때 비운다 — 채우는 것은 사람이 볼 때다
...
function aiStudioBatch_() {
  ...
  try {
     ...
     *   ([09-04] 함께 적혀 있던 «연습노트»는 걷혔다 — 판단은 그대로다.) */
    const 성취 = 성취맵_(ss);
    /* [㉡-2 · 09-07] 학생이 **스스로 한 말**(testimony_log · 엔진_수집.js 증언맵_) — ㉢ 삶 이해의 첫 소비자.
     *   응원 한 문장이 그 말을 «되받는다»(인용이 아니라 되받기). ① 절에서만 부른다 — ③반브리핑은 안 건드린다
     *   (반 단위 글에 개인의 말을 실으면 그 자리가 곧 노출이다). 이름은 안 싣는다(방향 불변식 4). */
    const 증언 = typeof 증언맵_ === 'function' ? 증언맵_(ss) : { 맵: {}, 건수: 0, 학생수: 0 }; // 조각으로 태우는 회귀(성취·재출제·복귀 창)가 그릇 없이도 ① 절을 돌린다
    Logger.log('스스로 한 말: 대상 ' + stus.length + '명 = 말 있는 '
      + stus.filter(s => 증언.맵[s.id]).length + '명 + 없는 '
      + stus.filter(s => !증언.맵[s.id]).length + '명 · 창 안 ' + 증언.건수 + '건');
    ...
    for (let off = 0; off < stus.length && can(); off += AI_DAILY_BATCH_SIZE) {
      ...
      const userMsg = chunk.map((s, i) => {
        ...
        const ac = 성취.맵[s.id] || '';
        // [㉡-2 · 09-07] 별도 칸 — 약점 칸에 합치면 slice(-2)가 강사 손메모를 밀어낸다(v9.247·v9.252 와 같은 이유)
        const tm = 증언.맵[s.id] || '';
      ...
      try {
        ...
        const out = aiCall_(apiKey,
          ...
          /* [㉡-2 · 09-07] 되받기 지시 — 재료만 실으면 모델이 무시한다(v9.247·v9.252 가 배운 자리). 학생이 제 입으로 말한
           *   바람·이유가 미니미션의 «방향»이 된다. 🚫 평가·교정 금지 — 그 말은 채점 대상이 아니라 그 사람이다(철학 Ⅰ-4). */
          '「스스로 한 말」이 실려 있으면 응원 한 문장이 그 말을 되받는다 — 그대로 인용하지 말고, 학생이 말한 바람·이유를 미니미션의 방향으로 쓴다. ' +
          '🚫 스스로 한 말을 평가하거나 고치지 않는다.',
          '학생 목록:\n' + userMsg, schema, 8096);
        const rowsN = [];
  ...
  /* [발전안 20260911 갈래 ③] error_bank 는 09-11 실측으로 **읽는 코드가 0** 이다 — 모으는 곳만 있고 쓰는 곳이 없다(Prism 집계층이 서는 날 소비자를
   *   얻는다 · 엔진 v3 §8 U1). 호출 자체는 새 유효 첨삭 행이 있을 때만이라 학생 0명인 지금 지출은 0 이지만, 학생이 오면 읽는 이 없는 표에
   *   밤마다 돈이 든다. 철학 §2 「미래에 쓸 수 있다는 설명이 현재 소비자의 부재를 대신하지 않는다」. 기본은 «켜짐»(동작 변화 0) —
   *   끄는 것은 Script Properties `오류사전_OFF=1` 한 줄이고, 끌지·소비자를 먼저 세울지는 발전안 §9 유호 자리다. 판정은 `오류사전꺼짐_` 하나. */
  try {
    const fb = ss.getSheetByName('hw_feedback');
...
function sweepLevelTest_() {
  ...
  for (let i = 0; i < rows.length && done < 10 && Date.now() - t0L < 60000; i++) {
    ...
    if (!report) report = nm + ' — Таны оноо: ' + score + '/15\nТүвшин: ' + lvl.n + '\n' +
      'SYNK LAB-д тохирох анги: ' + lvl.n + ' анги.\nДэлгэрэнгүй зөвлөгөө авахыг хүсвэл зөвлөх багштай холбогдоорой!\n\n(한국어 요약) ' + nm + '님의 레벨은 ' + lvl.n + ' — ' + lvl.d + '.';
...
const MN_CONTENTS_G = {
  // ---- homework ----
  "HW101": "Өнөөдөр сурсан эх бичвэрээ 1 удаа чанга уншаад шинэ 3 үгэнд од тавиарай — үг тус бүрээр нэг өгүүлбэр зохиогоод ирээрэй.",
  "HW102": "Өнөөдөр уншсан эх бичвэрээс хамт хэрэглэгддэг 2 хос үг олж бичээрэй (жишээ: 사진을 찍다).",
  "HW103": "Шинэ 3 үгийн эсрэг эсвэл ойролцоо утгатай үгийг толь бичгээс олоод, таалагдсан 1 жишээ өгүүлбэрийг хуулж бичээрэй.",
  "HW104": "Өнөөдөр сурсан 5 үгээр мини үгийн дэвтэр хийгээрэй — жишээ өгүүлбэрийг эх бичвэрээс олж хуулаад, зөвхөн 1-ийг нь өөрөө зохиогоорой.",
  ...
  "HW106": "Өнөөдөр уншсан эх бичвэрт шинэ гарчиг өгөөд, үндэслэл болсон 2 үгийн доогуур зураарай.",
  "HW107": "Өнөөдөр сурсан нэг үгээр эхлээд үгийн сүүл залгах 끝말잇기 тоглоомоор 5 үг залгаад ирээрэй.",
  "HW108": "Өнөөдөр уншсан эх бичвэрийн 1 зурагт таарах 3 үгийг эх бичвэрээс олж бичээрэй.",
  ...
  "HW113": "Өнөөдөр уншсан эх бичвэрээс 1 өгүүлбэр сонгоод, өөрийнхөө тухай өгүүлбэр болгон өөрчилж бичээрэй.",
  "HW114": "Өнөөдөр уншсан эх бичвэрээс 이·그·저 (эсвэл 여기·거기)-г олж дугуйлаад, юуг зааж байгааг сумаар холбоорой.",
  ...
  "HW118": "Өнөөдөр уншсан эх бичвэрээс 2 өгүүлбэр сонгоод, эсрэг утгатай үгээр эсрэгээр нь хувиргаад үзээрэй.",
  "HW119": "Гэр бүлдээ өнөөдөр сурсан 3 үгийг зааж өгөөд, ямар байсныг нь тэмдэглээрэй.",
  "HW120": "Өнөөдөр уншсан эх бичвэрийн агуулгаар үнэн бол O, худал бол X гэсэн 2 асуулт зохиогоорой — хичээл дээр найз чинь бөглөнө.",
  ...
  "HW123": "Дуудлага нь төстэй хос үгийг (달/딸 гэх мэт) толь бичгээс эсвэл өнөөдөр уншсан эх бичвэрээсээ олж, ялгааг нь тэмдэглээрэй.",
  "HW124": "Өнөөдөр уншсан эх бичвэрээс нэр үг 3, үйл үг 2-ыг олж тэмдэглээд — 1 үйл үгээр 1 өгүүлбэр зохиогоорой.",
  "HW125": "Өнөөдөр сурсан 3 үгээр хоёр мөр харилцан яриа зохиогоорой.",
  "HW126": "Хараахан сураагүй эх бичвэрээс мэдэхгүй 1 үг сонгоод — утгыг нь эхлээд агуулгаар нь таамаглаж бичээд, дараа нь толь бичгээс шалгаарай.",
  "HW127": "Өнөөдөр сурсан 5 үгийг эможигоор илэрхийлээд ирээрэй.",
  "HW128": "5 ширхэг үгийн карт хийгээрэй (урд талд нь үг / ард талд нь утга) — ард талд нь өнөөдөр сурсан өгүүлбэрийг шууд хуулж бичээрэй.",
  ...
  "HW202": "Өнөөдөр уншсан эх бичвэрээс өнөөдөр үзсэн дүрэм орсон 2 өгүүлбэр олж доогуур нь зураад — 1-ийг нь асуулт болгон өөрчилж, маргааш найзаасаа асуугаарай.",
  ...
  "HW205": "Өнөөдөр сурсан 2 өгүүлбэрийг үгүйсгэсэн хэлбэрт оруулаарай.",
  "HW206": "Өнөөдөр уншсан эх бичвэрийн нэг догол мөрийг уншихдаа 조사(нөхцөл)-г хуруугаараа халхалж өөрөө нөхөж уншаарай — хэдийг зөв хэлснээ тэмдэглээрэй.",
  ...
  "HW209": "Өнөөдөр уншсан эх бичвэрийн нэг догол мөрийн 3 өгүүлбэрийг цаасан дээр тус тусад нь бичээд дарааллыг нь холиорой — маргааш найз чинь зөв дарааллаар нь эмхэлнэ.",
  ...
  "HW212": "Шалтгаан заасан -아/어서 хэлбэрээр 2 өгүүлбэр зохиогоод — өнөөдөр уншсан эх бичвэрээс шалтгаан орсон 1 өгүүлбэр олж хуулж бичээрэй.",
  ...
  "HW215": "Өнөөдөр уншсан эх бичвэрээс 반말 хэлбэрийн 2 өгүүлбэр олоод хүндэтгэлийн хэлбэрт оруулаарай.",
  "HW216": "반말 хэлбэрийн 2 өгүүлбэрийг 존댓말 болгож, 존댓말-ын 2 өгүүлбэрийг 반말 болгож хөрвүүлээрэй.",
  "HW217": "Өнөөдөр үзсэн дүрмийг өөрийнхөө үгээр нэг мөрөнд багтааж бичээд, өнөөдөр сурсан 1 жишээ өгүүлбэр хуулж бичээрэй.",
  "HW218": "Өнөөдөр сурсан нэг өгүүлбэрийн үгсийг холиод, дараа нь буцааж зөв дарааллаар нь эмхлээрэй.",
  "HW219": "Эсрэгцүүлсэн -지만 хэлбэрээр 2 өгүүлбэр бичээд — 1-ийг нь өнөөдөр уншсан эх бичвэрээсээ 하지만 эсвэл -지만 орсон өгүүлбэр олж хуулж бичээрэй.",
  "HW220": "Яг одоо хийж байгаа 3 зүйлээ бичээрэй (-고 있다).",
  "HW221": "Өнөөдөр уншсан эх бичвэрийн агуулгаар 누가·언제·어디 гэсэн 3 асуулт зохиогоорой — хариулт нь эх бичвэр дотор байх ёстой.",
  ...
  "HW226": "Өнөөдөр уншсан эх бичвэрээс 그리고·그래서·하지만-г олж дугуйлаад — нэгийг нь өөрөөр соливол утга яаж өөрчлөгдөхийг нэг мөрөнд бичээрэй.",
  "HW227": "Өнөөдөр уншсан эх бичвэрээс богино 1 өгүүлбэр сонгоод, 3 мэдээлэл багтаасан урт өгүүлбэр болгож өргөжүүлээрэй.",
  "HW228": "Өнөөдөр уншсан эх бичвэрээс урт 1 өгүүлбэр сонгоод 2 өгүүлбэр болгож хуваагаарай.",
  "HW229": "그리고·그래서·하지만 гэсэн үг тус бүрээр өгүүлбэр холбож, тус бүр 1 өгүүлбэр бичээрэй.",
  "HW230": "Энэ долоо хоногийн хамгийн дуртай 1 жишээ өгүүлбэрээ хуулж бичээд, од өгч, шалтгаанаа бичээрэй.",
  "HW301": "Өнөөдөр сурсан хэллэгээрээ 30 секунд ганцаараа ярьж, утсандаа бичлэг хийгээрэй.",
  ...
  "HW309": "Өөрийгөө танилцуулсан 15 секундийн видео бичлэг хийгээрэй (илгээх шаардлагагүй).",
  ...
  "HW312": "Зам асуух 3 өгүүлбэрийг чанга хэлж дасгал хийгээрэй.",
  "HW313": "Өнөөдрийн сэтгэл санаагаа 3 өгүүлбэрээр хэлээд үзээрэй.",
  "HW314": "Өнөөдөр уншсан догол мөрийг 2 дахин хурдан уншиж үзээрэй.",
  ...
  "HW321": "Дуудахад хэл орооцолддог 1 өгүүлбэрийг 5 удаа давтаарай.",
  ...
  "HW405": "Өнөөдөр үзсэн нэг зургийг 3 өгүүлбэрээр дүрслээрэй.",
  ...
  "HW410": "Энэ улирал дуусах үеийн өөртөө зориулж 2 өгүүлбэр бичээрэй.",
  ...
  "HW505": "Энэ долоо хоногийн шинэ 10 үгээ сонсож бичих дасгалаар шалгаарай — гэр бүлийн хэн нэгнээр уншуулж, эсвэл өөрийн бичлэгээ сонсонгоо бичээрэй.",
  ...
  "HW514": "Найздаа өгөх жижиг квизийн 3 асуулт зохиогоорой — 1 асуултыг нь өөрөө чанга уншиж өгөх сонсголын асуулт болгоорой.",
  ...
  "HW523": "Солонгос бичлэгийн 30 секундийг дэлгэцийг нь халхалж зөвхөн дуугаар нь сонсоорой — хаана болж байгааг, хэдэн хүн байгааг таамаглаад, дэлгэцээ нээж шалгаарай.",
  "HW524": "Сурсан зүйлээсээ 1-ийг ээж аавдаа тайлбарлаж өгөөрэй (монголоор ч болно).",
  "HW525": "Энэ долоо хоногт сурсан эх бичвэрийг чангаар бүтнээр нь уншиж бичлэг хийгээрэй — дуусаад дурын 10 секундийг нь сонсоод үзээрэй.",
  ...
  "HW627": "Солонгос үсэг харагдсан 1 зураг аваад, юу гэж бичсэнийг нь уншаад ирээрэй.",
  ...
  "HW704": "Ширээгээ цэгцэлж, үгийн дэвтрээ байранд нь тавиарай (1 мөр бичиж баталгаажуулаарай).",
  ...
  "HW710": "Энэ долоо хоногт сайн хийсэн 1 зүйлээ өөрийгөө магтан бичээрэй.",
  ...
  "HW715": "Хамгийн амархан 1 өгүүлбэрээ төгс дуудлагаар 3 удаа хэлээрэй.",
  ...
  "BT07": "Үгийг зурагтай хамт цээжлээрэй — хоёр замаар хадгалагдсан ой санамж хоёр дахин бат бөх 🖼️",
...
const MN_FORTUNES = [
  "Өнөөдөр ярьж хариулах аз ихтэй өдөр ✨ Гараа өргөх мөч чинь өнөөдрийн сорилт болно",
  ...
  "Хөтөч чинь өглөөнөөс л сайхан ааштай, ая аялж байна 🎵",
  ...
  "Өнөөдөр чиний зохиосон нэг өгүүлбэр удаан хадгалагдах өдөр 🔥",
...
const MN_SPEAK = {
  crown: ["Өнөөдөр зориглон оролдсон чинь дэлхийд хамгийн гоё байлаа 🔥 Шөнөжин гайхуулна шүү!","Өчигдрийн өөрийгөө даван гарлаа — үнэхээр гялалзаж байлаа 🌱","Чамайг гараа өргөхийг би хамгийн түрүүнд харсан! ✨"],
...
const MN_STORY_SCENES = [
  ...
  [ // 7월
    ...
    "Зуны амралтаар кру нар KTX-д сууж Бусан руу аяллаа. Хэүндэгийн элсэн эрэгт хөл тавимагц давлагаа шагайг нь гижигдэв.",
    "Мильмён, үртэй хотток авч бариад Бусан аялгаар \"마, 억수로 맛있네!\" (Ёстой аймаар амттай юм аа!) гэж дуурайж хэлээд бүгд нэг зэрэг инээлдлээ.",
    "Тэр үед тэнгисийн хаяаны цаадах тэнгэр бэхэн хар өнгөөр бүрхэгдэж, давлагаа сонин хэмнэлээр цалгилж эхэллээ.",
  ],
  [ // 8월
  ...
  [ // 9월
    ...
    "Намрын нэгэн үдээс хойш кру нар Букчоны вааран дээвэртэй байшингуудын хоорондох нарийн гудамжнуудаар сонирхон алхлаа. Гудамж бүрд өөр өөр Сөүл нуугдаж байв.",
    "Ханок кафед тэчүча буюу нанжин чавганы цай ууж суутал цаасан хаалгаар нэвт гэрэлтэх наранд бүгд үг дуугүй болов. \"Эрт цагийн хүмүүс ийм байшинд сууж хичээлээ хийдэг байж дээ.\"",
  ...
  [ // 13월
    ...
    "Яг тэр үед дээврийн ирмэг дэх салхин хонхны дуу тэс зогсож, гудамжны манан нэг том бөөгнөрөл болон хуйлран босов.",
  ],
  [ // 14월
    "Каннын хотын Анмок эргийн кофе гудамж",
    "Хоёрдугаар сарын Каннынд кру нар Анмок эргийн кофены гудамжинд хүрч ирлээ. Өвлийн далай шил мэт гялалзаж, дөнгөж хуурсан кофены үнэр давлагааны чимээтэй холилдон ирнэ.",
  ...
  [ // 16월
    ...
    "Дөрөвдүгээр сарын Босонд кру нар ногоон давлагаа шат мэт үргэлжлэх цайны талбайн толгод өөд гарлаа. Хаврын бороог угтаж буй цайны навчис ногоон туяагаар гялалзана.",
  ...
  [ // 17월
    ...
    "Тавдугаар сарын Тамянд кру нар тэнгэр өөд сүндэрлэх хулснуудын хоорондох жимээр орлоо. Салхи үлээх бүрд хулсны навчис далайн давлагаа мэт шаржигнана.",
  ...
  [ // 18월
    ...
    "Гэтэл далайн мандал дээрх гэрлүүд найган нэг тийш цугларч, зөөлхөн желе мэт хөөж агаарт хөөрөв.",
  ],
  [ // 19월
    ...
    "Тэр үед ус татарч ил гарсан шаварлаг газрын гол хэсэг буцалж, шаврын хөөс уул мэт овоорон оргилон босов.",
  ],
  [ // 20월
  ...
  [ // 22월
    ...
    "Гэтэл мөрний салхи эргэлдэж эхлэн, нэг бөөрөнхий жижигхэн хуй салхи мөрөн дээр бууж ирээд дэнлүүнүүдийн дундуур дүрсгүйтэн сүлжин тоглов.",
  ],
  [ // 23월
    "Сунчонман булангийн зэгсэн талбайн нар жаргалт",
    "Арван нэгдүгээр сарын Сунчонманд кру нар үзүүр хязгааргүй үргэлжлэх зэгсэн талбайн модон замаар алхлаа. Салхи өнгөрөх бүрд мөнгөлөг зэгс нэг зүг рүү давлагаална.",
    "Ажиглалтын тавцан дээр гарахад жаргах нарны туяан дунд хар тогоруунуудын сүрэг V үсэг зурсаар нисэн хөөрөв. \"겨울을 나러 시베리아에서 여기까지 온대.\" (Өвөлжихөөр Сибирээс энд хүртэл нисэж ирдэг гэнэ.) Мянга мянган километр туулж ирсэн шувуудын өмнө кру нар удтал тэнгэр ширтэн зогсов.",
...
const MN_HOMEWORK_CHECKPOINT = {
  ...
  "HW104": "Уншсан бичвэрээсээ олсон жишээ + өөрийн 1 өгүүлбэр",
  ...
  "HW226": "Соливол утга яагаад өөр болохыг ярьж үзэх",
  "HW227": "Өргөжүүлэх чадвар",
  "HW228": "Хоёр өгүүлбэр болгон хуваасан цэг нь зөв эсэх",
...
function academicMsg_(s) {
  ...
  if (s.hasDelta && s.delta > 0) return [
    ...
    'Өнгөрсөн сараас +' + s.delta + ' оноо 📈 Тууштай байдал чинь чадвар болж байна'
  ];
...
function setupOnboarding() {
  ...
  const rows = [
    ...
    ['student', 'SYNK에 온 걸 환영해! 🧶', '가이드를 하나 고르면, 함께한 날이 쌓일수록 네 곁으로 다가와. 네 손으로 맞힌 말이 늘수록 새 장면이 열리고, 한국어도 쑥쑥 자라!', 'Хөтчөө сонговол, хамт өнгөрүүлсэн өдөр олшрох тусам тэр чам руу ойртоно. Өөрөө зөв хэлсэн үг чинь нэмэгдэх тусам шинэ дүр зураг нээгдэж, солонгос хэл чинь ч сайжирна!', '🐣'], // [함께한날 막7] 진화·도감 세계관 → 함께한 날
    ['parent', '자녀의 성장을 매주 받아보세요 🌱', '매주 자녀의 한국어 성장 리포트가 몽골어로 도착해요. 출석·급수·칭찬을 한눈에 확인하고 함께 응원해 주세요.', 'Долоо хоног бүр хүүхдийнхээ солонгос хэлний ахицын тайланг монголоор хүлээн авна. Ирц, зэрэг, магтаалыг нэг дороос харж, хамтдаа дэмжээрэй.', '💌'],
