 ...
 *     (팩 주석 `contents/토픽퀴즈문항.js` — 「급수 대응을 지어내지 않는다 · 교재·급수 담당 몫」).
 *     교재 6권이 확정되면 **`표` 만 재검토**하면 된다. 재출현급은 요목 직결분만 적는다(추정 금지).
 ...
 *
 * ■ 🔴 실측이 드러낸 것 (이 태그가 붙어서 처음 셀 수 있게 된 숫자):
 *   1급 21 · 2급 23 · 3급 21 · **4급 6 · 5급 1** — 4~5급이 72종 중 **7개(9.7%)** 뿐이다.
 *   머리 주석의 「TOPIK 1~2급 기준 선정」은 낡았고(실제로는 1~5급에 걸친다), G7xx 주석의
 *   「2급 상단+3급 진입」도 낡았다(G704=5급·G705·G710~712=4급). 유호님이 겨냥한 **4~5급 구간이
 *   바로 이 뱅크의 빈 곳**이고, 그 갭은 태그가 없었으면 셀 수 없었다.
 * ================================================================================ */
const GRAMMAR_BANK = [
  ...
  ['G512', '-다고 하다', '간접화법 기초', 3, 4, '요'],       // Lv3 간접화법 도입 → Lv4 인용 복합·축약 확장
  // G6xx — 미엘로→플로우
  ['G601', '-거든요', '이유 설명(구어)', 3, null, '표'],
  ['G602', '-잖아요', '상기시키기', 3, null, '표'],
  ['G603', '-(으)ㄹ 때', '시점·때', 2, null, '표'],
  ['G604', '-던', '회상 수식', 3, null, '표'],
  ...
  ['G609', '-대요/-래요', '축약 간접화법', 4, 5, '표'],      // 도입=표준 4급 · 재출현=Lv5 요목 「-대/-래/-냬 구어 심화」
  ...
  ['G707', '-기 마련이다', '당연한 이치', 3, null, '요'],    // Lv4 중복 제거 → Lv3 «전용» 확정이라 재출현 없음
...
function aiWeakMap_(ss, 복귀) {
   ...
   *   소비처 3곳(퀴즈 프롬프트 slice(-2)·반브리핑/연습노트 slice(-1))의 항목 수가 안 변해 손메모 자리가 안 밀린다.
...
function aiStudioBatch_() {
  ...
  try {
     ...
     *   ③반브리핑·연습노트는 안 건드린다: 반 단위 글에 개인 성취를 실으면 그 자리가 곧 비교다. */
    const 성취 = 성취맵_(ss);
...
const MN_CONTENTS_G = {
  ...
  "HW114": "Сурах бичгийн эх бичвэрээс 이·그·저 (эсвэл 여기·거기)-г олж дугуйлаад, юуг заж байгааг сумаар холбоорой.",
  ...
  "HW506": "Энэ долоо хоногт эргэлзсэн 1 зүйлээ «Миний дасгалын тэмдэглэл» болгоорой (яагаад эргэлзсэн + зөв өгүүлбэр).",
...
const MN_HOMEWORK_CHECKPOINT = {
  ...
  "HW114": "Заж буй зүйлийг зөв олсон эсэх",
 ...
 *    월 어휘·화 문법 = «읽는 실물»(교재 지문·간판·포장·사전·폰 화면)에 닻 — 지문은 박아 넣지 않는다
 *    (뱅크는 레벨·차시 무관이라 고정 지문은 레벨을 못 타고, 교재·실물은 학생 레벨을 저절로 탄다).
...
function setupHomework() {
  ...
  replaceContentType(ss, 'homework', [
    ['HW101','homework','어휘','교재에서 오늘 배운 쪽을 소리 내어 1번 읽고 새 단어 3개에 별표 — 각 단어로 한 문장씩 만들어 오세요.','조사(이/가·을/를) 확인',101],
    ['HW102','homework','어휘','교재 지문에서 짝이 되는 말 2쌍을 찾아 적어 오세요(예: 사진을 찍다).','지문에서 찾은 쌍인지 + 자연스러운지',102],
    ['HW103','homework','어휘','새 단어 3개의 반대말이나 비슷한 말을 사전에서 찾고, 마음에 드는 사전 예문 1개를 베껴 쓰세요.','뜻을 내 말로 설명해 보기',103],
    ['HW104','homework','어휘','오늘 배운 단어 5개로 미니 단어장 만들기 — 예문은 교재에서 찾아 베끼고 1개만 내 문장으로.','교재에서 찾은 예문 + 내 문장 1개',104],
    ['HW105','homework','어휘','집에 있는 한국 제품 포장·라벨에서 한국어 5개를 읽고 적어 오세요(없으면 물건 이름 5개).','받침 발음 확인',105],
    ['HW106','homework','어휘','교재 지문에 새 제목을 붙이고, 근거가 된 단어 2개에 밑줄.','제목과 근거가 어울리는지',106],
    ['HW107','homework','어휘','오늘 배운 단어 하나로 시작하는 끝말잇기 5개 이어 오기.','실제 있는 단어인지',107],
    ['HW108','homework','어휘','교재의 그림·사진 1개에 어울리는 단어 3개를 지문에서 찾아 적어 오세요.','그림만 보고 단어 말해 보기',108],
    ...
    ['HW113','homework','어휘','교재 지문의 문장 1개를 골라 나에 관한 문장으로 바꿔 쓰기.','진짜 자기 얘기인지',113],
    ['HW114','homework','어휘','교재 지문에서 이·그·저(또는 여기·거기)를 찾아 동그라미 — 무엇을 가리키는지 화살표로 잇기.','가리키는 것을 맞게 찾았는지',114],
    ...
    ['HW118','homework','어휘','교재 지문에서 문장 2개를 골라 반대말로 뒤집어 보세요.','반의어 정확성',118],
    ['HW119','homework','어휘','가족에게 오늘 배운 단어 3개를 가르쳐 주고 반응 메모.','가르친 경험 공유',119],
    ['HW120','homework','어휘','교재 지문 내용으로 맞으면 O 틀리면 X 문제 2개 만들기 — 수업에서 친구가 풉니다.','답의 근거 문장 찾기',120],
    ...
    ['HW123','homework','어휘','발음이 비슷한 단어 한 쌍(달/딸 같은)을 교재나 사전에서 찾아 차이 메모.','최소대립쌍 확인',123],
    ['HW124','homework','어휘','교재 지문에서 명사 3개·동사 2개를 찾아 표시 — 동사 1개로 문장 1개 만들기.','품사 나누기 + 동사 문장 1개',124],
    ['HW125','homework','어휘','오늘 배운 단어 3개로 두 줄 대화 만들기.','대화 자연스러움',125],
    ['HW126','homework','어휘','교재에서 아직 안 배운 쪽의 모르는 단어 1개 — 뜻을 문맥으로 먼저 추측해 쓰고 사전으로 확인.','어떻게 추측했는지 말해 보기',126],
    ['HW127','homework','어휘','오늘 배운 단어 5개를 이모지로 표현해 오기.','이모지 보고 단어 복원',127],
    ['HW128','homework','어휘','단어 카드 5장 만들기(앞 단어/뒤 뜻) — 뒤에는 교재 문장을 그대로 베껴 쓰기.','카드로 즉석 테스트',128],
    ...
    ['HW202','homework','문법·문장','교재 지문에서 오늘 배운 문법이 든 문장 2개를 찾아 밑줄 — 1개를 질문으로 바꿔 내일 친구에게 묻기.','질문 억양으로 읽기',202],
    ...
    ['HW206','homework','문법·문장','교재 한 문단을 읽으며 조사만 손가락으로 가리고 스스로 채워 보기 — 몇 개 맞았는지 기록.','다 읽고 원문과 대조',206],
    ...
    ['HW209','homework','문법·문장','교재 한 문단의 문장 3개를 종이에 한 문장씩 쓰고 순서를 섞기 — 내일 친구가 순서를 복원합니다.','순서의 근거(먼저·그다음) 말하기',209],
    ['HW210','homework','문법·문장','오늘 배운 문법 + 시간 표현(아침·어제 등) 문장 2개.','시간 부사 위치',210],
    ['HW211','homework','문법·문장','오늘 배운 문장을 명령문과 청유문으로 바꾸기.','(으)세요 / -읍시다',211],
    ['HW212','homework','문법·문장','이유 표현(-아/어서) 문장 2개 만들기 — 교재 지문에서 이유가 든 문장 1개를 찾아 베끼기.','인과 연결',212],
    ['HW213','homework','문법·문장','조건 표현(-으면)으로 문장 2개.','가정 의미',213],
    ['HW214','homework','문법·문장','오늘 배운 문법으로 몽골을 소개하는 문장 1개.','고유명사 표기',214],
    ['HW215','homework','문법·문장','교재에서 반말 문장 2개를 찾아 높임말로 바꾸기.','-(으)세요 / 께서',215],
    ['HW216','homework','문법·문장','반말 2문장을 존댓말로, 존댓말 2문장을 반말로.','종결어미 전환',216],
    ['HW217','homework','문법·문장','오늘 배운 문법의 규칙을 내 말로 한 줄 정리 + 교재 예문 1개 베끼기.','메타 이해',217],
    ['HW218','homework','문법·문장','교재 문장 하나를 단어 순서 섞었다가 복원하기.','어순 감각',218],
    ['HW219','homework','문법·문장','대조 표현(-지만) 문장 2개 — 1개는 교재 지문에서 하지만·-지만이 든 문장을 찾아 베끼기.','앞뒤 대조 성립',219],
    ['HW220','homework','문법·문장','지금 하고 있는 일 3개 쓰기(-고 있다).','진행형',220],
    ['HW221','homework','문법·문장','교재 지문 내용으로 누가·언제·어디 질문 각 1개 만들기 — 답이 지문 안에 있어야 합니다.','답이 지문에 있는지',221],
    ...
    ['HW226','homework','문법·문장','교재 지문에서 그리고·그래서·하지만을 찾아 동그라미 — 하나를 다른 것으로 바꾸면 뜻이 어떻게 변하는지 한 줄.','바꾸면 뜻이 왜 달라지는지 말해 보기',226],
    ['HW227','homework','문법·문장','교재 지문의 짧은 문장 1개를 정보 3개 담긴 긴 문장으로.','확장 능력',227],
    ['HW228','homework','문법·문장','교재 지문의 긴 문장 1개를 두 문장으로 나누기.','분리 지점',228],
    ...
    ['HW410','homework','쓰기','이번 시즌(지금 교재 한 권)이 끝날 때의 나에게 보내는 2문장.','미래 표현',410],
    ...
    ['HW506','homework','복습','이번 주 헷갈렸던 것 1개를 내 연습 노트로 — 왜 헷갈렸는지 + 바른 문장.','원인 분석',506],
    ...
    ['HW525','homework','복습','교재 이번 주 페이지를 소리 내어 통독하며 녹음 — 끝나고 아무 데나 10초만 들어 보기.','유창성',525],
