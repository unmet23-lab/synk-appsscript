...
function setupMonsters() {
  ...
  replaceContentType(ss, 'monster', [ // [v9.11] 도감 스토리 — 아기자기·1~2문장, 뇌과학 은유 유지
    ...
    ['M07','monster','싱크마스터','빛나는 지혜의 관과 황금 홀을 든 시냅스의 왕 👑 그가 지나가면 반 전체가 환해져요 — 배움의 끝에서, 모든 것이 이어졌어요.','',2400]
  ]);
  calcAll();
}
...
function unknownReasonScan_(ss) {
  const KNOWN_RS = ['숙제', 'MVP', '시냅스', '오늘의 도전', '오늘의 성장', '칭찬', '정정', '생일', '레이드', '발표', '일일한도',
...
function setupTitleLore() {
  ...
  replaceContentType(ss, 'lore', [
    ...
    ['LR09','lore','🌟 이달의 스타','어제의 나를 넘은 모든 사람에게. 기준을 채우면 누구든.','🔵 레어',''],
 ...
 *    원어민 검수 대상). 기계 불변식은 tests/숙제뱅크.test.js 가 지킨다(고아·의문문만 — 짝 «내용» 대조는 여전히 손).
...
function syncToNotion_() {
  ...
  rows.forEach(r => {
    ...
    const crowns = (Number(r[48]) || 0) + (Number(r[49]) || 0); // AW+AX 도전·성장 누적
    const bestStreak = Number(r[27]) || 0;                      // AB 최고스트릭
    const rk = String(r[24] || '하').charAt(0);                 // Y 이탈위험 → 상/중/하
    const risk = ['상', '중', '하'].indexOf(rk) > -1 ? rk : '하';
    const summary = (stage || '뉴로') + ' · ' + (level ? level + '급' : '평가전') + ' · ' + points + 'P · 도전·성장 ' + crowns + '회 · 최장연속 ' + bestStreak + '일';
    const properties = {
      '이름': { title: [{ text: { content: name } }] },
      '학생ID': { rich_text: [{ text: { content: id } }] },
      '반': { rich_text: [{ text: { content: cls } }] },
      '몬스터단계': { rich_text: [{ text: { content: stage } }] },
      '총포인트': { number: points }, '왕관수': { number: crowns }, '최장연속출석': { number: bestStreak }, // ⚠키='왕관수'는 Notion DB 스키마 계약 — 화면이 아니라 개명하지 않는다(08-20 검수 dc7e1039)
