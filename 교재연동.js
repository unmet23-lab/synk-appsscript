/* ============================================================
 * SYNK 교재연동 엔진 [v9.59] — 목소리 타임랩스(A) + AI 문법 판정(C)
 *   ⚰ [09-04] **B 연습 노트는 걷혔다**(유호 확정 09-04 「연습 노트는 걷어줘」). 아래 B 설명은
 *     되살릴 때의 재료 목록으로 남긴다 — 지금 도는 기능이 아니다.
 ...
 *   B. 연습 노트 ⚰ **걷혔다(09-04)** — 아래는 되살릴 때의 재료 목록이지 도는 기능이 아니다.
 *      만들던 것: mastery_log(미도달 문법)·student_errors(강사 약점 메모)·
 *      hw_feedback(AI 첨삭)을 모아 "너의 약점은 이 문법"을 학생별 생성했다.
 *      ([09-04] "교재 몇 과를 펴라" 꼬리는 걷혔다 — 유호 지시 · 확정 09-03 「교재와 앱은 별개」).
 *   C. AI 문법 판정 [v9.59, 유호님 지시 "교사 손 0"] — 학생이 숙제폼으로 낸 문장을
 *      AI가 판정해 mastery_log를 자동 축적. 강사 마감폼 문법태그 없이도 진화 게이트가
 *      완전 작동한다(마감폼·약점메모폼은 선택 보강으로 강등).
 *      ([09-04] 여기 함께 적혀 있던 「연습 노트」는 걷혔다 — C 는 그대로 돈다.)
 *      열의 있는 학생일수록 제출↑ → 도달↑ → 진화↑ — 학생 주도 완결 루프.
 *
 * 설계 원칙
 *   ① Glide update 0 — 폼 제출(인바운드)·야간 배치 쓰기만. 앱은 읽기 전용.
 *   ② AI 신규 호출 0 — B는 이미 생성된 첨삭 문구를 재조립«했다»(비용 증가 없음). [09-04] B 는 걷혔다.
 ...
 *   DB 목소리폼URL(학생별 미리채움) · DC 목소리성장카드
 *   DD 연습노트 — [09-04] **더는 안 채워진다**(B 폐지). 라이브 열은 그대로 둔다 —
 *     빼면 뒤 열이 한 칸씩 밀려 위치로 읽는 다른 기능이 조용히 어긋난다.
 ...
 *   ⇒ 그때 판단은 「연습 노트 본체는 교재 없이 그대로 선다」였다. **그 본체도 같은 날 걷혔다**
 *     (유호 확정 09-04 「연습 노트는 걷어줘」) — 이 줄은 그 시점의 기록으로 남긴다.
...
function 교재연동Nightly() {
   ...
   *   ⚠ [09-04 정정] 여기 「mastery_log 의 «유일한 소비자»였고 이제 아무도 안 읽는다」고 적었던 것은
   *     **거짓이다**(이종 검수 d6eaa9b5670f). 그 시트의 '도달' 행은 지금도 읽힌다.
   *     ⚠ [09-04 재정정] 처음엔 「셋」이라 적었는데 **그것도 과소집계였다**(이종 검수 202ea3533e81).
   *       실측 = **다섯 자리 · 파일 넷**: Code.js:2460(함께한날) · 엔진_운영배치.js:2774·:2803 ·
   *       엔진_콘텐츠AI.js:417(도달맵) · 엔진_폼리포트.js:4089. 그중 둘이 학생 화면으로 나간다.
   *     🔑 남의 틀린 수를 고치면서 내 수를 안 셌다 — 수는 «세어서» 적는다.
   *     걷힌 것은 '연습'(미도달) 행을 읽던 자리 하나뿐이다.
   *   🔑 같은 사실을 두 곳이 쥐면 갈린다 — 장부(엔진_셋업확장.js)는 고쳤는데 이 주석이 옛말을
   *     그대로 들고 있었다. 되살리려면 git 이력에 그대로 있다. */
}
 ...
 *   그래서 「글로는 아직 안 썼는데 말로는 쓰는 문법」은 진화 어디에도 없고,
 *   ([09-04] 여기 함께 적혀 있던 「연습 노트」는 걷혔다 — 빠지는 축은 진화 하나로 줄었다.)
