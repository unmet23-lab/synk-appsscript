...
/* 문법 ID → 교재 위치. 정본 대조 = docs/_archive/교재_앱_연동_매핑_v1.md §③
 * ⚰ **그 매핑 문서는 보관됐다**(2026-09-03 · 유호 확정 「교재랑 앱이랑은 아예 별개라고 생각해줄래?
 *   교재는 기각하고 어디 안보이는곳에 보관만해줘」 · 교재 제작은 재헌님 몫).
 * 🔴 그래서 **이 표는 더 자라지 않는다** — 「권2 집필 시 G3xx 추가」는 이제 우리 일이 아니다.
 *   앞으로 앱·엔진 설계는 교재를 전제하지 않는다(memory `textbook-separate-from-app`).
 *   이 표를 읽는 «연습 노트»(교재 몇 과를 펴라)가 그 전제에 기댄 마지막 자리라, 그 갈래의
 *   존폐는 유호님 판정거리로 트랙 6절에 올렸다. 지금은 있는 값 그대로 둔다(지우면 라이브가 바뀐다). */
