...
test('🔑 복수 축(P3;P9)이 스냅샷에 그대로 담긴다 — 규격이 「코드는 안 고쳐도 된다」고 적은 그 주장', () => {
  /* ⚠ 같은 절이 예전에 그 문장을 «틀리게» 적어 결함을 세웠다(P0-② — 읽기는 읽되 하나만 담았다).
   *   그래서 이번엔 주장을 시험으로 못 박는다. 담는 자리가 문자열이라 세미콜론이 살아남아야 한다. */
  const out = 함수뽑기([])(목록시트([['HW306', 'P3;P9', '읽었어요', '겹받침이자 연음']]));
  assert.deepStrictEqual(out.HW306, ['P3;P9:읽었어요']);
});
