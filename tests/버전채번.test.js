...
/* [v9.114] 버전 자동 채번 — 2026-08-01 하루에 6번 겹친 동시 발번(v9.80·81·87·100·107·112)의 근본 대책.
 * 이 테스트가 지키는 것은 채번기의 "유일성 보장 경로"다. 여기가 뚫리면 충돌이 조용히 돌아온다. */

test('[v9.114] 버전 비교는 숫자다 — v9.9 < v9.100 (문자열 비교면 정반대가 된다)', () => {
...
test('[v9.114] 최댓값·다음 번호 — 자릿수 경계와 null 혼입에서 무너지지 않는다', () => {
...
test('[v9.114] SYNK_VERSION 기입은 그 한 줄만 바꾼다 — 파일 파괴 금지', () => {
  const before = [
    '// 머리말',
...
test('[v9.114] 유일성 보장 경로 — 예약은 원격 push로만 확정된다', () => {
...
test('[v9.114] 채번기는 라이브로 배포되지 않는다 (.claspignore 허용목록 밖)', () => {
