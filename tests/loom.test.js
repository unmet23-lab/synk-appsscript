...
function 흠찾기(css) {
  ...
  // ⑧ 하드코딩 hex — 값은 전부 토큰에서 온다. `:root{…}` 변수 선언 블록만 예외다.
  //   ⚠기준점을 `*{box-sizing` 이라는 **랜드마크 문자열**로 잡았었다(2026-08-16 교체). 그 줄은
  //     `바탕` 조각에만 있어서, 바탕을 안 싣는 프리셋(부품만·인쇄부품)에선 랜드마크를 «못 찾아»
  //     전체를 몸통으로 읽었고 — :root 안의 «정상» 선언까지 흠으로 냈다. 그래서 프리셋 «이름»으로
  //     예외를 파야 했고, 새 프리셋이 생길 때마다 그 예외를 또 파야 했다(실측: 인쇄부품이 그랬다).
  //   ⇒ 랜드마크 대신 `:root{…}` 블록 자체를 걷어낸다 — 이름 예외가 아예 필요 없어진다.
  const 몸통 = css.replace(/:root\{[^}]*\}/g, '');
...
test('Loom 지면 프리셋 전량 — 부품을 쓰는 프리셋은 모두 규칙을 지킨다', () => {
  ...
  for (const 이름 of Object.keys(loom.지면들)) {
    ...
    /* 🔑 이름으로 판 예외가 **0개**다 — ⑧이 `:root{…}` 를 직접 걷어내게 고쳐서(위 주석),
     *   바탕을 안 싣는 프리셋도 같은 눈으로 잰다. 예외가 있으면 새 프리셋마다 또 파야 한다. */
    const 흠 = 흠찾기(css);
...
/* ══ 구운 층은 border-image 를 안 쓴다 (2026-08-16 · 같은 결함 두 번에서 나왔다) ═══
   실측: «판»(카드)과 «칩»(태그)을 각각 9분할 border-image 로 얹었더니 둘 다 깨졌다 —
   카드는 둥근 모서리 위에 각진 액자가 얹혀 ㄱ자 표식이 남았고, 칩은 알약 대신 글자 아래로
   어긋난 검은 사각이 그려졌다(1배 렌더에서도 그대로). 뿌리는 하나다:
   **border-image 는 border-radius 를 무시한다.** 우리 부품은 전부 둥글다.
   ⚠한 건씩 지우면 세 번째가 온다 — 그래서 **부류를 막는다**(지침 「3번째 = 원인을 쓸 수 없게」).
   ⚠이 검사는 «구운 층»만 본다 — 흉내 층·골격이 border-image 를 쓰는 것까지 막지 않는다
     (막을 근거가 없다. 실측된 것은 «구운 사각 자산 + 둥근 부품»의 충돌이다). */
test('구움표는 border-image 를 안 쓴다 — 9분할은 둥근 부품과 원리적으로 안 겹친다', () => {
  const 구움만 = loom.css({ 지면: '문서' }).split('══ 구운 재질')[1] || '';
  assert.ok(구움만.length > 200, '픽스처 확인 — 구운 층이 비었다(자산이 없으면 이 검사는 헛것이다)');
  const 걸린것 = [...구움만.matchAll(/^\s*([^\n{]*)\{[^}]*border-image/gm)].map((m) => m[1].trim());
  assert.deepStrictEqual(걸린것, [],
    '구운 층이 border-image 를 쓴다 — border-radius 를 무시해 액자·어긋난 사각으로 그려진다: '
    + 걸린것.join(' / ') + '\n  늘어나는 부품을 구우려면 9분할이 아니라 «세 겹 배경»(마구리·가운데·마구리)이다.');
});

/* 실측 08-16: 절 실금(h2 뒤로 사라지는 선)이 `:not(:has(.번호))` 안에 «번호와 함께» 갇혀 있었다.
   실금은 번호와 아무 상관이 없는데, 그래서 번호를 손으로 적은 옛 원고 — 소개서 6벌 전부 — 에서
   실금이 통째로 없었다. 번호 자동/수동 어느 쪽이든 h2 는 실금을 가진다. */
test('절 실금은 «번호를 손으로 적은» 지면에서도 선다', () => {
  const css = loom.css({ 지면: '문서' });
  /* 실금 = ::after 중 «남은 폭을 flex 로 먹는» 것. 선택자만 긁어 번호 게이트가 붙었는지 본다. */
  const 실금 = [...css.matchAll(/([^\s{}][^{}\n]*::after)\{content:'';flex:1 1 3em/g)].map((m) => m[1].trim());
  assert.ok(실금.length, '픽스처 확인 — 절 실금(::after · flex:1 1 3em) 규칙이 아예 없다');
  const 갇힌것 = 실금.filter((s) => /:not\(:has\(\.번호\)\)/.test(s));
  assert.deepStrictEqual(갇힌것, [],
    '실금이 번호 유무에 갇혔다 — .번호 span 을 쓴 지면에서 절 실금이 사라진다: ' + 갇힌것.join(' / '));
});
...
const 픽스처 = [
  ...
  ['불릿에 코랄을 가둔다',
    (c) => c.replace(/(ul>li::before\{[^}]*?)rgba\(228,228,231,0?\.58\)/, `$1rgba(${코랄rgb},.58)`), /신호 1점/],
  ['레진에서 코어 글로우를 뺀다',
  ...
  ['하드코딩 hex 를 몸통에 심는다',
    (c) => c.replace('html{scroll-padding-top:96px;background:var(--graphite)',
      'html{scroll-padding-top:96px;background:#08090C'), /하드코딩 hex/],
  ['원판 값을 호출부가 겹쳐 쓴다',
