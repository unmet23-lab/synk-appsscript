...
function 태우기(옵션) {
  ...
  return { 호출, 로그, 메일, 숙달, 속성, 예외, 포인터: 속성['문법판정_대화포인터'], 벽: 속성['문법판정_벽_대화'] };
}
...
/* ─────────── 벽의 «만료» (#Q106) ───────────
 * 위 둘은 벽이 **선다**를 재고, 여기는 그 벽이 **영원히 서지는 않는다**를 잰다. 둘은 한 쌍이다 —
 * 한쪽만 있으면 처방이 반대쪽 병(영구 유실 ↔ 영구 정지)으로 넘어가도 초록이다.
 * 재료로는 못 잰다(미개원 · talk_log 0행)이라 탐지력은 전부 픽스처가 진다(가드 맹점 ②). */
const 창 = Number(교재.match(/const TB_JUDGE_WALL_RECOVERY_MS = (\d+) \* 86400000;/)[1]) * 86400000;
const 벽시계 = (자리, 전) => ({ '문법판정_벽_대화': 자리 + '|' + (Date.now() - 전) });
const 세명 = () => [대화행({ sid: 'S1' }), 대화행({ sid: 'S2' }), 대화행({ sid: 'S3' })];
const S2실패 = (n) => (n === 2 ? new Error('내용 필터') : { used: ['G201'], wrong: [] });

test('[#Q106] 🔒 창 «안»에서는 벽이 그대로 선다 — 1~6밤은 재시도라 일시 실패가 스스로 낫는다', () => {
  const r = 태우기({ 대화행들: 세명(), 응답: S2실패, 속성: 벽시계('S2@3',창 - 86400000) });
  assert.equal(r.포인터, '2', `창이 안 지났는데 넘어갔다 — 그 문장이 영구 유실이다(받은 값: ${r.포인터})`);
  assert.equal(r.메일.length, 0, '아직 안 버렸는데 버렸다고 통보했다');
});

test('[#Q106] 🔴 창이 지나면 그 학생을 버리고 포인터가 넘어간다 — 영구 실패 하나가 큐를 영영 막던 자리', () => {
  const r = 태우기({ 대화행들: 세명(), 응답: S2실패, 속성: 벽시계('S2@3',창 + 86400000) });
  assert.equal(r.포인터, '4',
    `🔴 벽이 안 내려갔다 — 매 밤 같은 앞쪽만 다시 읽고 그 뒤 학생은 무기한 미판정이다(받은 값: ${r.포인터})`);
...
test('[#Q106] ⏭ 버릴 땐 **말한다** — 조용히 넘기면 재료가 사라지는데 아무 화면도 안 바뀐다', () => {
  ...
  assert.ok(r.메일[0].제목.includes('S2'), '제목이 누구를 버렸는지 안 말한다: ' + r.메일[0].제목);
  assert.ok(/되읽고 싶으면/.test(r.메일[0].몸), '되돌리는 길을 안 줬다 — 따를 수 없는 통보는 통보가 아니다');
...
test('[#Q106] ☠️ 벽이 «바뀌면» 시계는 0 에서 다시 간다 — 옛 시계를 물려받으면 엉뚱한 학생을 버린다', () => {
  const r = 태우기({ 대화행들: 세명(), 응답: S2실패, 속성: 벽시계('S1@2', 창 * 3) });
  assert.equal(r.포인터, '2', '🔴 남의 시계로 S2 를 첫 밤에 버렸다');
...
test('[#Q106] ☠️ 깨진 시계는 «새 벽»으로 읽는다 — 못 읽는 값을 「오래됐다」로 접으면 그 밤에 즉시 버린다', () => {
  for (const 값 of ['', '쓰레기', 'S2@3|', 'S2@3|NaN', 'S2@3|0', 'S2@3|' + (Date.now() + 86400000)]) {
    const r = 태우기({ 대화행들: 세명(), 응답: S2실패, 속성: { '문법판정_벽_대화': 값 } });
    assert.equal(r.포인터, '2', `깨진 시계(${값})로 학생을 버렸다 — 틀릴 때 방향은 「지연」이어야 한다`);
    assert.equal(r.메일.length, 0, `깨진 시계(${값})로 통보까지 냈다`);
  }
});

test('[#Q106] 🔑 실패가 둘이면 «만료된 쪽»만 버리고 다음 벽 앞에 선다 — 한 밤에 전부 쓸어 넘기지 않는다', () => {
  const r = 태우기({
    대화행들: 세명(),
    응답: (n) => (n === 1 || n === 2 ? new Error('내용 필터') : { used: ['G201'], wrong: [] }),
    속성: 벽시계('S1@2', 창 + 86400000)
  });
  assert.equal(r.포인터, '2', `만료 안 된 S2 까지 넘어갔다 — 그 문장이 영구 유실이다(받은 값: ${r.포인터})`);
  assert.ok(String(r.벽).startsWith('S2@'), `다음 벽(S2)의 시계를 안 걸었다: ${r.벽}`);
  assert.equal(r.메일.length, 1, 'S1 만 버려야 한다: 통보 ' + r.메일.length + '건');
});

test('[#Q106] 🔑 벽이 아예 없으면 시계를 지운다 — 안 지우면 나중에 같은 자리에 선 벽이 첫 밤에 만료된다', () => {
  const r = 태우기({ 대화행들: 세명(), 속성: 벽시계('S2@3',창 * 2) });
  assert.equal(r.포인터, '4', '실패가 0인데 포인터가 안 갔다');
