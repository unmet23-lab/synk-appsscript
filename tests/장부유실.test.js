 ...
 * ⚠ `claude/*` 는 이 목록에 **안 넣는다** — 아래 F123 픽스처가 그 경계를 지킨다. 그 갈래는
 *   이름이 아니라 **세기**가 받는다(`유실()` 의 `내가지움`/`밖에만` · F617 델타). */
...
function 살아있는후보(cwd) {
   ...
   *     · `claude/*` = 이 목록에는 **안 넣는다**. 아래 F123 픽스처가 `origin/claude/phone` 으로
   *       그 판정을 지키고 있어 목록을 넓히면 확정된 회귀를 무르는 일이 되고, 애초에 **이름으로는
   *       못 가르는 갈래**다(그 결론과 수리는 `유실()` 안 F617 델타 주석에 있다 — 세기로 가른다).
   *       ⚠ **근거를 낡은 것으로 적지 마라.** 이 줄은 처음 「클라우드·폰 세션 가지라 방치된다」로
   *       썼다가 유호님 교정 2026-08-18 을 받았다: **폰으로 작업하는 것이 없다.** 실측도 그렇다 —
   *       그때 origin 의 `claude/*` 7개가 전부 그날 커밋된 클라우드 세션이었고(`synk-continue-*`·
   *       `f605-*` 등) 폰 가지는 0개였다. 「폰이라 방치된다」는 지금 없는 전제이고, 그 위에 세운
   *       판정은 근거가 아니라 관성이다. F123 픽스처의 가지 이름은 **당시 사례**이지 판정 재료가
   *       아니다 — 이 목록이 지키는 명제는 「죽은 판에만 남은 것은 유실」 하나다.
...
function 유실(cwd) {
  ...
  if (!후보.length) return { 유실: [], 뒤처짐: [], 어디: new Map(), 내가지움: [], 밖에만: [] };
  ...
  const 밖에만 = 안온것.filter((f) => !어디.has(f));
  return {
    유실: [...내가지움, ...밖에만].sort(),
    뒤처짐: 안온것.filter((f) => 어디.has(f)),
    어디,
    /* 🔑 `유실` 을 **두 조각으로도** 돌려준다(F617 델타 · 합은 그대로라 아래 픽스처는 한 줄도 안 바뀐다).
     *   위 `열린트랙가지` 사면이 「어느 기계냐」의 대부분을 없앴는데 **한 갈래가 남는다**:
     *   클라우드·폰 세션은 `claude/*` 가지에서 일하고 거기서 마찰을 신고한다. 그 이름은 F123 의
     *   «죽은 폰 가지»와 **글자가 같아서** 사면 목록으로는 영영 못 가른다.
     *   실측 2026-08-18(ubuntu · 위 사면이 이미 든 판): F619 가 그대로 「유실」로 떴다 —
     *   `origin/claude/synk-continue-cbjau8` 에서 지금 도는 세션의 신고다. 같은 날 `claude/*` 가지의
     *   PR #113 이 실제로 머지됐으니 그 가지는 살아 있는 통로다(버려진 폰 가지가 아니다).
     *   ⇒ 이름으로 가르려는 시도를 여기서 멈추고 **세기**로 가른다:
     *     · `내가지움` = 내 가지 이력에 있던 행이 지금 없다 = **삭제**. 기계와 무관하다.
     *       정본이 「append-only 장부는 행 삭제 금지 · 이 파일이 fail 로 잡는다」고 약속한 것이 이 갈래다.
     *     · `밖에만`  = 한 번도 내 계보에 온 적 없고 살아있는 판 목록에도 없다. 답이 «이 체크아웃»에
     *       걸린다(무엇을 fetch 했나 · 워크트리가 걸려 있나 · 남이 언제 머지했나).
     *   실 저장소 게이트는 앞엣것에만 걸고 뒤엣것은 이름과 함께 드러내기만 한다 — 정본 F296 그대로
     *   「탐지는 픽스처가 지고, 실저장소 검사는 fail 아닌 skip 으로 드러낸다」. 탐지력은 안 깎인다:
     *   F123·F195·F215·F469 픽스처가 `유실`(합)을 그대로 보므로 전부 살아 있다. */
    내가지움,
    밖에만,
  };
}
...
/* ── F617 델타 회귀 ─ `유실` 한 덩어리가 **두 세기**로 갈리는지 ─────────────────────
 * 실 저장소 게이트가 이 둘 중 «내가지움» 에만 걸리므로, 갈래가 뭉개지면 게이트가 통째로 갈린다:
 *   · «내가지움» 쪽으로 쏠리면 → CI·새 클론이 다시 상시 적색(F617 이 잡은 그 모양).
 *   · «밖에만»  쪽으로 쏠리면 → append-only 보호(F195)가 실 저장소에서 조용히 경고로 강등된다. **더 나쁘다.**
 * 합(`유실`)은 안 건드리므로 위·아래 F123·F195·F215·F469 픽스처는 한 줄도 안 바뀐다. */

test('🔑 F617 — 내가 지운 행은 `내가지움` 이다(기계와 무관 · 실 저장소 게이트가 걸리는 자리)', () => {
  const d = 픽스처([판([1, 2, 3]), 판([1, 3])]);
  const r = 유실(d);
  assert.deepStrictEqual(r.내가지움, ['F002'], '삭제가 «내가지움» 에 안 들면 실 저장소 게이트가 통째로 꺼진다(F195)');
  assert.deepStrictEqual(r.밖에만, [], '삭제인데 «밖에만» 으로 새면 경고로 강등돼 append-only 보호가 죽는다');
  assert.deepStrictEqual(r.유실, ['F002'], '합은 안 바뀐다 — 기존 회귀들이 이 합을 본다');
  치우기(d);
});

test('🔑 F617 — 도는 세션의 `claude/*` 가지에만 있는 행은 `밖에만` 이다(게이트가 안 걸린다)', () => {
  const d = 픽스처([판([1, 2]), 판([1, 2, 3])]);
  git(d, 'remote', 'add', 'origin', '.');
  git(d, 'update-ref', 'refs/remotes/origin/claude/synk-continue-어떤것', 'HEAD');
  git(d, 'update-ref', 'refs/remotes/origin/master', 'HEAD~1');
  git(d, 'branch', '--set-upstream-to=origin/master');
  git(d, 'reset', '--hard', '-q', 'HEAD~1');
  const r = 유실(d);
  assert.deepStrictEqual(r.밖에만, ['F003'],
    '이 갈래가 «내가지움» 으로 새면 CI·새 클론이 상시 적색이 된다 — F619 가 실제로 그 모양이었다(F617)');
  assert.deepStrictEqual(r.내가지움, [], '내 계보에 온 적 없는 행을 「내가 지웠다」로 세면 안 된다');
  assert.deepStrictEqual(r.유실, ['F003'], '합은 안 바뀐다 — 바로 아래 F123 회귀가 이 합을 본다');
  치우기(d);
});
...
test('🔴 실 저장소 — 이력에 올라왔던 행이 지금도 전부 있다', () => {
  ...
  /* 🔴 **게이트는 여기만** — 「내 가지 이력에 있던 행이 지금 없다」는 삭제이고, 기계와 무관하다(F617 델타). */
  assert.deepStrictEqual(내가지움, [],
    '장부에서 행이 사라졌다 — **내 가지 이력에 있던 행**이 지금 없다(내가 지웠거나 반입이 덮었다 · F195).\n'
    + '되살리는 법 — 그 번호가 살아있는 판을 찾아 그 줄만 지금 장부에 되돌려 넣는다:\n'
    + `  git log --all -S "| F123 |" --oneline -- ${장부} ${조각폴더}\n`
    + `  git show <그 커밋>:${장부} | grep "^| F123 |"\n`
    + '(번호는 위 목록의 것으로 바꾼다.)');

  /* ⚠ 「내 계보에 온 적 없고 살아있는 판 목록에도 없다」는 **빨갛게 하지 않는다**(F617 델타).
   *   위 `열린트랙가지` 사면이 `worktree-*` 를 걷어냈지만 `claude/*` 는 원리상 못 가른다 —
   *   지금 도는 클라우드 세션의 가지와 F123 의 죽은 폰 가지가 **글자가 같다**. 실측 2026-08-18:
   *   그 사면이 든 판에서도 F619(`origin/claude/synk-continue-cbjau8` · 지금 도는 세션의 신고)가
   *   그대로 빨갰다. 그 적색은 주인이 없어 아무도 못 고치고, 「평소 그 N건」으로 외워지는 순간
   *   **진짜 N+1 번째를 가린다** — 새는 방향이 「빨간 것을 안 믿게 만든다」다.
   *   ⇒ 잃는 것은 «실 저장소» 적색 하나뿐이고, 그 탐지력은 위 F123·F469 픽스처가 그대로 진다. */
  if (밖에만.length) console.log(
    `  ⚠ **내 계보 밖에만 있는** 행 ${밖에만.length}건 — 삭제가 아니라 «아직 안 온 것»일 수 있다(F617):\n`
    + 밖에만.map((f) => `      · ${f}`).join('\n')
    + '\n    → 도는 세션의 가지(`claude/*` 포함)가 들고 있으면 그냥 둔다. 그 가지가 **버려진** 것이면\n'
    + `      그 줄만 되돌려 넣는다: git log --all -S "| ${밖에만[0]} |" --oneline -- ${장부} ${조각폴더}\n`
    + '    🔑 이 개수는 «이 체크아웃이 무엇을 받아 뒀나»에 따라 달라진다 — 판정으로 쓰지 않는다(F207).');
});

/* 위 게이트가 «어느 갈래에 걸려 있는지»는 픽스처가 못 잡는다 — 픽스처는 `유실()` 을 직접 부르지
 * 실 저장소 단언문을 안 읽기 때문이다. 그래서 그 한 칸만 등록층으로 못박는다(F617 델타).
 * 이게 없으면 누가 합(`유실`)으로 되돌려도 **유호님 노트북에서는 초록이라** 안 걸리고, 그 판이
 * CI 에 가서야 빨개진다 — 잡히는 자리와 난 자리가 갈리는 것이 이 신고의 병 그 자체다. */
test('🔴 등록층 — 실 저장소 게이트는 `내가지움` 에만 걸린다(합에 걸면 «이 체크아웃»이 게이트가 된다 · F617)', () => {
  const src = fs.readFileSync(__filename, 'utf8');
  assert.match(src, /assert\.deepStrictEqual\(내가지움, \[\],/,
    '실 저장소 게이트가 «내가지움» 을 안 본다 — append-only 보호(F195)가 꺼졌거나 갈래가 갈렸다');
  assert.ok(!/assert\.(deepStrictEqual|deepEqual)\(\s*\[\s*\.\.\.내가지움/.test(src),
    '게이트가 다시 합에 걸렸다 — 「무엇을 받아 뒀나」가 곧 판정이 되어 CI·새 클론이 상시 적색이 된다(F617)');
});
