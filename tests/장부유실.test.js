...
function 유실(cwd) {
  ...
  return {
    ...
    어디,
    /* 🔑 `유실` 을 **두 조각으로도** 돌려준다(F617) — 합은 그대로라 위 픽스처는 한 줄도 안 바뀐다.
     *   실 저장소 게이트가 이 둘을 갈라 써야 하기 때문이다. 세기가 다르다:
     *     · `내가지움` = 「내 가지 이력에 있던 행이 지금 없다」. **기계와 무관하다** — 정본이
     *       「append-only 장부는 행 삭제 금지 · 이 파일이 fail 로 잡는다」고 약속한 것이 정확히 이것이다.
     *     · `밖에만`  = 「한 번도 내 계보에 온 적 없고, «살아있는 판» 목록에도 없다」. 이쪽은
     *       **어느 ref 를 받아 뒀는지·워크트리가 걸려 있는지**에 답이 걸린다 — 즉 기계 상태다.
     *   실측 2026-08-18(ubuntu 컨테이너): `밖에만` 이 F595·F610·F612·F614 를 냈는데 넷 다 안 지워졌고,
     *   같은 기계에서 남이 PR #107 을 머지하자 40분 만에 4건→1건이 됐다(내가 고친 줄 0).
     *   판정이 장부 무결성이 아니라 «남의 머지 시각»을 따라간 것이다. 그 사이 처방문은
     *   「그 줄을 되돌려 넣어라」라고 시키는데, 따르면 **남이 지금 들고 있는 미머지 번호를 베끼는 것**이라
     *   그 트랙이 머지되는 순간 위 `F586 조각이상` 이 「겹침」으로 문다(F103 의 최악판). */
    내가지움,
    밖에만,
  };
}
...
/* ── F617 회귀 ─ `유실` 한 덩어리가 **두 세기**로 갈리는지 ────────────────────────────
 * 실 저장소 게이트가 이 둘 중 «내가지움» 에만 걸리기 때문에, 갈래가 뭉개지면 게이트가 통째로 갈린다:
 *   · 갈래가 «내가지움» 으로 쏠리면  → CI 상시 적색이 되살아난다(F617 이 잡은 그 모양).
 *   · 갈래가 «밖에만» 으로 쏠리면    → append-only 보호(F195)가 실 저장소에서 조용히 꺼진다. **더 나쁘다.**
 * 그래서 두 방향을 각각 못박는다. 합(`유실`)은 안 건드리므로 위 F123·F469 회귀는 한 줄도 안 바뀐다. */

test('🔑 F617 — 내가 지운 행은 `내가지움` 이다(기계와 무관 · 실 저장소도 여기서 빨개진다)', () => {
  const d = 픽스처([판([1, 2, 3]), 판([1, 3])]);
  const r = 유실(d);
  ...
  assert.deepStrictEqual(r.유실, ['F002'], '합은 안 바뀐다 — 기존 회귀가 이 합을 본다');
  치우기(d);
});

test('🔑 F617 — 갈라진 가지에만 있는 행은 `밖에만` 이다(답이 «이 체크아웃»에 걸린 갈래)', () => {
  const d = 픽스처([판([1, 2]), 판([1, 2, 3])]);
  ...
  git(d, 'reset', '--hard', '-q', 'HEAD~1');
  const r = 유실(d);
  assert.deepStrictEqual(r.밖에만, ['F003'], '이 갈래가 «내가지움» 으로 새면 CI·새 클론이 상시 적색이 된다(F617)');
  assert.deepStrictEqual(r.내가지움, [], '내 계보에 온 적 없는 행을 「내가 지웠다」로 세면 안 된다');
  assert.deepStrictEqual(r.유실, ['F003'], '합은 안 바뀐다 — 바로 위 F123 회귀가 이 합을 본다');
  치우기(d);
});
...
test('🔴 실 저장소 — 이력에 올라왔던 행이 지금도 전부 있다', () => {
  ...
  /* 🔴 **여기만 빨갛게 한다**(F617) — 「내 가지 이력에 있던 행이 지금 없다」는 삭제이고, 기계와 무관하다.
   *   정본이 「append-only 장부는 행 삭제 금지 — 이 파일이 fail 로 잡는다」고 약속한 것이 정확히 이 갈래다. */
  ...
  /* ⚠ 「한 번도 내 계보에 온 적 없고 살아있는 판 목록에도 없다」는 **빨갛게 하지 않는다**(F617).
   *   답이 장부가 아니라 «이 체크아웃»에 걸려 있어서다 — 어느 ref 를 받아 뒀나 · 워크트리가 걸려 있나.
   *   유호님 노트북에서는 트랙마다 로컬 워크트리가 있어 사면되지만, CI(`fetch-depth: 0` · 워크트리 0개)와
   *   클라우드 컨테이너·새 클론에서는 같은 커밋이 **상시 적색**이 된다(실측 2026-08-18 ubuntu).
   *   그 적색은 주인이 없어 아무도 못 고치고, 「평소 그 N건」으로 외워지는 순간 **진짜 N+1 번째를 가린다.**
   *   ⇒ 정본 F296 규율 그대로: **탐지력은 위 픽스처가 지고**(F123·F469 회귀가 이 갈래를 전건 못박는다),
   *      실 저장소는 이름과 함께 **드러내기만** 한다. 잃는 것 = 죽은 가지에만 남은 행의 «실 저장소» 적색
   *      하나뿐이고, 그건 픽스처에서 그대로 빨갛다. */
  if (밖에만.length) console.log(
    `  ⚠ **내 계보 밖에만 있는** 행 ${밖에만.length}건 — 삭제가 아니라 «아직 안 온 것»일 수 있다(F617):\n`
    + 밖에만.map((f) => `      · ${f}`).join('\n')
    + '\n    → 도는 트랙(다른 세션의 가지)이 들고 있으면 그냥 둔다. 그 가지가 **버려진** 것이면\n'
    + `      그 줄만 되돌려 넣는다: git log --all -S "| ${밖에만[0]} |" --oneline -- ${장부} ${조각폴더}\n`
    + '    🔑 이 목록은 «이 체크아웃이 무엇을 받아 뒀나»에 따라 달라진다 — 개수를 판정으로 쓰지 않는다.');
});

/* 위 게이트가 «어느 갈래에 걸려 있는지»는 픽스처가 못 잡는다 — 픽스처는 `유실()` 을 직접 부르지
 * 실 저장소 단언문을 안 읽기 때문이다. 그래서 그 한 칸만 등록층으로 못박는다(F617).
...
test('🔴 등록층 — 실 저장소 게이트는 `내가지움` 에만 걸린다(합에 걸면 «이 체크아웃»이 게이트가 된다 · F617)', () => {
  ...
  assert.ok(!/assert\.(deepStrictEqual|deepEqual)\(\s*\[\s*\.\.\.내가지움/.test(src),
    '게이트가 다시 합에 걸렸다 — 「무엇을 받아 뒀나·워크트리가 걸려 있나」가 곧 판정이 되어 CI 가 상시 적색이 된다(F617)');
});
