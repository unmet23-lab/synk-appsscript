 ...
 *   ④ 반출(증언반출_) — 개발·공개 사용의 기록된 명시적 동의 확인이 없어 현재는 중지(null). 비식별만으로 허용하지 않는다.
...
test('[증언] 정본 자리에 «다른 이름»이 서 있으면 덮지 않고 멈춘다 — 그 열의 값이 남의 이름을 뒤집어쓴다(코덱스 3차 P0 · 결정 09-03)', () => {
  ...
  const 몸 = section('function 증언헤더충돌_(', '/** 밖으로 나가기 «전» 검문');
  assert.ok(!/cur\[i\] \+ '」/.test(몸) && !/'「' \+ cur\[i\]/.test(몸), '충돌 로그에 셀값을 싣지 않는다(코덱스 4차 P3) — 열 번호와 정본 이름만');
});
...
test('[증언] 검문 — 이름 조각 · 긴 숫자열 · 이메일 · 손잡이 · 학생 번호 꼴 · 주소가 들면 null · 명단이 없으면 검문 불가(null) · 통과하면 원문 그대로', () => {
  ...
  assert.equal(E.증언비식별_(이름들, '세종대로 1번길 근처예요'), null, '도로명 번길');
  /* [v9.328] 코덱스 5차 — 4차에서 좁힌 검문의 빈틈: 생년 문맥 · 글자로 쓴 몽골어 달 · 복수형·붙여 쓴 기관명 · 띄어쓰기 변형 주소 */
  [
    '2020년생이에요', '2023년에 입학했어요', 'Тавдугаар сарын 3-нд төрсөн', '3 дугаар сарын 5-нд төрсөн',
    'TEST Academies에 다녀요', 'ExampleAcademy에 다녀요', '테헤란로  12에 살아요', '종로 1에 살아요', '주소는 가상로 1입니다',
  ].forEach(v => assert.equal(E.증언비식별_(이름들, v), null, '식별 꼴이 통과했다: ' + v));
  assert.equal(E.증언비식별_(이름들, '다시 해 볼게요, 친구랑 같이'), '다시 해 볼게요, 친구랑 같이', '「다시」·「친구」 같은 보통 말은 통과한다');
  /* [v9.327] 코덱스 4차 — 검문은 «식별 꼴»만 건다. 보통 문장은 엔진 재료다(철학 Ⅰ-3②④). */
  [
    '한국어로 3문장을 썼어요', '앞으로 4급에 도전할 거예요', 'Өдөр бүр давтдаг болсон', 'I want to improve academic writing.',
    '2027년에 4급을 따고 싶어요', '매일 30분씩 공부해요',
  ].forEach(v => assert.equal(E.증언비식별_(이름들, v), v, '보통 문장이 걸렸다: ' + v));
...
/* ───────────────────── ④ 반출 ───────────────────── */
test('[증언] 증언반출_ — 비식별 가능한 답도 목적별 명시적 동의 확인 전에는 반출하지 않는다', () => {
  const rows = [
  ...
  assert.equal(r, null, '기록된 목적별 동의 확인 없이 학생·보호자 원문을 반출했다');
  const b = 엔진({ sheet: 시트(rows), names: null });
  assert.equal(b.E.증언반출_(b.ss), null, '명단이 없어도 동의 없는 반출은 차단한다');
});
