...
const 원천밖 = {
  ...
  '#FAFAF9': 'Paper — 08-18 확정 중성축의 라이트 정박점(바탕·라이트). 유도기 tools/중성축.js · 근거 = 조항 ⓗ',
  ...
  '#1B1B1A': 'Ink — 본문·대형 타이포(라이트). 구 Ink 의 Paper 대비 16.53 을 보존해 유도(16.50). 근거 = 조항 ⓗ',
  ...
  '#8C8C8C': 'Ash — 다크 위 3층 잉크. 구 Slate 의 다크 대비 5.78 을 보존해 유도한 뒤 **바닥 게이트**로 2/255 밝혔다(Graphite 3 위 4.38 → 4.50 · 2026-08-19). 바닥 한정은 조항 ⓔ 가 그대로 진다',
  '#666666': 'Ash 2 — 라이트 위 3층 잉크. 구 Slate 2 의 Paper 대비 5.39 를 보존해 유도한 뒤 **바닥 게이트**로 1/255 어둡혔다(Chalk 판 위 4.46 → 4.53 · 유호 확정 2026-08-19). 바닥 한정은 조항 ⓔ',
};
...
const 퇴역 = {
  ...
  '#171820': 'Ink → Ink #1B1B1A', '#8A93AD': 'Slate → Ash #8C8C8C', '#5F657D': 'Slate 2 → Ash 2 #666666',
};

/* 🔴 옛 «유도값» — 퇴역(구 킷 11색)과 **다른 갈래**다. 여기 든 것은 구 킷 hex 도 DEAD 도 아니고
 *   «한때 새 hex 였던 것»이라 두 그물에 다 안 걸린다: 퇴역 map 은 구hex 를 찾고, DEAD 는 유호님이
 *   폐기한 디자인 결정의 목록이다. 그 틈으로 이미 구워진 지면이 조용히 낡는다.
 *   ⚠퇴역 목록에 섞지 않는다 — 그 목록은 「조항 ⓗ 가 갈아탄 11색」이라는 **분모**를 지고 있어서
 *     한 줄만 더해도 그 단언이 깨진다(실측 2026-08-19).
 *   목록의 정본은 `tools/중성축.js` 의 `옛유도` 이고, 여기서는 그것을 **읽어서** 검사한다. */
const 옛유도 = require(path.join(ROOT, 'tools', '중성축.js')).옛유도;
...
/** 옛 유도값 → 「어디로 가야 하나」 문장. 위 `퇴역` 과 같은 모양으로 만들어 한 루프에서 함께 잰다. */
const 옛유도맵 = () => Object.fromEntries(
  Object.entries(옛유도).map(([옛, v]) => [옛.toUpperCase(), `옛 유도값 ${v.신} → ${v.지금} (${v.사유})`]));

test('중성축 — 옛 «유도값» 목록이 비지 않았고 킷에 되살아나 있지 않다', () => {
  const 목록 = Object.entries(옛유도);
  assert.ok(목록.length >= 1,
    '옛 유도값이 0건이다 — 유도 규칙을 고칠 때마다 `tools/중성축.js` 의 `옛유도` 에 한 줄을 더해야 '
    + '그 회차의 잔재가 잡힌다(0건이면 위 지면 검사가 그 갈래에서 조용히 헛돈다 · F207)');
  for (const [옛, v] of 목록) {
    assert.ok(!KIT[옛.toUpperCase()], `옛 유도값 ${옛} 가 킷에 살아 있다 — 그러면 지면 검사가 무의미하다`);
    assert.ok(KIT[v.지금.toUpperCase()], `«지금 값» ${v.지금} 가 킷에 없다 — 옛유도 표가 낡았다(${v.신})`);
  }
});
...
for (const p of 퇴역과녁) {
  test(`중성축 — ${path.relative(ROOT, p)} 에 퇴역한 구 킷 색이 없다`, () => {
    ...
    for (const [hex, 갈길] of Object.entries({ ...퇴역, ...옛유도맵() })) {
