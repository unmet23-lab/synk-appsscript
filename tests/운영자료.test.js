...
const { 다음번호, 바로가기냐, 안전한이름, 이미있나, 짝찾기, 정본후보, 폴더명, 기저키, 짝의심 } = require('../tools/운영자료.js');
  ...
  const 걷기 = (d) => {
    for (const e of fs.readdirSync(d, { withFileTypes: true })) {
      ...
      fs.readFileSync(p, 'utf8').split('\n').forEach((줄, i) => {
        const m = 줄.match(번호참조());
        if (m) 샌것.push(`${라벨}/${rel.replace(/\\/g, '/')}:${i + 1}  ${m.join(' · ')}`);
      });
    }
  };
  걷기(뿌리);
...
test('실저장소 docs/ — 바탕화면을 번호로 가리키는 줄이 없다', () => {
  ...
  assert.deepEqual(샌것, [],
    '번호는 재편·삭제 때마다 밀린다 — 갈래 + 이름으로 적어라(예: 운영 「이해 대장 (어디가 비었나)」)');
});
...
test('메모리 — 바탕화면을 번호로 가리키는 줄이 없다 (repo 밖이라 없으면 skip)', (t) => {
  ...
  assert.deepEqual(훑기(뿌리, 'memory'), [],
    '번호는 재편·삭제 때마다 밀린다 — 갈래 + 이름으로 적어라');
});
