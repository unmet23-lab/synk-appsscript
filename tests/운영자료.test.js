...
const { 바로가기냐, 안전한이름, 이미있나, 짝찾기, 정본후보, 상태파일, 기저키, 짝의심,
  상태판정, 스냅샷원본 } = require('../tools/운영자료.js');

/* ── 번호를 안 붙인다 (유호 픽 2026-08-17 ㉰) ──────────────────
 * 옛 도구는 넣는 파일마다 `01_`·`02_` 를 붙였다. 그 번호는 재편·삭제 때마다 밀리는데
 * 이미 박힌 참조는 안 따라간다 — 2026-08-14 실측에서 **번호 참조 18곳 중 16곳**이
 * 딴 파일을 가리키고 있었다. 규칙은 08-13 에 세웠지만 프로즈라 앞으로만 지켜졌다.
 *
 * 🔑 이 회귀가 무는 것은 **쓰는 쪽 하나**다. 읽는 쪽(`이미있나`·`짝찾기`·`이름정리`)의
 *    `^\d{2}_` 벗기기는 살아 있어야 한다 — 「보관」으로 내린 옛 파일을 관대하게 읽는 몫이다.
 *    그쪽 탐지력은 아래 「짝찾기: 번호를 뗀 이름으로…」 테스트들이 이미 지고 있다.
 * ⚠ 대가(틀릴 때의 모습): 이건 **소스 검사**라, 번호를 다른 변수명으로 조립하면 못 본다.
 *    그 대신 픽스처가 「옛 코드 그대로의 모양」을 못박아 조용한 부활은 막는다. */

/** 「넣는 이름에 번호를 붙이는 코드」의 모양. 탐지력은 바로 아래 픽스처가 진다.
 *  ⚠ `\b` 를 쓰지 않는다 — `\w` 는 ASCII 전용이라 **한글 식별자 앞에서는 단어경계가 안 선다**.
 *     처음에 `\b다음번호` 로 적었다가 픽스처가 통째로 안 잡혔다(이 저장소가 한글 정규식에서
 *     반복해 당한 자리다 · `지도대장.js` 엣지 정규식 주석에 같은 사고가 적혀 있다). */
const 번호붙임 = () => /\$\{\s*번호\s*\}\s*_|다음번호\s*\(/;

test('번호붙임 탐지: 걷어낸 옛 코드를 그대로 문다 (픽스처)', () => {
  for (const s of [
    'const 파일명 = 링크냐 ? `${번호}_${베이스}.lnk` : `${번호}_${베이스}${path.extname(절대)}`;',
    'const 번호 = 다음번호(항목.map((x) => x.이름));',
    'const 이름 = 로그찾기(있는것) || `${다음번호(있는것)}_${로그이름}`;',
  ]) assert.ok(번호붙임().test(s), `못 잡았다: ${s}`);
});

test('번호붙임 탐지: 번호를 «떼는» 코드는 안 문다 — 거짓양성 0 (읽는 쪽은 관대해야 한다)', () => {
  for (const s of [
    "const 번호뗀 = (n) => n.replace(/^\\d{2}_/, '').toLowerCase();",
    "  .replace(/^\\d{2}_/, '')           // 운영자료가 붙인 일련번호",
    'const base = 사본이름.replace(/^\\d{2}_/, and).toLowerCase();',
  ]) assert.ok(!번호붙임().test(s), `거짓양성: ${s}`);
});

test('실소스 — 넣는 파일에 번호를 다시 붙이지 않는다 (부활 방지)', () => {
  const fs_ = require('node:fs');
  for (const rel of ['tools/운영자료.js', 'tools/뉴스적재.js']) {
    const src = 코드만(fs_.readFileSync(path.join(__dirname, '..', rel), 'utf8'));
    assert.ok(!번호붙임().test(src),
      `${rel} 이 번호를 다시 붙인다 — 유호 픽 08-17 ㉰ 「번호를 뗀다」가 무너진다`);
  }
});

test('`다음번호` 는 export 에서 사라졌다 — 부르는 쪽이 다시 생기지 못하게', () => {
  const 운영자료 = require('../tools/운영자료.js');
  assert.strictEqual(운영자료.다음번호, undefined,
    '되살리면 위 회귀가 무는 자리를 우회할 통로가 다시 생긴다');
});
...
test('`_지금상태` 는 「SYNK 코어」 안에 굽는다 — 바탕화면 최상위가 아니다', () => {
  /* 뿌리가 둘이 되면서 이 한 장을 어디 둘지가 판정이 됐다. 최상위는 유호님이 배치한 자리라
   * 우리 파생물이 끼면 그 자체가 어수선함이고, 뿌리마다 굽게 두면 **안 연 한 장이 낡는다**. */
  assert.strictEqual(상태파일, path.join('SYNK 코어', '_지금상태.html'));
});
...
test('갈래는 셋(유호 지시 08-17 재편)이고, 모르는 갈래는 조용히 기본으로 안 떨어진다', () => {
  assert.deepEqual(Object.keys(갈래들), ['코어', '운영', '홍보']);
  /* 🔑 **뿌리가 둘이다** — 「SYNK 코어」는 바탕화면에 바로 서고, 나머지 둘은 「SYNK LAB」 아래로
   * 들어간다. 이걸 안 물면 세 폴더가 평평하게 서서 유호님 화면이 셋으로 갈라진다(재편 전 병). */
  assert.match(갈래폴더('C:\\뿌리', '코어'), /뿌리[\\/]SYNK 코어$/);
  assert.match(갈래폴더('C:\\뿌리', '운영'), /뿌리[\\/]SYNK LAB[\\/]운영$/);
  assert.match(갈래폴더('C:\\뿌리', '홍보'), /뿌리[\\/]SYNK LAB[\\/]홍보·시안$/);
  // 옛 갈래 이름으로 부르면 «조용히 운영에 쌓이지 않고» 던진다 — 안 그러면 이관 누락이 안 보인다.
  for (const 옛 of ['철학', '정본', '엔진', '룸', '몽글']) {
    assert.throws(() => 갈래폴더('C:\\뿌리', 옛), /모르는 갈래/, `옛 갈래 「${옛}」 가 아직 산다`);
  }
