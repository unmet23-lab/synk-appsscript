...
const 호출부스냅샷 = {
  '교재연동.js': 3,        // masteryFromFeedback_(쓴 글) + [v9.248] masteryFromVoice_(말한 것의 전사) + [v9.249] masteryFromTalk_(대화에서 스스로 쓴 문장) — 셋 다 문법 목록+문장만(식별자 0 · 자리별 검사 아래 ①)
...
const 자리들 = [
  ...
  /* [v9.249 · #Q99] 세 번째 판정관(자유 대화) — 이 층은 앞의 둘과 재료의 성질이 다르다.
   *   전사문·제출문은 과제 산출이라 학교 이야기가 대부분이지만, **자유 대화는 학생이 화제를 고른다**
   *   — 친구 이름·가족·사는 곳이 학생 문장 안에 그대로 들어올 수 있다. 그래서 이 자리는 「식별자를
   *   이어 붙이지 않았나」만으로는 부족하고, **실리는 것이 `글[sid]` 한 덩이인지**를 못박는다
   *   (원문을 보내는 것 자체는 판정에 필요하다 — 막을 것은 «우리가» 이름을 덧붙이는 일이다). */
  {
    id: '문법판정-대화입력', file: '교재연동.js',
    from: 'out = aiCall_(apiKey, 대화지문,', to: 'schema, 2048);',
    필수: [{ re: /bankList\.join/, 왜: '판정 목록이 문법 뱅크 파생이 아니다' },
      { re: /글\[sid\]/, 왜: '실리는 것은 묶인 학생문뿐이어야 한다' }],
    금지: [{ re: /\bsid\s*\+|\+\s*sid\b/, 왜: '대화 판정 입력에 student_id 가 문자열로 이어 붙었다' },
      { re: /rows\[|getValues\(\)/, 왜: '대화 행을 통째로 보내면 AI답·급수·모델명까지 벤더로 나간다' }],
  },
  {
