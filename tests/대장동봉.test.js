...
test('🔴 급소 — 게이트의 발동 목록이 생성기 자신을 포함한다 (F520 이 난 바로 그 칸)', () => {
  const 게이트 = fs.readFileSync(path.join(ROOT, 'tools', '대장동봉검사.js'), 'utf8');
  assert.match(게이트, /'tools\/이해대장\.js'/,
    '생성기가 발동 목록에 없다 — 도구만 담은 커밋에서 이 게이트는 눈이 먼다(F520)');
});
...
test('🔴 급소 — 생성기가 새로 읽기 시작한 in-repo 부품이 발동 목록에서 빠지지 않는다', () => {
  ...
  const 빠진것 = 읽는것.filter((p) => !게이트.includes(p));
