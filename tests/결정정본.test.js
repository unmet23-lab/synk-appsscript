...
const { 읽기문, 주입선택, 관찰일, 구속일, 시작표식, 끝표식, 성격들, 기본성격, 아는플래그, 정본 } = 결정;
...
test('②-b 짧은 요약은 과거 안전 기록으로 현재 결정을 밀어내지 않는다', () => {
  const r = 읽기문(본문(
    줄('2016-01-01', '안전', '과거 보호 기록'),
    줄('2026-08-19', '확정', '현재 결정'),
  ), '2026-08-19');
  const 선택 = 주입선택(r.목록, 1);
  assert.equal(선택.후보.length, 1);
  assert.equal(선택.실을것[0].결정, '현재 결정');
});
 ...
 *   `tests/표칸.test.js`(⚠삭제됨 08-19 e75fc7fc) 가 그 자리를 잡았다. 이 정본은 백틱 인용이 빽빽해서
