...
/* ── ③-b 재료가 없는 줄도 한 덩어리가 아니다 ────────────────────
 * 실측 분류(08-12): 재료 없는 7건 중 2건은 **과녁이 유호님 답·외부 기관**에 있었다.
 * 기계가 그 일을 못 재는 건 맞지만 **「내가 집을 줄이 아니다」는 잴 수 있다** — 뭉치면
 * 다음 세션이 7줄을 다 열고, 갈라 주면 둘은 «정확한 이유로» 안 열어도 된다. */

test('☠️ ⏳·⏸ 가 붙은 재료 없는 줄은 「집을 게 아니다」로 갈라 낸다', () => {
  const { 밖, as } = 임시작업공간();
  형제세우기(밖);
  const 유호 = 낡음.증거('| 2026-08-12 | **상표 출원** | x | ▶출원서 임시저장 · ⏳**유호=견본 문자냐 로고냐** |', as);
  assert.equal(유호.잰것, false);
  assert.match(유호.대기, /유호님 답 대기/);
  const 시점 = 낡음.증거('| 2026-08-12 | **상표 출원** | x | ▶다음=번호 부여 확인 · ⏸서식작성기 |', as);
  assert.match(시점.대기, /시점 대기/);
});

test('🔑 잴 수 있었던 줄에는 대기 표식을 안 붙인다 — 「대조하라」와 「집지 마라」가 싸운다', () => {
  const { 밖, as } = 임시작업공간();
  fs.writeFileSync(path.join(형제세우기(밖), 'lib', '멘탈게이지.js'), '//\n');
  const ev = 낡음.증거('| 2026-08-11 | **G1 — `lib/멘탈게이지.js`** | x | 🎫다음=신설 · ⏳유호=뭔가 |', as);
  assert.equal(ev.잰것, true);
  assert.equal(ev.대기, null);
});
