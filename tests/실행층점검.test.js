...
test('자동 범위는 초록이어도 「무엇만 봤는지」를 밝힌다', { skip: 이력없음 }, () => {
  /* 항목 0건일 때야말로 기준을 말해야 한다 — 예전엔 여기서 기준을 버려서
   * 「마지막 커밋만 봤다」가 하필 초록일 때만 사라졌다. */
  const r = execFileSync(process.execPath, [path.join(ROOT, 'tools', '실행층점검.js')],
    { cwd: ROOT, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] });
  assert.match(r, /HEAD~1\.\.HEAD 만 봤다/, '항목 0건일 때 기준을 버렸다');
});
