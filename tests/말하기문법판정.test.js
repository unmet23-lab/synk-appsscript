...
function 태우기(옵션) {
  ...
  return { 호출, 로그, 숙달, 속성, 마크: 속성['문법판정_음성마크'] };
}
...
test('[#Q99] 🔒 말하기 근거 **단독**으로는 «도달» 승격이 안 된다 (재는 층이 ASR 이라서)', () => {
  const r = 태우기({
    음성행들: [음성행({ sid: 'S1' })],
    숙달행들: [숙달행({ sid: 'S1', gid: 'G201', 상태: '연습', 출처: 'AI음성' })],
    응답: { used: ['G201'], wrong: [] }
  });
  assert.deepEqual(r.숙달.map((x) => x.상태), ['연습'],
    '어제도 말하기·오늘도 말하기인데 «도달»로 올라갔다 — 자동 전사가 다듬은 문장 하나로 진화 게이트가 열린다');
});
...
test('[#Q99] 야간 오케스트레이터가 전사 **뒤에** 이 판정을 부른다 (그날 전사분이 그날 판정에 들어간다)', () => {
  ...
  assert.ok(전사 > -1 && 판정 > -1, '야간 오케스트레이터에서 두 호출을 못 찾았다');
