/* [v9.94] 반 편성 회귀 테스트 — 반 수·정원·좌석 합계가 정본과 어긋나면 여기서 잡힌다.
 *
...
test('[v9.94] 개원 = 3실 18반 · 전 반 16명 · 288석 (급여 정본 v1.1 §1)', () => {
...
test('[v9.94] 반 수 = 슬롯 수 × 실 수 (표를 늘려도 산수가 깨지지 않는다)', () => {
  [1, 2, 3, 4].forEach((n) => {
...
test('[v9.94] 반 이름은 전부 고유하고, 4실을 열어도 3실 이름이 하나도 안 바뀐다', () => {
...
test('[v9.94] 코어 슬롯은 11·14·15:30·18시 + 토 11·14시이고, 시각 단일 소스는 시작시간 열이다', () => {
...
test('[v9.94] 표시명은 키와 분리돼 있고, 개원 기본값은 시각이다', () => {
...
test('[v9.94] 정원은 이름이 아니라 시트가 정한다 — 구 v8.3 이름 판정 잔존 0', () => {
...
test('[v9.94] 보스 HP = 정원 × 계수 — 16명이면 평일 448 · 주말 128', () => {
...
test('[v9.94] setupSchedule 재실행이 각 반이 지은 표시명을 지우지 않는다', () => {
...
test('[v9.94] 4실 개방은 표시명을 물려받는다 (3실 이름이 D 추가로 안 날아간다)', () => {
...
test('[v9.94] 리그 짝짓기가 18반(짝수)·홀수 반 수 모두에서 안 깨진다', () => {
