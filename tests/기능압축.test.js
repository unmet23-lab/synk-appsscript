...
test('[v9.254] 주간 리포트 AI 해설은 재료가 0이면 안 불린다(개원 전에도 매주 나가던 자리)', () => {
  // 정기 트리거 11개를 전수로 훑었더니 **재료 가드가 없는 유일한 자리**였다(2026-08-18 실측 · 유호 승인).
  // 학생 0명·데이터 0건이어도 매주 월 07시에 Opus 호출 1회가 나갔다 — 막던 것은 `aiText_` 의 키·리허설
  // 관문뿐이라 키가 선 라이브에서는 그대로 나갔고, 그 회차가 읽는 것은 '(내용 없음)' 뿐이라 값이 0이다.
  const w = section('function weeklyJobs()', 'function 주간해설재료_');
  assert.ok(/if \(!집계실물 \|\| 재료행 === 0\)/.test(w),
    'H7 해설이 재료 게이트를 안 탄다 — 집계가 전부 비어도 매주 Opus 호출이 나간다');
  // 🔴 [v9.255] 렌더된 «글자»만으로는 못 막는다 — 화이트리스트 섹션 둘이 무데이터에도 상수 서식을
  //    돌려주기 때문이다(①배포 검수 P1 `222c989ac23f`). 원본 «행»을 세는 축이 반드시 함께 있어야 한다.
  assert.ok(/const 재료행 = 주간해설재료_\(/.test(w),
    '원본 행 축이 없다 — `집계실물` 만으로는 미개원(0 이 서식으로 찍히는 주)을 못 막는다');
  // 🔑 KPI 주입치를 **넘겨야** 상담 갈래가 산다 — 값 시험은 함수를 직접 부르므로 이 이음매를 못 본다.
  //    안 넘기면 상담만 있는 주에 해설이 조용히 사라진다(P2 `dab842724084` 와 같은 자리).
  assert.ok(/주간해설재료_\(SpreadsheetApp\.getActiveSpreadsheet\(\), kpiInjection\)/.test(w),
    '재료 계수에 KPI 주입치를 안 넘긴다 — 상담 축이 통째로 죽어 fail-closed 로 샌다');
  // 🔑 초기값도 못박는다 — `1` 로 시작하면 위 게이트가 서 있어도 «언제나 열린다»(구조 검사의 사각).
  assert.ok(/let 집계실물 = 0;/.test(w),
    '실물 카운터가 0에서 시작하지 않는다 — 게이트가 모양만 남고 늘 통과한다');
  // 🔑 게이트는 **호출보다 앞**에 있어야 한다 — 뒤면 이미 나간 뒤다(순서로 잰다 · F287 과 같은 축).
  assert.ok(w.indexOf('if (!집계실물 || 재료행 === 0)') < w.indexOf('cmtH7 = aiText_('),
    '재료 게이트가 aiText_ 호출보다 뒤에 있다 — 서 있어도 못 막는다');
  // 🔑 세는 대상은 폴백 «전»의 원문이어야 한다. `aiBody` 는 빈 값을 '(내용 없음)' 으로 채우므로
  //    구조적으로 안 빈다 — 그 문자열을 세면 가드가 언제나 통과한다(그게 이 결함의 원인이었다).
  assert.ok(/if \(txt && String\(txt\)\.trim\(\)\)\s*집계실물\+\+/.test(w),
    '실물 판정이 원문(txt)이 아니다 — 폴백 문자열을 세면 가드가 항상 참이 된다');
});
