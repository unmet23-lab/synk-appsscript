...
test('🔑 표식 머리도 자리다 — `F583`·`②-20` 축이 이관 뒤에도 산다 (F203 이 닫은 자리)', () => {
  /* 🔴 변이 ⑧ 이 잡은 구멍이다: 표식은 **트랙 칸 맨 앞**에서만 읽히는데 첫 판은 질의를 파일 칸에만
   *   넣어서 원리상 안 닿았다 — 「비었다」로 새는 방향이라 조용했다. 토큰마다 양쪽 칸에 놓아 닫았다. */
  const root = 저장소({ [남지문]: `| 2026-08-18 | **F583** — 남의 트랙 | docs/아무거나.md | 작업중 (\`local_${남지문}\`) |` });
  assert.strictEqual(점유(root, ['F583']).코드, 1, '표식 자리를 못 물었다 — F203 이 닫은 축이 이 도구에서만 죽는다');
  assert.strictEqual(점유(root, ['docs/딴것.md', 'F583']).코드, 1,
    '🔴 표식이 **두 번째 토큰**일 때 놓쳤다 — 질의를 한 줄로 이어 붙이면 머리가 아니게 된다(변이 ⑧)');
});
