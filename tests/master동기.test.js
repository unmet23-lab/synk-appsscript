...
test('⑥-b master 에 커밋하는 도구는 **전부** 밀기와 짝을 이룬다 (옛 통로 금지)', () => {
   ...
   * ⚠ [2026-08-18] 하한 5 → 4. `tools/폰작업반입.js` 를 접어(유호 확정 · 실측 11일간 0회 사용 ·
   *   같은 일을 PR 동선이 대신) 그 도구가 분모에서 빠졌다. **정규식이 죽은 것이 아니라 대상이
   *   하나 줄었다** — 둘은 같은 숫자를 내지만 다른 사건이라, 줄일 때 사유를 안 적으면 다음 사람이
   *   「탐지가 죽었나」부터 의심하며 시간을 쓴다. 새 도구가 늘면 하한이 아니라 아래 짝 검사가 잡는다. */
  assert.ok(커밋하는것.length >= 4,
    `master 에 커밋하는 도구를 ${커밋하는것.length}개만 찾았다 — 탐지 정규식이 죽었다(2026-08-18 실측 4개)`);
