...
test('[회고 · 발전안 20260911 갈래 ②] ㉠ 「아니야」의 효력 — «아니에요» 고침이 붙은 시작 진단의 다음 자리는 편지에 다시 안 낸다 · 「맞아요」·칸 없음은 옛 동작', () => {
  const 고침 = (판정) => JSON.stringify({ 판정, 한줄: '', 시각: '2026-12-01T00:00:00.000Z', 다음문형: ['G502', 'G708'] });
  const 진단 = (판정) => 시트(['세션번호', '학생번호', '시작점', '표본급수', '다음문형', '학생고침'],
    [['d1', 'S1', '✓', 3, '["G502","G708"]', 판정 === null ? '' : 고침(판정)]]);
  const ml = 시트(['student_id', 'grammar_id', '상태', '첫기록일', '도달일'], [['S1', 'G502', '도달', D('2026-10-01'), D('2026-10-20')]]);
  const 잰다 = (판정) => { const { E, ss } = 엔진({ sheets: { 진단세션: 진단(판정), mastery_log: ml } }); return E.회고_알게된것_(ss, 'S1', 시즌).줄들[0].글; };
  const 부정 = 잰다('아니에요'), 긍정 = 잰다('맞아요'), 없음 = 잰다(null);
  /* 분모 — 세 판이 같은 재료에서 나왔고, 긍정·없음은 옛 동작 그대로 다음 자리를 싣는다(회귀가 새 동작만 재면 옛 동작이 깨져도 초록이다). */
  assert.ok(/시작 진단 3급/.test(부정) && /시작 진단 3급/.test(긍정) && /시작 진단 3급/.test(없음), '시작 진단 줄 자체가 안 선다');
  assert.ok(/그때 다음 자리 「/.test(긍정) && /그때 다음 자리 「/.test(없음), '「맞아요」·칸 없음에서 다음 자리가 사라졌다 — 옛 동작이 깨졌다: ' + 긍정);
  assert.ok(/다음 자리 중 1개에 닿았습니다/.test(긍정), '「맞아요」의 닿음 셈이 사라졌다: ' + 긍정);
  /* 새 동작 — «아니에요»는 다음 자리도 닿음 셈도 안 내고, 왜 안 냈는지를 한 줄로 말한다(원관측은 시트에 그대로다). */
  assert.ok(!/그때 다음 자리 「/.test(부정), '「아니에요」인데 다음 자리를 다시 냈다 — 철학이 약속한 정정의 효력이 끝까지 안 갔다: ' + 부정);
  assert.ok(!/닿았습니다/.test(부정), '「아니에요」인데 닿음을 셌다: ' + 부정);
  assert.ok(/학생이 「아니에요」라고 해서 싣지 않았습니다/.test(부정), '왜 안 실었는지를 편지가 말해야 한다: ' + 부정);
  assert.notEqual(부정, 긍정, '부정과 긍정의 편지가 같다 — 고침이 출력을 못 바꿨다');
});
