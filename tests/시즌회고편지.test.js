...
test('[회고 · 발전안 20260911 갈래 ②] ㉠ 「아니야」의 효력 — «아니에요» 고침 스냅샷의 자리만 편지에서 빠진다 · 「맞아요」·값 없음·열 없음·깨진 JSON 은 옛 동작', () => {
  const 고침 = (판정, 스냅) => JSON.stringify({ 판정, 한줄: '', 시각: '2026-12-01T00:00:00.000Z', 다음문형: 스냅 });
  const 진단 = (칸값, 열있음 = true) => 시트(
    열있음 ? ['세션번호', '학생번호', '시작점', '표본급수', '다음문형', '학생고침'] : ['세션번호', '학생번호', '시작점', '표본급수', '다음문형'],
    [열있음 ? ['d1', 'S1', '✓', 3, '["G502","G708"]', 칸값] : ['d1', 'S1', '✓', 3, '["G502","G708"]']]);
  const ml = 시트(['student_id', 'grammar_id', '상태', '첫기록일', '도달일'], [['S1', 'G502', '도달', D('2026-10-01'), D('2026-10-20')]]);
  const 잰다 = (칸값, 열있음) => { const { E, ss } = 엔진({ sheets: { 진단세션: 진단(칸값, 열있음), mastery_log: ml } }); return E.회고_알게된것_(ss, 'S1', 시즌).줄들[0].글; };
  const 전부부정 = 잰다(고침('아니에요', ['G502', 'G708']));
  const 일부부정 = 잰다(고침('아니에요', ['G502']));
  const 긍정 = 잰다(고침('맞아요', ['G502', 'G708']));
  const 값없음 = 잰다('');
  const 열없음 = 잰다(null, false);
  const 깨짐 = 잰다('{아니에요');
  /* 분모 — 다섯 판이 같은 재료에서 나왔고, 옛 동작 넷(맞아요·값 없음·열 없음·깨진 JSON)은 다음 자리와 닿음 셈을 그대로 낸다
   *   (회귀가 새 동작만 재면 옛 동작이 깨져도 초록이다). */
  for (const [이름, 줄] of [['긍정', 긍정], ['값없음', 값없음], ['열없음', 열없음], ['깨짐', 깨짐]]) {
    assert.ok(/시작 진단 3급/.test(줄), 이름 + ': 시작 진단 줄 자체가 안 선다');
    assert.ok(/그때 다음 자리 「/.test(줄), 이름 + ': 다음 자리가 사라졌다 — 옛 동작이 깨졌다: ' + 줄);
    assert.ok(/다음 자리 중 1개에 닿았습니다/.test(줄), 이름 + ': 닿음 셈이 사라졌다: ' + 줄);
    assert.ok(!/아니에요/.test(줄), 이름 + ': 부정 문장이 잘못 섰다: ' + 줄);
  }
  /* 새 동작 ① 전부 부정 — 다음 자리도 닿음 셈도 안 내고, 몇 개를 왜 안 냈는지를 말한다(원관측은 시트에 그대로다). */
  assert.ok(/시작 진단 3급/.test(전부부정), 전부부정);
  assert.ok(!/그때 다음 자리 「/.test(전부부정), '「아니에요」인데 다음 자리를 다시 냈다 — 정정의 효력이 끝까지 안 갔다: ' + 전부부정);
  assert.ok(!/닿았습니다/.test(전부부정), '「아니에요」인데 닿음을 셌다: ' + 전부부정);
  assert.ok(/다음 자리 2개는 학생이 「아니에요」라고 해서 싣지 않았습니다/.test(전부부정), '몇 개를 왜 안 실었는지 편지가 말해야 한다: ' + 전부부정);
  /* 새 동작 ② 일부 부정 — 스냅샷에 든 G502 만 빠지고 G708(-더라도)은 남는다 · 닿음 셈은 남은 자리(G708 미도달)로만 센다. */
  assert.ok(/그때 다음 자리 「-더라도」/.test(일부부정), '부정 안 한 자리까지 빠졌다 — 스냅샷이 아니라 현재 열을 통째로 비웠다: ' + 일부부정);
  assert.ok(/다음 자리 1개는 학생이 「아니에요」라고 해서 싣지 않았습니다/.test(일부부정), 일부부정);
