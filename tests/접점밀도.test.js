...
test('[접점밀도] 원장이 읽는 한 줄 — 점수와 그 점수를 만든 접점 이름이 같이 있고, 표가 비어도 빈 줄이 아니다', () => {
  ...
  assert.ok(/3점 ← 2자리 전부/.test(전부) && /⚠ 표에 2자리 — 열둘이어야 한다/.test(전부), '자리가 열둘보다 적으면 「열두 자리 전부」라 말하면 안 된다(코덱스 09-07 P1): ' + 전부);
  const 열둘 = E.접점한줄_(E.접점최저_(E.접점열둘_().map(r => [r[0], r[1], 3])));
  assert.ok(/3점 ← 열두 자리 전부/.test(열둘) && !/⚠/.test(열둘), '열둘이 다 3점일 때만 「열두 자리 전부」: ' + 열둘);
  const 열셋 = E.접점한줄_(E.접점최저_(E.접점열둘_().map(r => [r[0], r[1], 3]).concat([[13, '덤', 3]])));
  assert.ok(/3점 ← 13자리 전부/.test(열셋) && /⚠ 표에 13자리/.test(열셋), '열둘을 넘어도 「열두 자리 전부」라 말하면 안 된다(코덱스 2차): ' + 열셋);
