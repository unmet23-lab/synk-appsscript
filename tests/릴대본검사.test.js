'use strict';
/* `tools/릴대본검사.js` 회귀 — 이 자가 실제로 «무는지» 잰다.
 *
...
const { 검사, 대본재기, 편번호 } = require('../tools/릴대본검사.js');
...
/** 밀도만 재는 견본 — 훅은 성한 물음으로 깔아 둔다(훅이 없으면 자가 «못 쟀다»를 낸다). */
const 훅 = '\n**[훅 자막]** (0~2.5초)\n> 이건 뭐라고 할까요?\n\n';
...
test('훅이 물음이면 통과한다', (t) => {
  ...
  const r = 검사({ 뿌리: f.뿌리 });
  assert.equal(r.종료코드, 0);
  assert.equal(r.셈.서술훅수, 0);
});

test('🔴 훅이 서술문이면 종료코드 1 이고 그 화와 훅 글을 이름 대어 낸다', (t) => {
  const f = 픽스처(t);
  f.쓰기('01_견본.md', `## 1편 — 가\n\n**[훅 자막]** (0~2.5초)\n> 채팅에서는 두 글자로.\n\n${대사(80)}`);
  const r = 검사({ 뿌리: f.뿌리 });
  assert.equal(r.종료코드, 1, '답을 미리 말하는 훅은 궁금할 자리를 없앤다');
  assert.equal(r.셈.서술훅수, 1);
  assert.ok(r.출력.some((줄) => 줄.includes('01편 1화') && 줄.includes('두 글자로')));
});

test('🔴 훅 자막을 못 찾으면 종료코드 2 다 — 「서술 훅 0」이 아니다', (t) => {
  const f = 픽스처(t);
  f.쓰기('01_견본.md', `## 1편 — 가\n${대사(80)}`);
  const r = 검사({ 뿌리: f.뿌리 });
  assert.equal(r.종료코드, 2, '훅이 «없는» 것과 «서술문인» 것은 다른 병이다');
  assert.equal(r.셈, null);
});

test('밀도는 성한데 훅만 서술문이어도 잡는다 — 두 자가 따로 문다', (t) => {
  const f = 픽스처(t);
  f.쓰기('01_견본.md', `## 1편 — 가\n\n**[훅 자막]** (0~2.5초)\n> 그냥 서술문.\n\n${대사(40)}`);
  const r = 검사({ 뿌리: f.뿌리 });
  assert.equal(r.셈.굵은화수, 0, '밀도는 성하다');
  assert.equal(r.셈.서술훅수, 1);
  assert.equal(r.종료코드, 1);
});

test('저장소 실물 대본 아홉 벌이 밀도·훅 둘 다 성하다', () => {
  const r = 검사({});
  assert.equal(r.종료코드, 0, `흠이 있다:\n${r.출력.join('\n')}`);
