...
test('🔴 보드 줄이 동점이면 뒤쪽(내 줄)을 집는다 — 남의 선언을 내 것이라 말하지 않는다', (t) => {
  if (!있나) return t.skip('git 없음');
  /* 신설 당일 라이브 오탐: 남의 줄과 내 줄이 1:1 동점이었는데 **앞줄(남의 것)** 이 이겨서
   * 내가 선언한 적 없는 파일을 「내가 선언한」이라고 보고했다. 보드는 위쪽이 오래된 트랙이라
   * 먼저 만난 줄을 계속 쥐면 방향이 정반대다. 세션은 자기 줄을 끝에 붙인다. */
  const dir = 픽스처저장소();
  const st = 상태폴더();
  fs.writeFileSync(path.join(dir, 'docs', '세션보드.md'),
    '| 날짜 | 트랙 | 만지는 파일 | 상태 |\n|---|---|---|---|\n'
    + '| 2026-08-04 | 남의 트랙 | **Code.js·남의전용.js** | 진행중 |\n'
    + '| 2026-08-04 | 내 트랙 | **Code.js·내전용.js** | 진행중 |\n');
  커밋(dir, 'Code.js', 'a', 'init', 'sess-B');
  ...
  const r = 훅(dir, { file: 'Code.js', stateDir: st });
  assert.equal(r.조용, true,
    `앞줄(남의 줄)을 내 줄로 집어 남의 선언 파일을 내 것이라 보고했다: ${r.요약}`);
});
