...
const { engineSource, ENGINE_FILES } = require('./_engine-source');
 ...
 *   🔴 ⚰`tests/시트흉내계약.test.js` 의 변이 픽스처가 양방향으로 물었다 — ⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다.
 *   **그 다섯 계약을 지금 무는 자가 없다.**
...
test('🔴 반 키 접기가 «한 곳»에서만 산다 — 호출부마다 손으로 적으면 새로 난 자리가 그 통로를 안 탄다', () => {
   ...
   * [v9.238·#Q79] 과녁을 **엔진 전량**으로 넓혔다 — 옛 판은 `엔진_셋업확장.js` 한 파일만 잠갔고,
   *   그동안 `엔진_운영배치.js` 가 같은 접기를 두 곳에 손으로 들고 있었다(지금은 `반키_` 를 부른다).
   *   파일 목록은 `_engine-source.js` 하나에서 파생시킨다 — 여기 손으로 적으면 새 엔진 파일이 생긴 날
   *   그 파일만 검사 «밖»이 되고, 빠진 쪽은 언제나 «통과»로 샌다.
   * 🔑 탐지력은 픽스처로 못박는다(실저장소는 이미 초록이라 그것만으로는 아무것도 안 잰다 — 가드 맹점 ②).
   *   표기 폭도 여기서 넓힌다: 사람은 겹따옴표로도, 사이에 공백을 넣고도 적는다(맹점 ①). */
  const 접기 = /\.split\(\s*(['"])\(\1\s*\)\s*\[\s*0\s*\]/g;
  const 센다 = (src) => (코드만(src).match(접기) || []).length;

  // ① 픽스처 — 이 규칙이 실제로 «잡는지»를 여기서 잰다
  assert.equal(센다("const bare = raw.split('(')[0].trim();"), 1, '홑따옴표 표기를 못 잡는다');
  assert.equal(센다('const bare = raw.split("(")[0].trim();'), 1, '겹따옴표 표기를 못 잡는다 — 사람은 이렇게도 적는다');
  assert.equal(센다("const bare = raw.split( '(' ) [ 0 ];"), 1, '공백이 낀 표기를 못 잡는다');
  assert.equal(센다("const bare = 반키_(raw);"), 0, '공용 통로를 부르는 코드를 위반으로 센다 — 거짓양성');
  assert.equal(센다("const 조 = raw.split(',')[0];"), 0, '괄호가 아닌 구분자까지 센다 — 거짓양성');
  assert.equal(센다("/* 옛 판은 raw.split('(')[0] 였다 */"), 0, '주석 안의 서술을 위반으로 센다 — 문서를 못 쓰게 만든다');

  // ② 실저장소 — 엔진 «전량»에서 딱 1곳(반키_ 정의)이어야 한다
  const 파일별 = ENGINE_FILES.map((f) => ({ f, n: 센다(fs.readFileSync(path.join(ROOT, f), 'utf8')) }));
  const 합 = 파일별.reduce((a, x) => a + x.n, 0);
  assert.equal(합, 1,
    `엔진 전량에 «(» 접기가 ${합}곳이다 — 반키_ 정의 1곳이어야 하고 호출부는 반키_ 를 부른다: `
    + JSON.stringify(파일별.filter((x) => x.n)));
  assert.equal(파일별.filter((x) => x.n)[0].f, '엔진_셋업확장.js',
    '그 1곳이 반키_ 가 사는 파일이 아니다 — 정의가 옮겨갔거나 엉뚱한 자리가 잡혔다');
  // 그 1곳이 «반키_ 본문 안»인지까지 못박는다 — 같은 파일 다른 함수로 새 나가면 위 두 단언은 초록이다
  assert.equal(센다(section('function 반키_(', '/* [v9.237·검수 c59f24d9]')), 1,
    '유일한 접기가 반키_ 본문 밖에 있다 — 정의는 통로를 잃고 다른 자리가 손으로 접고 있다');
});
...
/* [vNEXT] #Q99 — 헤더·탭 이름은 손으로 안 적는다. 정본에서 파생시켜야 열이 늘거나 이름이 바뀔 때
 *   이 시험이 «함께» 움직인다(`GROUPS_HEADERS` 와 같은 규약 — 손사본은 갈라지고, 갈라진 쪽의
 *   증상은 언제나 「통과」다). */
const TALK_INDEX_LOG_HEADERS = new Function(
  `${section('const TALK_INDEX_LOG_HEADERS = [', '\n')}\nreturn TALK_INDEX_LOG_HEADERS;`)();
const TALK_INDEX_LOG_SHEET = new Function(
  `${section('const TALK_INDEX_LOG_SHEET = ', '\n')}\nreturn TALK_INDEX_LOG_SHEET;`)();

/* 반들 = profiles E열에 그대로 들어갈 «표시명» 목록(학생 1명씩). groups 시트는 새로 안 짓고
 *   `fakeLogSheet()` 를 쓴다 — 아포스트로피 소비·Date 삼킴·텍스트 서식이 이미 정직하게 들어 있다.
 * [vNEXT] #Q99 — 침묵 축의 **두 원천**을 둘 다 꽂을 수 있게 넓혔다(`옵션.지수행들`·`옵션.마감폼행들`).
 *   안 주면 종전 그대로 둘 다 없음 → `발화지수침묵맵_` null → `quietScoreMap_` 빈 맵이라
 *   위쪽 옛 검사들의 뜻이 하나도 안 바뀐다(리그를 새로 짓지 않는 이유 · 통로는 하나). */
function 배정Rig(반들, 옵션) {
  ...
  const o = 옵션 || {};
  /* 헤더까지 담은 통짜 격자(첫행: 1) — `발화지수침묵맵_` 이 1행을 «열 이름»으로 읽기 때문이다.
   *   헤더를 빼면 위치로 읽는 구현도 통과해 버려, 이 시험이 정작 재려는 것을 못 잰다. */
  const 지수시트 = o.지수행들 ? 시트흉내({ 첫행: 1, 행들: [(o.지수헤더 || TALK_INDEX_LOG_HEADERS).slice()].concat(o.지수행들) }) : null;
  const 마감시트 = o.마감폼행들 ? 시트흉내({ 첫행: 1, 행들: [['일시', '미발화자']].concat(o.마감폼행들) }) : null;
  const rig = {
    groups,
    산행: () => groups.data.filter((r) => String(r[0] || '') !== ''),   // clearContent 로 비운 꼬리는 뺀다
    /* 학생별 `why`(groups I열) — 강사가 편성표에서 실제로 읽는 칸이다. */
    사유: () => { const m = {}; rig.산행().forEach((r) => { m[String(r[2])] = String(r[8] || ''); }); return m; },
    ss: {
      getSpreadsheetTimeZone: () => 'Asia/Ulaanbaatar',
      // 안 꽂으면 둘 다 null → 지수 없음 + quietScoreMap_ 빈 맵(옛 검사들의 전제 그대로)
      getSheetByName: (n) => (n === 'profiles' ? pf
        : n === TALK_INDEX_LOG_SHEET ? 지수시트
        : n === 'lesson_close' ? 마감시트 : null)
    },
    stub: {
...
/* ── [vNEXT] #Q99 침묵 축을 «원신호 → 요약» 으로 갈아탄다 ──────────────────────────────
 * `talk_index_log` 는 시트층 수집 9종 중 **도달 0 이던 5종**의 하나였다(설계 §8). 읽는 코드는
 * 있었지만(주간 리포트 꼬리) 읽어서 **보여줄 뿐**이라, 이 층 고유의 함정 —「사람 화면을 도달로
 * 세는 것」— 의 표본이었다. 여기서 그 요약이 처음으로 «다음 조»를 바꾼다.
 *
 * 🔑 **이 묶음이 장부의 천장을 넘는 자리다.** `tools/lib/시트도달.js` 가 재는 것은 「지목한 파일의
 *   코드에 탭 이름이 실재하는가」까지다(설계 §8 「⚠ 천장」 — 소비자를 적어 두고 읽는 몸을 비워도
 *   그 장부는 초록이다). 아래는 **실제로 태워서** 편성표에 찍히는 이름이 바뀌는지를 잰다.
 * ⚠ 「지수를 쓴다」를 강제하지 않는다 — 폴백은 결함이 아니라 설계다(지수 행이 없는 시즌 첫 주에
 *   침묵 축을 통째로 잃는 것이 더 나쁘다). 재는 것은 **어느 축으로 갔는지 말하는가** 쪽이다. */

/* 시즌 라벨은 손으로 안 적는다 — 실물 `seasonLabelOf_` 가 내는 값을 한 번 받아 쓴다. 박아 두면
 *   시즌 규칙이 바뀌는 날 이 시험만 조용히 「지수 행 없음」으로 흘러 **폴백을 재고 초록**이 된다. */
const 시즌라벨 = (() => {
  const r = 배정Rig(['정규반1']);
  loadAssign()(r.stub)('정규반1');
  return String(r.산행()[0][0]);
})();

/** 지수 한 행 — 열은 이름으로 채운다(위치를 박으면 헤더가 바뀔 때 이 시험이 먼저 거짓말한다). */
const 지수행 = (주차, 반, sid, pct, 시즌) => {
  const r = new Array(TALK_INDEX_LOG_HEADERS.length).fill('');
  const 넣기 = (이름, v) => { r[TALK_INDEX_LOG_HEADERS.indexOf(이름)] = v; };
  넣기('logged_at', new Date(2027, 0, 11));
  넣기('season', 시즌 === undefined ? 시즌라벨 : 시즌);
  넣기('week', 주차); 넣기('class', 반); 넣기('student_id', sid); 넣기('name', sid);
  넣기('got', 0); 넣기('max', 10); 넣기('quiet', 0); 넣기('pct', pct);
  return r;
};
const 여덟반 = () => new Array(8).fill('정규반1');
const 지목된 = (사유) => Object.keys(사유).filter((s) => /침묵|발화 지수/.test(사유[s])).sort();

test('🔑 #Q99 — 축이 «요약»으로 갈아탄다: 같은 반인데 편성표가 짚는 학생이 달라진다(도달의 실체)', () => {
  /* 원신호(마감폼)는 S1·S2 를 조용하다 하고, 주간 지수는 S7·S8 을 조용하다 한다. 갈아타기가
   *   실재하면 **강사가 읽는 칸의 이름이 바뀐다** — 이것이 「읽는 코드가 있다」와 「다음에 줄 것이
   *   바뀐다」를 가르는 자리다(설계 §5). 지수가 이 둘을 짚는 근거는 마감폼에 한 줄도 안 남는
   *   원인 둘(결석으로 역할을 통째로 못 받음 · 3인 조라 '질문'이 없음)이다. */
  const 마감 = [[new Date(2027, 0, 5), 'S1,S2'], [new Date(2027, 0, 6), 'S1,S2']];

  const 원 = 배정Rig(여덟반(), { 마감폼행들: 마감 });
  loadAssign()(원.stub)('정규반1');

  const 요 = 배정Rig(여덟반(), {
    마감폼행들: 마감,                       // 🔑 원신호는 그대로 둔다 — 바뀐 것은 «원천» 하나뿐(F045)
    지수행들: ['S7', 'S8'].map((s) => 지수행(3, '정규반1', s, 0))
      .concat(['S1', 'S2', 'S3', 'S4', 'S5', 'S6'].map((s) => 지수행(3, '정규반1', s, 100)))
  });
  loadAssign()(요.stub)('정규반1');

  assert.deepEqual(지목된(원.사유()), ['S1', 'S2'],
    '원신호 판이 마감폼 미발화자를 안 짚었다 — 기준선이 없으면 아래 비교가 무의미하다: ' + JSON.stringify(원.사유()));
  assert.deepEqual(지목된(요.사유()), ['S7', 'S8'],
    '주간 지수를 꽂았는데 지목이 안 바뀌었다 — 요약이 편성에 «안 닿았다»(도달 0 그대로): ' + JSON.stringify(요.사유()));
});

test('🔴 #Q99 — 방향을 뒤집어 넘긴다(pct 는 클수록 «많이 말했다») · 라벨이 단위를 밝힌다', () => {
  /* 안 뒤집으면 **가장 말을 많이 한 학생이 「조용한 학생」으로 묶인다** — 편성이 정확히 거꾸로 간다.
   *   그리고 라벨을 안 갈면 편성표에 「침묵 60회」가 찍힌다(맞는 얼굴로 틀린 값 · 맹점④). */
  const rig = 배정Rig(여덟반(), {
    지수행들: [지수행(2, '정규반1', 'S1', 100), 지수행(2, '정규반1', 'S2', 40)]
      .concat(['S3', 'S4', 'S5', 'S6', 'S7', 'S8'].map((s) => 지수행(2, '정규반1', s, 95)))
  });
  loadAssign()(rig.stub)('정규반1');
  const 사유 = rig.사유();

  assert.ok(!/침묵|발화 지수/.test(사유.S1),
    'pct 100(전부 말했다)인데 조용한 학생으로 짚였다 — 방향을 안 뒤집었다: ' + 사유.S1);
  assert.match(사유.S2, /발화 지수 40%/,
    'pct 를 라벨로 되돌리지 못한다(왕복이 깨졌다): ' + 사유.S2);
  assert.ok(!/침묵 \d+회/.test(사유.S2),
    '지수 판인데 단위가 「회」다 — 눈금(0~100 %)과 말이 갈렸다: ' + 사유.S2);
});

test('🔴 #Q99 — 값이 시즌 «누적»이라 최신 주차만 본다(섞으면 편성이 몇 주 전 사진으로 굳는다)', () => {
  const 전부 = ['S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7', 'S8'];
  const rig = 배정Rig(여덟반(), {
    지수행들: 전부.map((s) => 지수행(1, '정규반1', s, s === 'S1' ? 0 : 100))
      .concat(전부.map((s) => 지수행(4, '정규반1', s, s === 'S8' ? 0 : 100)))
  });
  loadAssign()(rig.stub)('정규반1');
  assert.deepEqual(지목된(rig.사유()), ['S8'],
    '1주차 행이 4주차를 이겼거나 둘이 섞였다 — 같은 학생이 두 번 세진다: ' + JSON.stringify(rig.사유()));
});

test('🔴 #Q99 — 남의 시즌·남의 반은 안 섞이고, 반 키는 접어서 맞춘다', () => {
  /* 반 키를 안 접으면(「정규반2(9시)」 ↔ 「정규반2」) 그 반이 통째로 «미측정»으로 흘러, 지수가
   *   있는데도 조용히 폴백이 된다 — v9.235 가 적재 쪽에서 이미 물린 함정의 읽기 쪽 짝이다. */
  const rig = 배정Rig(['정규반2(9시)', '정규반2', '평일11A', '정규반2'], {
    지수행들: [
      지수행(2, '정규반2', 'S1', 0),                  // 접힌 키 — 적재기(`talkIndexSnapshot_`)가 내는 모양
      지수행(2, '정규반2(9시)', 'S2', 0),             // 표시명 — 손으로 만진 행. 접어야 맞는다
      지수행(2, '평일11A', 'S3', 0),                  // 남의 반
      지수행(9, '정규반2', 'S4', 0, '2099-겨울')      // 남의 시즌 — 주차가 더 커서, 섞이면 이쪽이 이긴다
    ]
  });
  loadAssign()(rig.stub)('정규반2(9시)');
  const 사유 = rig.사유();

  assert.match(사유.S1 || '', /발화 지수 0%/,
    '적재기가 내는 접힌 키가 안 맞았다 — 그 반이 통째로 미측정이 된다: ' + JSON.stringify(사유));
  assert.match(사유.S2 || '', /발화 지수 0%/,
    '표시명으로 적힌 행을 안 접었다 — 같은 판정을 두 곳에 다르게 적으면 갈라진다(v9.235): ' + JSON.stringify(사유));
  assert.ok(!/침묵|발화 지수/.test(사유.S4 || ''),
    '남의 시즌 행이 섞였다 — 지난 시즌의 조용함이 이번 편성을 잠식한다(v9.125 가 원신호에서 닫은 그 축): ' + JSON.stringify(사유));
  assert.ok(!('S3' in 사유), '남의 반 학생이 이 편성에 들어왔다: ' + JSON.stringify(사유));
});

test('🔑 #Q99 — 0 은 분모와 함께 읽는다: 잰 명수·미측정 명수를 반환 문구가 말한다(유호 확정 08-14)', () => {
  /* 이 줄이 없으면 「발화 지수로 편성했다」가 **잰 사람 0명이어도 참**이 된다 — 좋은 0과
   *   「안 재봤다」가 한 얼굴이 되는 자리다. 미측정을 0 으로 주는 것은 의도된 보수 처리이고
   *   (안 재본 사람을 흩지 않는다), 그래서 몇 명이 그랬는지는 반드시 드러나야 한다. */
  const rig = 배정Rig(여덟반(), { 지수행들: ['S1', 'S2', 'S3'].map((s) => 지수행(2, '정규반1', s, 50)) });
  const out = loadAssign()(rig.stub)('정규반1');

  assert.match(out, /발화 지수 2주차/, '어느 축·어느 주차로 편성했는지를 안 말한다: ' + out);
  assert.match(out, /8명 = 잰 3명 \+ 미측정 5명/, '분모를 안 밝혔다: ' + out);
});

test('🔑 #Q99 — 지수 행이 없으면 원신호로 돌아가고 «그 사실을 말한다»(축을 통째로 잃지 않는다)', () => {
  const rig = 배정Rig(여덟반(), { 마감폼행들: [[new Date(2027, 0, 5), 'S1']] });
  const out = loadAssign()(rig.stub)('정규반1');

  assert.match(out, /마감폼 원신호/,
    '폴백이 조용하다 — 강사는 어느 축으로 묶였는지 모르고, 「지수로 편성했다」와 구분이 안 된다: ' + out);
  assert.match(rig.사유().S1 || '', /침묵 1회/, '폴백인데 라벨이 지수 단위다 — 눈금과 말이 갈렸다');
});

test('🔴 #Q99 — 못 읽는 pct 칸은 «미측정»이다: 뒤집기 때문에 0 으로 읽으면 「최대로 조용함」이 된다', () => {
  /* 부호가 반대라서 생기는 자리다. 원신호는 값이 없으면 0 = 「조용하지 않다」(보수)인데,
   *   `Number('') === 0` 을 그대로 뒤집으면 `100 - 0` = **가장 조용한 학생**이 된다 — 안 잰
   *   사람이 재본 사람을 QUIET 자리에서 밀어낸다. 빈 칸·글자 둘 다 잰다. */
  const 빈칸 = 지수행(2, '정규반1', 'S5', 0); 빈칸[TALK_INDEX_LOG_HEADERS.indexOf('pct')] = '';
  const 글자 = 지수행(2, '정규반1', 'S6', 0); 글자[TALK_INDEX_LOG_HEADERS.indexOf('pct')] = '—';
  const rig = 배정Rig(여덟반(), {
    지수행들: [빈칸, 글자, 지수행(2, '정규반1', 'S7', 10)]
      .concat(['S1', 'S2', 'S3', 'S4', 'S8'].map((s) => 지수행(2, '정규반1', s, 100)))
  });
  const out = loadAssign()(rig.stub)('정규반1');

  assert.deepEqual(지목된(rig.사유()), ['S7'],
    '못 읽는 칸이 「최대로 조용함」으로 들어갔다 — 안 잰 학생이 재본 학생을 밀어낸다: ' + JSON.stringify(rig.사유()));
  /* 🔑 **분모로도 잰다.** 숫자가 아닌 칸(`'—'`)은 편성 결과로는 조용하다 — `NaN` 이 호출부의
   *   `|| 0` 에 삼켜져 미측정과 같은 자리에 떨어지기 때문이다. 그래서 그 축만 보면 가드가 있으나
   *   없으나 초록이고(변이가 실제로 그렇게 났다), 갈리는 것은 **몇 명을 쟀다고 말하는가** 쪽이다:
   *   맵에 `NaN` 을 넣으면 그 학생이 「잰 사람」으로 세어져 분모가 부풀고, 0 은 분모와 함께
   *   읽는다는 규약이 그 자리에서 거짓이 된다(유호 확정 08-14). */
  assert.match(out, /8명 = 잰 6명 \+ 미측정 2명/,
    '못 읽는 칸 둘(빈 칸·「—」)이 「잰 사람」으로 세어졌다 — 분모가 부풀었다: ' + out);
});

test('🔴 #Q99 — 열은 «이름»으로 찾는다: 헤더가 낡으면 조용히 틀린 값을 내지 말고 원신호로 돌아간다', () => {
  /* 위치로 읽는 구현은 열이 하나만 늘거나 밀려도 «맞는 얼굴로 틀린 값»을 낸다(v9.119 가 같은
   *   자리에서 두 번 물린 함정). pct 열 이름만 바꿔 두면 지목이 사라지고 폴백이 서야 한다. */
  const rig = 배정Rig(여덟반(), {
    지수헤더: TALK_INDEX_LOG_HEADERS.map((h) => (h === 'pct' ? '비율' : h)),
    지수행들: [지수행(2, '정규반1', 'S1', 0)],
    마감폼행들: [[new Date(2027, 0, 5), 'S4']]
  });
  const out = loadAssign()(rig.stub)('정규반1');

  assert.match(out, /마감폼 원신호/, '헤더가 낡았는데 지수를 읽었다고 한다 — 위치로 읽고 있다: ' + out);
  assert.match(rig.사유().S4 || '', /침묵 1회/, '폴백이 안 섰다: ' + JSON.stringify(rig.사유()));
});
