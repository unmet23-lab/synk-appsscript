...
test('로고 주입기의 주입 정본이 키트 색이다 — 여기가 낡으면 주입 한 번에 전 파일로 번진다', () => {
  ...
  assert.ok(src.includes('#E4E4E7') && src.includes('#F96859'),
    'W5 주입 정본에 Chalk(#E4E4E7)·Coral(#F96859 · 조항 ⓙ)이 없다 — 브랜드킷 §3 v2.0 도형 정본과 어긋난다');
});
...
test('코랄 면 위 글자는 Ink 다 — 흰 글자는 2.8 로 미달한다', () => {
  ...
  assert.ok(/color:#2B2320/i.test(rec[0]), `.id.rec 글자가 Ink 가 아니다 — ${rec[0]}`);
});
