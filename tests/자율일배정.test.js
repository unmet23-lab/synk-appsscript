...
test('1차시(문형없음) 목표는 요목 열쇠 · 굳히기는 COHORT1_STYLE_ITEMS 가 채워져야 나온다', () => {
  const 낭답 = { 낭독: [{ 문장: 'a' }, { 문장: 'b' }], 답하기: [{ 물음: 'a' }, { 물음: 'b' }] };
  const b0 = C.자율일묶음_(주(1), 'S1', 낭답);
  같다(b0.목표, ['요목:말투두벌']);
  assert.match(b0.공급상태, /굳히기 0\/8/, '굳히기 재료가 없으면 «공급실패:굳히기» 로 남는다 — 다른 문항으로 조용히 메우지 않는다');
  // 재료 생성기(`자율일재료생성_` 문형없음 갈래)가 하는 옮김을 그대로 재현한다 — 1차시는 AI 를 안 부르고 이 표를 쓴다.
  const 굳히기문항 = C.COHORT1_STYLE_ITEMS.map((q) => ({ 문장: q[0], 보기: q[1], 정답: q[2], 판: 'c1:' + C.COHORT1_WEEKS_VER, 목표: '요목:말투두벌' }));
  assert.equal(굳히기문항.length, C.COHORT1_BUNDLE_SIZE.굳히기, '말투 문항 수가 굳히기 정원과 다르다 — 표를 고쳤으면 정원도 같이 본다');
  const b = C.자율일묶음_(주(1), 'S1', Object.assign({ 굳히기문항: 굳히기문항 }, 낭답));
  assert.equal(종류수(b).굳히기, C.COHORT1_BUNDLE_SIZE.굳히기);
  assert.equal(b.공급상태, '정상', '문항이 다 있는데 공급실패가 남았다');
});

test('말투 두 벌 문항 여덟 — 빈칸 하나 · 보기 넷 · 정답 자리는 0~3 에 흩어진다(찍어서 못 맞힌다)', () => {
  const 자리 = {};
  C.COHORT1_STYLE_ITEMS.forEach((q, i) => {
    const [문장, 보기, 정답] = q;
    assert.equal(q.length, 3, `${i}번 문항의 칸이 셋이 아니다`);
    assert.equal((문장.match(/___/g) || []).length, 1, `${i}번 문항의 빈칸이 하나가 아니다`);
    assert.equal(보기.length, 4, `${i}번 보기가 넷이 아니다`);
    assert.equal(new Set(보기).size, 4, `${i}번 보기에 같은 것이 둘 있다 — 답이 둘로 읽힌다`);
    assert.ok(Number.isInteger(정답) && 정답 >= 0 && 정답 <= 3, `${i}번 정답 자리가 0~3 정수가 아니다`);
    보기.forEach((o) => assert.ok(String(o).trim().length > 0, `${i}번 보기에 빈 것이 있다`));
    자리[정답] = (자리[정답] || 0) + 1;
  });
  [0, 1, 2, 3].forEach((n) => assert.ok(자리[n] >= 1, `정답이 한 번도 ${n}번 자리에 안 온다 — 자리로 찍힌다`));
});
...
test('필수진단 주 — 진단 항목이 «필수»로 든다 · 8차시 전체복습 목표는 그 학생이 놓친 문형 셋(없으면 열둘 순환)', () => {
  ...
  assert.equal(진단.length, 1); assert.equal(진단[0].역할, '중간', '진단은 «역할 이름»으로 — 숫자 회차는 실물(0·1·2)과 어긋난다');
  const w8 = 주(8);
  const 재료8 = { 굳히기문항: [0, 1, 2, 3, 4, 5, 6, 7].map((i) => 문항(i)), 낭독: [{ 문장: 'a' }, { 문장: 'b' }], 답하기: [{ 물음: 'a' }, { 물음: 'b' }], 전체복습문형: ['G724', 'G705'] };
  const b8 = C.자율일묶음_(w8, 'S1', 재료8);
  같다(b8.목표, ['G724', 'G705']);
  assert.equal(b8.항목.filter((a) => a.종류 === '진단')[0].역할, '끝');
...
/* ── 심문 2회차 — 발행 뒤 불변 · 문항 거르기 ─────────────────────────────── */
test('발행 뒤 불변 — 항목이 실린 묶음은 다시 짓지 않고, 항목 0 인 행(공급실패:전부)만 다시 짓는다', () => {
  assert.equal(C.자율일다시지을까_(null), true);
  assert.equal(C.자율일다시지을까_({ 항목: [], 공급상태: '공급실패:전부' }), true);
  assert.equal(C.자율일다시지을까_({ 항목: [{ 항목ID: 'x' }], 공급상태: '정상' }), false, '한 번 실린 묶음을 갈아 끼우면 오프라인에서 먼저 푼 제출이 다른 문항에 붙는다');
});

test('굳히기 거르기 — 빈칸 하나 · 보기 넷 · 보기 중복 0 · 진단 문장 배제 · 같은 문장 둘은 하나 · 검문 false 는 뺀다', () => {
  const ok = { 문장: '밥을 ___ 가요.', 보기: ['먹고', '먹어서', '먹지만', '먹으니'], 정답: 0 };
  const 진단같음 = { 문장: '밥을 먹___ 텔레비전을 봐요.', 보기: ['a', 'b', 'c', 'd'], 정답: 1 };
  const 빈칸둘 = { 문장: '___ 과 ___', 보기: ['a', 'b', 'c', 'd'], 정답: 0 };
  const 중복보기 = { 문장: '물을 ___ 요.', 보기: ['a', 'a', 'c', 'd'], 정답: 0 };
  const 같은문장 = { 문장: '밥을  ___ 가요.', 보기: ['a', 'b', 'c', 'd'], 정답: 2 };
  const 전부 = [ok, 진단같음, 빈칸둘, 중복보기, 같은문장];
  const r = C.자율일문항거르기_(전부, ['밥을 먹___ 텔레비전을 봐요.'], 전부.map(() => true));
  같다(r.map((q) => q.문장), [ok.문장]);
  assert.equal(C.자율일문항거르기_([ok], [], [false]).length, 0, '검문이 아니라고 하면 뺀다');
  같다(C.자율일문항거르기_([{ ...ok, 정답: 1 }, ok], [], [false, true]).map((q) => q.정답), [0], '검문에 떨어진 같은 문장이 멀쩡한 후보까지 지우지 않는다(P2 ae3d659c03c3)');
  assert.equal(C.자율일문항거르기_([{ ...ok, 정답: '0' }, { ...ok, 정답: 0.5 }, { ...ok, 정답: null }], [], [true, true, true]).length, 0, '정답 자리는 정수 0~3 만');
  const b = C.자율일묶음_(주(2), 'S1', 가득(주(2)));
  assert.ok(b.항목.filter((a) => a.종류 === '오답').every((a) => a.원오답 && a.원오답.퀴즈ID), '원오답 고리 = 퀴즈ID(행 번호·사건 ID 가 아니다)');
  assert.equal(C.자율일문항거르기_([ok], [], null).length, 1, '검문 배열이 없으면 거르기만 한다(호출부가 실패 때 전부 false 를 준다)');
  const src = 읽기('엔진_자율일.js').replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/.*$/gm, '');
  assert.match(src, /검문 \|\| 초벌\.map\(function \(\) \{ return false; \}\)/, '검문 호출이 실패했을 때 굳히기가 통째로 빠지지 않는다 — 조용히 통과한다');
});

/* ── 1기 숙제 완료자 = 그 주 앱 제출자 전원 (유호 확정 09-07 ⓑ) ───────────────── */
test('주간 창은 그 토요일의 월~토 · 창 안에 하루라도 낸 학생만 완료자 · 같은 학생은 한 번', () => {
  같다(C.자율일주간창_('2026-12-12'), { 부터: '2026-12-07', 까지: '2026-12-12' });
  const 학생 = [{ sid: 'S1' }, { sid: 'S2' }, { sid: 'S3' }];
  const 낸 = { S1: ['2026-12-06', '2026-12-08'], S2: ['2026-12-06'], S3: ['2026-12-12', '2026-12-12'] };
  같다(C.자율일완료자_(학생, 낸)(C.자율일주간창_('2026-12-12')), ['S1', 'S3'], '일요일(12-06)에만 낸 S2 는 다음 주 몫이 아니라 지난주 몫');
  같다(C.자율일완료자_(학생, {})(C.자율일주간창_('2026-12-12')), []);
  const 셋업 = 읽기('엔진_셋업확장.js');
  const n = 셋업.indexOf('function nightJobs('), a = 셋업.indexOf("safeRun('cohort1HwBatch', cohort1HwBatchWeekly_)", n), b = 셋업.indexOf("safeRun('expandHwBatch', expandHwBatch)", n);
  assert.ok(a > n && b > a, '1기 완료자 행이 expandHwBatch 보다 «앞»에 걸려 있어야 그날 밤 5점이 나간다');
});
