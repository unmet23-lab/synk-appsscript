...
test('Ⓒ 🔴 태그의 이름은 escHtml_ 를 거친다 — 안 거치면 학생 이름이 밖으로 나간다', () => {
   ...
   * 09-03 보안 검토가 실제로 잡은 결함이고, 같은 폴더에 굽는 서클 인쇄물은 이미 전량을 감싼다. */
  const i = 코드정제.indexOf('function jacketPrintTags_');
  assert.notEqual(i, -1);
  const 태그 = 코드정제.slice(i, i + 3000);
  assert.ok(/escHtml_\(p\.이름\)/.test(태그), '이름이 이스케이프 없이 HTML 에 들어간다');
