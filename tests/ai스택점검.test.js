...
test('🔴 「돈」 열쇠가 죽으면 적색이고, 거기 매달린 것을 이름으로 부른다 — 글은 멀쩡하다고 갈라 말한다', () => {
  const a = 키생존축({
    글: 산것('글'),
    돈: 죽은것('돈', { 상태: 429, 종류: '한도·결제', 사유: 'prepayment credits are depleted' }),
  });
  const 전문 = a.적색.join('\n');
  assert.ok(a.적색.length > 0, '죽었는데 조용하면 그게 09-02 의 그 사고다');
  assert.match(전문, /「돈」 열쇠/);
  assert.match(전문, /그림 굽기|Lyria/, '무엇이 멈추는지를 그 줄에서 알아야 한다');
  assert.match(전문, /prepayment credits/, '구글이 한 말을 그대로 물고 와야 유호님이 어느 화면을 열지 안다');
  assert.match(전문, /크레딧 충전/, '돈 열쇠의 처방은 충전이다');
