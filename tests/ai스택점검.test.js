 ...
 *   돈 열쇠에 매달린 것은 그림 굽기·음성 실측 둘뿐이고 **자동으로 도는 것이 하나도 없다**
 *   (사람이 그 도구를 부를 때만 쓴다). 안 쓰기로 한 자리가 매 세션 빨갛게 울면 그게 모래주머니고,
 *   그러면 진짜 적색까지 안 읽힌다. 그래서 돈은 «알림», 글은 «적색»이다. */
test('🔴 「돈」 열쇠가 죽으면 **알림**이다 — 지금 멈추는 것이 없으므로 적색으로 울지 않는다', () => {
  const a = 키생존축({
    글: 산것('글'),
    돈: 죽은것('돈', { 상태: 429, 종류: '한도·결제', 사유: 'prepayment credits are depleted' }),
  });
  assert.strictEqual(a.적색.length, 0, '안 쓰는 자리가 적색으로 울면 진짜 적색이 파묻힌다');
  const 전문 = a.알림.join('\n');
  assert.ok(a.알림.length > 0, '죽었는데 아무 말도 안 하면 그게 09-02 의 그 사고다');
  assert.match(전문, /「돈」 열쇠/);
  assert.match(전문, /그림 굽기|음성/, '무엇이 매달렸는지를 그 줄에서 알아야 한다');
  ...
  assert.doesNotMatch(전문, /몽골어 검문/, '🔑 글이 살아 있는데 글까지 멈춘 것처럼 말하면 안 된다');
  assert.strictEqual(a.셈.돈열쇠, '**아니오**');
  assert.strictEqual(a.셈.글열쇠, '예');
});

test('🔴 「글」 열쇠가 죽으면 처방이 다르다 — 충전이 아니라 «공짜 열쇠를 새로 만든다»', () => {
  const a = 키생존축({ 글: 죽은것('글', { 상태: 429, 종류: '한도·결제', 사유: 'quota' }), 돈: 산것('돈') });
  const 전문 = a.적색.join('\n');
  assert.match(전문, /몽골어 검문/);
  assert.match(전문, /공짜 몫 열쇠를 새 칸에서/, '글은 공짜라 충전을 권하면 안 된다');
  assert.doesNotMatch(전문, /크레딧 충전/);
});
...
test('🟠 서버 쪽 5xx 는 열쇠 죽음이 아니다 — 알림으로 내리고 처방은 «잠시 뒤 다시»뿐이다', () => {
  const a = 키생존축({ 글: 죽은것('글', { 상태: 503, 종류: '서버·일시', 사유: 'This model is currently experiencing high demand.' }), 돈: 산것('돈') });
  ...
  assert.strictEqual(a.셈.글열쇠, '**아니오**', '셈 칸은 «답 못 받았다»를 그대로 둔다');
});

test('자격(401·403)이면 처방이 또 다르다 — 충전도 새 칸도 아니고 키 교체다', () => {
  const a = 키생존축({ 글: 죽은것('글', { 상태: 403, 종류: '자격', 사유: 'API key not valid' }), 돈: 산것('돈') });
