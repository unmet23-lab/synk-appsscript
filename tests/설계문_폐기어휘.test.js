 ...
 * 🔄 08-22 좁힘(심문 D5 · 유호 확정 「진행해」): «대체»(살아 있는 지시 「~로 대체한다」에 자연 출현)와
 *   «v5.2»(판본 표기 — 살아 있는 서술에도 붙는다)를 뺐다. 단독 방패 감사(실측)로 실문서에서 이 둘이
 *   단독 방패인 줄은 0이라 좁혀도 초록 그대로다 — 아래 «남용 방향» 픽스처가 이 좁힘을 못박는다.
 */
const 역사표기 = /역사|걷었다|걷어|철회|폐기|되돌리기|되살리|옛 어휘|아니었다|🚫/;
...
test('픽스처 — 살아 있는 서술이 약한 낱말(대체·v5.2)을 품어도 잡는다 (남용 방향 · 심문 08-22 D5)', () => {
  const 가짜 = [
    '- 새 행은 `generation_claims`(v5.2 계보) 에 적재한다.', // 판본 표기는 방패가 아니다
    '- `jobs_적재` 를 `jobs_load` 로 대체한다.', // 살아 있는 지시의 「대체」는 방패가 아니다
  ].join('\n');
  assert.deepEqual(
    살아있는잔존(가짜).map((x) => x.낱말).sort(),
    ['generation_claims', 'jobs_적재'],
    '약한 낱말이 살아 있는 서술을 가리면 안 된다',
  );
});
