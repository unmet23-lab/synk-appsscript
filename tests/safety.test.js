...
test('[v9.268] 🧰 직장 경험 폼 — 재실행 안전·익명 회수·활용 동의·«방해» 칸·워치독 편입', () => {
  ...
  assertOrder(기존경로, ['const shT = ss.getSheetByName(WORK_TAB)', 'let 곁다리허용 = tabOk', '직장폼제목_(exForm, 곁다리허용) !== WORK_FORM_TITLE', "getState(st, '직장폼완료')", '직장폼서명_(exForm, 곁다리허용)']);
  /* 🔴 [①배포 검수 8812afdbcd6c 채택] 탭이 없을 때 곁다리를 «대신» 세우는 자 — 앞 회차의 조임이 하필
   * 이 경로의 존재 이유(응답 탭이 지워진 옛 폼 되살리기)까지 닫았다. 탭 대신 «더 강한 형상»을 요구한다:
   * 서명의 두 칸이 아니라 WORK_REQUIRED_ 다섯 전부. 이 문이 막는 것은 공격이 아니라 사고(낡은 ID·
   * 복사본·동명 폼)이고, 그 셋은 문서 제목과 필수 문항 다섯을 동시에 맞추지 못한다. */
  assert.ok(/WORK_REQUIRED_\.every\(/.test(기존경로),
    '탭이 없을 때 «필수 문항 전수»로 형상을 재지 않는다 — 두 칸만 보면 약하고, 안 보면 옛 폼을 못 되살린다');
