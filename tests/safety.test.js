...
test('[v9.113] 인센티브 배점 3종 — 구간 경계 + 미측정은 점수 자체가 없다', () => {
  ...
  const 호출 = code.slice(code.indexOf('incentiveTotal_(['), code.indexOf('incentiveTotal_([') + 220);
  ['promotionScore_', 'absenceReturnScore_', 'reenrollScore_'].forEach((f) => assert.ok(호출.includes(f),
