...
test('[v9.268] 🧰 직장 경험 폼 — 재실행 안전·익명 회수·활용 동의·«방해» 칸·워치독 편입', () => {
  ...
  /* 🔴 [09-08 검수 5회차] 한 줄이던 것이 «값을 읽는 자리 + 가르는 자리 + 안내를 만드는 자리» 셋으로
   *   나뉘었다(표식이 이제 「y / 빈 값」이 아니라 「y / 필수누락:… / 빈 값」 셋이라 안내가 갈래를 가른다).
   *   그래서 옛 한 줄 정규식이 못 찾았다 — 뜻은 그대로이므로 셋을 각각 문다.
   *   ⚠ 실제 «동작»은 여기서 안 잰다. 그 층은 tests/직장폼서명_묻는문항.test.js ⑦⑧ 이 진다
   *     (도장 블록과 안내 함수를 소스에서 떼어 내 실제로 돌린다). */
  assert.ok(/getState\(st, '직장폼완료'\)/.test(body), '기존 폼 경로가 완료 표식을 읽지 않는다');
  assert.ok(/완료값 !== 'y'/.test(body), "완료 표식을 'y' 기준으로 가르지 않는다 — 문항이 빠진 폼을 완성으로 보고한다");
  assert.ok(/직장폼미완안내_\(완료값/.test(body), '미완 안내가 표식 값을 안 받는다 — 받아야 「무엇이 빠졌는지」를 이름으로 말한다');
