 ...
 *      회귀는 tests/리포트카드공개.test.js가 **실행 결과로** 잡는다(문자열이 아니라 setSharing 호출을 관찰).
...
test('[v9.63] 격리·오류 카드는 학생 표면(포인트 정산·성장카드 짝·오류사전·약점퀴즈 재료)에 새어들지 않는다', () => {
  ...
  assert.ok(code.includes('오류뱅크전진_(슬라이스.map(r => r[8]))'), '오류사전 로더가 커서 통로(오류뱅크전진_ · 안에서 노출카드_)를 안 탄다'); // [v9.211] 커서 수리로 통로가 바뀜
...
const 커서정본 = () => {
  ...
  return new Function(`${f1[0]}; ${f2[0]}; ${f3[0]}; return 오류뱅크전진_;`)();
};

test('🔑 오류뱅크 커서는 판정 전(대기·빈칸·낯선 값) 행 앞에서 멈춘다 — 지나가면 승인 뒤에도 영구 누락', () => {
  const 걷기 = 커서정본();
  assert.deepEqual(걷기(['노출', '격리:품질미달', '노출']), { 집을행: [0, 2], 전진: 3 }); // 닫힌 행은 안 집고 지나간다(커서 정체 방지)
  assert.deepEqual(걷기(['노출', '대기', '노출']), { 집을행: [0], 전진: 1 });             // 대기 앞까지만 — 뒤를 집으면 다음 밤 같은 행이 두 번 집힌다
  assert.deepEqual(걷기(['대기', '노출']), { 집을행: [], 전진: 0 });                       // 머리가 대기면 이번 밤은 쉰다(다음 밤 재시도)
  assert.deepEqual(걷기(['오류:API', '격리:저품질']), { 집을행: [], 전진: 2 });            // 전부 닫힘 — 커서만 간다
  assert.deepEqual(걷기(['', '노출']), { 집을행: [], 전진: 0 });                           // 빈칸·낯선 값=판정 전 취급 — 틀릴 때 방향이 유실이 아니라 지연
});

test('🔑 대기 카드는 승인(대기→노출) 뒤 다음 밤 같은 자리부터 집힌다 — 재검수 P1 시나리오 재현', () => {
  const 걷기 = 커서정본();
  assert.equal(걷기(['대기', '노출']).전진, 0, '커서가 대기를 지나갔다 — 승인해도 영구 누락된다');
  // 검토자가 승인한 다음 밤: 커서가 안 움직였으니 같은 from 에서 다시 읽은 슬라이스가 이렇게 보인다
  assert.deepEqual(걷기(['노출', '노출']), { 집을행: [0, 1], 전진: 2 }); // 승인된 카드가 집힌다 — 유실 0
});
...
test('☠️ 오류사전 커서가 필터 전 행 수(takeN)로 전진하지 않는다 — 그 표기가 대기 행을 영구 통과시키던 자리다', () => {
  ...
  assert.ok(src.includes('오류뱅크전진_(슬라이스.map(r => r[8]))'), '오류사전 로더가 커서 통로를 안 탄다');
});
