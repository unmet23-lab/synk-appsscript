...
/* 손 접기 판별기 — 「줄끝을 손으로 접는」 표기 하나를 **코드에서만** 본다.
 *
 * 🔴 두 번 눌러 보고 두 번 고쳤다(둘 다 실측이다 — 이 자리가 가드 맹점의 교과서다):
 *   ③ 첫 판 `readFileSync\([^)]*\)\s*\.replace\(\/\\r` 는 **제가 금지한 되돌림을 못 잡았다** —
 *     실제 되돌림은 `readFileSync(path.join(ROOT, …), 'utf8').replace(…)` 라 `[^)]*` 가 안쪽 `)`
 *     에서 끊긴다. 차단 사유가 시키는 명령을 그 가드에 되먹여 보고 알았다.
 *   ① 넓힌 판은 이번엔 **자기 주석에 눈이 멀었다** — 내가 위 세 파일에 「그전엔 손으로 이렇게
 *     접었다」고 적어 둔 설명 문장이 그대로 위반으로 잡혔다. 즉 이 검사는 정확히 «가장 잘 적어
 *     둔 파일»에서 깨진다(F401 이 그 사고다). 그래서 `코드만()` 으로 주석을 걷고 본다 —
 *     같은 모듈이 이미 그 답을 들고 있었다. */
const 손접기인가 = (src) => /\.replace\(\/\\r/.test(소스검사.코드만(src));

test('🔴 [탐지력] 손 접기 판별기가 «되돌린 그 줄»을 실제로 잡는다 — 실저장소가 깨끗하면 눈멀어도 초록이다', () => {
  assert.ok(손접기인가("const SRC = fs.readFileSync(path.join(ROOT, 'tools', 'x.js'), 'utf8').replace(/\\r\\n/g, '\\n');"),
    '되돌림의 실물 표기를 못 잡는다 — 첫 판이 정확히 여기서 눈멀었다');
  assert.ok(손접기인가("const s = 원문.replace(/\\r\\n/g, '\\n');"), '변수를 거쳐 접으면 빠져나간다');
  assert.ok(!손접기인가("const s = 파일소스(p);"), '거짓양성 — 공용 통로를 위반으로 읽는다');
  assert.ok(!손접기인가("const s = x.replace(/\\s+$/, '');"), '거짓양성 — 줄끝과 무관한 replace 까지 잡는다');
  assert.ok(!손접기인가("/* 그전엔 손으로 `.replace(/\\r\\n/g,'\\n')` 만 했다 */\nconst s = 파일소스(p);"),
    '거짓양성 — «금지»를 설명하는 주석을 위반으로 읽는다(F401 그 사고 · 실제로 여기서 한 번 깨졌다)');
});
