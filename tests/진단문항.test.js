...
/* ── 🔴 «최대한 쉽게» (유호 지시 09-07) ────────────────────────────────────
 * 문항이 길면 문형이 아니라 «긴 문장을 읽어내는 힘»을 재게 된다. 그러면 3급 문형을 아는 학생이
 * 독해에서 걸려 급수가 내려간다. 그 새는 자리는 채점기 어디에도 안 뜨므로 길이로 막는다. */
test('모든 문항이 다섯 어절을 넘지 않는다', () => {
  const 상한 = 5;
  모든급.forEach((급) => 급별[급].forEach((g) => g[2].forEach((q, i) => {
    const n = q[0].split(/\s+/).filter(Boolean).length;
    assert.ok(n <= 상한,
      `${g[0]}[${i}] 이 ${n}어절이다(상한 ${상한}) — 문형이 아니라 독해를 재게 된다: ${q[0]}`);
  })));
});

test('두 문장짜리는 근거가 필요한 문형에만 있다', () => {
  const 두문장허용 = ['G705']; // -나 보다 — 앞 문장이 추측의 근거라 한 문장으로 못 줄인다
  모든급.forEach((급) => 급별[급].forEach((g) => g[2].forEach((q, i) => {
    const 마침표 = (q[0].match(/[.?!]/g) || []).length;
    if (마침표 > 1) {
      assert.ok(두문장허용.includes(g[0]),
        `${g[0]}[${i}] 이 두 문장이다 — 앞뒤 사정을 상상해야 풀리면 그 자체가 독해 시험이다: ${q[0]}`);
    }
  })));
});
...
test('보기도 짧다', () => {
  ...
  모든급.forEach((급) => 급별[급].forEach((g) => g[2].forEach((q, i) => {
    q[1].forEach((c) => {
      assert.ok(c.length <= 상한,
        `${g[0]}[${i}] 의 보기 「${c}」 가 ${c.length}자다(상한 ${상한}) — 보기가 길면 넷을 견주는 데 시간이 간다`);
    });
  })));
});
