...
const 한판소스 = 떼어오기(셋업, 'function 시트칸맞추기한판_(');

/** 아침 배치가 부르는 «한 판»을 통째로 만든다 — 맞추기 + 기록이 한 몸으로 돌아야 예외 경로를 잴 수 있다.
 *  🔑 담을곳에는 기록 함수가 실제로 적은 값이 들어온다(글자 검사가 아니라 «적힌 것»을 본다). */
function 한판만들기(골격, 담을곳) {
  const 칸들 = {};
  const st = { _칸: 칸들, getLastRow: () => Object.keys(칸들).length, getRange: () => ({ setValue() {}, setValues() {} }) };
  return new Function(
    'sheetSkeleton_', 'ensureSheet', 'setState', 'Logger',
    보정소스 + '\n' + 값있나소스 + '\n' + 스위치소스 + '\n' + 맞추기소스 + '\n' + 기록소스 + '\n' + 한판소스
    + '\nreturn 시트칸맞추기한판_;'
  )(
    () => 골격,
...
test('아침 배치가 «한 판»을 부른다 — 몸은 시트칸맞추기한판_ 이 쥔다', () => {
  const i = 셋업.indexOf('function morningJobs()');
  const 몸 = 셋업.slice(i, 셋업.indexOf('\n}', i));
  const 칸블록 = 몸.slice(몸.indexOf("safeRun('시트칸맞추기'"), 몸.indexOf("safeRun('첨삭서명트리거'"));
  assert.ok(칸블록.includes('시트칸맞추기한판_('), '아침 배치가 한 판을 안 부르면 09-07 구멍이 그대로다');
});

/* ══════════ 예외 경로 — 09-08 검수 P1 a8832d0e0226·33079b2b2699 ══════════
 * 여기서부터는 «소스에 글자가 있나»가 아니라 **함수를 실제로 돌려** 적힌 값을 본다.
 * 09-08 검수가 그 자리를 정확히 짚었다: 정규식 검사는 finally 를 빈 블록으로 만들고 기록을
 * 그 뒤로 옮겨도 초록이었다. 자가 결함을 못 잡으면 초록은 거짓이다. */

/** 두 번째 표에서 던지는 가짜 문서 — 앞 표는 정상으로 처리되고 그 뒤 예외가 난다. */
function 던지는문서(첫이름, 첫시트, 둘째이름) {
  return {
    getSheetByName(n) {
      if (n === 첫이름) return 첫시트;
      if (n === 둘째이름) return { getMaxColumns() { throw new Error('일부러 낸 예외'); } };
      return null;
    },
  };
}

test('🔴 예외로 끝나도 «그때까지의 실적»이 그대로 남는다 — 0 으로 지우지 않는다 (09-08 검수 P1 a8832d0e0226)', () => {
  const 담김 = {};
  const 한판 = 한판만들기([['앞표', ['A', 'B', 'C']], ['뒷표', ['A']]], 담김);
  const 앞 = 가짜시트(['A']);                      // 3칸 정본에 1칸뿐 → 두 칸이 늘어난다
  assert.throws(() => 한판(던지는문서('앞표', 앞, '뒷표'), { 밀기: true }), /일부러 낸 예외/,
    '예외는 다시 던져야 한다 — 삼키면 safeRun 이 실패 메일을 못 낸다');

  const 적힌것 = JSON.parse(담김['시트칸맞추기_마지막']);
  assert.strictEqual(적힌것.더한칸, 2, '앞 표에서 실제로 늘린 두 칸이 0 으로 지워졌다');
  assert.strictEqual(적힌것.맞춘표, 1, '이미 맞춘 표가 기록에서 사라졌다(기록은 «수»로 적는다)');
  assert.strictEqual(적힌것.밀기, true, '켜져 있던 스위치가 «꺼짐»으로 잘못 남았다');
  assert.ok(적힌것.건너뛴표.some((x) => /예외로 끝났다/.test(x)),
    '사람이 「돌다 죽었다」로 알아볼 줄이 있어야 한다');
});

test('예외가 없으면 건너뛴표에 «예외» 줄이 안 붙는다 — 멀쩡한 날을 죽은 날로 적지 않는다', () => {
  const 담김 = {};
  const 한판 = 한판만들기([['앞표', ['A', 'B', 'C']]], 담김);
  const 앞 = 가짜시트(['A']);
  한판({ getSheetByName: (n) => (n === '앞표' ? 앞 : null) }, { 밀기: true });
  const 적힌것 = JSON.parse(담김['시트칸맞추기_마지막']);
  assert.strictEqual(적힌것.더한칸, 2);
  assert.ok(!적힌것.건너뛴표.some((x) => /예외로 끝났다/.test(x)), '멀쩡한 판에 죽음 표식이 붙었다');
});

test('🔴 늘린 칸은 «아무것도 만지기 전» 폭으로 센다 — 값으로 잰다 (09-08 검수 P1 bddf9a7aad1d)', () => {
  /* 정본 4칸인데 라이브는 1칸 + 남의 열 하나 → 폭을 늘리는 자리가 둘(정본 맞추기 · 미는 자리)이다.
   * 옛 판은 헤더보정_ 직전에 폭을 잡아 그 앞의 증설이 통째로 빠졌다. */
  const 정본 = ['A', 'B', 'C', 'D'];
  const sh = 가짜시트(['A', 'X'], [['a', 'x']]);
  const r = 만들기([['늘린표', 정본]])(가짜문서({ 늘린표: sh }), { 밀기: true });
  const 잰폭 = sh.칸().length;
  assert.ok(잰폭 > 4, '폭이 정본보다 커져야 한다(남의 열이 밖으로 나갔다)');
  assert.strictEqual(r.더한칸, 잰폭 - 2, '늘린 칸을 «시작 폭 2» 기준으로 세야 한다');
});

test('🔴 막혀서 되돌아설 때도 이미 늘어난 폭을 센다 — 값으로 잰다 (09-08)', () => {
  /* 순서가 섞인 표는 «막힘»으로 되돌아선다. 그 전에 정본 폭까지는 이미 늘려 둔 뒤다. */
  const 정본 = ['A', 'B', 'C', 'D'];
  const sh = 가짜시트(['A', 'C', 'B']);            // 2·3열이 뒤바뀐 라이브 · 폭 3
  const r = 만들기([['막힌표', 정본]])(가짜문서({ 막힌표: sh }));
  assert.strictEqual(r.건너뛴표.length, 1, '순서가 섞였으니 멈춰야 한다');
  assert.strictEqual(r.더한칸, 1, '막히기 «전»에 정본 폭까지 늘린 한 칸이 안 세어졌다');
});
