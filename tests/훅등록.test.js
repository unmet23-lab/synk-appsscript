...
/* ⚠ track-collision 은 이 CASES 목록에 넣지 않는다 — **의도적으로 deny 를 안 내는 훅**이라
 * 위 불변식(「훅이 막는 입력은 라우팅도 통과시킨다」)의 검사 대상이 아니다.
 * 판정이 트리 상태(남의 커밋 착지)에 따라 변해 불안정 테스트가 되기도 한다.
 * 그쪽 등록·라우팅은 tests/트랙충돌.test.js 가 지킨다(매처 3종 + 앞단 필터 부재 + deny 부재). */
