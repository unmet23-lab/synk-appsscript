...
/* ── 위키링크 (2026-08-07 · F222 ①) ────────────────────────────────────────
 * 앞 판은 별칭 있는 `[[막힘>x]]` 만 검사해서 **별칭 없는 `[[x]]` 가 통째로 지워지는 것**을
 * 8일간 못 봤다. 유호님 화면에서만 깨지고 파일에선 멀쩡해 아무도 눈치 못 챈다(F119 계열).
 * 실측 08-07: 배달 36건 중 9건이 이 모양이었고, mongolia-trademark 는 주어가 지워져
 * 「— 가 요구한 2건 중」으로 나갔다. 탐지력은 이 픽스처가 진다(실메모리는 그때그때 달라진다). */
test('별칭 없는 위키링크는 이름을 남긴다 — 조사만 남고 주어가 사라지면 안 된다', () => {
  assert.equal(q.stripMd('이 한 번이 [[x-y]] 의 v9.103'), '이 한 번이 x-y 의 v9.103');
  assert.equal(q.stripMd('⏳ 상세 [[engine-pipeline-design]]'), '⏳ 상세 engine-pipeline-design');
  // 별칭 두 표기 다 — `|` 는 앞 판이 파이프를 그대로 흘렸다(`[[a|b]]` → `|b`)
  assert.equal(q.stripMd('[[막힘>x]]'), 'x');
  assert.equal(q.stripMd('[[a|b]]'), 'b');
});

/* ── 소프트랩 꼬리 (2026-08-07 · F222 ②) ───────────────────────────────────
 * 판정은 줄 단위인데 마크다운은 빈 줄까지가 한 문단이라, ⏳ 항목이 줄바꿈되면 뒤가 안 실렸다.
 * 앞 세션의 처방은 「그 줄을 한 줄로 붙여 둔다」는 프로즈였고 그 파일 하나만 고쳤다.
 * 실측 08-07: s1a-delivery-batch:91 의 도전안이 「…20~30개 순환으로」에서 끊겨 동사 없이 나갔다. */
test('줄바꿈된 ⏳ 항목은 뒤가 이어서 실린다', () => {
  const dir = fixture({
    'MEMORY.md': '- [w](w.md)',
    'w.md': ['# W', '   ⏳유호님 답 대기: 이 갈래를 빼고 고정 문장으로', '   대체하면 다음 칸이 바로 화면이다.'].join('\n'),
  });
  const items = q.extract(dir);
  assert.equal(items.length, 1, '이어 붙이다 항목 수가 변했다');
  assert.match(items[0].text, /대체하면 다음 칸이 바로 화면이다\./, '줄바꿈 뒤가 안 실렸다 — 동사 없이 배달된다');
});

test('꼬리는 다음 항목을 삼키지 않는다 — 빈 줄 없이 다른 표식이 시작하는 자리', () => {
  // 실측 referral-prereg:122~123 — 빈 줄 없이 `⚠` 로 다른 항목이 시작한다.
  // 삼키면 유호님 결정 하나에 남의 문단이 붙어 나간다(잘림보다 나쁘다).
  const dir = fixture({
    'MEMORY.md': '- [w](w.md)',
    'w.md': ['# W', '⏳2026-11 굿즈 단가 미확인 — 견적 문구는 정본에 넣어 뒀다.', '⚠ 상점 포인트 가격은 원가가 아니다.', '', '- ⏳ 다음 항목', '- 표식 없는 다음 불릿'].join('\n'),
  });
  const items = q.extract(dir);
  assert.equal(items.length, 2, `항목 수가 변했다: ${JSON.stringify(items.map((i) => i.text))}`);
  assert.doesNotMatch(items[0].text, /상점 포인트/, '표식으로 시작하는 다음 항목을 삼켰다');
  assert.doesNotMatch(items[1].text, /표식 없는 다음 불릿/, '표식이 없어도 새 불릿은 다음 항목이다');
  // 줄 중간에서 표식이 시작하는 형태도 같은 자리에서 끊는다(실측 homepage-draft:53)
  const d2 = fixture({
    'MEMORY.md': '- [h](h.md)',
    'h.md': ['# H', '- ⏳ 유호님 Recraft 2장 대기', '  (아래 절차). 🔴 점눈 판에도 K 필수 — 깜빡임이'].join('\n'),
  });
  assert.equal(q.extract(d2)[0].text, '⏳ 유호님 Recraft 2장 대기 (아래 절차).');
});

test('꼬리를 이어도 배달·폐기함·절제목 분모는 그대로다', () => {
  // 이 수리는 **더하기만 한다** — 판정에 손대지 않으므로 지금 배달되는 항목이 사라질 길이 없다.
  // 실저장소 실측(08-07)은 36→36·버려진 85→85·절뿐 25→25·문구만 9건 복구였다.
  const dir = fixture({
    'MEMORY.md': '- [w](w.md)',
    'w.md': ['# W', '## ⏳ 절 제목', '- ⏳ 배달되는 항목', '  이어지는 꼬리', '- ~~⏳ 죽인 항목~~', '- 앞 문장 ⏳ 지시어'].join('\n'),
  });
  const items = q.extract(dir);
  assert.equal(items.length, 1);
  assert.equal(items.버려진.length, 2);
  assert.equal(items.절뿐.length, 0, '배달이 있으니 절뿐이 아니다');
  assert.match(items[0].text, /이어지는 꼬리/);
});
...
/* ── 절 제목만 걸고 아무것도 못 내는 파일 (2026-08-07 실측) ─────────────────
 * 위 헤더 필터는 옳았는데 **반환값이 `null`** 이라 그 줄은 배달도 폐기함도 아닌 곳으로
 * 사라졌다. 「헤더 아래 불릿이 항목」이라는 전제가 안 맞는 파일이 실저장소에 34개였고
 * (불릿에 ⏳ 를 안 단다), 그중 `abandoned-audio-orphan` 은 소급불가 ①-2·①-12 를 여는
 * 유호님 결정이라 **한 번도 배달되지 않았다.** 필터 자체가 아니라 무기록이 결함이었다. */
test('절 제목만 있고 배달 0건인 파일은 조용히 사라지지 않는다', () => {
  const dir = fixture({
    'MEMORY.md': '- [h](h.md)',
    'h.md': ['# H', '## ⏳ 유호님 결정 1건', '- 이름을 계약에 넣을까요', '- 비용은 값 1개'].join('\n'),
  });
  const items = q.extract(dir);
  assert.equal(items.length, 0, '절 제목이 항목으로 배달됐다');
  assert.equal(items.절뿐.length, 1, '결정을 내걸고 아무것도 못 내는 파일이 흔적 없이 사라졌다');
  assert.equal(items.절뿐[0].topic, 'h');
  assert.match(items.절뿐[0].제목[0].text, /유호님 결정 1건/, '사람이 어느 절인지 알아야 고친다');
});

test('절 아래에 진짜 항목이 있으면 깨진 약속이 아니다 — 거짓양성 금지', () => {
  const dir = fixture({
    'MEMORY.md': '- [h](h.md)',
    'h.md': ['# H', '## ⏳ 남은 것', '- ⏳ 진짜 답해야 하는 것'].join('\n'),
  });
  const items = q.extract(dir);
  assert.equal(items.length, 1);
  assert.equal(items.절뿐.length, 0, '배달이 되는 파일까지 경고에 실리면 34개 목록이 다시 안 읽힌다');
});

test('⏳ 줄은 배달·폐기함·절제목 셋 중 정확히 하나에 든다 — 무기록 0', () => {
  // 통과와 미실행이 같은 모양이면 안 된다(이 파일의 취소선·날짜게이트 테스트와 같은 원칙).
  // a.md 는 절 제목뿐(배달 0) — 여기서 세어지지 않으면 그 줄은 어느 화면에도 안 뜬다.
  const a = ['# A', '## ⏳ 절 제목'];
  const b = ['# B', '- ⏳ 배달되는 항목', '- ~~⏳ 죽인 항목~~', '- 앞 문장 ⏳ 지시어로 쓴 줄'];
  const dir = fixture({ 'MEMORY.md': '- [a](a.md)\n- [b](b.md)', 'a.md': a.join('\n'), 'b.md': b.join('\n') });
  const items = q.extract(dir);
  const 대기줄수 = [...a, ...b].filter((l) => l.includes('⏳')).length;
  const 합 = items.length + items.버려진.length + items.절뿐.reduce((n, f) => n + f.제목.length, 0);
  assert.equal(합, 대기줄수, `⏳ ${대기줄수}줄 중 ${대기줄수 - 합}줄이 무기록으로 사라졌다`);
});

test('절뿐이 있으면 경고 문구가 실제로 나온다 — 세지기만 하고 안 뜨면 없는 것과 같다', () => {
  const 문구 = q.버려진줄({ 버려진: [], 절뿐: [{ topic: 'h', 제목: [{ line: 1, text: '## ⏳ x' }] }] });
  assert.match(문구, /배달 0건.*1개/s, `절뿐 경고가 안 나왔다: ${JSON.stringify(문구)}`);
  assert.match(문구, /--절/, '사람이 목록을 볼 명령이 문구에 있어야 한다');
  assert.equal(q.버려진줄({ 버려진: [], 절뿐: [] }), '', '둘 다 없으면 조용해야 한다');
});
...
/* [2026-08-11 · a831951c] 네 번째 배달 실패 모양 — **재료가 표식 앞에 있는 꼬리 조각**.
 * 실측: mvp-eta-measured:50 「…남의 왕복시험이 전부 막힌다(F313…). ⏳유호님 승인 자리.」가
 * 「⏳유호님 승인 자리.」 7글자 껍데기로 배달됐다 — 리허설 재배포 승인(전 세션의 왕복시험을
 * 여는 유일한 결정)이 답할 수 없는 모양으로 나갔고 어떤 경고도 안 울렸다. 판정 자체는 옳다
 * (「항목이면 앞은 배경」) — 고칠 것은 배달이 아니라 경고다. 탐지력은 이 픽스처가 못박는다. */
test('재료가 표식 앞인 꼬리 조각 — 배달은 되되 경고로 센다 (mvp-eta-measured:50 모양)', () => {
  const dir = fixture({
    'frag.md': '# F\n- 🔴 **남은 것 = 리허설 재배포 4종**(deliver·events — 동봉 셋). 배포 전까지 서버는 옛 계약으로 판정하고, 그 사이 남의 왕복시험이 전부 막힌다(F313 · 내 화면은 초록이라 증상 0). ⏳유호님 승인 자리.',
  });
  const items = q.extract(dir);
  assert.equal(items.length, 1, '조각도 배달은 된다 — 껍데기도 침묵보다 낫다');
  assert.equal((items.조각 || []).length, 1, '꼬리 조각을 못 셌다 — 이 경고가 없으면 껍데기 배달이 무증상이다');
  assert.equal(items.조각[0].topic, 'frag');
  const 경고 = q.버려진줄({ 버려진: [], 절뿐: [], 조각: items.조각 });
  assert.match(경고, /--조각/, '경고가 처방(--조각)을 안 준다 — 따를 수 없는 경고는 무시가 정상 통로가 된다');
});

test('짧은 완결 항목·긴 완결 항목은 조각이 아니다 — 거짓양성 0', () => {
  const dir = fixture({
    'ok.md': [
      '# OK',
      '- ⏳ 티원',   // 앞에 버려진 재료가 없다 — 짧아도 완결이다(장식뿐 필터와 같은 이유)
      '- 긴 설명이 앞에 배경으로 붙어 있어도 표식 뒤가 완결이면 조각이 아니다: ⏳ 리허설 재배포를 승인해 주실까요? 운영 아님 · 되돌리기 한 번',
    ].join('\n'),
  });
  const items = q.extract(dir);
  assert.equal(items.length, 2);
  assert.equal((items.조각 || []).length, 0,
    `완결 항목이 조각으로 잡혔다(문턱이 넓다): ${JSON.stringify(items.조각)}`);
});

test('소프트랩 꼬리가 재료를 이어주면 조각이 아니다 — 판정은 이어붙임 뒤에 한다', () => {
  const dir = fixture({
    'wrap.md': '# W\n- 앞 설명이 길게 이어지는 배경 문장이고 표식은 줄 끝에 있다 그래서. ⏳유호 답:\n  이어지는 다음 줄에 질문 전문이 있다 — 요금제를 어느 것으로 할까요',
  });
  const items = q.extract(dir);
  assert.equal(items.length, 1);
  assert.equal((items.조각 || []).length, 0, '이어붙어 완결된 항목이 조각으로 잡혔다');
});
