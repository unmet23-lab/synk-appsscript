...
// 🔴 위 픽스처는 **한글 경계만** 졌다 — 같은 커밋이 함께 고친 영문 경계
// (`/Wear(?![a-z])/`·`/Pantry(?![a-z])/`)는 오탐 픽스처가 0이었다. 변이로 실측했다
// (2026-08-16 · F497 2차): 둘을 부분 문자열로 되돌려도 26건이 **전부 초록**이라
// 「변이 2/4 · 구멍 2」가 나왔다. 신고의 처방은 「탐지·오탐 둘 다 픽스처가 진다」였는데
// 절반만 집행돼 있었고, 초록이 그 절반을 가렸다(지침 v9.2 맹점 ④).
test('🔑 철회 검사가 영문 합성어를 오탐하지 않는다 (거짓양성 · F497 2차)', () => {
  // Wearable 은 에듀테크 발표물에 실제로 나온다. Pantry 쪽은 흔한 합성어가 없어
  // **대칭 방어**다 — 실재하는 오탐을 막는 게 아니라, 경계를 지우는 「단순화」가
  // 조용히 통과하지 못하게 모양을 못박는다(구멍은 실재 여부와 무관하게 구멍이다).
  for (const 문구 of ['Wearable 기기와도 붙습니다', 'Wearables 시장은 다릅니다', 'Pantryless 구성입니다']) {
    const v = run(page({ text: 문구 }));
    assert.doesNotMatch(v.out, /철회된 제품 라인/, `영문 합성어를 철회로 오탐했다: ${문구}`);
  }
});
