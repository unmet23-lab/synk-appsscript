...
/* ── 머리줄 대기 (자판 v2 · 08-31 신설 · 유호 지시) ────────────────────────────
 * 이 갈래가 태어난 까닭: 접기 하루에 **여섯 번** 「본문은 갱신됐는데 머리줄(description)이 안 따라와
 * 다음 세션이 이미 답하신 것을 또 여쭙는」 무늬를 잡았다. 실물 하나엔 「유호님이 이 답을 두 번
 * 주셨다」가 적혀 있었다.
 * 🔴 이 시험이 가장 중요하게 막는 것 — **고치는 것을 벌하는 검사가 되는 것.** 정정문은 옛 표기를
 *   인용으로 들고 다니므로(「옛 4건 대기 표기는 낡았다」) 그걸 안 빼면 **오늘 고친 파일이 내일 운다**,
 *   그리고 우는 검사는 꺼진다(자판 머리말 §짓지 말 것). 여기서 그 자리를 이름으로 잰다. */

const 머리 = (name, d, 본문 = '몸통') =>
  `---\nname: ${name}\ndescription: ${d}\nmetadata:\n  node_type: memory\n  type: project\n---\n\n${본문}\n`;
const 대기목록 = (파일들) => M.위생(M.load(임시폴더(파일들)), '판').판정거리.머리줄대기.map((x) => x.파일);

test('머리줄 대기 — 「누가 무엇을 기다리는지」가 있어야 센다(낱말 하나로는 안 센다)', () => {
  const r = 대기목록({
    '가.md': 머리('가', '유호 판정 대기 — 셋 중 하나'),
    '나.md': 머리('나', '칸바 구독 유지 판정(⏳유호 픽)'),
    '다.md': 머리('다', '아직 안 정한 것 둘은 파일에'),
    // 🔴 아래 넷은 «울면 안 되는» 것들 — 32% 정밀도로 울던 무늬가 정확히 이것이다.
    '라.md': 머리('라', '앱 모션 스택 확정 — Skia 채택·Lottie 자작·Reanimated 보류'),  // 「보류」는 «내려진» 판정이다
    '마.md': 머리('마', '시즌 회고 그릇 — 판정 대기실 설계는 본문에'),                   // 대기«실» — 어법이 아니다
    '바.md': 머리('바', '몽골 결제 판정 — QPay 가 유일 경로'),
    '사.md': 머리('사', '한 줄'),
  });
  assert.deepStrictEqual(r.sort(), ['가', '나', '다']);
});

test('🔴 머리줄 대기 — «이미 고친» 정정문은 안 운다(고치는 것을 벌하면 검사가 꺼진다)', () => {
  const r = 대기목록({
    '고침1.md': 머리('고침1', '유호 판정은 «전건 확정»(옛 「4건 대기」 표기는 낡았다)'),
    '고침2.md': 머리('고침2', '심문 게이트는 «켜져» 있다 — 옛 OFF 표기는 무효'),
    '고침3.md': 머리('고침3', '저장소 공개 범위 — 유호 판정 대기였으나 닫혔다'),
    '아직.md': 머리('아직', '무보호 셋은 유호 결정 대기'),
  });
  assert.deepStrictEqual(r, ['아직'], '정정문이 울면 다음 세션이 이 검사를 끈다');
});

test('머리줄 대기 — 정본 대조는 «없다»와 «못 쟀다»를 가른다', () => {
  const 정본 = (본문) => `---\nname: yuho-open-decisions\ndescription: 유호님 앞에 걸린 것\nmetadata:\n  node_type: memory\n  type: project\n---\n\n${본문}\n`;
  const 뽑기 = (파일들) => M.위생(M.load(임시폴더(파일들)), '판').판정거리.머리줄대기;

  // ① 정본이 그 파일을 쥔다
  const a = 뽑기({ '가.md': 머리('가', '유호 판정 대기'), 'yuho-open-decisions.md': 정본('- [[가]] 아직 안 정하셨다') });
  assert.strictEqual(a.find((x) => x.파일 === '가').정본, true);

  // ② 정본은 있는데 그 파일을 안 쥔다
  const b = 뽑기({ '가.md': 머리('가', '유호 판정 대기'), 'yuho-open-decisions.md': 정본('- [[다른것]]') });
  assert.strictEqual(b.find((x) => x.파일 === '가').정본, false);

  // ③ 🔴 정본 자체가 없다 — false 가 아니라 null 이다(「안 쥔다」와 「못 쟀다」가 같은 얼굴이면 자가 아니다)
  const c = 뽑기({ '가.md': 머리('가', '유호 판정 대기') });
  assert.strictEqual(c.find((x) => x.파일 === '가').정본, null);
});

test('머리줄 대기 — 정본 «자신»은 안 센다(제 머리줄이 대기를 말하는 것은 그 파일의 일이다)', () => {
  const r = 대기목록({
    'yuho-open-decisions.md': `---\nname: yuho-open-decisions\ndescription: 유호님만 할 수 있어 내가 못 넘는 자리들 — ⏳ 대기\nmetadata:\n  node_type: memory\n  type: project\n---\n\n몸통\n`,
  });
  assert.deepStrictEqual(r, []);
});

test('🔴 자판을 고쳤으면 판 수가 올라야 한다 — 래칫이 «폴더 변화»와 «자 변화»를 가른다', () => {
  assert.ok(M.위생자판.판 >= 2, '머리줄 대기 갈래를 더했는데 자판 판이 1 그대로면 옛 상한이 새 갈래에 그대로 적용된다');
});
