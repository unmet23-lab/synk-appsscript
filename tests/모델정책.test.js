...
/* [2026-08-06] 역할마다 기본이 다르다 — 유호님 확정: ①배포 검수 = luna/max · ②설계 심문 = sol/max.
 * 두 기본이 **서로 안 새는지**가 이 회귀의 요점이다. 새는 방향은 언제나 싼 쪽(luna)이고,
 * 그러면 「심문을 sol 로 돌렸다」고 믿으면서 luna 로 도는 상태가 된다 — 이 파일이 막으려는 병 그대로. */
test('역할별 기본이 안 섞인다 — ②심문=sol/max · ①검수=luna/max', () => {
  const 심문 = 정책.분석설정([], 정책.심문기본);
  assert.strictEqual(심문.model, 'gpt-5.6-sol', '심문 기본이 sol 이 아니다');
  assert.strictEqual(심문.effort, 'max');

  const 검수 = 정책.분석설정([]);
  assert.strictEqual(검수.model, 'gpt-5.6-luna', '심문 기본이 배포 검수로 샜다 — 매 배포 비용이 오른다');
  assert.strictEqual(검수.effort, 'max');
});
