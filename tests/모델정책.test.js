...
/* [2026-08-09 2차] 유호님 확정: ②심문만 **luna ×2 + sol ×1**. ①검수는 luna ×2 그대로.
 * 편성이 조용히 바뀌는 것(특히 sol 이 빠지는 것)은 겉보기가 똑같다 — 그래서 목록째 못박는다. */
test('심문 편성 = luna/max ×2 + sol/max ×1 · 검수(①)로 새지 않는다', () => {
  const 런 = 정책.심문런들([]);
  assert.deepStrictEqual(런.map((p) => p.model), ['gpt-5.6-luna', 'gpt-5.6-luna', 'gpt-5.6-sol'],
    '심문 편성이 바뀌었다 — sol 이 빠지면 관점 다양성 축이 통째로 사라진다');
  assert.deepStrictEqual(런.map((p) => p.effort), ['max', 'max', 'max'], '한 런이라도 얕으면 세 벌이 비교가 안 된다');
  // ①검수는 이 편성을 안 탄다 — 매 배포라 sol 이 섞이면 비용이 빈도만큼 는다
  assert.strictEqual(정책.회차설정([]), 2, '검수 회차가 심문 편성 길이로 샜다');
  assert.strictEqual(정책.분석설정([]).model, 'gpt-5.6-luna');
});

test('심문 편성: --회차 N 은 앞 N개(자르는 쪽이 비싼 sol) · --검수 X 는 그 픽으로 회차만큼', () => {
  ...
  // 효력 1회성 조정이 **모든 런에** 걸린다 — 한 런만 걸리면 세 벌이 비교가 아니게 된다
  assert.deepStrictEqual(정책.심문런들(['--효력', 'high']).map((p) => p.effort), ['high', 'high', 'high']);
  // 오타는 기본으로 접지 않는다
