...
test('기본 픽은 sol — 유호 확정 2026-09-04 "아스트라 전까지 sol 1회" · luna 는 그날만 내리는 손잡이로 남는다', () => {
  assert.strictEqual(정책.검수기본, 'sol');
  assert.strictEqual(정책.검수선택().model, 'gpt-5.6-sol');
  assert.strictEqual(정책.검수선택('luna').model, 'gpt-5.6-luna', 'luna 가 선택지에서 사라졌다 — 내릴 손잡이가 없어진다');
});
...
test('역할별 기본 — ①검수·②심문 첫 런 둘 다 sol/xhigh (모델만 올리고 효력이 조용히 갈리지 않는다)', () => {
  /* 🔑 09-04 에 모델이 luna→sol 로 올라갔다. 이 검사가 지키는 것은 «값»이 아니라 «일치»다 —
   *   유호님이 셋을 한 값으로 움직이셨으므로, 한쪽만 따라가면 여기서 운다. */
  for (const [이름, 설정] of [['심문', 정책.분석설정([], 정책.심문기본)], ['검수', 정책.분석설정([])]]) {
    assert.strictEqual(설정.model, 'gpt-5.6-sol', `${이름} 기본이 sol 이 아니다`);
    assert.strictEqual(설정.effort, 'xhigh', `${이름} 효력이 xhigh 가 아니다 — 모델만 맞고 깊이가 갈렸다`);
  }
  assert.strictEqual(정책.실행설정([]).model, 'gpt-5.6-sol', '③실행자만 옛 값에 남았다 — 유호 확정은 「싹다」였다');
});
...
test('심문 편성: --회차 N 은 편성을 «늘리지» 않는다 · 명시 픽은 «모델만» 간다', () => {
  ...
  assert.deepStrictEqual(정책.심문런들(['--회차', '1']).map((p) => p.이름), ['sol']);
  assert.deepStrictEqual(정책.심문런들(['--회차', '2']).map((p) => p.이름), ['sol', 'gemini']);
  assert.deepStrictEqual(정책.심문런들(['--회차', '3']).map((p) => p.이름), ['sol', 'gemini'],
  ...
  assert.deepStrictEqual(정책.심문런들(['--검수', 'luna']).map((p) => p.이름), ['luna', 'gemini'],
    '명시 픽이 심문 회차를 ①검수 쪽 값으로 끌어갔다 — 값은 편성 하나에서만 나와야 한다');
  assert.deepStrictEqual(정책.심문런들(['--검수', 'luna', '--회차', '1']).map((p) => p.이름), ['luna']);
  /* 🔴 09-04 — `--검수` 는 «코덱스» 픽이다. 제미나이 자리까지 갈아끼우면 그 회차가 코덱스 모델로
   *   바뀌어 집안이 하나가 되고, 그러면 「두 집안이 봤다」가 거짓이 된다. */
  assert.strictEqual(정책.심문런들(['--검수', 'luna'])[1].model, 정책.제미나이설정().model,
  ...
  assert.deepStrictEqual(정책.심문런들(['--검수', 'luna']).map((p) => p.effort), ['xhigh', 'high'],
...
test('🚪 열림 대기 이름은 «쓰는 표»에 없다 — 있으면 조용히 죽을 이름을 아는 척하는 것이다', () => {
  const 대기 = Object.keys(정책.열림대기);
  assert.ok(대기.length > 0, '열림 대기 표가 비었다 — 감시할 이름이 0개면 정찰이 아무것도 안 본다');
  ...
  // 그 이름을 실제로 부르면 도구가 거절해야 한다(아는 척하지 않는다).
  assert.throws(() => 정책.코덱스플래그({ model: 'gpt-6-astra', effort: 'xhigh' }),
...
test('🔑 돈 열쇠가 필요한 제미나이 역할은 그렇다고 적혀 있다 — 공짜 몫 하루 20발로는 한 판도 안 도는 것들', () => {
  ...
  const 초벌 = 정책.제미나이역할.find((r) => /초벌/.test(r.이름));
  assert.ok(초벌 && 초벌.돈열쇠필요 === true, '콘텐츠 초벌이 공짜 몫으로 열려 있다 — 한 판 80요청이라 20발 한도를 첫 판에 넘긴다');
});
...
// ───────────────────────────────── ⑤배치표 — 일감 32자리가 각각 어느 손인가 (2026-09-04)
...
test('🔒 GPT 자리는 «루나 한 벌»이다 — 아스트라가 정식으로 열릴 때까지 (유호 확정 09-04)', () => {
  /* 유호 원문: "아스트라가 정식 출시하기전까지는 gpt로 심문이나 이런거 하는거 루나 1회로 바꾸자".
  ...
  /* 🚫 아스트라 이름이 «쓰는 표»에 미리 들어오는 것도 막는다(08-06) — 열리는 날 프로브로 재고 옮긴다. */
  assert.ok(!Object.keys(정책.코덱스효력).some((m) => /gpt-6/.test(m)),
    '아스트라 이름이 쓰는 표에 먼저 들어왔다 — 벤더가 거절하면 검수가 종료 2 로 죽는다');
});
