...
/* [2026-08-09 갱신] 유호님 확정: ①배포 검수 · ②설계 심문 **둘 다 luna/max, 각 2회**
 * (08-06 판의 ②=sol 은 「빈도가 낮으니 비싸도 된다」였고, luna 가 공짜가 되면서 전제가 바뀌었다).
 * 이 회귀가 지키는 것은 이제 **효력**이다 — 새는 방향은 언제나 얕은 쪽이고, 모델이 같아진 지금
 * 조용히 갈라질 수 있는 자리는 effort 하나뿐이다(모델만 박고 효력을 안 박아 데인 게 이 파일의 출발점). */
test('역할별 기본 — ①검수·②심문 첫 런 둘 다 luna/max (효력이 조용히 안 내려간다)', () => {
  for (const [이름, 설정] of [['심문', 정책.분석설정([], 정책.심문기본)], ['검수', 정책.분석설정([])]]) {
    assert.strictEqual(설정.model, 'gpt-5.6-luna', `${이름} 기본이 luna 가 아니다`);
    assert.strictEqual(설정.effort, 'max', `${이름} 효력이 max 가 아니다 — 모델만 맞고 얕게 돈다`);
  }
});

/* [2026-08-14] 유호님 확정: 심문·검수 **둘 다 1회 · luna/max**(원문 「심문 검수 둘다 1회로 줄이자
 * luna max로」). 08-09 2차의 `luna×2 + sol×1` 을 유호님이 직접 대체했다.
 * 편성이 조용히 바뀌는 것은 겉보기가 똑같다 — 방향이 뒤집혔어도 **목록째 못박는 규율은 그대로다**
 * (지금 막을 것은 「sol 이 빠지는 것」이 아니라 「아무도 안 시켰는데 다시 늘거나 줄어드는 것」이다). */
test('심문 편성 = luna/max ×1 · 검수(①)도 1회 — 유호님 확정 08-14', () => {
  const 런 = 정책.심문런들([]);
  assert.deepStrictEqual(런.map((p) => p.model), ['gpt-5.6-luna'],
    '심문 편성이 바뀌었다 — 유호님 확정값은 luna 한 벌이다');
  assert.deepStrictEqual(런.map((p) => p.effort), ['max'], 'max 가 아니면 유호님이 고른 깊이가 아니다');
  assert.strictEqual(정책.회차설정([]), 1, '검수 회차가 유호님 확정값(1)에서 벗어났다');
  assert.strictEqual(정책.분석설정([]).model, 'gpt-5.6-luna');
});

test('심문 편성: --회차 N 은 앞 N개 · --검수 X 는 그 픽으로 회차만큼', () => {
  // 편성이 한 벌이라 --회차 2 를 줘도 슬라이스가 1개다 — 「2회 돌렸다」는 착각이 안 생겨야 한다
  assert.deepStrictEqual(정책.심문런들(['--회차', '2']).map((p) => p.이름), ['luna']);
  assert.deepStrictEqual(정책.심문런들(['--회차', '1']).map((p) => p.이름), ['luna']);
  // 명시 픽은 편성을 이긴다(옛 1회성 통로 보존) — 그때 횟수는 검수 기본값 1
  assert.deepStrictEqual(정책.심문런들(['--검수', 'sol']).map((p) => p.이름), ['sol']);
  assert.deepStrictEqual(정책.심문런들(['--검수', 'sol', '--회차', '2']).map((p) => p.이름), ['sol', 'sol']);
  // 효력 1회성 조정이 **모든 런에** 걸린다
  assert.deepStrictEqual(정책.심문런들(['--효력', 'high']).map((p) => p.effort), ['high']);
  // 오타는 기본으로 접지 않는다
  assert.throws(() => 정책.심문런들(['--검수', 'lnua']), /모르는 검수 선택지/);
  assert.throws(() => 정책.심문런들(['--검수']), /뒤에 픽이 없다/);
});

/* 회차 — 「1회」는 유호님 확정값이라 기본이 조용히 2 로 올라가면 안 된다(올라가도 겉보기가 같다). */
test('회차: 기본 1 · --회차 로 1회성 조정 · 상한과 쓰레기값은 거절한다', () => {
  assert.strictEqual(정책.회차설정([]), 1, '기본 회차가 1 이 아니다 — 유호님 확정값');
