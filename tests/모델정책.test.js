...
/* ✅ 2026-08-29 유호님 확정 — 심문 = `luna/xhigh ×1`(원문 *"심문 루나 extra high 1회로 바꾸자"*).
 * 08-15 의 「심문 검수 모두 luna high ×2」 중 **심문 몫만** 대체됐다. 그래서 이 검사는 이제
 * ①과 ②가 **같음**이 아니라 **의도적으로 다름**을 못박는다 — 둘이 우연히 같아지면 그건 누가
 * 한쪽만 고치고 나머지를 따라 옮긴 것이라, 유호님이 가른 축이 조용히 접힌 상태다. */
test('심문 편성 = luna/xhigh ×1 — 유호님 확정 08-29 (①검수는 high ×2 그대로)', () => {
  const 런 = 정책.심문런들([]);
  ...
  assert.deepStrictEqual(런.map((p) => p.effort), ['xhigh'],
    'xhigh 가 아니면 유호님이 고른 깊이가 아니다 — 회차를 내주고 산 것이 이 한 칸이다');
  assert.strictEqual(정책.회차설정([]), 2, '검수 회차가 유호님 확정값(2)에서 벗어났다');
  assert.strictEqual(정책.분석설정([]).model, 'gpt-5.6-luna');
  assert.strictEqual(정책.분석설정([]).effort, 'high',
    '①검수까지 xhigh 로 따라 올라갔다 — 08-29 지시는 심문 몫이었다(매 배포에 걸리는 비용이 다르다)');
  /* 🔑 역할이 갈렸으니 **다름**을 못박는다 — 전엔 같음을 못박던 자리다(08-15). */
  assert.notStrictEqual(런.length, 정책.회차설정([]),
    '②심문 회차가 ①검수와 같아졌다 — 유호님이 08-29 에 가른 축이 접혔다');
});

test('심문 편성: --회차 N 은 편성을 늘리지 않는다 · 명시 픽도 심문 회차를 따른다', () => {
  // 편성이 한 벌이라 --회차 2·3 을 줘도 슬라이스는 1개 — 「2회 돌렸다」는 착각이 안 생겨야 한다
  ...
  // 명시 픽도 심문의 역할 기본 효력을 받는다 — sol 만 얕게(그 모델 기본 low) 도는 일이 없어야 한다
  assert.deepStrictEqual(정책.심문런들(['--검수', 'sol']).map((p) => p.effort), ['xhigh'],
    '명시 픽이 심문 효력을 안 받으면 「sol 로 깊게 봤다」고 믿으며 얕게 돈다');
  // 효력 1회성 조정이 역할 기본을 이긴다 (기본 xhigh 와 다른 값으로 재야 얹힘이 보인다)
