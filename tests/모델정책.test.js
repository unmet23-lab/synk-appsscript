...
test('심문 편성: --회차 N 은 편성을 «늘리지» 않는다 · 명시 픽은 «모델만» 간다', () => {
  ...
  assert.strictEqual(정책.심문런들(['--검수', 'sol'])[1].model, 정책.제미나이설정('최상').model,
...
test('🔒 ⓐ 키 명부 — 이름표가 바뀌거나 사라지면 여기가 운다(부르는 쪽이 조용히 깨지는 것을 막는다)', () => {
  ...
  const 명부 = [
    ...
    '대규모코드', '명령창붙들기', '엔진코드', '시험짓기',   // 09-07 잔손고침 접음(배치표 그 자리 주석)
    '배포전검수', '보안구멍', '거짓초록', '글검사', '밖의사실',   // 09-07 숨은명령 접음
    '화면조작', '웹조사', '시트점검',
...
test('🔴 ⑤ 검사 자리는 지은 쪽 없이 물으면 손을 «안» 준다 — 조용히 접으면 그게 곧 「지은 자가 검사」다', () => {
  /* 🔴 09-07 — 여기가 «13» 이라는 번호로 불렀다가 자리 둘이 접히면서 엉뚱한 자리를 재게 됐다.
   *   배치표 머리말이 「부르는 쪽은 키를 쓴다 · 번호는 자리를 끼우면 밀리는 값」이라고 이미 못 박고 있었는데
   *   이 시험만 그 규율 밖에 있었다. **재는 대상은 그대로 두고 부르는 법만 키로 옮긴다**(무르게 한 것이 아니다). */
  const [답] = 정책.배치('배포전검수');
