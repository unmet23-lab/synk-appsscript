...
test('기본 픽은 luna — 유호 확정 2026-09-05 "전부 다 luna 1회" · sol 은 그날만 올리는 손잡이로 남는다', () => {
...
test('심문 편성: --회차 N 은 편성을 «늘리지» 않는다 · 명시 픽은 «모델만» 간다', () => {
  ...
  assert.deepStrictEqual(정책.심문런들(['--회차', '2']).map((p) => p.이름), ['luna', 'gemini']);
  assert.deepStrictEqual(정책.심문런들(['--회차', '3']).map((p) => p.이름), ['luna', 'gemini'],
