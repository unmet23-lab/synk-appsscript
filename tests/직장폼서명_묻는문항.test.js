...
/* 🔴 [09-08 검수 6회차 P1 46f56e7949f2] 「고치고 다시 누르면 저절로 찍힌다」는 약속을 재는 자리.
 *   첫 실행이 ID 를 저장하므로 재실행은 «ID 없는 갈래»의 문항 재검사에 안 들어온다. 그 자리를 떼어 낸다. */
function 재검사블록뽑기() {
  const i = 폼.indexOf('if (완료값.indexOf(WORK_DONE_MISSING_) === 0) {');
  assert.ok(i > 0, '필수누락 재검사 블록을 못 찾았다 — 모양이 바뀌었으면 이 뽑기도 함께 고친다');
  const j = 폼.indexOf('\n      }', i);
  assert.ok(j > i, '재검사 블록의 닫는 괄호를 못 찾았다');
  return 폼.slice(i, j + 8);
}

/** 재검사 블록을 실제로 돌린다 — 돌려주는 것은 그 자리에서 정해진 완료값이다. */
function 재검사돌리기(폼객체, 옛완료값, 담을곳) {
  const { 묻는문항_ } = 서명만들기();
  const WORK_REQUIRED_ = new Function(필수소스[0] + '\nreturn WORK_REQUIRED_;')();
  const f = new Function('exForm', '묻는문항_', 'WORK_REQUIRED_', 'WORK_DONE_MISSING_', 'setState', 'st', 'Logger', '완료값',
    재검사블록뽑기() + '\nreturn 완료값;');
  return f(폼객체, 묻는문항_, WORK_REQUIRED_, WORK_DONE_MISSING_, (_st, k, v) => { 담을곳[k] = v; }, {}, { log() {} }, 옛완료값);
}

test('🔴 ⑨ 문항을 채우고 다시 누르면 도장이 «저절로» 찍힌다 (검수 6회차 P1 46f56e7949f2)', () => {
  /* 앞 판은 이 약속을 못 지켰다. 첫 실행이 ID 를 저장해 재실행이 문항 재검사에 안 들어왔고,
   *   그래서 유호님이 빠진 문항을 다 더하고 다시 누르셔도 같은 경고만 영영 반복됐다. */
  const 담김 = {};
  const 결과 = 재검사돌리기(가짜폼(TITLE, 진짜문항), '필수누락:자료활용동의', 담김);
  assert.strictEqual(결과, 'y', '문항을 다 채웠는데도 도장이 안 찍히면 안내가 거짓말이 된다');
  assert.strictEqual(담김['직장폼완료'], 'y', '도장을 app_state 에 다시 적어야 다음 실행이 지나간다');
});

test('⑩ 아직 빠져 있으면 그대로 막고, «무엇이» 빠졌는지 갱신한다', () => {
  const 담김 = {};
  const 아직 = 가짜폼(TITLE, [
    ['일한 곳', T.TEXT], ['무슨 일', T.PARAGRAPH_TEXT], ['시킨 일 그대로', T.PARAGRAPH_TEXT],
    ['예정에 없던 일이 생긴 적', T.PARAGRAPH_TEXT],
    ['자료활용동의', T.SECTION_HEADER],                 // 묻는 자리가 아니다 — 아직 빠진 것이다
  ]);
  const 결과 = 재검사돌리기(아직, '필수누락:일한 곳·자료활용동의', 담김);
  assert.strictEqual(결과, '필수누락:자료활용동의', '다시 세어 «지금» 빠진 것으로 갱신해야 한다');
  assert.strictEqual(담김['직장폼완료'], '필수누락:자료활용동의');
});
