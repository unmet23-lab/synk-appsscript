...
const 갯수하한 = 51;   /* 교체 직전 실측 53에서 «의도적 퇴역 2»를 뺀 값(09-03). 아래로 내려가면
...
test('🔴 실저장소 — 넓힌 분모가 그대로 산다(표식 28종 = #Q100 22 + Ⅰ-④ 상담로그 + scene_log + 발음 사건 원장 둘 + 동의 이력 + 순간 · 신규 전건)', () => {
  ...
  const 신규 = ['point_logs', 'attendance', 'attendance_batch', 'achievements', 'weekly_topics',
    ...
    'voice_labels', 'voice_delivery', 'consent_log', 'portfolio_moments'];
  const 빠짐 = 신규.filter((n) => 수집.indexOf(n) === -1);
  assert.deepEqual(빠짐, [], `넓힌 수집 탭이 분모 밖으로 나갔다 — ${빠짐.join(', ')}`);
  assert.equal(수집.length, 28, `수집 표식 ${수집.length}종 — 28이 아니면 판정표(docs/엔진도달_설계.md §8-0 · 그 문서는 파생이라 코드가 정본)와 갈라졌다`);
