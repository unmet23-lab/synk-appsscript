...
/* ── 「모름」 안이 또 갈린다 (F579 · #Q110 · 2026-08-17) ────────────────────
 * 형제 쪽은 이미 `미배포`(=재봤더니 라이브에 없다 · HTTP 404)를 `--json` 으로 내주는데
 * 이 화면이 안 읽어서 「안 잰 것이지 최신이 아니다」라고만 말했다. 그건 덜 참되다 — 쟀고,
 * 없다는 걸 안다. 처방도 갈린다: 「안 쟀다」는 다시 재면 풀리지만 「없다」는 재도 같다. */

test('🕳 «라이브에 없음» 을 갈라서 낸다 — 처방이 대조가 아니라 배포다 (F579 · #Q110)',
  { skip: !git있나 && 'git 없음' }, () => {
    const { repo, state } = 픽스처();
    const 형제 = 픽스처();
    빚도구세운다(형제, 빚JSON({
      리허설: { 전체: 16, 낡음: ['auth'], 최신: 15, 모름: [], 미배포: [] },
      운영: { 전체: 16, 낡음: [], 최신: 14, 모름: ['companion', 'radio-round'], 미배포: ['companion', 'radio-round'] },
    }));
    const out = 돌린다({ repo, state, 나: 'local_q110a001', 형제: [형제.repo], 인자: ['--hook'] });
    assert.match(out, /모름 2\(그중 라이브에 없음 2\)/,
      `「모름」 안을 안 갈랐다 — 다시 재도 안 풀리는 것과 재면 풀리는 것이 한 칸에 있다:\n${out}`);
    assert.match(out, /배포된 적이 없다/, `「없다」를 「안 쟀다」로만 말한다:\n${out}`);
    assert.match(out, /처방은 대조가 아니라 \*\*배포\*\*/,
      `처방이 안 갈렸다 — 「재라」는 이 칸에서 영영 안 꺼진다(F103):\n${out}`);
    /* 🔴 분모 등식은 그대로다 — `미배포` 는 `모름` 의 **부분집합**이라 더하지 않는다.
     *   더하면 이 줄이 이중 계상되고, 그 순간 화면의 산술이 조용히 틀린다. */
    assert.match(out, /전체 16 = 낡음 0 \+ 최신 14 \+ 모름 2/,
      `분모 등식이 깨졌다(미배포를 더했다):\n${out}`);
  });

test('🕳 미배포가 0이면 그 줄을 안 낸다 — 거짓양성이 진짜를 덮지 않는다 (#Q110)',
  { skip: !git있나 && 'git 없음' }, () => {
    const { repo, state } = 픽스처();
    const 형제 = 픽스처();
    빚도구세운다(형제, 빚JSON({
      리허설: { 전체: 16, 낡음: ['auth'], 최신: 14, 모름: ['tasks'], 미배포: [] },
      운영: { 전체: 16, 낡음: [], 최신: 16, 모름: [], 미배포: [] },
    }));
    const out = 돌린다({ repo, state, 나: 'local_q110a002', 형제: [형제.repo], 인자: ['--hook'] });
    assert.match(out, /모름 1/, `모름 자체가 사라졌다:\n${out}`);
    assert.doesNotMatch(out, /라이브에 없음/, `미배포 0인데 있다고 말한다:\n${out}`);
    assert.doesNotMatch(out, /배포된 적이 없다/, `미배포 0인데 그 처방을 낸다:\n${out}`);
  });

test('🚫 갚기 안내가 «--운영» 을 권하지 않는다 — 조용히 삼켜지고 F462 에도 어긋났다 (#Q110)',
  { skip: !git있나 && 'git 없음' }, () => {
    /* 🔴 2026-08-17 실측: `node tools/배포빚.js --운영` 은 인자 없을 때와 **출력이 한 글자도
     *   다르지 않다**(그 도구는 원래 두 판을 다 내고, 모르는 플래그를 조용히 삼킨다). 그러면서
     *   F462 가 금지한 「읽기에 쓰기 승인 키 붙이기」를 정확히 권하고 있었다. 이 회귀가 없으면
     *   그 문구는 「운영도 봐야 하니까」라는 옳은 직관을 타고 **되살아난다.** */
    const { repo, state } = 픽스처();
    const 형제 = 픽스처();
    빚도구세운다(형제, 빚JSON({
      리허설: { 전체: 16, 낡음: ['auth'], 최신: 15, 모름: [], 미배포: [] },
    ...
    const out = 돌린다({ repo, state, 나: 'local_q110a003', 형제: [형제.repo], 인자: ['--hook'] });
    const 갚기줄 = out.split('\n').find((l) => l.includes('갚기:'));
    assert.ok(갚기줄, `갚기 안내가 통째로 사라졌다 — 무엇이 낡았는지만 알고 어떻게 갚는지를 안 준다:\n${out}`);
    assert.doesNotMatch(갚기줄, /--운영/,
      `읽기 도구에 쓰기 승인 키를 권한다(F462) — 게다가 그 플래그는 아무 일도 안 한다: ${갚기줄}`);
    assert.match(갚기줄, /두 판\(리허설·운영\)을 한 번에/,
      `두 판을 한 번에 낸다는 사실을 안 밝혔다 — 그러면 읽는 쪽이 판마다 따로 돌릴 방법을 찾는다: ${갚기줄}`);
  });
...
/* ── 대기 가지가 많아지면 «접는다» — 그래도 경고 축은 운다 (2026-08-19 · 유호 확정 ②) ──────
 * 🔴 실측: 실저장소 대기 가지 28개일 때 이 훅 하나가 **15,726 B**를 냈다(SessionStart 총량의 76%).
 *   가지마다 제목 + 충돌 파일 목록을 전부 폈기 때문이다. 그런데 「깨끗이 붙는다」는 반입을
 *   «정할 때» 볼 것이지 세션 시작마다 볼 것이 아니고, 상주 비용은 남은 턴 수만큼 곱해진다.
 *
 * ⚠ 접는 것이 위험한 이유는 이 절이 **사각을 메우려고** 생겼기 때문이다(track-collision 은 로컬
 *   HEAD 만 본다). 그래서 접히는 것은 «괜찮은 것»뿐이고, 아래가 그 경계를 기계로 문다.
 *   위 F192 검사는 가지가 적을 때(=펴는 쪽)를 재므로, 접는 쪽은 여기서 따로 잰다 —
 *   안 그러면 접기 로직이 통째로 회귀 밖에 산다(F207: 안 본 것과 깨끗한 것이 같은 초록).
 *
 * 대가: 한도(6)를 넘겨야 접히므로 픽스처가 그만큼 가지를 만들어야 한다. 닫은 것 = 「많을 때」가
 *   회귀 밖에 있던 상태.
 */
test('🔑 대기 가지가 한도를 넘으면 접되, 충돌·공유 선언판·못 쟀다는 계속 운다', () => {
  const 뼈대 = { git못부름: false, 나: 'local_me00', 항목: [], 세션수: 1, 못본형제: [], 보드: [], 뒤늦음: { 항목: [], 못잼: 0 } };
  const 가지 = (i, 충돌) => ({이름: `origin/claude/g${i}`, 언제: '1 hour ago', 제목: `제목${i}`, 충돌});
  // 깨끗 8 · 충돌 1(공유 선언판 포함) · 못 쟀다 1 = 10개 → 한도 6 을 넘어 접힌다.
  const 원격 = [
    ...Array.from({ length: 8 }, (_, i) => 가지(i, [])),
    가지(8, ['docs/세션보드.md', 'tools/a.js']),
    가지(9, null),
  ];
  const 접힌 = 소유자.보고({ ...뼈대, 원격 }, { 훅: true });

  // ① 접혀도 «세 갈래»는 여전히 갈린다 — 모름을 깨끗함으로 접으면 그게 이 파일의 최우선 위반이다.
  assert.match(접힌, /깨끗이 붙는 것 8개/, '깨끗한 것의 개수를 안 알렸다 — 접는 것과 안 세는 것은 다르다');
  assert.match(접힌, /못 쟀다/, '못 잰 가지가 접힘 속으로 사라졌다 — 0건과 같은 모양이 됐다(F192)');
  assert.ok(!/충돌 \*\*0\*\*/.test(접힌.split('못 쟀다')[1] || ''), '모름을 0건으로 표시했다');

  // ② 경고 축은 **이름으로** 계속 운다 — 접기가 사각을 다시 만들면 안 된다.
  assert.match(접힌, /origin\/claude\/g8/, '충돌 나는 가지를 이름으로 안 불렀다 — 접기가 경고를 삼켰다');
  assert.match(접힌, /2개 파일 충돌/, '충돌 규모를 안 알렸다');
  assert.match(접힌, /공유 선언판[\s\S]*F187/, '공유 선언판 되돌림 위험이 접힘 속으로 사라졌다');

  // ③ 접었으면 «펴는 문»을 준다 — 안 주면 따를 수 없는 처방이 된다(F103).
  assert.match(접힌, /--가지/, '전문을 보는 통로를 안 알렸다');

  // ④ 접힌 쪽은 실제로 짧다 — 이게 이 변경의 목적이다(펴면 가지마다 제목·충돌 파일이 다시 붙는다).
  const 편것 = 소유자.보고({ ...뼈대, 원격 }, { 훅: true, 가지전문: true });
  assert.ok(접힌.length < 편것.length,
    `접었는데 안 짧아졌다(${접힌.length} vs ${편것.length}) — 접기 로직이 안 걸린 것이다`);
  assert.match(편것, /제목8/, '전문인데 가지 제목이 없다 — --가지 가 아무것도 더 안 준다');
  assert.ok(!/제목0/.test(접힌), '접혔는데 깨끗한 가지 제목이 실렸다 — 접기가 안 걸렸다');
});
