...
test('🔴 살아있는 세션의 인계문은 **이름만으로** 주인이 잡힌다 (만진 기록 0)', { skip: !git있나 && 'git 없음' }, () => {
  const { repo, state } = 픽스처();
...
/** 배포 커밋(`[vN]`) 하나 + 그 뒤 커밋 한 개. `.claspignore` 가 없어 폴백 패턴(`*.js`)이 선다. */
function 배포픽스처(뒤에고칠파일) {
  const { repo, g } = 픽스처();
  fs.writeFileSync(path.join(repo, 'Code.js'), 'ㄱ\n');
  fs.writeFileSync(path.join(repo, '문서.md'), 'ㄱ\n');
  g('add', 'Code.js', '문서.md'); g('commit', '-qm', '[v9.100] 배포 커밋');
  if (뒤에고칠파일) {
    fs.writeFileSync(path.join(repo, 뒤에고칠파일), 'ㄴ\n');
...
test('🔑 기준은 태그가 아니라 커밋 제목이다 — 엉뚱한 커밋에 붙은 태그에 안 속는다',
  { skip: !git있나 && 'git 없음' }, () => {
    const { repo, g } = 픽스처();
    fs.writeFileSync(path.join(repo, 'Code.js'), 'ㄱ\n');
