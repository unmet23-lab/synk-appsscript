...
test('목록에 현행 정본 폴더가 들어 있다 — 빠지면 그 세트가 통째로 안 보인다', () => {
  assert.ok(마스코트폴더.includes('펠트코랄_0815'), '현행 정본(펠트코랄_0815)이 목록에서 빠졌다');
});

test('🔴 퇴역 세트도 «알아보기»는 한다 — 빼면 구판 경로에 린트가 눈을 감는다', () => {
  /* 체리 퇴역(08-19)으로 파일은 지웠지만 이름은 남긴다. 알아보는 목록과 허용 목록은 다르다 —
   * 여기서 빼면 누가 옛 경로를 다시 적었을 때 「마스코트가 아니다」로 통과해 버린다. */
  const { 퇴역폴더 } = require('../tools/lib/마스코트자산');
  for (const 옛 of 퇴역폴더) {
    assert.ok(마스코트폴더.includes(옛), `퇴역 세트 ${옛} 가 «알아보는» 목록에서도 빠졌다`);
    assert.ok(마스코트경로다(`docs/캐릭터/${옛}/본체.png`), `${옛} 경로를 못 알아본다`);
  }
});
