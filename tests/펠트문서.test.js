...
/* ── 죽은 천 변수 — 결이 «조용히» 빠진다 (실측 2026-08-19) ──────────────────
 * 중성축 개명(Cream→Chalk · 08-18)이 스킨의 `var(--천-cream)` 소비처를 못 따라가
 * 표지 오브가 결 없는 민 면으로 렌더됐다 — CSS 는 죽은 var 를 에러 없이 삼키므로
 * (background-image 가 그냥 안 그려진다) 어느 검사도 빨개지지 않았다. 맞는 얼굴로 틀린 값.
 * ⚠대가: 이 눈은 `--천-` 접두 변수만 본다 — 다른 접두의 죽은 var 는 여전히 못 잡는다
 *   (닫을 것: 전 접두 일반화는 스킨 밖 var 소비처가 하나 더 생기는 날 그 자리에서).
 * 반박 패스 L-1 로 닫은 맹점 2형: ①폴백형 `var(--천-x, none)` 이 참조 정규식을 비껴갔다
 *   ②주석 속 `--천-x:` 가 산 정의로 세어졌다 — 그래서 주석을 벗기고 `[,)]` 까지 문다. */
function 죽은천변수(css) {
  const 몸 = css.replace(/\/\*[\s\S]*?\*\//g, '');
  const 정의 = new Set([...몸.matchAll(/--천-([a-z0-9]+)\s*:/g)].map((m) => m[1]));
  return [...몸.matchAll(/var\(--천-([a-z0-9]+)\s*[,)]/g)].map((m) => m[1])
    .filter((n) => !정의.has(n));
}

test('스킨이 참조하는 천 변수는 전부 정의돼 있다 — 죽은 var(--천-…) 금지', () => {
  const css = 도구.스킨();
  const 참조 = [...css.matchAll(/var\(--천-([a-z0-9]+)\)/g)];
  assert.ok(참조.length >= 1, '픽스처 확인 — 스킨에 천 참조가 하나는 있어야 한다(없으면 이 검사가 헛것이다)');
  assert.deepStrictEqual(죽은천변수(css), [],
    '스킨이 정의 안 된 천 변수를 참조한다 — 오브 결이 조용히 빠진 채 렌더된다(08-19 실측: --천-cream)');
});

test('탐지력 픽스처 — 천 정의 한 줄을 지우면 죽은천변수가 빨개진다', () => {
  const css = 도구.스킨();
  const 병든판 = css.replace(/--천-[a-z0-9]+:/, '--천죽임:');
  assert.ok(병든판 !== css, '픽스처 확인 — 지울 천 정의가 스킨에 있어야 한다');
  assert.ok(죽은천변수(병든판).length >= 1, '천 정의를 지웠는데 검사가 조용하다 — 탐지력이 없다');
});
