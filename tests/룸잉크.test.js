...
test('잉크 부품 목록은 `loom.css()` 에서 «캔다» — 손 목록은 낡고, 낡은 분모는 조용히 통과한다', () => {
  ...
  /* 🔑 대표 둘은 «지금 실제로 잉크를 지는» 부품이라야 한다.
     09-03 에 「칩」이 여기서 빠졌다 — 분류 칩이 배경을 걷고 «표식»으로 가면서 글자색을
     `inherit` 로 넘겼기 때문이다(유호 확정 · 결정.md). 즉 목록이 낡은 게 아니라 «사실이 바뀐» 것이다.
     ⚠그래서 「칩」을 «히어로수»로 갈았다. 이 줄이 지키려는 것은 특정 부품 이름이 아니라
       「목록을 손으로 안 들고 CSS 에서 캔다」이므로, 대표는 살아 있는 부품이면 된다. */
  assert.ok(부품.has('번호') && 부품.has('히어로수'), `대표 부품이 빠졌다: ${[...부품].join('·')}`);
  for (const c of 부품) assert.ok(/[가-힣]/.test(c), `한글 부품이 아닌 것이 섞였다: ${c}`);
});

/* ── ④-b 인쇄 규칙은 «구움이 덮을 것»을 선언하지 않는다 (2026-09-04 신설) ──────────
 * 🔴 왜 있나 — `문서` 지면의 조립 순서는 «낮(인쇄) 뒤 구움»이고, 구움은 언제나 마지막이다.
 *   그래서 `@media print` 가 구움이 다시 쓰는 성질을 선언하면 **뒤가 이기는 CSS 성질대로 죽는다.**
 *   09-04 실측: 인쇄 규칙이 칩에 배경·테두리를 주고 있었는데 구움표의 투명 배경이 그것을 덮어
 *   **아무 일도 안 하고 있었다**(인쇄 855,537 · 구움 958,306 · codex e55a9bbbad23).
 *   게다가 그 선언 자체가 유호 확정 09-03(칩은 배경을 걷고 표식으로 간다) «전»의 옛 판이었다.
 * 🔑 소스에 그 글자가 있나가 아니라 **조립된 CSS 에서 «누가 뒤에 오나»**를 잰다 —
 *   글자만 보면 순서가 바뀌어도 초록이다(test-guards-the-defect). */
test('🔴 인쇄 규칙이 «구움이 덮을» 칩 배경을 선언하지 않는다 — 맞는 얼굴로 틀린 값이던 자리', () => {
  const css = loom.css({ 지면: '문서' });
  const i = css.indexOf('@media print');
  assert.ok(i > 0, '문서 지면에 @media print 가 없다 — 이 시험의 과녁이 사라졌다');

  // 인쇄 블록 안에서 .칩 에 배경을 주는 선언을 찾는다
  const 인쇄블록 = css.slice(i);
  const 칩배경 = /\.칩\s*\{[^}]*background\s*:/.test(인쇄블록);
  assert.ok(!칩배경,
    '인쇄 규칙이 칩에 배경을 준다 — 구움이 뒤에서 덮어 «아무 일도 안 하는» 선언이 되고, '
    + '유호 확정 09-03(칩은 배경을 걷는다)과도 어긋난다');

  // 그리고 «구움이 정말 뒤에 오는가»를 같이 확인한다 — 이 전제가 깨지면 위 판정의 근거가 사라진다
  const 구움 = css.search(/\.칩\s*\{[^}]*background-color\s*:\s*transparent/);
  if (구움 >= 0) {
    assert.ok(구움 > i,
      `구움이 인쇄보다 «앞»에 왔다(구움 ${구움} · 인쇄 ${i}) — 조립 순서가 바뀌었다면 이 절의 판정을 다시 세운다`);
  }
});
