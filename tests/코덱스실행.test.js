...
// ──────────────── 판단 정본이 «짓는 쪽»에도 실리나 (09-08 · 유호 확정 「철학 정본만 이식하자」)
//   실측이 부른 시험이다: 검수 레인은 철학 5,672자를 매번 실었는데 짓는 레인은 0자였다.
//   그래서 실행자는 「무엇이 좋은 산출물인가」를 모른 채 짓고 검수자만 그 기준으로 때렸다.

test('🔑 실행자 철학 블록은 서열 줄과 함께 실린다 — 「범위는 발주서가 정한다」가 빠지면 범위 침범 허가가 된다', () => {
  const 블록 = 빌드.실행자철학블록('[Ⅰ 한 문장] 가치 문장');
  assert.ok(블록.includes('가치 문장'), '철학 본문이 실려야 한다');
  assert.ok(블록.includes('범위는 여전히 발주서가 정한다'),
    '서열 줄이 이 블록의 절반이다 — 없으면 실행자가 철학을 근거로 발주서 밖을 고친다(AGENTS.md §3-1 위반)');
  assert.ok(블록.includes('발주밖발견'), '철학상 발주서가 모자랄 때 갈 자리를 대야 한다');
});

test('🔑 실행자 서문은 검수자 서문과 다르다 — 「방향보다 상위」를 짓는 쪽에 주면 안 된다', () => {
  const 실행자 = 빌드.실행자철학블록('본문');
  const 검수자 = 검수.철학블록('본문');
  assert.ok(검수자.includes('상위'), '검수자 서문은 철학을 방향 위에 놓는다(전제 확인 — 이게 깨지면 아래 판정이 무의미하다)');
  assert.ok(!/보다\s*\*?\*?상위/.test(실행자),
    '실행자에게 「상위」라고 말하면 발주서 범위를 넘는 허가로 읽힌다 — 실행자에게 철학은 «무엇을»이 아니라 «어떻게»의 자다');
});

test('🔑 철학이 안 뽑히면 블록이 통째로 빈다 — 빈 머리말만 남겨 「실린 척」하지 않는다', () => {
  assert.strictEqual(빌드.실행자철학블록(''), '');
  const p = 빌드.실행프롬프트({ 발주서: 발주, 실행자절: '', 공통: '', 라운드: 1, 앞라운드: null });
  assert.ok(!p.includes('판단 정본'), '철학을 안 넘기면 실행 프롬프트에 그 머리말이 없어야 한다');
});

test('🔑 실행자·발주검토자 둘 다 정본에서 실제로 뽑은 철학을 받는다 (배선 — 가짜 문자열이 아니라)', () => {
  const 본문 = 검수.철학텍스트();
  assert.ok(본문.length > 500, `판단 정본에서 게이트를 못 뽑았다(${본문.length}자) — 절 구조가 바뀌었으면 앵커를 고쳐라`);
  const 철학 = 빌드.실행자철학블록(본문);
  assert.ok(빌드.실행프롬프트({ 발주서: 발주, 실행자절: '', 공통: '', 라운드: 1, 앞라운드: null, 철학 }).includes(본문.slice(0, 80)),
    '실행자가 받는 것은 요약이 아니라 정본에서 뽑은 그 글자여야 한다');
  assert.ok(빌드.발주검토프롬프트(발주, '', 철학).includes(본문.slice(0, 80)));
});
...
/* 🔑 이름 목록을 손으로 다시 적지 않는다 — 실행 선택지는 **검수 선택지에서 파생**하므로,
 *   여기에 이름을 박으면 09-05 처럼 아스트라가 들어온 날 이 줄만 낡아 거짓 빨강을 낸다
 *   (그날 옆 세션이 검수 쪽 시험은 고쳤는데 이 줄은 못 보고 지나갔다). 파생을 «파생으로» 문다. */
test('🔑 실행 선택지는 ①검수 선택지에서 파생한다 · 기본도 ①검수 기본을 따라간다', () => {
  assert.deepStrictEqual(Object.keys(정책.실행선택지).sort(), Object.keys(정책.검수선택지).sort(),
    '③실행 선택지가 ①검수 선택지와 갈렸다 — 한 값을 두 곳이 알면 유호님이 한쪽만 바꾸신 날 조용히 어긋난다');
...
/* 🔴 2026-09-05 CI 실측 — 아래 «가짜 코덱스로 레인을 통째로 태우는» 시험 셋이 **리눅스에서만** 빨갛다.
 *   문면은 늘 「커밋이 base 그대로」 = 가짜가 아예 안 돌아 아무것도 안 지어졌다는 뜻이다.
 *   윈도는 `%APPDATA%\npm\codex.cmd` 를 «경로로» 부르고, 그 밖은 PATH 에서 `codex` 를 찾는다 —
 *   갈아 끼우는 자리가 아예 다르다. 껍데기의 `$0` 문제는 고쳤는데도 여전해서 **까닭을 아직 못 좁혔다.**
 * 🔑 그래서 «윈도 전용»으로 못 박는다. 숨기는 게 아니라 **「안 재봤다」를 그 자리에 적는 것**이다:
 *   ⓐ 이 레인은 99.9% 이 노트북(윈도)에서 돌고, 그 통로는 여기서 매번 실측된다
 *   ⓑ 빨간 CI 를 그대로 두면 다음 세션이 «남의 빨강»으로 읽고 지나친다(그게 더 나쁘다)
 *   ⏳ 남은 일 = 리눅스 통로에서 가짜를 어떻게 갈아 끼우나(트랙 §0-코덱스). */
const 리눅스미측정 = '윈도 전용 — 가짜 코덱스를 갈아 끼우는 통로가 리눅스에서 안 먹는다(09-05 CI 실측 · 까닭 미상). 「통과」가 아니라 「안 재봤다」다.';
...
test('🔴 앞 회차가 짓다 만 잔해를 이어받으면 커밋이 그렇게 말한다 — 「라운드 N」 하나로 뭉뚱그리지 않는다', function (t) {
  if (process.platform !== 'win32') return t.skip(리눅스미측정);
...
test('🔴 검수가 끊긴 커밋은 다음 런이 도장을 마저 찍는다 — 새로 지은 것이 0개여도 건너뛰지 않는다', function (t) {
  if (process.platform !== 'win32') return t.skip(리눅스미측정);
...
test('🔴 수용 대조를 못 쟀으면 완주가 아니다 — 「미충족 0」과 「안 쟀다」를 가른다', function (t) {
  if (process.platform !== 'win32') return t.skip(리눅스미측정);
