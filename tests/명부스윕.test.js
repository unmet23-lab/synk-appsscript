...
/* ⑥-b 🔴 **「편성이 없다」와 「모른다」는 다른 몸통이다**(반박 P2-④ · 앞판 뒤집음).
 *   앞판은 셋을 다 빈 스냅샷으로 보냈고 그 근거는 「키를 빼면 서버가 못 비운다」였다 — 비우는
 *   길은 맞았지만 **모르는 자리까지 같이 비웠다.** 시즌 셀 하나가 지워지면 groups 에 편성이
 *   멀쩡히 있는데도 `[]` 가 나가 전원 조 해제가 되고, 그건 사람이 시트를 잘못 만진 흔한 사고다.
 *   ⇒ 비우는 자리(시트·시즌을 **아는데** 편성이 0)는 그대로 `[]`, 모르는 자리(시트 없음·시즌
 *     미설정)는 **키를 뺀다**(서버 계약에서 키 부재 = 무동작). */
test('명부스윕 ⑥-b 미편성은 빈 스냅샷 · 모름(시트 없음·시즌 미설정)은 키를 뺀다', () => {
  const 미편성 = 판({ 조표: [] });
  미편성.fn();
  assert.deepEqual(JSON.parse(미편성.요청[0].opt.payload).조편성, [],
    '빈 groups 가 빈 스냅샷이 아니다 — 편성이 0인 사실을 서버가 못 받으면 옛 조가 영영 남는다');

  const 시트없음 = 판({});
  시트없음.fn();
  assert.equal('조편성' in JSON.parse(시트없음.요청[0].opt.payload), false,
    'groups 시트가 없는데 빈 스냅샷을 보냈다 — 「모른다」가 「전부 지워라」가 된다');

  /* 급소: 편성은 있는데 **시즌만** 못 읽는 판. 앞판이 전원 해제를 내던 바로 그 자리다. */
  const 시즌없음 = 판({
    시즌: '',
    조표: [['S1', 'A반', 'SYNK-001', '바트', 1, 2, '확정', '', '', '']],
  });
  시즌없음.fn();
  assert.equal('조편성' in JSON.parse(시즌없음.요청[0].opt.payload), false,
    '시즌 셀이 지워졌을 뿐인데 편성 있는 판에서 빈 스냅샷이 나갔다 — 아침 배치가 반을 통째로 흩뜨린다');
});
...
/* ⑥-d 🔴 **대량 조 해제는 여태 아무도 안 울렸다**(반박 P2-④). 위 무동작 갈래가 원인 대부분을
 *   막지만, 막는 것과 **보이는 것**은 다른 일이다 — 남은 원인(시트가 비었다·번호가 전부 안 맞다)은
 *   시트 쪽이라 우리가 못 막는다. 해제만 나고 배정이 0이면 그건 재편성이 아니라 「통째로 비었다」다. */
test('명부스윕 ⑥-d 해제만 있고 배정이 0이면 운다 — 조용히 반이 흩어지면 아무도 못 센다', () => {
  const p = 판({ 응답: JSON.stringify({ ok: true, 문제들: [], 막힘: [], 조갱신: [], 조해제: 14 }) });
  p.fn();
  assert.equal(p.메일.length, 1, '학생 14명의 조가 해제됐는데 알림이 0이다');
  assert.match(p.메일[0].본문, /해제/u, '알림 본문이 조 해제를 말하지 않는다');
  assert.match(p.메일[0].본문, /14/u, '몇 명인지 안 센다 — 원장은 규모를 모른 채 시트를 뒤진다');
  assert.match(p.상태.명부스윕_상태, /\/u14(?=[/#])/, '서명이 통째 해제를 안 센다 — 다음 아침 조용해진다');
});

test('명부스윕 ⑥-e 정상 재편성(해제+배정)은 안 운다 — 매 아침 우는 알림은 안 읽힌다', () => {
  const p = 판({ 응답: JSON.stringify({ ok: true, 문제들: [], 막힘: [], 조갱신: ['SYNK-001'], 조해제: 3 }) });
  p.fn();
  assert.equal(p.메일.length, 0, '조를 바꾼 아침마다 운다 — 그런 알림은 곧 안 읽힌다');
  assert.equal(p.상태.명부스윕_상태, '', '정상 재편성이 문제 서명으로 굳었다');
  // 해제가 0인 판도 조용해야 한다(탐지 픽스처 — 위 ⑥-d 가 「항상 운다」로 통과하는 것을 막는다)
  const 없음 = 판({ 응답: JSON.stringify({ ok: true, 문제들: [], 막힘: [], 조갱신: [], 조해제: 0 }) });
  없음.fn();
  assert.equal(없음.메일.length, 0, '아무 일도 없는 아침에 울었다');
});
