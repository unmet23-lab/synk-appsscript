...
test('자정을 넘긴 재처리는 보상을 다시 안 태운다 — 어제 지급은 오늘 장부로 못 보니 두 번 줄 수 있다(4차 P1)', () => {
  ...
  assert.deepEqual(첫.지급, ['S1'], '첫 적재는 날짜와 무관하게 지급 재료다(늦은 스위프)');
  const 재처리 = 스위프돌리기({ 응답행들: [어제], 기존로그: 첫.행들 });
  assert.equal(재처리.행들.length, 1, '어제 응답이 다시 쌓였다');
  assert.deepEqual(재처리.지급, [], '어제 응답의 재처리가 오늘 보상 재료로 넘어갔다 — 두 번 준다');
  assert.equal(재처리.포인터, '2');
});

test('응답시각 칸이 없던 판의 행(구형)도 같은 날·같은 답이면 재처리로 본다 — 옛 판이 적재 뒤 죽은 채 올라온 첫 스위프(4차 P1)', () => {
  ...
  const 재처리 = 스위프돌리기({ 응답행들: [지금], 기존로그: [구형] });
  assert.equal(재처리.행들.length, 1, '구형 행과 같은 응답이 시도 2 로 또 쌓였다');
  assert.deepEqual(재처리.지급, ['S1']);
  /* 같은 날 다른 답은 진짜 재제출 — 구형 행이 시도 1 이므로 시도 2 */
  const 새답 = 스위프돌리기({ 응답행들: [지금, 응답(0.0005, 'Q1', '에')], 기존로그: [구형] });
  assert.equal(새답.행들.length, 2);
  assert.equal(새답.행들[1][열('시도')], 2); assert.equal(새답.행들[1][열('고른답')], '에');
});
