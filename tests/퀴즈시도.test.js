...
test('🔴 지급은 «응답의 날» 이후 지급이 있으면 건너뛴다 — 자정 경계·늦은 적재·재처리가 같은 답을 낸다(5차 P1 둘)', () => {
  ...
  /* ④ 한 호출 안 같은 학생 셋 — 같은 날 둘은 1회, 어제 몫은 따로 1회(밀린 배치가 이틀치를 한 번에 읽어도 날마다 1회 · 소넷 검토 09-06) */
  assert.deepEqual(지급돌리기({ 행들: [응답행('S3', 날짜(지금)), 응답행('S3', 날짜(지금)), 응답행('S3', 날짜(어제))] }), ['S3', 'S3']);
  /* ④-2 어제 몫을 먼저 준 뒤 오늘 몫 — 「어제 이후 지급 있음」이 오늘을 막지 않는다(날짜 키) */
  assert.deepEqual(지급돌리기({ 행들: [응답행('S6', 날짜(어제)), 응답행('S6', 날짜(지금))] }), ['S6', 'S6']);
  /* ⑤ 제출일이 빈 행은 실행일로 본다 */
