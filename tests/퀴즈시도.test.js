...
test('🔴 같은 응답을 다시 읽어도(적재 뒤 포인터 저장 실패 → 재스위프) 새 시도로 안 센다 — 정체는 폼이 찍은 시각이다', () => {
   ...
   * 순서는 엔진이 넘기는 순서 = 새 적재(둘째·셋째) 뒤에 재처리(원본) — out.concat(재처리)(7차 P2) */
  assert.deepEqual(재제출.지급, ['S1@' + 날짜(둘째[0]), 'S1@' + 날짜(셋째[0]), 'S1@' + 날짜(원본[0])], '지급 재료는 적재 행 + 재처리 행 전부다(하루 1회 상한은 지급 함수가 쥔다)');
});
...
test('🔴 지급은 «응답의 날» 이후 지급이 있으면 건너뛴다 — 자정 경계·늦은 적재·재처리가 같은 답을 낸다(5차 P1 둘)', () => {
  ...
  /* ④ 한 호출 안 같은 학생 셋(오늘 둘·어제 하나) → 1회. 「하루 1회」의 하루는 **지급일**이다 — 야간 dailyGuard 가 지급일로 같은 상한을 다시 재서,
   *   응답일마다 주면 밤에 회수된다(코덱스 v9.313 P1 7d6fc1a63437 · 소넷의 「응답일마다」 제안은 그래서 반려). 응답일은 «이미 준 응답인가»(재처리·늦은 적재)를 가르는 데만 쓴다. */
  assert.deepEqual(지급돌리기({ 행들: [응답행('S3', 날짜(지금)), 응답행('S3', 날짜(지금)), 응답행('S3', 날짜(어제))] }), ['S3']);
  /* ④-2 어제 응답을 먼저 주면 오늘 응답은 같은 지급일이라 안 준다 */
  assert.deepEqual(지급돌리기({ 행들: [응답행('S6', 날짜(어제)), 응답행('S6', 날짜(지금))] }), ['S6']);
  /* ⑤ 제출일이 빈 행은 실행일로 본다 */
