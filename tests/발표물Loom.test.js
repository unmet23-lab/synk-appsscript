...
test('실저장소 — 얹는 프리셋은 «종이와 밝은 화면» 둘 다 사는 판이다', () => {
  /* 🔴 `부품만` 으로 얹으면 부품 색이 Cream 계열로 남는다. 크롬 인쇄는 「배경 그래픽」이
   *   기본 꺼짐이라 흰 종이 위 Cream = 1.13:1 — 화면에선 멀쩡하고 종이에서만 사라진다.
   * 🔴 그리고 `인쇄부품` 도 아니다(2026-08-19 교체): 그 프리셋의 낮 교정은 통째로 `@media print`
   *   안이라 **화면에서는 안 걸린다**. 발표물은 띄워서도 본다 — Chalk 제목 on Paper = 1.21:1 로
   *   밝은 칸 전부에서 제목이 사라졌다(실측). `밝은부품` = 인쇄부품 + 밝은글. */
  assert.equal(빌드.LOOM_지면, '밝은부품');
  const css = loom.css({ 지면: 빌드.LOOM_지면 });
  assert.ok(/@media print\{/.test(css), '낮 층이 안 실렸다 — 종이에서 부품이 사라진다');
  const 화면 = css.split('@media print{')[0];
  assert.ok(/\.룸 h2\{color:var\(--ink\);\}/.test(화면),
    '화면 층에 제목 교정이 없다 — 밝은 칸에서 제목이 사라진다(이게 교체한 사유다)');
});
