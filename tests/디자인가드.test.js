...
test('처방이 시킨 DESIGN.md 읽기가 read-budget 에 막히지 않는다 (가드끼리 물리면 우회가 정상이 된다)', () => {
  ...
  const m = 소스.match(/const BIG\s*=\s*(\d+)\s*\*\s*(\d+)/);
  assert.ok(m, 'read-budget 의 BIG 상수를 못 찾았다 — 이 검사가 조용히 무의미해진다');
  const BIG = Number(m[1]) * Number(m[2]);
  const 크기 = fs.statSync(DESIGN_MD).size;
  assert.ok(크기 < BIG,
