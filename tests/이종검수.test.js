...
for (const [갈래, 만들기] of [
  ...
  test(`☠️ ${갈래} 처방은 **과녁을 댄다** — 인자 없는 호출은 작업본을 검수해 이 게이트를 못 채운다`, () => {
...
test('🔑 과녁은 **프로젝트별**이다 — 루트 커밋을 crewcard 에 처방하면 따라도 게이트가 그대로다', () => {
  const 근 = (p) => 검수.게이트판정(p, ROOT, 장부([], []))
    .lines.join('\n').match(/--commit\s+([0-9a-f]+)/);
  const 루트 = 근(루트프로젝트);
  const 크루 = 근(path.join(ROOT, 'crewcard'));
  assert.ok(루트 && 크루, '두 프로젝트 다 과녁이 나와야 한다');
  assert.notStrictEqual(루트[1], 크루[1],
    '같은 커밋을 처방하면 한쪽은 그 diff 에 자기 배포 파일이 없어 범위 밖으로 기록된다');
});
...
const 기능체크타임아웃인자 = (src) => {
  const m = src.match(/스키마실행\(프롬프트, 기능체크스키마경로, ([^,]+),/);
  return m && m[1].trim();
};
...
test('🔑 기능체크 패스가 --timeout 을 **깎지 않는다** — 처방이 안 닿는 자리가 있으면 그 처방은 거짓이다 (F103 · 08-10 실측)', () => {
  const src = fs.readFileSync(path.join(ROOT, 'tools', 'codex-review.js'), 'utf8');
  const 인자 = 기능체크타임아웃인자(src);
  assert.ok(인자, '기능체크 호출부를 못 찾았다 — 검사가 실제 코드를 못 짚고 있다');
  assert.doesNotMatch(인자, /Math\.min/,
    `기능체크 타임아웃이 호출부에서 눌린다(${인자}) — 도구는 「--timeout 을 늘려 다시 돌린다」고 처방하는데 그 플래그가 이 자리엔 안 닿는다`);
  assert.match(인자, /초 \* 1000/, `기능체크가 사용자 지정 타임아웃(초)을 안 쓴다: ${인자}`);
});
...
test('🔑 실저장소 정본에서 게이트 다섯 자리가 **전부** 뽑힌다 — 하나라도 빠지면 반쪽 기준으로 검수가 돈다', () => {
  ...
  for (const 자리 of ['[궁극 가치]', '[「똑똑하게」 3층', '[하지 않는 것 넷]', '[적용 대상', '[이해 3층]']) {
...
test('🔴 자리를 못 뽑으면 «반쪽이다»를 블록 본문에 박는다 — stderr 는 심문 결과 파일에 안 남는다', () => {
  ...
  assert.match(t, /하지 않는 것 넷/, '어느 자리가 빠졌는지 안 밝혔다 — 「반쪽」만으로는 고칠 자리를 모른다');
});
...
test('심문: 치환이 실제로 일어난다 — 대상 전문 + 🚫 줄 전량', () => {
  ...
  assert.match(p, /「똑똑하게」 3층/, '판단 정본 게이트가 안 들어갔다 — 기준 없이 돈 심문은 「지적 없음」과 같은 모양이다');
...
test('심문: 결과 경로가 모델로 갈린다 — 안 갈리면 두 모델 대조가 첫 결과를 덮는다', () => {
  ...
  assert.match(a.경로, /-sol-1\.md$/, '경로에 모델 이름이 없다');
  assert.match(b.경로, /-luna-1\.md$/, '경로에 모델 이름이 없다');
});
