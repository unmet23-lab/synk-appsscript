...
test('☠️ 심문: 완료 행은 조건 지문을 «던짐 행에서 옮겨 적는다» — 재계산하면 갈린 조건이 감춰진다 (F411)', () => {
  ...
  const 옮긴 = 검수.던짐지문읽기(장부p, 검수.심문결과경로(대상, 픽().이름, 1).지문, 1, 픽().model);
...
test('☠️ 심문: 던짐 행이 없으면 조건 지문을 «지어내지 않고» null 을 박는다 (F411)', () => {
  const 장부p = 임시장부();
  const 없는것 = 검수.던짐지문읽기(장부p, '없는지문aaaa', 1, 'gpt-5.6-luna');
...
test('☠️ 표식은 남이 청소하는 폴더에 두지 않는다 — 거기 두면 증거가 조용히 사라진다 (F333 재발 경로)', () => {
  ...
  try {
    ...
    // ⚠ 세션 id 도 **그 통로에서** 받는다(F633). 여기서 env 를 직접 읽으면, 클라우드처럼
    //   HOST 가 빈 환경에서 기대값이 `safeId('')`='unknown' 으로 굳어 검사가 공허해진다.
    const 세션식별 = require(path.join(ROOT, '.claude', 'hooks', 'lib', '세션식별.js'));
    assert.ok(표식.includes(store.safeId(세션식별.자리id())),
