...
test('🔑 과녁은 **프로젝트별**이다 — 처방한 커밋에 그 프로젝트의 배포 파일이 들어 있어야 한다', () => {
  ...
  for (const [proj, 이름] of [[루트프로젝트, '(루트)'], [path.join(ROOT, 'crewcard'), 'crewcard']]) {
    ...
    assert.ok(담김.some((f) => 집합.includes(f)),
      ...
      /* 🔴 09-05: 이 자리가 원격에서만 빨갛다. 「0건」만 적으면 «커밋이 그런 것»인지 «배포집합이
       *   딴것»인지 못 가른다 — 재는 자에 분모가 없으면 다음 사람도 로컬 재현부터 다시 한다. */
      + '\n     · diff ' + 담김.length + '건: ' + 담김.slice(0, 6).join(' ')
      + '\n     · 배포집합 ' + 집합.length + '건: ' + 집합.slice(0, 6).join(' ')
      + '\n     · git 종료 ' + r.status + ' · ROOT=' + ROOT);
  }
});
...
test('🔑 실저장소 정본에서 게이트 다섯 자리가 **전부** 뽑힌다 — 하나라도 빠지면 반쪽 기준으로 검수가 돈다', () => {
  ...
  const 구획 = require('../.claude/hooks/philosophy-card.js').철학구획(fs.readFileSync(검수.철학경로, 'utf8'));
  for (const [이름] of 구획.자리) {
    const 자리 = '[' + 이름 + ']';
...
test('불완전한 구획은 빈 결과와 누락 경고로 구분하고 전달하지 않는다', () => {
  const p = 심문픽스처('# 철학\n\n**한 문장: 한 사람을 깊이 이해하는 교육 시스템\n', '철학.md');
  const 경고 = 경고모으기(() => assert.strictEqual(검수.철학텍스트(p), ''));
  assert.match(경고, /게이트 자리를 못 뽑았다/);
  assert.match(경고, /하지 않는 것 셋/);
});
...
test('🔴 선파악·기능체크 프롬프트가 게이트와 **서열 한 줄**을 둘 다 싣는다', () => {
  ...
  for (const [이름, p] of [
    ...
    const 방향자리 = p.indexOf('\n앱의 방향 (');
    assert.ok(방향자리 >= 0 && p.indexOf('docs/SYNK_철학.md') < 방향자리,
...
test('심문: 템플릿에 마커가 없으면 거절 — 치환 없이 보내면 빈 심문이 「통과」로 보인다', () => {
  ...
  const 금지 = 심문픽스처('- 🚫 어떤 것 재제안 금지', '검토기준.md');
...
test('심문: 현행 기준 원문 없음·빈 문서는 거절하고 금지 표식 0건은 정상으로 읽는다', () => {
  const 대상 = 심문픽스처('# 계약 초안');
  const 템플릿 = 심문픽스처('머리말\n\n---\n{{검토기준}}\n{{철학게이트}}\n{{대상문서}}', '템플릿.md');
  assert.throws(() => 검수.심문프롬프트(대상, 템플릿, 대상 + '.없음'), /검토 기준 원문을 못 읽었다/);
  assert.throws(() => 검수.심문프롬프트(대상, 템플릿, 심문픽스처('  ', '기준.md')), /원문이 비었다/);
  const p = 검수.심문프롬프트(대상, 템플릿, 심문픽스처('현재 원문을 확인한다.', '기준.md'));
  assert.match(p, /현재 원문을 확인한다/);
});

test('심문: 현행 기준 전문과 대상 전문을 함께 싣고 옛 마커도 호환한다', () => {
  const 대상 = 심문픽스처('# 계약 초안\n필드 A 는 소급 불가다');
  const 기준 = 심문픽스처('# 현행 기준\n새 근거로 재검토한다.\n보호 경계는 유지한다.', '기준.md');
  for (const 마커 of ['{{검토기준}}', '{{금지목록}}']) {
    const 템플릿 = 심문픽스처('머리말\n\n---\n' + 마커 + '\n{{철학게이트}}\n{{대상문서}}\n', '템플릿.md');
    const p = 검수.심문프롬프트(대상, 템플릿, 기준);
    assert.match(p, /필드 A 는 소급 불가다/);
    assert.ok(p.includes(검수.철학텍스트()));
    assert.match(p, /새 근거로 재검토한다/);
    assert.match(p, /보호 경계는 유지한다/);
...
test('심문(실저장소): 템플릿에 마커 셋이 실재한다 — 없으면 도구가 통째로 죽는다', () => {
  ...
  assert.ok(t.includes('{{검토기준}}') || t.includes('{{금지목록}}'));
  for (const 마커 of ['{{철학게이트}}', '{{대상문서}}']) {
...
test('심문: 첫 --- 위(운영 메모)는 심문자에게 가지 않는다 — 머리말이 실행을 뒤집는다', () => {
  const 대상 = 심문픽스처('# 계약 초안');
  const 금지 = 심문픽스처('- 🚫 무엇 재제안 금지', '검토기준.md');
  const 템플릿 = 심문픽스처(
...
test('심문(실저장소): 개인 memory 없이 현행 원문으로 조립하며 운영 메모는 제외한다', () => {
  const p = 검수.심문프롬프트(검수.방향경로);
  assert.match(p.slice(0, 60), /당신은 독립 (?:설계 검토자|반대심문자)다/);
  assert.match(p, /docs\/AI_운영원칙\.md/);
  assert.match(p, /docs\/_ops\/결정\.md/);
});
...
test('심문: 표식 유무와 관계없이 실제로 싣는 현행 원문이 바뀌면 지문도 바뀐다', () => {
  const 기준 = 심문픽스처('현재 판단 기준', '기준.md');
  const 전 = 검수.금지지문(기준);
  fs.appendFileSync(기준, '\n새로운 근거를 우선한다.');
  assert.notStrictEqual(전, 검수.금지지문(기준));
});
