...
test('F561 ③ 마누스 표 — waiting 은 결과가 있고, 상한 초과는 없다 (F509 를 같이 못박는다)', () => {
  새방();
  const 런 = 런모듈();
  // 코드 2 = waiting. 비고가 스스로 「결과 파일은 있다」라고 적는 자리다.
  런.런쓰기({ 런ID: 'q5', 종류: '사실심문', 상태: '미결', 종료코드: 2, 작업id: 'T1', pid: 1, 시작: new Date().toISOString(), 대상: 't' });
  const 판5 = 런.판정(런.런읽기('q5'));
  assert.strictEqual(판5.갈래, '미결미처분', 'waiting 런이 「주울 답 없음」으로 나가면 비고와 알림이 서로를 부정한다');
  assert.strictEqual(판5.결과, true);
  // 코드 3 = 폴러 상한 초과 · 결과 파일 없음. F509 의 그 런이다 — 여기가 뚫리면 44KB 404 가 재발한다.
  런.런쓰기({ 런ID: 'q6', 종류: '사실심문', 상태: '완주', 종료코드: 3, 작업id: 'T2', pid: 1, 시작: new Date().toISOString(), 대상: 't' });
  const 판6 = 런.판정(런.런읽기('q6'));
  assert.strictEqual(판6.갈래, '실패미처분', '결과 0바이트가 완주·미결 어느 쪽으로도 새면 안 된다');
  assert.strictEqual(판6.결과, false);
});

test('F561 ④ 뜻을 모르는 코드는 «모름» 이다 — 「결과 없음」으로 접지 않는다', () => {
  새방();
  const 런 = 런모듈();
  /* 생산자를 못 가르는 런(작업id 도 인자 배열도 없다) = 아직 표에 없는 도구.
...
test('F561 ⑤ 생산자 판별자는 «이어받기» 와 같은 한 곳이다 — 갈리면 그날 어긋난다', () => {
  const 런 = 런모듈();
  assert.strictEqual(런.생산자({ 작업id: 'T' }), '마누스');
  ...
  const 마 = { 작업id: 'T', 런ID: 'x' };
  assert.strictEqual(런.생산자(마), '마누스');
  assert.ok(/--이어받기/.test(런.이어받기명령(마).명령), '이어받기가 다른 판별자를 쓰고 있다');
  assert.strictEqual(런.상태이름(1, 'codex'), '미결', '생산자를 주면 라벨까지 정직해야 JSON 을 여는 사람이 안 속는다');
  assert.strictEqual(런.상태이름(1, '마누스'), '미결');
  assert.strictEqual(런.상태이름(3, '마누스'), '실패');
});

test('F561 ⑥ 새 칸을 읽는 곳이 **셋 다** 있다 — 칸만 늘리고 라우팅을 안 넓히면 조용히 샌다', () => {
  for (const f of ['.claude/hooks/review-runs.js', 'tools/codex-review.js', 'tools/마누스.js']) {
    const s = fs.readFileSync(path.join(ROOT, f), 'utf8');
...
test('F561 ⑧ 진짜 실패는 여전히 🔴 다 — 새 갈래가 F509 를 되돌리면 안 된다', () => {
  const d = 새방();
  const 런 = 런모듈();
  런.런쓰기({ 런ID: '사실심문-F561-2', 종류: '사실심문', 상태: '실패', 종료코드: 3, 작업id: 'T9', pid: 1, 시작: new Date().toISOString(), 대상: 't', 로그: 'C:/tmp/w.log', 비고: '폴러 상한 초과 — 결과 파일 없음' });
  const r = 훅돌리기({ SYNK_REVIEW_RUNS: path.join(d, 'runs') });
...
/* ──────────────────────────────────────────────────────────────────────────
 * 이어받기 처방 — F508. 장부에 런을 적는 생산자가 **둘**(`codex-review.js`·`마누스.js`)인데
 * 처방은 codex 쪽 필드(`인자`) 하나에만 기대고 있었다. 마누스 런엔 그 칸이 없어서
...
test('이어받기 ① 마누스 런은 런ID 로 다시 붙는다 — 그리고 남의 설명이 안 붙는다', () => {
  const r = 런모듈().이어받기명령({ 런ID: '사실심문-X', 종류: '사실심문', 작업id: 'mfrRbcs' });
  assert.match(r.명령, /^node tools\/마누스\.js --이어받기 사실심문-X$/);
  assert.ok(!/체크포인트/.test(r.비고),
    '마누스 이어받기는 체크포인트가 아니라 재부착이다 — 그 문장을 붙이면 그냥 거짓이다');
});
...
test('F511 ③ `--이어받기` 분기가 **실제로** 재개덧() 을 부른다 — 함수만 살아 있고 안 불리면 아무것도 안 막는다', () => {
  const d = 새방();
  const 런 = 런모듈();
  const 런ID = '사실심문-분기-1';
  런.런쓰기({ 런ID, 종류: '사실심문', 상태: '완주', 종료코드: 3, 끝: new Date().toISOString(), 비고: '상한 초과', 작업id: 'T', 시작: new Date().toISOString() });

  /* 키 파일을 없는 곳으로 돌린다 — 첫 폴링이 fetch 전에 지고(벤더를 안 친다), 도장은 그보다
   * 먼저 찍힌다. 폴러는 재시도하며 살아 있으니 확인한 뒤 우리가 끈다 — 상한까지 안 기다린다.
   * 🔑 모양을 여기 베끼지 않는다 — 베끼면 마누스를 망가뜨려도 이 시험이 초록이다(실측). */
