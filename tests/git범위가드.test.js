...
/* SYNK_TEST_되돌림판정 = 변이 실험용 이음매(SYNK_TEST_SCOPE_HOOK 과 같은 목적, 층만 다르다 —
 * 이쪽은 훅 프로세스가 아니라 **순수 판정 모듈**을 갈아끼운다. 이음매가 검사 파일에만 있으므로
 * 훅 자체에는 새 문이 안 생긴다). */
const 판정9 = require(process.env.SYNK_TEST_되돌림판정
  || path.join(ROOT, '.claude', 'hooks', 'lib', '되돌림판정.js'));
const 임시8 = [];
...
/* ⑨-a **제자리 수정 vs 순수 삭제** (F189 · 2026-08-07).
 *
 * ⑨의 첫 판은 「그 줄이 작업본에 **문자열로** 남아 있나」 하나로만 갈랐다. 그러면 공유 문서의 그 줄을
 * **고치는** 순간 원문이 사라져 되돌림으로 잡힌다 — 그리고 새는 방향이 나빴다: 처방(그 줄 복원)을
 * 따르면 내 수정이 사라지므로 **따를 수 없고**, 남는 문이 전 규칙을 끄는 BYPASS 뿐이다(F103 형태).
 * 아래 두 검사는 **한 쌍으로만 의미가 있다** — 수리가 F187 을 다시 열지 않는다는 것이 절반이다. */
test('🔴 ⑨ **제자리 수정**은 막지 않는다 — 「지웠다」와 「고쳤다」는 다르다 (F189 실사고 재현)', () => {
  const f = 픽스처8();
  남이커밋(f, '절단문서.md', '| 진행 | 상태 |\n| ①-5·①-6·①-7 종결 | 회귀 초록 |\n');
  만진기록(f.state, f.repo, 'local_peer', [], 1);
  만진기록(f.state, f.repo, 'local_me', ['절단문서.md'], 0);
  /* 그 줄을 **지우는** 게 아니라 그 자리에 8번을 끼워 넣는다 — 원문 문자열은 똑같이 사라진다.
   * 작업본은 HEAD 기준이라 스테일도 아니다(실측 `02cd32d` 가 정확히 이 모양으로 차단됐다). */
  fs.writeFileSync(path.join(f.repo, '절단문서.md'),
    '| 진행 | 상태 |\n| ①-5·①-6·①-7·①-8 종결 | 회귀 초록 |\n');
  const r = 가드8(f, 'git commit -m "①-8 닫음" -- 절단문서.md');
  assert.equal(r.차단, false,
    `제자리 수정을 되돌림으로 읽었다 — 처방을 따르면 내 수정이 사라져 **따를 수 없는 처방**이 되고,`
    + ` 남는 문은 ⑨만이 아니라 전 규칙을 끄는 BYPASS 뿐이다(F103):\n${r.사유}`);
});

test('🔴 ⑨ **이전 판이 돌아온 것**은 닮았어도 되돌림이다 — F189 수리가 F187 을 열지 않는다', () => {
  /* 「+ 줄이 있으면 수정」으로 갈랐다면 F187 이 통째로 새어 나간다: 도구가 옛 판으로 덮어쓴 자리도
   * diff 에선 `-남의줄 / +내줄` 한 블록이고, 그 `+` 줄은 원본과 **아주 닮았다**(한 글자만 다른 기본값).
   * 가르는 것은 닮음이 아니라 **그 줄이 남의 커밋이 지운 줄인가**다 — 그러면 이전 판의 복귀다. */
  const f = 픽스처8();
  fs.writeFileSync(path.join(f.repo, '왕복시험.js'), 'const 기본값 = { correlation_id: null };\n');
  f.g('add', '왕복시험.js'); f.g('commit', '-qm', '내가 처음 읽은 판');
  남이커밋(f, '왕복시험.js', 'const 기본값 = { correlation_id: "왕복" };\n');
  만진기록(f.state, f.repo, 'local_peer', [], 1);
  만진기록(f.state, f.repo, 'local_me', ['왕복시험.js'], 0);
  // Edit 이 「file state is current」라며 내가 처음 읽은 판 위에 치환했다 — 남의 줄이 조용히 되돌아간다
  fs.writeFileSync(path.join(f.repo, '왕복시험.js'), 'const 기본값 = { correlation_id: null };\n');
  const r = 가드8(f, 'git commit -m "내 수정" -- 왕복시험.js');
  assert.equal(r.차단, true,
    `되돌림이 「닮은 + 줄이 있다」는 이유로 통과했다 — F189 수리가 F187 을 그대로 열었다:\n${r.사유}`);
  assert.match(r.사유, /correlation_id/, '무엇이 되돌아가는지 안 짚었다');
});

/* ⑨-a 관문별 탐지력 — **순수 판정 모듈**을 직접 부른다 (F189).
 *
 * 왜 통합 검사만으로는 부족한가: 위 두 검사는 관문 셋을 통째로 지나는 **한 경로씩**만 밟는다.
 * 실제로 변이 3건이 그 사이로 샜다 — 「판정 불가를 통과로」·「블록 못 찾으면 통과로」·「문맥 줄에서
 * 블록이 안 끊긴다」. 셋 다 훅을 띄워서는 재현할 자리가 없다(git 이 그 상태를 안 만들어 준다).
 * 판정이 순수 함수라 여기서 직접 못박을 수 있고, **새는 방향은 셋 다 「통과」**였다. */
const diff9 = (...줄) => ['--- a/f', '+++ b/f', '@@ -1,3 +1,3 @@', ...줄].join('\n');

test('🔴 ⑨-a 판정을 **못 쟀으면** 되돌림 쪽으로 센다 — 판정 불가는 통과가 아니다', () => {
  assert.equal(판정9.제자리수정인가('남의 긴 줄입니다', null, new Set()), false,
    '내 diff 를 못 쟀는데 「수정이다」로 통과시켰다 — 있는데 안심시키는 가드가 된다');
});

test('🔴 ⑨-a 그 줄이 **어느 블록에서** 사라졌는지 못 짚으면 되돌림 쪽으로 센다', () => {
  const 블록 = 판정9.변경블록(diff9('-다른 줄이 사라졌다', '+대신 들어온 줄이다'));
  assert.equal(판정9.제자리수정인가('남의 긴 줄입니다', 블록, new Set()), false,
    '못 짚은 줄을 수정으로 읽었다 — 되돌림이 통째로 조용해진다');
});

test('🔴 ⑨-a **문맥 줄이 블록을 끊는다** — 멀리 있는 `+` 줄이 삭제를 수정으로 세탁하면 안 된다', () => {
  /* 한 파일 안에서 남의 줄을 지우고 **다른 자리**에 내 줄을 더하는 건 흔하다. 블록이 안 끊기면
   * 그 둘이 한 덩어리로 붙어, 순수 삭제가 「대체한 + 줄이 있다」로 읽힌다(관문①이 통째로 무력해진다). */
  const 블록 = 판정9.변경블록(diff9(
    '-const 남의기능 = true;',
    ' 문맥 한 줄',
    '+const 내가더한기능 = true;',
  ));
  assert.equal(블록.length, 2, `문맥 줄에서 블록이 안 끊겼다 — 덩어리 ${블록.length}개`);
  assert.equal(판정9.제자리수정인가('const 남의기능 = true;', 블록, new Set()), false,
    '멀리 있는 + 줄이 삭제를 수정으로 세탁했다');
});

test('⑨-a 관문① 순수 삭제 · 관문② 이전 판 복귀 · 관문③ 제자리 수정 (세 갈래를 한 자리에서)', () => {
  const 원본 = '| ①-5·①-6·①-7 종결 | 회귀 초록 |';
  // ① 대체한 것이 없다 = 순수 삭제
  assert.equal(판정9.제자리수정인가(원본, 판정9.변경블록(diff9(`-${원본}`)), new Set()), false, '관문①');
  // ② 그 커밋이 지웠던 줄이 그대로 돌아왔다 = 되돌림(닮았어도 아니다)
  const 옛판 = 'const 기본값 = { correlation_id: null };';
  const 새판 = 'const 기본값 = { correlation_id: "왕복" };';
  assert.equal(판정9.닮았나(새판, 옛판), true, '전제 확인 — 이 둘은 닮았다(관문③만으론 통과한다)');
  assert.equal(판정9.제자리수정인가(새판, 판정9.변경블록(diff9(`-${새판}`, `+${옛판}`)), new Set([옛판])),
    false, '관문② — 이전 판이 돌아왔는데 수정으로 읽었다(F187 이 그 문으로 샌다)');
  // ③ 끼워 넣기 = 제자리 수정
  const 고친것 = '| ①-5·①-6·①-7·①-8 종결 | 회귀 초록 |';
  assert.equal(판정9.제자리수정인가(원본, 판정9.변경블록(diff9(`-${원본}`, `+${고친것}`)), new Set()),
    true, '관문③ — 끼워 넣기를 되돌림으로 읽었다');
});

test('🔴 ⑨-a **갈아치운 자리**는 수정이 아니다 — 닮지 않은 `+` 줄은 되돌림 쪽이다 (관문③)', () => {
  /* 여기가 F187 과 F189 를 가르는 자리다. 「+ 줄이 있으면 수정」으로 갈랐다면 도구가 옛 판으로
   * 덮어쓴 자리도 diff 에선 `-남의줄 / +내줄` 한 블록이라 통째로 새어 나간다. */
  const 원본 = 'const 새학생모드 = true;';
  const 새것 = 'const 내수정 = 2;';
  assert.equal(판정9.닮았나(원본, 새것), false, '전제 확인 — 이 둘은 안 닮았다(문턱이 죽으면 여기가 먼저 붉어진다)');
  assert.equal(판정9.제자리수정인가(원본, 판정9.변경블록(diff9(`-${원본}`, `+${새것}`)), new Set()), false,
    'F187 실사고가 그대로 통과한다 — 갈아치운 것을 고친 것으로 읽었다');
});

test('⑨-a 짧은 `+` 줄은 대체로 안 센다 — 짧을수록 「닮았다」가 우연히 참이 된다', () => {
  /* 하한이 없으면 한두 글자 차이의 **토막**이 곧바로 87% 닮음으로 잡혀 되돌림을 세탁한다.
   * 하한을 넘는 쪽도 함께 못박는다 — 안 그러면 「전부 거짓」인 판정도 이 검사를 통과한다. */
  const 원본 = '가나다라마바사아';                       // 8자 — 하한과 같은 길이
  assert.equal(판정9.닮았나(원본, '가나다라마바사'), true, '전제 확인 — 토막은 원본과 아주 닮았다');
  assert.equal(판정9.제자리수정인가(원본, 판정9.변경블록(diff9(`-${원본}`, '+가나다라마바사')), new Set()),
    false, '7자짜리 토막 하나로 되돌림이 수정으로 세탁된다');
  assert.equal(판정9.제자리수정인가(원본, 판정9.변경블록(diff9(`-${원본}`, '+가나다라마바사자차')), new Set()),
    true, '하한을 넘는 제자리 수정까지 막았다 — 그러면 F189 가 그대로 남는다');
});

test('⑨-a `--- a/f` · `+++ b/f` 머리말은 변경이 아니다', () => {
  const 블록 = 판정9.변경블록(diff9('-남의 줄입니다', '+내 줄입니다'));
  assert.equal(블록.length, 1, `머리말을 변경으로 셌다 — 덩어리 ${블록.length}개`);
  assert.deepEqual(블록[0].빼기, ['남의 줄입니다']);
  assert.deepEqual(블록[0].더하기, ['내 줄입니다']);
});
