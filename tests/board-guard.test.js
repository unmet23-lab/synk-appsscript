...
test('🔴 [F556] `🎫없음` 은 놓은 게 아니다 — 부정을 표식으로 읽으면 남의 산 자리가 열린다', { skip: git있음 ? false : 'git 없음' }, () => {
  /* 실보드에 실제로 있던 형태다(아카이브 포함 표줄 1097 중 1건). 사본이 `includes('🎫')` 라
   * 이걸 놓음으로 읽었고, 새는 방향이 「연다」였다 — 두 세션이 같은 자리를 조용히 판다.
   * 판정을 `보드lib.놓은줄()` 하나로 모으면서 이 형태만 좁게 닫았다(F556). */
  /* ⚠ 상태는 **활성**이어야 한다 — `✅종결` 을 넣으면 그 줄이 애초에 분모에서 빠져서
   *   deny 가 안 나오고, 그러면 이 검사는 놓음 축을 하나도 안 잰다(미실행이 통과로 읽힌다). */
  const r = 남을바꿔(티켓줄('🔵**착수**(`local_82404266`) · 🎫없음** · 🆕F372 열림'), 이어받는줄);
  assert.strictEqual(r.결정, 'deny',
    '🔴 「🎫없음」을 놓음으로 읽어 남의 산 줄을 열어 줬다 — 이건 표식이 아니라 그 표식의 부정이다');
});

test('🔑 [F556] 판정은 **사본이 아니라 통로 하나**다 — 훅과 대기열이 같은 답을 낸다', () => {
  /* 갈라지면 증상이 「자리가 잠긴 채 조용함」이라 아무도 신고를 안 한다(F556 이 그 자리였다).
   * 훅 안의 술어는 이제 껍데기라, 여기서는 그 정본이 실제로 갈라 내는지만 못박는다. */
  const 보드lib = require(path.join(__dirname, '..', 'tools', 'lib', '보드.js'));
  assert.strictEqual(typeof 보드lib.놓은줄, 'function', '🔴 정본이 export 되지 않았다 — 훅이 다시 사본을 갖게 된다');
  assert.strictEqual(보드lib.놓은줄(티켓줄('🎫**이어받아도 된다** — 실측만 하고 안 잡는다')), true);
  assert.strictEqual(보드lib.놓은줄(티켓줄('🔵**착수**(`local_82404266`)')), false,
    '🔴 트랙 칸의 🎫 를 정본이 놓음으로 읽는다 — 훅의 좁힘(F285)이 통로로 안 옮겨졌다');
});
...
/* ── ④ 생사 게이트 — 죽은 주인의 자리는 **차단이 아니라 알림** (F582·F589 · ✅유호 픽 2026-08-17 ㉠) ──
 *
 * 실사고: 죽은 세션의 활성 줄이 `tools/board-move.js` 를 잡고 있어 다음 세션이 선언을 못 했고,
 * ④ 가 내미는 처방 넷이 전부 막혔다 — 그 줄이 ⏳유호 결정을 들고 있어 아카이브로도 못 보낸다.
 * 하루 만에 2번째가 났고 그때는 **일감을 넘긴 세션 자신의 줄**이 잠금 하나를 먹였다(F589):
 * 잠금은 사고의 잔해가 아니라 **인계의 부산물**이라, 인계가 성립할수록 는다.
 *
 * 🔑 아래 셋은 **박동 하나만** 다르다(F281) — 보드 두 파일·겹치는 경로·편집이 전부 같다.
 *   ㉠ 나만 산다 → 남은 죽음 → **allow + 알림**   ㉡ 둘 다 산다 → **deny**   ㉢ 아무 박동도 없다 → **deny**
 * 🔴 ㉢ 이 이 수리의 급소다. 검사 ⑧ 은 「못 쟀다」를 `산지문 === null` 로만 보는데, 박동이 하나도
 *   없는 기계에서는 집합이 **비어서** 모든 주인이 「죽었다」가 되고 ④ 가 통째로 꺼진다 — 그게
 *   「안 도는 장치」의 모양이고, 그 판이 실사용의 기본 모양이다. 그래서 ④ 는 ⑧ 보다 빡빡한
 *   `생사쟀나`(집합이 비지 않았고 **내 지문이 그 안에 있다**)를 문턱으로 쓴다. */
const 남세션582 = 'local_f582aaaa-1111-2222-3333-444455556666';
const 내세션582 = 'local_f582bbbb-1111-2222-3333-444455556666';
const 겹칠경로 = 'tools/겹치는파일.js';
const 남의줄582 = `| 2026-08-17 | **남의 트랙 — 죽은 채 자리를 쥔 줄** | ${겹칠경로} | 🔵착수 (\`local_${지문278(남세션582)}\`) |`;
const 내옛줄582 = `| 2026-08-17 | **내 첫 트랙** | z582.js | 작업중 (\`local_${지문278(내세션582)}\`) |`;
const 내새줄582 = `| 2026-08-17 | **내 새 트랙 — 같은 파일을 집는다** | ${겹칠경로} | 🔵착수 (\`local_${지문278(내세션582)}\`) |`;

/** 남의 활성 줄 1개 + 내 줄 1개. 상한(활성 12)에 안 걸리게 **작게** 짓는다 — 두 검사가 같이
 *  울면 무엇이 막았는지 못 가른다(그러면 대조가 판정을 못 낸다 · F281). */
function F582픽스처() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'boardf582-'));
  const g = (...a) => 실행('git', a, { cwd: dir, encoding: 'utf8' });
  g('init', '-q'); g('config', 'user.email', 't@t'); g('config', 'user.name', 't');
  fs.writeFileSync(path.join(dir, 'a.js'), '// x\n', 'utf8');
  g('add', '--', 'a.js'); g('commit', '-q', '-m', '초기');
  ...
  fs.writeFileSync(path.join(dir, 'docs', '세션보드_아카이브.md'),
    ['# 아카이브', '', '---', '', '| 2026-08-01 | **옛 트랙** | c.js | 완료 |', ''].join('\n'), 'utf8');
  fs.writeFileSync(보드파일(dir, 지문278(남세션582)), `${남의줄582}\n`, 'utf8');
  const 내파일 = 보드파일(dir, 지문278(내세션582));
  fs.writeFileSync(내파일, `${내옛줄582}\n`, 'utf8');
  return { dir, 내파일 };
}
const 내환경582 = { CLAUDE_CODE_HOST_SESSION_ID: 내세션582 };
const 붙임582 = (내파일) => 판정(
  { tool_name: 'Write', tool_input: { file_path: 내파일, content: `${내옛줄582}\n${내새줄582}\n` } }, 내환경582);

test('🔴 [F582·㉠] 자리를 쥔 줄의 **주인이 죽었으면** 막지 않는다 (아무도 못 집던 자리를 연다)', { skip: !hasGit && 'git 없음' }, () => {
  const { dir, 내파일 } = F582픽스처();
  const 내박동 = 박동찍기(dir, 내세션582);        // 남의 박동은 안 찍는다 = 남은 죽었다
  try {
    const r = 붙임582(내파일);
    assert.strictEqual(r.결정, 'allow',
      `🔴 F582 재발 — 죽은 줄이 파일을 영구 예약한다. 그 줄이 ⏳유호 결정을 들고 있으면 처방이 하나도 안 돈다(F103):\n${r.사유.slice(0, 400)}`);
    assert.match(r.알림 || '', /주인이 죽었다/,
      '조용히 통과시켰다 — 비켜 준 자리에 침묵을 두면 둘이 같은 파일을 집고도 서로를 못 본다(F278·F285 와 같은 규칙)');
    assert.ok((r.알림 || '').includes(겹칠경로),
      '무엇이 겹쳤는지 이름을 안 댄다 — 이름 없는 알림은 다음 세션이 대조할 재료가 못 된다');
  } finally { 박동치우기(내박동); }
});

test('🔴 [F582·㉡] 주인이 **살아 있으면** 그대로 막는다 (탐지력 — 박동 하나만 다르다)', { skip: !hasGit && 'git 없음' }, () => {
  const { dir, 내파일 } = F582픽스처();
  const 내박동 = 박동찍기(dir, 내세션582);
  const 남박동 = 박동찍기(dir, 남세션582);        // ㉠ 과 갈리는 것은 이 한 줄뿐이다
  try {
    const r = 붙임582(내파일);
    assert.strictEqual(r.결정, 'deny',
      '🔴 산 세션의 자리를 통과시켰다 — 그건 완화가 아니라 중복 구현이고 되돌릴 수 없다');
    assert.match(r.사유, /남이 이미 선언했다/,
      '막긴 했는데 ④ 가 아닌 다른 검사(상한 등)가 막았다 — 그러면 이 대조는 생사를 안 잰 것이다');
  } finally { 박동치우기(내박동); 박동치우기(남박동); }
});

/* 🔴 아래 둘은 **변이가 낸 구멍을 메운 것**이다(2026-08-17 · 변이 3/6 → 6/6). 위 셋은 게이트의
 * «본선»만 물었고, **경계 두 개**에는 회귀가 0이었다 — 둘 다 새는 방향이 「통과」다:
 *   ㉣ 주인을 못 읽는 줄(파일 이름이 지문 꼴이 아니다 · ⑧-c 가 따로 드러내는 그 판)
 *   ㉤ 가지 안의 선언(F528) — 이 게이트가 **일부러 안 건드리는** 자리 */
test('🔴 [F582·㉣] 주인을 **못 읽는** 줄은 산 것으로 센다 (모르면 막는 쪽 · 파일 이름이 지문 꼴이 아니다)', { skip: !hasGit && 'git 없음' }, () => {
  const { dir, 내파일 } = F582픽스처();
  /* 지문 꼴이 아닌 이름 = `보드id.파일지문` 이 빈 문자열을 낸다. 그 줄의 주인은 «죽었다»가 아니라
   * **모른다** — 여기서 죽음 쪽으로 접으면 이름 하나 어긋난 파일이 자리를 통째로 놓아 버린다. */
  fs.writeFileSync(보드파일(dir, 'mine0002'),
    `| 2026-08-17 | **이름이 지문 꼴이 아닌 파일의 트랙** | ${겹칠경로} | 🔵착수 |\n`, 'utf8');
  const 내박동 = 박동찍기(dir, 내세션582);   // 생사는 «쟀다» — 그래도 이 줄은 못 읽는다
  try {
    const r = 붙임582(내파일);
    assert.strictEqual(r.결정, 'deny',
      '🔴 주인을 못 읽는 줄을 「죽었다」로 접었다 — 이름이 어긋난 파일 하나가 자리를 통째로 놓는다(F207: 모름 ≠ 죽음)');
    assert.match(r.사유, /남이 이미 선언했다/, '④ 가 아닌 다른 검사가 막았다면 이 시험은 아무것도 안 잰다');
  } finally { 박동치우기(내박동); }
});

test('🔴 [F582·㉤] **가지 안의 선언**에는 이 게이트를 안 붙인다 (경계를 넓히면 갇힌 PR 이 조용해진다)', { skip: !hasGit && 'git 없음' }, () => {
  /* F528 — 워크트리 가지의 선언도 ④ 의 분모다. 그 줄의 주인이 죽었어도 **막는 쪽 그대로** 둔다:
   * 가지 줄에는 따를 수 있는 처방이 이미 있고(`gh pr list` → 죽었으면 그 PR 을 닫는다), F103 이
   * 안 걸리는 자리까지 넓히면 「죽은 주인의 열린 PR」이 조용해진다 — 그건 F403 이 지키는 층이다. */
  const { dir, 내파일 } = F582픽스처();
  const 가지줄582 = `| 2026-08-17 | **가지 안의 남의 트랙** | ${겹칠경로} | 🔵착수 |`;
  const 주입 = 보드lib.가지끝줄('worktree-남의가지', 'f582cccc.md', 가지줄582);   // 주인 f582cccc = 박동 없음 = 죽음
  const 내박동 = 박동찍기(dir, 내세션582);
  try {
    const r = 판정(
      { tool_name: 'Write', tool_input: { file_path: 내파일, content: `${내옛줄582}\n${내새줄582}\n` } },
      { ...내환경582, SYNK_BOARD_BRANCH_LOG: 주입 });
    assert.strictEqual(r.결정, 'deny',
      '🔴 가지 줄까지 생사로 놓아 줬다 — 죽은 주인의 열린 PR 이 조용해지고 그 커밋은 워크트리에 갇힌다(F403)');
    assert.match(r.사유, /남이 이미 선언했다/, '④ 가 아닌 다른 검사가 막았다면 이 시험은 아무것도 안 잰다');
    assert.match(r.사유, /가지/, '가지 줄이라는 사실을 안 말하면 board-move 처방을 따라가다 못 찾는다(F528)');
  } finally { 박동치우기(내박동); }
});

test('🔴 [628] **`origin/master` 에만 있는 선언**도 ④ 의 분모다 — 안 세우면 그 자리가 「빈 자리」로 나간다', { skip: !hasGit && 'git 없음' }, () => {
  /* F628 — 세 번째 층. 남이 master 에 직접 올린 선언은 내 디스크에도 없고 가지에도 없다
   * (`가지끝` 은 `--no-merged 기준` 이라 기준 자신을 정의상 뺀다). 옛 판은 그 줄을
   * `if (!r.가지) continue` 로 버려서, 정본이 실어 보내도 **이 훅에서 다시 사각**이 됐다.
   * ⚠ 처방이 가지와 **정반대**임을 사유가 말해야 한다 — 「PR 을 닫아라」를 내면 없는 PR 을 뒤진다(F103). */
  const { dir, 내파일 } = F582픽스처();
  const 원격줄628 = `| 2026-08-18 | **master 에 이미 앉은 남의 트랙** | ${겹칠경로} | 🔵착수 |`;
  const 주입628 = 보드lib.가지끝줄(보드lib.원격기준, 'f628cccc.md', 원격줄628);
  const 내박동 = 박동찍기(dir, 내세션582);
  try {
    const r = 판정(
      { tool_name: 'Write', tool_input: { file_path: 내파일, content: `${내옛줄582}\n${내새줄582}\n` } },
      { ...내환경582, SYNK_BOARD_BRANCH_LOG: '', SYNK_BOARD_REMOTE_LOG: 주입628 });
    assert.strictEqual(r.결정, 'deny',
      '🔴 origin/master 에 있는 남의 선언을 못 봤다 — 뒤처진 트리는 그 자리를 「빈 자리」로 읽고 중복 구현한다(F628)');
    assert.match(r.사유, /남이 이미 선언했다/, '④ 가 아닌 다른 검사가 막았다면 이 시험은 아무것도 안 잰다');
    assert.match(r.사유, /origin\/master/, '어느 층에서 온 줄인지 안 말하면 받을 방법을 못 찾는다');
    assert.match(r.사유, /merge --ff-only/, '처방이 없으면 막기만 하고 길을 안 준다(F103)');
    assert.doesNotMatch(r.사유, /가지 안의 선언이면/,
      '🔴 가지 처방이 붙었다 — 이미 머지된 트랙인데 없는 PR 을 뒤지게 된다(처방이 정반대다 · F542)');
  } finally { 박동치우기(내박동); }
});

test('🔑 [628] 원격 층을 **못 훑으면** 그렇게 말한다 — 「거기 선언 0」과 같은 모양이면 안 된다', { skip: !hasGit && 'git 없음' }, () => {
  const { dir, 내파일 } = F582픽스처();
  const 내박동 = 박동찍기(dir, 내세션582);
  try {
    /* 가지만 주입하고 원격은 안 준다 = 원격 «모름». 그때 셈설명이 그 사실을 말해야 한다(F207). */
    const r = 판정(
      { tool_name: 'Write', tool_input: { file_path: 내파일, content: `${내옛줄582}\n${내새줄582}\n` } },
      { ...내환경582, SYNK_BOARD_BRANCH_LOG: '' });
    /* 🔑 **통과할 때의 자리**를 본다 — 좁아진 분모는 막지 않고 «알림»으로 나간다(F235 통로).
     *   `사유` 만 보면 이 검사는 원리상 아무것도 안 잰다: 겹침이 없으면 사유가 빈 문자열이다. */
    assert.strictEqual(r.결정, 'allow', '이 시험은 통과 경로의 알림을 재는 자리다 — 막혔으면 딴 것을 쟀다');
    assert.match(`${r.알림}${r.사유}`, /`origin\/master` 쪽 선언을 \*\*못 훑었다\*\*/,
      '🔴 원격 층을 안 보고도 아무 말이 없다 — 좁아진 분모가 조용하면 그 초록은 「집어라」다(F207)');
  } finally { 박동치우기(내박동); }
});

test('🔴 [F582·㉢] 박동이 **하나도 없으면** 막는다 — 빈 집합은 「다 죽었다」가 아니라 「못 쟀다」다', { skip: !hasGit && 'git 없음' }, () => {
  /* 검사 ⑧ 의 문턱(`산지문 === null`)을 그대로 썼다면 여기서 **통과**한다: 집합이 비어 모든
   * 주인이 죽은 것으로 세어지기 때문이다. 새는 방향은 언제나 「통과」다. */
  const { 내파일 } = F582픽스처();               // 박동 0개
  const r = 붙임582(내파일);
  assert.strictEqual(r.결정, 'deny',
