...
test('🔴 [F372·경계] 죽은 줄도 **전체 상한**에서는 그대로 센다 (보드가 죽은 줄로 부푸는 것을 막는 자리)', { skip: !hasGit && 'git 없음' }, (t) => {
  ...
  try {
    ...
    if (미실행이면건너뛴다(t, r)) return;
    assert.strictEqual(r.결정, 'deny',
      `🔴 죽음 제외가 **전체 상한까지** 번졌다 — 그러면 보드가 죽은 줄로 무한히 부풀고 치우라 말할 자리가 사라진다:\n${(r.알림 || '').slice(0, 300)}`);
    assert.match(r.사유, /표가 20줄/, `전체 상한이 아니라 딴 사유로 막혔다:\n${r.사유.slice(0, 300)}`);
  } finally { 박동치우기(내박동); }
});
...
/* 🔴 새는 방향 봉인 — 대기 줄이 **전체** 상한에서도 빠지면 보드가 대기 줄로만 무한히 부푼다.
 *   활성 분모에서만 빼고 전체(18)는 그대로 센다는 것을 값으로 못박는다. */
test('🔴 [F395] 대기 줄도 **전체 상한**에는 그대로 센다', { skip: !hasGit && 'git 없음' }, (t) => {
  const 기존 = `| 2026-08-09 | **내 첫 트랙** | z.js | 작업중 (\`local_${지문278(내세션278)}\`) |`;
  ...
  try {
    ...
    if (미실행이면건너뛴다(t, r)) return;
    assert.strictEqual(r.결정, 'deny',
      `🔴 대기 줄이 전체 상한에서도 빠졌다 — 보드가 대기 줄로만 무한히 부푼다:\n${(r.알림 || '').slice(0, 300)}`);
    assert.match(r.사유, /표가 19줄/, `전체 상한이 아니라 딴 사유로 막혔다:\n${r.사유.slice(0, 300)}`);
  } finally { 박동치우기(박동); }
});
 ...
 * 탐지력 짝은 위 F234 시험(`/전체 20줄/`)이 진다 — **진짜** 물려받은 위반은 그 머리글로 계속 나온다. */
