...
test('세션보드 변경은 따로 센다 — 트랙 선택의 재료가 몇 번 바뀌었나', { skip: !있나 }, () => {
  const { A, B } = 판만들기();
  밀기(A, 3, 'seed.txt');
  /* 🔑 보드는 **세션마다 파일이 갈린다**(F250) — 픽스처도 폴더 «안»에 세션 파일을 만든다.
   *   상수를 파일 이름처럼 쓰면 폴더 필터가 실제로 도는지 못 재고, 옛 단일 파일 구조로
   *   되돌려도 초록이 된다. 두 세션을 섞어 「폴더 전체를 센다」까지 잰다. */
  밀기(A, 1, `${훅.보드}/aaaa1111.md`);
  밀기(A, 1, `${훅.보드}/bbbb2222.md`);
  git(B, 'fetch', '--quiet', 'origin', 'master');
  const r = 훅.재다(B, { fetch: false });
...
/* 🔴 **경로가 죽으면 이 훅은 「조용함」으로 틀린다** — 실사고 2026-08-12(F250 이주의 낙진).
 *   `보드` 상수가 옛 `docs/세션보드.md`(안내 스텁)에 남아 08-08~08-12 내내 `보드변경` 이
 *   구조적으로 0 이었다. 실측: 그 기간 옛 경로 커밋 **0** · 새 폴더 **858**. 화면에서
 *   「보드 안 바뀜」과 구별이 안 돼 나흘간 아무도 못 봤다.
 *   🔑 그래서 «값»을 손으로 적어 두지 않고 **보드 정본 모듈에서 파생시켜 대조한다** —
 *     다음 이주 때 훅만 남으면 그 자리에서 빨개진다(board-guard 도 같은 모듈을 읽는다).
 *   🚫 여기 기대값을 문자열 리터럴로 박기 — 그러면 이주 때 «둘 다» 고쳐야 해서 같은 병이 돈다. */
test('🔴 보드 경로는 정본 모듈에서 파생된 자리와 같다 (죽은 경로를 세면 알림이 영영 침묵한다)', () => {
  const 보드lib = require(path.join(ROOT, 'tools', 'lib', '보드.js'));
  const 정본 = 보드lib.폴더('.').split(path.sep).join('/');
  assert.strictEqual(훅.보드, 정본,
    `repo-staleness 가 «${훅.보드}» 를 세는데 보드 정본은 «${정본}» 에 산다 — 그 사이 보드가 몇 번 바뀌어도 0 으로 나온다`);
  // 스텁으로의 복귀를 따로 못박는다 — 위 대조만으로는 «둘 다» 스텁이 되면 통과한다.
  assert.notStrictEqual(훅.보드, 'docs/세션보드.md',
    '옛 단일 보드는 안내 스텁이라 표 행이 원리상 안 온다(board-guard 가 쓰기를 막는다)');
});
