...
/* ── ①-b 표본 «둘째»에만 있는 git (①배포 검수 P1 · 2026-08-17) ────────────────────
 * 병: 판정이 **첫 표본만** 훑어, 두 표본 사이에 새로 뜬 git 이 어느 갈래에도 안 잡혔다.
 *   그 침묵은 `없음` 으로 나오고, `가름문` 의 `없음` 갈래는 「✅ 가름 = 고아다 · 도는 git 이 0개다 ·
 *   재시도는 영영 안 통한다 · Remove-Item 으로 치운다」다 — **도는 git 의 잠금을 지우라고 권하는** 모양이다.
 *   새는 방향이 「통과」인 데다 그 처방이 비가역이라, 이 계열에서 가장 나쁜 자리다.
 * 🔑 앞판이 없어 CPU 증가분을 원리상 못 재므로 고아 후보로 올리지 않고 **산 것**으로 센다 —
 *   이 판정에서 틀릴 때의 방향은 언제나 「기다려라」여야 한다. */

/** 첫 표본엔 git 이 0개인데 그 사이 하나가 떴다(부모 pid 2 = 살아있는 목록 안). */
const 새로뜬판 = { a: { git: [] },
  b: { git: [{ pid: 4242, ppid: 2, cmd: 'git fetch origin master', cpu: 0 }] }, pids: [1, 2, 3] };

test('🔴 P1 — 첫 표본이 비어도 둘째에 git 이 있으면 «없음»이 아니다 (잠금을 치우라고 권하던 자리)', () => {
  const r = 판정(새로뜬판);
  assert.equal(r.상태, '살아있다', '«없음»으로 접으면 처방이 「고아다 · 치운다」로 뒤집혀 도는 git 의 잠금을 지우게 한다');
  assert.equal(r.산것.length, 1);
  assert.equal(r.산것[0].pid, 4242);
  assert.equal(r.고아.length, 0, '앞판이 없어 멎음을 원리상 못 가른다 — 고아 후보로 올리면 안 된다');
});

test('🔑 지적보다 한 칸 넓다 — 첫 표본이 «안 비어도» 새로 뜬 git 은 잡힌다', () => {
  const 섞임 = { a: { git: [고아판.git[0]] },
    b: { git: [고아판.git[0], { pid: 4242, ppid: 2, cmd: 'git fetch origin master', cpu: 0 }] },
    pids: [1, 2, 3] };
  const r = 판정(섞임);
  assert.equal(r.상태, '살아있다', '옛 판은 첫 표본만 훑어 여기서 «고아»를 냈다 — 산 git 옆에서 치우게 된다');
  assert.ok(r.산것.some((g) => g.pid === 4242), '새로 뜬 git 이 산것에 없으면 그 존재가 어느 줄에도 안 남는다');
  assert.equal(r.고아.length, 1, '고아는 그대로 세어 보여준다 — 사람의 판단 재료다');
});

test('🔑 두 표본이 다 비면 그대로 «없음» — 가드가 정상 통로까지 끄면 안 된다', () => {
  assert.equal(판정(빈판).상태, '없음', '이 칸이 죽으면 진짜 고아 잠금을 아무도 못 치운다(F103 — 따를 수 없는 처방)');
});

test('🔴 새로 뜬 것은 판정문이 «못 쟀다»고 말한다 — CPU +0.000초 는 «멎음»과 같은 모양이다',
  { skip: !git있나 && 'git 없음' }, () => {
    const 줄 = 처방(잠금세움().repo, 새로뜬판);
    assert.match(줄, /가름 = 살아있다/, '이 갈래의 결론은 「치우지 마라」다');
    assert.match(줄, /못 쟀다/, '0 을 잰 값처럼 적으면 사람이 「멎었으니 치워도 되겠다」로 읽는다(맹점 ④)');
    assert.ok(!/CPU \+0\.000초/.test(줄), '못 잰 칸에 0 을 찍는 것이 바로 「맞는 얼굴로 틀린 값」이다');
  });
