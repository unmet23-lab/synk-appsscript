...
// ── 훅이 «무엇을 말하나» — 이종 검수가 P1 넷을 낸 자리 (09-03) ────────
/* 이 도구의 위험은 「틀린 말을 한다」가 아니라 **「아무 말도 안 한다」**다.
 * 조용함이 「다 최신」으로 읽히면 감시가 영원히 죽는다([[zero-is-a-success-face-taxonomy]]). */

const 잰것 = (이름, 더할것) => Object.assign({ 이름, 저장소: 'o/r', 경로: 'p', 상류sha: 'a', 상류날짜: '2026-09-01', 기준선: 'a', 움직였나: false, 처음: false }, 더할것 || {});
const 못잰것 = (이름, 까닭) => ({ 이름, 저장소: 'o/r', 경로: 'p', 못쟀다: 까닭 || 'GitHub 응답 429' });

test('🔴 전부 못 쟀으면 «확인 불가»라고 말한다 — 조용히 나가면 「변경 없음」과 구분이 사라진다', () => {
  const { 줄들, 다쟀나 } = 점검.훅판정([못잰것('가'), 못잰것('나')], []);
  assert.ok(줄들.length > 0, '아무 말 없이 끝났다 — 「다 최신」으로 읽힌다');
  assert.ok(줄들.join('\n').includes('확인 불가'), `「확인 불가」를 안 말한다: ${줄들.join(' / ')}`);
  assert.equal(다쟀나, false, '못 쟀는데 「쟀다」로 쳤다 — 7일 동안 다시 안 잰다');
});

test('🔴 하나만 못 쟀어도 「쟀다」로 치지 않는다 — 그 하나가 7일 동안 안 보이던 자리', () => {
  const { 줄들, 다쟀나 } = 점검.훅판정([잰것('가'), 잰것('나'), 못잰것('다')], []);
  assert.equal(다쟀나, false, '부분 실패인데 잰 때를 적으러 간다');
  assert.ok(줄들.join('\n').includes('다'), `못 잰 스킬 이름을 안 말한다: ${줄들.join(' / ')}`);
});

test('🔴 「못 읽은 줄」은 이동 경보와 «함께» 나온다 — 한쪽이 다른 쪽을 묻던 자리', () => {
  const { 줄들 } = 점검.훅판정([잰것('가', { 움직였나: true })], ['깨진줄']);
  const 글 = 줄들.join('\n');
  assert.ok(글.includes('움직였다'), '이동 경보가 없다');
  assert.ok(글.includes('깨진줄'), '이동 경보가 있으면 「못 읽은 줄」이 사라진다 — 감시 대상이 조용히 준다');
});

test('다 쟀고 아무것도 안 움직였으면 조용하다 — 평소 세션이 안 시끄럽게', () => {
  const { 줄들, 다쟀나 } = 점검.훅판정([잰것('가'), 잰것('나')], []);
  assert.deepEqual(줄들, [], `할 말이 없는데 말했다: ${줄들.join(' / ')}`);
  assert.equal(다쟀나, true);
});

test('기준선이 없는 스킬은 «도장을 찍으라»고 알린다 — 처음 세우는 자리', () => {
  const { 줄들 } = 점검.훅판정([잰것('가', { 기준선: null, 처음: true })], []);
  const 글 = 줄들.join('\n');
  assert.ok(/기준선이 없는/.test(글), '기준선 없음을 안 알린다');
