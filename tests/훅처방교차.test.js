...
/* ── 축 ③ — 처방이 가리키는 «대상»이 실재한다 ──────────────────────────────
 * 「훅이 따를 수 없는 것을 시킨다」는 이 저장소에서 네 번째다: F103(원조) · F212(다른 가드가 막는다) ·
 * F511(빈 명령·틀린 도구) · F515(없는 탈출구). CLAUDE.md 신뢰성 — 3번째부터는 고치는 게 아니라
 * **원인을 쓸 수 없게 만든다**. 위 두 축이 «막힘»과 «없음»을 지므로, 남은 자리는 처방이 부르는
 * 도구 파일과 그 플래그다. 경량화(유호 상시 08-13)로 도구를 지우면 처방은 **조용히** 죽는다 —
 * 훅은 자기 사유문을 실행해 보지 않기 때문에 화면에는 끝까지 멀쩡한 명령으로 찍힌다.
 *
 * 🔑 실저장소 상태는 오늘 실측으로 **깨끗하다**(node 처방 27/27 · 플래그 14/14). 그러니 이 검사는
 *    버그가 남아 있기를 요구하지 않는다 — 거짓양성만 본다(CLAUDE.md 가드 맹점 ②).
 *
 * ⚠ 이 검사가 **못 보는 것**(통과와 미실행이 같은 모양이면 안 되므로):
 *   · F511 형태 — `node tools/codex-review.js ` 처럼 파일은 실재하는데 **인자가 비어** 못 쓰는 명령.
 *     대상 존재만 보므로 통과한다. 그 층은 PR #15 가 따로 진다.
 *   · 표기가 바뀐 처방(`node "$ROOT/tools/x.js"` 같은 변수 경로)은 수확이 0이 되고, 0은 통과와
 *     같은 모양이다 → 아래 「미측정 방어」가 분모를 지킨다. */

/** 훅 사유문이 부르는 저장소 안 도구 — [경로, 플래그|null] 쌍. */
function 도구처방수확(src) {
  const 것들 = [];
  for (const m of String(src).matchAll(/node\s+((?:tools|\.claude)[^\s`'")\\]+\.js)(\s+(--[^\s`'")\\]+))?/g)) {
    것들.push([m[1], m[3] || null]);
  }
  return 것들;
}

test('🔑 미측정 방어 — 도구 처방이 실제로 수확된다', () => {
  const 전부 = fs.readdirSync(훅방).filter((n) => n.endsWith('.js'))
    .flatMap((n) => 도구처방수확(fs.readFileSync(path.join(훅방, n), 'utf8')));
  assert.ok(new Set(전부.map(([p]) => p)).size >= 15,
    `도구 처방 ${new Set(전부.map(([p]) => p)).size}종만 수확됐다 — 표기가 바뀌었으면 도구처방수확을 넓혀라`);
  assert.ok(전부.some(([, f]) => f), '플래그가 붙은 처방이 0건이다 — 플래그 축이 통째로 공회전한다');
});

test('🔑 탐지력 — 없는 파일·없는 플래그를 판정기가 잡는다 (픽스처가 진다)', () => {
  assert.deepStrictEqual(도구처방수확('→ node tools/없는도구.js --없는플래그 로 확인한다'),
    [['tools/없는도구.js', '--없는플래그']], '경로·플래그를 못 뽑는다 — 아래 검사가 통째로 공회전한다');
  assert.deepStrictEqual(도구처방수확('→ node tools/board.js 를 본다'), [['tools/board.js', null]],
    '플래그 없는 처방을 못 뽑는다');
  assert.deepStrictEqual(도구처방수확('→ npx something --flag'), [], '남의 명령을 도구 처방으로 셌다');
});

/* 🔑 **자기 자신을 부르는 처방은 `includes` 로 못 잰다** — 변이가 그 구멍을 드러냈다(실측 08-16:
 *   `voice-guard --check` 를 `--없는플래그` 로 망가뜨렸는데 10건이 전부 초록이었다). 처방 문자열이
 *   그 파일 «안»에 있으니 자기 소스를 뒤지면 언제나 맞는다 — 가드가 자기 전처리에 눈이 머는 자리다.
 *   그래서 자기참조(6건 실측: credential·memory-index·memory-status·read-budget·screenshot·voice)일
 *   때만 **argv 문맥**을 요구한다. 여섯 다 `process.argv.includes('--x')` 한 표기라 대조가 성립한다.
 *
 * ⚠ argv 문맥은 「그 도구가 이 플래그를 **안다**」까지만 잰다 — 「그 분기가 살아 있다」는 **안 잰다.**
 *   실측 08-16: `voice-guard` 의 `--check` 분기 조건을 죽여도 바로 다음 줄의 인자 파싱이
 *   `argv.indexOf('--check')` 를 쓰고 있어 통과했다. 억지로 넓히면 파싱기를 짜야 하고, 그런 검사는
 *   표기가 조금만 달라져도 거짓양성을 뿜다가 하루 만에 꺼진다. 그 층은 각 도구의 자기 회귀가 진다. */
function 플래그실재(abs, 플래그, 자기참조) {
  const src = fs.readFileSync(abs, 'utf8');
  if (!자기참조) return src.includes(플래그);
  const e = 플래그.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return new RegExp(`argv[^\\n]*${e}`).test(src);
}

test('🔴 훅이 부르는 도구와 플래그가 실재한다 (F511·F515 계열 · 2026-08-16 실측)', () => {
  const 깨진것 = [];
  for (const n of fs.readdirSync(훅방).filter((f) => f.endsWith('.js'))) {
    for (const [경로, 플래그] of 도구처방수확(fs.readFileSync(path.join(훅방, n), 'utf8'))) {
      const abs = path.join(ROOT, 경로);
      if (!fs.existsSync(abs)) { 깨진것.push(`  ${n} → node ${경로}  (그 파일이 없다)`); continue; }
      const 자기참조 = path.resolve(abs) === path.resolve(훅방, n);
      if (플래그 && !플래그실재(abs, 플래그, 자기참조)) {
        깨진것.push(`  ${n} → ${path.basename(경로)} ${플래그}`
          + (자기참조 ? '  (자기 자신을 부르는데 argv 로 받는 자리가 없다)' : '  (그 도구가 모르는 플래그다)'));
      }
    }
  }
  assert.deepStrictEqual(깨진것, [],
    '훅이 «없는 것»을 부른다 — 도구를 지우거나 플래그 이름을 바꿀 때 처방은 조용히 죽는다.\n'
    + '훅은 자기 사유문을 실행해 보지 않으므로 화면에는 끝까지 멀쩡한 명령으로 찍힌다(F511 계열).\n'
    + '고치는 쪽은 **처방**이다 — 도구를 되살릴 이유가 없다면 사유문이 살아 있는 통로를 가리켜야 한다.\n'
    + 깨진것.join('\n'));
});
