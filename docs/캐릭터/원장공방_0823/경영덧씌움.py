...
안여백 = 124
글좌 = 카["좌"] + 안여백
기록.append(("제목", 글(카["좌"] + 8, 카["위"] - 150, "경영", 60, 잉크, True)))
...
for 줄 in ("재적이 두 달째 늘고 있어요", "활성률은 지난달과 같아요"):
    기록.append(("큰줄%d" % len([k for k, _ in 기록 if k.startswith("큰줄")]),
                 글(글좌, y, 줄, 50, 잉크, True)))
    y += 78
기록.append(("열기", 글(글좌, 밑줄y - 76, "숫자는 시트가 계산해요", 34, 보조, False)))
...
for 번, (값, 이름표) in enumerate(숫자):
    칸 = 칸들["숫자%d" % 번]
    가 = (칸["좌"] + 칸["우"]) / 2
    기록.append(("숫자%d" % 번, 글(가, 칸["위"] + 26, 값, 66, 잉크, True, 가운데=True)))
    기록.append(("이름%d" % 번, 글(가, 칸["위"] + 108, 이름표, 28, 보조2, False, 가운데=True)))

칸 = 칸들["월보열기"]
중 = (칸["위"] + 칸["아래"]) / 2
기록.append(("월보열기", 글(칸["좌"] + 안여백, 중 - 26, "월보 전체 보기", 40, 보조, True)))
기록.append(("월보꼬리", 글(칸["우"] - (안여백 - 40), 중 - 16, "시트로", 28, 보조2, False, 오른쪽=True)))
