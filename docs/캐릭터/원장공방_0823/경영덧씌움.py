# -*- coding: utf-8 -*-
"""원장 「경영」 탭에 산 글자를 붓는다 — 그릇의 화면 칸(<출력>.png.칸.json)을 읽어 앉힌다.

손좌표를 안 쓰는 규율은 앞 네 화면과 같다: **그릇을 옮기면 글이 따라와야 한다.**


쓰기:  python 경영덧씌움.py 경영판_v1_그릇만 [없음|층별|전면]
       → 경영판_v1[_층별].png  +  대비 실측표
"""
...
이름 = sys.argv[1] if len(sys.argv) > 1 else "경영판_v1_그릇만"
...
안여백 = 픽(124)
글좌 = 카["좌"] + 안여백
기록.append(("제목", 글(카["좌"] + 픽(8), 카["위"] - 픽(150), "경영", 60, 잉크, True)))

밑줄y = 카["위"] + 카높 * 0.8429
y = 카["위"] + 픽(70)
기록.append(("라벨", 글(글좌, y, "8월 월보", 34, 보조2, False)))
y += 픽(62)
for 줄 in ("재적이 두 달째 늘고 있어요", "활성률은 지난달과 같아요"):
    기록.append(("큰줄%d" % len([k for k, _ in 기록 if k.startswith("큰줄")]),
                 글(글좌, y, 줄, 50, 잉크, True)))
    y += 픽(78)
기록.append(("열기", 글(글좌, 밑줄y - 픽(76), "숫자는 시트가 계산해요", 34, 보조, False)))

# ── ② 기록 칸 넷 — 성장판 「12 / 7」 문법의 2×2 판 ─────────────────────────────
숫자 = ((u"128", u"재적"), (u"71%", u"활성률"), (u"9", u"신규"), (u"3", u"이탈"))
for 번, (값, 이름표) in enumerate(숫자):
    칸 = 칸들["숫자%d" % 번]
    가 = (칸["좌"] + 칸["우"]) / 2
    기록.append(("숫자%d" % 번, 글(가, 칸["위"] + 픽(26), 값, 66, 잉크, True, 가운데=True)))
    기록.append(("이름%d" % 번, 글(가, 칸["위"] + 픽(108), 이름표, 28, 보조2, False, 가운데=True)))

칸 = 칸들["월보열기"]
중 = (칸["위"] + 칸["아래"]) / 2
기록.append(("월보열기", 글(칸["좌"] + 안여백, 중 - 픽(26), "월보 전체 보기", 40, 보조, True)))
기록.append(("월보꼬리", 글(칸["우"] - (안여백 - 픽(40)), 중 - 픽(16), "시트로", 28, 보조2, False, 오른쪽=True)))
...
낸것 = os.path.join(여기, "경영판_v1" + 꼬리 + ".png")
im.save(낸것)
...
bl = 바탕.load()
색표 = {"라벨": 보조2, "열기": 보조, "월보열기": 보조, "월보꼬리": 보조2}
색표.update({"이름%d" % i: 보조2 for i in range(4)})
색표.update({"탭%d" % k: (보조 if k == 2 else 보조2) for k in range(3)})
