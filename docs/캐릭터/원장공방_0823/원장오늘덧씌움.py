...
안여백 = 124
글좌 = 카["좌"] + 안여백
기록.append(("제목", 글(카["좌"] + 8, 카["위"] - 150, "오늘", 60, 잉크, True)))
...
for 줄 in ("16반 중 9반이 돌고 있어요", "등원 74명 · 말한 학생 51명"):
    기록.append(("큰줄%d" % len([k for k, _ in 기록 if k.startswith("큰줄")]),
                 글(글좌, y, 줄, 50, 잉크, True)))
    y += 78
기록.append(("열기", 글(글좌, 밑줄y - 76, "공지 쓰기 — 10분 뒤 몽골어까지", 34, 보조, False)))
...
for 번, (이름표, 값, 조용) in enumerate(임계):
    ...
    단추x = 칸["좌"] + 폭 * 0.0950
    중 = (칸["위"] + 칸["아래"]) / 2
    기록.append(("임계%d" % 번, 글(단추x + 92, 중 - 26, 이름표, 40,
                                 보조2 if 조용 else 보조, not 조용)))
    기록.append(("값%d" % 번, 글(칸["우"] - (단추x - 칸["좌"]), 중 - 16, 값, 28, 보조2, False, 오른쪽=True)))

# ── ③ 데모 모드 — 단추 없는 줄(눌러서 연다) ──────────────────────────────────
칸 = 칸들["데모"]
중 = (칸["위"] + 칸["아래"]) / 2
기록.append(("데모", 글(칸["좌"] + 안여백, 중 - 26, "데모 모드로 보기", 40, 보조, True)))
기록.append(("데모꼬리", 글(칸["우"] - (안여백 - 40), 중 - 16, "가상 크루 8명", 28, 보조2, False, 오른쪽=True)))
