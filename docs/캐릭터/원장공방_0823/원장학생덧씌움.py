...
안여백 = 픽(124)
글좌 = 카["좌"] + 안여백
기록.append(("제목", 글(카["좌"] + 픽(8), 카["위"] - 픽(150), "학생", 60, 잉크, True)))
...
for 줄 in ("두 명이 2주째 말이 없어요", "한 명은 결석이 늘고 있어요"):
    기록.append(("큰줄%d" % len([k for k, _ in 기록 if k.startswith("큰줄")]),
                 글(글좌, y, 줄, 50, 잉크, True)))
    y += 픽(78)
기록.append(("열기", 글(글좌, 밑줄y - 픽(76), "가장 급한 한 명 보기", 34, 보조, False)))
...
for 번, (이름표, 신호, 챙김) in enumerate(위험):
    ...
    단추x = 칸["좌"] + 폭 * 0.0950
    중 = (칸["위"] + 칸["아래"]) / 2
    기록.append(("위험%d" % 번, 글(단추x + 픽(92), 중 - 픽(26), 이름표, 40,
                                 보조2 if 챙김 else 보조, not 챙김)))
    기록.append(("신호%d" % 번, 글(칸["우"] - (단추x - 칸["좌"]), 중 - 픽(16), 신호, 28, 보조2, False, 오른쪽=True)))
