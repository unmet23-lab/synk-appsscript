# 유리실험굽기 — 요소굽기.py 의 «실험 사본»(세션 06168e3f · 2026-08-20 유호 지시).
# 실험 둘: ① 유리판 조명 교정(위아래 밝기 기울기 제거) ② 명품 글자(몰드·에칭) 시제.
# ⚠원본 규율 「글자는 굽지 않는다」와 어긋나는 실험이다 — 유호님 명시 요청으로 사본에서만 시험.
#   본문 글자는 여전히 HTML 층 몫이고, 여기서 굽는 것은 «번호·라벨급 오브제 글자» 한 점이다.
# 원본: 2699bf78 워크트리 tools/요소굽기.py — 무변경.
# ──────────────────────────────────────────────────────────────────────────
...
def 유리글자(본문, 크기, 위치, 재질, 돌출=0.032, 베벨=0.006):
    """유리판 위 «오브제 글자» — 잉크가 아니라 재질로 서는 글자.
    몰드(같은 유리로 도톰하게 솟음)와 에칭(짙은 서리 얇게)이 이 하나로 갈린다 — 재질·돌출만 다르다.
    베벨이 핵심이다: 모서리가 둥글어야 빛을 물어 «부어 만든» 유리로 읽힌다(각지면 아크릴 간판)."""
    bpy.ops.object.text_add()
    t = bpy.context.object
    t.data.body = 본문
    try:
        t.data.font = bpy.data.fonts.load(r'C:\Windows\Fonts\malgunbd.ttf')
    except Exception:
        pass                                   # 폰트 없으면 기본체 — 클라우드 통로 대비
    t.data.size = 크기
    t.data.extrude = 돌출
...
def 유리판몰드():
    """명품 글자 시제 ① — 판과 «같은 유리»로 부어 만든 숫자. 잉크 대비가 아니라
    굴절 모서리로 읽히므로, 평면 글자가 탈락한 판 위쪽에서도 산다(가설 — 렌더로 검증)."""
    판뒤구()
    재질, 두께 = 판재질('유리')
    판 = 베개몸((1.35, 두께, 0.95), 크리스=0.38, 레벨=4)
    판.data.materials.append(재질)
    유리글자('03', 0.62, (-0.72, -두께 - 0.016, 0.40), 재질, 돌출=0.034, 베벨=0.007)
    return (0, -6.8, 0.0), 90


def 유리판에칭():
    """명품 글자 시제 ② — 샌드블라스트 에칭. 서리 유리 위에 «더 짙은 서리»로 새긴 낱말.
    향수병·위스키 잔의 문법 — 얇게(돌출 0.006), 광을 더 죽인 서리(거칠기 0.85)로."""
    판뒤구()
    재질, 두께 = 판재질('유리')
    판 = 베개몸((1.35, 두께, 0.95), 크리스=0.38, 레벨=4)
    판.data.materials.append(재질)
    서리 = bpy.data.materials.new('짙은서리')
    서리.use_nodes = True
    p = 서리.node_tree.nodes['Principled BSDF']
    p.inputs['Base Color'].default_value = 리니어(색['Paper'])
    p.inputs['Roughness'].default_value = 0.85
    if 'Transmission Weight' in p.inputs:
        p.inputs['Transmission Weight'].default_value = 1.0
    p.inputs['IOR'].default_value = 1.45
    유리글자('한국어', 0.36, (-0.55, -두께 - 0.004, 0.02), 서리, 돌출=0.006, 베벨=0.002)
    return (0, -6.8, 0.0), 90
...
def 자수글자():
    """㉮ 자수 글자 — 펠트 아플리케 「가」 + 윤곽 테두리 땀(와펜 테두리 문법을 글자에).
    글자를 펠트로 오려 붙이고 실땀이 윤곽을 두른다. 자리 = 로고·수료 도장급 «한 점»."""
    재질, 두께 = 판재질('다린천')
    바닥 = 베개몸((1.35, 두께, 0.95), 크리스=0.34, 레벨=4)
    바닥.data.materials.append(재질)
    # 펠트 글자 — 텍스트를 메시로 바꿔 짧은 퍼를 심는다
    bpy.ops.object.text_add()
    t = bpy.context.object
    t.data.body = '가'
    try:
    ...
    글y = -0.075
    t.location = (0, 글y, 0.05)
    bpy.ops.object.convert(target='MESH')
    짧은퍼(t, 염료이름, 살재질(염료이름, 색[염료이름]),
          털재질(염료이름, 색[염료이름]), 길이=0.032, 개수=7000)   # 0.05 는 획이 뭉개졌다(시험 실측)
    # 테두리 땀 — 윤곽을 실땀(Stitch)이 두른다
    실재 = 매끈재질('테두리실', 색['Stitch'], 거칠기=0.5)
    for (px, py, 각) in 글자곡선표본('가', 1.15, 간격=0.115):
        땀하나(px, 글y - 0.026, py + 0.05, 0.046, 0.018, 실재, 회전y=-각)
    return (0, -6.8, 0.0), 90


def 레터프레스():
    """㉯ 레터프레스 — 다린 천에 다리미로 눌러 찍은 오목 글자.
    불리언 실절단(단춧구멍 문법): 글자 모양대로 천을 파내면 눌린 벽의 그늘이 획을 긋는다."""
    재질, 두께 = 판재질('다린천')
    판 = 베개몸((1.35, 두께, 0.95), 크리스=0.34, 레벨=4)
    판.data.materials.append(재질)
    bpy.ops.object.text_add()
    ...
    커터.data.body = '숙제'
    try:
        커터.data.font = bpy.data.fonts.load(r'C:\Windows\Fonts\malgunbd.ttf')
    except Exception:
        pass
    커터.data.size = 0.62
    커터.data.extrude = 0.03                  # 0.02 는 그늘이 여렸다(시험 실측) — 더 깊이 누른다
    커터.data.align_x = 'CENTER'
    커터.data.align_y = 'CENTER'
    커터.rotation_euler = (math.radians(90), 0, 0)
    커터.location = (0, -두께, 0.06)          # 앞면(-두께)에 걸쳐 파인다
    bpy.ops.object.convert(target='MESH')
    m = 판.modifiers.new('눌림', 'BOOLEAN')
    m.operation = 'DIFFERENCE'
    m.object = 커터
    커터.hide_render = True                   # 커터는 렌더에서 숨긴다 — 파인 자국만 남는다
    return (0, -6.8, 0.0), 90


def 라벨태그():
    """㉰ 직조 라벨 태그 + 실 글자 — «3 / 10» 메타데이터가 옷 케어라벨처럼 짜여 붙는다.
    직조라벨(제안 #10)을 그대로 세우고, 테두리 실과 같은 재질의 도톰한 실 글자를 놓는다."""
    카메라 = 직조라벨()
    실재 = 매끈재질('글실', 색['Deep Wool'], 거칠기=0.72)
    bpy.ops.object.text_add()
    t = bpy.context.object
    t.data.body = '3 / 10'
    try:
        ...
        pass
    t.data.size = 0.30
    t.data.extrude = 0.011
    t.data.bevel_depth = 0.004
    t.data.bevel_resolution = 3
    t.data.align_x = 'CENTER'
    t.data.align_y = 'CENTER'
    t.rotation_euler = (math.radians(90), 0, 0)
    t.location = (0, -0.066, 0.30)            # 라벨 앞면(-0.055)에 등이 닿게
    t.data.materials.append(실재)
    # 아래엔 더 작은 보조 줄 — 태그다운 두 줄 구성
    bpy.ops.object.text_add()
    소 = bpy.context.object
    소.data.body = 'SYNK'
    try:
        소.data.font = bpy.data.fonts.load(r'C:\Windows\Fonts\malgunbd.ttf')
    except Exception:
        pass
    소.data.size = 0.15
    소.data.extrude = 0.008
    소.data.bevel_depth = 0.003
    소.data.align_x = 'CENTER'
    소.data.align_y = 'CENTER'
    소.rotation_euler = (math.radians(90), 0, 0)
    소.location = (0, -0.063, -0.55)          # 주 숫자와 간격을 벌린다 — 태그의 두 줄 문법
    소.data.materials.append(매끈재질('보조실', 색['Ash Wool'], 거칠기=0.72))
    return 카메라


def 자수글자2():
    """㉮ 2차 — 밤천(Ink) 위 코랄 아플리케. 앱 다크(양모 밤)와 같은 땅에서 가시성을 세운다.
    1차 실측 교정: 털 0.032→0.026(윤곽 또렷) · 글자 1.15→1.28(존재감) · 개수 7000→9000(밀도)."""
    ...
    bpy.ops.object.convert(target='MESH')
    # 밤천 실측 교정 2벌 — 어두운 바닥은 잔털 실루엣을 전부 드러내는데,
    # 텍스트 변환 메시는 법선·밀도가 들쭉날쭉해 털이 제멋대로 뻗쳤다(시험 2회 재현).
    # ① 복셀 리메시로 표면을 고르게 깐 뒤 심는다 ② 짧게 + 아이 가닥을 줄인다.
    ...
    실재 = 매끈재질('테두리실', 색['Stitch'], 거칠기=0.5)
    for (px, py, 각) in 글자곡선표본('가', 1.28, 간격=0.115):
        땀하나(px, 글y - 0.026, py + 0.03, 0.046, 0.018, 실재, 회전y=-각)
    return (0, -6.8, 0.0), 90


def 레터프레스2():
    """㉯ 2차 — 먹을 눌러 찍은 레터프레스. 빈 눌림(1차)의 바닥에 Ink 상감을 채운다:
    눌린 그늘 + 먹색이 겹으로 획을 그어 가시성이 선다(Oat 위 Ink ≈ 11:1)."""
    재질, 두께 = 판재질('다린천')
    판 = 베개몸((1.35, 두께, 0.95), 크리스=0.34, 레벨=4)
    판.data.materials.append(재질)
    # 커터 — 눌림 홈
    ...
    m.object = 커터
    커터.hide_render = True
    # 상감 — 홈 바닥에 채운 먹(살짝 안쪽으로 파고 앉는다: 표면보다 0.017 낮게)
    먹 = 매끈재질('먹', 색['Ink'], 거칠기=0.92)
    유리글자('숙제', 0.6696, (0, -0.026, 0.06), 먹, 돌출=0.012, 베벨=0.0)
    return (0, -6.8, 0.0), 90


def 라벨태그2():
    """㉰ 2차 — 진행 숫자만 신호로. «지금 하는 3»은 코랄 글자(Coral 3 — 철칙 ②),
    나머지(/ 10)는 양모 실 그대로: 위계가 색 1점으로 선다."""
    카메라 = 직조라벨()
    강조 = 매끈재질('진행실', 색['Coral 3'], 거칠기=0.6)
    실재 = 매끈재질('글실', 색['Deep Wool'], 거칠기=0.72)
    유리글자('3', 0.40, (-0.26, -0.070, 0.30), 강조, 돌출=0.012, 베벨=0.004)
    유리글자('/ 10', 0.26, (0.20, -0.064, 0.285), 실재, 돌출=0.010, 베벨=0.004)
...
def 자수글자3():
    """㉮ 3차 — 손맛. 기계가 놓은 땀은 완벽해서 차갑다 — 손바느질은 땀마다 조금씩 다르다.
    각도 ±8° · 길이 ±12% · 자리 ±0.004 지터(고정 씨앗 — 재현 가능). 조명은 «결광»으로."""
    바닥 = 베개몸((1.35, 0.055, 0.95), 크리스=0.34, 레벨=4)
    바닥.data.materials.append(직물결(매끈재질('밤천', 색['Ink'], 거칠기=0.96), 규모=200.0, 세기=0.045))
    bpy.ops.object.text_add()
    t = bpy.context.object
...
def 레터프레스3():
    """㉯ 3차 — 스침광. 눌림은 빛이 표면을 «스칠 때» 사는 공예다(그늘이 깊어진다).
    + 눌린 모서리를 살짝 둥글린다(커터 베벨) — 실제 압인은 각이 눌려 무너진다."""
    재질, 두께 = 판재질('다린천')
    판 = 베개몸((1.35, 두께, 0.95), 크리스=0.34, 레벨=4)
    판.data.materials.append(재질)
    bpy.ops.object.text_add()
    ...
    커터.data.size = 0.68
    커터.data.extrude = 0.03
    # ⚠커터에 베벨 금지 — 베벨 텍스트는 획 모서리에서 자기교차가 생겨 불리언이 천을 통째로 삼킨다(시험 2회 재현).
    #   눌린 모서리의 부드러움은 스침광의 그늘이 대신 낸다.
    커터.data.align_x = 'CENTER'
    커터.data.align_y = 'CENTER'
    커터.rotation_euler = (math.radians(90), 0, 0)
    커터.location = (0, -두께, 0.06)
...
def 라벨태그3():
    """㉰ 3차 — 테두리 땀에도 손맛(지터) + 스침광으로 실이 그림자를 문다.
    직조라벨을 그대로 세우되 테두리를 손 지터판으로 다시 놓는다."""
    라벨 = 베개몸((0.95, 0.055, 1.3), 크리스=0.42)
    ...
    for (sx, sz, ex, ez) in ((-w, h, w, h), (w, h, w, -h), (w, -h, -w, -h), (-w, -h, -w, h)):
        ...
        for i in range(n):
            t = (i + 0.5) / n
            땀하나(sx + (ex - sx) * t + 손.uniform(-0.006, 0.006), 앞,
                  sz + (ez - sz) * t + 손.uniform(-0.006, 0.006),
...
def 글자등(받는것들, 위치=(0, -2.2, 0.4), 크기=1.6, 힘=150):
    """«글자만 받는» 보조광 — 라이트 링킹의 역방향 사용.
    무대의 분위기(결광·스침)는 안 건드리고 글자의 명도만 반 단계 올린다.
    받는것들 = 이 빛을 받을 오브젝트 목록(글자·땀). 힘은 은은하게 — 세면 무대와 그림자가 어긋난다."""
...
def 자수글자4():
    """㉮ 4차 — 3차 그대로 + 글자등. 글자 메시와 땀들만 은은한 정면광을 받아
    결광의 비네트·림 후광은 유지한 채 글자 명도만 반 단계 오른다."""
    바닥 = 베개몸((1.35, 0.055, 0.95), 크리스=0.34, 레벨=4)
    바닥.data.materials.append(직물결(매끈재질('밤천', 색['Ink'], 거칠기=0.96), 규모=200.0, 세기=0.045))
    bpy.ops.object.text_add()
    t = bpy.context.object
    ...
    받는것 = [t]
    for (px, py, 각) in 글자곡선표본('가', 1.28, 간격=0.115):
        받는것.append(땀하나(px + 손.uniform(-0.004, 0.004), 글y - 0.026,
                          py + 0.03 + 손.uniform(-0.004, 0.004),
                          0.046 * 손.uniform(0.88, 1.12), 0.018, 실재,
                          회전y=-각 + 손.uniform(-8, 8)))
    글자등(받는것, 위치=(0, -2.2, 0.3), 크기=1.6, 힘=100)   # 170·140 은 코랄이 분홍으로 씻겼다
    return (0, -6.8, 0.0), 90


def 레터프레스4():
    """㉯ 4차 — 3차 그대로 + 역방향 글자등: 어두운 글자는 띄우면 죽는다 —
    대신 «천만» 은은한 정면광을 받아 먹과의 대비가 오른다."""
    재질, 두께 = 판재질('다린천')
    판 = 베개몸((1.35, 두께, 0.95), 크리스=0.34, 레벨=4)
    판.data.materials.append(재질)
    bpy.ops.object.text_add()
    ...
    m.object = 커터
    커터.hide_render = True
    먹 = 매끈재질('먹', 색['Ink'], 거칠기=0.85)   # 0.92→0.85 — 인쇄 잉크의 새틴 한 방울(모서리 하이라이트)
    유리글자('숙제', 0.6696, (0, -0.026, 0.06), 먹, 돌출=0.012, 베벨=0.0)
    # 힘은 인자 「등힘=」으로 조율 — 200 은 스침의 명암이 씻겼다(4차), 150 도 위계가 평평했다(유호 소감).
    글자등([판], 위치=(0, -2.4, 0.3), 크기=2.2, 힘=float(인자.get('등힘', '150')))
    return (0, -6.8, 0.0), 90


def 라벨태그4():
    """㉰ 4차 — 3차 그대로 + 글자등(실 글자 셋만). 테두리 땀·천은 스침광의 그늘을 유지."""
    라벨 = 베개몸((0.95, 0.055, 1.3), 크리스=0.42)
    라벨.data.materials.append(직물결(매끈재질('라벨천', 색['Oat'], 거칠기=0.88)))
    실재 = 매끈재질('실', 색['Deep Wool'], 거칠기=0.72)
    손 = random.Random(20260820)
    ...
    # ⚠글자에 직접 쏘면 죽는다(시험 실측) — 코랄 3 은 «어두운» 글자색이라 빛을 받으면 바래서
    #   분홍으로 흐려진다. 레터프레스와 같은 갈래: 천만 받아 어두운 글자의 대비를 올린다.
    글자등([라벨], 위치=(0, -2.4, 0.0), 크기=2.0, 힘=float(인자.get('등힘', '140')))
    return (0, -9.2, 0.0), 90
...
형태들 = {'오브': 오브, '알약': 알약, '아이콘': 아이콘, '스티치': 스티치, '털실진행바': 털실진행바,
        ...
        '유리판몰드': 유리판몰드, '유리판에칭': 유리판에칭,
        '자수글자': 자수글자, '레터프레스': 레터프레스, '라벨태그': 라벨태그,
        '자수글자2': 자수글자2, '레터프레스2': 레터프레스2, '라벨태그2': 라벨태그2,
        '자수글자3': 자수글자3, '레터프레스3': 레터프레스3, '라벨태그3': 라벨태그3,
        '자수글자4': 자수글자4, '레터프레스4': 레터프레스4, '라벨태그4': 라벨태그4,
...
# ── 조명 변주(인자 「조명=」) — 유리판의 위아래 밝기 기울기(실측 154→44)가 표적이다 ──
조명 = 인자.get('조명', '원본')
if 조명 == '원본':
    ...
    # 키를 판 정면 높이로 끌어내린다 — 위에서 쏟는 대신 얼굴을 마주 본다
    등('키', (-2.5, -2.7, 0.7), (math.radians(80), math.radians(-24), 0), 5.0, 1150)
    등('필', (2.6, -2.2, 0.5), (math.radians(82), math.radians(26), 0), 4.0, 90)
    등('림', (0.6, 2.6, 2.8), (math.radians(-42), 0, 0), 3.0, 560)
elif 조명 == '중간':
    # 절충 — 키를 반쯤 내리고 반쯤 눕힌다(입체는 남기고 기울기만 줄인다)
    등('키', (-2.7, -2.5, 1.9), (math.radians(63), math.radians(-27), 0), 4.5, 1250)
    등('필', (2.6, -2.2, 0.5), (math.radians(80), math.radians(27), 0), 4.0, 85)
    등('림', (0.6, 2.6, 2.8), (math.radians(-42), 0, 0), 3.0, 560)
elif 조명 == '채움':
    # 원본 그대로 두고 아래에서 채운다 — 기울기의 어두운 끝만 들어올린다
    등('키', (-2.8, -2.2, 3.4), (math.radians(46), math.radians(-30), 0), 4.0, 1250)
    등('밑채움', (0.2, -2.5, -1.9), (math.radians(112), 0, 0), 4.0, 430)
    등('필', (2.6, -2.2, 0.6), (math.radians(78), math.radians(28), 0), 4.0, 80)
    등('림', (0.6, 2.6, 2.8), (math.radians(-42), 0, 0), 3.0, 560)
elif 조명 == '약키':
    # 빛을 «빼는» 길 ① — 키만 약하게. 판 전체가 아래쪽처럼 가라앉는다(밝은 잉크의 땅).
    등('키', (-2.8, -2.2, 3.4), (math.radians(46), math.radians(-30), 0), 4.0, 450)
    등('필', (2.6, -2.2, 0.6), (math.radians(78), math.radians(28), 0), 4.0, 80)
    등('림', (0.6, 2.6, 2.8), (math.radians(-42), 0, 0), 3.0, 560)
elif 조명 == '가림':
    # 빛을 «빼는» 길 ② — 라이트 링킹: 키가 «판만» 비추지 않는다.
    # 구슬·다른 것은 원본 그대로 밝고, 판 얼굴만 그늘에 남는다 — 빛을 재단하는 셈이다.
    등('키', (-2.8, -2.2, 3.4), (math.radians(46), math.radians(-30), 0), 4.0, 1400)
    등('필', (2.6, -2.2, 0.6), (math.radians(78), math.radians(28), 0), 4.0, 80)
    등('림', (0.6, 2.6, 2.8), (math.radians(-42), 0, 0), 3.0, 560)
    유리들 = set()
    for o in bpy.data.objects:
        if o.type in ('MESH', 'CURVE', 'FONT') and getattr(o.data, 'materials', None):
            if any(m and ('유리' in m.name or '서리' in m.name) for m in o.data.materials):
                유리들.add(o)
    받는 = bpy.data.collections.new('키_받는것')
    씬.collection.children.link(받는)
    for o in bpy.data.objects:
        if o.type in ('MESH', 'CURVE', 'FONT', 'SURFACE') and o not in 유리들:
            받는.objects.link(o)
    bpy.data.objects['키'].light_linking.receiver_collection = 받는
elif 조명 == '림아래':
    # 기울기를 «뒤집는» 길 — 림을 뒤 아래로. 서리 유리는 역광이 닿는 쪽이 발광하므로
    # 위(글자 자리)는 가라앉고 아래가 진열장 받침처럼 은은히 빛난다.
    등('키', (-2.8, -2.2, 3.4), (math.radians(46), math.radians(-30), 0), 4.0, 1400)
    등('필', (2.6, -2.2, 0.6), (math.radians(78), math.radians(28), 0), 4.0, 80)
    등('림', (0.6, 2.6, -2.4), (math.radians(-138), 0, 0), 3.0, 560)
elif 조명 == '가림림아래':
    # 결정판 후보 — 키에서 판을 빼고(가림) + 림을 아래로(림아래).
    # 위는 온전히 어둡고(글자의 땅) 아래는 발광(입체의 땅) — 빛의 상하 재단.
    등('키', (-2.8, -2.2, 3.4), (math.radians(46), math.radians(-30), 0), 4.0, 1400)
    등('필', (2.6, -2.2, 0.6), (math.radians(78), math.radians(28), 0), 4.0, 80)
    등('림', (0.6, 2.6, -2.4), (math.radians(-138), 0, 0), 3.0, 560)
    유리들 = set()
    ...
    # 펠트 «결»의 조명 — 키를 작고 가깝게(자연 비네트) + 림을 세게(잔털 실루엣이 빛을 문다).
    등('키', (-1.9, -2.3, 1.6), (math.radians(62), math.radians(-27), 0), 2.2, 950)
    등('필', (2.6, -2.2, 0.5), (math.radians(80), math.radians(27), 0), 4.0, 50)
    등('림', (0.3, 2.4, 2.6), (math.radians(-44), 0, 0), 2.0, 900)
elif 조명 == '결광2':
    # 결광의 과유불급 교정(유호 소감 「너무 밝게 해놨나」 → 실측 재판정 2회):
    # 분홍 씻김의 진범 = 키의 «거리». 결광 키는 원본보다 1.5배 가까워 역제곱으로 두 배 세게 때린다
    # (950@3.4 ≈ 2050@5.1 상당). 거리 보정하면 650 이 원본 1400 의 노출과 같다.
    # 림도 900→560 으로 — 후광은 은은하게. 비네트(작은 키)는 남긴다.
    ...
    # 14° 는 천이 통째로 꺼졌다(코사인 소멸 · 시험 실측) — 33° 로 세운다.
    ...
    raise SystemExit('모르는 조명: ' + 조명 + ' — 원본·정면·중간·채움·약키·가림·림아래·가림림아래·결광·스침')
