...
안여백 = 픽(124)
글좌 = 카["좌"] + 안여백
기록.append(("제목", 글(카["좌"] + 픽(8), 카["위"] - 픽(150 if not 전면 else 186), "검수", 60, 잉크, True)))

# 🔑 카드 세로의 **51% 지점에 가르는 실땀**이 구워져 있다(굽기 층). 글은 그 위아래로 갈린다 —
#   이 화면이 묻는 것이 «원문과 교정 사이가 맞나»라, 그 사이가 눈에 보여야 판정이 쉬워진다.
가름y = 카["위"] + 카높 * 0.51

기록.append(("라벨", 글(글좌, 카["위"] + 픽(58), "이번 주 5건 중 2건째", 34, 보조2, False)))
기록.append(("원문표", 글(글좌, 가름y - 픽(152), "학생이 쓴 것", 26, 보조2, False)))
기록.append(("원문", 글(글좌, 가름y - 픽(110), "어제 학교에 가고 있었다", 46, 보조, True)))
기록.append(("교정표", 글(글좌, 가름y + 픽(42), "AI 가 고친 것", 26, 보조2, False)))
기록.append(("교정", 글(글좌, 가름y + 픽(84), "어제 학교에 갔다", 46, 잉크, True)))
if 키릴줄:
    기록.append(("카드몽", 글(글좌, 가름y + 픽(158), 몽["카드"], 26, 보조2, False, 서체=키릴)))
...
for 번, 말 in enumerate(판정):
    ...
    가 = (칸["좌"] + 칸["우"]) / 2
    중 = (칸["위"] + 칸["아래"]) / 2
    기록.append(("판정%d" % 번, 글(가, 중 - 픽(26), 말, 40, 보조, True, 가운데=True)))
