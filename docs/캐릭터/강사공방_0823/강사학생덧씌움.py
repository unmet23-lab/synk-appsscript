...
안여백 = 픽(124)
글좌 = 카["좌"] + 안여백
기록.append(("제목", 글(카["좌"] + 픽(8), 카["위"] - 픽(150 if not 전면 else 186), "학생", 60, 잉크, True)))
...
기록.append(("열기", 글(글좌, 밑줄y - 픽(76), "약점 메모 쓰기", 34, 보조, False)))

# ── ② 칭찬 4태그 — 어휘 넷은 **개수가 불변**이다(오늘의 도전·성장 · 크루의 눈의 재료) ────────
태그 = ("잘 물어봤다", "끝까지 했다", "친구를 도왔다", "용기를 냈다")
for 번, 말 in enumerate(태그):
    칸 = 칸들["태그%d" % 번]
    가 = (칸["좌"] + 칸["우"]) / 2
    기록.append(("태그%d" % 번, 글(가, 칸["아래"] + 픽(24), 말, 24, 보조2, False, 가운데=True)))
