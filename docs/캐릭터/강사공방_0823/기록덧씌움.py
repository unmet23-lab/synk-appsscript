...
배 = (칸들["안닫힌차시"]["우"] - 칸들["안닫힌차시"]["좌"]) / _기준카드폭
...
안여백 = 픽(124)
글좌 = 카["좌"] + 안여백
기록.append(("제목", 글(카["좌"] + 픽(8), 카["위"] - 픽(150 if not 전면 else 186), "기록", 60, 잉크, True)))
...
기록.append(("열기", 글(글좌, 밑줄y - 픽(76), "지금 마감하기 — 7항목", 34, 보조, False)))
...
for 번, (이름표, 진도) in enumerate(차시):
    ...
    단추x = 칸["좌"] + 폭 * 0.0950
    중 = (칸["위"] + 칸["아래"]) / 2
    기록.append(("차시%d" % 번, 글(단추x + 픽(92), 중 - 픽(26), 이름표, 40, 보조2, False)))
    기록.append(("진도%d" % 번, 글(칸["우"] - (단추x - 칸["좌"]), 중 - 픽(16), 진도, 28, 보조2, False, 오른쪽=True)))
