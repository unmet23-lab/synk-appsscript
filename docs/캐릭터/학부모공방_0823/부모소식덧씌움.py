...
기록.append(("제목", 글(카["좌"] + 8, 카["위"] - (150 if not 전면 else 186), "소식", 60, 잉크, True)))
if 전면:
...
기록.append(("열기", 글(글좌, 밑줄y - 76, "자세히 보기", 34, 보조, False)))
...
for 번, (이름표, 날짜, 읽음) in enumerate(공지):
    ...
    단추x = 칸["좌"] + 폭 * 0.0950
    중 = (칸["위"] + 칸["아래"]) / 2
    기록.append(("공지%d" % 번, 글(단추x + 92, 중 - 26, 이름표, 40,
                                 보조2 if 읽음 else 보조, not 읽음)))
    기록.append(("날짜%d" % 번, 글(칸["우"] - (단추x - 칸["좌"]), 중 - 16, 날짜, 28,
                                 보조2, False, 오른쪽=True)))

# ── ③ 문의하기 — 단추가 없는 줄이라 «눌러서 여는 것»이다 ──────────────────────
칸 = 칸들["문의"]
중 = (칸["위"] + 칸["아래"]) / 2
기록.append(("문의", 글(칸["좌"] + 안여백, 중 - 26, "선생님께 문의하기", 40, 보조, True)))
기록.append(("문의꼬리", 글(칸["우"] - (안여백 - 40), 중 - 16, "열기", 28, 보조2, False, 오른쪽=True)))
