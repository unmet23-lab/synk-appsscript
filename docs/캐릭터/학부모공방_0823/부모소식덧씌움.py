...
쓰기:  python 부모소식덧씌움.py 부모소식판_v1_그릇만 [없음|층별|전면]
       → 부모소식판_v1[_층별].png  +  대비 실측표
"""
...
이름 = sys.argv[1] if len(sys.argv) > 1 else "부모소식판_v1_그릇만"
...
밑줄y = 카["위"] + 카높 * 0.8429
y = 카["위"] + 픽(70)
기록.append(("라벨", 글(글좌, y, "새 공지", 34, 보조2, False)))
y += 픽(62)
for 줄 in ("8월 27일은", "학원이 쉽니다"):
    기록.append(("큰줄%d" % len([k for k, _ in 기록 if k.startswith("큰줄")]),
                 글(글좌, y, 줄, 56, 잉크, True)))
    y += 픽(84)
if 키릴줄:
    기록.append(("카드몽", 글(글좌, y + 픽(12), 몽["카드"], 28, 보조2, False, 서체=키릴)))
기록.append(("열기", 글(글좌, 밑줄y - 픽(76), "자세히 보기", 34, 보조, False)))

# ── ② 지난 공지 셋 — 안 읽은 것이 위 ────────────────────────────────────────
공지 = ((u"9월 수업료 안내", u"어제", False),
        (u"여름 특강 결과", u"5일 전", True),
        (u"학부모 상담 주간", u"지난주", True))
for 번, (이름표, 날짜, 읽음) in enumerate(공지):
    칸 = 칸들["지난%d" % 번]
    폭 = 칸["우"] - 칸["좌"]
    단추x = 칸["좌"] + 폭 * 0.0950
    중 = (칸["위"] + 칸["아래"]) / 2
    기록.append(("공지%d" % 번, 글(단추x + 픽(92), 중 - 픽(26), 이름표, 40,
                                 보조2 if 읽음 else 보조, not 읽음)))
    기록.append(("날짜%d" % 번, 글(칸["우"] - (단추x - 칸["좌"]), 중 - 픽(16), 날짜, 28,
                                 보조2, False, 오른쪽=True)))

# ── ③ 문의하기 — 단추가 없는 줄이라 «눌러서 여는 것»이다 ──────────────────────
칸 = 칸들["문의"]
중 = (칸["위"] + 칸["아래"]) / 2
기록.append(("문의", 글(칸["좌"] + 안여백, 중 - 픽(26), "선생님께 문의하기", 40, 보조, True)))
기록.append(("문의꼬리", 글(칸["우"] - (안여백 - 픽(40)), 중 - 픽(16), "열기", 28, 보조2, False, 오른쪽=True)))
...
낸것 = os.path.join(여기, "부모소식판_v1" + 꼬리 + ".png")
im.save(낸것)
...
색표 = {"라벨": 보조2, "열기": 보조, "카드몽": 보조2, "제목몽": 보조2,
        "공지0": 보조, "공지1": 보조2, "공지2": 보조2,
        "문의": 보조, "문의꼬리": 보조2,
        "탭0": 보조2, "탭1": 보조2, "탭2": 보조}
색표.update({"날짜%d" % i: 보조2 for i in range(3)})
