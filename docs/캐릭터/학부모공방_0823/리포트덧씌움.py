...
기록.append(("제목", 글(카["좌"] + 픽(8), 카["위"] - 픽(150 if not 전면 else 186), "리포트", 60, 잉크, True)))
if 전면:
...
기록.append(("열기", 글(글좌, 밑줄y - 픽(76), "열어보기 — 8쪽", 34, 보조, False)))
...
for 번, (이름표, 성취) in enumerate(지난):
    ...
    단추x = 칸["좌"] + 폭 * 0.0950
    중 = (칸["위"] + 칸["아래"]) / 2
    기록.append(("지난%d" % 번, 글(단추x + 픽(92), 중 - 픽(26), 이름표, 40, 보조2, False)))
    기록.append(("성취%d" % 번, 글(칸["우"] - (단추x - 칸["좌"]), 중 - 픽(16), 성취, 28, 보조2, False, 오른쪽=True)))
