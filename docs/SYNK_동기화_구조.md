...
| BA (53) | 핵심비전 | `row[21]` = **V**, 비면 `진로비전_()` 합성 | 케어 대화용 한 줄. **V가 비는 것이 기본값이 됐다** — 접수가 크루카드로 넘어오면서 이 칸을 채우던 「사람이 하는 상담」이 앞에 없다. 비면 선택형 답(진로·과정·전공·TOPIK)으로 한 줄을 만들고 `자기신고:` 접두를 붙인다(원장 手記가 언제나 우선) |
