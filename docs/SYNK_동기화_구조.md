...
| EA~ED (131~134) | 졸업후진로 · 희망진학과정 · 목표비자 · 관심전공 | 상담시트 동명 열(관심전공만 `관심전공분야`) — 이름 해석 | **궤적의 「의도」 4칸.** 사람이 읽는 DT124~DX128과 달리 **집계가 읽는다**(「학사를 목표한 N명 중 몇 명이 실제로 학사로 갔나») — 그래서 한 문장으로 접지 않고 4열 그대로 둔다. 결과 쪽은 `outcome_log`(§8). ⚠ TOPIK목표는 여기 없다 — 이미 DU125에 기한과 함께 있다 |

> **열 자리는 눈으로 세지 않는다.** 105·106~108·119·124~128·129·130이 이미 주인 있는 열이라
> 「DT128 다음이니 129」로 세면 두 개를 밟는다. 빈자리 정본 = `tests/safety.test.js`의 열 레지스트리(`reserved`)이고,
> 새 블록은 거기 등록까지 해야 끝난다. 기입 통로는 `profilesBlockWrite_()` 하나 — 점거 가드·헤더 보장·
> writeIfChanged·tail-clear가 그 안에 있다(블록마다 손으로 베끼면 다음 블록에서 한 줄이 빠지고, 빠지는 방향은 언제나 「그냥 덮어쓴다」다).
