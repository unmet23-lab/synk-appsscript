...
> 1. 「DT128 다음이니 129」로 셌는데 **129는 오늘의만남·130은 대화폼URL**이 주인이었다 → `tests/수집.test.js`의 선점 구간 검사가 잡았다.
...
| `r[48]` AW MVP누적 **+** `r[49]` AX 시냅스누적 | **왕관수** | number | 도전 기록+성장 기록 합. ⚠키 이름은 Notion DB 스키마 계약이라 개명하지 않는다(엔진_콘텐츠AI.js 주석) |
...
회귀 = `tests/궤적.test.js`(15건 · 변이 12/12 탐지).
