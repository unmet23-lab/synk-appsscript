export const meta = {
  name: 'engine7-b2a-repair',
  description: '엔진 7종 상향 설계 B2a — 수리 7 ∥ 통합 반박 4 → 통합 수리',
  phases: [
...
// ───────────────────────────── 실행 B2a: 수리 7 ∥ 통합 반박 4 → 통합 수리 (파일 기반) ─────────────────────────────
...
log('B2a — 수리 7 ∥ 통합 반박 4 → 통합 수리')
const repairs = parallel(ENGINES.map((engine) => () => agent(repairPromptF(engine, `${SYN(engine)} (파일을 열어 읽는다)`, `${VER(engine)} (파일을 열어 읽는다 · 배열)`), { label: `수리:${engine}`, phase: '반박', schema: REPAIR_SCHEMA, effort: 'high' })))
...
log(`엔진 수리 ${ok.length}/7 · 통합 수리 ${sysOut && sysOut.sys ? 1 : 0}`)
return { repaired: ok, system: sysOut ? sysOut.sys : null, sysVerdicts: sysOut ? sysOut.sysVerdicts : [], missing: ENGINES.filter((e, i) => !reps[i]) }
