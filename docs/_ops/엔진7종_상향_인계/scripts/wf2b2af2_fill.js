export const meta = {
  name: 'engine7-b2af2-fill',
  description: '엔진 7종 상향 설계 B2a-fill2 — 남은 수리 5(Core·Loom·Vellum·Prism·Temper) + 통합 수리 1 · 반박은 파일로 다 있다',
  phases: [
...
function repairSystemPromptF(newVerdicts) {
  return `${COMMON}
【과제】 일곱 통합 설계를 반박 4벌(전부 파일)로 수리한다.
먼저 ${MAT}/철학.json · ${MAT}/횡단.json · 통합 설계 ${SYS0} · 반박 4벌 ${SYSV}(배열) 를 읽는다.
반박은 전부 그 파일에 있다(4벌 · 철학·차원·실현·확정).
...
log('B2a-fill2 — 남은 수리 5 ∥ 통합 수리 1(반박 4벌은 파일)')
const TODO = ['Core', 'Loom', 'Vellum', 'Prism', 'Temper']
const repairs = parallel(TODO.map((engine) => () => agent(repairPromptF(engine, `${SYN(engine)} (파일을 열어 읽는다)`, `${VER(engine)} (파일을 열어 읽는다 · 배열 4벌)`), { label: `수리:${engine}`, phase: '반박', schema: REPAIR_SCHEMA, effort: 'high' })))
const systemWork = agent(repairSystemPromptF([]), { label: '수리:통합', phase: '반박', schema: SYSTEM_SCHEMA, effort: 'max' }).then((sys) => ({ sys, newVerdicts: [] }))
const [reps, sysOut] = await Promise.all([repairs, systemWork])
const ok = reps.filter(Boolean)
log(`엔진 수리 ${ok.length}/5 · · 통합 수리 ${sysOut && sysOut.sys ? 1 : 0}`)
return { repaired: ok, system: sysOut ? sysOut.sys : null, newVerdicts: sysOut ? sysOut.newVerdicts : [], missing: TODO.filter((e, i) => !reps[i]) }
