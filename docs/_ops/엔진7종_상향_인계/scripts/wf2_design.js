export const meta = {
  name: 'engine7-design-verify-write',
  description: '엔진 7종 상향 설계 — 엔진별 3안 → 심사 2 → 종합 → 7종 통합 → 반박 4갈래 → 수리 → 재반박 → 정본 집필 → 완결 비평 → 수리',
  phases: [
...
const synths = await pipeline(
  ...
  ({ designs, judges }, engine) => agent(synthPrompt(engine, designs, judges), { label: `종합:${engine}`, phase: '심사', schema: DESIGN_SCHEMA, effort: 'high' }),
)
const synthOk = synths.filter(Boolean)
log(`종합 완료 ${synthOk.length}/7`)
if (synthOk.length < 7) log('⚠ 종합 빠짐: ' + ENGINES.filter((e, i) => !synths[i]).join(', '))

log('통합 — 일곱을 한 시스템으로')
let system = await agent(integratePrompt(synthOk), { label: '통합:7종', phase: '통합', schema: SYSTEM_SCHEMA, effort: 'max' })
if (!system) throw new Error('통합 실패')

log('반박 4갈래 × (엔진 7 + 통합 1)')
const engineMaterials = (e) => [`${MAT}/철학.json`, `${MAT}/횡단.json`, `${MAT}/${e}_독해.json`, `${MAT}/${e}_정찰.json`]
const repaired = await pipeline(
  synthOk,
  (design) => parallel(REFUTE_LENSES.map((lens) => () => agent(refutePrompt(`엔진 ${design.engine} 종합 설계`, lens, design, engineMaterials(design.engine)), { label: `반박:${design.engine}:${lens.key.slice(0, 2)}`, phase: '반박', schema: VERDICT_SCHEMA, effort: 'high' }))).then((vs) => ({ design, verdicts: vs.filter(Boolean) })),
  ({ design, verdicts }) => agent(repairPrompt(design.engine, design, verdicts), { label: `수리:${design.engine}`, phase: '반박', schema: REPAIR_SCHEMA, effort: 'high' }),
  (rep) => {
    if (!rep) return null
    return parallel([REFUTE_LENSES[0], REFUTE_LENSES[3]].map((lens) => () => agent(refutePrompt(`엔진 ${rep.engine} 수리 설계(2차)`, lens, rep.design, engineMaterials(rep.engine)), { label: `재반박:${rep.engine}:${lens.key.slice(0, 2)}`, phase: '반박', schema: VERDICT_SCHEMA, effort: 'high' })))
      .then((vs2) => {
        ...
        return agent(repairPrompt(rep.engine, rep.design, v), { label: `재수리:${rep.engine}`, phase: '반박', schema: REPAIR_SCHEMA, effort: 'high' }).then((rep2) => rep2 ? { ...rep2, second_round: v } : { ...rep, second_round: v })
      })
  },
)
...
const sysVerdicts = (await parallel(REFUTE_LENSES.map((lens) => () => agent(refutePrompt('일곱 통합 설계', lens, system, [`${MAT}/철학.json`, `${MAT}/횡단.json`, `${MAT}/세계1등.json`]), { label: `반박:통합:${lens.key.slice(0, 2)}`, phase: '반박', schema: VERDICT_SCHEMA, effort: 'high' })))).filter(Boolean)
const systemRepaired = await agent(repairSystemPrompt(system, sysVerdicts), { label: '수리:통합', phase: '반박', schema: SYSTEM_SCHEMA, effort: 'max' })
if (systemRepaired) system = systemRepaired

log('집필 → 완결 비평 → 수리')
const finalDesigns = repairedOk.map((r) => r.design)
const written = await agent(writerPrompt(system, finalDesigns), { label: '집필:정본', phase: '집필', effort: 'max' })
let critic = await agent(criticPrompt(), { label: '비평:완결', phase: '집필', schema: CRITIC_SCHEMA, effort: 'high' })
let fixLog = null
if (critic && critic.verdict === '수리 필요') {
  fixLog = await agent(fixDocPrompt(critic), { label: '수리:정본', phase: '집필', effort: 'high' })
  const critic2 = await agent(criticPrompt(), { label: '비평:완결2', phase: '집필', schema: CRITIC_SCHEMA, effort: 'high' })
  if (critic2 && critic2.verdict === '수리 필요') {
    const fix2 = await agent(fixDocPrompt(critic2), { label: '수리:정본2', phase: '집필', effort: 'high' })
    fixLog = { first: fixLog, second_critic: critic2, second_fix: fix2 }
  } else {
    fixLog = { first: fixLog, second_critic: critic2 }
  }
}
return { system, repaired: repairedOk, sysVerdicts, written, critic, fixLog, synthMissing: ENGINES.filter((e, i) => !synths[i]) }
