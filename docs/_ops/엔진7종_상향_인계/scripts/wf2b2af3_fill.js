export const meta = {
  name: 'engine7-b2af3-fill',
  description: '엔진 7종 상향 설계 B2a-fill3 — 남은 수리 1(Loom) + 통합 수리 1 · 반박은 파일로 다 있다',
  phases: [
...
log('B2a-fill3 — 남은 수리 1(Loom) ∥ 통합 수리 1(반박 4벌은 파일)')
const TODO = ['Loom']
...
log(`엔진 수리 ${ok.length}/1 · 통합 수리 ${sysOut && sysOut.sys ? 1 : 0}`)
return { repaired: ok, system: sysOut ? sysOut.sys : null, newVerdicts: [], missing: TODO.filter((e, i) => !reps[i]) }
