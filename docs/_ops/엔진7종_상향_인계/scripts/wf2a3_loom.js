export const meta = {
  name: 'engine7-a3-loom',
  description: '엔진 7종 상향 설계 A3단 — Loom 종합 하나(설계_Loom.json·심사_Loom.json 을 읽는다)',
  phases: [
...
const results = await pipeline(
  TODO,
  (t) => t.judgeAngle ? agent(judgePromptF(t.engine, t.judgeAngle), { label: `심사:${t.engine}:보충`, phase: '심사', schema: JUDGE_SCHEMA, effort: 'high' }) : Promise.resolve({ none: true }),
  (extraJudge, t) => agent(synthPromptF(t.engine, extraJudge && extraJudge.none ? null : extraJudge), { label: `종합:${t.engine}`, phase: '심사', schema: DESIGN_SCHEMA, effort: 'high' }).then((s) => ({ engine: t.engine, synth: s, extraJudge })),
)
