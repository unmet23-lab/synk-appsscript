export const meta = {
  name: 'engine7-system-repair',
  description: '엔진 7종 상향 설계 — 통합 수리 하나만(반박 4벌은 파일 · max) · 창 맨 앞에서 혼자 돌린다',
  phases: [
...
log('B2a-통합수리 — 통합 수리 «하나만»(반박 4벌은 파일 · 강도 max · 40~60분)')
const sys = await agent(repairSystemPromptF([]), { label: '수리:통합', phase: '반박', schema: SYSTEM_SCHEMA, effort: 'max' })
log(`통합 수리 ${sys ? 1 : 0}/1`)
return { repaired: [], system: sys || null, newVerdicts: [], missing: sys ? [] : ['통합'] }
