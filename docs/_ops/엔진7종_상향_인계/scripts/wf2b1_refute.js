export const meta = {
  name: 'engine7-b1-refute',
  description: '엔진 7종 상향 설계 B1단 — 통합 1 + 반박 4갈래×7 (종합_*.json 을 읽는다)',
  phases: [
...
log('B1 — 통합 1 + 반박 4갈래 × 7 (동시)')
const jobs = [
  () => agent(integratePromptF(), { label: '통합:7종', phase: '통합', schema: SYSTEM_SCHEMA, effort: 'max' }),
  ...ENGINES.flatMap((engine) => REFUTE_LENSES.map((lens) => () => agent(refutePromptF(`엔진 ${engine} 종합 설계`, lens, `${SYN(engine)} (파일을 열어 읽는다)`, engineMaterials(engine)), { label: `반박:${engine}:${lens.key.slice(0, 2)}`, phase: '반박', schema: VERDICT_SCHEMA, effort: 'high' }))),
]
const out = await parallel(jobs)
const system = out[0]
const verdicts = out.slice(1).filter(Boolean)
log(`통합 ${system ? 1 : 0} · 반박 ${verdicts.length}/28`)
return { system, verdicts, missingRefutes: ENGINES.flatMap((e, i) => REFUTE_LENSES.map((l, j) => out[1 + i * 4 + j] ? null : `${e}:${l.key}`)).filter(Boolean) }
