export const meta = {
  name: 'engine7-c-fill',
  description: '엔진 7종 상향 설계 C단 — 집필 건너뜀(파일 있음) → 완결 비평 → 수리 최대 3회전',
  phases: [
...
⑦ 대가·닫을 것·자가 항목마다 있나.
⑧ 🔴 «실물 대조» — 문서가 「있다」·「없다」·「0건」·「코드 0」이라고 말한 실물(파일·함수·열·계약 칸·시트 헤더)을 저장소에서 실제로 확인한다(${REPO} 와 ${TALK} 를 grep). 틀린 것은 wrong 에 파일:줄 근거와 함께 넣는다. **이 항목이 재반박 단계를 대신한다** — 수리가 크게 고치면서 새로 들어온 «없는 실물»이 여기서 잡힌다. 최소 15곳을 표본으로 열고 고른 근거도 적는다.
...
log('집필 → 완결 비평 → 수리')
const written = '(집필은 한도 전 파일로 섰다 · 런 wf_fe2ebdbf-a06 · 6조각 · 530,087바이트 · 15:34)'
log('집필 건너뜀(파일 있음) → 완결 비평 → 수리')
