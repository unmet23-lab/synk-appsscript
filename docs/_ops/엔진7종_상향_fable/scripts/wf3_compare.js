...
심사 결과 = 파일 ${verdictsRef} (JSON 배열 · 절 17 × 렌즈 3). 숫자 집계는 이미 아래에 있다 — 다시 세지 말고, 근거 인용(evidence_*·why·factual_errors·best_of_each)만 원문에서 읽는다(큰 파일이니 node 로 절·렌즈별로 뽑아 읽는다).
집계(본세션 스크립트가 낸 값 · 이것이 정본): ${JSON.stringify(args.aggregates || {})}
기계 자(에이전트 없이 잰 값): ${JSON.stringify(MEASURED)}
🔴 쓰는 법: 한 번에 쓰지 않는다 — 먼저 §0~§7 제목만 있는 뼈대를 Write 로 저장하고, 절 하나를 완성할 때마다 Edit 로 그 자리에 채운다(중간에 끊겨도 쓴 만큼 남게).
보고서 규격:
