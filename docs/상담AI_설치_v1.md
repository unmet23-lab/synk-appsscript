...
5. **스크립트 속성 2개**:
   ...
   - 첫 저장 뒤 매일 아침 수명 점검이 돌되, Meta의 제한에 맞춰 새 토큰은 최초 확인 후 24시간이 지나기 전에는 갱신하지 않습니다. 이후 확인된 만료 14일 전부터 `refresh_access_token`으로 자동 갱신합니다. 갱신 응답의 만료시각은 `상담AI_IG토큰만료시각`에 저장하며 실패하면 토큰값 없이 주 1회 관리자에게 알립니다.
