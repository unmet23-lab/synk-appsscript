...
4. **Instagram 전용 토큰 발급**: 앱 대시보드 → **이용 사례 → Instagram API → Instagram 로그인이 포함된 API 설정 → 2. 액세스 토큰 생성**으로 이동합니다.
   - **계정 추가**를 눌러 `@synk.mn`에 로그인하고, 이 앱에 `instagram_business_basic`과 `instagram_business_manage_messages`를 허용합니다. 게시·댓글 관리는 이 상담봇 범위가 아니므로 요청하지 않습니다.
   - 추가된 계정 행에서 **토큰 생성**을 눌러 발급합니다. 발급 화면이 짧은 수명 토큰이라고 표시하면, 같은 화면의 장기 토큰 전환 절차를 완료한 뒤 저장합니다. 토큰 문자열은 문서·채팅·코드에 붙이지 않습니다.
   - 발급 직후 Apps Script → **프로젝트 설정 → 스크립트 속성**에 직접 저장하고, Meta 화면이나 클립보드에 남겨두지 않습니다.
5. **스크립트 속성 2개**:
   ...
   - 새 장기 토큰을 저장한 직후 `상담AI_IG토큰발급시각기록`을 한 번 실행합니다. 매일 아침 수명 점검이 돌되 Meta 제한에 맞춰 이 발급시각부터 24시간은 갱신하지 않습니다. 발급시각이 없는 기존 토큰은 만료 직전일 수 있어 즉시 갱신을 시도합니다. 이후 확인된 만료 14일 전부터 `refresh_access_token`으로 자동 갱신합니다. 갱신 응답의 만료시각은 `상담AI_IG토큰만료시각`에 저장하며 실패하면 토큰값 없이 주 1회 관리자에게 알립니다.
6. **확인**: 유호님 개인 인스타 계정으로 SYNK 계정에 DM → 답장이 오고 `상담로그`에 쌓이면 완료. 인계 메일에는 채널이 **「인스타그램」**으로, 상대가 우리를 팔로우하는지도 함께 표시됩니다.
