...
(function () {
  ...
  /* 시차 = **0.110 · 유호 확정 08-28 「두 배로 가자」.**
   *   계보: 생명공방 지면의 0.055(유호 08-27 「과하지는 않아」) → 홈페이지에서 ×1·×2 를 «한 지면에»
   *   나란히 세워 같은 손짓에 동시에 반응시키고 눈으로 고르셨다. 실측(표시 300px · 시선 좌↔우):
   *   머리 +1.41px → **+2.64px** · 치맛단 +0.62 → +1.30 · 깊이의 서명(머리−단) +0.79 → **+1.34**.
   *   ⚠ **빛세기는 안 건드린다(0.42).** 대조판에서 «시선 하나만» 갈라 보여 드렸으므로 유호님이
   *     고르신 것은 시차 하나다 — 안 보여 드린 값을 같이 올리면 픽의 뜻을 넘는다.
   *   ⚠ 이 기본값은 **홈페이지의 값**이다. 릴(`영상/…/살아있는마스코트.tsx` 시차 0.075)과
   *     생명공방 지면(0.055)은 각자 제 값을 쥔다 — 한 수를 세 곳이 나눠 갖지 않는다.
   *   지면이 `data-시차` 를 주면 그쪽이 이긴다. */
  var 시차세기 = parseFloat(방.getAttribute('data-시차') || '') || 0.110;
