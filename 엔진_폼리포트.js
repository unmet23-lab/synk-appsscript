 ...
 *
 * ■ 왜 여기인가 (GRAMMAR_BANK 를 «안» 늘린 이유)
 *   진화 게이트는 「각 단계 문법 12개 중 9개(75%) 도달」이 **문서화된 확정**이다
 *   (`docs/몬스터_진화_임계값_v1.md` §2-2 · 캐릭터 7단계는 철학 정본에 대외 약속으로도 나가 있다).
 *   그래서 뱅크에 4~5급을 «더하면» 그 조항이 깨지고, 3급을 «빼면» 학습 손실이다 — 둘 다 못 한다.
 *   그런데 이 배열에 **3~5급 문형이 96개, 몽골어판(MN_STORY_GRAMMAR)까지 1:1로** 이미 있었다.
 *   급수만 안 붙어 있어서 「4~5급이 몇 개냐」에 아무도 답할 수 없었을 뿐이다.
 *
...
function buildMonthlyCards_() {
  ...
  const rows = stus.map(s => {
    ...
    return [ym, s.id, CARD_WEBFONT + '<div style="' + CARD_FONT + 'background:' + tier[2] + ';border:2px solid #FBB7A3;border-radius:18px;padding:10px;max-width:230px;box-shadow:0 6px 18px rgba(255,107,92,.14);">' +
