...
function createWorkLogForm_(알림기록) {
  ...
  if (exId) {
    try {
      ...
      let 설문지제목 = '';
      try { 설문지제목 = String(exForm.getTitle() || '').trim(); } catch (eT) { 설문지제목 = ''; }
      if (!설문지제목 && !tabOk) {
        const mL = '⚠️ 이 폼이 우리 직장 경험 폼이 맞는지 «확인할 수 없습니다» — 설문지 맨 위 제목이 비어 있고, 응답 탭도 없습니다.\n'
          + '(폼을 만드는 명령이 파일 이름만 세우던 옛 폼입니다. 파일 이름은 누구나 똑같이 지을 수 있어 그것만으로는 근거가 못 됩니다.)\n\n'
          + '→ 아래 편집 링크로 폼을 열고, 맨 위 제목 칸에 「' + WORK_FORM_TITLE + '」 을 그대로 적은 뒤 이 메뉴를 다시 누르세요.\n'
          + '   그 한 줄이면 확인이 서고, 끊긴 응답 탭 연결도 그때 함께 복구됩니다. 아무것도 고치지 않았습니다.\n\n'
