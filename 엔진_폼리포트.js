...
function 직장폼제목보임_(form) {
  return 직장폼제목_(form) || '(읽지 못했거나 비어 있음)';
}
    ...
    try {
      ...
      if (editUrl) {
        ...
        if (직장폼서명_(f0)) {
          ...
          const mX = '⚠️ 탭 「' + WORK_TAB + '」에 연결된 폼이 직장 경험 폼이 아닙니다(제목 「' + 직장폼제목보임_(f0) + '」'
  ...
  if (exId) {
    try {
      ...
      if (직장폼제목_(exForm) !== WORK_FORM_TITLE) {
        const mI = '⚠️ app_state 의 「직장폼ID」가 가리키는 폼의 제목이 「' + 직장폼제목보임_(exForm) + '」입니다 — 직장 경험 폼이 아닙니다.\n'
...
function migrateWorkFormMn() {
  ...
  if (!직장폼서명_(form)) {
    const m = '⚠️ 직장폼ID 가 가리키는 폼이 직장 경험 폼이 아닙니다(제목 「' + 직장폼제목보임_(form) + '」'
  ...
  /* 🔴 «응답이 오는 폼»인지까지 본다(①배포 검수 ed25df351411 채택) — 이번 판의 급소가 여기다.
   *   설문지 제목이 비면 문서 제목(Drive 파일 이름)으로 신원을 보는데, 그 이름은 **남이 제 폼에 붙일 수 있는**
   *   값이다. 문항 두 개까지 흉내 낸 폼을 직장폼ID 에 꽂으면 이 통로가 그 폼의 설명·안내를 덮어쓸 수 있었다.
   ...
   * ⇒ 이걸 세우면 «쓰기»는 문서 제목 되돌아보기가 있어도 예전보다 엄격해진다: 제목이 맞아도 응답 탭에
   *   안 붙은 폼이면 거절이다. 되돌아보기는 «판정»을 넓히지만 이 대조가 «쓰기»를 도로 좁힌다. */
  const shT = ss.getSheetByName(WORK_TAB);
  let tabOk = false;
