...
function createConsultForm() { // [v9.19] 상담지 시트 v18.3 정합 — 문항 제목=시트 헤더명(19개 재정렬), 서술형→📝자유서술→노션. (구 v8.4=v18.1) · [v9.66] v18.4 — Crew Dossier 문항 통일(증분 8문항+관심K분야 '예능') + 재실행 안전 가드
  ...
  para('한국어고충').setHelpText('암기·집중·발음·문법·말하기 울렁증 등 — 수업 설계에 그대로 반영됩니다'); // [v9.66] v18.4 서술형 → 상담시트 자유서술
  ...
  para('SYNK선택이유').setHelpText('SYNK를 선택한 결정적 이유 (예: 비자 관리 · 확실한 성적 향상 · K컬처 클래스)'); // [v9.66] v18.4 서술형 → 상담시트 자유서술 — 전환 결정타 수집
  para('SYNK 기대사항');
  para('이전 학원 경험');
  para('담당크루께 바라는 점');
  para('기타 질문');
...
function migrateConsultV184() {
  ...
  const report = '🔀 상담 정본 v18.4 마이그레이션 결과\n' + out.join('\n') +
    '\n\n다음 확인: checkFormMapping ▶ — 증분 6문항이 열에 매핑되고, 서술형 2문항(한국어고충·SYNK선택이유)이 상담시트 자유서술로 표시되면 정상입니다.';
  Logger.log(report);
 ...
 * 현재 migrateConsentV186은 폼의 동의 문구와 적용 마커를 갱신한다. 기존 동의를 소급 변경하지 않는다.
 * Notion 전송은 2026-08-26 제거됐다. 이 값을 바꾸거나 폼을 갱신해도 되살아나지 않는다. */
...
//   타임스탬프와 함께 상담시트에 보존한다. 열 이름은 과거 호환 키이며 Notion으로 전송하지 않는다. ② 멱등 — 같은 제목이 있으면 스킵(V184와 같은 계급).
...
/* 완료 표식을 «왜» 뗐는지 남기는 앞머리 — 이 값이 서면 아래 안내가 갈래를 가른다(09-08 검수 5회차). */
const WORK_DONE_MISSING_ = '필수누락:';
...
/* 🔴 [09-08 검수 5회차 P1 d8eb1dbcd4b9] 「완료 표식이 없다」의 **갈래가 둘**인데 안내가 하나였다.
 *   ⓐ 문항을 붙이다 실행이 끊긴 폼 — 표식이 애초에 없다. 무엇이 빠졌는지 우리도 모른다.
 *   ⓑ **필수 문항이 빠져서 이 자가 방금 표식을 «뗀» 폼** — 무엇이 빠졌는지 우리가 «안다».
 *   앞 판은 둘 다 「섹션 7개가 다 있는지 보고, 다 있으면 y 를 적으세요」로 보냈다. ⓑ 에서 그대로
 *   따르면 **방금 찾아낸 누락이 그대로인 채** 재실행이 「이미 있습니다」로 끝난다 — 특히
 *   「자료활용동의」가 빠진 폼이 정상으로 지나가면 동의 없는 응답이 조용히 쌓인다.
 * ⇒ 표식을 뗄 때 «까닭»을 함께 적고(`필수누락:…`), 이 안내가 그 까닭을 읽어 갈래를 가른다.
 * 🔑 ⓑ 에서는 «손으로 y 를 적지 말라»고 말한다 — 고치고 다시 누르면 도장은 자동으로 찍힌다. */
function 직장폼미완안내_(완료값, 배포링크) {
  const 누락 = String(완료값 || '').indexOf(WORK_DONE_MISSING_) === 0
    ? String(완료값).slice(WORK_DONE_MISSING_.length) : '';
  if (누락) {
    return '⚠️ 직장 경험 폼에 «필수 문항»이 빠져 있습니다: ' + 누락 + '\n'
      + '그래서 완료 도장을 뗐습니다. 이 폼으로 응답을 받으면 안 됩니다 — 특히 「자료활용동의」가 빠지면 동의 없는 응답이 쌓입니다.\n'
      + '→ 폼을 열어 위 문항을 «묻는 문항»으로 더한 뒤 이 메뉴를 다시 누르세요. 그때 도장은 **저절로** 찍힙니다.\n'
      + '🔴 app_state 의 「직장폼완료」에 y 를 손으로 적지 마세요 — 빠진 문항이 그대로인 채 「이미 있습니다」로 지나갑니다.\n배포 링크: ' + 배포링크;
  }
  return '⚠️ 직장 경험 폼이 «만들다 만 상태»일 수 있습니다 — 폼은 있는데 완료 표식이 없습니다(문항을 붙이는 도중에 실행이 끊긴 자리).\n'
...
/* 🔴 문서 제목은 «신원»이 아니다 — 곁다리 증거일 뿐이다(①배포 검수 6ae0f9351269 채택 · 같은 뿌리를
 *   세 번 짚였다: ed25df351411 · ed44cb1528a8 · 이것). Drive 파일 이름은 **남이 제 폼에 그대로 붙일 수
 *   있는** 값이라, 그것만으로 「우리 폼」이라 판정하면 통로마다 문이 하나씩 열린다.
 * 🔑 그래서 되돌아보기를 «켜야 켜지는» 것으로 바꾼다 — 두 번째 인자는 「우리 응답 탭이 바로 이 폼에
 *   붙어 있음을 부르는 쪽이 이미 확인했다」는 뜻이다. 그 연결은 우리 시트의 setDestination 이 정하므로
 *   남이 못 만든다. 안 켜면 예전처럼 설문지 제목 하나만 본다(엄격).
 * ⇒ 켜는 곳은 셋뿐이다: 응답 탭에서 폼을 «찾아온» 복구 경로(그 폼이 곧 탭의 폼이다) · 탭 대조를 이미
 *   통과한 마이그레이션 둘. 그 밖(기존 ID 채택 · 워치독)은 엄격이다. */
function 직장폼제목_(form, 탭연결확인됨) {
  /* 🔴 «못 읽음»도 거절로 떨어뜨린다(①배포 검수 854bb25d87cb 채택) — 첫 판은 getTitle() 이 try «밖»이라
   *   그 호출이 던지면 판정이 「거절」이 아니라 **예외**로 튀었다. 옛 판도 같았지만, 이 함수의 주석과
   *   회귀는 「못 읽으면 거절」을 약속하고 있었다 — 약속과 물건이 갈린 자리라 물건을 약속에 맞춘다.
   * ⚠ 설문지 제목을 «못 읽었을 때»는 문서 제목으로 넘어가지 «않는다» — 「비었다」와 「모른다」는 다른 말이고,
   *   모르는 채 문서 제목만으로 통과시키는 것이 곧 이 자가 느슨해지는 자리다. */
  let t;
  try { t = String(form.getTitle() || '').trim(); }
  catch (e) { return ''; }
  if (t) return t;
  if (!탭연결확인됨) return '';   // 🔴 곁다리 증거는 «부를 때 켜야» 쓴다 — 안 켜면 여기서 끝(거절)
  try { return String(DriveApp.getFileById(form.getId()).getName() || '').trim(); }
  catch (e) { return ''; }
}
/* 사람에게 보일 제목 — 빈칸이면 낫표 사이가 비어 「무슨 일인지 모를 경고」가 된다(①배포 검수 eb8c0edb35dc 채택).
 * 유호님이 09-03 에 보신 그 「제목 「」」가 정확히 이 자리다. 이제 «비었다»와 «못 읽었다»가 한 낱말로 뭉치지 않는다.
 * ⚠ 판정에는 안 쓴다 — 판정하는 자는 직장폼제목_ 하나다(한 판정엔 자 하나). 이건 오직 사람에게 보이는 줄용이다. */
function 직장폼제목보임_(form, 탭연결확인됨) {
  return 직장폼제목_(form, 탭연결확인됨) || '(읽지 못했거나 비어 있음)';
}
...
/* 🔑 세 갈래를 «갈라서» 낸다(①배포 검수 947f3f044f72 채택) — 첫 판은 「이미 서 있다」와 「쓰다 실패」가
 *   둘 다 빈 문자열이라, 쓰기가 조용히 안 먹어도 보고에 아무 말이 없었다. 그러면 빈칸이 그대로 남은 채
 *   「고쳤다」로 읽힌다(0건이 성공 얼굴을 쓰는 자리). 그래서 쓴 «뒤 다시 읽어» 확인하고, 셋을 갈라 말한다.
 * 반환값은 보고에 그대로 붙는 «완성된 토막»이다 — 부르는 두 곳이 문구를 따로 쥐면 갈린다. */
function 직장폼제목치유_(form) {
  /* 읽기도 감싼다(같은 검수 854bb25d87cb) — 여기서 던지면 「고치러 온 통로」가 통째로 죽는다. */
  let 앞;
  try { 앞 = String(form.getTitle() || '').trim(); }
  catch (e) { return '\n\n⚠️ 설문지 제목을 읽지 못해(' + e + ') 제목 치유를 건너뛰었습니다 — 비어 있었다면 그대로 남습니다.'; }
  if (앞) return '';   // 이미 서 있다 — 할 일 없음(조용해도 되는 유일한 갈래)
  const 까닭 = '\n   → 이 폼은 «설문지 제목»이 빈 채로 태어났습니다(폼을 만드는 명령이 문서 제목만 세웁니다).'
    + ' 그 빈칸 때문에 서명 검사가 이 폼을 «남의 폼»으로 찍고 있었습니다. 응답자 화면에 보이는 글자는 그대로입니다.';
  let 뒤;
  try { form.setTitle(WORK_FORM_TITLE); 뒤 = String(form.getTitle() || '').trim(); }
  catch (e) {
    return '\n\n⚠️ 설문지 제목을 못 채웠습니다(' + e + ') — 빈칸이 그대로라 다음에도 문서 제목으로 우회합니다.' + 까닭;
  }
  if (뒤 === WORK_FORM_TITLE) return '\n\n🔧 함께 고친 것: 설문지 제목(빈칸 → 「' + WORK_FORM_TITLE + '」)' + 까닭;
  return '\n\n⚠️ 설문지 제목을 채우려 했지만 쓴 뒤 다시 읽으니 「' + 뒤 + '」입니다 — 빈칸이 남았을 수 있습니다.' + 까닭;
}
...
function createWorkLogForm_(알림기록) {
  ...
  if (!exId) { // ID만 유실된 경우(시트 수기 편집·app_state 정리) 응답 탭의 연결 폼에서 복구 — 없으면 중복 폼이 생기고 회수가 두 곳으로 갈린다
    try {
      ...
      if (editUrl) {
        ...
        /* 🔑 곁다리 증거(문서 제목)를 «켠다» — f0 는 탭이 준 폼이다(shR.getFormUrl() 로 열었다).
         *   「우리 응답 탭이 이 폼에 붙어 있다」가 이 자리에서는 구성상 참이라, 설문지 제목이 빈 옛 폼도
         *   문서 제목으로 알아본다. 켜는 조건이 곧 안전이다 — 다른 데서는 안 켠다. */
        if (직장폼서명_(f0, true)) {
          exId = f0.getId();
          ...
          if (빠진필수.length === 0) {
            ...
            /* 🔴 [5회차 P1 d8eb1dbcd4b9] 빈 값이 아니라 «까닭»을 적는다 — 그래야 아래 「만들다 만 상태」
             *   안내가 갈래를 갈라 「무엇이 빠졌는지」를 이름으로 말하고, 손으로 y 를 적지 말라고 할 수 있다.
             *   빈 값으로 두면 그 안내가 「섹션 7개를 보고 y 를 적으세요」로 나가 누락이 그대로 지나간다. */
            setState(st, '직장폼완료', WORK_DONE_MISSING_ + 빠진필수.join('·'));
          ...
          const mX = '⚠️ 탭 「' + WORK_TAB + '」에 연결된 폼이 직장 경험 폼이 아닙니다(제목 「' + 직장폼제목보임_(f0, true) + '」'
            + (직장폼제목_(f0, true) === WORK_FORM_TITLE ? ' · 제목은 같지만 필수 문항 「시킨 일 그대로」·「예정에 없던 일이 생긴 적」이 없습니다' : '') + ').\n'
  ...
  if (exId) {
    try {
      ...
      /* 탭 연결은 «먼저 읽어만» 둔다 — 읽기라 위 ①②③ 거절 순서를 안 흔든다. 이 값이 곧 「문서 제목을
       * 곁다리 증거로 써도 되는가」의 열쇠다(①배포 검수 6ae0f9351269) — 탭이 이 폼에 안 붙어 있으면
       * 설문지 제목 하나만 본다. 그래서 «탭도 없는데 이름만 같은 폼»을 setDestination 으로 채택하던
       * 아래 경로가 닫힌다 — 그 자리가 이 판이 새로 연 마지막 문이었다. */
      /* ⚠ 탭의 연결 폼 URL 을 «한 번만» 읽어 두고 그 값을 계속 쓴다(①배포 검수 7887232d1d77·656171cbbd8c 채택).
       *   tabOk 는 참/거짓 하나라, 아래 안내가 그걸 안 보고 「탭이 있나」만 보면 사정 넷이 둘로 뭉친다 —
       *   실제로 «어느 폼에도 안 붙은» 시트를 「다른 폼에 연결돼 있습니다」로 말했다(따라갈 폼이 없는 처방).
       *   갈래를 여는 자리는 바로 위 제목 쪽과 같은 관례다(제목읽기실패) — «못 읽었다»와 «비었다»를 안 뭉친다.
       * 🔑 판정은 한 글자도 안 바뀐다 — tabOk 는 전과 똑같이 「이 폼의 ID 가 그 URL 안에 있나」 하나다.
       *   갈리는 것은 «말»뿐이다. 그리고 두 번 읽지 않는다 — 두 번 읽으면 두 값이 갈릴 수 있다. */
      const shT = ss.getSheetByName(WORK_TAB);
      let 탭연결URL = '';
      let 탭연결못읽음 = false;
      if (shT) { try { 탭연결URL = String(shT.getFormUrl() || ''); } catch (eU) { 탭연결못읽음 = true; } }
      const tabOk = !!탭연결URL && 탭연결URL.indexOf(exForm.getId()) !== -1;
      /* 🔴 증거가 약하면 «기계가 정하지 않는다» — 사람에게 한 줄을 부탁한다(①배포 검수 08515c3f7f53·61e7fc738d02 채택).
       *   여기 오는 폼은 셋 다 참일 때가 문제였다: 설문지 제목이 비었고 · 응답 탭도 없고 · 그래서 남은
       *   증거가 «문서 제목(파일 이름)»뿐이다. 그 이름은 누구나 같게 지을 수 있어 근거가 못 된다.
       *   앞 회차에 필수 문항 다섯으로 대신 재봤지만, 그것도 «제목 문자열»일 뿐이라 섹션 머리·선택 문항으로
       *   흉내 낼 수 있었다(검수 61e7fc738d02). 형상을 더 파고들수록 흉내의 문턱만 조금씩 오른다.
       * 🔑 그래서 방향을 바꾼다 — 약한 증거를 «더 짜내는» 대신 **사람이 1초에 끝낼 수 있는 확인**을 부탁한다.
       *   유호님이 폼을 열어 맨 위 제목 칸에 그 이름을 적으면, 그 순간 설문지 제목이 서고 이 함수는
       *   «엄격한 길»로 그냥 지난다 — 약한 증거가 아예 필요 없어진다. 곁다리도, 형상 대체도 걷었다.
       * ⇒ 되살릴 길은 0 이 아니다(8812afdbcd6c 가 걱정한 자리) — 한 줄 적고 다시 누르면 라우팅까지 함께 선다. */
      /* ⚠ «못 읽었다»와 «비었다»를 갈라 둔다(①배포 검수 기능체크 P2) — 둘을 뭉치면 권한이 끊긴 자리에
       *   「제목을 적으세요」라는 따를 수 없는 처방이 나간다. 판정은 둘 다 거절로 같지만 «할 일»이 다르다. */
      let 설문지제목 = '';
      let 제목읽기실패 = false;
      try { 설문지제목 = String(exForm.getTitle() || '').trim(); } catch (eT) { 제목읽기실패 = true; }
      if (제목읽기실패) {
        const mE = '⚠️ 직장폼ID 가 가리키는 폼의 제목을 읽지 못했습니다 — 폼이 지워졌거나 권한이 끊긴 자리입니다.\n'
          + '아무것도 고치지 않았습니다. 폼이 살아 있는지, 이 시트 계정이 그 폼을 열 수 있는지 먼저 보세요.\n'
          + '폼이 정말 없어졌으면 app_state 의 「직장폼ID」 행을 지운 뒤 다시 실행하세요 — 새로 만들거나 응답 탭에서 복구합니다.';
        Logger.log(mE);
        return mE;
      }
      if (!설문지제목 && !tabOk) {
        /* 🔑 탭의 사정은 «넷»이다 — 없다 · 어느 폼에도 안 붙었다 · 다른 폼에 붙었다 · 연결을 못 읽었다.
         *   뭉쳐 말하면 유호님이 엉뚱한 곳을 고치신다. 셋째만 「다른 폼」이고, 둘째를 그렇게 말하면
         *   찾아갈 폼이 없는 처방이 나간다(이름만 같은 보통 시트를 만든 자리가 정확히 그것이다). */
        let 탭말;
        if (!shT) 탭말 = '응답 탭 「' + WORK_TAB + '」도 없습니다';
        else if (탭연결못읽음) 탭말 = '응답 탭 「' + WORK_TAB + '」이 어느 폼에 연결돼 있는지 읽지 못했습니다';
        else if (!탭연결URL) 탭말 = '응답 탭 「' + WORK_TAB + '」은 있지만 어느 폼에도 연결돼 있지 않습니다(이름만 같은 보통 시트입니다)';
        else 탭말 = '응답 탭 「' + WORK_TAB + '」은 있지만 «다른 폼»에 연결돼 있습니다';
        const mL = '⚠️ 이 폼이 우리 직장 경험 폼이 맞는지 «확인할 수 없습니다» — 설문지 맨 위 제목이 비어 있고, ' + 탭말 + '.\n'
          + '(폼을 만드는 명령이 파일 이름만 세우던 옛 폼입니다. 파일 이름은 누구나 똑같이 지을 수 있어 그것만으로는 근거가 못 됩니다.)\n\n'
          + '→ 아래 편집 링크로 폼을 열어 **이 폼이 정말 직장 경험 폼인지 눈으로 확인**하시고, 맞으면 맨 위 제목 칸에\n'
          + '   「' + WORK_FORM_TITLE + '」 을 그대로 적은 뒤 이 메뉴를 다시 누르세요.\n'
          + '   아니면 app_state 의 「직장폼ID」 행을 지우고 다시 실행하세요. 지금은 아무것도 고치지 않았습니다.\n\n'
          + '편집용: ' + exForm.getEditUrl();
        Logger.log(mL);
        return mL;
      }
      ...
      /* 🔴 «완료라고 적혀 있으면 완료된 폼의 모양이어야 한다»(①배포 검수 ed44cb1528a8 채택) —
       *   위 제목 대조는 설문지 제목이 비면 문서 제목(Drive 파일 이름)으로 떨어지는데, 그 이름은
       *   **남이 제 폼에 붙일 수 있는** 값이다. 완료 표식까지 남아 있으면 문항 없는 남의 폼이 «정상»으로
       *   지나가고, 그 뒤의 라우팅 복구·URL 저장이 그 폼을 정본으로 굳힌다.
      ...
      if (!tabOk) {
        /* 여기 오는 탭은 «있는» 탭이다(위에서 없는 경우가 이미 돌아갔다) — 그래서 사정이 셋이고 처방도 셋이다.
         *   ⚠ 탭의 「양식」 메뉴는 «폼에 붙은» 탭에만 있다 — 안 붙은 시트에 그걸 시키면 못 따르는 처방이 된다.
         *   같은 결함을 위 안내에서 고치며 이 자리도 같이 갈랐다(①배포 검수 7887232d1d77 계보). */
        let mM;
        if (탭연결못읽음) {
          mM = '⚠️ 탭 「' + WORK_TAB + '」이 어느 폼에 연결돼 있는지 읽지 못했습니다 — 제출이 어디에 쌓이는지 확인할 수 없습니다.\n'
            + '일시 오류이거나 이 시트 계정의 권한이 모자란 자리입니다. 잠시 뒤 다시 실행해 보시고, 그래도 같으면 그 탭의 「양식」 메뉴로 어느 폼인지 직접 보세요. 지금은 아무것도 고치지 않았습니다.';
        } else if (!탭연결URL) {
          mM = '⚠️ 탭 「' + WORK_TAB + '」은 어느 폼에도 연결돼 있지 않습니다 — 이름만 같은 보통 시트라 제출이 이 탭에 안 쌓입니다.\n'
            + '그 시트의 이름을 바꾸신 뒤(예: ' + WORK_TAB + '_옛것) 다시 실행하세요 — 그때 이 폼의 제출이 새 응답 탭으로 오도록 다시 이어 줍니다. 이름을 바꾼 시트의 내용은 그대로 남습니다.';
        } else {
          mM = '⚠️ 탭 「' + WORK_TAB + '」이 직장폼ID 와 «다른 폼»에 연결돼 있습니다 — 제출이 쌓이는 곳과 워치독이 세는 곳이 갈라져 있습니다.\n'
...
function migrateWorkFormMn() {
  ...
  /* 🔴 «응답이 오는 폼»인지를 **맨 먼저** 본다(①배포 검수 ed25df351411 · 6ae0f9351269) — 이 통로의 뿌리 자다.
   *   응답 탭이 «어느 폼에» 붙어 있는지는 우리 시트의 setDestination 이 정한다 — 남이 못 고치는 값이다.
   *   반대로 문서 제목(Drive 파일 이름)은 남이 제 폼에 그대로 붙일 수 있어 혼자로는 신원이 못 된다.
   * 🔑 그래서 «이 대조를 먼저 통과»한 뒤에야 아래 서명이 문서 제목을 곁다리 증거로 쓴다(둘째 인자 true).
   *   순서가 곧 안전이다 — 뒤에 두면 서명이 먼저 문서 제목만으로 남의 폼을 「우리 폼」이라 부른다.
  ...
  if (!tabOk) {
    const mT = (shT
      ? '⚠️ 탭 「' + WORK_TAB + '」이 직장폼ID 와 «다른 폼»에 연결돼 있습니다 — 지금 고치면 응답이 안 오는 폼을 고칩니다.'
      : '⚠️ 응답 탭 「' + WORK_TAB + '」이 없습니다 — 폼과 시트 연결이 끊긴 상태입니다.')
  ...
  if (!직장폼서명_(form, true)) {
    const m = '⚠️ 직장폼ID 가 가리키는 폼이 직장 경험 폼이 아닙니다(제목 「' + 직장폼제목보임_(form, true) + '」'
      + (직장폼제목_(form, true) === WORK_FORM_TITLE ? ' · 제목은 같지만 필수 문항 「시킨 일 그대로」·「예정에 없던 일이 생긴 적」이 없습니다' : '') + ').\n'
   ...
   * 🔑 즉시 로그에 남긴다(①배포 검수 P2) — 아래 쓰기가 던지면 요약은 못 나가는데 «제목은 이미 바뀐» 상태다.
   *   그 사실이 실행 기록에 없으면 다음 사람이 「제목이 왜 바뀌었지」를 영영 못 잇는다. */
...
function migrateWorkFormOutcome_(알림기록, 치유기록) {
  ...
  /* 🔴 «응답이 오는 폼»인지를 **맨 먼저** 본다(codex P1 c1bc92a6a834 · ①배포 검수 6ae0f9351269).
   *   ①직장폼ID 가 서명은 같지만 응답 탭에 «안 붙은» 폼(복사본·옛 폼)을 가리키면 새 문항은 응답이 안
   *   들어오는 폼에 붙고 엔진이 읽는 탭에는 결과 칸이 영영 안 생긴다 — 그런데 보고는 성공이다.
   *   ②그리고 이 대조가 곧 «문서 제목을 곁다리 증거로 써도 되는가»의 열쇠다 — 응답 탭이 어느 폼에
   *   붙어 있는지는 우리 시트가 정하지만, 문서 제목(Drive 파일 이름)은 남이 제 폼에 그대로 붙일 수 있다.
   *   그래서 이 대조를 통과한 뒤에야 아래 서명이 그 곁다리를 쓴다(둘째 인자 true). 순서가 곧 안전이다. */
  const shT = ss.getSheetByName(WORK_TAB);
  let tabOk = false;
  if (shT) { try { tabOk = String(shT.getFormUrl() || '').indexOf(form.getId()) !== -1; } catch (eU) { tabOk = false; } }
  if (!tabOk) {
