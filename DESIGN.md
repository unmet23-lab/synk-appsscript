...
- 정적 4줄(인쇄물·지도·발표물 — 스킬 판단이 아니라 **고정값**이라 킷이 진다): 제목 `text-wrap: balance` · 본문 `text-wrap: pretty` · 루트 `-webkit-font-smoothing: antialiased` + `-moz-osx-font-smoothing: grayscale` · 숫자 `font-variant-numeric: tabular-nums`. ⏏**빼려면 이 줄만 지운다** — 훅이 이 앵커만 **선택**으로 읽어 없으면 그냥 넘어간다(2026-08-06 유호님 승인 · 근거 = 기기별 지도에서 `mifb` 스킬이 실제로 잡아준 2건).
