...
(async () => {
  ...
  let 뜨는중 = false, 센것 = 0, 다시쓴것 = 0, 거른것 = 0, 실패 = 0, 마지막장 = null;
  ...
  setInterval(() => {
    ...
    if (ff.stdin.writableLength > 마지막장.length * 8) { 거른것++; return; }
    ff.stdin.write(마지막장);
    다시쓴것++;
  }, Math.round(1000 / 뜨기));
  ...
  setInterval(() => {
    ...
    말(`층 ${센것}장 떴다(초당 ${뜬속}) · 내보낸 것 ${다시쓴것}(초당 ${낸속}/${뜨기}) · 실패 ${실패}`
      + ` · 거른 박자 ${거른것} · 실시간대비 ${진척.speed} · 버린장 ${진척.drop_frames} · 겹친장 ${진척.dup_frames}`);
  }, 60000);
