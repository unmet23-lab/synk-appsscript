...
(async () => {
  ...
  const 장부 = 장부읽기();
  /* 🔴 [09-08 재검수 P1] **한도는 «새로 만드는 것»만 막는다 — 되살리기는 안 막는다.**
   *   여기서 막으면, 3/4 에서 하나를 만든 뒤 전환이 한 번 실패한 순간 그 방송은 «이미 4/4»가 되어
   *   날짜가 바뀔 때까지 못 살아난다(만들어 놓고 못 켜는 자리). 같은 모의에서 앞 판은 다시 써서
   *   live 까지 갔는데 이 판은 ready 에 멈췄다 — 내가 만든 회귀다.
   *   ⇒ 검사를 «새로 여는 갈래» 바로 앞으로 내린다. 아래 그 자리에 있다. */
  ...
  let 자리 = 안끝난[0];
  let 새로만듦 = false;
  if (자리) 말(`트는 자리 0개 · 아직 안 튼 자리 ${자리.id}(${자리.status.lifeCycleStatus}) 를 올린다 — 되살리기는 한도를 안 본다`);
  else {
    /* 🔑 한도 검사가 여기 있는 까닭 = 위 주석. «만드는 것»만 막고 «되살리는 것»은 통과시킨다. */
    if (장부.연것 >= 한도) { 말(`🔴 트는 자리 0개 · 되살릴 자리도 0개 · 오늘 이미 ${장부.연것}번 열었다(한도 ${한도}) — 더 안 연다. 사람이 볼 자리다`); return; }
    말('🔴 트는 자리 0개 — 새로 연다');
  ...
  /* 🔴 [09-08 재검수 P1] **한 자리는 «한 번»만 센다 — 그 자리의 id 로 가른다.**
   *   앞 판은 다시 쓸 때마다 또 셌다. 그러면 같은 방송 하나를 세 번 되살리는 동안 한도가
   *   3 만큼 줄어, 정작 «새로 열어야 할 때» 못 연다. 셈의 뜻은 「오늘 몇 개를 만들었나」이지
   *   「몇 번 손댔나」가 아니다. */
  const 이미센것 = (장부.자리들 || []).some((x) => x && x.id === 자리.id);
  if (이미센것) {
    const 그줄 = 장부.자리들.filter((x) => x && x.id === 자리.id).pop();
    if (그줄) { 그줄.단계 = 지금; 그줄.마지막손댐 = 이제(); }
  } else {
    장부.연것 += 1;
