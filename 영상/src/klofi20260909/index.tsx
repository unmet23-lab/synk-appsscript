...
type Plan={title:string;duration:number;faces:string[];files:Record<string,string>};
...
const Film:React.FC<{plan:Plan}>=({plan})=>{
 ...
 return <AbsoluteFill style={{background:theme.ink,fontFamily:theme.fonts.body,color:theme.paper}}>
  ...
  <div style={{position:'absolute',left:652.8,bottom:94.5,width:614.4,height:614.4}}>
   <div style={{position:'absolute',left:'7%',width:'86%',height:'14%',bottom:'15.6%',borderRadius:'50%',transform:'translateY(50%)',background:`radial-gradient(ellipse,${theme.shadow(.36)} 0%,transparent 70%)`}}/>
...
registerRoot(()=> <Composition id="klofi-full-track" component={Film} width={1920} height={1080} fps={30} durationInFrames={1848} defaultProps={{plan:{title:'',duration:61.6,faces:[],files:{}}}}
