...
import {Artifact, Entrance, Headline, Progress} from './Frame';
...
export const ContentScene: React.FC<{scene: Scene; item: ContentItem; index: number}> = ({scene, item, index}) => {
  ...
  return <AbsoluteFill ref={ref} style={{fontFamily: theme.fonts.body, color: theme.ink,
    ...
    <div style={{position: 'absolute', top: 270, left: theme.safe.left, width: theme.safe.width}}>
      <Entrance><div data-critical="eyebrow" style={{fontSize: 27, lineHeight: 1.35, fontWeight: 600, color: accent, marginBottom: 34}}>{scene.eyebrow || label}</div></Entrance>
      <Entrance delay={0.1}><Headline text={scene.title} size={headingSize} /></Entrance>
    </div>

    {isLab ? <>
      <div style={{position: 'absolute', top: scene.kr ? 520 : 570, left: theme.safe.left, width: theme.safe.width}}>
      ...
      {scene.asset && <Entrance delay={0.6} style={{position: 'absolute', top: labOptions.length ? 1100 : 990, left: labOptions.length ? 200 : 140}}><Artifact asset={scene.asset} width={labOptions.length ? 640 : 760} height={labOptions.length ? 310 : 410} duration={duration} /></Entrance>}
      {labOptions.length > 0 && <div style={{position: 'absolute', top: 750, left: theme.safe.left, width: theme.safe.width}}>{labOptions.map((line, i) => <Entrance key={line} delay={0.45 + i * 0.13}><div data-critical={`line-${i}`} style={{fontSize: 46, lineHeight: 1.35, marginBottom: 23}}>{line}</div></Entrance>)}</div>}
    </> : isDocument ? <>
      <Entrance delay={0.25} style={{position: 'absolute', top: 635, left: theme.safe.left}}><Artifact asset={scene.asset!} duration={duration} document width={820} height={514} /></Entrance>
      <div style={{position: 'absolute', top: 1210, left: theme.safe.left, width: theme.safe.width}}>
        {scene.body && <Entrance delay={0.5}><div data-critical="body" style={{fontSize: 39, lineHeight: 1.43, whiteSpace: 'pre-line', wordBreak: 'keep-all'}}>{scene.body}</div></Entrance>}
      ...
      <div style={{position: 'absolute', top: 670, left: theme.safe.left, width: theme.safe.width}}>
      ...
      {scene.asset && <Entrance delay={0.7} style={{position: 'absolute', top: lines.length > 2 ? 1160 : 1000, left: lines.length > 2 ? 530 : 300}}><Artifact asset={scene.asset} width={lines.length > 2 ? 360 : 560} height={lines.length > 2 ? 260 : 405} duration={duration} /></Entrance>}
    </>}
