...
/* 공방 펠트 자산을 «이름으로» 꺼낸다. 없는 이름을 주면 그 자리에서 던진다 — 조용한 빈 화면이 안 난다.
   ⚠ 굽기 전에 `node 영상/자산모으기.js` 를 먼저 돌린다(목록이 파생물이라서). */
import { 요소 } from "../킷/공방";
...
/**
 * 훅 바탕 — 첫 2초에 «자리»(장소)를 깐다. 대본이 `**자리**: 교실` 로 이름을 주면 그 사진이 선다.
 *
 * 🔴 왜 팠나(09-06 실측): 여덟 편의 첫 화면이 **글자만 다르고 나머지가 한 장이었다** — 로고 자리·
 *   몽글 자리·바탕이 같아서, 틱톡에서 둘째 편부터 「본 것」으로 읽힌다. 로고를 훅 뒤로 미루자
 *   위 삼분의 일이 그냥 비기도 했다. 그 두 구멍을 한 층이 메운다.
 *
 * 🚫 **움직임을 넣지 않는다.** 09-06 에 훅 몸짓이 「멀미」로 기각됐다(두 번째 같은 교정). 이 층은
 *   처음부터 끝까지 **가만히 있는 사진**이고, 하는 일은 훅이 끝날 때 종이로 녹아 사라지는 것뿐이다.
 *
 * 🔑 종이 너울을 위에 덮는 까닭 — 훅 글자는 코랄(Coral 3)이고 자리 사진도 펠트라 색이 서로 가깝다.
 *   맨 사진 위에 그대로 얹으면 첫 2초의 글자가 안 읽히고, 그러면 훅이 통째로 죽는다.
 *   ⇒ ① 화면 전체에 옅은 종이 너울 ② 글자가 앉는 위쪽에만 짙은 너울. 색은 킷의 `지면.바탕` 하나뿐이고
 *      «얼마나 덮나»만 여기서 정한다(생 hex 를 적으면 발행검사 ③ 이 잡는다).
 */
const 훅바탕: React.FC<{ 자리: string }> = ({ 자리 }) => {
  const frame = useCurrentFrame();
  const { fps, durationInFrames } = useVideoConfig();
  /* 훅 자막과 «같은 결»로 나간다 — 글자는 사라지는데 사진만 남으면 컷처럼 끊긴다. */
  const 나감f = Math.round(fps * 연출.나감초);
  const 나감 = interpolate(frame, [durationInFrames - 나감f, durationInFrames - 2], [1, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  ...
  return (
    <AbsoluteFill style={{ opacity: 나감 }}>
      {/* 🔑 «아래쪽»을 보여 주는 잘라내기 — 자리 사진은 정사각이고 릴은 9:16 이라 한가운데로 자르면
          윗벽만 남아 화면 위 절반이 통째로 빈다(로고를 뒤로 미룬 09-06 뒤 그 자리가 특히 눈에 띈다).
          65% 로 내리면 책상·창·게시판이 몽글 뒤로 올라와 «방 안»으로 읽힌다. */}
      <Img
        src={staticFile(요소(자리))}
        style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "50% 65%" }}
      />
      {/* ① 전체 너울 — 사진이 «배경»으로 물러나 앉는 정도 */}
      <AbsoluteFill style={{ backgroundColor: 지면.바탕, opacity: 0.38 }} />
      {/* ② 위쪽 너울 — 훅 글자가 앉는 띠. 자막 앵커가 y470 이라 그 아래로 풀린다 */}
      <AbsoluteFill
        style={{
          background: `linear-gradient(180deg, ${지면.바탕} 0%, ${지면.바탕} 14%, transparent 54%)`,
          opacity: 0.66,
        }}
      />
    </AbsoluteFill>
...
export const 리드크루클립: React.FC<{ 클립: 클립 }> = ({ 클립 }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: 지면.바탕 }}>
      ...
      {/* ── 훅 (0~3초) ─────────────────────────────────────────────── */}
      {/* 자리 사진은 «자막보다 먼저» 선다 — 뒤에 서면 글자를 덮는다. 대본이 자리를 안 주면 이 층이 없다
          (45화 중 자리를 적은 화만 탄다 · 옛 판과 픽셀이 같다). */}
      {클립.자리 ? (
