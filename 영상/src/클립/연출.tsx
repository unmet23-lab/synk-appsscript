...
export const 연출 = {
  ...
  /* 🔴 인스타/틱톡은 화면 «하단»을 자기 UI(계정·캡션·버튼)로 덮는다. 마스코트를 바닥에 붙이면
     그 UI 에 다리가 잘린다. 틱톡·쇼츠 하한은 250 이다.
     ⚠ 발행 때는 인스타 «실물 앱»으로 다시 잰다 — 서드파티 출처가 420/450/320 으로 서로 다르다.

     🔄 **09-07: 300 → 200 으로 내렸다**(유호 지적 「글씨가 몽글이 머리에 좀 가리거든?
        몽글이 배치를 약간 아래로 내려야할것같은데?」).
     🔑 내릴 수 있었던 까닭 = **이 값은 «그림 상자»의 아래지 «몸»의 아래가 아니다.** 마스코트 자산은
        정사각 칸 안에 몸이 떠 있어서 아래쪽에 빈 곳이 163px 있다(09-07 실측: 상자 바닥 1620 인데
        몸 발끝은 1457). 그래서 300 일 때 몸 발밑 여백이 실제로는 463px 였고, 하한 250 위로 213px 가
        남아 있었다. 200 으로 내려도 몸 발밑은 363px 라 하한을 넉넉히 넘는다.
        ⚠ 이 163px 는 «지금 자산»의 값이다 — 마스코트를 다시 구우면 다시 잰다(표지 카드에서도 같은
          빈 곳이 글자와 겹치게 만든 적이 있다).
     🔑 그 결과 자막 아래끝과 머리 꼭대기 사이가 이만큼 벌어진다(09-07 실측 → 예상):
        릴 훅 226 → 326px · 카운트다운 훅 230 → 330px · 카운트다운 마무리 155 → 255px.
        가장 좁은 자리는 «훅»이 아니라 **카운트다운 마무리**다(문장이 길어 세 줄이 된다). */
  안전여백아래: 200,
