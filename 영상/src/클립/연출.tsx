...
export const 배경음악 = {
   ...
   *   · 받은 곡  — `소리/받은BGM/<파일명>` (유호님이 `docs/홍보물/BGM/` 에 넣은 것)
   * 유호 판정 08-26: 「배경음은 무료 배경음 사용하는게 더 좋아보이는데?」 ⇒ 받은 곡이 서면 여기를 갈아끼운다.
   */
  /* 🔑 기본값 = **몽글의 곡**이다. 리드크루 클립 10편이 전부 몽글이라, 여기가 곧 그 10편의 곡이다.
     유호 픽 08-26: 몽글 ⑤ Toddler · 까몽 ① Joyful(카운트다운은 가이드 데이터가 덮어쓴다).
     ⚠ 08-26 에 한 번 어긋났다 — 카운트다운만 가이드별로 갈아끼우고 이 기본값을 안 바꿔서
        몽글 릴 10편이 「지은 곡」을 계속 깔고 있었다. 통로가 둘이면 한쪽이 조용히 뒤처진다. */
  파일: "소리/받은BGM/05_Toddler-Kids_alex-morgan.mp3",
  볼륨: 0.28,
  /**
   * 곡의 «어디부터» 쓸 것인가(초).
   *
   * 🔑 받아 온 곡은 앞 8~16초가 인트로(한 악기만 나오는 도입)인 경우가 많다. 릴은 30초라
   *   그 인트로를 그대로 쓰면 절반이 밋밋해진다 — 곡이 «가장 꽉 찬» 지점을 골라 시작한다.
   *   지은 곡은 첫 마디부터 다 나오므로 0 이다.
   */
  시작초: 21,
} as const;
