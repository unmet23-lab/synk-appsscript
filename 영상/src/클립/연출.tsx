 ...
 *   ⇒ `영상/BGM만들기.js` 로 **짓는다.** 같은 잣대 실측: 변동계수 0.64 · 온셋 3.0개/초.
 *
 * 🔑 30.00초를 통째로 낸다 — 릴 길이와 같아 **이음매가 아예 없다.**
 *   첫 편이 앓았던 「루프 볼륨 커브가 되감겨 끝 페이드가 영원히 안 온다」가 원리상 안 생긴다.
 *
 * 🔑 볼륨 0.46 — 0.34 로 먼저 잡았다가 **실측으로 올렸다**: 그 값에서 완성본이
 *   count-01 −20.4 / count-02 −23.1 LUFS 로 얇았다(까몽판은 옹알이가 절반이라 더 조용하다).
 *   0.46 에서 −19.3 / −21.2 다. 여기서 더 올리면 옹알이(0.8)를 덮기 시작한다.
 *
 * ⛔ **완성본 라우드니스는 아직 «얇다»** — 인스타는 큰 소리를 −14 LUFS 쯤으로 «내리기만» 하고
 *   조용한 것을 올려 주지 않는다. 지금 −19~−21 은 피드에서 작게 들린다.
 *   고칠 자리는 «한 층을 더 키우는 것»이 아니라 **완성본에 마스터 단(리미터+게인)을 붙이는 것**이다 —
 *   층별로 올리면 균형이 무너진다. 별건이라 트랙으로 민다.
 */
export const 배경음악 = {
  /**
   * 두 갈래가 나란히 선다:
