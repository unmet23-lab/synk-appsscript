 ...
 *
 * 🔴 08-26 유호님 「약간 멀미나는 느낌을 줄여줬으면」 ⇒ 재고 세기를 반으로 내렸다.
 *   화면에 실제로 나는 픽셀(캔버스를 2배로 그려 CSS 로 반 줄이므로 캔버스값 ÷2):
 *     몽글(폭 821px) 가로 ±26.5 → ±13.7px · 세로 ±28.6 → ±15.7px
 *     까몽(폭 670px) 가로 ±21.6 → ±11.2px · 세로 ±25.9 → ±14.3px
 *   갈래별로 다르게 깎았다 — **세로와 기울임이 멀미의 축**이라 거기를 더 깎았고,
 *   가로 시차는 «옆을 본다»로 읽히는 바로 그 축이라 가장 덜 깎았다(0.42→0.27).
