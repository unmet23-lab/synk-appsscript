...
export const 카운트다운: React.FC<{ 클립: 카운트다운형 }> = ({ 클립 }) => {
  ...
  let ㄱ = 훅프레임;
  const 칸경계 = 클립.순위들.map((r) => {
    const b = { 시작: ㄱ, 끝: ㄱ + r.프레임, 표정: r.표정 };
    ㄱ += r.프레임;
    return b;
  });
  const 마무리시작 = ㄱ;
  const 경계표: 경계[] = [
    { 시작: 0, 끝: 훅프레임, 표정: "본체" },
    ...칸경계,
    { 시작: 마무리시작, 끝: durationInFrames, 표정: 클립.마무리표정 },
  ];
  ...
  const 얹기 = (시작: number, 길이: number, 목록: string[]) => {
    목록.forEach((이름, i) => {
      const 자리 = i === 0 ? 시작 + 6 : 시작 + Math.round(길이 * 0.55);
      옹알이들.push({ 프레임: 자리, 파일: `옹알이_${클립.가이드}/${이름}.wav` });
    });
  };
  얹기(0, 훅프레임, 클립.훅옹알이);
