...
export const 카운트다운: React.FC<{ 클립: 카운트다운형 }> = ({ 클립 }) => {
  ...
  const 칸경계 = 클립.순위들.map((r) => {
    ...
    const b = { 시작: ㄱ, 끝: ㄱ + r.프레임, 표정: r.표정, 몸짓: 몸짓고르기(`${r.표현} ${r.뜻}`) };
    ㄱ += r.프레임;
    return b;
  });
  const 마무리시작 = ㄱ;
  const 경계표: 경계[] = [
    { 시작: 0, 끝: 훅프레임, 표정: "본체" as const, 몸짓: 몸짓고르기(클립.훅) },
    ...칸경계,
    { 시작: 마무리시작, 끝: durationInFrames, 표정: 클립.마무리표정, 몸짓: 몸짓고르기(클립.마무리) },
  ];
