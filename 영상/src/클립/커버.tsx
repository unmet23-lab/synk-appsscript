...
/* 로고 판·크기와 Graphite 틴트의 정본은 `연출.tsx` 하나다 — 복제본을 두면 지면마다 갈린다 */
 ...
 *
 * 🔴 08-26: 이 파일도 로고를 «민라이트»로 박고 있었다 — 본편에서 그것을 고치고도 표지 둘에는
 *   안 옮겼던 자리다(이 저장소가 반복해 앓는 「고치고 옆 지면에 안 옮김」 무늬).
 *   그래서 이제 판 이름을 여기서 안 적는다. 정본은 `연출.로고` 하나다.
 */

export const 커버: React.FC<{ 클립: 클립 }> = ({ 클립 }) => {
  /* 표지에 세우는 것은 «표현» 한 장면뿐이다 — 피드에서 0.5초 안에 읽혀야 한다.
     🔴 첫 판은 `장면들[0]` 을 표현이라고 «알고» 있었다. 옛 표형에서는 맞았지만(첫 칸이 표현)
       장면형이 서면서 첫 칸이 **상황**이 됐고, 그러자 표지가 「오늘의 한 문장」이라는 알약 밑에
       「손님이 셋. 나는 하나.」를 세우게 됐다 — 자리를 «위치»로 안 것이 병이다.
       표지를 안 다시 굽고 있어서 소리도 안 났다(오늘 네 번 밟은 그 무늬 · memory
       `constant-known-in-two-places`). ⇒ 위치가 아니라 **꼴로 찾는다.** */
  const 표현 = 클립.장면들.find((s) => s.꼴 === "표현") ?? 클립.장면들[0];
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: 지면.바탕 }}>
      ...
      >
        ...
        {/* 🔴 표지는 «표현 한 줄»이 주인공이라 길이에 따라 크기가 따라와야 한다.
            152 고정으로 두면 열 글자짜리 표현(「주문 도와드릴까요?」)이 상자를 넘고,
            그때 CSS 기본값이 한국어를 **낱말 한가운데서** 끊는다 — 실측으로 표지 10장 중 셋이
            「주문 도와드릴까 / 요?」·「잠시만 기다려 주 / 세요」·「바로 확인해 드릴 / 게요」였다.
            본편 자막에는 `keep-all` 이 이미 걸려 있었는데 표지에만 안 걸려 있었다. */}
        <div
          style={{
            fontFamily: 본문스택,
            fontSize: 표현.한국어.length <= 6 ? 152 : 표현.한국어.length <= 9 ? 124 : 104,
      ...
      >
        <Img src={staticFile(로고.파일)} style={{ width: 250 }} />
        <div
          style={{
            fontFamily: 본문스택,
            fontSize: 34,
