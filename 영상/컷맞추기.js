 ...
 *   갈래 넷이고, 어느 쪽이 답했는지 **찍어서 낸다**(조용한 폴백 금지 — 이 저장소의 규율).
 *     ① 초록 덩어리 — 몸이 무채색인 캐릭터(까몽: 검은 털 위 초록 눈). 노이즈와 아예 축이 다르다.
 *     ② 노랑 덩어리 — 몸이 남색·회색인 캐릭터(마린: 노란 렌즈 눈). ① 의 짝이다.
 *     ③ 어두운 덩어리 — 몸이 «밝은» 캐릭터(몽글: 코랄 펠트 위 검은 구슬 눈).
 *     ④ 차이 덩어리 — 본체↔컷 차이의 큰 덩어리. 같은 장면을 다시 렌더한 세트에서만 선다.
 *   🔑 앞의 셋이 «색»축이고 넷째만 «차이»축이다. 09-05 에 정본이 4K 로 갈리며 컷마다 따로 그린
 *     그림이 되자 차이축이 원리상 못 서게 됐고(③ 머리말), 그래서 색축이 기본이 됐다.
 *   ⚠ 갈래를 여럿 둔 것은 게을러서가 아니다. 08-26 에 «차이»축으로 여섯 가지를 재봤는데
...
const 상한 = 0.3;
/**
 * 눈자리가 몸의 이 비율에 못 미쳐도 «못 찾은 것»이다.
 * 🔴 [2026-09-05] 왜 생겼나: 상한만 있고 하한이 없어서 **마린에서 초록 갈래가 눈이 아니라
 *   가슴의 초록 장식을 집고도 통과했다**(몸의 0.1%). 그대로 두면 이 도구가 장식만 갈아 끼우고
 *   「성공」을 보고한다 — 상한이 막으려던 바로 그 실패가 반대편 끝에서 다시 난 자리다.
 * ⚠ 표본 셋으로 정했다(몽글 7.7% · 까몽 3.7% · 마린 16.8%). 가장 작은 참값의 1/3.7 이고
 *   반례의 10배다. 넷째 가이드가 오면 이 값을 다시 잰다.
 */
const 하한 = 0.01;
/** «어두운 덩어리» 갈래가 눈으로 보는 밝기 문턱(max(r,g,b)). 몽글 실측 09-05: 몸 평균 216 · 이 값이면 몸의 7.7%. */
const 어두움 = 80;
/** «노랑» 갈래의 문턱(min(r,g)-b). 마린 실측 09-05: 이 값이면 눈 둘만 붉게 서고 몸은 안 걸린다. */
const 노랑 = 100;
/**
 * 한 덩어리가 «가장 큰 덩어리»의 이 비율에 못 미치면 눈이 아니라 곁의 것으로 보고 버린다.
 * 🔑 크기(px)가 아니라 비율이라 4K·1024 어디서나 같은 뜻이다 — 규격이 바뀌어도 안 낡는다.
 * 🔴 [2026-09-05] 왜 생겼나: 마린 가슴의 «꽃 장식 노란 점» 하나가 눈과 함께 잡혀 상자를
 *   아래로 늘렸다(몸의 27.1%). 그 점을 버리니 16.8% 로 줄고 눈 둘만 남는다.
 *   실측으로 2%·5%·10%·20% 가 셋 다 같은 상자를 냈고, 눈의 작은 조각을 가장 덜 버리는 2% 를 골랐다.
 * ⚠ 몽글 7.7% · 까몽 3.7% 는 이 거르개 앞뒤로 **한 자리도 안 바뀐다**(회귀 없음을 그렇게 쟀다).
 */
const 곁가지 = 0.02;
 ...
 * 🔑 합치기 «전»에 곁가지를 버린다(위 `곁가지` 머리말). 담을 때 바로 키우면 작은 장식 하나가
 *   상자를 늘려도 되돌릴 수가 없으므로, 다 모은 «뒤»에 가장 큰 것과 견주어 거른다.
 */
const 상자합치기 = () => {
  const 담긴것 = [];
  return {
    담기: (b) => { 담긴것.push(b); },
    낸다: () => {
      if (!담긴것.length) return null;
      const 최대 = 담긴것.reduce((a, b) => (b.n > a ? b.n : a), 0);
      let 상자 = null;
      for (const b of 담긴것) {
        if (b.n < 최대 * 곁가지) continue;
        상자 = 상자
...
/**
 * ② 노랑 — 몸이 남색·회색인 캐릭터(마린: 남색 후드 아래 노란 렌즈 눈). ① 초록의 짝이다.
 *   자 = `min(r,g) - b` (노랑은 빨강·초록이 둘 다 높고 파랑이 낮다).
 *
 * 🔴 [2026-09-05] 왜 생겼나: 마린이 정본 가이드 셋째로 서면서 이 도구에 들어왔는데,
 *   ① 초록이 눈이 아니라 **가슴의 초록 장식**을 집었다(몸의 0.1%). 마린 눈은 노랑이라
 *   초록 축으로는 원리상 안 잡힌다. 그 오답은 위 `하한` 이 막고, 정답은 이 갈래가 낸다.
 * 🔑 마린은 눈이 «렌즈»라 감기지 않고 밝기·모양으로 표정을 낸다(자산 표 주석). 그래도 변하는
 *   자리가 눈이므로 「눈 밖을 본체로 덮는다」는 이 도구의 뜻이 그대로 선다.
 */
function 노랑갈래(컷그림, w, h) {
  const 모음 = 상자합치기();
  let 노랑수 = 0;
  for (const A of Object.values(컷그림)) {
    const bin = new Uint8Array(w * h);
    for (let i = 0; i < w * h; i++) {
      const o = i * 4;
      if (A.d[o + 3] < 128) continue;
      if (Math.min(A.d[o], A.d[o + 1]) - A.d[o + 2] >= 노랑) { bin[i] = 1; 노랑수++; }
    }
    덩어리들(bin, w, h, 2).filter((b) => b.n >= 60).forEach(모음.담기);
  }
  if (노랑수 < 300) return null;
  return { 상자: 모음.낸다(), 방법: `노랑 덩어리 ${노랑수}px` };
}
...
function 눈자리찾기(컷그림) {
  ...
  const 갈래들 = [초록갈래, 노랑갈래, 어두운갈래, 차이갈래];
  for (const 갈래 of 갈래들) {
    const 낸것 = 갈래(컷그림, w, h);
    if (!낸것 || !낸것.상자) { 버린것.push(`${갈래.name}: 아무것도 못 집었다`); continue; }
    const b = 낸것.상자;
    const 비율 = ((b.x1 - b.x0 + 1) * (b.y1 - b.y0 + 1)) / 몸넓;
    /* 🔑 위아래로 다 막는다 — 몸을 통째로 덮어도, 눈보다 훨씬 작은 것을 집어도 «못 찾은 것»이다.
       한쪽만 막으면 반대편 끝에서 같은 실패가 조용히 난다(마린 초록 장식 0.1% 가 그 자리였다). */
    if (비율 > 상한 || 비율 < 하한) {
      const 어디 = 비율 > 상한 ? '몸을 덮었다' : '눈이라기엔 너무 작다';
      버린것.push(`${갈래.name}: 몸의 ${(비율 * 100).toFixed(1)}% — ${어디}(${낸것.방법})`);
      continue;
    }
    return { 상자: b, 방법: `${낸것.방법} · 몸의 ${(비율 * 100).toFixed(1)}%` };
  }
  throw new Error(
    `눈자리를 못 찾았다 — 갈래 ${갈래들.length}개가 다 헛짚었다.\n  ` + 버린것.join('\n  ')
    + '\n  🔑 컷 셋이 «눈만 다른» 한 벌인지 먼저 본다(각도·몸짓 컷이 섞이면 여기서 걸린다).'
    + '\n  🔑 새 가이드라면 눈 «색»이 몸과 갈리는지 본다 — 갈리면 그 색 갈래를 하나 더 짓는다.',
  );
}
...
/* 컷 목록은 손으로 안 든다 — 누끼판이 있는 컷을 창구가 안다(마스코트자산 `몽글누끼컷`·`까몽누끼컷`).
   ...
   🆕 [2026-09-05] **마린이 셋째로 들어왔다.** 09-05 에 마린이 정본으로 서면서 가이드 셋이
     처음으로 같은 폴더·같은 규격에 나란히 섰는데, 이 목록은 둘인 채였다 — 영상에서 마린만
     털이 튀는 자리였다. 창구 이름이 «누끼»가 아닌 것은 새 세트가 전부 투명이라 그렇다
     (자산 표 `마린표정컷` 주석). 셋 다 각도판은 창구에서 걸러 온다. */
const 가이드들 = [
  { 이름: '몽글', 컷: 마스코트.몽글누끼컷, 경로: (c) => 마스코트.경로(c, { 누끼: true }) },
  { 이름: '까몽', 컷: 마스코트.까몽누끼컷, 경로: (c) => 마스코트.까몽경로(c, { 누끼: true }) },
  { 이름: '마린', 컷: 마스코트.마린표정컷, 경로: (c) => 마스코트.마린경로(c) },
];

/* 시험이 갈래·자를 따로 부를 수 있게 낸다.
   🔑 `require` 로 부를 때는 아래 본체가 «안» 돈다 — 돌면 4K 스물한 장을 읽느라 시험이 몇 분씩
   걸리고, 시험이 느리면 아무도 안 돌린다. 합성 그림으로 갈래와 자만 재는 것이 이 문의 뜻이다. */
module.exports = {
  눈자리찾기, 초록갈래, 노랑갈래, 어두운갈래, 차이갈래, 상자합치기,
  덩어리들, 몸상자, 마스크, 맞춘것, 가이드들,
  상한, 하한, 곁가지, 어두움, 노랑, 깃,
};
