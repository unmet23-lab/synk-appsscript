...
const 깃 = 14;
/** 눈자리가 몸의 이 비율을 넘으면 «못 찾은 것»으로 본다 — 넘으면 던진다(조용히 몸을 다 덮으면 뜻이 없다). */
const 상한 = 0.3;
...
function 눈자리찾기(컷그림) {
  ...
  /* ① 초록 — 몸이 무채색인 캐릭터. 노이즈는 밝기를 흔들지 «색상»을 안 옮긴다. */
  let 초록수 = 0;
  for (const A of Object.values(컷그림)) {
    ...
    덩어리들(bin, w, h, 2).filter((b) => b.n >= 60).forEach(담기);
  }
  if (초록수 >= 300 && 상자) return { 상자, 방법: `초록 덩어리 ${초록수}px` };

  /* ② 차이 덩어리 — 색으로 안 갈리는 캐릭터. 큰 덩어리만 집어 낱알 노이즈를 버린다. */
  상자 = null;
  const 본체 = 컷그림['본체'];
  for (const [이름, B] of Object.entries(컷그림)) {
    ...
    덩어리들(bin, w, h, 1).filter((b) => b.n >= 400).forEach(담기);
  }
  if (!상자) throw new Error('눈자리를 못 찾았다 — 초록도 없고 큰 차이 덩어리도 없다');
  return { 상자, 방법: '차이 덩어리 ≥400px' };
}
...
/* 컷 목록은 손으로 안 든다 — 누끼판이 있는 컷을 창구가 안다(마스코트자산 `까몽누끼컷`).
   ⚠ 08-29 에 까몽 컷이 3→8 로 늘었는데 그중 누끼가 있는 것은 여전히 셋이다. 여기가
     「구운 것 전부」를 집으면 없는 누끼를 읽다 죽는다 — 그래서 «누끼 있는 것»만 묻는다. */
const 컷이름 = 마스코트.까몽누끼컷;
const 가이드들 = [
  { 이름: '몽글', 경로: (c) => 마스코트.경로(c, { 누끼: true }) },
  { 이름: '까몽', 경로: (c) => 마스코트.까몽경로(c, { 누끼: true }) },
];

let 쓴것 = 0;
for (const 가이드 of 가이드들) {
  const 컷그림 = {};
  for (const c of 컷이름) 컷그림[c] = png읽기(path.join(저장소, 가이드.경로(c)));

  const { 상자, 방법 } = 눈자리찾기(컷그림);
  const 본체 = 컷그림['본체'];
  const 몸 = 몸상자(본체);
  const 몸넓 = (몸.x1 - 몸.x0 + 1) * (몸.y1 - 몸.y0 + 1);
  const 눈넓 = (상자.x1 - 상자.x0 + 1) * (상자.y1 - 상자.y0 + 1);
  const 비율 = 눈넓 / 몸넓;
  if (비율 > 상한) {
    throw new Error(
      `${가이드.이름} 눈자리가 몸의 ${(비율 * 100).toFixed(1)}% 다 — 눈을 못 찾고 몸을 덮은 것이다. ` +
      `그대로 두면 이 도구가 «아무 일도 안 하면서 성공을 보고»한다(방법 = ${방법}).`,
    );
  }
  ...
  console.log(
    ...
    + `${상자.x1 - 상자.x0 + 1}×${상자.y1 - 상자.y0 + 1} = 몸의 ${(비율 * 100).toFixed(1)}% · 방법 «${방법}»`,
  );
