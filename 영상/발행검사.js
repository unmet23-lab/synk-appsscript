...
function 소스가르기(원문) {
  ...
  while (i < n) {
    ...
    if (모드 === '코드') {
      ...
      if (ch === '"' || ch === "'" || ch === '`') { 모드 = '문자열'; 닫음 = ch; 버퍼 = ''; 버퍼줄 = 줄; 버퍼앞 = 코드.length; 코드.push(ch); i += 1; continue; }
    ...
    if (닫음 === '`' && ch === '$' && 다음 === '{') {
      문자열들.push({ 글: 버퍼, 줄: 버퍼줄, 앞: 코드.slice(Math.max(0, 버퍼앞 - 48), 버퍼앞) });
      버퍼 = '';
      스택.push('tpl');
      모드 = '코드';
      코드.push('${');
    ...
    if (ch === 닫음) { 문자열들.push({ 글: 버퍼, 줄: 버퍼줄, 앞: 코드.slice(Math.max(0, 버퍼앞 - 48), 버퍼앞) }); 모드 = '코드'; 코드.push(ch); i += 1; continue; }
    if (ch === '\n') {
      줄 += 1;
...
function 소리검사(클립들, 카운트들, 소리표) {
  const 위반 = [];
  const 있나 = (이름) => Object.prototype.hasOwnProperty.call(소리표, 이름);
  for (const c of 클립들) {
    for (const s of c.장면들) {
      if (s.소리 && !있나(s.소리)) 위반.push(`${c.id}/${s.꼴 || s.라벨} 소리 「${s.소리}」 — 소리 표에 없는 이름(정본 = 토큰 사운드.이벤트 + 옹알이 7 · _사운드킷.md 「이 순서 밖으로 만들지 않는다」)`);
    }
  }
  for (const c of 카운트들) {
    const 전부 = [
