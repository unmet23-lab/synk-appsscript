...
/* 🔴 태그 안에 «임자 말고 다른 것»이 들어간 꼴이 실물에 여럿이다 — `[기계 09-06]`·`[유호 확정]`·
 *   `[기계 → 구현]`·`[유호 확정 09-03 저녁]`. 첫 판은 `\[(유호|기계|사람)\]` 로만 잡아서
 *   그 줄들을 「임자 없음」으로 세었다(09-06 실측 71 중 스물 남짓이 이 오탐이었다).
 *   ⇒ 임자 «낱말로 시작하는» 태그면 받는다. 임자가 아닌 `[닫힘 …]`·`[옆 세션]` 은 그대로 안 받는다. */
const 일감꼴 = /^-\s*[^[]*\[(유호|기계|사람)(\s[^\]]*)?\]/;
/** 임자가 아니라 «상태»를 다는 태그 — 닫힌 줄은 주인이 없어도 규약 위반이 아니다. */
const 닫힘꼴 = /^-\s*[^[]*\[닫힘/;
/** 남은 일이 아니라 «다음에 밟을 것»을 적은 줄 — 규칙·자·포인터·금지·시한 표시. */
const 규칙꼴 = /^-\s*(자:|규칙:|자\(규칙\):|[🔑🔒📌🚫🧭📏🗂🔗💰📄🎨🔢🎯🪤🌩🧵⏱])/u;
  ...
  for (const s of 절) {
    ...
    for (let k = 0; k < s.줄들.length; k++) {
      ...
      if (!일감꼴.test(l)) {
        ...
        if (/^- /.test(l)) {
          /* 「- 」로 시작해도 «일감»이 아닌 것이 셋 있다. 그건 규약 위반이 아니라 다른 갈래다:
           *   ㉠ 닫힘 기록 `- ✅ [닫힘 09-05] …`   — 임자가 아니라 상태다
           *   ㉡ 규칙·자·포인터 `- 🔑 …`·`- 자: …`  — 남은 일이 아니라 «다음에 밟을 것»이다
           *   그 둘을 빼야 「임자 없는 «일감»」이 남는다(안 빼면 62 중 마흔이 오탐이다). */
          if (!닫힘꼴.test(l) && !규칙꼴.test(l)) {
            임자없음.push({ 절: s.제목.slice(0, 30), 줄: i + 1, 머리: l.slice(0, 70) });
          }
          continue;
        }
