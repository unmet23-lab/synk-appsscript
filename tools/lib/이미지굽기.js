 ...
 *   ② 돈열쇠생존()  텍스트 추론 1회 — Vertex 호출 가능 여부를 확인한다.
 *      이 호출도 종량제이며 무료 체험 계정에서는 크레딧을 쓴다. AI Studio의 무료 등급과
 *      다르다. 성공은 크레딧 잔액·체험 상태·카드 청구 여부의 증명이 아니다.
...
/** Vertex 텍스트 추론 1회. 호출 가능 여부만 확인하며 잔액 조회나 0원 검사가 아니다. */
...
/** 배치 시작 «전» 게이트 — 예상비용 계산은 0원, Vertex 프로브는 텍스트 추론 1회다.
...
async function 배치게이트(장수, 크기 = '1K') {
  ...
  console.log('  Vertex 호출 확인은 텍스트 추론 1회 비용이 별도다. 잔액·체험 만료·카드 청구는 결제 화면에서 확인한다.');
  const s = await 돈열쇠생존();
  if (s.살았나 === true) { console.log('  ✅ Vertex 텍스트 호출에 성공했다 — 이미지 생성을 진행한다(잔액 확인은 아님).'); return true; }
  if (s.살았나 === null) {
    if (s.종류 === '유료차단') {
      console.log('  🛡 유료 Vertex가 기본 차단됐다 — --유료-api 없이 한 장도 굽지 않는다.');
      return false;
    }
    console.log(`  🟠 **못 물어봤다**(${s.종류}) — 「살았다」가 아니다. 굽되 첫 장에서 벽을 만나면 거기서 선다.`);
    return true;
  }
  console.log(`  🔴 Vertex 호출이 막혀 있다(${s.상태} ${s.종류}) — 이미지 생성 0회.`);
  if (s.사유) console.log(`     ${String(s.사유).slice(0, 200)}`);
  console.log('     원인·잔액은 Google Cloud → 결제에서 확인한다. 이 도구는 충전하거나 유료 계정으로 전환하지 않는다.');
  return false;
}
