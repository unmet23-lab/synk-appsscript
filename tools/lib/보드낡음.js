...
function 증거(줄, root, 캐시) {
   ...
   * **해시**는 남긴 일감 조각에서만 센다(그 밖의 해시는 영수증이다 · `일감조각` 머리말). */
  const 일감 = 일감조각(줄);
  const text = `${트랙텍스트(줄)} ${일감}`;
  ...
  return { 경로, 커밋, 잰것: !!(경로.실재.length || 경로.없음.length || 경로.못잼.length || 커밋.length) };
}
...
module.exports = { 일감조각, 트랙텍스트, 경로들, 해시들, 저장소들, 검수된커밋들, 증거, 말들, 줄별증거 };
