 ...
 *   · 기본 표현 = 잉크 카드 «벡터 펠트». 2026-09-10 대외 synk는 네 글자의 실땀만 빼고
 *     보풀·몸체·두께 2톤을 유지한다. 내부 꺾쇠·기호·도장·알록 특별판의 실땀은 보존한다.
...
function 글자펠트(참조, 몸색, 그늘색, 필터, { 땀보조색 = 색['Ink Deep'], 심 = false, 땀세기 = '보통', 실땀 = true } = {}) {
  const { w, op } = 땀세기들[땀세기];
  return `
    ...
    <g filter="url(${필터})"><use href="${참조}" fill="${몸색}"/>${심 ? `<use href="${참조}" fill="url(#sl-core)"/>` : ''}</g>${실땀 ? `
...
/* 알록 실 — **소프트 한 벌로 통일**(유호 확정 08-25 「소프트 버전이 정본이야」).
 *   08-25 중간에 교차 픽(synk=S2 · syn<=J2)을 거쳤으나 눈 판정으로 되돌아왔다.
 *   통일이 낫는 이유: 두 락업은 «신호 한 글자»만 다른 한 세트이고, 실까지 갈리면
 *   두 브랜드처럼 보인다. 파스텔 다섯이 하얀 펠트 위에서 가장 «공방»답다.
 *   ⚠ 절제(J) 벌은 버리지 않았다 — 대안 서랍(알록대안 · 실:'절제')에 살아 있다. */
const 소프트실 = [색['Lapis Soft'], 색['Meadow Soft'], 색['Butter'], 색['Pop Soft'], 색['Coral Soft']];
const 알록실 = { k: 소프트실, 꺾쇠: 소프트실 };
 ...
 *   2026-09-09 대외 synk 기본은 '단색': 네 글자 Ink/Paper, 사업명만 고유색.
 *   '코랄' → 앱 내부 꺾쇠의 기본. 기존 특별판/역사 재현은 명시적으로 지정한다.
 *   '단색' → 신호에서 색을 뺀다. **흰색이 아니라 «syn 과 같은 색»** 이다 — 다크면 Paper,
 *            라이트면 Ink. 흰색으로 못 박으면 밝은 바탕에서 신호가 그대로 사라진다.
 *            몸색·재질·심을 syn 과 맞춘다. 대외 k는 네 글자 모두 실땀을 제외한다.
 ...
 *
 * ■ 2026-09-10: 대외 신호 k는 색갈래와 관계없이 네 글자에서 실땀만 제외한다.
 *   명시적 유채색도 몸색은 그대로 두며, 알록은 색실 자체가 정체성인 별도 특별판으로 보존한다.
 *   내부 꺾쇠와 알록의 기존 실땀 강도(syn 보통·신호 기호급)는 바꾸지 않는다.
 */
...
function 워드마크안쪽({ 판 = '다크', 신호 = 'k', 색갈래 = 신호 === 'k' ? '단색' : '코랄' } = {}) {
  const 라 = 판 === '라이트';
  const 실땀 = 신호 !== 'k' || 색갈래 === '알록';
  const syn = 라
    ? 글자펠트('#sl-syn', 색['Ink'], 색['Ink Deep'], '#sl-felt-ink', { 땀보조색: 색['Deep Wool'], 실땀 })
    : 글자펠트('#sl-syn', 색['Paper'], 색['Stone'], '#sl-felt-paper', { 심: true, 실땀 });
  const 참조 = 신호 === '꺾쇠' ? '#sl-chev' : '#sl-k';
  const 유채 = 신호색들()[색갈래];
  ...
  const 몸 = 색갈래 === '알록'
    ...
    : 색갈래 === '단색'
      ? (라 ? 글자펠트(참조, 색['Ink'], 색['Ink Deep'], '#sl-felt-ink', { 땀보조색: 색['Deep Wool'], 땀세기: '기호급', 실땀 })
            : 글자펠트(참조, 색['Paper'], 색['Stone'], '#sl-felt-paper', { 심: true, 땀세기: '기호급', 실땀 }))
      : 글자펠트(참조, 유채.몸, 유채.그늘, 유채.필터, { 땀세기: '기호급', 실땀 });
  return syn + 몸;
}
 ...
 * 색갈래:'단색'(대외 k 기본 — 다크 Paper · 라이트 Ink) | '코랄'(내부 꺾쇠 기본).
 * 표현:  '펠트'(기본) | '민'(작은 자리·폴백 — 필터 0, 어디서나 가볍게).
 * 슬래시: true 면 W1 락업(synk //) — «//» 는 잉크의 저채도(세 번째 색이 아니다).
 */
function 워드마크({ 판 = '다크', 표현 = '펠트', 신호 = 'k', 색갈래 = 신호 === 'k' ? '단색' : '코랄', 슬래시 = false, 클래스 = '' } = {}) {
...
if (require.main === module) {
  ...
  if (args.includes('--json')) {
    process.stdout.write(JSON.stringify({
      ...
      /* 알록 특별판(큰 판 전용 · 상시 금지 — 단 앱 스플래시는 상시) — synk=S2 · syn<=J2 */
      알록synk: 워드마크({ 판: '다크', 색갈래: '알록' }),
      알록꺾쇠: 워드마크({ 판: '다크', 신호: '꺾쇠', 색갈래: '알록' }),
      /* 신호색 갈래 — 코랄 밖 셋(자리를 골라 쓴다) */
      버터다크: 워드마크({ 판: '다크', 색갈래: '버터' }),
      메도우다크: 워드마크({ 판: '다크', 색갈래: '메도우' }),
      팝다크: 워드마크({ 판: '다크', 색갈래: '팝' }),
