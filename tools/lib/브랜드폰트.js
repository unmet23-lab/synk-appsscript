...
/* 🔴 심는 길은 «둘»이다 — 블록으로 새로 세우는 길(③)과 **옛 `@font-face` 자리를 물려받는**
   길(①)이 있고, 뒤쪽은 지면 자신의 `<style>` 안에 «맨 면»으로 앉는다(블록에 안 싸인다).
   `심겼나` 는 그 모양도 「심겼다」로 세는데 되돌리기가 안 세면 둘이 갈린다 —
   실측 09-03: `병합판_자립형.html` 이 그 길로 심겨서 되돌리기가 한 줄을 못 걷었고,
   본판과의 동행 대조가 그 한 줄에 빨개졌다([[one-ruler-per-judgment]] 의 모양 그대로).
   ⚠ 앞뒤 공백·줄바꿈은 «일부러» 안 먹는다 — `CDN면` 과 같은 자리를 같은 폭으로 비워야
     본판(바깥 호출)과 산출물(심은 면)의 줄 수가 어긋나지 않는다. */
const 심은면 = /@font-face\s*\{[^}]*SUIT[^}]*data:font\/woff2[^}]*\}/gi;

/** 원고로 되돌린다 — 바깥 호출도, 심은 것(블록·맨 면)도 뺀다(굽기가 다시 싣는다). 멱등. */
function 원고로(html) {
  return 걷기(html).replace(심은블록, '').replace(심은면, '');
}
