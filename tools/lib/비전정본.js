...
const optionalSubtext = new Set(['lab', 'pulse']);

function loadVisions(input = fs.readFileSync(sourcePath, 'utf8')) {
  ...
  for (const [key, name] of Object.entries(brands)) {
    ...
    if (!headline || (!subtext && !optionalSubtext.has(key))) throw new Error('비전 정본에서 전면 문장과 필요한 서브텍스트를 찾을 수 없습니다: ' + name);
...
function withVisionBlock(markdown, key) {
  ...
  const body = ['**' + vision.headline + '**', vision.subtext].filter(Boolean).join('\n\n');
