 ...
 *
 * ■ 입는 규칙 (유호 확정 09-06)
 *   의상 최대 1 + 악세 최대 1 (09-08 변경, 09-09 재확인). 항목마다 `갈래`가 붙는다.
 *
 * ■ 고유 항목은 지어내지 않았다
...
/* ══════════════ 옷 그림이 «어디에» 있나 (2026-09-08) ══════════════
 *
 * 🔴 경로를 다른 파일에 새로 적지 않는다 — 옷의 정본이 여기이므로 경로도 여기서만 난다
 *    (.claude/rules/bake-tools.md · 옛판이 여럿이면 옛판이 더 많이 참조된다).
 *
 * ■ 다섯 방을 지나며 자란다. 앞이 끝나야 뒤가 선다.
 *   ① GPT              구운 원본       흰 바탕 2560   tools/옷GPT굽기.js
 *   ② GPT_표정          표정 얹은 것    흰 바탕 2560   tools/옷표정얹기.py
 *   ③ …_누끼            배경 오림       투명 2560     tools/옷누끼.py
 *   ④ …_누끼_틀         정본 틀에 앉힘  투명 4096     tools/옷틀맞추기.py
 *   ⑤ …_avif           담는 꼴 바꿈    투명 4096     tools/옷담는꼴.py   ← 앱이 읽는 것
 *
 * ■ 파일 이름 = `<마스코트>_<옷들>[_<표정>].<꼴>`
 *   옷 이름에서 빈칸을 빼고, 겹쳐 입으면 `+` 로 잇는다. 목록에 적힌 «차례대로» 잇는다
 *   — 순서가 흔들리면 같은 차림이 두 이름을 가져 앱이 못 찾는다.
 *     까몽_목도리.png              옷 한 벌 · 표정 없음
 *     까몽_목도리+3급왕관.png      두 벌 겹쳐
 *     까몽_목도리_윙크.avif        옷 한 벌 · 윙크 · 앱이 읽는 꼴
 */
const 옷방뿌리 = 'docs/Loom_자산/옷';
const 방 = {
  구운것: 'GPT',
  표정: 'GPT_표정',
  누끼: 'GPT_누끼',
...
/** 옷 이름 여럿을 파일 이름의 «옷» 자리로. 목록 차례를 지킨다. */
function 옷토막(마스코트이름, 옷이름들) {
  const 벌 = 목록(마스코트이름);
  const 차례 = (n) => 벌.findIndex((v) => v.이름 === n);
  for (const n of 옷이름들) {
    if (차례(n) < 0) throw new Error(`${마스코트이름} 목록에 없다 — ${n}`);
  }
  return [...옷이름들].sort((a, b) => 차례(a) - 차례(b))
    .map((n) => n.replace(/ /g, '')).join('+');
}

/**
 * 옷 그림 한 장의 경로(저장소 기준 상대).
 * @param 마스코트이름 '까몽'|'몽글'|'마린'
 * @param 옷이름들 ['목도리'] 또는 ['목도리','3급 왕관'] — 목록에 적힌 이름 그대로
 * @param 표정 '윙크' 등. 없으면 표정 안 얹은 판.
 * @param 어디 방의 열쇠 — 기본 '앱'(AVIF). 옷만 필요하면 '앱옷만'.
 */
function 옷경로(마스코트이름, 옷이름들, 표정, 어디) {
  const 옷들 = Array.isArray(옷이름들) ? 옷이름들 : [옷이름들];
  const 칸 = 방[어디 || (표정 ? '앱' : '앱옷만')];
  if (!칸) throw new Error(`그런 방이 없다 — ${어디} (${Object.keys(방).join('·')})`);
  const 꼴 = 칸.endsWith('_avif') ? 'avif' : 'png';
  const 이름 = `${마스코트이름}_${옷토막(마스코트이름, 옷들)}${표정 ? `_${표정}` : ''}.${꼴}`;
  return `${옷방뿌리}/${칸}/${이름}`;
}

/**
 * 현재 허용 차림 수 — 「의상 최대 1 + 악세 최대 1」(09-08 변경).
 * 맨몸은 제외. 재고/검수 통과 수가 아니라 조합 분모다. 옛 세벌 필드는 0으로 보존한다.
 */
function 차림수(마스코트이름) {
  const 벌 = 목록(마스코트이름);
  const 의 = 벌.filter((v) => v.갈래 === '의상').length;
  const 악 = 벌.filter((v) => v.갈래 === '악세').length;
  return {
    의상: 의, 악세: 악,
    한벌: 의 + 악,
    두벌: 의 * 악,
...
module.exports = { 마스코트들, 공용, 성과더, 고유, 목록, 옷방뿌리, 방, 옷토막, 옷경로, 차림수 };
