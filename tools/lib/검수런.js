...
const 종료코드뜻 = {
  ...
  /* 마누스.js 폴러 — 장부에 적히는 코드는 0·1·2·3·4·5. `결과쓰기()` 를 지난 것만 결과가 있다. */
  마누스: {
    0: { 이름: '완주', 결과: true },   // stopped — 결과 파일 있음
    1: { 이름: '미결', 결과: true },   // error — 결과 파일은 썼다(원문이 들어 있다)
    2: { 이름: '미결', 결과: true },   // waiting — 사람 확인 대기 · 비고가 「결과 파일은 있다」
    3: { 이름: '실패', 결과: false },  // 폴러 상한 초과 — 결과 파일 없음 (F509 의 그 런)
    4: { 이름: '실패', 결과: false },  // 조회 회복불가
    5: { 이름: '실패', 결과: false },  // 상태 판독불가
  },
};

/** 이 런을 만든 도구 — 종료코드의 뜻을 고르는 키.
 *  🔑 **`이어받기명령` 과 같은 판별자**를 쓴다: 종류 «이름»이 아니라 **칸**으로 가른다
 *    (이름은 갈아치울 수 있지만 `작업id` 는 마누스 런만, `인자` 는 codex 런만 쓴다).
 *    같은 판별을 두 곳에 적으면 그날 갈라진다(등록층 ④). */
function 생산자(런) {
  if (!런) return null;
  if (런.작업id) return '마누스';
 ...
 *  실측 F508: 장부에 런을 적는 생산자가 둘(`codex-review.js`·`마누스.js`)인데 이어받기 처방은
 *  codex 쪽 필드(`인자`) 하나에만 기대고 있었다. 마누스 런엔 그 칸이 아예 없어서
 ...
 *    붙는다**(실측: 마누스 런에 「체크포인트가 있어 끝난 회차는 안 다시 돈다」가 붙어 나갔다.
 *    마누스 이어받기는 체크포인트가 아니라 **재부착**이라 그 문장은 그냥 거짓이다). */
function 이어받기명령(런) {
  if (!런) return { 명령: '⚠ 런을 못 읽었다 — 이어받을 대상이 없다', 비고: '' };
  /* 마누스 런은 인자를 다시 주는 게 아니라 **런ID 로 다시 붙는다**.
   * 판별자는 `생산자()` 하나다 — 종료코드 뜻표와 **같은 키**를 써야 둘이 안 갈린다(F561). */
  if (생산자(런) === '마누스') {
    return {
      명령: `node tools/마누스.js --이어받기 ${런.런ID}`,
      비고: '재던지기가 아니라 **다시 붙는 것**이다 — 마누스 쪽 작업은 그대로 살아 있고 크레딧도 안 든다.',
    };
  }
