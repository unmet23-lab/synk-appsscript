...
/* ⚠ 종류는 **이어받기표에 실린 것만** 받는다. 안 실린 종류로 런이 생기면, 그 런이 멈춘 날
 *   훅이 「이어받기 명령을 못 낸다」만 낼 수 있다(F511 ⚠ 절).
 * ⚠ 여기는 **마지막 벽이지 첫 벽이 아니다.** codex 쪽은 이 호출이 spawn 앞이라 값이 안 나가지만,
 *   마누스 쪽은 `task.create` **뒤**라 여기서 걸리면 크레딧은 이미 나갔는데 장부엔 아무것도 안
 *   남는다(유령). 그래서 값이 드는 호출 앞에도 같은 검사를 세워 뒀다(`마누스.js 던지기` 머리). */
function 런ID발급(종류) {
  if (!아는종류(종류)) {
    throw new Error(`모르는 런 종류 「${종류}」 — tools/lib/검수런.js 의 이어받기표에 먼저 등록한다(안 하면 멈춘 날 이어받을 방법이 없다 · F511)`);
  }
...
/* 멈춤을 재는 시계는 **「던진 지」가 아니라 「지금 붙은 폴러가 붙은 지」**다 (F511 후속 · 실측 08-16).
 * 🔴 왜 갈랐나: `--이어받기` 로 실제로 다시 붙였는데(pid 살아 있음 · 상태 `진행`) 목록은 여전히
 *   「멈춤 — 이어받아라」로 읽었다. `시작` 이 3시간 전 그대로라 시간 상한을 계속 넘기 때문이다.
 *   처방을 따라도 **따랐다는 사실이 어디에도 안 보이니**, 다음 세션이 같은 원격 작업에 폴러를
 *   하나 더 붙인다(F103 의 모양 — 성공을 못 보여 주는 처방은 계속 다시 적용된다).
...
function 판정(런, now) {
   ...
   * 처방을 따른 사람이 그 사실을 못 보고, 다음 세션이 폴러를 또 붙인다. */
  const 무 = 무응답분(런, now);
  if (무 != null && 무 > 멈춤분) {
...
/* ── 이어받기 명령 ────────────────────────────────────────────────────────────
 * 멈춘 런에 **다시 붙는** 명령은 종류마다 도구가 다르다. 그 대응을 여기서만 적는다.
 *
 * 🔴 왜 있나 (F511 · 2026-08-16 실측). 세션 시작 훅이 멈춘 `사실심문` 런을 두고
 *   `이어받기: node tools/codex-review.js ` — **인자 빈칸** — 을 냈다. 세 겹으로 틀렸다:
 *     ① 그 런엔 `인자` 칸이 아예 없다(마누스가 적는 레코드는 `작업id`·`질문` 을 쓴다).
 *     ② 사실심문은 codex 가 이어받을 수 없다. 원격 작업id 에 다시 붙는 `마누스.js --이어받기` 다.
 *     ③ 그리고 그 빈 명령은 **아무것도 안 하지 않는다** — 인자 없는 `codex-review.js` 는
 *        기본 검수를 새로 돌린다(main 이 플래그 분기를 다 지나쳐 기본 동작으로 떨어진다).
 *        「이어받기」라 적힌 줄을 붙여 넣으면 이어받는 대신 **새 실탄이 나간다.**
 *   같은 자리를 `codex-review --런목록` 은 `x.런.인자.join` 으로 **TypeError**(exit 2)를 냈고 —
 *   멈춤 절에서 끊겨 그 뒤 「도는 중」 목록이 통째로 안 나왔다 — `마누스 --런목록` 만 맞았다.
 *   셋이 손으로 적어 갈라진 결과다.
 *
 * 🔑 F509 가 «상태» 를 `상태이름()` 한 곳에 모은 것과 **같은 처방**이다. 같은 병의 3번째라
 *   호출부를 하나씩 고치지 않고 잘못 쓸 수 없는 판독으로 만든다(CLAUDE.md 신뢰성).
 *
 * ⚠ 이 장치가 틀릴 때의 모습: 종류가 하나 더 늘고 이 표에 안 실리면 「모른다」가 나간다.
 *   지어낸 명령보다는 낫지만 사람이 로그를 직접 열어야 한다. 그래서 **쓰는 쪽에서 막는다** —
 *   `런ID발급()` 이 표에 없는 종류를 거절하므로, 표에 안 실린 종류의 런은 애초에 못 생긴다.
 */
const 이어받기표 = {
  검수:   { 도구: 'tools/codex-review.js', 인자로, 꼬리: '(체크포인트가 있어 끝난 회차는 안 다시 돈다)' },
  심문:   { 도구: 'tools/codex-review.js', 인자로, 꼬리: '(체크포인트가 있어 끝난 회차는 안 다시 돈다)' },
  선파악: { 도구: 'tools/codex-review.js', 인자로, 꼬리: '(체크포인트가 있어 끝난 회차는 안 다시 돈다)' },
  /* 마누스는 원격에서 계속 돌고 있을 수 있다 — 재던지기가 아니라 붙기라 크레딧이 안 든다. */
  사실심문: { 도구: 'tools/마누스.js', 인자로: 이어받기인자, 꼬리: '(재던지기 아님 · 크레딧 0)' },
};

/** codex 계열 — 던질 때 적어 둔 인자를 그대로 되쓴다. 없으면 **지어내지 않는다**(위 ③). */
function 인자로(런) {
  const a = (런 && 런.인자) || [];
  return Array.isArray(a) && a.length ? a : null;
}

/** 마누스 계열 — 런ID 하나로 원격 작업에 다시 붙는다. */
function 이어받기인자(런) {
  return 런 && 런.런ID ? ['--이어받기', 런.런ID] : null;
}

function 아는종류(종류) { return Object.prototype.hasOwnProperty.call(이어받기표, 종류); }

/**
 * 멈춘 런 하나를 이어받는 명령. **모르면 모른다고 답한다** — 명령을 지어내지 않는다.
 * @returns {{명령:string, 꼬리:string}|{모름:true, 왜:string}}
 */
function 이어받기(런) {
  const 종류 = 런 && 런.종류;
  const 규격 = 아는종류(종류) ? 이어받기표[종류] : null;
  if (!규격) {
    return { 모름: true, 왜: `종류 「${종류 || '(없음)'}」 는 이어받는 통로가 등록돼 있지 않다 (→ 로그 끝을 직접 본다)` };
  }
  const 인자 = 규격.인자로(런);
  if (!인자) {
    /* 여기서 빈 명령을 내면 그게 **새 런을 던지는 명령**이 된다(위 ③). 차라리 못 낸다고 말한다. */
    return { 모름: true, 왜: '이어받을 인자가 이 런의 장부에 안 적혔다 (→ 로그 끝을 보고 손으로 다시 던진다)' };
  }
  return { 명령: `node ${규격.도구} ${인자.join(' ')}`, 꼬리: 규격.꼬리 };
}

/** 훅·조회가 **같은 문구**로 내는 이어받기 줄(들여쓰기만 부른 쪽이 정한다). */
function 이어받기줄(런, 들여 = '      ') {
  const r = 이어받기(런);
  return r.모름
    ? `${들여}↩ 이어받기 명령을 못 낸다 — ${r.왜}`
    : `${들여}↩ 이어받기: ${r.명령}  ${r.꼬리}`;
}
...
module.exports = {
  ...
  살아있나, 경과분, 무응답분, 상태이름, 판정, 요약, 처분, 자식인자, 던지기플래그, 멈춤분,
  이어받기표, 아는종류, 이어받기, 이어받기줄,
  HEAD움직였나, 중단,
