 ...
 *  실측 F509: `codex-review.js` 는 여기를 `코드===0 ? '완주' : '실패'` 로 갈랐는데
 *  `마누스.js` 는 세 종료 경로(stopped·waiting·상한초과)를 **전부 '완주'** 로 적었다.
 *  그래서 종료코드 3(폴러 상한 초과 · 결과 0바이트)이 「✅ 완주」로 장부에 앉았다 —
 *  지침 신뢰성 조항 「같은 판정을 두 곳에 적으면 갈라진다」 그 자리다. */
function 상태이름(종료코드) { return Number(종료코드) === 0 ? '완주' : '실패'; }
...
function 판정(런, now) {
  ...
  if (런.상태 === '완주' || 런.상태 === '실패') {
    ...
    /* 🔑 실패를 완주와 **같은 바구니에 넣지 않는다.** 부르는 쪽이 이 한 칸만 보고 말을 짓기
     * 때문에, 뭉쳐 두면 실패가 「✅ 결과가 이미 나와 있다」로 나간다(실측 F509: 사람이 그 말을
     * 믿고 44KB 로그를 열었더니 404 가 167줄이었다). 갈래가 갈려야 다른 말을 할 수 있다.
     *
     * 🔑 그리고 **쓰는 쪽을 믿지 않는다.** `상태` 와 `종료코드` 가 어긋나면 비관 쪽으로 읽는다 —
     *   F509 의 그 런은 `상태:'완주'` + `종료코드:3`(결과 0바이트)이었다. 여기서 두 칸을 함께
     *   보면 ①이미 장부에 앉은 옛 기록이 고쳐 읽히고 ②앞으로 어느 쓰는 쪽이 '완주'를 잘못
     *   적어도 안 샌다. 호출부마다 고치는 대신 **잘못 쓸 수 없는 판독**으로 만든 자리다.
     * ⚠ 종료코드가 아예 없는 옛 런은 그대로 '완주'로 둔다(없음을 실패로 접으면 거짓 적색이 된다). */
    const 실패 = 런.상태 === '실패'
      || (런.종료코드 != null && 상태이름(런.종료코드) === '실패');
    return { 갈래: 실패 ? '실패미처분' : '완주미처분', 분 };
  }
...
function 요약() {
  ...
  return {
     ...
     * 읽는 곳 셋을 같은 커밋에서 함께 고쳤다: `review-runs` 훅 · `codex-review --런목록` · `마누스 --런목록`. */
...
module.exports = {
  ...
  살아있나, 경과분, 상태이름, 판정, 요약, 처분, 자식인자, 던지기플래그, 멈춤분,
  HEAD움직였나, 중단,
