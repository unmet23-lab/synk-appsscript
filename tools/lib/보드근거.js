...
/**
 * 장부 모듈에게 **직접 물어서** 열린 번호를 뽑는다 — 순수 함수라 픽스처가 탐지력을 진다(F614).
 *
 * 🔑 판정은 여기서 다시 안 적는다. `friction.js` 머리말이 못박은 그대로 `열렸나` 를 그냥 쓴다
 *   (「소비자는 자기 정규식을 쓰지 않고 이것을 쓴다」) — rot-check·friction-close-guard 가
 *   이미 따르는 규약이고, 이 자리만 우회하고 있었다.
 */
function 열림뽑기(장부) {
  const { rows } = 장부.read();
  return rows.filter(장부.열렸나).map((r) => r.id);
}

/**
 * 열린 장부 번호들 — 못 부르면 `null`(「열린 게 없다」와 다르다).
 *
 * ■ 왜 자식 프로세스 + 정규식을 버렸나 (F614 · 2026-08-18 실측)
 *   옛 판은 `node tools/friction.js --open` 을 돌린 뒤 그 **stdout 에서** `\bF\d{2,4}\b` 를 전부
 *   긁었다. 그런데 그 출력은 신고문 전문을 싣고, 이 저장소의 신고문은 서로를 상시 인용한다
 *   (F604 하나가 F400·F435·F436·F493·F562·F543 을 든다). 그래서 **인용이 열림으로 둔갑했다** —
 *   실측 08-18: 긁힘 49 · 진짜 열림 14 · **헛것 35** · 놓침 0. 세 배 넘게 부푼 목록이
 *   「열린 장부」라는 이름을 달고 사람 앞에 놓였다.
 *   급소는 부풀기 자체가 아니라 **판정 입력이 「사람에게 읽어 줄 글」에 매달린 것**이다:
 *   안내문에 근거 표기를 한 줄 더하면 그날로 값이 조용히 바뀐다(F610 수리가 실제로 그랬다).
 *
 * ■ 대가 — 틀릴 때의 모습 (CLAUDE.md 신뢰성 ④)
 *   ① `root` 가 이 저장소가 아니면 `require` 가 죽어 `null` 이다 — 즉 새는 방향은 「미측정」이고,
 *      부르는 쪽은 그것을 「0건」과 이미 가른다(⑤ 갈래가 `null` 을 따로 처방한다 · F207).
 *   ② `read()` 는 장부 전문을 읽으므로 자식 프로세스 하나가 사라진 대신 이 프로세스가 파일을 읽는다.
 *      `board-move` 거절문 한 번에만 도는 자리라(훅 아님) 벽시계에 안 얹힌다.
 *   ▶ 닫을 것 1개: `execFileSync` 로 `friction.js` 를 부르는 통로가 이 파일에서 **사라졌고**,
 *      회귀가 그 옛 통로의 재등장을 막는다(`tests/보드근거.test.js`).
 */
function 열림장부(root) {
  try {
    return 열림뽑기(require(path.join(root, 'tools', 'friction.js')));
  } catch (_) { return null; }   // 못 봄 ≠ 0건 (F207)
}
...
module.exports = { 근거후보, 근거글, 경로뽑기, 번호뽑기, 해시뽑기, F번호뽑기, 열림장부, 열림뽑기, 기본git };
