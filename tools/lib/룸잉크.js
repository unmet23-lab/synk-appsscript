...
/** 중첩 중괄호와 한 줄 압축을 모두 견디며 `@media print { ... }` 블록을 걷는다. */
function 인쇄미디어걷기(css) {
  const 원문 = String(css);
  // 쉼표가 있으면 screen 등과 공유하는 미디어 목록일 수 있다. print 전용 블록만 걷는다.
  const 시작꼴 = /@media\s+print\b[^\{,]*\{/gi;
  let 결과 = '', 커서 = 0, m;
  while ((m = 시작꼴.exec(원문))) {
    결과 += 원문.slice(커서, m.index);
    let 깊이 = 1, i = 시작꼴.lastIndex;
    while (i < 원문.length && 깊이 > 0) {
      if (원문[i] === '{') 깊이++;
      else if (원문[i] === '}') 깊이--;
      i++;
    }
    // 깨진 CSS면 나머지를 임의로 버리지 않는다. 기존 보수적 판정으로 돌아간다.
    if (깊이 > 0) return 결과 + 원문.slice(m.index);
    커서 = i;
    시작꼴.lastIndex = i;
  }
  return 결과 + 원문.slice(커서);
}
...
module.exports = { 문턱, 조각맞나, 빈태그, 색파싱, 변수표, 색풀기, 얹기색, 대비, 부품바탕, 허용표식, 껍질벗기기, 인쇄미디어걷기, 잉크부품들, 특정도, 더센가, 부품단요소들, 끝이닿나, 색값, 충돌들, 허용됐나, 남의글자재도색 };
