...
const 구움표 = [
  ...
  { 부품: ['머리나침반', '머리졸업모', '머리지구본', '머리여권', '머리계단', '머리공책', '머리편지', '머리시계', '머리지도', '머리책', '머리열쇠', '절머리'],
    수요: true, 자산: '공방_나침반',
    또: ['공방_졸업모', '공방_펠트지구본', '공방_펠트여권비자', '공방_펠트계단성장',
        '공방_공책과연필', '공방_편지봉투', '공방_탁상시계', '공방_지도', '공방_펠트책한권', '공방_열쇠'],
    css: (P, A) => `
  /* ── 절 머리 그림 열 · 09-08 (유호 지시 「소개서·발표물에 넣어주고 새로 만든거 활용 제대로」) ──
     🔴 왜 이제야 서나 — 09-08 실측: 구운 이름 1,028가지 중 지면이 부르는 것이 «약 10»이었다.
        「큰 그림 2차」 묶음은 계획서에 쓰는 곳이 «발표물 표지 · 소개서 절 머리»라 적혀 있는데
        한 자리도 안 실려 있었다. 구워 놓고 안 쓴 자리가 정확히 여기다.
     🔑 고르는 자 = «절과 짝이 맞나». 예쁜 것이 아니라 그 절이 말하는 것을 그림이 대신
        말해 주는 것만 골랐다 — 나침반은 나침반 3문항, 졸업모는 TOPIK 4급, 여권은 유학생.
        짝 없는 그림은 장식이고 장식은 지면 무게만 늘린다.
     ⚠ 수요:true 라 «그 이름표를 쓴 지면»에만 실린다. 열 장을 다 실으면 한 지면이 72KB 늘고,
        09-01 에 그렇게 불어난 지면을 브라우저가 열기를 거부한 적이 있다.
     🔴 단위가 em 이면 안 된다 — 절 제목의 글자 크기가 지면마다 8.6mm·12mm·9.6mm 로 제각각이라
        2.6em 이 22~31mm 까지 튀었다(09-08 실측). A4 절 머리에 3cm 짜리 그림은 제목을 잡아먹는다.
        그래서 «뿌리 글자»에 매단다 — 두 지면 다 html/body 에 font-size 가 없어 뿌리가 16px 기본이라,
        rem 은 지면이 달라도 같은 크기를 낸다(인쇄 약 12mm · 화면 45화소).
     ⚠ 이 주석에 백틱을 쓰지 않는다 — 여기는 템플릿 리터럴 «안»이라 하나로 CSS 가 통째로 끊긴다
        (09-08 에 밟았다 · 이 파일이 아래 「태그」 절에 같은 경고를 이미 적어 두었는데도).
     ⚠ 이보다 작게 내리지 않는다. 1.62rem(체크·별 자리)에서는 지구본·나침반처럼 «안이 복잡한»
        그림이 얼룩으로 읽힌다 — 작은 자리는 애초에 단순한 형태의 부품이 맡는다. */
  /* 🔴 절 머리에 그림이 서면 «절 번호 단추»는 내린다 (유호 지적 09-08 「둘 다 있으니까 가시성이
     떨어져 · 하나만 남겨」). 둘 다 두면 동그란 펠트 물건이 둘 나란히 서서 어느 쪽도 안 읽힌다.
     번호를 버려도 되는 까닭 = 그 지면의 «차례»가 순서를 이미 쥔다. 남기는 쪽을 그림으로 고른 것은
     번호가 어느 절에서나 같은 말(01·02·03)만 하는 데 견줘, 그림은 그 절이 무엇인지 말하기 때문이다.
     ⚠ counter 자체는 살려 둔다 — 잔번호·각주가 같은 counter 를 쓴다. 내리는 것은 «보이는 것»뿐이다. */
  h2:has(.절머리)::before{display:none;}
  .머리나침반,.머리졸업모,.머리지구본,.머리여권,.머리계단,.머리공책,.머리편지,.머리시계,.머리지도,.머리책,.머리열쇠{
    width:2.8rem;height:2.8rem;border-radius:0;box-shadow:none;
    background-color:transparent;vertical-align:-.7rem;display:inline-block;
    margin-right:.5rem;}
  .머리나침반{${얹기(P['공방_나침반'])}}
  .머리졸업모{${얹기(A['공방_졸업모'])}}
  .머리지구본{${얹기(A['공방_펠트지구본'])}}
...
function 엔진전시() {
  ...
  @media print{body.엔진전시{--chalk:${c.Ink};--ash:${c['Deep Wool']};--ink:${c.Ink};background:${c.Paper}!important;color:${c.Ink}!important}.엔진전시 .판{display:block;padding:0;max-width:none}.엔진전시 .레일{display:none}.엔진전시 .표지{min-height:0;padding:0 0 var(--켜);break-after:page}.엔진전시 .표지 .오브{max-width:280px;filter:none}.엔진전시 h1,.엔진전시 p,.엔진전시 strong,.엔진전시 .꼭지,.엔진전시 .메타,.엔진전시 .흐름>li,.엔진전시 .번호{color:${c.Ink}!important}.엔진전시 h2{margin-top:var(--켜)}.엔진전시 .유리{background:${c.Paper};color:${c.Ink}!important}.엔진전시 footer{display:block}}
  `;
}
