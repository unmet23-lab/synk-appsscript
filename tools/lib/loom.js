...
function 구움층(자산) {
  ...
  /* 유리 카드 — 크기가 지면마다 달라 통짜 렌더로는 못 쓴다. 직교로 구운 판을 9분할로 늘린다.
     ⚠몸(안쪽)은 늘리면 굴절 결이 늘어지므로 «테두리만» 가져오고 안쪽은 CSS 반투명이 남는다.
       그래서 이 부품은 «반쯤 구운» 상태다 — 분모가 그렇게 센다. */
...
function 분모() {
  ...
  const 자산 = 구운재질();
  const 구운부품 = 자산 ? Object.keys(자산['부품']) : [];
  ...
  const 재질축 = 목록.filter((p) => p.있나).map((p) => {
    ...
    return { 이름: p['이름'], 상태: 구운부품.includes(필요) ? '구움' : '흉내', 자산: 필요 };
  });
