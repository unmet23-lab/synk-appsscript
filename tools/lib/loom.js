...
/* ── 기본 천 — 지정 없는 펠트가 입는 염료 ──────────────────────────────────
   ...
   물러선다. 이 함수의 사진 패치 사용은 해당 CSS 통로의 구현이며 다른 생성·렌더 방식의 금지가 아니다.
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   왜 이 층이 따로 있나:
     이 CSS 통로는 구운 자산의 섬유·빛을 지면에 재사용한다. 기존 렌더는 `tools/룸굽기.js`가
     담당하며 새 생성·실시간 방식은 현재 DESIGN과 Loom 표현 기준에서 선택한다.
     과거 CSS 재질 실험의 실패를 사진 픽셀·Blender만 허용하는 전역 제한으로 쓰지 않는다.
