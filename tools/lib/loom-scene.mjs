...
function advance(state, dt, pointer) {
  ...
  if (t >= state.nextLeaf) {
    state.leafStart = t;
    state.nextLeaf = t + c.leafMin + random(state) * (c.leafMax - c.leafMin);
    // The same gust that carries the leaf reaches every prop in this scene.
    state.gustStart = t;
    state.gustDirection = 1;
  }
  const gustAge = t - state.gustStart;
  const gust = gustAge >= 0 && gustAge < 4
    ? Math.sin(Math.PI * gustAge / 4) ** 2 * c.gustStrength * state.gustDirection : 0;
