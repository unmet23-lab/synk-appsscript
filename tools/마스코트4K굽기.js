...
async function main() {
  ...
  /* 🔴 [09-08 배포 검수 P2] **거른 뒤에 또 거른다 — `판들` 로 돌아가지 않는다.**
   *   옛 판은 뒤의 두 거름망이 전체 목록에서 다시 시작해서, `--누구 마린 --이름 좌34,우34` 를 주면
   *   «마린만» 이라는 제한이 조용히 사라졌다(재현에서 고른 여섯 중 넷이 부른 적 없는 캐릭터였다).
   *   이 도구는 **돈이 나가는 굽기**로 이어지고 `--다시` 까지 있으면 남의 후보를 덮어쓴다. */
  if (하나) {
    const 집은것 = 판들[하나 - 1];
    if (집은것 && 대상.indexOf(집은것) < 0) {
      console.error(`--판 ${하나} 는 ${집은것.누구}/${집은것.이름} 인데 --누구 ${누구} 로 좁혀 놓았다 — 어느 쪽인지 몰라 안 굽는다`);
      process.exit(1);
    }
    대상 = 집은것 ? [집은것] : [];
  }
  if (이름들) 대상 = 대상.filter((p) => 이름들.some((n) => p.이름.includes(n)));
