...
/* ⏱ 한 요청 6초 · 전체 25초. 둘 다 필요하다 — 세션 시작 훅의 상한이 45초인데
 *   요청이 아홉이라 «한 요청 상한»만 두면 6×9=54초로 그 상한을 넘는다(검수 P2 666effccb34d).
 *   예산이 끝나면 남은 것은 «못 쟀다»로 남고, 그러면 잰 때를 안 적어 다음 세션이 다시 잰다. */
const 한요청밀리 = 6000;
const 전체예산밀리 = 25000;
...
async function 상류마지막커밋(저장소, 경로) {
  ...
  const 답 = await fetch(주소, {
    ...
    signal: AbortSignal.timeout(한요청밀리),
  });
...
/* 훅이 «무엇을 말할까»를 정하는 순수 함수 — 네트워크 없이 시험할 수 있게 갈라 둔다.
 * 🔴 여기 담긴 판정 셋이 전부 「거짓 초록」을 막는 자리다(이종 검수 09-03):
 *   ① 전부 못 쟀으면 «확인 불가»라고 «말한다» — 조용히 나가면 「변경 없음」과 구분이 사라진다.
 *   ② 하나라도 못 쟀으면 잰 때를 안 적는다 — 적으면 7일 동안 그 스킬을 안 본다.
 *   ③ 「못 읽은 줄」은 이동 경보와 «따로» 낸다 — 한쪽이 있으면 다른 쪽이 묻히던 자리. */
function 훅판정(결과, 못읽은줄) {
  const 못잰것 = 결과.filter((r) => r.못쟀다);
  const 움직인것 = 결과.filter((r) => r.움직였나);
  const 처음본것 = 결과.filter((r) => r.처음);
  const 줄 = [];

  if (결과.length && 못잰것.length === 결과.length) {
    줄.push(`⬜ 스킬 판 점검을 **못 쟀다** — ${결과.length}벌 전부 상류에 못 물었다(${못잰것[0].못쟀다}).`);
    줄.push('   이건 「밖에서 들여온 스킬이 다 최신」이 아니라 «확인 불가»다. 다음 세션이 다시 잰다.');
  } else {
    if (움직인것.length) {
    ...
    if (못잰것.length) {
      줄.push(`⬜ 그중 ${못잰것.length}벌은 **못 쟀다**(${못잰것.map((r) => r.이름).join(' · ')}) — 「그대로」가 아니다.`);
    }
  }

  // ③ 이동 경보와 «독립»으로 낸다 — 위 갈래 어디로 갔든 이 사실은 따로 말해야 한다
  if (못읽은줄 && 못읽은줄.length) {
    줄.push(`⚠ 표에서 «세지 못한» 줄 ${못읽은줄.length}개(${못읽은줄.join(' · ')}) — 그만큼 위 숫자는 덜 센 값이다.`);
  }

  return { 줄들: 줄, 다쟀나: 결과.length > 0 && 못잰것.length === 0 };
}
...
async function 재기() {
  ...
  const 마감 = Date.now() + 전체예산밀리;
  for (const 것 of 항목) {
    const 적힌것 = 장부.스킬[것.이름] || {};
    if (Date.now() > 마감) { 결과.push({ ...것, 못쟀다: '전체 시간 예산 초과' }); continue; }
    try {
...
async function 본체() {
  ...
  if (오류) {
    /* 훅에서도 «말한다». 조용히 나가면 「표가 통째로 없다」와 「다 최신이다」가 같은 얼굴이 된다
     * — 표를 지우거나 옮기는 순간 이 감시가 영원히 초록으로 죽는 자리다. 막지는 않는다. */
    console.error(`⬜ 스킬 판 점검을 **못 쟀다** — ${오류}. 이건 「다 최신」이 아니라 «확인 불가»다.`);
  ...
  if (도장) {
    도장찍기(결과, 장부);
    // 도장도 같은 규칙이다 — 못 잰 것이 하나라도 있으면 「쟀다」로 치지 않는다(검수 P1 1608648d940f)
    if (!못잰것.length) 잰때쓰기();
    console.log(`✅ 지금 상류를 기준선으로 적었다 — ${결과.length - 못잰것.length}벌 (못 잰 것 ${못잰것.length})`);
    if (못잰것.length) {
      console.log(`   ⬜ 못 잰 ${못잰것.length}벌(${못잰것.map((r) => r.이름).join(' · ')})은 **기준선을 안 남겼다** — 다음 세션이 다시 잰다.`);
    }
    return;
  }

  if (훅모드) {
    const { 줄들, 다쟀나 } = 훅판정(결과, 못읽은줄);
    if (줄들.length) console.log(줄들.join('\n'));
    /* 🔴 **다 쟀을 때만** 잰 때를 적는다(검수 P1 1608648d940f). 하나라도 못 쟀는데 적으면
     *   그 스킬은 7일 동안 안 보인다 — 「못 쟀다」가 「그대로」로 굳는 자리다.
...
module.exports = { 표읽기, 훅판정, 지난날, 장부읽기, 잰때읽기, 잰때쓰기, 표경로, 장부경로, 잰때경로, 조용한날, 한요청밀리, 전체예산밀리 };
