...
/* ■ 클로드 — 이 저장소를 실제로 짓는 층(유호 확정 2026-08-10 「하이쿠였던걸 opus로 바꾸면 안되나」).
 *
 * 옛 3층(코드=Opus · 콘텐츠=Sonnet · 기계적 변환=Haiku)에서 **등급 하향을 비용 근거로 쓰는 축**을 걷어냈다.
 * 그 축은 이 파일이 이미 한 번 기각한 것이다 — 구조화(2단계)에서 「변환이라 얕은 쪽이 옳다」는 논증이
 * 반대 근거로 제시됐고 판정은 `max` 였다(:222 「🚫 low 로 되돌리는 재제안 금지」). 같은 논증을
 * 클로드 쪽에서 다시 세우지 않는다.
 *
 * ⚠ **비용을 근거로 쓰지 않는다** — 두 번 데였다.
 *   ① `token-audit.js` 의 가중치는 **캐시 계층 배수**지 모델 단가가 아니다. 그 분해(재읽기 50.4% 등)는
 *      등급에 대해 스케일 불변이라 등급 판정을 지지도 반박도 못 한다. 등급 축을 재는 도구는 아직 없다.
 *   ② 구독이면 단가가 아니라 **한도 소진 속도**가 비용인데 그 값을 모른다(유호님 답 대기).
 *   그래서 아래 값들은 비용이 아니라 **되돌림 비용**으로 정당화한다 — 틀리면 얼마나 비싼가.
 *
 * ⚠ 위임 오판의 원인은 등급이 아니다(F305 · 2026-08-10): 대조군 없는 프로브였고 **같은 프로브엔
 *   Opus 도 같이 속는다**(실제로 그 답을 받을 뻔했다). 처방은 등급이 아니라 짝이다 —
 *   「~인가/~하나」를 위임할 때 **막히는 것과 안 막히는 것을 한 벌로** 준다. 등급으로 갈음하지 않는다.
 *
 * 🚫 옛 「기계적 변환·분류=Haiku급」 부활 금지 — 근거였던 비용 산술이 위 ①로 무효다. */
const 클로드 = {
  기본: { model: 'claude-opus-5', effort: 'xhigh', 설명: '대화 밖 전부 — 설계·판정·검증·보안·콘텐츠·위임' },
  짓기: { model: 'claude-fable-5', effort: 'high', 설명: '명세가 이미 선 것을 처음부터 구현(.claude/agents/builder.md)' },
};
const 클로드기본 = '기본';

/* 대화 모델은 여기서 정하지 않는다 — 유호님만 바꾼다(CLAUDE.md). 이 표는 **위임·자동 호출** 몫이다. */

function 클로드설정(이름) {
  const p = 클로드[이름 || 클로드기본];
  if (!p) throw new Error(`모르는 클로드 픽: ${이름} (있는 것: ${Object.keys(클로드).join(', ')})`);
  return p;
}

function 출력() {
  const 분석 = 검수선택(검수기본);
  console.log('■ 클로드 — 이 저장소를 짓는 층(위임·자동 호출 몫 · 대화 모델은 유호님만 바꾼다)');
  for (const [k, v] of Object.entries(클로드)) {
    console.log(`  ${k === 클로드기본 ? '▶' : ' '} ${k.padEnd(5)} ${v.model} / ${v.effort}   ${v.설명}`);
  }
  console.log('  🚫 등급 하향을 비용 근거로 쓰지 않는다 — token-audit 은 캐시 계층 배수라 등급 축을 못 잰다');
  console.log('\n■ 코덱스(GPT) — 이종 적대 검수');
