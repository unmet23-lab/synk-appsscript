...
const 검수선택지 = {
  sol: {
    model: 'gpt-5.6-sol',
    effort: 'max',
    설명: '프런티어 — 되돌림 비용이 큰 변경(기본값)',
  },
  luna: {
    model: 'gpt-5.6-luna',
    effort: 'max',
    설명: '빠르고 싼 쪽 — 급할 때. ultra 는 지원 안 함',
  },
};
const 검수기본 = 'sol';
...
/* 2단계 = 산문 → JSON. **판단이 아니라 변환**이므로 최하급 + 가장 얕게가 옳다.
 *   똑똑할수록 「고쳐서」 옮긴다(STT 가 학생 발음 오류를 고쳐 전사해 데이터를 없애는 것과 같은 계열).
 ...
 *   같은 luna 가 1단계에선 max, 여기선 low 인 것이 이 파일의 요지다 — **효력이 레버다.** */
const 구조화설정 = {
