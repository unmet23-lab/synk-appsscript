 ...
 *   플래시의 «버전»은 유호님이 고르신다. 현행 = `gemini-3.7-flash` — **유호 지시 2026-08-31
 *   「제미나이는 3.7을 유지해줘」.** 그 전 픽은 3.6 이었다(2026-08-05 라이브 목록 실측).
 *   ⚠ 08-29 evals 실측(`evals/몽골어검문.yaml` · 14문항)은 3.6/3.7 을 **13/14 동점**으로 냈고
 *   틀리는 «방향»만 갈렸다 — 3.7 은 놓침(실제 결함을 통과), 3.6 은 헛경보. 그래서 3.7 로 가는 동안
 *   **놓침이 감시 대상**이다: 검문이 「이상 없음」을 내면 그게 진짜인지 한 번 더 본다.
 *   🔴 **가용성이 진짜 위험이다** — 그날 3.7 은 22호출 중 **503 이 9번**(3.6 은 13호출 중 0번),
 *   14칸을 채우는 데 실행이 3회 필요했다(3.6 은 1회 · `evals/README.md` 「곁으로 잰 것」).
...
const 제미나이 = {
  ...
  무료최상: { model: 'gemini-3.7-flash', thinking_level: 'high', 무료등급: true },
  기본: '무료최상',
  폴백허용: false,
};
