...
function 파이썬(인자들) {
  ...
  return { ok: r.status === 0, 글: `${r.stdout || ''}${r.stderr || ''}`.trim() };
}
...
(async () => {
  ...
  for (let i = 0; i < 할것.length; i++) {
    ...
    if (!걷기.ok) { 말(`🔴 ${x.이름} — 흰 배경을 못 걷었다: ${걷기.글.slice(-160)}`); 실패++; continue; }
    const 맞춤 = 파이썬(['tools/옷자리맞추기.py', x.누끼, x.씌운, x.얹음,
      '--몸', path.join(루트, x.마스코트.참조), '--눈', x.마스코트.눈, '--층', x.층]);
    if (!맞춤.ok) { 말(`🔴 ${x.이름} — 자리를 못 맞췄다: ${맞춤.글.slice(-160)}`); 실패++; continue; }

    됨++;
    말(`✅ ${머리} — 조각까지 났다 (누적 ${(됨 * 2 * 장당).toLocaleString()}원)`);
