...
def 화면단추(부, cx, cz, 깊이, 꿰맴, 지금=False, r=0.085, 실뜻=None):
    ...
    if 꿰맴:
        ...
        재 = 부['지금'] if 지금 else (매끈재질('실뜻' + 실뜻, 뜻색(실뜻), 거칠기=0.78) if 실뜻 else 부['실'])
...
def 우리아이판():
    ...
    for 행, 주 in enumerate(출석):
        for 열, 상태 in enumerate(주):
            if 상태 == 0:
                continue
            # ⚠**작은 실에 Soft 단을 넣어 봤고 실패했다**(08-23 시안) — Meadow Soft(#D3E8BE)가
            #   실 두 가닥에서는 «거의 흰색»으로 씻겼다. X 가 또렷해지긴 했으나 «성장»이라는
            #   뜻은 전달되지 않는다. 이 조명에서 밝은 색은 흰색이 된다(오늘 네 번째 자리).
            화면단추(부, 첫열 + 열간격 * 열, 첫행 - 행간격 * 행, 달력깊이, 꿰맴=(상태 == 2))
    ...
    칸들['범례1'] = 화면단추(부, -0.02, 범례z, 달력깊이, 꿰맴=True)
...
def 검수판():
    ...
    #   🔑 **판정 셋은 뜻이 셋이라 색이 셋이다** — 시맨틱이 이미 그 셋을 갖고 있다
    #      (정답 Meadow Deep · 안내 Lapis Deep · 신호 Coral 3). 「고칠 데가 있다」가 «안내»인 까닭은
    #      그것이 틀림이 아니라 **다음 손이 갈 자리를 가리키는 것**이기 때문이다.
    #   ⚠셋 다 어둡다(0.064~0.118) — 코랄 신호(0.308)를 안 넘으므로 4계 ②가 안 깨진다.
    알천 = ['정답_면', '안내_면', None]     # None = 무채(원문이 맞다 = 아무 일도 안 일어난다)
