...
def 구분선():
    ...
    🔑 **실은 `Stitch`(#FBCAAB)** — 킷의 실땀 정본이고 크로마 0.157 로 섬유 문턱(0.16) «안»이라
...
def 김치():
    ...
    # ⚠12판: Stitch(#FBCAAB)로 뽑은 잎맥은 붉은 잎 위에서 **흰 실**로 읽혔다 — 「바느질한 천」이라는
...
def _태그천():
    ...
    🔴 「천색=」·「실색=」 (09-03 · 유호 물음 「왜 가독성이 약간 떨어져보이는 느낌이지?」)
      실측이 «느낌»을 구조로 바꿨다: 밝은 Oat 천(밝기 174.7) 위 어두운 글자는 대비가 7.41:1 인데
      같은 지면의 본문은 15.08:1 이다 — 절반이다. 게다가 **상한이 9.4:1** 이라(글자를 완전한
      검정으로 해도) 밝은 바탕에서는 본문 수준에 «구조적으로» 못 간다.
      곁: 펠트 결이 ±12 로 출렁여(밝기 126~203) 작은 글자 획이 그 무늬에 묻힌다.
      ⇒ 방향을 뒤집는 갈래를 연다: **어두운 천 + 밝은 글자**. 지면이 어두우니 대비 방향이
        본문과 같아지고 15:1 이 가능해진다. 어느 쪽이 나은지는 문장 옆에서 눈이 고른다."""
    천 = 직물결(매끈재질('라벨천', 색[인자.get('천색', 'Oat')], 거칠기=0.88), 지름=1.90)
    실 = 매끈재질('실', 색[인자.get('실색', 'Deep Wool')], 거칠기=0.72)
    return 천, 실
...
def _태그띠땀(실재, x중심=0.0):
    ...
    🔴 「땀안쪽=」 (09-03 · 유호 지적 「공간이 작아서 그런가 가시성이 좀 떨어지고 보기가 불편한데?」)
      옛 0.14 는 땀을 띠 반높의 **70%** 자리에 놓아, 글자가 앉을 자리를 위아래로 파먹었다.
      실측: 24px 칩에서 띠는 18.5px 이고 땀이 위아래 각 ~2px → 글자 자리 14px 에 글자 12.5px.
      값을 줄이면 땀이 가장자리로 밀려 가운데가 넓어진다(0.08 = 83% · 0.04 = 91%).
      ⚠너무 밀면 땀이 띠 경계에 걸려 «잘린» 것처럼 보인다 — 어디까지가 경계인지는 구워야 안다."""
    안쪽 = float(인자.get('땀안쪽', '0.14'))
    for sz in (_태그높 - 안쪽, -(_태그높 - 안쪽)):
        for i in range(5):
...
def 태그중():
    ...
    ⚠가로로 **반복**되는 유일한 조각이다. 좌우 끝이 곧아야 하므로 마구리와 같은 수를 쓴다 —
      몸을 창(±0.60)보다 넓게 두어 둥근 끝이 프레임 밖에 떨어지게 한다.
      크리스는 마구리와 **같은 0.62** 다: 위·아래 결이 다르면 이어 붙인 자리에 턱이 진다.

    🔴 **0.92 로는 모자랐다**(09-03 실측 · 「몸폭=」 을 그래서 열었다).
      알파는 좌우 끝까지 꽉 찼는데(열별로 재보니 769px 균일) 화면에서는 «알약 여러 개»로 보였다.
      까닭은 좌우 «변»이 아니라 **위아래 모서리 라운드**다: 크리스 0.62 의 서브서프가 만드는
      라운드가 폭 0.92 에서는 창(±0.60) «안»까지 들어와, 반복할 때 그 코너가 주기적으로 나타나
      알약 경계처럼 읽힌다. 마구리를 빼도·해상도를 올려도·위아래 여백을 잘라도 같았다.
      ⇒ 몸을 훨씬 넓게 두어 코너를 확실히 프레임 밖으로 보낸다. 얼마가 충분한지는 구워야 안다."""
    천, 실 = _태그천()
    몸 = 베개몸((float(인자.get('몸폭', '0.92')), 0.055, _태그높), 크리스=0.62)
    몸.data.materials.append(천)
    # 「띠땀=없음」 — 실땀을 아예 뺀 «맨 펠트 띠»(09-03 시안 갈래). 글자만 얹는 안의 바탕이 된다.
    if 인자.get('띠땀', '있음') != '없음':
        _태그띠땀(실)
    return (0, -_태그거리, 0.0), 90
...
def 와펜숫자():
    ...
    판 = bpy.context.object
    # 🔴 「납작=」 은 **y** 다(09-03 · 처음에 z 에 걸었던 것을 바로잡았다 — 이름과 뜻이 어긋나 있었다).
    #   z 는 «세로 지름»이라 키우면 그냥 커지고, 패치가 납작한 까닭은 y 가 0.3 이어서다.
    #   실측이 이 손잡이를 필요하게 만들었다: 털을 폼폼과 «같은» 0.32 까지 늘려도 밝은 쪽이
    #   160.1 에 그쳐 폼폼(147.2)까지 12.9 가 남았다 ⇒ 남은 차이는 털이 아니라 **납작함**이다.
    #   납작한 원판은 정면 면적이 커서 빛을 통째로 받고, 구는 가장자리로 빛이 흘러 떨어진다.
    판.scale = (1.05, float(인자.get('납작', '0.3')), 1.05)
    bpy.ops.object.shade_smooth()
    ...
    for i in range(18):
        ...
        땀하나(r * math.cos(a), -0.21, r * math.sin(a), 0.13, 0.03, 실재,
              회전y=-math.degrees(a) - 90)
    # 🔴 「숫자크기=」 (09-03 · 유호 물음 「숫자를 조금 키우는건 어떻게 생각해?」)
    #   지면에서 이 부품은 44px 로 앉는데 숫자가 겉보기 지름의 27% 라 «11px 남짓»이다 —
    #   본문 글자(17px)보다 작다. 지면당 한 점뿐인 «눈이 되는 자리»인데 두 자리 수가 빡빡하다.
    #   자수는 클수록 안전하기도 하다: 글자를 복셀(0.018)로 다시 짜서 털을 심으므로 작으면
    #   그 과정이 가는 획을 먹는다.
    #   ⚠한계 = 아래 테두리 땀이 도는 자리(r=0.78)를 숫자가 넘으면 겹친다. 어디까지가 경계인지는
    #     재서 못 알고 구워야 안다(색·미학 채택은 숫자로 안 한다 · 08-24 확정).
    #   ⚠테두리도 «같은 크기»를 받아야 한다 — 따로 두면 글자와 땀이 어긋난다.
    _글크 = float(인자.get('숫자크기', '0.62'))
    # 🔴 [09-08 검수 P2 e795ea22a5fa] 자수는 «앞면»에 붙어야 한다 — 앞면은 「납작=」 이 옮긴다.
    #   앞 판은 앞면만 옮기고 자수는 y=-0.34·-0.44 에 박아 두어서, 납작=0.60 으로 구우면
    #   숫자와 테두리가 몸(과 털) «안»에 묻혀 **숫자 없는 패치**가 나왔다.
    #   ⇒ 앞면(-납작)에서 같은 거리만큼 띄운다. 기본값 0.3 에서는 -0.34·-0.44 그대로다(무변경).
    #   ⚠ 굽지 않고는 못 잰다 — 여기서 아는 것은 «기본값에서 값이 같다»까지다.
    _앞 = -float(인자.get('납작', '0.3'))
    자수덩이(본문, _글크, (0, _앞 - 0.04, 0.0), 색이름='Oat', 털길이=0.016, 털수=7000)
    자수테두리(본문, _글크, (0.0, 0.0), _앞 - 0.14, 실색='Stitch', 땀길이=0.030, 굵기=0.013)
    return (0, -7.2, 0.0), 90
...
def 폼폼숫자():
    ...
    짧은퍼(몸, 염료이름, 살재질(염료이름, 색[염료이름]), 털재질(염료이름, 색[염료이름]),
          길이=0.32, 개수=9000)
    # 「숫자크기=」 — 와펜숫자()와 같은 손잡이(09-03 · 그쪽 주석이 까닭을 쥔다).
    # ⚠여기 한계는 저쪽과 다르다: 숫자를 방울 «앞»에 띄워 놓은 구조라(y=-0.98) 너무 키우면
    #   방울을 덮어 「방울 위의 수」가 아니라 그냥 «수»가 된다. 그 경계도 구워야 안다.
    # ✅ 기본값 0.80 — 유호 확정 09-03 「오케이 조금으로 가자」+「폼폼으로 가자」.
    #   이 형태가 **지면 히어로 번호의 채택판**이다(레진구를 이 자리에서 걷었다).
    #   왜 0.80 인가: 옛 0.66 은 44px 실크기에서 숫자 높이가 7.3px 로 두 자리 수가 뭉갰다.
    #     0.80 이면 8.5px 로 풀린다. 0.92(10.1px)도 읽히지만 방울 아래 털 몫을 먹어 «뻗침»이 옅어졌다.
    #   왜 와펜이 아닌가: 44px 에서 와펜은 점선 링과 숫자가 둘 다 밝아 «흰 고리 둘»로 경쟁한다.
    #     여기는 밝은 것이 숫자 하나뿐이라 눈이 바로 간다. 와펜은 큰 자리(인쇄물 표지·증서)용으로 남긴다.
    _글크 = float(인자.get('숫자크기', '0.80'))
    # 🔑 「본문=없음」 이면 숫자를 안 얹고 «민방울»이 나온다(09-03 신설 · 단추숫자와 같은 갈래).
    #   왜 필요한가: 지면의 `.히어로수` 는 **숫자를 HTML 글자로 얹고** 구운 것은 그릇만 쓴다.
    #   그래서 숫자가 수놓인 판을 그대로 배선하면 «숫자가 둘»이 된다.
    #   그리고 절번호와 달리 여기 값은 미리 못 정한다 — 「16명 중 17명」류 핵심 수라 지면마다 다르다.
    #   ⇒ 지면 배선은 민방울 + CSS 글자로 간다. 자수 숫자가 꼭 필요한 자리(인쇄물 한 장 · 증서)는
    #     그때 그 값을 「본문=」 으로 낱장 굽는다(손잡이는 열려 있다).
    if 본문 != '없음':
        자수덩이(본문, _글크, (0, -0.98, 0.0), 색이름='Oat', 털길이=0.016, 털수=7000)
        # 테두리도 «글자 윤곽»을 따라가므로 함께 묶는다 — 안 묶으면 「없음」 이라는 글자에 땀을 놓는다.
        자수테두리(본문, _글크, (0.0, 0.0), -1.08, 실색='Stitch', 땀길이=0.032, 굵기=0.014)
    return (0, -7.6, 0.0), 90
