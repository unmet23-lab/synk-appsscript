...
def 직물결(m, 규모=200.0, 세기=0.05):
    ...
    🔴 **규모는 «물체 바운딩 박스» 기준이다**(노이즈가 Generated 좌표를 쓴다) — 세계 단위가 아니다.
       그래서 같은 규모를 작은 물체에 쓰면 결의 파장이 물체 크기에 비례해 짧아지고, 범프의
       «기울기»는 그만큼 가팔라진다. 어느 선을 넘으면 법선이 뒤집혀 면이 통째로 **새까맣게**
       렌더된다(김밥 속 실측 08-22: 지름 0.21 물체에 규모 90 → 화소값 정확히 (0,0,0)).
       ⇒ 어림자: 규모 ÷ 물체 지름 ≲ 250 을 지킨다. 작은 부품은 규모를 «내리거나» 결을 안 준다."""
    nt = m.node_tree
...
def 음식접시(천색='Ink', 반경=1.62):
    ...
    상.data.materials.append(직물결(매끈재질('상', 색['Ink Deep'], 거칠기=0.98), 규모=90.0, 세기=0.09))
    접시천 = 직물결(매끈재질('접시천', 색[천색], 거칠기=0.96), 규모=200.0, 세기=0.045)
...
def 만두():
    ...
    음식접시('Ink')
    도우 = 직물결(분필재질('도우', 색['Stone']), 규모=26.0, 세기=0.10)  # ⚠규모는 물체 크기에 매인다
    ...
    주름재 = 직물결(분필재질('주름', 색['Oat']), 규모=5.0, 세기=0.06)   # 접힌 마루는 «밝다»
...
def 김밥():
    ...
    🔴 ② 작은 부품에 «작은 원기둥 + 베벨 + 스무딩»을 쓰면 뚜껑이 뒤집혀 정확히 (0,0,0)으로
         렌더된다(3·4·5판 화소 실측 — 검은 무대라 «구멍»으로 보였다). 지름 0.23 에 베벨 0.010 은
         28각 캡의 변(0.026)을 양쪽에서 갉아 면을 무너뜨린다. **작은 부품은 구로 만든다** —
         밥알(구)은 같은 장면에서 흰색으로 멀쩡히 나왔다. 이것이 가른 유일한 차이다.
    ...
    for k, (cx, cy, 돌) in enumerate(((-0.74, 0.14, -9), (0.02, -0.20, 6), (0.74, 0.16, 14))):
        통(0.500, 0.42, (cx, cy, 0.090), 김재)              # 김 윗면 0.30
        통(0.395, 0.40, (cx, cy, 0.080), 밥재)              # 밥 윗면 0.28 — 김 테가 0.105 남는다
        for j in range(3):                                  # 속 셋 — 삼각(줄 세우면 신호등이 된다)
            a = math.radians(90 + 120 * j + 돌 * 3)
            bpy.ops.mesh.primitive_uv_sphere_add(radius=0.115, segments=28, ring_count=14,
                location=(cx + 0.165 * math.cos(a), cy + 0.165 * math.sin(a), 0.272))
            속 = bpy.context.object
            속.scale = (1.0, 1.0, 0.42)                      # 납작 구 = 자른 속의 단면
            bpy.ops.object.shade_smooth()
            속.data.materials.append(매끈재질('속%d_%d' % (k, j), 색[속색[j]], 거칠기=0.86))
