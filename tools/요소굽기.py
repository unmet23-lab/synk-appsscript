...
def 초밥():
    ...
    def 밥재질():
        """밥알 — 잔 범프. 지름 0.14 = 알갱이 하나가 밥알만 하다(세기 0.95 = 깊게 파여 그림자가 진다)."""
        return 직물결(매끈재질('샤리', 색['Paper'], 거칠기=0.86), 지름=0.14, 세기=0.95)

    def 샤리(cx=0.0, cy=0.0, 각=0.0, s=1.0, z=0.06):
        """샤리(밥덩이) — 모든 초밥의 밑. 둥근 사각에 «짧고 촘촘하고 세게 뭉친» 털 = 밥알."""
        몸 = 베개몸((0.40 * s, 0.23 * s, 0.125 * s), 위치=(cx, cy + 0.06, z + 0.125 * s),
                  회전z=각, 크리스=0.88, 레벨=4)
        몸.data.materials.append(밥재질())
        return z + 0.250 * s                     # 밥 윗면

    def 네타(색이름, 윗면, cx=0.0, cy=0.0, 각=0.0, s=1.0, 두께=0.040, 거칠기=0.62, 폭=0.44, 높=0.255):
        """네타 — 밥 위에 «덮은» 한 판. 조금 크고 조금 기울어야 «얹힌 것»이 된다."""
        n = 베개몸((폭 * s, 높 * s, 두께 * s), 위치=(cx, cy + 0.06, 윗면 + 두께 * s * 0.92),
                 회전z=각, 크리스=0.92, 레벨=4)
        n.rotation_euler = (math.radians(-6), math.radians(3), math.radians(각))
    ...
    if 종류 in ('연어', '참치', '광어', '새우', '문어', '장어', '계란'):
        ...
        for k, (cx, cy, 각) in enumerate(((-0.34, 0.20, 14), (0.36, -0.16, -22))):
            ...
            if 종류 == '연어':
                ...
                네타('Butter', 윗, cx, cy, 각, s, 두께=0.085, 거칠기=0.78, 폭=0.42, 높=0.24)
        ...
        for k, (cx, cy, 각, s) in enumerate(((-0.34, 0.18, 12, 1.00), (0.36, -0.18, -20, 0.94))):
            ...
            주.data.materials.append(직물결(매끈재질('유부%d' % k, 색['Butter Deep'], 거칠기=0.78),
                                        지름=0.44, 세기=0.85))
            밥 = 베개몸((0.30 * s, 0.185 * s, 0.075 * s), 위치=(cx, cy + 0.06, 0.335 * s),
                     회전z=각, 크리스=0.95)
            밥.data.materials.append(밥재질())
    else:
