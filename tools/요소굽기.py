...
def 강사학생판():
    """강사 ②「학생」 — 한 사람을 보는 자리.
      ...
      ② 칭찬 4태그 — 강2. 🔑**어휘 넷은 개수가 불변이다**(학부모 「크루의 눈」·왕관의 재료라
...
def 검수판():
    ...
    까닭이 하나뿐이다: **Glide 가 폐기되면 강사 판정의 유일한 입구가 죽는다.**
...
#    · 노출=   스톱(+0.5 = 반 단 밝게)
#
# ✅**채택값 = Khronos PBR Neutral + 노출 0.25** (유호 지시 08-24 「제대로 명품화 재굽기」 · A/B 실측).
#   네 벌(AgX 현행·AgX Punchy·PBR 중립·Filmic)을 대표 넷에 걸어 «재서» 골랐다 — 나란히 보고 고르지
#   않았다(08-23 색 오보의 그 배움). 현행 AgX 대비:
#     · 채도 **+55~73%** (별 0.288→0.447 · 삼겹살 0.113→0.191 · 녹음 0.179→0.311)
#     · 대비 **유일하게 오른 변주**(+3.7~29.3% · Punchy 와 Filmic 은 내려갔다)
#     · 밝기는 −12~18% 내려가므로 **노출 +0.25 로 되민다**(밝기 +11~12% 회복 · 채도는 +43% 유지).
#   🔑 이것이 「밝은 색은 전부 흰색이 된다」·「연어핑크로 씻긴다」·**김치 15판**의 진범이었다.
#      재료(hex)를 열다섯 번 바꾼 자리에서 정작 **자(뷰 변환)를 한 번도 안 봤다.**
#   ⚠그래서 색을 조명에 맞춰 손으로 내려 둔 자리들(김치·튀김옷·국물·장어소스…)은 이제 «두 번»
#     보정된 상태다. 다시 구운 뒤 눈으로 훑고, 과보정된 것만 원래 팔레트 값으로 되돌린다.
씬.view_settings.view_transform = 인자.get('색관리', 'Khronos PBR Neutral')
try:
    씬.view_settings.look = 인자.get('룩', 'None')
except TypeError:
    print('모르는 룩:', 인자.get('룩'), '— 룩 없이 간다')
씬.view_settings.exposure = float(인자.get('노출', '0.25'))
