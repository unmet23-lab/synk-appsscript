...
def 검수판():
    ...
    #      셋째를 **보상(Butter Deep)** 으로 준다: 「원문이 맞다」는 학생의 글이 인정받는 순간이고,
    #      엔진에는 «AI 오탐» 이라는 가장 값진 라벨이 된다. 나쁜 일이 아니므로 코랄은 안 쓴다
    #      (코랄을 «면»으로 넓히면 경고로 읽히고, 이 저장소에서 코랄은 «점 하나»의 언어다).
    #   ⚠셋 다 어둡다(정답 0.118 · 안내 0.064 · 보상 0.118) — 코랄 신호(0.308)를 안 넘으므로
    #     4계 ②가 안 깨진다. 다만 정답과 보상은 **휘도가 같다** — 갈리는 것은 밝기가 아니라 색상이라
    #     이 자리는 눈이 아니라 렌더로 확인했다(초록 대 황토는 이 조명에서 뚜렷이 갈린다).
    알천 = ['정답_면', '안내_면', '보상_면']
