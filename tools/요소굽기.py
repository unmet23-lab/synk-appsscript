...
def 매듭점():
    ...
    ⚠불릿은 «반복되는 것»이라 갇힌 빛이 무채다(철칙 ④ — 수십 개가 코랄이면 신호 1점이 죽는다).
      그래서 기본 염료를 안 쓰고 무채(Ash Wool)로 짓는다. 색을 보고 싶으면 「색=」으로 덮는다."""
    실색 = 인자.get('색', 'Ash Wool')
...
def 단추숫자():
    ...
    ⚠자수 숫자는 밝은 무채 실로 얹는다 — 단추 몸(Stone)과 명도가 붙으면 번호가 안 읽힌다."""
    본문 = 인자.get('본문', '01')
    몸재 = 매끈재질('단추', 색['Stone'], 거칠기=0.55)
    구멍재 = 매끈재질('구멍', 색['Deep Wool'], 거칠기=0.9)
    ...
    for dx, dz in ((-0.60, 0.60), (0.60, 0.60), (-0.60, -0.60), (0.60, -0.60)):
        ...
        bpy.ops.object.shade_smooth()
        구멍.data.materials.append(구멍재)
    자수덩이(본문, 0.60, (0, -0.24, 0.0), 색이름='Oat', 털길이=0.016, 털수=7000)
...
if 인자.get('받침'):
    _받침배 = 1.0 if str(인자['받침']) in ('1', '켬', 'on') else float(인자['받침'])
