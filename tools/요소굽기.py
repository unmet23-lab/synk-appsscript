...
def 떡볶이():
    ...
    소스.data.materials.append(직물결(매끈재질('소스', 색['Coral Rim'], 거칠기=0.34), 지름=2.32, 세기=0.35))
...
def 라면():
    ...
    bpy.ops.object.shade_smooth()
    국물.data.materials.append(직물결(매끈재질('국물', 색['Coral Rim'], 거칠기=0.34), 지름=2.36, 세기=0.32))
...
def 김치():
    ...
    맥재 = 매끈재질('김치맥', 색['Coral Rim'], 거칠기=0.60)
...
def 김치찌개():
    ...
    **두부 흰 사각 둘**이 그 표식이다. 국물 색은 같아도 얹힌 것이 다르면 다른 음식이다.
    ...
    손 = random.Random(20260822)
    국그릇(1.10, 'Graphite 4', z=0.10, 손잡이=True)   # 뚝배기 — 손잡이 둘이 이 그릇의 이름표다
    bpy.ops.mesh.primitive_cylinder_add(radius=1.06, depth=0.22, location=(0, 0.06, 0.07), vertices=96)
    국물 = bpy.context.object
    b = 국물.modifiers.new('bev', 'BEVEL'); b.width = 0.05; b.segments = 3
    bpy.ops.object.shade_smooth()
    국물.data.materials.append(직물결(매끈재질('찌개국물', 색['Coral Rim'], 거칠기=0.34), 지름=2.1, 세기=0.32))
...
def 송편():
    ...
    떡색 = (('Paper', -0.44, 0.26, 16), ('Meadow Soft', 0.22, 0.34, -28), ('Coral Soft', 0.52, -0.05, 44),
            ('Paper', -0.15, -0.29, 68), ('Meadow Soft', -0.61, -0.24, -12))
...
def 국그릇(반경, 천색='Oat', z=0.09, 두께=0.115, 세로=1.55, 손잡이=False):
    ...
    """
    천 = 직물결(매끈재질('국그릇' + 천색, 색[천색], 거칠기=0.80), 지름=반경 * 2.1, 세기=0.5)
