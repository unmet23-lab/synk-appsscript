...
def 소식판():
    ...
    없어진다: 여기엔 내가 «할» 것을 두지 않고, 남이 «준» 것만 둔다.
      ...
      ② 소식 넷 — 우리 반(학24→N19 학생 뷰) · 원장 공지(부6) · 선생님 칭찬 왕관(학19) ·
    ...
    def 단추(cx, cz, 깊이, 꿰맴, r=0.085):
        """소식 한 줄의 «읽었나» — 오늘판 할 일 단추와 **완전히 같은 값**이다(r 0.085).
        갈리면 같은 부품이 화면마다 다른 크기로 서서 «다른 앱»이 된다."""
        bpy.ops.mesh.primitive_cylinder_add(radius=r, depth=0.038, location=(cx, 깊이, cz),
                                            rotation=(math.radians(90), 0, 0))
        몸 = bpy.context.object
        bev = 몸.modifiers.new('bev', 'BEVEL')
    ...
    카드z = 1.32
    카드 = 베개몸((1.21, 0.09, 0.70), 위치=(0, 0.5, 카드z), 크리스=0.80, 레벨=4)
    카드.data.materials.append(직물결(매끈재질('첨삭카드', 색['Ink'], 거칠기=0.95), 규모=180.0, 세기=0.055))
    키제외.append(카드)
    칸들['새소식'] = 카드
    러닝스티치(둥근사각걷기(1.21 - 0.09, 0.70 - 0.09, 0.10, 0.115), 0.40, (0, 카드z),
             매끈재질('카드실', 색['Deep Wool'], 거칠기=0.78), 0.040, 0.0085, 손, 흔들=0.004, 각흔들=4.0)

    # 코랄 실 — 화면의 «유일한» 코랄. 여기 말고 어디에도 두지 않는다(4계 ②).
    #   ⚠깊이 0.40 은 카드 앞면(0.41)보다 앞이다 — 오늘판 1판이 0.42 로 걸쳐 아예 안 보였다.
    for i in range(7):
        t = (i + 0.5) / 7
        땀하나(-0.89 + 0.90 * t + 손.uniform(-0.004, 0.004), 0.40,
              카드z - 0.48 + 손.uniform(-0.004, 0.004),
              0.036 * 손.uniform(0.92, 1.08), 0.0105, 지금실, 회전y=손.uniform(-4, 4))
    ...
    if 인자.get('탭바', '있음') != '없음':
        ...
        for i in range(7):
            t = (i + 0.5) / 7
            땀하나(탭자리[2] - 0.20 + 0.40 * t + 손.uniform(-0.003, 0.003), 0.55,
                  탭z + 0.155 + 손.uniform(-0.003, 0.003),
                  0.020 * 손.uniform(0.92, 1.08), 0.0085, 밑실, 회전y=손.uniform(-4, 4))
...
def 우리아이판():
    ...
    def 단추(cx, cz, 깊이, 꿰맴, r=0.085):
        """앞 세 화면과 **완전히 같은 값**이다(r 0.085). 갈리면 같은 부품이 화면마다 다른 크기로
        서서 «다른 앱»이 된다 — 역할이 갈려도 부품은 안 갈린다."""
        bpy.ops.mesh.primitive_cylinder_add(radius=r, depth=0.038, location=(cx, 깊이, cz),
                                            rotation=(math.radians(90), 0, 0))
        몸 = bpy.context.object
        bev = 몸.modifiers.new('bev', 'BEVEL')
    ...
    카드z = 1.32
    카드 = 베개몸((1.21, 0.09, 0.70), 위치=(0, 0.5, 카드z), 크리스=0.80, 레벨=4)
    카드.data.materials.append(직물결(매끈재질('요약카드', 색['Ink'], 거칠기=0.95), 규모=180.0, 세기=0.055))
    키제외.append(카드)
    칸들['요약'] = 카드
    러닝스티치(둥근사각걷기(1.21 - 0.09, 0.70 - 0.09, 0.10, 0.115), 0.40, (0, 카드z),
             매끈재질('카드실', 색['Deep Wool'], 거칠기=0.78), 0.040, 0.0085, 손, 흔들=0.004, 각흔들=4.0)
    ...
    for 행, 주 in enumerate(출석):
        for 열, 상태 in enumerate(주):
            if 상태 == 0:
                continue
            단추(첫열 + 열간격 * 열, 첫행 - 행간격 * 행, 달력깊이, 꿰맴=(상태 == 2))
    ...
    범례z = -1.12
    칸들['범례0'] = 단추(-0.92, 범례z, 달력깊이, 꿰맴=False)
    칸들['범례1'] = 단추(-0.02, 범례z, 달력깊이, 꿰맴=True)
    ...
    키제외.append(줄)
    칸들['보내'] = 줄
    글자등(단추들, 위치=(0.15, -2.4, -0.30), 크기=2.4, 힘=38).name = '단추등'
    ...
    if 인자.get('탭바', '있음') != '없음':
        ...
        키제외.append(탭판)
        칸들['탭바'] = 탭판
        탭자리 = (-0.78, 0.0, 0.78)          # 우리 아이 · 리포트 · 소식
        for 번, tx in enumerate(탭자리):
            앵2 = 베개몸((0.34, 0.02, 0.17), 위치=(tx, 0.55, 탭z + 0.01), 크리스=0.9)
            앵2.hide_render = True
            칸들['탭%d' % 번] = 앵2
        밑실 = 매끈재질('탭실', 색['Deep Wool'], 거칠기=0.78)
        for i in range(7):
