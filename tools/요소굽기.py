...
def 김치():
    ...
    ⚠어려운 형태다: 붉은 뭉치로 뭉개지기 쉽다. 그래서 «포기»를 통째로 안 그리고
      **자른 조각 셋을 겹쳐** 놓는다 — 잘린 단면의 켜(잎↔줄기)가 김치를 김치로 만든다.
    줄기(흰 아래)와 잎(붉은 위)의 **두 색 켜**가 이 형태의 전부다."""
    음식접시('Ink')
    손 = random.Random(20260822)
    잎재 = 직물결(매끈재질('김치잎', 색['Coral 3'], 거칠기=0.70), 지름=0.9, 세기=0.6)
    줄기재 = 직물결(매끈재질('김치줄기', 색['Stitch'], 거칠기=0.76), 지름=0.5, 세기=0.7)
    # 🔴**1판은 «분홍 천 조각»이었다.** 켜를 «위아래»로 쌓았더니 내려보기(40°)에서 흰 줄기가
    #   붉은 잎에 통째로 가려 두 색이 하나가 됐다. 김치를 김치로 만드는 것은 두 색의 «대비»인데
    #   그 대비가 카메라에서 안 보이면 없는 것이다.
    #   ⇒ 켜를 **끝과 끝**으로 옮긴다: 한쪽 끝은 흰 줄기, 다른 끝은 붉은 잎. 위에서 둘 다 보인다.
    #   ⚠보풀도 걷었다 — 작은 조각의 짧은 털은 형태를 흐린다(떡볶이 어묵과 같은 배움).
    for x, y, 각, s in ((-0.40, 0.24, 22, 1.00), (0.34, 0.30, -30, 0.92), (0.00, -0.34, 6, 1.06)):
        코, 사 = math.cos(math.radians(각)), math.sin(math.radians(각))
        # 줄기 — 조각의 «한쪽 끝». 두툼하고 희다.
        줄 = 베개몸((0.20 * s, 0.26 * s, 0.115 * s),
                  위치=(x - 0.30 * s * 코, y + 0.06 - 0.30 * s * 사, 0.115), 회전z=각, 크리스=0.62)
        줄.data.materials.append(줄기재)
        # 잎 — 반대쪽 끝. 넓고 얇게 퍼진다(잎은 퍼지고 줄기는 뭉친다).
        잎 = 베개몸((0.34 * s, 0.30 * s, 0.062 * s),
                  위치=(x + 0.22 * s * 코, y + 0.06 + 0.22 * s * 사, 0.145), 회전z=각 + 6, 크리스=0.82)
        잎.data.materials.append(잎재)
        # 잎맥 — 줄기에서 잎끝으로 뻗는 실금 셋. 이것이 있어야 «천 조각»이 아니라 «잎»이다.
        for i in range(3):
            ox = (-0.10 + 0.16 * i) * s
            땀하나(x + 0.20 * s * 코 - ox * 사, y + 0.06 + 0.20 * s * 사 + ox * 코, 0.212,
                  0.15 * s, 0.017, 매끈재질('맥%d' % i, 색['Coral Rim'], 거칠기=0.55),
