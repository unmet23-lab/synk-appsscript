...
function main() {
  ...
  /* 🔴 [09-08 검수 P2 0db56169a006] 누끼 파일의 «자리»는 창구가 안다 — 여기서 폴더를 짓지 않는다.
   *   09-05 판올림으로 「한 컷에 파일 하나(전부 투명)」가 되면서 `정본_4K/누끼/` 폴더 자체가
   *   없어졌는데 이 지면만 그 폴더를 세고 있었다 ⇒ **여덟 장이 다 있는 날에도 「없다」**고 말했다.
   *   한 값을 두 곳이 알면 갈린다([[constant-known-in-two-places]]) — 창구 하나만 안다. */
  const 누끼경로 = (n) => path.resolve(루트, 자산.까몽경로(n, { 누끼: true }));
  const 있누끼 = (n) => fs.existsSync(누끼경로(n));
  ...
  const 누끼줄 = `합계 ${누끼있.length}/${누끼대상.length} = ${갈래('정본')} + ${갈래('시안')}`
    + (누끼있.length
      ? ` · 가장 새것 ${esc(시각(Math.max(...누끼있.map((n) => fs.statSync(누끼경로(n)).mtimeMs))))}`
      : ' — 아직 한 장도 없다')
