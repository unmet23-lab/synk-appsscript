...
function main(argv) {
  ...
  for (let 라운드 = 1; 라운드 <= 라운드상한; 라운드++) {
    ...
    if (파일들.length) {
      ...
      const 제목 = `[codex 실행자] ${검사.제목.replace(/^발주\s*[—-]\s*/, '')} (라운드 ${라운드}${이어받은.length ? ` · 앞 회차 이어받음 ${이어받은.length}` : ''})`;
    ...
    마지막 = { 라운드, 지적들, 미충족: 진짜미충족, 시험실패, sha: 기록.sha, 이어받음: 기록.이어받음.filter((f) => 파일들.includes(f)) };
