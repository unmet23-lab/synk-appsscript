 ...
 *                             [--라운드 2] [--timeout 초] [--버그만] [--발주검토안함] [--검수안함] [--던지기]
...
const 홑플래그 = ['--버그만', '--발주검토안함', '--검수안함', '--마른손', '--던지기', '--런목록', '--공통지침대조', '--help', '-h'];
...
/** `- \`tools/x.js\`` → `tools/x.js` · `- tools/ — 도구` → `tools/` (첫 마디만 경로다). */
function 경로항목(s) {
  const 백틱 = /`([^`]+)`/.exec(s);
...
function 발주서검사(text) {
  ...
  if (절들['시험'] && !시험명령들.length) 오류들.push('«시험» 에 명령이 0개다 — 도구가 직접 돌릴 명령을 적는다(예: `node --test tests/x.test.js`)');
  ...
  return { 제목, 절들, 허용경로들, 시험명령들, 수용기준들, 금지항목들, 맥락들, 오류들 };
}
...
function main(argv) {
  ...
  for (let 라운드 = 1; 라운드 <= 라운드상한; 라운드++) {
    const 기록 = { 라운드, sha: null, 실행: null, 범위밖: [], 비밀: [], 시험: [], 검수: null, 수용: null };
    장부행.라운드들.push(기록);
    ...
    if (파일들.length) {
      const 제목 = `[codex 실행자] ${검사.제목.replace(/^발주\s*[—-]\s*/, '')} (라운드 ${라운드})`;
    ...
    // ⑤ 검수 — 다른 세션
    let 지적들 = [];
    if (기록.sha && !argv.includes('--검수안함')) {
      console.log(`⑤ 검수(codex-review · 새 세션 · 읽기 전용)… 최대 ${Math.min(초, 1800)}초`);
      const r = 검수부르기(wt, 기록.sha, Math.min(초, 1800), argv.includes('--버그만'));
    ...
    if (기록.sha) {
      try {
        const diff = git(wt, ['show', '--format=%H %s', 기록.sha]).slice(0, 검수.디프상한 || 200000);
        ...
        console.error('🟡 수용 대조가 안 돌았다(확인 불가 · 판정에서 뺀다): ' + e.message);
      }
    }

    const 시험실패 = 기록.시험.filter((r) => !r.통과).map((r) => `${r.명령} → ${r.시간초과 ? '시간초과' : '종료 ' + r.종료}\n${r.꼬리}`);
    const 진짜미충족 = 미충족.filter((x) => x.판정 === '미충족');
    마지막 = { 라운드, 지적들, 미충족: 진짜미충족, 시험실패, sha: 기록.sha };
    if (!지적들.length && !진짜미충족.length && !시험실패.length && 기록.sha) {
      장부행.상태 = '완주'; 장부적기(장부행);
      break;
    }
    앞라운드 = 마지막;
  ...
  마감(1, `남음 — 라운드 ${마지막 ? 마지막.라운드 : 0}`); return 1;
}
