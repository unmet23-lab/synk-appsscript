...
function main(argv) {
  ...
  if (마른손 || 프롬프트확인) {
    if (프롬프트확인) {
      ...
      const 실 = 잰다('실행자', 실행프롬프트({ 발주서, 실행자절, 공통, 라운드: 1, 앞라운드: null, 철학 }));
      const 발 = 잰다('발주검토자', 발주검토프롬프트(발주서, 공통, 철학));
      if (!실 || !발) {
  ...
  if (!argv.includes('--발주검토안함')) {
    try {
      const 검토 = 스키마호출(읽기플래그, 판단설정, 검토스키마, 발주검토프롬프트(발주서, 공통, 철학), Math.min(초, 1200), '발주 검토', wt);
      장부행.검토 = 검토;
  ...
  for (let 라운드 = 1; 라운드 <= 라운드상한; 라운드++) {
    ...
    try {
      기록.실행 = 스키마호출(쓰기플래그, 실행설정, 실행스키마, 실행프롬프트({ 발주서, 실행자절, 공통, 라운드, 앞라운드, 철학 }), 초, `실행 ${라운드}`, wt);
    } catch (e) {
