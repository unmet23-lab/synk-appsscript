 ...
 *   그 뒤는 라우터 몫이다 — 가지를 읽고 의미·통합을 확인해 master 에 합치고, 유호님이 운영 배포를 승인한다.
 *
 * ■ 왜 이렇게 지었나
...
const 홑플래그 = ['--버그만', '--발주검토안함', '--검수안함', '--마른손', '--프롬프트확인', '--던지기', '--런목록', '--공통지침대조', 한도무시플래그, '--help', '-h'];
...
/* ⚠ 이 목록은 **문서와 짝을 맞추는 용도**다 — 검사가 「모르는 절」을 거절하지는 않는다.
 *   발주서는 전문이 그대로 프롬프트에 실리므로(실행프롬프트 «발주서 전문»), 여기 없는 절을
 *   써도 실행자는 읽는다. 구조로 «파싱해 쓰는» 절만 위 `필수절` 과 아래 파서가 다룬다.
 * 🆕 '부품'(09-08) — 이미 깔린 꾸러미를 「이건 써도 된다」고 알리는 자리. 실행자는 인터넷이
 *   끊긴 채 돌아 **새로 못 깐다**(까닭 = 머리말 ① · 홈의 열쇠가 읽히는데 나갈 길이 없다는 것이
 *   지금 유일한 벽이다). 도구도 안 깐다 — 사람이 발주 «전»에 깔고 여기 이름만 적는다. */
const 선택절 = ['금지', '맥락', '시험 수정 허용', '부품'];
...
/* ── 판단 정본을 «짓는 쪽»에도 싣는다 (2026-09-08 · 유호 확정 「철학 정본만 이식하자」) ─────
 * 실측이 부른 일이다: 검수 레인(codex-review.js)은 철학 게이트 5,672자를 매번 실었는데
 * **짓는 레인(이 파일)은 0자였다** — 이 파일 전문에 `철학` 이 «고치지 마라» 목록 한 줄뿐이었다.
 * 그래서 실행자는 「무엇이 좋은 산출물인가」를 모른 채 짓고, 검수자만 그 기준으로 때렸다.
 * 라운드가 도는 까닭의 일부가 여기다(짓는 쪽이 못 본 기준으로 맞는다).
 *
 * 🔴 서문이 이 블록의 절반이다 — 검수자용 서문(검수.철학블록)을 그대로 빌리면 안 된다.
 *   검수자에게 철학은 «방향보다 상위»지만, 실행자에게 그렇게 말하면 **발주서 범위를 넘는
 *   허가**로 읽힌다(AGENTS.md §3-1 정면 위반). 실행자에게 철학은 *무엇을 지을지*가 아니라
 *   *어떻게 지을지*를 가르는 자다. 그 서열을 같은 줄에 박지 않으면 모델은 둘을 나란히 놓는다.
 * 본문은 빌리고 서문만 역할별로 다르다 — 한 값을 두 곳이 적으면 갈리고, 갈린 쪽이 조용하다. */
const 실행자철학블록 = (철학) => (철학 ? `
--- 판단 정본 — docs/SYNK_철학.md (유호님 확정) ---
이것은 «무엇을 지을지»가 아니라 «어떻게 지어야 좋은 산출물인가»를 가르는 기준이다.
🔴 **범위는 여전히 발주서가 정한다** — 철학을 근거로 발주서 «범위» 밖을 고치지 마라.
철학상 발주서가 모자라 보이면 고치지 말고 «발주밖발견»에 적어라. 그게 이 자리의 옳은 답이다.
쓰는 자리는 둘이다: ①범위 «안»에서 어떻게 지을지 고를 때 ②시험을 무엇으로 잡을지 정할 때.
${철학}
` : '');

/* ── 프롬프트 셋 — 순수 함수(회귀가 문다) ─────────────────────────────────────────── */
function 발주검토프롬프트(발주서, 공통, 철학 = '') {
...
⑥아래 판단 정본이 실렸으면, 이 발주서대로 지은 것이 그 게이트를 지나는가 — 못 지나면 그 자리를 대라.
«막힘» = 이대로 구현하면 틀린 것을 짓거나 판정을 못 한다 · «경고» = 구현은 되지만 실행자가 짐작으로 메워야 한다.
🔴 ⑥ 도 이 두 등급으로만 판정한다 — 「철학상 아쉽다」는 구멍이 아니다. 게이트를 «못 지난다»를 근거와 함께 대야 지적이다.
...
function 실행프롬프트({ 발주서, 실행자절, 공통, 라운드, 앞라운드, 철학 = '' }) {
  const 수리 = 앞라운드 ? `
...
기준을 빠뜨리거나 중복하지 마라. 화면·저장·외부 연동은 코드 존재만으로 충족이 아니다. 기준이 요구하는 실제 시험·조작·저장 결과를 확인하지 못하면 확인불가다.
...
/* ── 장부 ─────────────────────────────────────────────────────────────────── */
function 장부적기(행) {
  행.종료시각 = new Date().toISOString();
  행.소요ms = Math.max(0, Date.parse(행.종료시각) - Date.parse(행.시각));
  const 마지막 = (행.라운드들 || []).at(-1);
  // 원문·가설을 새 파일에 복사하지 않는다. 기존 장부의 도구 관측 결과와 경로만 이어 준다.
  행.인계 = {
    발주지문: 행.발주.지문, 가지: 행.가지, 상태: 행.상태,
    대상커밋: 마지막 && (마지막.대상커밋 || 마지막.검수대상 || 마지막.sha) || null,
    시험: 마지막 ? (마지막.시험 || []).map(({ 명령, 통과, 종료, 시간초과 }) => ({ 명령, 통과, 종료, 시간초과 })) : [],
    수용: 마지막 && 마지막.수용검사 ? { 기준수: 마지막.수용검사.기준수, 충족수: 마지막.수용검사.충족수,
      확인불가: 마지막.수용검사.확인불가, 누락수: 마지막.수용검사.누락.length } : null,
    검수: 마지막 && 마지막.검수 || null,
    다음: 행.상태 === '완주' ? '라우터가 의미·통합을 확인한다. 라이브 배포는 별도 승인 통로다.' : '기존 기록의 미해결 원인을 확인한 뒤 같은 발주로 이어간다.',
  };
...
function 앞실행찾기(지문, wt) {
  let rows;
  try { rows = fs.readFileSync(기록경로, 'utf8').split('\n').filter(Boolean); } catch (_) { return null; }
  for (let i = rows.length - 1; i >= 0; i--) {
    try {
      const r = JSON.parse(rows[i]);
      if (r.발주 && r.발주.지문 === 지문 && r.워크트리 === wt) return r;
    } catch (_) { /* 깨진 장부는 재사용 근거가 아니다 */ }
  }
  return null;
}
...
function main(argv) {
  ...
  const 프롬프트확인 = argv.includes('--프롬프트확인');
  if (검사.오류들.length) {
  ...
  const 이름 = 저장소이름(root);
  const 지문 = 발주지문(발주서);
  /* 🔴 확인 모드도 워크트리를 안 만든다(09-08 · GPT 발주검토 지적 🟡). 재기만 하는 부름이
   *   폴더·가지를 남기면 읽기 전용 자리에서 «재기 전에» 실패하고, 성공해도 쓰레기가 쌓인다. */
  const { wt, 가지, 있음 } = 워크트리준비(root, 지문, 마른손 || 프롬프트확인);
  ...
  const 공통 = 정본절('공통');
  const 실행자절 = 정본절('실행자');
  /* 판단 정본 — 검수 레인과 **같은 추출기**에서 빌린다(사본을 안 만든다 · 갈리면 짓는 쪽과
   * 때리는 쪽이 다른 기준으로 돈다). 저장소가 `--저장소` 로 갈려도 철학은 이 저장소 것이다
   ...
   * 처음엔 확인 모드가 프롬프트를 «따로» 조립했다. 그래서 아래 모델용 호출에서 철학 인자만
   * 빼도 확인은 「실렸다」·종료 0 을 냈다 — 재는 자가 제가 재려던 결함을 지킨 것이다.
   * GPT 가 그 사본을 만들어 재현해 보이고 막힘으로 돌려보냈다(내가 만든 자였다).
   * ⇒ 두 자리를 이 클로저 하나로 합친다. 여기서 철학을 빼면 확인도 실제 호출도 같이 0 이
   *   되므로, 확인이 초록이면 모델에게 가는 그 문자열도 초록이다. 두 벌이면 갈리고,
   *   갈린 쪽이 조용하다 — 이 파일이 오늘 두 번 같은 자리에서 물렸다. */
  const 실행프롬프트를만든다 = (라운드, 앞라운드) =>
    실행프롬프트({ 발주서, 실행자절, 공통, 라운드, 앞라운드, 철학 });
  const 발주검토프롬프트를만든다 = () => 발주검토프롬프트(발주서, 공통, 철학);

  /* ── --마른손 은 여기서 끝난다 (09-08 에 철학 조립 «뒤»로 옮겼다) ───────────────────────
   * 왜 옮겼나: GPT 발주검토자가 잡은 구멍이다(09-08 · 막힘 1건). 「블록을 만들 수 있다」와
   * 「실제 호출부가 그것을 넘긴다」는 다른 명제인데, 밖에서 이 둘을 가를 자가 없었다 —
   * 마른손이 철학 조립 «앞»에서 끝나서, 이 레인의 실제 배선을 안 태우고 재는 길이 아예
   * 없었다. 그래서 대조 도구가 스스로 철학을 조립해 세게 되고, 그러면 아래 실행프롬프트
   * 호출에서 철학 인자를 빼도 그 도구는 여전히 「실렸다」고 답한다(자가 결함을 지킨다).
   * ⇒ 마른손을 조립 뒤로 내려 **실제로 조립된 프롬프트**를 재게 한다.
   * `--프롬프트확인` 은 거기서 한 걸음 더 간다: 두 역할의 프롬프트를 실제로 만들어
   * 그 안에 정본 본문이 들어갔는지 세어 낸다. 코덱스는 여전히 안 태운다. */
  if (마른손 || 프롬프트확인) {
    if (프롬프트확인) {
      const 본문 = 검수.철학텍스트();
       ...
       * 모델에게 가는 그 문자열도 초록이다. 따로 조립하면 이 자가 거짓 초록만 낸다. */
      console.log('실제로 조립한 프롬프트 — 코덱스는 안 태운다');
      const 실 = 잰다('실행자', 실행프롬프트를만든다(1, null));
      const 발 = 잰다('발주검토자', 발주검토프롬프트를만든다());
      if (!실 || !발) {
        console.error('🔴 판단 정본이 실제 프롬프트에 안 들어갔다 — 조립은 됐는데 «넘겨지지» 않은 자리다.');
        return 1;
      }
    }
    console.log(`(--${프롬프트확인 ? '프롬프트확인' : '마른손'}: 여기까지 — 코덱스를 안 태운다)`);
    return 0;
  }

  const 이전실행 = 앞실행찾기(지문, wt);
  const 수용만재개 = !argv.includes('--검수안함') && 수용재개가능(이전실행, 가지끝실행자커밋(wt), 변경파일들(wt).length);
  const 장부행 = {
  ...
  if (!수용만재개 && !argv.includes('--발주검토안함')) {
    try {
      const 검토 = 스키마호출(읽기플래그, 판단설정, 검토스키마, 발주검토프롬프트를만든다(), Math.min(초, 1200), '발주 검토', wt);
      장부행.검토 = 검토;
  ...
  for (let 라운드 = 1; 라운드 <= 라운드상한; 라운드++) {
    ...
    if (수용만재개 && 라운드 === 1) {
      기록.실행 = { 상태: '이어받음', 요약: '같은 커밋의 시험·검수가 끝났다. 실행자를 다시 부르지 않고 시험 재실행과 수용 대조를 한다.', 남긴것: [], 발주밖발견: [] };
      기록.재개원본 = 이전실행.시각;
    } else {
      console.log(`\n② 실행 라운드 ${라운드}/${라운드상한} — 코덱스가 워크트리에 짓는다(최대 ${초}초)…`);
      try {
        기록.실행 = 스키마호출(쓰기플래그, 실행설정, 실행스키마, 실행프롬프트를만든다(라운드, 앞라운드), 초, `실행 ${라운드}`, wt);
      } catch (e) {
    ...
    const 수용결과 = 수용검사(검사.수용기준들, 기록.수용);
    기록.수용검사 = 수용결과;
    const 진짜미충족 = 수용결과.미충족;
    ...
    const 수용못잼 = 수용결과.확인불가;
    console.log(`⑥ 도구 전수 대조: 충족 ${수용결과.충족수}/${수용결과.기준수} · 미확인 ${수용결과.미확인.length} · 누락 ${수용결과.누락.length}`);
    수용결과.문제.forEach((s) => console.error('🟡 ' + s));
    ...
    if (완료판정({ 수용: 수용결과, 시험: 기록.시험, 차단: 지적들, sha: 가지끝, 검수생략: argv.includes('--검수안함') })) {
      장부행.상태 = '완주'; 장부적기(장부행);
      break;
    }
    ...
    if (argv.includes('--검수안함') && 수용결과.완료 && !시험실패.length) {
      장부행.상태 = '검수생략'; 장부적기(장부행);
      console.log('🟡 독립 검수를 생략했다. 시험·수용 기준이 충족돼도 완주가 아니다.');
      break;
    }
    앞라운드 = 마지막;
  ...
  if (장부행.상태 === '완주') {
    console.log('✅ 완주 — 모든 수용 기준 확인 · 독립 검수 · 시험 통과. 다음은 라우터 몫: 가지를 읽고 의미·통합을 확인한 뒤 master 에 합친다.');
  ...
  if (마지막) {
    ...
    if (마지막.수용못잼) {
      ...
      console.log('      같은 발주·같은 커밋·깨끗한 작업본이고 시험·검수가 끝났으면 실행자를 생략하고 시험 재실행과 수용 대조를 이어간다.');
    }
  }
  console.log('   라우터가 읽고 정한다 — 오판이면 검수 지적은 `node tools/codex-review.js --기각 <키> --사유 "…"`, 남은 일은 발주서를 고쳐 같은 명령으로 다시(같은 워크트리에서 이어 돈다).');
