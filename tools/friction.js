...
/* 기록물은 뺀다 — 장부는 **모든** 신고·해소 커밋에 딸려 들어와서, 안 빼면 살아있는 행 전부가
 * 매번 걸린다(신호가 아니라 소음이 된다). 판정은 friction-close-guard 의 같은 규칙과 축을 맞춘다.
 * 🔑 2026-08-20 위임 2단계 — 보드·인계문 이름(8자리 hex · `인계문.md` · `세션보드_아카이브.md`)은
 *   그 층을 걷으면서 뺐다. 없는 파일을 계속 거르면 「왜 있는지 아무도 모르는 규칙」이 된다. */
...
function main() {
  ...
  if (cmd === 'add') {
     ...
     *   하면 안 된다」. 예언한 자리와 난 자리가 같다. `tests/지문축.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)` 가 이 축을 못박는다. */
