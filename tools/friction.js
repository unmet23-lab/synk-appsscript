...
function 점유(ids, 옵션) {
  ...
  for (const id of ids) {
    ...
    if (남.length) 표.set(id, { 상태: '잡힘', 자리: 남.map((x) => (x.가지 ? `가지 ${x.가지}` : `보드 ${x.파일}`)) });
  ...
  return { 모름: !!판.모름, 못본층: 판.못본층 || [], 표, 가지분모: (판.분모 && 판.분모.가지분모) || null };
}
...
function report(mode) {
  ...
  if (mode === 'open' || mode === '보류') {
    ...
    if (mode === 'open') {
      ...
      console.log('  🔒 점유는 **메인 보드 + 미머지 가지**를 함께 본다(F521) — 코드 트랙의 선언은 가지 안에 앉아서'
        + ' 메인만 읽으면 그 트랙이 통째로 「빈 자리」로 보인다(F610 실측: 잡힌 4건이 전부 가지에만 있었다).');
      /* 🔑 **몇 개의 가지를 훑었나**를 함께 낸다(장부 619) — 이 수가 없으면 「잡힘 0」이 「빈 자리」인지
       *   「가지를 못 봤다」인지 읽는 쪽이 못 가른다. 새 클론·클라우드 세션은 트랙 가지가 `origin/*` 로만
       *   살아서, 옛 판은 로컬만 훑고도 `모름:false` 로 답했다 — 그 0 이 「집어라」로 읽혔다. */
      const 출처 = 점유판.가지출처;
      console.log(출처
        ? `     📐 훑은 가지 **로컬 ${출처.로컬} · 원격 ${출처.원격}** — 이 수가 0이면 위 「잡힘」은 «빈 자리»가 아니라 «안 봤다»다.`
          + '\n        ⚠ 원격 ref 가 낡으면 그만큼 못 본다(fetch 시점은 이 도구 밖이다 · 장부 619 ㉢).'
        : '     ❔ 훑은 가지 수를 **못 쟀다** — 위 「잡힘」의 분모가 없다(0건이 아니다 · F207).');
