...
function 점유(ids, 옵션) {
  ...
  } catch (_) { return { 모름: true, 못본층: ['보드'], 표: new Map() }; }
  ...
  /* 🔑 **가지 분모를 그대로 나른다**(장부 619) — 「잡힘 0」이 「빈 자리」인지 「가지를 못 봤다」인지는
   *   몇 개를 훑었나가 가른다. 새 클론·클라우드 세션은 트랙 가지가 `origin/*` 로만 사는데, 옛 판은
   *   로컬 가지만 훑고도 `모름:false` 로 답해 그 0 이 「집어라」로 읽혔다. */
  return { 모름: !!판.모름, 못본층: 판.못본층 || [], 표, 가지출처: (판.분모 && 판.분모.가지출처) || null };
}
