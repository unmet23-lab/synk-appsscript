...
function seenElsewhere(opts) {
  ...
  /* 훑는 범위에 **`claude/*` 브랜치**도 넣는다 — 그 환경은 채번 태그 push 가 막혀 손으로 번호를
   * 매기는데(F196), 여기가 origin/master + 로컬만 보면 그 손번호를 못 봐서 **양쪽이 서로를 못 보는**
   * 상태가 된다.
   * ⚠ [2026-08-18 유호 교정] 이 자리를 오래 「폰 브랜치」라 불렀는데 **폰으로 작업하는 것이 없다** —
   *   지금 그 환경은 클라우드(웹) 세션이고, 태그 push 가 막히는 성질은 그대로다(F623 · 실측: 이 세션이
   *   F620·F623 을 손번호로 달았다). 아래 08-07 실측 문장은 사실이라 그대로 둔다.
...
/* ── 「이 번호를 지금 누가 잡고 있나」 (F610) ──────────────────────────────────────
 *
 * ■ 무엇이 샜나 (2026-08-18 실측 · 이 세션이 그대로 걸렸다)
 *   `--open` 은 열림 행을 **점유 표기 없이** 나열한다. 그 목록에서 F605 를 골랐는데 이미 두 세션이
 *   잡고 있었고 내가 세 번째였다(PR #105·#106). 근거로 돌린 `board.js | grep` 이 0 을 낸 이유는
 *   그 선언들이 **미머지 가지 안**에 있어서다 — 08-15 「워크트리+PR」 확정 뒤 코드 트랙은 거기 앉는다(F521).
 *   전수로 재니 열림 10건 중 **4건이 이미 잡혀 있었고 그 4건이 전부 가지에만** 있었다(40%).
 *
 * 🔑 같은 병을 이 저장소가 이미 한 번 고쳤다 — `대기열.js` 의 F406(「표식은 붙인 시점의 사진이라
 *   남이 집는 순간 늙는다」). 그 처방이 옆 목록으로 안 옮겨 왔을 뿐이다. 새는 방향은 저기 머리말이
 *   이름 대어 적은 그대로 **「부른다」**이고, 그게 제일 나쁜 방향이다.
 *
 * 🔑 신호는 **F번호 하나**다(대기열의 3신호와 다르다 — 장부 행은 영구 번호를 갖는다).
 *   대신 «점유가 아닌 것» 둘을 갈라 낸다. 안 가르면 1차 셈처럼 거짓양성이 절반이 된다:
 *     · 내 줄 — 내 지문 파일의 선언(board-guard 의 F291 과 같은 축)
 *     · 🎫 놓인 줄 — 뜻이 정반대다(주인이 놓았으니 집어도 된다). 이걸 막으면 놓아 준 일감이 잠긴다.
 *
 * ■ 대가 — 틀릴 때 어떤 모습인가 (장치 예산)
 *   ㉠ 신고문을 **인용만** 한 줄도 잡는다 — 방향은 「못 고름」이고 이 저장소는 그쪽을 안전으로 친다
 *     (대기열 점유 머리말의 기존 판단을 승계했다 · 거짓양성률은 **안 쟀다** · F348).
 *     그래서 조용히 지우지 않는다 — 잡힌 줄은 **어디에 있는지와 함께** 낸다.
 *   ㉡ 보드를 못 읽으면 `모름` 이다 — 「빈 자리」로 접지 않는다(F207). 접으면 그 순간 이 장치가
 *     자기가 고치려던 병(안 재고 부른다)을 그대로 낸다.
 *   · 닫는 것 = **없다.** 사람이 손으로 점유를 적던 통로가 장부엔 애초에 없었다(그게 이 신고다).
 */
function 점유(ids, 옵션) {
  const o = 옵션 || {};
  let 판;
  try {
    판 = o.판 || require(path.join(__dirname, 'lib', '보드.js'))
      .활성줄전량(path.resolve(__dirname, '..'), o.보드옵션);
  } catch (_) { return { 모름: true, 못본층: ['보드'], 표: new Map(), 가지분모: null }; }
  ...
  for (const id of ids) {
    ...
    const 건 = 판.줄들.filter((x) => RE.test(x.줄));
    if (!건.length) continue;
    const 남 = 건.filter((x) => !x.놓음 && !내것(x));
    /* 🔑 갈래 순서가 판정을 정한다 — **내 줄이 🎫 보다 먼저다.** 내가 놓았든 잡았든 그건 내 트랙이고,
     *   「집어도 된다」는 **남이** 놓은 자리에만 붙는 말이다.
     *   🔴 이 순서를 안 정하면 실물이 뒤집힌다(실측 2026-08-18 · 이 트랙 자신이 밟았다):
     *   내 상태 칸에 분모 보고로 적은 «🎫1»(=놓임 1건) 을 `놓은줄()` 이 표식으로 읽어, 내가 방금
     *   선언한 F610 이 「🎫 놓인 줄 — 집어도 된다」로 나왔다. 그 함수 머리말은 이 형태를 「실측 0건」
     *   이라 적어 뒀는데 **1건이 났다** — 보고문에 숫자를 적으면 그 글자가 표식이 된다(F610 조각에 남겼다). */
    /* F628 — 원격 줄의 자리는 「보드 <파일>」로 적으면 **거짓말**이다: 그 파일은 이 트리에 없거나
     *   그 줄이 없다. 사람이 열어 보고 「없는데?」가 되면 다음부터 이 화면을 안 믿는다. */
    if (남.length) {
      표.set(id, { 상태: '잡힘', 자리: 남.map((x) => (x.가지 ? `가지 ${x.가지}`
        : (x.원격 ? `${x.원격} (내 트리가 아직 안 받았다)` : `보드 ${x.파일}`))) });
    }
    else if (건.some(내것)) 표.set(id, { 상태: '내줄', 자리: [] });
    else if (건.some((x) => x.놓음)) 표.set(id, { 상태: '놓임', 자리: [] });
  }
  /* `가지분모` = **몇 개의 가지를 훑었나** `{로컬, 원격, 훑음}`(F619). `null` = 안 쟀다 — 0 과 다르다.
   *   여기까지 올리는 이유는 하나다: **안 읽히는 분모는 없는 것과 같다.** F619 를 고칠 때 이 값을
   *   `보드.js` 가 돌려주게는 했지만 화면에 내는 소비자가 0이라, 「원격 가지를 하나도 안 봤다」와
   *   「원격 가지가 없다」가 출력에서 여전히 같은 모양이었다. 그 둘이 갈리는 자리가 아래 머리줄이다. */
  return {
    모름: !!판.모름,
    못본층: 판.못본층 || [],
    표,
    가지분모: (판.분모 && 판.분모.가지분모) || null,
    /* F628 — `null` = 그 층을 못 봤다(0 과 다르다). `점유()` 가 안 나르면 아래 인쇄기가 원리상 못 낸다. */
    원격몫: 판.분모 && Object.prototype.hasOwnProperty.call(판.분모, '원격') ? 판.분모.원격 : null,
  };
}

/** 「가지를 몇 개 훑고 낸 점유인가」 한 조각 (F619 남은 일 · 2026-08-18).
 *
 * ■ 왜 순수 함수인가 — 같은 절이 방금 그걸로 샜다
 *   F608 은 판정을 «요약줄(순수 함수)» 과 «붙임 블록(CLI 인라인)» 두 곳에 적어 갈라졌고, 갈라진
 *   쪽이 회귀 0 인 인라인이었다. 이 줄도 CLI 안에 문자열로 두면 같은 자리가 된다 — 그래서 꺼낸다.
 *
 * ■ 무엇을 가르나 — **「빈 자리 N」의 분모**
 *   위 `점유셈` 의 `빈자리` 는 「아무도 안 잡았다」로 읽힌다. 그런데 가지를 하나도 안 훑었으면
 *   그건 「빈 자리」가 아니라 **모름**이다(F619 가 실측한 그 모양: 로컬 가지만 봐서 원격 14개가
 *   통째로 사각이었고, 그 0 을 믿고 착수해 F608 을 두 세션이 지었다).
 *   `가지끝()` 은 이제 로컬·원격을 다 세지만 **원격 ref 는 마지막 fetch 만큼만 새것**이라
 *   그 사각이 완전히 사라지지는 않는다 — 남은 몫을 이 줄이 사람 눈에 넘긴다.
 *
 * ■ 대가 — 틀릴 때 어떤 모습인가
 *   ㉠ `null`(=안 쟀다)을 `0` 으로 안 적는다. 적으면 이 함수가 없애려는 병을 자기가 낸다.
 *   ㉡ 훑음 0 일 때만 안내를 붙인다 — 늘 붙이면 상시로 우는 경보가 되어 사람이 무시하는 법을
 *     배우고, 그때는 진짜가 와도 안 읽힌다(F113). ⚠ 훑음 0 은 **정상일 수도 있다**(다 머지된
 *     노트북). 그래서 단정하지 않고 「둘 중 하나다」로 적는다 — 가르는 것은 사람 몫이다.
 *   ㉢ 셈 칸에 **표식 글자(🎫 등)를 안 쓴다**(F612) — 이 줄은 사람이 보드 상태 칸에 그대로
 *     옮겨 적는 재료다. 표식과 수가 붙으면 `놓은줄()` 이 그 줄을 「놓았다」로 읽는다.
 */
function 가지분모줄(분모) {
  if (!분모) return '  · 훑은 가지 = **안 쟀다**(0개가 아니다) — 이 점유는 메인 보드만 보고 낸 것이다.';
  const { 로컬 = 0, 원격 = 0, 훑음 = 0 } = 분모;
  const 앞 = `  · 훑은 가지 **${훑음}개**(로컬 ${로컬} · 원격 ${원격})`;
  if (훑음 > 0) return 앞;
  return `${앞} — 둘 중 하나다: 미머지 가지가 정말 없거나, **이 클론이 fetch 를 안 했거나**.\n`
    + '    가지 선언은 그때 원리상 안 보인다(F619). 가르려면: git fetch origin \'+refs/heads/*:refs/remotes/origin/*\'';
}

/** 「`origin/master` 층을 봤나」 한 조각 (F628 · 2026-08-18).
 *
 * ■ 왜 가지와 **따로** 적나 — 처방이 정반대다
 *   가지 줄 = 「아직 안 머지됐다 · PR 을 봐라」 / 이 줄 = 「이미 머지됐는데 내가 안 받았다 ·
 *   `git merge --ff-only` 로 받아라」. 한 줄에 뭉치면 받은 사람이 없는 PR 을 뒤진다(F103).
 *
 * ■ 대가 — 틀릴 때 어떤 모습인가
 *   ㉠ `null`(=못 봤다)을 `0` 으로 안 적는다 — 그게 이 층을 지은 이유 자체다(F207).
 *   ㉡ **0 일 때는 조용하다.** 늘 붙이면 상시 경보가 되고, 그때는 진짜가 와도 안 읽힌다(F113).
 *     0 은 「내 트리가 origin/master 를 다 받았다」는 참인 상태라 알릴 것이 없다.
 *   ㉢ 0 이 **참인 0 이 아닐 수 있는 자리**는 하나 남는다 — `origin/master` ref 자체가 낡았을 때다.
 *     그건 fetch 시점 문제이고 이 함수 밖이다(F619 ㉢). 그래서 「받아라」에 fetch 를 함께 적는다.
 */
function 원격몫줄(몫) {
  if (몫 === null || 몫 === undefined) {
    return '  · `origin/master` 층 = **안 쟀다**(거기 선언 0 이 아니다) — 이 점유는 그만큼 좁다(F628).';
  }
  if (!몫) return '';                                   // 참인 0 — 알릴 것이 없다(위 대가 ㉡)
  return `  · 🔴 \`origin/master\` 에 **내가 아직 안 받은 보드 줄 ${몫}개**가 있다 — 위 「잡힘」은 그만큼 좁게 나왔다.\n`
    + '    받고 다시 본다: git fetch origin master && git merge --ff-only origin/master';
}

/** 점유 표의 갈래별 셈 — 0 은 분모와 함께 쓴다(유호 확정 08-14). */
function 점유셈(표, 전체) {
  const 셈 = { 잡힘: 0, 놓임: 0, 내줄: 0 };
  for (const v of 표.values()) if (셈[v.상태] !== undefined) 셈[v.상태] += 1;
  return { ...셈, 빈자리: 전체 - 셈.잡힘 - 셈.놓임 - 셈.내줄 };
}
...
function report(mode) {
  ...
  if (mode === 'open' || mode === '보류') {
    ...
    /* 🔑 점유를 **머리에서** 먼저 말한다(F610) — 목록을 다 읽은 뒤에 알려 주면 이미 고른 뒤다.
     *   0 은 분모와 함께 쓴다: 「집을 수 있는 것」이 몇인지가 이 목록의 진짜 답이다. */
    const 점유판 = 점유(낼것.map((r) => r.id));
    const 셈 = 점유셈(점유판.표, 낼것.length);
    console.log(`\n[마찰 신호] ${mode === 'open' ? '열림(아직 아무도 판정 안 함)' : '보류(판정하고 열어둔 것)'} ${낼것.length}건`
      + (점유판.모름
        ? `\n  ❔ 점유를 **못 쟀다**(${점유판.못본층.join(' · ')}) — 아래는 「빈 자리」가 아니라 **모름**이다(F207).`
        /* 🔑 셈 칸에는 «놓음 표식 글자»를 안 쓴다 (F612) — 이 줄은 사람이 보드 상태 칸에
         * 그대로 옮겨 적는 재료다. 표식과 수가 붙으면 그 순간 `놓은줄()` 이 그 줄을
         * 「이 트랙을 놓았다」로 읽고, 남이 그 위에 선언해도 board-guard 가 안 막는다.
         * 실측 2026-08-18: F612 가 그렇게 났다(「🎫1」). 산문 규약 대신 **발생원**을 막는다. */
        : `  =  🔒남이 잡음 ${셈.잡힘} · 놓임 ${셈.놓임} · 내 줄 ${셈.내줄} · **집을 수 있는 것 ${셈.빈자리}**`)
      /* 🔑 「집을 수 있는 것 N」 **바로 아래**에 분모를 붙인다(F619 남은 일) — 떨어뜨려 두면
       *   읽는 사람이 두 줄을 잇지 않고, 안 이어진 분모는 없는 분모다. 못 잰 갈래(`모름`)일 때도
       *   낸다: 그때는 「메인만 보고 냈다」가 참이라 오히려 더 필요하다. */
      + '\n' + 가지분모줄(점유판.가지분모)
      + (원격몫줄(점유판.원격몫) ? '\n' + 원격몫줄(점유판.원격몫) : '') + '\n');
    const 착지 = mode === 'open' ? 착지한고침(낼것.map((r) => r.id)) : new Map();
    for (const r of 낼것) {
      console.log(`  ${r.id}  ${r.date}  [${r.kind}]  ${mode === 'open' ? r.signal : r.resolved}`);
      const p = 점유판.표.get(r.id);
      if (p && p.상태 === '잡힘') {
        console.log(`      🔒 **이미 잡혔다** — ${p.자리.join(' · ')}`);
        console.log('         → 다른 자리를 고른다(중복 구현은 되돌릴 수 없다). 그 세션이 죽었다고 보이면'
          + ' `node tools/작업본소유자.js` 로 생사부터 본다.');
      } else if (p && p.상태 === '놓임') {
        console.log('      🎫 놓인 줄이 이 번호를 든다 — **점유가 아니다**(주인이 「나는 이걸 안 한다」고 놓았다 · 집어도 된다).');
      } else if (p && p.상태 === '내줄') {
        console.log('      · 내 선언이 이 번호를 든다 — 내 트랙이다.');
      }
    ...
    if (mode === 'open') {
      ...
      /* ⚠ 이 문장은 **층 목록**이라 층이 늘면 같이 늘어야 한다 — 안 늘리면 「두 층만 봤다」고
       *   정직하게 적힌 화면이 실은 세 층을 본 것이 되고, 반대로 층을 빼면 안 본 층을 봤다고 말한다.
       *   `tests/장부점유.test.js` 가 이 문장과 `활성줄전량` 의 층 수를 맞춰 본다(F628). */
      console.log('  🔒 점유는 **메인 보드 + 미머지 가지 + `origin/master`** 세 층을 함께 본다(F521·F628) — 코드 트랙의'
        + ' 선언은 가지 안에 앉고(F610 실측: 잡힌 4건이 전부 가지에만 있었다), 단발 트랙의 선언은 master 에 직접 앉는다.');
      /* 🔑 **분모는 순수 함수 한 벌에서만 나온다**(`가지분모줄`·`원격몫줄`).
       * 🔴 여기 인라인 사본이 하나 있었고 **그게 정확히 F608 의 병이었다** — 그 사본은
       *   `점유판.가지출처` 를 읽는데 정본이 그 칸을 `가지분모` 로 고친 뒤라, 값이 멀쩡히 있는데도
       *   화면엔 「❔ 훑은 가지 수를 못 쟀다」가 떴다(2026-08-18 실측). 새는 방향이 **거짓 모름**이라
       *   시끄러운 쪽이었던 것이 운이었을 뿐, 갈라진 사실은 같다. 그래서 사본을 지우고 정본을 부른다. */
      console.log(`  ${가지분모줄(점유판.가지분모).trim()}`);
      console.log('        ⚠ 원격 ref 가 낡으면 그만큼 못 본다(fetch 시점은 이 도구 밖이다 · 장부 619 ㉢).');
      const 원격줄 = 원격몫줄(점유판.원격몫);
      if (원격줄) console.log(`  ${원격줄.trim()}`);
      console.log('     ⚠ 신고문을 «인용만» 한 줄도 잡힌다 — 방향은 「못 고름」이라 안전한 쪽이고,'
        + ' 그래서 조용히 지우지 않고 **어디에 있는지**를 함께 낸다(대기열 F406 과 같은 대가).');
    }
    console.log('');
    return;
  }
...
module.exports = {
  ...
  // F610 — 「이 번호를 지금 누가 잡고 있나」. 판정과 셈을 갈라 둔다(셈은 분모를 낸다 · F543·F207)
  점유, 점유셈, 가지분모줄, 원격몫줄,
