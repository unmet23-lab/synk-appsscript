...
const 누끼 = !!인자['누끼'];
const 방 = path.join(루트, 'docs', '캐릭터', '친구공방_0825', ...(누끼 ? ['누끼'] : []));
const 접두 = '까몽';
/* 무엇을 굽나 — 기본은 «확정 3장»이다(유호 확정 08-26 「3장에서 멈춘다」).
 *   우34·좌34 는 뺐다: 장당 100분대라 값이 안 맞는다는 판정이 섰다.
 *   `--표정 놀람,윙크,졸림` 으로 다른 목록을 줄 수 있다 — 새 감정 시안이 그 통로로 돈다. */
const 확정셋 = ['본체', '눈감음', '눈웃음'];
