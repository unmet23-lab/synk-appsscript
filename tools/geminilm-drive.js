#!/usr/bin/env node
/**
 * 제미나이LM **자동 갱신** 묶음 — 구글 드라이브 폴더로 굽는다.
 * (2026-08-22 개명: 「노트북LM」은 구글이 바꾼 옛 이름. repo 안에서는 여기부터 제미나이LM.)
 *
 * 왜 PDF인가 (실측 08-04):
 *   제미나이LM의 Drive 소스 피커가 받는 형식은 **문서·프레젠테이션·스프레드시트·PDF·폴더**뿐이다.
...
const E = require('./geminilm-export.js');   // 게이트·수집 규칙을 공유한다
...
<div class="머리">
  ...
  다시 만들기: <code>node tools/geminilm-drive.js</code> (아래 각 절의 제목이 원본 파일 경로다)
</div>
${본문}
</body></html>`;
}
