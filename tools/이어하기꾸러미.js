...
/* 아직 깃이 «모르는» 새 파일 — 09-05 에 드러난 구멍이다.
 *
 * 🔴 그날 실측: 마린 후보 64장 + 후보 폴더 둘 = **1.2GB 가 어디에도 없었다.**
 *   깃은 아직 모르고(새 파일), 꾸러미는 `git ls-files` 만 담아서 안 담고,
 *   구글 드라이브의 원본 자리(`tools/공방드라이브.js` 가 쓰는 곳)에도 없었다.
 *   ⇒ 이 노트북이 사라지면 그날 구운 그림이 통째로 사라진다. 다시 굽는 것은 «같은 것»이
 *   아니다 — 제미나이는 같은 말을 줘도 다른 판을 내고, 장당 돈이 든다.
 *
 * 🔑 그래서 «확정된 것만»이라는 자와 따로 선다: 이쪽은 확정 여부가 아니라
 *   **「지금 잃으면 되찾을 길이 있나」**를 묻는다. 후보는 확정이 아니지만 되찾을 길이 없다.
 *
 * .gitignore 가 막은 것은 여기 안 들어온다(다시 구울 수 있는 생성물이라 막힌 것들이다).
 * 즉 여기 담기는 것은 «곧 확정되거나 곧 버려질, 아직 판정 전인 것»뿐이다. */
function 깃밖새파일() {
  const out = execFileSync('git', ['status', '--porcelain', '-z', '--untracked-files=all'],
    { cwd: 저장소, maxBuffer: 1 << 28 });
...
/* 꾸러미 쪽에만 남은 것을 지운다 — «지금 깃 밖에 있는 것»의 거울로 유지한다.
 * 안 지우면 유호님이 후보 하나를 고르신 뒤에도 버린 63장이 꾸러미에 영영 남아,
 * 「어느 게 현행인가」가 다시 갈린다(이 저장소가 사본을 싫어하는 그 까닭). */
function 거울맞추기(방, 있어야할것) {
  if (!fs.existsSync(방)) return 0;

  /* 🔴 «0건»은 성공의 얼굴을 하고 온다. `git status` 가 어떤 까닭으로든 빈 출력을 내면
   *   있어야할것이 빈 배열이 되고, 그러면 이 함수가 **멀쩡한 백업을 통째로 지운다.**
   *   지우는 쪽은 되돌릴 수 없으니, 「비었다」를 만나면 지우기 전에 자를 먼저 의심한다.
   *   ⇒ 담을 것이 0인데 꾸러미에는 파일이 남아 있으면 손대지 않고 알린다.
   *     (깃 밖 파일이 진짜로 0이 된 날에는 다음 실행이 세어서 정상으로 지운다 —
   *      한 번 늦게 지우는 비용은 잘못 지우는 비용보다 훨씬 싸다) */
  if (있어야할것.length === 0) {
    let 남은것 = 0;
    const 세기 = (여기) => {
      for (const e of fs.readdirSync(여기, { withFileTypes: true })) {
        if (e.isDirectory()) 세기(path.join(여기, e.name)); else 남은것 += 1;
      }
    };
    세기(방);
    if (남은것 > 0) {
      console.log(`   ⚠ 담을 것이 0인데 꾸러미에는 ${남은것}벌이 있다 — 세는 자를 의심해 «지우지 않았다».`);
      return 0;
    }
  }

  const 살릴것 = new Set(있어야할것.map((p) => path.join(방, p)));
  let 지움 = 0;
  const 훑기 = (여기) => {
    for (const e of fs.readdirSync(여기, { withFileTypes: true })) {
      const 자리 = path.join(여기, e.name);
      if (e.isDirectory()) { 훑기(자리); try { fs.rmdirSync(자리); } catch { /* 안 비었다 */ } }
      else if (!살릴것.has(자리)) { try { fs.unlinkSync(자리); 지움 += 1; } catch { /* 못 지웠다 */ } }
    }
  };
  훑기(방);
  return 지움;
...
| \`4_아직깃밖\` | **아직 저장소에 안 담긴 새 파일**(그날 구운 후보 그림 같은 것). 확정 전이라 깃에는 없지만 잃으면 되찾을 길이 없어서 여기 둡니다. 확정되거나 버려지면 이 폴더에서도 사라집니다 |
...
function main() {
  ...
  const 파일들 = 추적파일();
  const 깃밖목록 = 깃밖새파일();
  /* 🔒 가드는 «두 목록 다» 훑는다 — 깃 밖 파일이야말로 .gitignore 의 보호를 안 받는 자리다. */
  const 샌것 = [...파일들, ...깃밖목록].filter(금지인가);
  if (샌것.length) {
  ...
  console.log(`📦 담을 것 — 글 ${문서목록.length}벌 ${사람크기(문서바이트)} · 확정 자산 ${자산목록.length}벌 ${사람크기(자산바이트)} · 아직 깃 밖 ${깃밖목록.length}벌 ${사람크기(깃밖바이트)}`);

  if (재기만) {
    console.log(`   원드라이브(글만)  ≈ ${사람크기(문서바이트)}`);
    console.log(`   구글드라이브(전부) ≈ ${사람크기(문서바이트 + 자산바이트 + 깃밖바이트)}${이력도 ? ' + 저장소 이력 ≈ 4.8GB' : ''}`);
  ...
  const 상태 = {
    ...
    문서수: 문서목록.length, 자산수: 자산목록.length, 깃밖수: 깃밖목록.length,
  ...
  for (const 곳 of 갈곳) {
    ...
    /* 🔴 09-05 저녁 수리 — 이 갈래는 **더하기만 했다.** 저장소에서 지운 파일이 꾸러미에는
     *   영원히 남아, 노트북을 잃고 이 꾸러미로 이어하는 사람이 «지운 옛 판»을 현행으로 읽는다.
     *   그날 실제로 그랬다: 옛 마스코트 세트를 저장소에서 걷었는데 꾸러미에는 그 설명 문서가
     *   그대로 살아 있었다(손으로 지웠다). 아래 `거울맞추기` 는 「깃 밖」 갈래에만 걸려 있었고,
     *   정작 «글»과 «확정 자산» 갈래가 새고 있었다.
     *   🔑 안전은 그 함수가 이미 든다 — 담을 것이 0이면 지우지 않고 알린다(자를 먼저 의심). */
    let 걷음 = 0;
    for (const [하위, 그갈래] of [['1_문서', 목록.filter(문서인가)], ['2_확정자산', 목록.filter((r) => !문서인가(r))]]) {
      if (곳.글만 && 하위 === '2_확정자산') continue;
      걷음 += 거울맞추기(path.join(곳.뿌리, 하위), 그갈래);
    }

    상태.곳[곳.이름] = { 담음: 목록.length, 새로쓴것: 쓴것, 그대로: 건너뛴것, 크기: 사람크기(바이트), 오류, 걷음 };
    console.log(`✅ ${곳.이름} — ${목록.length}벌 ${사람크기(바이트)} (새로 쓴 것 ${쓴것} · 그대로 ${건너뛴것}${걷음 ? ' · 사라져서 걷은 것 ' + 걷음 : ''}${오류 ? ' · 실패 ' + 오류 : ''})`);

    /* 아직 깃 밖인 것 — 구글드라이브에만 둔다(용량이 크고, 대개 곧 확정되거나 곧 버려진다). */
    if (!곳.글만) {
      const 방 = path.join(곳.뿌리, '4_아직깃밖');
      let 밖쓴것 = 0, 밖바이트 = 0;
      for (const 상대 of 깃밖목록) {
        const r = 복사(path.join(저장소, 상대), path.join(방, 상대));
        if (r.오류) continue;
        밖바이트 += r.바이트;
        if (!r.건너뜀) 밖쓴것 += 1;
      }
      const 지움 = 거울맞추기(방, 깃밖목록);
      상태.곳[곳.이름].아직깃밖 = { 담음: 깃밖목록.length, 크기: 사람크기(밖바이트), 새로쓴것: 밖쓴것, 지움 };
      console.log(`   📥 아직 깃 밖인 것 ${깃밖목록.length}벌 ${사람크기(밖바이트)} (새로 쓴 것 ${밖쓴것}${지움 ? ' · 사라져서 지운 것 ' + 지움 : ''})`);
    }
  }
