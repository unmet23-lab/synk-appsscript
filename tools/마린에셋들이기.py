...
def _시안(bpy, math, random, g):
    ...
    if 우주복글 and 우주복글 not in ('0', '끔'):
        ...
        if _인자.get('이마', '0') not in ('0', '끔'):
            이마z = float(_인자.get('이마높이', '0')) or (float(머리Pn[:, 2].max()) - 0.28)
            ...
            if len(띠f) > 40:
                ...
                bpy.ops.mesh.primitive_uv_sphere_add(radius=float(_인자.get('이마반', '0.075')),
                ...
                짧은퍼(방울f, 이마색, 살재질(이마색, 색[이마색]), 털재질(이마색, 색[이마색]),
                     길이=0.02, 개수=3000)    # 0.035/5200 은 이마를 삼키는 털구름이 됐다(24차)
                붙인것.append(방울f)
        ...
        for 반g, 위g, 크g in (((0.36, 0.14, 0.38), (bx, by + 0.46, 1.36), 0.26),
            ...
            붙인것.append(cg)
            상자들h.append((위g, 반g))
        for 부 in (-1, 1):                         # 어깨끈 — 몸통 앞면 곡선에 세 토막으로 스냅
            xs2 = bx + 0.23 * 부
            for zs2 in (1.32, 1.49, 1.66):
                남2 = max(0.0, 1.0 - abs((xs2 - bx) / 0.42) ** 2.4 - abs((zs2 - 1.38) / 0.44) ** 2.4)
                ys2 = (by + 0.02) - 0.36 * (남2 ** 0.42)
                끈g = 베개몸((0.062, 0.036, 0.115), 위치=(xs2, ys2, zs2), 크리스=0.45, 레벨=2)
                끈g.data.materials.append(가방재h)
                붙인것.append(끈g)
