...
def _시안(bpy, math, random, g):
    ...
    if 우주복글 and 우주복글 not in ('0', '끔'):
        ...
        for 부 in (-1, 1):
            조각h((0.16, 0.16, 0.34), (bx + 0.20 * 부, by + 0.01, 0.62), 복재)          # 다리(허벅지까지)
        ...
        조각h((0.30, 0.26, 0.09), (bx, by + 0.03, z0h + 올림h - 0.06), 복재, 크리스=0.40)
        # 볏 — 스파르타 선생님의 서명(유호 08-28 「스파르타 선생님처럼」). 투구 위 모호크:
        # 마디 일곱을 앞→뒤로 세우되, 높이는 «투구 실표면»에 마디마다 스냅한다(호를 손으로 안 긋는다).
        if _인자.get('볏', '0') not in ('0', '끔'):
            볏색 = _인자.get('볏색', 'Ink Deep')
            볏재 = 펠트재질('볏', 색[볏색], 배율=1.0) if 펠트로 else 매끈재질('에셋_볏', 색[볏색], 거칠기=0.9)
            # 🔑 볏의 축은 세계 y 가 아니다 — 세계 y 로 세우면 z30 돌린 머리 위에서 카메라가 칼날의
            #   정면을 봐 안테나 한 줄이 된다(17·18차). 머리 점군 xy PCA 로 재면 이 헬멧에선 «가로»가
            #   나온다(폭이 깊이보다 크다) — 가로 볏 = 켄투리온·스파르타 «지휘관»의 표식이라 그대로 채택.
            중m = 머리Pn[:, :2].mean(0)
            _, _, Vtm = _np.linalg.svd(머리Pn[:, :2] - 중m, full_matrices=False)
            축m = Vtm[0] / max(1e-9, _np.linalg.norm(Vtm[0]))
            if 축m[1] < 0:
                축m = -축m                            # 뒤(+y 쪽)를 양의 방향으로
            돌m = math.degrees(math.atan2(float(축m[1]), float(축m[0]))) - 90.0
            투m = (머리Pn[:, :2] - 중m) @ 축m
            s0m, s1m = float(_np.percentile(투m, 6)), float(_np.percentile(투m, 94))
            곁투m = (머리Pn[:, :2] - 중m) @ _np.array([-축m[1], 축m[0]])
            볏선 = 0
            for i in range(9):
                t = i / 8.0
