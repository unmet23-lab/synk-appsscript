...
def _시안(bpy, math, random, g):
    ...
    if 우주복글 and 우주복글 not in ('0', '끔'):
        ...
        if _인자.get('이마', '0') not in ('0', '끔'):
            이마z = float(_인자.get('이마높이', '0')) or (float(머리Pn[:, 2].max()) - 0.24)
            띠f = 머리Pn[(abs(머리Pn[:, 2] - 이마z) < 0.09) & (abs(투m - (s0m + s1m) * 0.5) < 0.16)]
            if len(띠f) > 40:
                깊f = 띠f[:, :2] @ 수직m
                앞f = float(_np.percentile(깊f, 97))
                cf = 중m + 수직m * (앞f - float(중m @ 수직m) + 0.012)
                Df = mathutils.Vector((float(수직m[0]), float(수직m[1]), 0.50)).normalized()
                qf = Df.to_track_quat('Y', 'Z')
                이마색, 이마심색 = _인자.get('이마색', 'Butter'), _인자.get('이마심색', 'Coral')
                for 반f, 밀f, 색f in (((0.105, 0.034, 0.105), 0.0, 이마색),
                                    ((0.050, 0.032, 0.050), 0.030, 이마심색)):
                    재f = (펠트재질('이마' + 색f, 색[색f], 배율=1.0) if 펠트로
                          else 매끈재질('에셋_이마' + 색f, 색[색f], 거칠기=0.85))
                    코f = 베개몸(반f, 위치=(float(cf[0]) + float(Df[0]) * 밀f,
                                        float(cf[1]) + float(Df[1]) * 밀f,
                                        이마z + float(Df[2]) * 밀f), 크리스=0.55, 레벨=2)
                    코f.rotation_mode = 'QUATERNION'
                    코f.rotation_quaternion = qf
                    코f.data.materials.append(재f)
                    붙인것.append(코f)
                print('이마 배지: z %.2f · %s/%s' % (이마z, 이마색, 이마심색))
