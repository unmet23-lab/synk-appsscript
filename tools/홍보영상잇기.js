...
const 조각들 = [
  ...
  { 컷: 7, 시작: 1.4, 길이: 3.4, 왜: '★까몽이 — 앞 1.4초는 정사각 그림을 넣은 검은 띠라 버린다', 강조: true },
  /* 🔴 09-05 유호 제안 — 「옆으로 움직였을 때 액자로 SYNK LAB 한국어 2027 2 11 이거 펠트로
   *   이쁘게 액자형식으로 벽에 붙여놔도 좋을것같은데?」. 장면이 «바뀌는» 엔딩 대신 영상 «안»에서
   *   정보가 나온다. 🔑 액자를 굽지 않고 여기서 얹는 까닭 = 저쪽은 글자를 못 지킨다(세 번 실측). */
  /* 🔴 09-05 유호 확정 「엔딩 화면은 빼고 액자로 끝내자」 — 따로 서던 마무리 화면을 걷었다.
   *   장면이 «바뀌지» 않고 영상 안에서 그대로 닫힌다. 그래서 이 조각이 길고, 끝에 페이드아웃이 붙는다. */
  { 컷: 8, 시작: 1.4, 길이: 6.5, 왜: '★둘이 나란히 · 벽에 펠트 액자 · 여기서 닫는다', 강조: true, 액자: true },
];
const 끝페이드 = 2.2;   // 초 — 마지막을 검게 닫는다(옛 마무리 화면이 하던 일)
...
(async () => {
  ...
  /* 마지막을 검게 닫는다 — 따로 서던 마무리 화면을 걷었으므로 그 일을 여기가 한다. */
  전환.push(`[이은것]fade=t=out:st=${(누적 - 끝페이드).toFixed(3)}:d=${끝페이드}[vout]`);
