 ...
 *
 * ■ 「이미 지났다」를 무엇으로 아나
 *   장부(`docs/_ops/몽골어검문.jsonl`)의 `대상지문` 이다 — 검문에 넘긴 파일 «전체»의 지문
 *   (한국어 + 구분선 + 몽골어). 그래서 **한쪽만 바뀌어도 다시 태운다.** 그게 맞다 — 고친 판은 다른 글이고,
 *   짝이 바뀐 것도 다른 시험이다.
 *   🔴 몽골어 지문만 보면 «같은 몽골어인데 짝이 되는 한국어가 바뀐 줄»을 「이미 쟀다」로 접는다(09-07 실측).
...
/* 🔴 **검수표가 «검수자에게 하는 말»을 걷는다 — 그건 문안이 아니다** (09-07 · 유호 「표식 걷고 다시 재줘」).
 *   검수표는 칸 안에 두 가지 표시를 섞어 둔다:
 *     · `⚠A`·`⚠C` — 표 아래 주석을 가리키는 꼬리표(브랜드 낱말·게임 차용어·유호 확정 등)
 *     · 백틱 — 「이 줄은 문안 그대로다」라는 마크다운 표시
 *   둘 다 **학생 화면에는 안 나간다.** 그런데 걷지 않고 검문에 넘겼더니 그대로 읽혔다 —
 *   「콜론 뒤에 오류 기호(⚠A)가 남아 있어 문맥이 완전히 깨져 있다」가 실제로 나온 판정문이다.
 *   ⚠ 자리표시자(`{이름}`·`{N}`)와 이모지는 **안 걷는다** — 그건 실제 문안에 있는 실물이다. */
function 표식걷기(s) {
  return String(s)
    .replace(/\s*⚠[A-Z]\b/g, '')   // 주석 꼬리표
    .replace(/`/g, '')             // 마크다운 코드 표시
    .trim();
}
...
function 표에서뽑기(파일이름) {
  ...
  while (i < 줄들.length) {
    ...
    for (const c of 몸통) {
      const ko = 표식걷기((c[한칸] || '').replace(/\*\*/g, ''));
      const mn = 표식걷기((c[키칸] || '').replace(/\*\*/g, ''));
      if (!한글있나(ko) || !몽골어인가(mn)) continue;   // 짝이 안 서는 줄은 검문할 것이 없다
      const 한벌 = { 파일: 파일이름, 표: 표번호, 칸: 이름, ko, mn };
      짝.push({ ...한벌, 지문: 지문(mn), 짝지문: 지문(넘길글(한벌)) });
    }
  }
  return { 짝, 건너뛴표 };
}

/** 검문에 넘기는 파일 한 벌의 내용. 지문을 이 꼴로 계산해야 장부와 맞는다. */
const 넘길글 = (s) => `${s.ko}\n---\n${s.mn}\n`;

/** 장부에 이미 있는 «대상지문» 모음 = 한국어와 몽골어를 «짝으로» 잰 기록.
 *
 * 🔴 **번역지문(몽골어)만 보면 안 된다** (09-07 셋째 회차에 밟았다).
 *   몽골어 지문만 대조하면 «같은 몽골어인데 짝이 되는 한국어가 바뀐 줄»을 「이미 쟀다」로 접는다.
 *   실제로 그랬다: 칸 고르는 자를 고치자 AI활용문안의 한국어 칸이 바뀌었는데, 몽골어는 그대로라
 *   옛 짝으로 낸 「뜻이 크게 다르다」 판정이 새 짝의 결과인 척 남아 있었다.
 *   ⇒ 검문자가 적는 `대상지문`(넘긴 파일 «전체»의 지문)으로 대조한다. 한쪽만 바뀌어도 달라진다. */
function 잰것들() {
  const 셋 = new Set();
  if (!fs.existsSync(장부)) return 셋;
  for (const l of fs.readFileSync(장부, 'utf8').split(/\r?\n/)) {
    if (!l.trim()) continue;
    try { const j = JSON.parse(l); if (j.대상지문) 셋.add(j.대상지문); } catch { /* 깨진 줄은 넘긴다 */ }
  }
  return 셋;
}
...
function 세기() {
  const { 짝, 겹침, 건너뛴표 } = 모으기();
  const 잰 = 잰것들();
  const 지난것 = 짝.filter((s) => 잰.has(s.짝지문));
...
async function 태우기() {
  ...
  const 잰 = 잰것들();
  const 다시 = 있나('--다시');
  let 대상 = 다시 ? 짝 : 짝.filter((s) => !잰.has(s.짝지문));
