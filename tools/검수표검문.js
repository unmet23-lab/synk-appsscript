...
function 표에서뽑기(파일이름) {
  ...
  while (i < 줄들.length) {
    ...
    /* 칸마다 «한글이 든 줄 수»와 «키릴이 든 줄 수»를 센다. 가장 많이 든 칸이 그 언어의 칸이다.
     * 🔑 머리 이름으로 고르지 않는 까닭: 표마다 이름이 다르고(「한국어」·「한국어 원문」·「자리」),
     *   이름이 맞아도 칸이 비어 있는 표가 있다(릴대본의 확정 칸). 실물이 자다. */
    const 칸수 = Math.max(...몸통.map((c) => c.length));
    const 한 = new Array(칸수).fill(0);
    const 키 = new Array(칸수).fill(0);
    for (const c of 몸통) {
      for (let k = 0; k < 칸수; k++) {
        const v = c[k] || '';
        if (한글있나(v)) 한[k]++;
    ...
    const 한칸 = 한.indexOf(Math.max(...한));
    const 키칸 = 키.indexOf(Math.max(...키));
    if (한[한칸] === 0 || 키[키칸] === 0 || 한칸 === 키칸) {
      건너뛴표.push(`${파일이름} 표${표번호}(줄 ${몸통.length} · ${한[한칸] === 0 ? '한국어 칸 없음' : 키[키칸] === 0 ? '몽골어 칸 없음' : '두 언어가 한 칸에'})`);
      continue;
    }
...
async function 태우기() {
  ...
  for (let n = 0; n < 대상.length; n++) {
    ...
    const 사유 = r.status === 1 ? String(글.trim().split('\n').pop() || '').slice(0, 90) : '';
    ...
    if (r.status === 1) console.log(`     확인 불가 — ${사유}`);
