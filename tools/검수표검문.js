...
/* 🔴 **자리표시자를 채워서 넘긴다 — 검수표가 이미 정해 둔 규약이다** (09-07).
 *   함께한날 검수표 §배너가 이렇게 적어 두었다:
 *     「검문에는 `{n}`=Бат · `{d}`=14 · `{s}`=5 를 채워 넣고 재고, 표에는 중괄호로 되돌려 적었습니다.」
 *   앞선 세션이 손으로 하던 일이라 이 도구가 안 따랐고, 그래서 검문이 중괄호를 결함으로 잡았다 —
 *   「플레이스홀더가 그대로 포함되어 있어 파손된 텍스트입니다」가 실제 판정문이다.
 *   실제 학생 화면에서는 그 자리에 이름과 숫자가 들어가므로, 채워서 재는 쪽이 실물에 가깝다.
 *
 *   🔴 `{f}`(문형)와 `{g}`(가이드 이름)는 **안 채운다** — 실물 값이 검수표에 없다.
 *     모르는 값을 지어내면 「이 글자로 재봤다」가 거짓이 된다. 그 둘이 든 줄은 중괄호인 채로 잰다.
 *   ⚠ 화면과 표에는 «원문»이 그대로 뜬다 — 채운 값은 검문에 넘기는 글에만 산다. */
const 자리값 = {
  '{n}': 'Бат', '{이름}': 'Бат',   // 학생 이름 (검수표 규약 = Бат)
  '{d}': '14', '{s}': '5',          // 날수 · 장면 번호 (검수표 규약)
  '{N}': '3', '{m}': '7',           // 횟수·점수 · 맞힌 말 개수
  '{반}': 'A',                      // 반 이름
};
function 자리채우기(s) {
  let t = String(s);
  for (const [k, v] of Object.entries(자리값)) t = t.split(k).join(v);
  return t;
}

/** 검문에 넘기는 파일 한 벌의 내용. 지문을 이 꼴로 계산해야 장부와 맞는다. */
const 넘길글 = (s) => `${자리채우기(s.ko)}\n---\n${자리채우기(s.mn)}\n`;
