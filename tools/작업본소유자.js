...
function 조사() {
  ...
  const 항목 = 파일들.map(({ 파일: f, 트리 }) => {
    ...
    if (트리) return { 파일: f, 트리, 종류: 판정.다른트리, 주인: [], 함께: [] };
    if (나 && 산주인.some((s) => s.sid === 나)) return { 파일: f, 트리, 종류: 판정.내것, 주인: [], 함께 };
    const 남 = 산주인.filter((s) => s.sid !== 나);
    if (남.length) return { 파일: f, 트리, 종류: 판정.남의살아있는, 주인: 남, 함께 };
    ...
    if (주인.length) {
      ...
      const 바뀐때 = 파일시각(f);
      if (바뀐때 !== null && 바뀐때 <= 박동 + 유물여유_분 * 60000) return { 파일: f, 트리, 종류: 판정.끝난세션, 주인, 함께 };
      return { 파일: f, 트리, 종류: 판정.모름, 주인: [], 함께, 반증: { 주인, 잰것: 바뀐때 !== null } };
    }
    return { 파일: f, 트리, 종류: 판정.모름, 주인: [], 함께 };
  });
  return { git못부름: false, 나, 항목, 세션수: ss.filter(살았나).length };
}
