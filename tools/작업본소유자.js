...
function 조사() {
  ...
  const ss = 세션들();
  const 파일들 = 미커밋들();
  if (파일들 === null) return { git못부름: true, 나, 항목: [], 세션수: ss.length, 못본형제: [], 원격: null, 보드: null };
  ...
  return { git못부름: false, 나, 항목, 세션수: ss.filter(살았나).length, 못본형제, 원격: 원격대기(), 보드: 보드선점() };
}
...
function 보고(r, { 훅 }) {
  ...
  if (훅 && !위험.length && !다른트리.length && !형제모름.length && !r.못본형제.length
      && !원격들.length && !원격못봄 && !보드들.length) return '';
...
module.exports = { 조사, 보고, 판정, 세션들, 살았나, 짧게, 형제세션들 };
