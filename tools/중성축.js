 ...
 *   슬롯을 합칠지는 킷 구조 변경이라 유호님 확정 사안이다(개수 잠금 = tests/토큰정본.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)).
 *
 * 쓰기:
...
const 갈래 = [
  ...
  { 구: 'Slate',    구hex: '#8A93AD', 신: 'Ash',        팔레트: '06 Ash',             바닥: '다크',   밝은쪽: true,  허용바닥: ['Graphite', 'Graphite 2', 'Graphite 3'], 직책: '보조 잉크 2(3번째 글자 층) — 바닥은 Graphite·Graphite 2·Graphite 3만' },
  { 구: 'Slate 2',  구hex: '#5F657D', 신: 'Ash 2',      팔레트: '06 Ash',             바닥: '라이트', 밝은쪽: false, 허용바닥: ['Paper', 'Chalk', 'Chalk 2'], 직책: '보조 잉크 2(3번째 글자 층) — 바닥은 Paper·Chalk·Chalk 2만' },
];

/* 「죽은 색」 — tests/브랜드색.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다) 의 DEAD 와 같은 목록. 같은 판정을 두 곳에 적으면 갈라지므로
...
/**
 * 🔴 **바닥 게이트** — 「자기 바닥」이 아니라 «실제로 앉는 판 전부»에 대해 4.5 를 넘긴다 (2026-08-19).
 *
 * 왜 생겼나(실측): 유도는 「각 색이 자기 바닥에 대해 갖던 대비」를 보존한다. 그건 맞게 잰 것이다.
 *   그런데 지면에서 Ash 2 는 바닥(Paper)이 아니라 **Chalk 판 위**에도 앉는다 —
 *   구 Cream(#F6F1E8)은 새 Chalk(#E4E4E7)보다 밝아서 그 쌍이 통과했고, 무채로 옮기며 판이
 *   어두워지자 **4.46** 으로 내려앉았다. 그래서 렌더린트가 실지면 **51건**을 물었다(§9).
 *   ⇒ 한 갈래만 재서 전체를 오단정한 전형(F183)이고, 빠진 자는 「색 × 색」이 아니라
 *     **「그 색의 직책이 이름 댄 판 전부」**다. 그 목록은 이미 `직책` 산문에 있었는데
 *     **산문이라 기계가 못 읽었다** — `허용바닥` 으로 데이터화한 것이 이 고침의 절반이다.
 *
 * ⚠ 틀릴 때의 모습: `허용바닥` 이 낡으면(새 판이 생겼는데 목록에 안 적히면) 그 쌍은 그대로 샌다.
 *   ⇒ 게이트는 **움직인 것을 찍는다**(아래 로그). 조용히 옮기면 다음 사람이 값의 출처를 못 찾는다.
 */
function 바닥게이트(표, 소리 = false) {
  const hex = Object.fromEntries(표.map((r) => [r.신, r.새hex]));
  for (const r of 표) {
    if (!r.허용바닥 || !r.허용바닥.length) continue;
    const 판들 = r.허용바닥.map((n) => hex[n]).filter(Boolean);
    if (판들.length !== r.허용바닥.length) {
      throw new Error(`허용바닥 이름이 유도표에 없다: ${r.신} → ${r.허용바닥.join('·')}`);
    }
    const 최소 = () => Math.min(...판들.map((b) => 대비(r.새hex, b)));
    if (최소() >= 4.5) continue;
    const 시작 = r.새hex;
    /* 잉크는 «바닥에서 멀어지는» 쪽으로만 간다 — 밝은 바닥 위 잉크는 어둡게, 어두운 바닥 위는 밝게. */
    let v = parseInt(r.새hex.slice(1, 3), 16);
    for (let i = 0; i < 256 && 최소() < 4.5; i += 1) {
      v += r.밝은쪽 ? 1 : -1;
      if (v < 0 || v > 255) break;
      r.새hex = 색조입히기(v, r.바닥 === '다크' ? hex.Graphite : hex.Paper, r.밝은쪽);
    }
    r.바닥게이트 = { 시작, 끝: r.새hex, 최소대비: Number(최소().toFixed(2)), 판: r.허용바닥.join('·') };
    if (소리) {
      console.log(`  ⚙ 바닥 게이트 — ${r.신} ${시작} → ${r.새hex} `
        + `(${r.허용바닥.join('·')} 최소 ${최소().toFixed(2)})`);
    }
  }
  return 표;
}
...
/**
 * 🔴 **옛 «유도값»** — 유도 규칙이 바뀌어 물러난 값들.
 *
 * 왜 따로 있나: 이건 «구 킷 hex»(Slate 2 `#5F657D`)도 아니고 DEAD 도 아니다 —
 *   **한때 «새 hex» 였던 것**이다. 그래서 두 그물에 다 안 걸린다:
 *     · `유도()` 의 map 은 «구hex → 새hex» 라 이미 새 값으로 구워진 지면을 안 본다
 *     · `DEAD` 는 유호님이 폐기한 «디자인 결정»의 목록이지 유도 잔재의 목록이 아니다
 *   ⇒ 그 틈으로 지면이 조용히 낡는다(구운 자산은 옛 값을 든 채 초록으로 남는다).
 *   `중성축_지면전파.js` 가 이 표를 map 에 합쳐 그 자리를 덮는다.
 *
 * ⚠ 값을 다시 유도할 때마다 **여기에 한 줄을 더한다**. 안 더하면 그 회차의 잔재가 영구히 남는다.
 */
const 옛유도 = {
  '#676767': { 신: 'Ash 2', 지금: '#666666', 사유: '바닥 게이트 신설 — Chalk 판 위 4.46 < 4.5 (2026-08-19 · 유호 확정 「진행」)' },
  '#8A8A8A': { 신: 'Ash', 지금: '#8C8C8C', 사유: '바닥 게이트 신설 — Graphite 3 위 4.38 < 4.5 (2026-08-19 · 같은 규칙이 잡은 곁가지)' },
};

function 유도(다크 = 기본다크, 라이트 = 기본라이트, 소리 = false) {
  const 표 = 갈래.map((g) => {
    const 구hex = g.구hex;
    if (g.고정) {
  ...
  /* 게이트는 **전부 유도된 뒤**에 돈다 — 허용바닥 자신이 유도값이기 때문이다. */
  바닥게이트(표, 소리);
  for (const r of 표) {
    if (r.고정) continue;
    const 새바닥 = r.바닥 === '다크' ? 다크 : 라이트;
    r.새대비 = 대비(r.새hex, 새바닥);
  }
  return 표;
}
...
function 적용(표) {
  ...
  /* 🔴 **구 이름«과» 새 이름 둘 다로 찾는다** (2026-08-19 실측 사고).
   *   옛 판은 `r.구`(= 'Slate 2') 로만 찾았다. 그래서 **한 번 반영한 뒤에는 영원히 아무것도 안 바뀐다** —
   *   토큰의 이름이 이미 'Ash 2' 라 map 이 안 맞는데, 함수는 `표.length` 를 돌려주므로
   *   화면에는 «11칸 반영» 이 찍힌다. 값이 0칸 바뀐 판과 글자가 똑같다.
   *   이 트랙에서 실제로 밟았다: 유도값을 고치고 `--적용` 했는데 토큰은 옛 hex 그대로였고,
   *   지면만 새 값으로 구워져 렌더린트가 「킷 밖 색 124건」으로 울었다. */
  const 맵 = Object.fromEntries(표.flatMap((r) => [[r.구, r], [r.신, r]]));
  ...
  토큰.색.킷 = 토큰.색.킷.map((c) => {
    const r = 맵[c.이름];
    if (!r) return c;
    const 새 = { 이름: r.신, hex: r.새hex, 팔레트: r.팔레트, 직책: r.직책 };
    if (JSON.stringify(새) !== JSON.stringify(c)) 바뀐칸 += 1;
    return 새;
  });
  ...
  /* 🔑 **찾은 수가 아니라 «바뀐 수»를 돌려준다** — 0 을 0 이라고 말해야 「반영했다」가 거짓이 안 된다. */
  return { 바뀐칸, 과녁: 표.length };
}

/* 회귀(tests/중성축.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다))가 이 유도를 다시 돌려 킷과 대조한다 — 값을 두 곳에 적지 않기 위해서다 */
module.exports = { 유도, 검산, 적용, 기본다크, 기본라이트, 갈래, 대비, Lstar, 채도폭, 옛유도};
...
if (argv.includes('--적용')) {
  const { 바뀐칸, 과녁 } = 적용(표);
  console.log(`\n${바뀐칸 ? '✅' : '·'} docs/디자인_토큰.json — 과녁 ${과녁}칸 중 **${바뀐칸}칸 바뀜**`
    + `(hex·이름·팔레트·직책 + 시맨틱·허용바닥 이름 참조)${바뀐칸 ? '' : ' — 이미 같다'}.`);
  console.log('   ▶ 다음: node tools/토큰빌드.js · DESIGN.md §2 표 · 컨셉 정본 §12 (옛 회귀 tests/브랜드색.test.js 의 DEAD 목록은 08-19 대청소로 걷혔다)');
} else {
  console.log('\n(파일은 안 건드렸다 — 반영하려면 --적용)');
}
