 ...
 *
 * 🔑 왜 필요한가:
 *   크레딧 시계가 «셋»인데 **먼저 죽는 것을 먼저 태워야 한다**(09-07 실측).
 *     · unmet23  ₩132,149 · 마감 **2026-11-13**  ← 가장 먼저 온다
 *     · 77yuhbs  ₩108,146 · 마감 2026-12-05  ← 자격은 ~/.synk-vertex-oauth.77yuhbs.json 에 보관
 *     · unmet27  ₩414,984 · 마감 2026-12-06  ← 09-07 에 충전 · 지금 도는 것
 *   안 태우고 11-13 을 넘기면 unmet23 것은 그냥 사라진다.
 *
 * 🔑 어떻게 갈리나 — `모델정책.js` 의 `붙인자격()` 이 이렇게 고른다:
 *     `~/.synk-vertex-oauth.json` **이 있으면** 그 계정(= unmet27)
 ...
 *   node tools/굽기계정.js --새      # unmet27 (마감 12-06 · 09-07 에 충전한 40만)
...
/* 🔴 09-07 에 지갑이 셋이 됐다 — 유호님이 unmet27 에 40만원을 충전하시고 「전부 통로 바꿔줘」 하셨다.
 *   `새` 가 지금 도는 것이고, 77yuhbs 는 잔액 ₩108,146 이 남아 있어 되살릴 수 있게 자격을 따로 뒀다:
...
function 보여주기() {
  ...
  console.log(`\n🔥 굽기가 지금 태우는 지갑 = **${어디 === '옛' ? 'unmet23 (옛)' : 'unmet27 (새)'}**\n`);
...
(async () => {
  ...
  if (process.argv.includes('--확인')) {
    ...
    for (const 어디 of ['옛', '새']) {
      ...
      결과[어디] = r;
      const 표 = 어디 === '옛' ? 'unmet23 (옛)' : 'unmet27 (새)';
    ...
    else if (새끝 && (!옛끝 || 새끝 > 옛끝)) console.log(`   ✅ **새 지갑(unmet27)**에서 굽고 있다 — 새 ${새끝} > 옛 ${옛끝 || '없음'}`);
    else console.log('   ⏸ 최근 1시간에 그림을 구운 흔적이 양쪽 다 없다(굽기가 안 도는 것으로 보인다)');
    console.log(`\n   설정상으로는 «${지금어디() === '옛' ? 'unmet23 (옛)' : 'unmet27 (새)'}» 이다.`);
