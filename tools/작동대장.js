 ...
 *     · `docs/시스템_대장.md`  → 「무엇이 있나」 (명사 목록 · 대외 설명용 — 줄 수는 Ⅶ이 그때그때 센다)
 ...
 *   node tools/작동대장.js --바로가기   → 생성 + 바탕화면 「SYNK 방향」에 .lnk (이해대장 옆)
 ...
 *   발동 조건 = **주기(수요일) 재심이 도래한 세션 시작** · 부르는 곳 = `tools/대기열.js(⚠삭제됨 39019553 2026-08-20 — 지금 없다)` 의 `자동기록()`.
 *   위 `--기록` 은 이제 «손으로도 칠 수 있는 같은 통로»이고, 판정은 `기록하기()` 한 곳이 진다.
 *   🔑 왜 자동이냐 — 발동을 사람 손에 걸어 둔 유호 상시 지시가 08-10 이후 **네 층 전수 0번** 돌았다
 *      (`tools/lib/재심신호.js(⚠삭제됨 921109d7 2026-08-20 — 지금 없다)` 머리말 실측). 그리고 기록은 **놓치면 소급이 안 된다** — 지나간 주의
 ...
 * ⚠ 행 삭제 금지(CLAUDE.md append-only 장부 규약) — `tests/작동대장.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)` 가 형식을 지킨다. */
...
/* ── ② 이해 사슬 — docs/철학_실물현황.md 부록 A-1 ─────────────────────────────
 * 여기서 «다시 판정하지 않는다» — 정본 표의 ㉠㉡㉢ 행에서 「엔진에 닿는가」 열만 읽어
 * 초록/빨강만 가른다. 이 표의 원문은 관찰 시점의 실물현황이고 이 화면은 인용이다(설계 ①). */

function 사슬읽기() {
  const md = 입력('철학 실물현황', 'docs/철학_실물현황.md', (p) => fs.readFileSync(p, 'utf8'));
  if (md == null) return null;
  const 층들 = [];
  for (const line of md.split(/\r?\n/)) {
 ...
 *   회귀 = `tests/작동대장.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)` 의 픽스처(한글 파일명) — 실저장소가 깨끗해도 탐지력이 안 죽는다. */
...
function 적체읽기() {
  ...
  /* 🔑 2026-08-20 위임 2단계 — 열린 일감의 정본이 `docs/_ops/작업대기열.md(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)` 에서 **열린 PR** 로 옮겼다.
 ...
 *    `tests/장부유실.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)`). 합치는 것은 읽는 쪽(화면)이 한다.
...
function 그리기(d, 도장) {
  ...
  }).join('\n') : '<p class="none">철학 실물현황 A-1 을 못 읽었다.</p>';

  const 안잰축 = [
    '<b>부품 각 줄의 개별 작동</b>(줄 수는 Ⅶ이 센다) — 「부품 → 파일」 자동 매핑이 안 선다(대장의 «최근» 버전 태그로 커밋을 못 찾는다 · 실측 08-17: <code>v9.245</code> 실패). 그래서 다섯 통은 <b>장치 층만</b> 본다.',
    ...
    '<b>「학생이 실제로 더 잘하게 됐나」</b> — 이 도구는 실학생 효과를 측정하지 않는다. 실제 사용 자료로 별도 확인해야 한다.',
  ];
...
<p class="lead">이 사슬 표는 <code>docs/철학_실물현황.md</code> A-1의 <b>관찰 시점 기록</b>을 인용한다. 오늘의 구현·배포·효과는 현재 증거로 따로 확인한다.</p>
${사슬띠}
...
function 본체() {
  ...
  if (argv.includes('--바로가기')) {
     ...
     * 그 「옆」이 08-20 재편으로 **「SYNK 방향」**이 됐다 — 갈래를 명시해야 따라간다.
