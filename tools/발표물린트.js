...
/* ── 학생이 받는 인쇄물의 금칙 (설계 `docs/가져가는것_설계_v1.md` §9-b) ─────────
 * 🔴 왜 여기 있나: 이 계약을 지키던 `tests/숙제서클.test.js` 는 08-19 대청소에 삭제됐고,
 *   그 뒤로 지키는 것은 `contents_서클.js` 머리말의 주석 하나뿐이었다. 설계 §12-c 가
 *   「새 테스트를 세우지 말고 발표물린트에 한 줄 얹으라」고 처방했고 이 절이 그 한 줄이다.
 *
 * 🔑 **표식이 있는 지면에만 건다** — `<body data-synk-print="학생">`.
 *   무조건 걸면 사업계획서·소개서의 정당한 「평균」·「점수」가 전부 빨개진다. 그러면 사람이
 *   가드를 끈다(이 파일 첫 절이 「졌다」에서 이미 배운 교훈이다). 금지되는 것은 낱말이 아니라
 *   **그 낱말이 학생 손에 가는 종이에 있는 것**이다.
 *
 * 목록은 여기가 정본이고 `tools/증서조판.js` 가 이걸 받아 쓴다 — 두 곳에 적으면 갈라진다. */
const STUDENT_PRINT_MARK = 'data-synk-print="학생"';
const STUDENT_BANNED = [
  ['점수', '종이는 등수가 되는 순간 비교표가 된다'],
  ['정답률', '같은 목록'],
  ['정답 개수', '같은 목록'],
  ['등수', '같은 목록'],
  ['석차', '같은 목록'],
  ...
  ['상위', '「상위 N%」 — 비교의 다른 이름'],
  ['다른 학생', '비교 대상은 어제의 나뿐'],
  ['승급', '승급은 도달제라 미달이면 재회전한다 — 박으면 미달한 학생의 종이가 실패 통지서가 된다'],
  ['미달', '결핍을 종이에 적지 않는다'],
  ['난도', '난도 라벨 금지'],
];
...
function lint(file) {
  ...
  // 학생이 받는 종이일 때만 — 표식이 없는 지면은 이 검사의 분모 밖이다(위 절의 까닭).
  if (html.includes(STUDENT_PRINT_MARK)) {
    const 걸린 = STUDENT_BANNED.filter(([w]) => text.includes(w));
    if (걸린.length) {
      bad.push(`학생 인쇄물 금칙(설계 §9-b): ${걸린.map(([w, 왜]) => `${w} — ${왜}`).join(' · ')}`);
    }
    info.push(`학생 인쇄물 표식 있음 — 금칙 ${STUDENT_BANNED.length}종을 같이 봤다`);
  }
  ...
  /* [09-04] 자를 넓혔다 — 규칙의 «뜻»은 「원어민 검수가 아직 안 끝났음을 밝혔나」인데,
   *   옛 판은 「검수를 앞두고」라는 «한 표현»만 찾아서 같은 사실을 다른 말로 밝힌 지면을
   *   빨갛게 냈다(실측 09-04: 브로셔 586·로드맵 419 가 「원어민 검수는 아직입니다」로 밝히는데도 적색).
   *   자가 틀린 자리라 지면을 고치는 대신 자를 고친다. 「마쳤다」류는 여전히 통과 못 한다. */
  const 검수밝힘 = /검수를 앞두고|원어민 검수는\s*(?:<[^>]*>\s*)?아직/.test(text);
  if (/시냅스 코어|코어 \d권|교재/.test(text) && !검수밝힘) {
...
module.exports = { lint, 넘침, main, STUDENT_BANNED, STUDENT_PRINT_MARK };
