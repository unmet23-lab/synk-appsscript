...
**후속 실행판 0.5는 [발전 구현 보고](../advance-20260912/REPORT.md)를 먼저 봅니다.** 아래 전략과 4개 실행 근거는 0.4 당시의 자료입니다. 미지원 도움 능력 코드 문제는 0.5에서 불명 보존·정밀화로 수정했습니다.
...
자료만 읽거나 PDF·HTML을 보는 데는 서버가 필요 없습니다. 공식 출처 링크만 인터넷을 사용합니다. HTML의 작업실 링크는 같은 PC의 로컬 서버를 가리킵니다. 기존 evidence.json은 보존 소스를 선택하는 projection-replay.cjs로 검증합니다. reproduce.cjs를 지금 재실행하면 현재 소스를 사용하므로 당시0.4 원본을 덮어쓰는 용도로 사용하지 않습니다.
