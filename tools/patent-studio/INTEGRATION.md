...
2026-09-12. 이 문서는 로컬 시연과 기존 제품의 연결 지점을 구분한다. 이번 어댑터는 기존 제품 DB·학생 상태·학습자료 내보내기에 연결하지 않았다.

0.5는 이 로컬 경로에 시간 가능 범위·출처 정밀화·복수 보류 확인 계획을 통합했다. 실제 producer → consumer → output은 [발전 보고](advance-20260912/REPORT.md)의 연결표와 [API](API.md)에 있다. 기존 제품 DB 연결 여부는 이 확장으로 바뀌지 않는다.

## 엔진 귀속

이 작업실의 판단 기술은 **SYNK Core의 근거 자격·관측·재판정 모듈**에 귀속한다. Core가 원음·전사·도움·응답·시점 사건의 판본을 받아 용도별 자격을 계산하고, 보류를 풀 관측을 선택하며, 새 사실이 오면 관련 효과만 다시 판정한다.

SYNK Loom은 마스코트와 화면 배치, L1 재질 시스템을 맡는다. L1 안에서 Felt·Resin·Glass를 각각 재질 프로필로 관리한다. 이 작업실과 발표물에는 Loom과 Felt 프로필을 적용했지만, Loom은 근거 자격을 계산하거나 판정을 바꾸지 않는다. 제품 연결 때도 이 경계를 유지한다.
