...
if (require.main === module) {
  ...
  /* 🔑 `--` 낱말 판정은 **공용 통로**로 옮겼다 — 손 사본이 남아 있으면 한쪽만 `--키=값` 을
   *   다르게 다루고 갈라진 쪽이 조용히 통과한다(F401 축).
   * ⚠ 아래 `인자검사` 는 **그대로 둔다** — 이 도구는 값 인자의 «개수»까지 세서 위치 인자도
   *   거절하는데(통로는 `--` 만 본다), 그 엄격함을 통로로 갈아 끼우면 거기서 힘이 빠진다. */
  const 아는플래그 = [...값플래그, ...단독플래그];
  const 플래그오류 = 인자게이트('실행층점검', argv, 아는플래그);
  const 모름 = 인자검사(argv);
  /* 🔑 **차단문은 이 도구의 것 하나다.** 통로가 자기 문장으로 먼저 죽으면 「모르는 인자」라는
   *   이 도구의 낱말이 사라지고, 그 낱말을 무는 회귀(`tests/실행층점검.test.js`)가 빨개진다.
   *   통로는 «판정»을 주고 문장은 여기서 짓는다 — 그게 통로를 「목록이 아니라 판정만 공용」으로
   *   둔 이유와 같은 축이다. */
  if (플래그오류 || 모름.length) {
