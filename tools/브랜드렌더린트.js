...
const 마스코트콘텐츠 = {
  ...
  'docs/영상도구_비교_0814.html':
    '영상 도구 결정 재료 — 램프 5단이 **논증의 증거**다(「확정 hex 가 생성 모델에서 사라진다」를 보이는 자리). 쓰임은 11px 스와치 5개뿐이고 면·글자로는 안 쓴다. 유호님은 비개발자라 hex 글자만으로는 안 읽힌다(F055).',
  'docs/캐릭터/자개_캐러셀_예시_0814.html':
    'SNS 캐러셀(자개 액자판) — 위 「괜찮아요」 세트와 같은 콘텐츠·같은 쓰임이다. 체리는 장 번호 배지와 밑줄에만 있고 바탕에 체리 «면»은 없다(08-13 처방 그대로). 자개 카드 2장에는 마스코트도 밑줄도 없다 — 자개가 그 지면의 신호 1점이라서다.',
};
...
const 대상 = [
  ...
  'docs/엔진/Loom_소개서.html',                //   + 홈페이지 재료의 뿌리라 장수 문서다. 첫 등재 시 4벌 전부 0건 실측(2026-08-15).
  'docs/엔진/Vellum_소개서.html',
  'docs/엔진/SYNK_엔진_지도.html',
  'docs/엔진/Trail_소개서.html',               // 자산층(쌓는 셋) 소개서 3벌 — 지도 v2.0 편입(2026-08-15 · 유호 확정).
  'docs/엔진/Prism_소개서.html',               //   같은 갈래 = 같은 등록 — 4벌만 남기면 새 3벌은 CI 가 영원히 안 본다.
  'docs/엔진/Temper_소개서.html',              //   ⚠이 칸은 **두 번** 갈렸다 — v2.1 이름 확정(처방→Pilot 08-15) → v2.3 개명(Pilot→Temper 08-17).
...
function main(argv) {
   ...
   * ⚠ 위치 인자(파일 경로)는 `--` 로 안 시작하니 안 걸린다 · 종료 코드를 돌려주는 꼴이다. */
  const 아는플래그 = ['--json'];
  const 플래그오류 = 인자게이트('브랜드렌더린트', argv, 아는플래그);
