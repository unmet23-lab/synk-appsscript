...
function 한절올리기(절) {
  ...
  try {
    ...
    /* 🔴 [09-05] 끝에 쓸 때는 장부를 «다시» 읽는다 — 시작 때 사본으로 덮어쓰면 그 사이 다른 절이 적은 「끝」이
     *   지워진다(절 여럿을 동시에 돌리는 날 실제로 밟을 자리 · 유호 「한번에 다 해버리고 싶은데」). */
    const 장부끝 = 장부읽기();
    장부끝.절[절.번호] = { 상태: '끝', 시작: 장부.절[절.번호].시작, 끝: new Date().toISOString(), 분: Number(분), 자: 크기, 제목: 절.제목 };
    장부쓰기(장부끝);
    ...
    const 장부끝 = 장부읽기();   // 위와 같은 까닭 — 동시 실행에서 남의 「끝」을 안 지운다
    장부끝.절[절.번호] = { 상태: '실패', 시작: 장부.절[절.번호].시작, 분: Number(분), 문면, 제목: 절.제목 };
    장부쓰기(장부끝);
