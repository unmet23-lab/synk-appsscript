...
//   탐지력은 tests/지도대장.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다) 의 픽스처가 지고, 실폴더에서는 거짓양성만 검사한다.
//
// 사용법:
...
function 지도폴더() {
   ...
   *   `tests/지도대장.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다):297` 이 fail 로 떴는데, 그 검사도·`시스템대장 --render` 도·`훑기()` 도
   ...
   *   뒤에만 산다(위 세 호출부가 그 모양이고, `tests/지도대장.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)` 가 못박는다). */
  const 찾음 = 바탕화면찾기();
  const 바탕 = 찾음.존재 ? 찾음.경로 : (찾음.경로 || path.join(os.homedir(), 'OneDrive', 'Desktop'));
  // 지도는 「그날 보는 자료」라 사업 갈래에 산다. HTML+PDF 쌍이라 평평하게 풀면 개수가 두 배로
  // 보여서, 사업 갈래 **안의** 폴더로 둔다(유호 확정 폴더 수는 최상위 얘기다).
  // 2026-08-17 재편으로 뿌리가 바탕화면 자체가 됐다 — 갈래 값이 「SYNK LAB\운영」을 들고 있다.
  return path.join(갈래폴더(바탕, '사업'), 'SYNK_지도');
}
