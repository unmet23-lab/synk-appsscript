...
/* 🔑 **`찾기`(안 던짐)** 지 `경로`(못 찾으면 던짐)가 아니다 — 여기는 **읽는 쪽**이다(F617).
 *   `경로()` 가 던지는 규율은 «쓰는 쪽»(빌드 산출물이 없는 곳에 나가면 안 된다) 것이고,
 *   이 파일이 그것을 쓰는 바람에 아래 `||` 폴백이 **원리상 도달 불가능한 죽은 코드**였다 —
 *   코드가 「없으면 대비했다」의 얼굴을 한 채 실제로는 던졌다(맹점 ④ · 실측 2026-08-18 ubuntu:
 *   `훑기()` 가 자기 계약(「폴더가 없으면 `있음:false`」)을 못 지켜 `tests/지도대장.test.js` 가
 *   준비해 둔 skip 에 닿지도 못하고 fail 했고, SessionStart 는 같은 뿌리를 「[검사기 고장]」으로 냈다).
 *   여기서 추측 경로가 «쓰기»로 새지 않는 근거: `훑기()` 가 `existsSync` 로 한 번 더 거르고,
 *   CLI 도 `있음:false` 면 그 자리에서 exit 0 이라 쓰는 통로가 0이다. */
 ...
 * `opt` 는 `lib/바탕화면.js` 의 주입구(`실존`·`조회`·`env`)를 그대로 흘린다 — 그 파일 머리말이
 * 이미 정한 규율이다(「안 그러면 픽스처가 이 기계의 진짜 레지스트리에 오염돼 기계마다 다른 답을 낸다」).
 * 이 이음매가 있어야 「바탕화면이 없는 기계」의 탐지력을 **어느 OS 에서나** 픽스처가 진다(F617 · 맹점 ②). */
function 지도폴더(opt) {
  ...
  const 바탕 = 바탕화면찾기(opt).경로 || path.join(os.homedir(), 'OneDrive', 'Desktop');
