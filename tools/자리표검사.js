 ...
 * 그래서 `tools/bump-version.js` 도 `.claude/hooks/auto-commit.js`(⚠삭제됨 08-19 e75fc7fc) 도 「커밋해도 안전」이라고
 * **적어 두었다.** 그 문장을 믿은 세션이 자리표를 커밋했고(실측 `694497ad`), 커밋되는 순간
 * `tests/버전채번.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)` 「실저장소에 [vNEXT] 자리표가 남아 있지 않다」가 빨개졌다.
 ...
 *   `tests/버전채번.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)` 는 자리표를 **설명하려고** 그 문자열을 적는다. 경로 필터를 여기 손으로
 ...
 *
 * ⏱ 실측(Windows · 2026-08-15): 배포 파일이 안 든 보통 커밋에서 **약 600ms**(5회 533·632·705·602·
 *   1450). 옆 게이트 `옛글자검사 --cached` 가 같은 조건에서 529ms 라 같은 급이고, 대부분이 node
 *   기동 비용이라 코드로 깎을 자리는 거의 없다. 느린 가드는 결국 꺼지고 꺼진 가드는 없는 가드이니,
 *   이 숫자가 눈에 띄게 커지면 그때는 최적화가 아니라 **게이트를 다시 판정할** 신호로 읽는다.
 */
...
function main(argv) {
  ...
  console.error('\n왜 막나 — 자리표는 **미커밋인 동안만** 안전하다. 커밋되면(옛 회귀 tests/버전채번.test.js 는 08-19 대청소로 걷혔다 — 지금 막는 자리는 이 검사기다) 채번 전 상수가');
