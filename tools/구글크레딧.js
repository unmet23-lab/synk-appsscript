...
/* 🔑 `확인잔액`·`확인때` = **콘솔 화면에서 마지막으로 눈으로 읽은 값**이다.
 *   남은 돈은 「원금 − 이 창에서 태운 것」으로 세면 틀린다 — 창 밖에서 태운 것이 빠지기 때문이다.
 *   그래서 «확인잔액에서, 확인한 때 이후로 태운 것»을 뺀다. 화면을 다시 읽으면 이 두 값을 갱신한다.
 *
 * 🔴🔴 **이 도구가 내는 «값(원)»은 크게 부풀 수 있다 — 모델을 안 가르기 때문이다.**
 *   아래 `토큰당` 은 **그림 모델**에서 잰 단가인데, 세는 대상은 그 프로젝트의 «출력 토큰 전부»다.
 *   글 모델(몽골어 검문·검수)의 토큰까지 그림 값으로 곱한다. 글은 그림보다 100배 넘게 싸다.
 *   09-08 밤 실측 = 72시간 출력 토큰 2,758,814 중 **글이 92.3%**(gemini-3.8-flash 89.4% +
 *   gemini-3.1-pro-preview 2.9%) · 그림은 7.7%(212,408 = 4K 84장)뿐이었다.
 *   그래서 이 도구는 「₩398,097 태웠다」고 냈지만 실제는 **약 ₩38,000**(그림 28,300 + 글 8,900)이었다.
 *
 *   🔑 **화면이 옳다.** 09-08 밤에 화면을 「며칠 늦는다」고 의심했는데 틀렸다 — 30분마다 값이
 *     실제로 움직였고(375,287 → 374,616 → 373,580), 비용 보고서의 「절감액 ₩37,998」과도 맞았다.
 *   ⇒ **이 도구와 화면이 크게 다르면 이 도구를 의심한다.** 그 차이는 «글 토큰을 그림 값으로 센 것»이다.
 *   ⏳ 고칠 자리 = `metric.labels`/`resource.labels.model_user_id` 로 **모델별로 갈라 세기**.
 *     가르는 법은 잰 적이 있다(09-08 · monitoring timeSeries 에 groupByFields 로 model_user_id). */
/* 🔴 지갑이 «셋»이다(09-07 에 unmet27 이 붙었다 — 유호님이 40만원을 충전하셨다).
 *   셋을 더해 읽지 말 것 — 각자 마감이 다르고, 먼저 죽는 것부터 태운다. */
const 지갑들 = {
  /* 🔑 갱신 규칙 — **지갑 셋 다 화면값으로 갱신한다.** 화면이 옳다(09-08 밤 실측으로 확정).
   *   ⚠ 그날 한때 「지금 쓰는 지갑은 화면으로 갈지 마라」로 적었다가 되돌렸다 — 그 규칙은
   *     「화면이 며칠 늦는다」는 틀린 전제에서 나왔다. 화면은 늦지 않는다.
   *   09-08 23시대 화면 실측 = unmet23 ₩130,682(30%) · 77yuhbs ₩169,832(41%) · unmet27 ₩373,580(90%).
   *   같은 시각 비용 보고서의 「크레딧이 덮은 금액」과도 맞았다(unmet27 ₩37,998 · 원금−사용=잔액). */
  옛: {
    ...
    확인잔액: 130682, 확인때: '2026-09-08T23:00:00+09:00',   // 화면 30% · ₩130,681.95
  },
  이전: {
    ...
    확인잔액: 169832, 확인때: '2026-09-08T23:00:00+09:00',   // 화면 41% · ₩169,831.68
  },
  새: {
    /* 마감은 09-08 화면에서 «12월 7일»로 읽었다(그전엔 12-06 으로 적혀 있었다). */
    이름: 'unmet27', 원금: 414984, 마감: '2026-12-07', 결제: '01864D-0E650C-40F3CB',
    프로: 'synk-bake-unmet27', 자격: path.join(os.homedir(), '.synk-vertex-oauth.json'),
    확인잔액: 373580, 확인때: '2026-09-08T23:55:00+09:00',   // 화면 90% · ₩373,579.90 (배너)
  },
};
...
(async () => {
  ...
  console.log(`   🔴 위 «값(원)»은 모델을 안 갈라서 크게 부풀 수 있다 — 글 토큰까지 그림 값으로 센다.`);
  console.log(`      09-08 실측 = 출력 토큰의 92%가 글(몽골어 검문)이었고, 이 도구는 실제 ₩38,000 을 ₩398,097 로 냈다.`);
  console.log(`   🔑 **믿을 것은 콘솔 화면이다** (30분마다 실제로 움직이는 것을 확인했다):`);
  console.log(`      console.cloud.google.com/billing/${지갑.결제}/credits  (authuser 를 맞춰야 보인다)`);
  console.log(`      화면과 여기가 크게 다르면 «여기»가 틀린 것이다. 화면값으로 확인잔액을 갱신한다.\n`);
