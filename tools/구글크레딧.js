 ...
 *
 * 🔴🔴 **화면 잔액은 «지금 잔액»이 아니다 — 청구가 며칠 늦게 따라온다.**
 *   09-08 밤에 이것으로 두 번 틀렸다. 화면이 unmet27 을 「₩375,287 · 90% 남음」으로 보였는데,
 *   같은 시각 이 도구는 「₩51,176 남음」이라 했다. 화면을 믿고 「도구가 9배 부풀렸다」고 적었다가
 *   공식 가격표로 검산하니 **도구가 맞았다** — 토큰당 실측 ₩0.1443 대 공식 ₩0.1333(8% 차).
 *   그 시각 실제로 쓴 것은 약 ₩357,800 이었고 화면엔 그중 ₩39,697 만 반영돼 있었다.
 *   ⇒ **화면과 이 도구가 크게 다르면, 그 차이가 «아직 안 따라온 청구»다.** 화면이 옳은 게 아니다.
 *   ⇒ 화면을 읽어 확인잔액을 갱신할 때도 그 값은 «그때까지 반영된 것»일 뿐이다. */
/* 🔴 지갑이 «셋»이다(09-07 에 unmet27 이 붙었다 — 유호님이 40만원을 충전하셨다).
 *   셋을 더해 읽지 말 것 — 각자 마감이 다르고, 먼저 죽는 것부터 태운다. */
const 지갑들 = {
  /* 🔑 갱신 규칙 — 09-08 밤에 세웠다. 지갑마다 다르다.
   *   · **안 쓰는 지갑**(옛·이전): 화면값으로 갱신해도 된다. 더 안 태우니 지연분이 곧 다 반영돼
   *     화면값이 참에 수렴한다.
   *   · **지금 쓰는 지갑**(새): 화면값으로 갈아 끼우면 «아직 안 따라온 청구»만큼 잔액을 부풀린다.
   *     ⇒ **원금 100% 시점을 기준으로 두고 그 뒤 전량을 이 도구가 센다.** 지금이 그 꼴이라 안 건드린다.
   *     (09-08 23시 화면은 ₩375,287 이었지만 그 시각 실제로는 ₩5~6만원대였다.) */
  옛: {
  ...
  새: {
    ...
    /* 🔴 여기는 «화면값»이 아니라 «원금 100% 시점»이다 — 위 갱신 규칙 참조. 화면으로 갈지 마라. */
...
(async () => {
  ...
  console.log(`   🔴 콘솔 화면은 «지금 잔액»이 아니다 — 청구가 며칠 늦게 따라온다.`);
  console.log(`      console.cloud.google.com/billing/${지갑.결제}/credits  (authuser 를 맞춰야 보인다)`);
  console.log(`      화면이 여기보다 «많이» 남았다고 하면, 그 차이가 아직 안 따라온 청구다.`);
  console.log(`      09-08 실측 = 화면 ₩375,287(90%) · 이 도구 ₩51,176 · 공식 가격표 검산은 도구 쪽이 맞았다.`);
  console.log(`   🔑 화면값으로 '확인잔액' 을 갈아 끼우는 것은 «안 쓰는 지갑»만이다(--옛 · --이전).`);
  console.log(`      지금 쓰는 지갑은 «원금 100% 시점»을 기준으로 두고 그 뒤 전량을 여기서 센다.\n`);
