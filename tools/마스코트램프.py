...
검증(--검증): 이 자로 «현행» 정본을 다시 재면 토큰의 5단이 재현되는가.
  그림도 램프도 **토큰에서 짝으로** 가져온다(`재질.펠트.정본사진.평상복` ↔ `색.마스코트.평상복램프`)
  — 경로를 여기 손으로 적으면 정본이 갈릴 때 이 검사가 조용히 옛 그림을 재게 된다(08-20 실제로 그랬다).
  재현이 깨지면 **자가 아니라 그림이 바뀐 것**이니 둘 중 무엇이 움직였는지부터 가른다.

  사용:  python tools/마스코트램프.py <png>          — 램프 5단 출력
         python tools/마스코트램프.py --검증          — 현행 정본 재현 검사
"""
import json
import os
import sys
...
def 검증():
    """자가 «현행» 정본을 재현하는가 — 탐지력은 여기서 산다.

    🔑 2026-08-20 수리: 이 함수가 **깨진 채 서 있었다.** 체리 퇴역(08-19) 때 경로가
       기계로 치환돼 `펠트코랄_0815/본체.png` 를 가리켰는데 그런 파일은 없다(펠트 세트는
       `재염색_*.png`). 게다가 대조 상대는 여전히 체리 램프라, 설령 열렸어도 «퇴역한 색»과
       «현행 그림»을 맞대는 판이었다. 그림도 램프도 **토큰에서 짝으로** 가져온다 —
       토큰 `_평상복출처` 가 「이 자로 재염색_본체.png 를 재서 만든 값」이라고 스스로 적어 뒀다.
    """
    토큰 = json.load(open(os.path.join(ROOT, 'docs', '디자인_토큰.json'), encoding='utf-8'))
    정본 = [c['hex'] for c in 토큰['색']['마스코트']['평상복램프']]
    사진 = 토큰['재질']['펠트']['정본사진']['평상복']
    잰것, n, 전체 = 램프(os.path.join(ROOT, *사진.split('/')))
    차 = [dE76(a, b) for a, b in zip(잰것, 정본)]
    print(f'현행 정본(평상복) 재현 — {사진} · 유채 {n:,}/{전체:,}')
