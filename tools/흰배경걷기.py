...
def 바탕색구멍걷기(원px, ap, w, h, 바탕색, 임계):
    """바탕이 흰색이 «아닐» 때, 물체가 감싼 바탕을 걷는다 (2026-09-07 · 크림 부품 회색 바탕 다시 굽기).

    왜: 크림 양모를 크림 종이에서 찍으면 흰 배경을 걷는 자가 물건을 파먹는다(09-07 실측 · 42벌 중
      19벌). 그래서 그 부품들을 회색 바탕(#8A8A8A)에서 다시 굽는데, 위 `구멍걷기` 는 「거의 순백」만
      보므로 물체가 감싼 «회색» 구멍(꽃 매듭의 고리 안 같은 곳)은 못 본다 — 그 자리가 회색 조각으로 남는다.
    어떻게: 씨앗(0,0)의 색을 바탕색으로 잡고, 아직 살아 있는 픽셀 중 그 색에 가까운 것을 지운다.
      가깝다의 자는 floodfill 과 같은 «채널 최대 차 ≤ 임계»다 — 두 단계가 다른 자를 쓰면 경계가 어긋난다.
    ⚠ 흰 바탕에서는 부르지 않는다(부르는 쪽이 가른다) — 흰 바탕의 그 일은 `구멍걷기` 가 이미 진다.
      크림 물건은 회색과 멀어서 안 다친다. 가장자리에서 회색과 섞인 잔털 한 겹은 같이 걷히는데,
      그건 floodfill 이 바깥 가장자리에서 이미 하는 것과 같은 일이다.
    """
    r0, g0, b0 = 바탕색
    센다 = 0
    for y in range(h):
        for x in range(w):
            if ap[x, y] == 0:
                continue
            r, g, b = 원px[x, y]
            if max(abs(r - r0), abs(g - g0), abs(b - b0)) <= 임계:
                ap[x, y] = 0
                센다 += 1
    return 센다
...
def 걷는다(경로, 출력, 임계, 여백, 정사각, 부드럼, 흰바닥=218, 채도문=22, 구멍문=238, 조각몫=2.0, 그대로=False):
    ...
    # 🔑 바탕이 흰색이 아니면(회색 바탕에서 구운 크림 부품) 감싸인 «그 색» 구멍도 걷는다 — 위 함수 머리말.
    #   가르는 자 = 씨앗(0,0)의 밝기. 흰 바탕(min ≥ 230)이면 안 부른다 — 옛 결과는 한 바이트도 안 바뀐다.
    바탕색 = 원.getpixel((0, 0))
    회색구멍수 = 바탕색구멍걷기(원.load(), ap, w, h, 바탕색, 임계) if min(바탕색) < 230 else 0
    ...
    print(f'   ✅ {os.path.basename(출력)}  배경 {지운수 * 100 // (w * h)}% · 구멍 {구멍수:,}점'
          ...
          f'{f" · 떨어진 조각 {조각수:,}점" if 조각수 else ""}'
          f'{f" · 회색 구멍 {회색구멍수:,}점" if 회색구멍수 else ""} · {out.size[0]}×{out.size[1]}')
    return 지운수 > 0
