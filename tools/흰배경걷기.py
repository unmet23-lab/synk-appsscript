...
🔴 **크림·흰 물건은 기본값으로 걷지 않는다**(09-05 저녁 실측 · 다섯 장이 파먹혔다).
   위 「밝기 임계로 자르지 않는다」는 floodfill 이야기고, 그 뒤에 도는 `흰그림자다듬기` 는
   «채도 낮고 밝은» 픽셀을 밝기에 비례해 투명하게 만든다. 크림 양모가 정확히 그 조건이라
   물건 «안쪽»이 벌집처럼 뚫린다 — 북마크는 몸통이, 넘김손가락은 엄지가 사라졌다.
   흰 지면에서는 안 보이고 색 바탕이나 마스코트 위에 얹어야 드러난다.

   ✅ 처방 = `--흰바닥 254 --임계 50 --조각몫 0`
      · 흰바닥 254 — 다듬기 단계를 사실상 끈다(이것이 급소다)
      · 조각몫 0  — 파먹힌 자리 때문에 엄지가 「떨어진 조각」으로 잡혀 지워지는 것을 막는다
   🚫 안 듣는 것 = 임계 34~62 · 흰바닥 234·244 · 채도문 6(더 나빠진다 · 북마크가 두 동강).
   ⚠ 이 처방은 그림자를 덜 걷으므로 가장자리가 조금 거칠다. 짙은 물건에는 쓰지 않는다.
   🔑 파먹혔는지 세는 자 = 알파의 바깥 윤곽을 채운 뒤(binary_fill_holes) 그 안의 빈자리 몫.
      「설계된 구멍」(가위 손잡이·숫자 0 의 속)과는 구멍 «개수»로 갈린다 — 파먹힘은 잘게 흩어진다.

사용:
  python tools/흰배경걷기.py docs/Loom_자산/구움/공방_가위.png
  python tools/흰배경걷기.py "docs/Loom_자산/구움/공방_*.png" --임계 70
  python tools/흰배경걷기.py <밝은 물건> --흰바닥 254 --임계 50 --조각몫 0
