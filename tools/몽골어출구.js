 ...
 * 🔑 갈래 셋 = 통과(✅ 또는 🟡 반쪽) · 검수(🔴 사람 눈으로) · 불가(**종합 줄이 아예 없다** = 못 쟀다).
 *   벽으로 세는 것은 «불가»뿐이다 — 「사람 눈으로 가라」는 판정이 났다는 뜻이지 막힌 게 아니다.
 * @param 글 자식이 낸 stdout+stderr
 * @param status 자식 종료코드(참고용 · 종합 줄이 없을 때만 쓴다)
 */
function 한판읽기(글, status) {
  const 판정있음 = /■ 종합:/.test(String(글));
  if (!판정있음) return { 갈래: '불가', 까닭: `종합 줄이 없다(종료코드 ${status})` };
  if (/✅ 기계 검문 통과/.test(글)) return { 갈래: '통과', 까닭: '다섯 겹 통과' };
  if (/🟡 반쪽/.test(글)) return { 갈래: '통과', 까닭: '반쪽(두 겹만 쟀다)' };
...
async function 태우기(인자) {
  ...
  for (let i = 0; i < 목록.length; i++) {
    ...
    if (판.갈래 === '통과') 통과 += 1; else if (판.갈래 === '검수') 검수 += 1; else 불가 += 1;
