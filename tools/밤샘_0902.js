...
말(`   ${문지기.상태줄()}`);
말('   사슬 = ⓪-a 부품 형태 13장 → ⓪-b 판정 지면 → ①까몽 → ②화면 열넷 → ③요소 이어 → ④엔진 히어로 7장 → ⑤로딩 → ⑤-b 로딩 앱용 → ⑥요소 시안 지면');
도장('돌는중', '돌고 있다 — 사슬 ⓪-a 부품형태 → ⓪-b 판정지면 → ①까몽 → ②화면 → ③요소 → ④히어로 → ⑤로딩 → ⑥지면 (이 도장이 그대로면 ⓪-a 도중에 죽은 것이다)');
...
단계('⑤ 로딩 루프(세트굽기 · 이미 있으면 건너뜀)',
  [path.join(루트, 'tools', '세트굽기.js'), '--세트', '로딩']);
