 ...
 *   → ⑤로딩+형태후보 → ⑤-b 로딩 앱용 → ⑤-c 부품 형태 13장 → ⑥요소 시안 지면 → ⑥-b 부품형태 판정 지면
 ...
 *   ⑤-c docs/캐릭터/요소공방_0822/부품형태/ 13장 ⑥-b **docs/부품형태_시안.html ← 유호님이 고르실 판**
...
말(`   ${문지기.상태줄()}`);
말('   사슬 = ①까몽 → ②화면 열넷 → ③요소 이어 → ④엔진 히어로 7장 → ⑤로딩+형태후보 → ⑤-b 로딩 앱용 → ⑤-c 부품 형태 새 형태 13장 → ⑥요소 시안 지면 → ⑥-b 부품형태 판정 지면');
도장('돌는중', '돌고 있다 — 사슬 ①까몽 → ②화면 → ③요소 → ④히어로 → ⑤로딩·형태후보 → ⑤-c 부품형태 → ⑥지면 → ⑥-b 판정지면 (이 도장이 그대로면 ① 도중에 죽은 것이다)');
...
/* ⑤ 로딩 루프 6칸 + 형태 후보 넷 — 유호 지시 09-01 「밤 굽기 일감에 등록해줘 · 잘때 이어서 전부」.
 *   ⚠ 둘 다 09-01 낮에 이미 한 벌 구웠다(로딩 51초 · 후보 5분 34초). 그래도 사슬에 남기는 까닭은
 *   **이어 굽기가 안전하기 때문**이다 — 세트굽기는 이미 있는 파일을 «건너뜀»으로 세고 몇 초에 끝난다.
 *   빼면 다음 밤에 「그때 구웠던가」를 사람이 기억해야 하고, 그 기억은 반드시 갈린다. */
단계('⑤ 로딩 루프 + 형태 후보(세트굽기 · 이미 있으면 건너뜀)',
  [path.join(루트, 'tools', '세트굽기.js'), '--세트', '로딩,형태후보']);
...
/* ⑤-c 부품 형태 후보 13장 — **새 형태**(⑤ 형태후보와 다른 질문이다) — «구» 탈피 판 다섯(유호 지시 09-01 저녁 「요소굽기.py에 형태 코드
...
단계('⑤-c 부품 형태 후보 13장(«구» 탈피 판 다섯 — 유호 지시 09-01)',
