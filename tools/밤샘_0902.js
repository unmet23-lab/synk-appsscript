#!/usr/bin/env node
/**
 * 밤샘 0902 — 오늘 밤의 사슬: ①까몽 몸판 넷 → ②화면 열넷 재굽기 → ③요소 라이브러리 이어굽기
...
말(`   ${문지기.상태줄()}`);
말('   사슬 = ①까몽 몸판 넷(밤샘_0831 승계) → ②화면 열넷(--화면만 한 벌) → ③요소 이어(밤굽기)');
도장('돌는중', '돌고 있다 — 사슬 ①까몽 → ②화면 → ③요소 (이 도장이 그대로면 ① 도중에 죽은 것이다)');
...
도장('완주', `끝까지 갔다 · 단계별 종료코드: ${단계기록.join(' · ')} · 로그 ${로그경로} — 그림이 «좋은가»는 여기서 안 잰다(사람 몫) · 다음 밤 일감 = 부품 형태 시안(트랙 §2-Loom)`);
도장끝 = true;
