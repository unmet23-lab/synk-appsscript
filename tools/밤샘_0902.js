...
말(`   ${문지기.상태줄()}`);
말('   사슬 = ①까몽 → ②화면 열넷 → ③요소 이어 → ④엔진 히어로 7장 → ⑤로딩+형태후보 → ⑤-b 로딩 앱용 → ⑤-c 부품 형태 새 형태 13장 → ⑥요소 시안 지면');
도장('돌는중', '돌고 있다 — 사슬 ①까몽 → ②화면 → ③요소 → ④히어로 → ⑤로딩·형태후보 → ⑤-c 부품형태 → ⑥지면 (이 도장이 그대로면 ① 도중에 죽은 것이다)');
 ...
 *   ⚠ 아침 몫 = 판정 지면(`docs/부품형태_시안.html`)을 세우고 «현행 ↔ 후보»를 나란히 놓는 것.
