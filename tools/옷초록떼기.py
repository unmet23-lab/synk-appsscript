...
def 떼어낸다(초록경로, 몸경로, 눈결='검정', 바탕='초록'):
    ...
    #   🔴 조건을 좁게 잡는다 — 크림 양모도 「밝고 색이 옅다」라서, 넓게 잡으면 크림 옷이
    #     바탕으로 먹힌다(09-06 실측 · SYNK 후드가 찢어져 보였다). 진짜 바탕은 결이 없어
    #     아주 매끈하고 아주 밝다. 그리고 화면의 한 조각이 아니라 «큰 덩어리»다.
    밝고민 = (a.max(axis=2) > 236) & ((a.max(axis=2) - a.min(axis=2)) < 13)
    표0, 수0 = ndimage.label(밝고민)
    큼 = a.shape[0] * a.shape[1] * 0.03
    테두리 = set(표0[0, :]) | set(표0[-1, :]) | set(표0[:, 0]) | set(표0[:, -1])
    테두리.discard(0)
    테두리 = {i for i in 테두리 if (표0 == i).sum() > 큼}
    흰 = np.isin(표0, list(테두리)) if 테두리 else (a > 244).all(axis=2)
