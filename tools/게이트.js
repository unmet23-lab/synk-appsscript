...
/**
 * 장부를 접는다 — 저장소별 «마지막 상태» + **못 읽은 줄 수**.
 *
 * 🔴 2026-08-17 ①배포 검수 P1 — 깨진 줄을 `catch { continue; }` 로 **조용히** 건너뛰었다.
 *   건너뛰는 것 자체는 옳다(줄 하나가 장부 전체를 죽이면 안 된다 · 회귀 ⑤가 그걸 지킨다).
 *   틀린 것은 **침묵**이다: append 가 잘려 `풀기` 줄이 깨지면 그 줄만 사라지고 그 앞의
 *   `걸기` 가 최신으로 남는다 → 네트워크를 안 쓰는 `--훅` 이 「🔒 걸려 있다」고 **틀린 안심**을
 *   주는데 서버측 게이트는 실제로 풀려 있다. 새는 방향이 「통과」다.
 *   그래서 건너뛰되 **몇 줄을 못 읽었는지 세서 부르는 쪽에 넘긴다** — 모르는 것을 0 으로
 *   접지 않는다(F543 · 「쓰는 쪽이 분모를 안 만들면 읽는 쪽은 못 읽는다」).
 *
 * @param {string} [경로] 테스트가 실장부를 안 건드리게 주입한다(픽스처).
 * @returns {{상태:object, 깨짐:number}}
 */
function 장부읽기(경로 = 장부경로) {
  if (!fs.existsSync(경로)) return { 상태: {}, 깨짐: 0 };
  const 상태 = {};
  let 깨짐 = 0;
  for (const 줄 of fs.readFileSync(경로, 'utf8').split('\n')) {
    if (!줄.trim()) continue;
    let r;
    try { r = JSON.parse(줄); } catch { 깨짐 += 1; continue; } // 건너뛰되 **세서 드러낸다**
    if (!r.저장소) { 깨짐 += 1; continue; }                     // 형식은 맞는데 저장소가 없는 줄도 못 읽은 것이다
...
/** 상태만 필요한 자리 — 옛 이름을 그대로 둔다(호출부·회귀가 이 모양을 쓴다). */
...
/** 이 한 저장소의 실측과 장부가 어긋났나 — 판정만 하는 순수 함수. */
function 어긋났나({ 찾음, 장부쪽 }) {
  const 실측있음 = Boolean(찾음);
  const 장부있음 = Boolean(장부쪽);
  const 실측걸림 = Boolean(찾음 && 찾음.enforcement === 'active');
  const 장부걸림 = Boolean(장부쪽 && 장부쪽.걸림);
  return 실측있음 !== 장부있음 || (실측있음 && 실측걸림 !== 장부걸림);
}

/* 대조 **전체**의 판정 — 순수 함수라 gh·네트워크 없이 픽스처로 잰다(탐지력은 픽스처가 진다).
 *
 * 🔴 2026-08-17 ①배포 검수 P1 — 옛 통로는 못 읽은 저장소를 **세지 않았다.**
 *   `규칙찾기` 가 실패하면 「❔ 못 읽었다」를 찍고 `continue` 했는데, 그 갈래는 `어긋남` 을
 *   못 올리므로 전 저장소가 실패해도 `어긋남 0` 문을 그대로 지나 **`✅ 실측과 장부가 같다`** 를
 *   찍고 0 으로 끝났다. 이 함수의 존재 이유가 「장부 말고 **실측**을 본다」인데, 실측을 한 줄도
 *   못 읽은 판이 «대조해 보니 같더라» 와 같은 모양으로 나온 것이다 — 머리말 「대가」가 적어 둔
 *   거짓 초록이 정확히 여기로 났다(F207 · 미실행은 통과와 같은 모양이다).
 *
 * 🔑 그래서 초록은 **분모와 함께만** 낸다: 전체 N 중 대조한 것이 몇 개인지 같이 적는다. */
function 상태판정(관측들) {
  let 어긋남 = 0;
  let 못읽음 = 0;
  for (const o of 관측들) {
    if (o.실패) { 못읽음 += 1; continue; }
    if (어긋났나(o)) 어긋남 += 1;
  }
  return {
    전체: 관측들.length,
    ...
    어긋남,
    못읽음,
    /* 어긋남이 먼저다 — 읽힌 것 중에 «틀린 안심»이 있으면 그게 더 급하다. */
    칸: 어긋남 ? '어긋남' : 못읽음 ? '못읽음' : '같음',
    종료코드: (어긋남 || 못읽음) ? 2 : 0,
  };
}
...
function 상태보기() {
  ...
  /* 여기서는 실측을 실제로 읽으므로 대조 자체는 성립한다 — 그래서 막지 않고 **말한다.**
   * (훅은 반대다: 장부가 유일한 근거라 깨진 줄이 곧 모름이다.) */
  if (깨짐) console.log(`  ❔ 장부에 못 읽은 줄 ${깨짐}개 — 아래 「장부」 칸은 그만큼 불완전하다(실측이 진실이다).\n`);

  const 관측들 = [];
  for (const 저장소 of 저장소들) {
    const { 찾음, 실패 } = 규칙찾기(저장소);
    const 장부쪽 = 장부[저장소];
    관측들.push({ 저장소, 찾음, 실패, 장부쪽 });
    if (실패) {
      console.log(`  ${저장소}\n    실측: ❔ 못 읽었다 (${실패})\n`);
      continue;
    }
  ...
  const r = 상태판정(관측들);
  if (r.어긋남) {
    console.log(`🔴 어긋남 ${r.어긋남}건 — 훅은 장부를 믿으므로 그동안 **틀린 안심**을 준다.`);
    console.log('   ▶ 실측에 맞춰 다시 걸거나 풀어 장부를 갱신한다.\n');
    process.exit(r.종료코드);
  }
  /* 못 읽은 것이 하나라도 있으면 **초록이라 말하지 않는다.** 「전부 실패했을 때만」으로 좁히면
   * 부족하다 — 2개 중 1개를 못 읽고 나머지가 맞았을 때도 「같다」는 그 하나에 대한 거짓말이다. */
  if (r.못읽음) {
    console.log(`❔ **대조를 못 했다** — 저장소 ${r.전체}개 중 ${r.못읽음}개의 실측을 못 읽었다(대조한 것 ${r.대조함}개).`);
    console.log('   실측 없이 「같다」고 적으면 그건 대조가 아니라 **침묵**이다 — 초록은 분모와 함께만 읽는다(F207).');
    console.log('   ▶ gh auth status 로 인증을 보고 다시 부른다.\n');
    process.exit(r.종료코드);
  }
  console.log(`✅ 실측과 장부가 같다 — 저장소 ${r.전체}개 전부 대조했다(대조 ${r.대조함}/${r.전체}).\n`);
}
...
function 훅() {
  ...
  /* 🔴 못 읽은 줄이 있으면 **조용할 자격이 없다.** 잘린 `풀기` 줄은 그 줄만 사라지고 앞의
   * `걸기` 가 최신으로 남아, 이 훅이 「다 걸려 있다」며 침묵한다 — 그게 이 장치가 막으려는
   * 바로 그 모양(풀어놓고 잊기)이다. 모르는 것은 「모름」으로 낸다. */
  if (깨짐) {
    console.log(`❔ **게이트 장부에 못 읽은 줄 ${깨짐}개** — 그만큼 이 판정은 모른다(장부 기준 · 네트워크 미조회).`);
    console.log('   잘린 줄이 `풀기` 였다면 화면은 「걸림」인데 서버는 풀려 있다.');
    console.log('   → 실측으로 가른다: node tools/게이트.js --상태');
  }
...
module.exports = { 저장소들, 규칙이름, 규칙정의, 요청몸통, 장부의현재상태, 장부읽기, 장부경로, 상태판정, 어긋났나 };
