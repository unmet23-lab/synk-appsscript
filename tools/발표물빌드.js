 ...
 * ⚠ 원고가 스스로 `룸`·`민` 을 달고 있으면 이 되돌리기가 그것까지 걷는다 — `_src_` 는 동결이라
 *   지금은 하나도 없다(실측 0건). 원고에 달 일이 생기면 이 자리부터 다시 본다.
 */
const 룸벗기기 = (html) => html
  .replace(/(<section\s+class="[^"]*?)\s+룸"/g, '$1"')
  .replace(/(<ol\s+class="[^"]*?)\s+민"/g, '$1"')
  .replace(/<ol\s+class="민"/g, '<ol');
 ...
 *   `tests/발표물Loom.test.js` 가 «발표물 관점»에서 그 통로를 계속 재기 때문이다 —
