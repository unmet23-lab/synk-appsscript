...
const LOOM_지면 = '인쇄부품';
/* 🔑 얹는 «절차»는 여기 안 산다 — `tools/lib/룸입히기.js` 하나다(2026-08-19).
 *   그전엔 이 파일 안에 있었고, 그건 **발표물에서만** 참이었다. 생성기·손파일이 같은 절차를
 *   필요로 하는 순간 호출부마다 베끼게 되고, 베끼면 갈라지고, 갈라지면 조용히 통과한다.
 *   아래 이름들은 **호출부 호환을 위한 얇은 껍질**이다 — 판정도 절차도 위임한다. */
const 룸입히기 = require('./lib/룸입히기.js');
/** 얹은 블록을 되뜯는 표식 — `--check` 가 원고와 1:1 로 대조하려면 되돌릴 수 있어야 한다.
 *  ⚠**비전역**으로 받는다 — 여기서는 `.test()` 로만 쓰므로 `/g` 의 `lastIndex` 가 붙으면
 *  같은 입력에 답이 번갈아 갈린다(`룸입히기` 머리말). 뜯는 쪽은 `loom뜯기` 가 전역으로 한다. */
const LOOM_열기 = 룸입히기.열기표식(LOOM_지면);
const LOOM_뜯기 = 룸입히기.뜯기정규식(false);
...
function 훅들(html, css = loom.css({ 지면: LOOM_지면 })) {
  return 룸입히기.훅들(html, { 지면: LOOM_지면, css });
}
 ...
 */
const loom얹기 = (html, css) => 룸입히기.얹기(html, css, LOOM_지면);
/** ⚠전역으로 뜯는다 — 어쩌다 두 겹이 된 지면에서 비전역이면 **한 겹만** 뜯겨,
 *  되뜯은 것이 원고와 안 맞는데 원인은 「빌드가 낡았다」로 잘못 읽힌다. */
const loom뜯기 = 룸입히기.뜯기;
