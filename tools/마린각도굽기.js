...
function main() {
  ...
  if (!인자['지면만']) {
    ...
    // 까몽 = 명품화판(유호 08-28 「흰 배경 하나만 · 감각적·트렌디」) — 눈빛(브라우 틈 발광)·볼터치·
    //   모노 팔레트(검은 연필·방패 · 색은 코랄 점만). 볼높이·눈줄높이 기본값은 실측 확정(1차 = 성난 눈).
    (굽기(BL, 길.primey, 'primey_해석_우주복까몽', '', '우리색', [...우주복공통,
      '갑옷=Ink', '갑옷배율=1.0', '장갑색=Ink', '속빛=0', '눈줄=1', '볼터치=1',
      '연필몸색=Ink Deep', '방패색=Ink Deep', '단추색들=Coral,Butter,Oat', '결규모=260', '결세기=0.30',
      '빛배=1.35', '받침=Oat', '받침대색=Ink Deep']) ? 난것++ : 죽은것++);
