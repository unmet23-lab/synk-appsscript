...
function main() {
  ...
  if (!인자['지면만']) {
    ...
    (굽기(BL, 길.primey, 'primey_해석_까몽', '', '우리색', [...미니, '돌리기=z30,x-8', '재질=펠트',
      ...
      '부속삭제=-0.50,-0.10,-0.60,-0.33,0.70,1.03;0.02,0.20,-0.55,-0.28,0.55,0.95', '허리책상자=-0.45,-0.10,0.80,1.06'])
      ? 난것++ : 죽은것++);
    // 뒷태 — 학생 가방이 보이는 컷(기능은 자기를 설명해야 완성).
    (굽기(BL, 길.primey, 'primey_해석_뒷태', '', '우리색', [...미니, '돌리기=z210,x-8', '재질=펠트',
      '갑옷=Coral', '갑옷배율=0.92', '눈=0', '장식=받침,연필가방', '받침대색=Oat', '가방=1'])
      ? 난것++ : 죽은것++);
    ...
    //   z30 = 정면 실측각(유호 08-28 「얼굴 정면으로」) · 볏·방패 = 스파르타 선생님(가로 볏 =
    //   켄투리온 지휘관 표식) · 버터 목 링은 유호 반려로 갔다(「입쪽 노란 줄 부자연」 → 슈트색 깃).
    const 우주복공통 = [...미니, '돌리기=z32,x-8', '재질=펠트', '우주복=1', '머리들기=12', '눈=0',
      '볏=1', '방패=1', '이마=1', '장식=받침,배지', '가방=0'];
    (굽기(BL, 길.primey, 'primey_해석_우주복몽글', '', '우리색', [...우주복공통,
      '갑옷=Coral', '갑옷배율=0.92', '속빛=4', '볏색=Ink Deep', '받침대색=Oat']) ? 난것++ : 죽은것++);
    (굽기(BL, 길.primey, 'primey_해석_우주복까몽', '', '우리색', [...우주복공통,
      '갑옷=Ink', '갑옷배율=1.0', '장갑색=Ink', '속빛=0', '볏색=Coral', '빛배=1.35', '받침=Oat',
      '받침대색=Ink Deep']) ? 난것++ : 죽은것++);
    (굽기(BL, 길.primey, 'primey_해석_우주복뒷태', '', '우리색', [...우주복공통,
      '갑옷=Coral', '갑옷배율=0.92', '속빛=0', '볏색=Ink Deep', '몸돌리기=196',
      '받침대색=Oat']) ? 난것++ : 죽은것++);
