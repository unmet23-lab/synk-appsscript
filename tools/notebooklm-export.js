#!/usr/bin/env node
/**
 * 노트북LM 묶음 생성기 — repo 정본에서 NotebookLM 업로드 폴더를 **생성**한다.
 *
 * 왜 생성기인가:
 ...
 *
 * 사용:
 *   node tools/notebooklm-export.js           # 바탕화면\SYNK_노트북LM 에 생성
 *   node tools/notebooklm-export.js --out <경로>
 *   node tools/notebooklm-export.js --dry     # 무엇을 쓸지·무엇이 막히는지만 출력
 *
...
// ── 담지 않을 폴더 (경로 단위) ──────────────────────────────────────────
// _archive·_구본 = 낡은 판. 노트북LM이 낡은 판을 출처로 찍어 답하는 것이
...
const PII = [
  ...
  { name: 'API 키',       re: /(?:sk-[A-Za-z0-9]{20,}|AIza[0-9A-Za-z_-]{30,}|ghp_[A-Za-z0-9]{30,})/g },
...
// NotebookLM은 **폴더 구조를 잃는다** — 업로드하면 파일명만 남는다.
...
> 다시 만들기: 저장소에서 \`node tools/notebooklm-export.js\`
>
...
**안전한 값으로 판명되면** \`tools/notebooklm-export.js\`의 \`ALLOW\` 목록에 그 값을
...
`;
  ...
  const 안내 = `# SYNK 노트북LM 묶음 — 올리는 방법

> **만든 날 ${today}** · 지침 ${VER} · 자료 **${written}개** (제외 ${blocked.length}개)

## 이 묶음의 자리 (왜 만드는가)
**클로드 한도가 찼을 때의 조회 창구.** NotebookLM은 클로드 한도를 **0** 쓴다.
...
4. 이 폴더(\`SYNK_노트북LM\`)를 열고 **Ctrl+A**로 전부 선택 → **열기**
   - 자료 ${written}개 (NotebookLM Plus 상한 300개 이내)
...
## 다시 만들 때
저장소가 바뀌면 이 묶음은 그날로 낡는다. 클로드에게 **"노트북LM 묶음 만들어줘"** 라고 하면
이 폴더가 새로 생성된다. 그 뒤 **노트북LM에서 옛 노트북은 지우고 새로 올린다**
