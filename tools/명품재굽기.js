 ...
 *   node tools/명품재굽기.js --라이브러리만 --이어 "2026-08-26 11:18"   (끊긴 자리부터 — 남은 것만)
...
if (전부 || 요소만) 요소굽기();
if (전부 || 화면만) 화면굽기();
if (전부 || 라이브만) 라이브러리굽기(인자['이어'] ? new Date(인자['이어']).getTime() : null);
...
if (!덧만) 검사(시작, 건드린갈래);      // 다시 구운 뒤에도 남았나 — 마지막 말은 파일이 한다
