...
/* ── 화면 열넷 — «어느 공방의 어느 그릇인가»의 정본은 `tools/lib/화면표.js` 다 ──────────
 * 09-03 에 이 파일 밖으로 옮겼다. 까닭은 이 파일이 **읽는 순간 굽기가 시작되는 스크립트**라
 * (`module.exports` 0줄) 클라우드 굽기 도구가 표를 빌릴 길이 «베끼기» 하나뿐이었기 때문이다.
 * 베끼면 그 순간부터 둘이 갈린다 — 램문지기가 08-31 에 선 것과 같은 무늬(lib/램문지기.js 머리말).
 * 화면이 늘면 «저기» 한 줄을 더한다. 이 파일은 이제 표를 안 쥔다. */
...
function 화면굽기() {
  ...
  화면들.forEach((s, i) => {
    ...
    spawnSync(블렌더, ['-b', '-P', path.join(루트, 'tools', '요소굽기.py'), '--',
      `형태=${s.형태}`, `비율=${화면비율}`, ...화면자, `샘플=${견본}`, `너비=${너비}`, `장치=${장치}`, `출력=${파일}`],
...
/* 🔴 값이 필요한 플래그가 «값 없이» 오면 여기서 선다 — 안 세우면 아래 분기를 못 타고 「전부」로
 *   조용히 떨어져, 검사하려던 손이 전량 굽기(GPU 수 시간)를 띄운다(09-01 실사고: `--검사` 단독
 *   호출이 낮 전량 굽기가 됐다 — blender 가 요소 0·1·2 를 굽던 중에야 잡았다). */
for (const 필수값 of ['검사', '빠진것', '이어']) {
  if (깃발.has(필수값) && !인자[필수값]) {
    console.error(`🔴 --${필수값} 는 기준 시각 값이 필요하다(공백 없이 · 예: --${필수값} 2026-08-25T07:33). 값 없이 주면 이 분기를 못 타고 «전량 굽기»로 떨어진다.`);
    process.exit(1);
  }
}
