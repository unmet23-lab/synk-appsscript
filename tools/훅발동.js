 ...
 *     세 번 그랬다. 그래서 세는 자리마다 분모를 같이 낸다 — `평가` 0(판정 자리 없음)·
 *     빈 전사·훅 기록 없는 전사는 **통과가 아니라 미측정**으로 따로 찍는다(F207).
 *   · **닫은 것 = 없다.** 이 도구가 대체할 옛 통로가 0 이다(그게 #Q98 의 제목이다).
 *     대신 **새 훅 0** 이고, 판정을 lib 로 올리며 훅에서 90여 줄을 덜었다 — 장치 수는 +1 이지만
 *     «판정 사본»은 0 으로 유지된다(사본이 생기는 쪽이 이 저장소가 실제로 새던 방향이다).
 *
 * 쓰는 법:
