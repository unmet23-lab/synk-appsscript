...
def 차림표():
    ...
    # 마린 — 해골을 «조각째» 뺀 우주복판에서 얼굴만 딴다(유호 08-28 「해골 빼고 아바타까지」).
    #   전신 렌더라 코어 상자로는 머리가 점이 된다 ⇒ 얼굴 상자를 비율로 직접 준다.
    마린원본 = 루트 / 'docs' / '캐릭터' / '마린공방_0827' / 'primey_해석_우주복몽글_해골뺌.png'
    if 마린원본.exists():
        # 상자는 «원형으로 잘린 꼴»을 네 후보(0.34·0.38·0.42·0.46) 나란히 굽고 눈으로 골랐다.
        #   정사각으로 멀쩡한 값이 원 안에서는 꽉 차 보인다 — 마린은 헬멧이 정사각을 채우는 몸이라
        #   그 차이가 크다(0.34 는 원 위쪽에 닿고 0.42 부터는 얼굴이 작아진다).
        #   0.38 = 헬멧이 온전히 들어가고 가슴 패널이 살짝 걸려 「우주복 입은 아이」로 읽히는 자리.
        표.append(('마린', 마린원본, dict(얼굴=(0.494, 0.244, 0.38))))
    else:
        print('[마린] 원본이 없다 — 먼저 굽는다: tools/마린각도굽기.js 의 우주복 레시피 + 빼기 인자')
    return 표
