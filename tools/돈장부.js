...
/** 지금 굽는 계정 — **자격 파일이 스스로 적어 둔 이름**을 읽는다.
 *
 * 🔴 09-08 사고: 여기 이름이 손으로 박혀 있었다(`'77yuhbs@gmail.com'`). 09-07 에 지갑을
 *   unmet27 로 갈아 끼웠는데 이 줄이 안 따라와서, 장부가 두 날 동안 «틀린 계정»을 가리켰다.
 *   그 값을 그대로 읽고 유호님께 잘못 보고했다(기억 constant-known-in-two-places).
 *   ⇒ 이름은 자격 파일이 쥔다. 갈아 끼우면 이 도구가 저절로 따라온다. */
const 지금굽는곳 = () => {
  try {
    const j = JSON.parse(fs.readFileSync(굽기자격, 'utf8'));
    if (j && j.계정) return String(j.계정);
  } catch { /* 파일이 없거나 못 읽으면 옛 계정으로 본다 */ }
  return 'unmet23@gmail.com';
};
...
(async () => {
  ...
  for (const a of 장부.계정) {
    ...
    /* 🔴 화면 잔액은 «지금 잔액»이 아니다 — 청구가 며칠 늦게 따라온다. 지금 굽는 지갑에서
     *   그 지연이 가장 크게 벌어진다(09-08 실측: 화면 ₩375,287 인데 실제는 ₩5~6만원대).
     *   그래서 그 지갑에서는 이 값을 그대로 읽지 말라고 여기서 소리를 낸다. */
    if (a.크레딧 && a.크레딧['🔴']) console.log(`       🔴 ${a.크레딧['🔴']}`);
    else if (굽나) console.log('       🔴 이 값은 화면값이라 «아직 안 따라온 청구»만큼 부풀어 있다 — 실상은 node tools/구글크레딧.js');
