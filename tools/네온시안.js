 ...
 *
 * ✅ **결과 둘 다 닫혔다**(유호 확정 2026-08-31):
 *   ① **색 = ㉡ 코랄 `#F96859` 단색**(「㉡ 코랄 단색으로 갈게」)
 *   ② **공법 = 아크릴 채널 글자 + LED**(「현지에서 가능하다면 아크릴 버전으로 가자」)
 *   정본은 **v7** 로 올랐고 공간설계 §6 표 C·D·§8 2차 발주·§11 합의 1번이 같이 닫혔다.
 *   이 파일은 이제 «판정 지면»이 아니라 **«판정 기록»**을 굽는다 — 무엇을 보고 골랐나가 남는다.
 *
 * 🔑 **공법을 가른 것은 취향이 아니라 조달이다**(유호 상시 지시 08-31 — 인테리어는 현지에서 되는 것만).
 *   UB 간판 업체 다섯을 실측하니 **아크릴 채널 + LED 만 현지 제작이 확인**됐고(Print house ХХК 가
 *   「3D 글자·라이트박스」 명시), 유리 네온·LED 네온 플렉스는 그 목록에서 **못 찾았다**(「없다」가 아니다).
 *   곁수확: 아크릴은 **로고를 안 건드린다** — 이 지면이 그리는 «채워진 글자»를 그대로 잘라 내면 된다.
 *   튜브 계열이었으면 글자가 «선»이 되어 로고 정본을 다시 그려야 했다.
 *   🔴 조건부다 — 「현지에서 가능하다면」이 문면이고 업체 전화 확인 전이다(공간설계 §9-15).
 *
...
<div class="폭">
  ...
  <p class="머리설명">
    ...
    <b>한 색</b>이고, 만드는 법은 <b>아크릴 채널 글자 + LED</b>입니다(같은 날 확정 · 아래). <b>네온 발주가 열렸습니다.</b>
  </p>
  <p class="머리설명">
  ...
  ${안내('✅ 만드는 법도 닫혔습니다 — 아크릴 채널 글자 + LED (유호 확정 08-31)',
    '「네온사인」은 낱말이고 만드는 법은 셋이었습니다. 가른 것은 취향이 아니라 ' +
    '<b>«울란바토르에서 만들 수 있는가»</b>입니다(유호 지시 — 인테리어는 현지에서 되는 것만). ' +
    '<b>아크릴 채널 글자 + LED</b>만 현지 제작이 확인됐고(Print house ХХК 가 「3D 글자·라이트박스」 명시), ' +
    'LED 네온 플렉스·유리 네온은 UB 업체 다섯 목록에서 <b>못 찾았습니다</b>(「없다」가 아닙니다). ' +
    '곁들여 아크릴은 <b>로고를 안 건드립니다</b> — 이 그림처럼 «채워진 글자»를 그대로 잘라 냅니다. ' +
    '튜브 계열이면 글자가 <b>선</b>이 되어 훨씬 가늘어지고 로고 정본을 다시 그려야 했습니다. ' +
    '🔴 <b>조건부입니다</b> — 「현지에서 가능하다면」이 유호님 문면이고 업체 전화 확인 전입니다(공간설계 §9-15).')}

  <p style="margin-top:34px; font-size:12.5px; color:#7d736c; font-family:var(--모노); letter-spacing:.02em">
    node tools/네온시안.js · 정본 = 07_SYNK_메인룸_인테리어.txt v7 §4 · 색·공법 둘 다 닫힘 2026-08-31
  </p>
</div>
</body>
</html>
