 ...
 *   node tools/라디오배경굽기.js --장르 chuseok  한 장만
...
/* 각인 로고 = 09-10 승인 Paper 사진형. SYNK는 무실땀 펠트이며 사업명·기호의 스티치는 별도다.
   파일이 없으면 현행 중립색 벡터를 임시 표시하고 경고한다. */
/* 🔴 «각인용 작은 판»을 쓴다 — 오려 낸 원본(2804px · 7.6MB)을 data URI 로 통째로 실었더니
   ...
   🔑 자리가 docs/라디오 인 까닭 = docs/Loom_자산/구움 은 굽기 산출물이라 git 이 안 담는다(.gitignore).
      ...
      원본은 현재 승인 킷의 배치용 SYNK-Paper.png이며 표시 크기는 지면 CSS가 정한다. */
const 각인로고길 = path.join(ROOT, 'docs/홍보물/마케팅실행_20260909/브랜드킷/배치용/SYNK-Paper.png');
function 각인로고() {
  if (!fs.existsSync(각인로고길)) {
    console.warn(`⚠ 각인 로고 실물이 없다: ${각인로고길} — 옛 벡터 판으로 그린다(정본이 아니다)`);
    return 워드마크({ 판: '다크', 표현: '펠트', 색갈래: '단색' });
  }
...
const 장르들 = {
  chuseok: {
    이름: '추석', 가이드: '몽글', 표정: '눈웃음',
    바탕: 색['Deep Wool'], 글자: 색['Paper'], 보조: 색['Stone'],
    천: { 보풀: 색['Paper'], 그늘: 색['Ink Deep'], 어둡나: true },
    빛: { 후광: 'rgba(255,238,196,0.52) 12px -10px 20px', 그림자각: '-14px 12px 20px rgba(30,18,10,0.52)' },
    접지: { 폭: 0.84, 높이: 0.13, 색: 'rgba(74,52,38,0.42)', 흐림: 20 },
    앉는선: 0.885,          // 마당의 앞쪽 — 소반(아래 가장자리)보다 «뒤», 흙바닥 위에 선다
    /* 무대에 탁자가 안 잡혀서 calm 처럼 키울 일이 없다. citypop 과 같은 값으로 둔다. */
...
function 무대판(키) {
  const p = path.join(무대방, 장르들[키].원본 || `${키}.png`);
...
<div class="판">
  ...
  <div class="각인막"></div>
  <div class="각인">
    <!-- 2026-09-10 승인 Paper SYNK 사진형: 펠트 재질 유지, SYNK 실땀 제거. -->
 ...
 */
const 썸네일 = {
  무대: 'chuseok',          // 2026-09-09 선별에서 보존한 보름달 한옥 마당
  가이드: '몽글', 표정: '눈웃음',
  ...
  /* 🆕 09-08 두 번째 교체 (유호 지시 「공부에 한정하지마. 카페에서 틀어도 될수있게」) —
   *   「KOREAN STUDY RADIO」·「공부할 때 켜 두세요」를 걷었다. 둘 다 «듣는 조건»을 달아
   *   그 밖의 사람을 잘라 냈다. 이름 K-LOFI 24 는 산다 — K-lofi 는 장르 이름이지 조건이 아니다. */
  작은줄: '24/7 CHILL RADIO',
  큰줄: 'K-LOFI 24',
  받침줄: '언제나 곁에 두는 노래',
};
...
<div class="판">
  ...
  <div class="로고">${워드마크({ 판: '다크', 표현: '펠트', 색갈래: '단색' })}</div>
</div>`;
}
...
function main() {
  ...
  if (i >= 0 && (!하나 || !Object.hasOwn(장르들, 하나))) throw new Error(`모르는 장르: ${하나 || '(없음)'} (있는 것: ${Object.keys(장르들).join(' · ')})`);
