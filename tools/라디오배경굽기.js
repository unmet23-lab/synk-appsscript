...
const 장르들 = {
  ...
  /* 🆕 2026-09-07 신설 — 추석 시즌(2026-09-25). 유호 지시 「이제 곧 추석인데 혹시 추석 배경으로
   *   바꿔줄수있어?」 · 후보 셋 중 「보름달 뜬 한옥 마당」 확정.
   * 🔑 가이드는 **몽글**이다 — 달밤이라 무대가 어두운데 까몽은 검은 털이라 사라진다(calm 과 같은 규율).
   *   그 대신 마당(아래 절반)을 화면에서 제일 밝게 구워 두어서 코랄이 또렷하게 뜬다.
   * 🔑 광원은 오른쪽 위의 보름달이다 ⇒ 그림자는 왼쪽 아래로 눕는다(calm 과 같은 방향).
   * 🔴 악세는 안 붙인다(유호 확정 09-07 「한복입히고 배경만 신경쓰는거로 하자」) — 차림표는
   *   `bots/오버레이/마스코트.html` 의 `추석보름달마당` 이 쥔다. */
  chuseok: {
    이름: '추석', 가이드: '몽글', 표정: '눈웃음',
