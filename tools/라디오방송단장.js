...
/* 간판은 `bots/오버레이/라디오간판.js` 한 곳이 쥔다 — 09-08 에 모았다.
   🔴 자리가 `tools/lib/` 가 아니라 `bots/` 인 까닭: **라디오 서버에는 `tools` 가 없다**(bots·docs 뿐).
   서버가 읽는 파일은 bots 안에 있어야 한다. 09-08 에 이 주석만 옛 경로에 남아 있어
   낡음 검사가 「이름은 부르는데 없는 파일」로 잡았다(부르는 줄은 아래처럼 이미 옳았다).
