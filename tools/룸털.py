...
━━ 축 다섯 — 전부 «양쪽 끝이 다 실패한다». 한쪽만 적으면 다음 사람이 그쪽으로 민다 ━━
  ...
  ④ 변이       크면 → 헝클어진 「양털」                | 작으면 → 매끈한 「면」
                ⚠**엔진마다 반대다**(옛 Canvas 판 기준 — 그 판은 2026-08-19 유호 확정으로 폐기했다):
...
🔑 형태는 **ico sphere 를 superellipsoid 로 접는다**. 큐브+베벨+서브서프는 면 크기가 극도로
   ...
   정점을 직접 접는 수식이라 형태가 값 하나로 재현된다(축 파라미터가 곧 형태다).
...
# ── 축 ⑦ 형태 (2026-08-19 · 유호 「1,2,3,4 전부 진행해」의 ④) ──────────────────────
#   설계 §3-4b D 가 「남은 가장 큰 지렛대는 **형태 자체**다 — 값이 아니라 디자인 결정」이라
#   적어 둔 자리. 지금 부품은 전부 «구»이고, 명품이 명품인 이유의 절반은 형태가 남과 다른 것이다.
#   ⚠**값을 고르는 것은 유호님**이라, 도구는 «고르게 해 주는 것»까지만 한다 —
#     그래서 상수를 레버로 뺐다(A/B 를 굽는 자리 = `--지수`·`--축`).
#   지수: 2.0 = 순수 구 · 4.2 = 지금 판(모난 쿠션) · 8+ = 거의 정육면체.
NEXP = float(인자("--지수", "4.2"))
축비 = tuple(float(x) for x in 인자("--축", "1.00,0.97,0.83").split(","))
...
def 덩어리(이름, 위치, 크기, 밝기, 뭉치수, 자식수, 길이, 축=None, 흔들=0.052):
    축 = 축 or 축비
