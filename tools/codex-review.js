...
function 선파악행들(대상, 결과, 현황, now, 방향fp = 방향지문()) {
  const 대상칸 = { 종류: 대상.종류, 값: 대상.값 };
  const 기록행들 = [
    { 종류: '기능지도', 시각: now, 대상: 대상칸, 방향: 방향fp, 기능지도: 결과.기능지도 || [] },
  ];
  const 신규 = [];
  for (const p of 결과.제안 || []) {
    ...
    기록행들.push({ 종류: '제안', 시각: now, 대상: 대상칸, 방향: 방향fp, 키: k, ...p });
    신규.push({ ...p, 키: k });
  }
  return { 기록행들, 신규 };
}
...
function 제안실행(argv, 대상, timeoutMs) {
  ...
  const { 기록행들, 신규 } = 선파악행들(대상, 결과, 현황, now);
...
function main(argv) {
  ...
  if (argv.includes('--제안판정')) {
     ...
     * 대상은 원 제안 행에서 그대로 복사한다(새로 계산하면 같은 판정을 두 곳에서 정하게 된다). */
    append(제안경로, {
      종류: '판정', 시각: new Date().toISOString(),
      대상: p.대상 || null, 방향: 방향지문(),
      키: k, 상태: 채택 ? '채택' : '기각', 사유,
    });
