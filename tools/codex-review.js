...
function 심문실행(argv, timeoutMs) {
  ...
  모델설정.분석 = 정책.분석설정(argv, 정책.심문기본);   // ②의 기본은 luna/max·2회 — 이유는 모델정책.js
  모델플래그(모델설정.분석);
  const 회차 = 정책.회차설정(argv);
  ...
  if (회차 > 1) console.log('두 벌은 **합쳐 읽는다** — 한쪽에만 있는 지적이 곧 2회차가 값한 부분이다(합치는 건 사람이 한다 · 모델에게 시키면 조용히 떨어진다).');
