...
function 심문실행(argv, timeoutMs) {
  ...
  모델설정.분석 = 정책.분석설정(argv, 정책.심문기본);   // ②의 기본은 sol/max — 이유는 모델정책.js
  모델플래그(모델설정.분석);
  ...
  append(심문경로, {
    ...
    모델: 모델설정.분석.model, 효력: 모델설정.분석.effort, 방향: 방향지문(), 금지: 금지지문(),
...
function main(argv) {
  ...
  if (!argv.includes('--버그만')) {
    ...
    try {
      기체 = 스키마실행(기능체크프롬프트(diff본문, 채택제안들, 방향텍스트()), 기능체크스키마경로,
