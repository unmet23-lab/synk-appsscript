...
function 병렬단계(방, 단계, 회차, timeoutMs) {
  const 남은 = [];
  for (let i = 1; i <= 회차; i++) if (!단계끝났나(방, 단계, i)) 남은.push(i);
  if (남은.length < 2) return;

  console.log(`⚡ ${단계} 회차 ${남은.join('·')} 를 동시에 돌린다(${남은.length}프로세스) — 벽시계가 회차 수만큼 줄어든다.`);
  ...
  for (;;) {
    const 끝 = 남은.filter((i) => 단계끝났나(방, 단계, i) || 런.조각읽기(방, `실패-${단계}`, i));
    if (끝.length === 남은.length) break;
    if (Date.now() > 마감) { console.error(`   ⚠ ${단계} 병렬 대기 상한 — 부모가 남은 회차를 순차로 돈다.`); break; }
    동기대기(1000);
  }
  for (const c of 자식들) { try { c.kill(); } catch (_) { /* 이미 끝났으면 그만 */ } }
}

function codex실행(대상, timeoutMs, 회차 = 1, 방 = null, argv = []) {
  const 방경로 = 방 || 검수방(대상, 회차, argv);
  /* 대상은 자식이 다시 만들 수 없다 — 미커밋 판정(`대상결정`)은 **시점 의존**이라 몇 초 뒤
   * 남의 세션이 커밋하면 딴 diff 를 가리킨다. 부모가 정한 그것을 그대로 넘긴다. */
  런.json쓰기(방경로, '입력', 0, { 대상, timeoutMs, 회차, 분석: 모델설정.분석, 구조화: 모델설정.구조화 });
  if (회차 > 1 && !(argv || []).includes(직렬플래그)) 병렬단계(방경로, '검수', 회차, timeoutMs);
  const 회차들 = [];
...
function main(argv) {
  ...
  let 결과;
  try {
    결과 = codex실행(대상, 초 * 1000, 회차, 방경로, argv);
  } catch (e) {
    if (e.확인불가) {
      console.error('🔴 확인 불가 — 검수가 **안 돌았다**(통과가 아니다): ' + e.message);
  ...
  if (!argv.includes('--버그만')) {
    ...
    try {
      ...
      런.json쓰기(방경로, '입력', 0, { ...(런.json읽기(방경로, '입력', 0) || {}), 기능프롬프트: 프롬프트 });
      if (회차 > 1 && !argv.includes(직렬플래그)) 병렬단계(방경로, '기능', 회차, 초 * 1000);
      for (let i = 1; i <= 회차; i++) {
