...
const 안들 = {
  ...
  도장밤: {
    이름: '② 도장 · 밤 바탕 — ✅ 확정 (유호 픽 09-01) · @yuhobuilds',
    설명: '같은 배지, 어두운 바탕. 흰 피드에서 원이 또렷하다 · 업로드본 = yuhobuilds_프로필.png',
    바탕: 색['Ink Deep'],
    속: (px) => 도장({ px: Math.round(px * 0.86) }),
    밀기: `translateY(${도장세로보정})`,
    /* 유호님 픽(09-01) — 개인 채널의 «얼굴 사진» 자리를 로고가 대신한다.
     * ⚠ `@synkbrief`(⑤ 도장꽉)와 **같은 배지**다. 32px 에서 둘을 가르는 것은 «바탕색» 하나뿐이고
     *   (밤 vs 양모), 배지 자체는 구별되지 않는다. 유호님이 그걸 아시고 고르셨다 —
     *   두 채널이 같은 사람의 것이라 오히려 이어지는 편이 맞다는 판단. */
    확정: 'yuhobuilds_프로필',
  },
  코랄기호: {
    이름: '③ 코랄 면 · 기호',
