...
const 설명 = {
  ...
  형태후보: ['Loom 본문 부품이 지금 전부 <b>«구»</b>다(유리 원판·레진 구슬). 명품의 절반은 형태라, 그 «구»를 공방 문법으로 바꾸면 어떻게 읽히는지 보는 <b>대조 재료</b>다(설계 = 부품형태공방_0901 인계).',
          '<b>새 형태를 안 지었다</b> — 넷 다 이미 있던 것이고, 이 굽기가 한 일은 «같은 자로 다시 굽기»다. 지금 실물(퍼프로브_0819)은 <b>색(08-24)과 털(08-25)이 갈리기 전 판</b>이라 그대로 현행 부품 옆에 놓으면 재질 차이가 형태 차이로 읽힌다.',
          '자리: <b>매듭</b>=불릿(가장 많이 반복되는 부품) · <b>직조라벨</b>=태그 · <b>라벨태그/단추</b>=절번호 두 갈래 · <b>와펜/폼폼</b>=히어로 숫자.',
          '🔴 <b>히어로 숫자가 늦게 들어온 까닭</b> — 첫 판은 「레진 = 1점 액센트 존속」을 근거로 뺐는데, 유호님이 09-01 저녁 지면 실물을 보시고 뒤집으셨다(「이 쓰레기같은 오브 유리 싹다 삭제해줘 · 우리 좋은 털재질 요소들 많은데 왜 이걸 쓰는거야」). <b>구 계열은 은퇴 대상</b>이라, 현행을 나란히 두는 것은 «되돌릴 후보»가 아니라 before 다.'],
};
