...
function main() {
  ...
  (async () => {
    ...
    /* 🔴 기본이 4K 다(유호 확정 09-05 「기존 라디오 배경같은것도 전부 일감에 넣어줘 · 4K로 제대로 하자」).
     *   무대는 1280×720 으로 «내려» 쓰이지만, 제미나이는 같은 그림을 다시 못 내므로
     *   작게 구워 두면 나중에 큰 화면·인쇄에 못 올린다 — 4K 는 화질이 아니라 보험이다. */
    const 크기 = (() => { const i = argv.indexOf('--크기'); return i >= 0 ? argv[i + 1] : '4K'; })();
    if (!(await 배치게이트(목록.length, 크기))) { process.exitCode = 3; return; }
    ...
    console.log(`\n[라디오무대굽기] 대상 ${목록.length} = 구움 ${구움} + 건너뜀 ${건너뜀}`
      + ` · 든 돈 ≈ ${구움 * (크기 === '4K' ? 336 : 190)}원 · ${path.relative(ROOT, 낼곳)}`);
