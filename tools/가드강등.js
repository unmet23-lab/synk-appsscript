...
/* 🔴 인자 게이트 — `process.argv[2]` 하나만 읽으면 **뒤에 붙은 낱말이 조용히 사라진다**:
 *   `--상태 --되돌리기` 가 상태만 찍고 끝나서 「되돌렸다」로 읽힌다. 이 도구가 되돌리는 것은
 *   가드 10개의 발동 여부라, 안 되돌아간 것을 되돌아갔다고 읽으면 **가드가 죽은 채 초록**이 된다.
 *   새는 방향이 언제나 「통과」인 그 형태(CLAUDE.md 등록층 ③)라 판정을 공용 통로로 옮긴다.
 *   ⚠ 이 게이트가 틀릴 때의 모습 = **거짓양성** — 아는 낱말을 빠뜨리면 되던 명령이 죽고,
 *     그건 따를 수 없는 처방이 된다(F103). 그래서 목록을 아래 분기문 **바로 위**에 두어
 *     한 자리에서 같이 고치게 한다. 회귀: `tests/도구인자게이트.test.js`. */
...
if (플래그오류) {
  console.error(플래그오류);
  console.error('위 낱말 중 하나를 쓴다 — 오타면 가드가 안 돌아온 채 「돌아왔다」로 읽힌다.');
  process.exit(1);
}

/* 🔑 게이트는 `--` 낱말만 본다. 두 개를 같이 준 자리는 여기서 막는다 — 이 도구는 상태를 바꾸므로
 *   「둘 중 어느 쪽을 시킨 것인지」가 애매한 채로 실행하면 안 된다. */
if (인자들.length > 1) {
