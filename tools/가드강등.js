...
/* 위치 인자(대시 없는 낱말)는 게이트가 못 본다 — 「가드강등 상태」 오타가 조용히 «적용»이 되면
 * 재운 줄 알고 안 재워진다(머지 해소 08-19: 두 세션이 같은 수리를 했고, 이 꼬리는 내 판을 보존). */
const 인자 = 인자들[0];
if (인자 === '--되돌리기') 되돌리기();
else if (인자 === '--개원') 개원();
else if (인자 === '--상태') 상태();
  ...
  console.error('쓰는 법: node tools/가드강등.js [--상태|--개원|--되돌리기] (낱말 없이 부르면 적용)');
  process.exit(1);
}
