...
function 보고(argv = []) {
  ...
  if (!r.없음.length && !r.중복.length) {
    /* 초록도 **분모와 함께** 낸다 — 옆 게이트들과 같은 규약(F207). 조용한 0 은 「이 커밋의 자리가
     * 아니었다」와 구별이 안 되고, 그 둘이 같은 모양이면 게이트가 죽어도 아무도 모른다.
     * ⚠ 대상이 아닐 때는 위에서 이미 조용히 빠졌다 — 여기 오는 것은 «실제로 쟀다»뿐이다. */
    console.error(`[채번] 깨끗하다 — 열린 ${r.분모}줄을 봤다 (최대 #Q${r.최대} · ${층}).`);
    return 0;
  }
