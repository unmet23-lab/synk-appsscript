...
function 현황() {
  ...
  const 항목 = 큐항목들();
  if (항목) {
    const 산것 = new Set(항목.map((it) => it.지문));
    const 채점대기 = 목록.filter((p) => !p.판정 && !산것.has(String(p.지문)));
    if (채점대기.length) {
      console.log(`\n🔔 채점 대기 ${채점대기.length}건 — 봉인했는데 **큐에서 사라졌다**(답이 왔거나 문구가 바뀌었다):`);
      for (const p of 채점대기) {
...
module.exports = {
  파싱, 봉인검증, 모양판정, ㄹ지형, 접기, 집계, 비율, 시점갈래,
