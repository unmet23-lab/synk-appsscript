...
/* ── 위임(처방 ③) — 유호 확정 2026-08-18 「2일로 하자」 ──────────────────
 * 무응답이 N일 이어지면 봉인한 예측을 그대로 확정한다. 유호님은 **뒤집을 것만 뒤집는다.**
 *
 * 🔑 안전선을 새로 만들지 않았다 — **축이 이미 안전선이다.** 축뜻을 그대로 읽으면
 *   「기술 = 재료가 저장소 안에 있다」 하나만 내가 근거를 들 수 있고, 나머지 넷은
 *   재료가 유호님 눈·지갑·관계·뜻에만 있다. 그런 질문은 100일을 기다려도 내 예측이
 *   근거를 못 얻는다 — 기다림이 정보를 안 낳는 축에 시한을 걸면 그건 위임이 아니라 추측이다.
 *
 * 🔴 대가 — 이 장치가 «맞는 얼굴로 틀린 값»을 내는 자리 셋:
 *   ① **재현율이 아직 0/0 이다**(봉인 16 · 채점 0 · 08-18 실측). 즉 「기술 축에서 내가 몇 %
 *      맞히는가」를 모르는 채로 위임이 시작된다. 처방 순서는 ①봉인→②백테스트→③위임인데
 *      ②를 건너뛰는 것이다. 이것을 견딜 수 있게 하는 유일한 근거가 「되돌림이 싸다」이므로,
 *      **되돌리기를 못 적는 건은 익어도 후보가 아니다**(`위임보기` 가 인쇄한다).
 *   ② **「큐에 살아 있다」는 「유호님이 안 봤다」가 아니다.** 지문은 줄 원문 해시라 문구만
 *      고쳐도 달라진다(급소 ②와 같은 한계). 그래서 익은 건도 **집행이 아니라 확정까지**만 간다.
 *   ③ **날짜는 봉인일이지 배달일이 아니다.** 항목별 배달 이력 장부는 이 저장소에 없다
 *      (큐가 아는 시각은 메모리 «파일» mtime 뿐이라 줄 하나만 고쳐도 전건이 리셋된다).
 *      봉인일 ≤ 배달일이므로 이 계산은 **유호님께 불리한 쪽으로 절대 안 틀린다** — 실제보다
 *      나이를 크게 잡는 일이 없다는 뜻이 아니라, 봉인 시점에 이미 큐에 있던 줄만 세기 때문이다.
 *
 * 닫을 것 1개: 익은 건을 **자동으로 확정하는 실행부는 여기 없다**(`--위임` 은 목록만 낸다).
 *   확정은 메모리 쓰기 + 자율기록 + 되돌리기 기록이 한 벌이라, 그 셋을 한 커밋에 못 넣는
 *   세션에서는 시작하지 않는다. 목록이 먼저 서고 유호님이 그 목록을 한 번 보신 뒤에 붙인다. */
const 위임축 = ['기술'];
const 위임일수 = 2;
...
/** 두 `YYYY-MM-DD` 사이의 날수(뒤 - 앞). 못 읽으면 **0이 아니라 null** —
 *  0을 돌려주면 「날짜가 깨진 줄」이 「오늘 봉인한 줄」과 같은 모양이 된다(F207). */
function 일수차(앞, 뒤) {
  const a = Date.parse(`${String(앞 || '').trim()}T00:00:00Z`);
  const b = Date.parse(`${String(뒤 || '').trim()}T00:00:00Z`);
  if (!Number.isFinite(a) || !Number.isFinite(b)) return null;
  return Math.floor((b - a) / 86400000);
}

/** 위임 후보를 가른다. **떨어진 것도 사유별로 돌려준다** — 「익음 0」만 내면
 *  그것이 「아직 안 익었다」인지 「축에서 전부 막혔다」인지 「봉인이 없다」인지 안 갈린다.
 *  @param 산지문들 지금 큐에 살아 있는 지문들(= 유호님이 아직 안 답한 것) */
function 위임후보(목록, 산지문들, 오늘값, 일수 = 위임일수) {
  const 산것 = new Set((산지문들 || []).map(String));
  const r = { 익음: [], 예고: [], 어림: [], 막힌축: [], 자유: [], 추론: [], 날짜깨짐: [] };
  for (const p of 목록 || []) {
    if (p.판정) continue;                       // 채점됨 = 유호님이 이미 답하셨다
    if (!산것.has(String(p.지문))) continue;    // 큐에서 사라짐 = 채점 대기(급소 ②)
    if (!닫힌모양.includes(p.모양)) { r.자유.push(p); continue; }
    if (p.근거 !== '권고있음') { r.추론.push(p); continue; }
    if (!위임축.includes(p.축)) { r.막힌축.push(p); continue; }
    const 나이 = 일수차(p.날짜, 오늘값);
    if (나이 == null) { r.날짜깨짐.push(p); continue; }
    const 그것 = { ...p, 나이 };
    if (나이 >= 일수) r.익음.push(그것);
    else if (나이 >= 일수 - 1) r.예고.push(그것);   // 내일 익는다 — 유호님이 오늘 뒤집을 수 있게
    else r.어림.push(그것);
  }
  return r;
}
...
/** 위임(처방 ③) 후보를 드러낸다. **목록만 낸다 — 확정하지 않는다**(상수 절의 「닫을 것」). */
function 위임보기(인자) {
  const 준일수 = Number(인자값(인자, '--일수'));
  const 일수 = Number.isFinite(준일수) && 준일수 > 0 ? 준일수 : 위임일수;
  const 항목 = 큐항목들();
  if (!항목) return 1;                       // 「못 읽었다」와 「0건」을 안 섞는다
  const { 지금배달 } = 시점갈래(항목);
  const { 목록 } = 접기(읽기());
  const r = 위임후보(목록, 지금배달.map((it) => it.지문), 오늘(), 일수);
  const 산것 = 목록.filter((p) => !p.판정 && 지금배달.some((it) => it.지문 === String(p.지문)));
  const 계 = 집계(목록);

  console.log(`\n[위임] 무응답 ${일수}일 → 봉인한 예측을 확정 (유호 확정 2026-08-18)`);
  console.log(`  분모 — 봉인 ${목록.length}건 · 그중 큐에 살아있고 미채점 ${산것.length}건 · 큐 배달 ${지금배달.length}건`);

  const 줄 = (p) => `     [${p.지문}] ${p.축}·${p.모양}·확신${p.확신}${p.나이 == null ? '' : ` · ${p.나이}일`}\n     예측: ${p.예측}\n     ${String(p.질문).slice(0, 140)}`;
  if (r.익음.length) {
    console.log(`\n  🟢 익음 ${r.익음.length}건 — 확정 대상`);
    r.익음.forEach((p) => console.log(줄(p)));
  } else {
    console.log(`\n  🟢 익음 0건`);
  }
  if (r.예고.length) {
    console.log(`\n  🟡 내일 익음 ${r.예고.length}건 — 오늘 뒤집으시면 확정 안 됩니다`);
    r.예고.forEach((p) => console.log(줄(p)));
  }
  console.log(`\n  떨어진 사유 — 아직 어림 ${r.어림.length} · ⛔축 ${r.막힌축.length} · 자유서술 ${r.자유.length} · 추론(권고 없음) ${r.추론.length} · 날짜깨짐 ${r.날짜깨짐.length}`);
  console.log(`     ⛔축 = ${축값.filter((a) => !위임축.includes(a)).join('·')} — 재료가 유호님 눈·지갑·관계·뜻에만 있어 기다림이 정보를 안 낳는다`);

  console.log(`\n  ⚠ 재현율 ${비율(계.적중, 계.분모)} — 「기술 축에서 내가 몇 %를 맞히는가」를 아직 못 쟀다.`);
  console.log(`     처방 순서는 ①봉인 → ②백테스트 → ③위임인데 ②가 비어 있다. 이것을 견디는 근거는 「되돌림이 싸다」 하나뿐이다.`);
  console.log(`  ⚠ 이 목록은 «판정 확정» 대상이지 «집행»이 아니다 — 집행은 기존 승인 게이트(돈·외부 발송·라이브·운영 DB·삭제)를 그대로 지난다.`);
  한계인쇄();
  return 0;
}
...
function main(argv) {
  ...
  if (인자.includes('--위임')) return 위임보기(인자);
  if (!인자.length || 인자.includes('--목록')) return 현황();
  console.error('사용: --대상 [--자유포함] | --봉인 --파일 x.json | --채점 <지문> --판정 … --실제 "…" --인용 "…" | --재현율 | --위임 [--일수 N]');
  return 1;
}
