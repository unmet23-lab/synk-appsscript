 ...
 *
 * ■ 쓰는 법
 *   아래 실제 API 조회·생성 명령에는 모두 `--유료-api`를 함께 붙인다.
 *   node tools/음성실측.js --목록 --유료-api        # 이 키로 보이는 TTS 모델을 API 에 물어본다(굽기 0회)
 *   node tools/음성실측.js --모델 --유료-api        # 후보 모델들 × 문장 1개 — 어느 모델이 사나
 *   node tools/음성실측.js --문장 --유료-api        # 사는 모델 × 문장 4개(몽골어 3 + 한국어 대조군 1)
 *   node tools/음성실측.js --목소리 --유료-api      # 사는 모델 × 문장 1개 × 목소리 여럿
 *   node tools/음성실측.js --역듣기 --유료-api      # 구운 wav 를 다시 받아쓰게 해 원문과 낱말 대조
 *   node tools/음성실측.js --전부 --유료-api        # 위 넷을 차례로
