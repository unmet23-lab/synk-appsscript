...
<header><div class="inner">
  ...
  ${/* ⚠ 숫자는 «합계 = 갈래+갈래»로 쪼갠다(유호 확정 08-14) — 「대기 26」만 크게 띄우면
        ...
        Annex B 로 통과시켜 진짜 사고를 숨긴다(F307 · 옛 회귀 tests/문서그래프.test.js 는 08-19 대청소로 걷혔다 — 지금 상시로 막는 기계는 없다).
...
function 등재() {
  try {
    /* 갈래를 **명시한다** — 기본 갈래에 기대면 운영자료.js 의 기본값이 바뀌는 날 이 기록의 자리가 조용히 옮겨진다
     *   (08-20 재편 때 실제로 그랬다 · codex P2 e90ba9e7aa36). 이 기록은 유호님이 «판단»하러 여는 것이라
     *   판단 층(「SYNK 방향」 — 철학·확정·계기판)에 둔다. 학원 실무(사업·운영) 사이에 묻히면 열리지 않는다. */
 ...
 *  ⚠ 한 인자를 파이프로 갈라 받지 않는다: `tools/` 안의 `.split('|')` 은 `tests/표칸.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)`
