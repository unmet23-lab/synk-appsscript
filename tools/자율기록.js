 ...
 *      맡기면 F296 이 그대로 재발한다(`friction.js` 에 `--파일` 이 없어서 인자 토큰이
  ...
  const 카드 = 목록
    .map((d) => {
      ...
      const 메모 = d.판정메모
        ...
        : '';
      return `<article class="card" data-r="${esc(d.결과)}" data-a="${esc(d.영역 || '')}">
  <div class="head">
...
<header><div class="inner">
  ...
  <div class="counts">
    <div class="hot">대기<b>${셈('대기')}</b></div>
    <div>유지<b>${셈('유지')}</b></div>
    <div>바꿈<b>${셈('바꿈')}</b></div>
    <div>보류<b>${셈('보류')}</b></div>
    <div>전체<b>${목록.length}</b></div>
  </div>
</div></header>
...
(function(){
  ...
  function 적용(){
    document.querySelectorAll('.card').forEach(function(c){
      var ok=(!상태.r||c.dataset.r===상태.r)&&(!상태.a||c.dataset.a===상태.a);
...
function main(argv) {
  ...
  console.error('사용: add --파일 x.json | 판정 --번호 N --결과 유지|바꿈|보류 | --목록 | --굽기');
  return 1;
}
