...
function 실물에서뽑기(text) {
  /* 🔴 09-06 — HTML 주석(`<!-- 몽골어검문: 대상 -->`)이 그대로 실려 나갔다. 다섯 층이 전부 그걸
     «본문»으로 읽는다: 맞춤법 자는 모르는 낱말로 세고, 뜻 대조는 「원문에 없는 문장」으로 센다. */
  const 줄들 = String(text)
    .replace(/^﻿/, '')
    .replace(/<!--[\s\S]*?-->/g, '')
    .split(/\r?\n/);
  const 뽑음 = [];
  ...
  /* 🔴 09-06 — 여기서 줄을 **빈칸으로** 이어 붙이고 있었다. 캐러셀 원고는 「제목 줄 + 본문 줄」이
     장마다 서는 슬라이드 묶음이고 제목 줄엔 마침표가 없다. 빈칸으로 이으면 제목이 다음 문장에
     한 문장으로 붙어(`…төлөвшдөг Хичээлээр…`) 문법 층이 **없는 파손**을 본다 — 09-06 검문에서
     캐러셀 넷이 「전건 탈락」한 까닭의 절반이 이것이다.
     ⇒ 줄바꿈으로 잇는다. 그러면 자가 «슬라이드 묶음»을 슬라이드 묶음으로 읽는다.
        (자를 먼저 의심한다 — [[zero-is-a-success-face-taxonomy]]) */
  return 뽑음.join('\n').trim();
}

/* --파일 모드: 한국어 블록 / 단독 줄 --- / 몽골어 블록. 형식이 어긋나면 null(크게 실패).
 * 🔴 09-06 — 몽골어«만» 든 원고에 `--원문` 없이 이 길로 들어오면 **조용히 엉뚱하게 갈렸다.**
 *   캐러셀 원고는 `---` 를 슬라이드 구분으로 쓰므로 첫 `---` 에서 잘리고, 앞의 머리말 네 줄이
 *   「한국어 원문」, 나머지 전부가 「몽골어 번역」이 된다. 그러면 뜻 대조 층이 «머리말 vs 본문»을
 *   재서 「원문에 없음 → …추가」를 여덟 줄씩 낸다 — 통째로 헛것이고, 그게 그대로 장부에 도장으로
 *   남았다(09-06 캐러셀 넷 「전건 탈락」의 나머지 절반).
 *   ⇒ 앞 토막에 한글이 한 자도 없으면 **짝 파일이 아니다.** 거절하고 `--원문` 을 가리킨다. */
function 파일분해(text) {
  ...
  /* 🔑 «한글이 있나»를 날글로 물으면 안 된다 — 캐러셀 원고의 머리말이 `훅:`·`언어:` 라 한글이 있다.
     머리말·표식을 걷고 나서 몸통에 한글이 남는지 본다(그 걷는 자는 `실물에서뽑기` 하나다). */
  if (!/[가-힣]/.test(실물에서뽑기(원문))) return null;
  return { 원문, 번역 };
}

// 산문 정규식 폴백 — 형식을 안 지킨 답 = null = 검수 필요(통과 아님).
// 🔴 09-06(철학 5회차 심문 A6): 「판정: 정상 아님. 문법이 파손되어 있습니다.」가 `정상` 으로 읽혔다 —
//    낱말만 잡고 그 뒤를 안 봤다. 낱말 뒤에 부정(아님·아니·않·없)이 붙으면 판정불능(null)으로 낸다.
//    「파손 아님」을 정상으로 «뒤집어» 읽지도 않는다 — 산문은 못 믿으니 사람 검수 쪽으로 실패한다.
function 판정추출(text) {
  const s = String(text || '');
  const m = /판정\s*[:：]?\s*(정상|어색|파손)/.exec(s);
  if (!m) return null;
  /* 한글은 음절이 한 글자다 — 「아님」·「아닙니다」에 「아니」는 «안 들어 있다»(님·닙은 다른 글자). 그래서 낱자로 잡는다.
   * 🔴 6회차 심문(아스트라): 「정상으로 볼 수 없습니다」·「정상이라 할 수 없다」처럼 부정이 몇 어절 뒤에 오는 꼴 3/3 이
   *    첫 수리를 지나갔다. 그래서 낱말 바로 뒤만 보지 않고 **그 문장 끝(마침표·줄바꿈)까지** 부정 낱자가 있으면 판정불능이다.
   *    「판정: 정상. 이유: 문제 없음」의 「없음」은 마침표 뒤라 안 잡힌다 — 새는 방향은 «판정불능 = 사람 검수»다. */
  const 뒤 = s.slice(m.index + m[0].length).split(/[.。!?\n]/)[0].slice(0, 40);
  if (/(?:아[니님닙닌]|않|없|못)/.test(뒤)) return null;
  return m[1];
}
...
async function main() {
  ...
  /* 🔑 **`--원문없음` — 한국어 짝이 없는 몽골어도 «잴 수 있는 만큼» 잰다**
   *   (2026-09-07 · 유호 확정 「몽골어가 들어가는 모든 번역은 노출 전에 1차 검수를 지난다」).
   *   실측이 이 갈래를 부른다: 학생 화면 코드의 몽골어 조각 880개 중 한국어 짝을 자동으로 뽑을 수
   *   있는 것은 열셋뿐이었다. 나머지를 «못 잰다»로 두면 877개가 영영 아무 겹도 안 지난다.
   *   🔴 **이건 반쪽이다.** 다섯 겹 중 «문법·맞춤법» 둘만 돈다 — 역번역·원문 대조·말투·뜻은
   *     원문이 있어야 서는 겹이라 여기서는 «미측정»으로 남는다(0건이 아니다).
   *   ⇒ 잡는 것 = 문법 파손·어색·오타. 못 잡는 것 = **뜻이 딴 데로 간 번역**(가장 위험한 갈래). */
  const 원문없음 = argv.includes('--원문없음');
  /* 🔑 인자가 «하나»뿐이면 그것이 몽골어다 — 위의 구조분해가 첫 자리(ko)에 넣어 두므로 옮긴다.
   *   안 옮기면 몽골어가 「한국어 원문」 자리에 앉아 아무 층도 안 돈다(09-07 에 그 자리를 밟았다). */
  if (원문없음) {
    if (ko && !mn) { mn = ko; }
    ko = '';
  }
  ...
  /* 원문이 없으면 역번역을 «부르지 않는다» — 되돌려 놓고 견줄 원문이 없으니 한 발이 통째로 낭비다.
   * 아래 층들은 `역실패` 를 보고 스스로 미실행로 남는다(원문 대조·말투·뜻). */
  const 역번역 = 원문없음 ? '' : await 제미나이(key, model,
  ...
  let 맞춤법 = await 맞춤법약속;
  /* 🔁 **한 번만 다시 던진다** (2026-09-07 · 유호 확정 「모든 번역에 1차 검수」).
   *   실측: 09-07 하루 229회 중 이 층이 **25회** 안 돌았다. 한 번 못 닿은 것이 곧 「사람 검수 필요」가
   *   되면, 사람이 볼 줄이 «진짜 결함»이 아니라 «우리 네트워크 사정»으로 늘어난다.
   *   🔴 **몫 초과에는 다시 안 던진다** — 벽에 대고 던지면 벽이 길어진다(아래 안내가 그렇게 말한다).
   *     그 밖의 실패(순간 장애·형식)만 3초 쉬고 한 번. 두 번은 안 한다. */
  if (맞춤법 === null) {
    const 첫사유 = require('./lib/몽골어맞춤법.js').맞춤법검사.마지막사유 || '';
    if (!/몫 초과|429/.test(첫사유)) {
      await new Promise((r) => setTimeout(r, 3000));
      맞춤법 = await 맞춤법검사(mn).catch(() => null);
    }
  }
  ...
  if (!역실패) {
    if (한증인) {
      ...
      뜻 = await 뜻대조(ko, 역번역);
      /* 🔁 여기도 한 번만 다시 (실측 09-07 = 229회 중 10회 안 돌았다).
       *   벤더 한도일 때는 `뜻대조()` 안의 게이트가 **0발로 즉시 null 을 돌려주므로** 이 재시도가
       *   벽을 때리지 않는다 — 그래서 사유를 따로 안 가른다(가르는 자는 그 함수가 이미 쥐고 있다). */
      if (뜻 === null) {
        await new Promise((r) => setTimeout(r, 3000));
        뜻 = await 뜻대조(ko, 역번역);
      }
   ...
   * 🔴 `--원문없음` 은 **통과를 못 낸다.** 두 겹만 잰 것을 「통과」라 적으면 그 도장이 거짓이 된다.
   *   대신 두 겹이 깨끗하면 「반쪽 깨끗」으로 적어 사람이 그 사실을 알고 읽게 한다. */
  const 두겹깨끗 = !!문법 && 문법.판정 === '정상' && !맞춤법미실행 && !맞춤법.의심.length;
  const 통과 = 원문없음 ? false : (!!문법 && 문법.판정 === '정상' && !역실패 && !말투미실행 && !말투.length
    && !맞춤법미실행 && !맞춤법.의심.length && !뜻미실행 && 뜻.같다);
  const 사유 = 원문없음
    ? (!문법 ? '판정불능(원문없음)'
      : 문법.판정 !== '정상' ? `문법 판정: ${문법.판정} (원문없음 · 두 겹만)`
      : 맞춤법미실행 ? '맞춤법 층 미실행 (원문없음 · 두 겹만)'
      /* 🔴 [09-08] **낱말을 적는다.** 여기만 낱말 없이 「N건」이라고만 남기고 있었다(다섯 겹 갈래는
       *   아래에서 낱말을 붙인다). 사람 눈으로 갈 수백 줄이 「1건」이라고만 적히면 다음 사람이
       *   그 줄을 전부 다시 돌려야 한다 — 장부는 「무엇이 걸렸나」까지 쥐어야 자 구실을 한다. */
      : 맞춤법.의심.length ? `맞춤법 의심 ${맞춤법.의심.length}건(${맞춤법.의심.slice(0, 5).join('·')}) (원문없음 · 두 겹만)`
      : '반쪽 깨끗 — 문법·맞춤법 두 겹은 깨끗하다. 뜻이 맞는지는 **안 쟀다**(한국어 원문이 없다)')
    : !문법 ? '판정불능'
    : 역실패 ? '역번역 실패'
    ...
    : '통과';
  const 얼굴 = 통과 ? '✅ 기계 검문 통과 — 대외 확정본은 그래도 사람 검수'
    : (원문없음 && 두겹깨끗) ? '🟡 반쪽 — 두 겹만 쟀다(뜻은 안 쟀다)'
    : '🔴 사람 검수 필요';
  console.log(`\n■ 종합: ${얼굴} (${사유})`);
  ...
  /* 🔴 장부에 못 남긴 검문은 «셈에 안 잡힌다» — 통과였어도 종료코드 1(실행 오류)로 낸다(09-06 · 철학 4회차 G1 = 5회차 G3).
   *   0 을 내면 CI·배포 게이트가 「검문했다」로 읽는데 드리프트를 잴 장부 행이 없다 — 안 잰 것이 통과로 읽히는 병.
   *   0 = 검문 통과 + 장부에 남음 · 1 = 실행 오류(장부 실패 포함) · 2 = 검문 불통과. */
  /* 🔴 **`process.exit()` 를 부르면 안 된다**(09-08 실측 · 이 자리에서 100% 재현).
   *   제미나이(Vertex)를 부른 뒤 `process.exit()` 로 끊으면 노드가 종료 도중 죽는다:
   *     `Assertion failed: !(handle->flags & UV_HANDLE_CLOSING), file src\win\async.c, line 94`
   *   죽으면 우리가 정한 0·1·2 대신 **크래시 코드**(bash 127 · spawnSync 3221226505)가 나가고,
   *   이 도구를 자식으로 부르는 쪽은 판정을 「확인 불가」로 읽는다. 실제 피해:
   *   `tools/몽골어출구.js --태우기` 가 정상 판정 셋을 «막힘»으로 세고 세 개 만에 멈췄다(855개 대청소가 안 돌던 까닭).
   *   ⇒ 종료코드는 **`process.exitCode` 로 «적어 두고» 자연 종료**한다(같은 자리 실측: 크래시 0 · 코드 그대로 나간다).
   *   ⚠ 그러니 이 아래에 「끝나고 도는 것」을 붙이지 마라 — 여기서 끊지 않는다. */
  process.exitCode = 실패 ? 1 : (통과 ? 0 : 2);
}

module.exports = { 키추출, 판정추출, 문법파싱, 파일분해, 실물에서뽑기, 재시도가능, 토큰대조, 말투대조, 지문, 장부쓰기, 장부경로 };

if (require.main === module) {
  main().catch((e) => {
    console.error('실행 오류:', e.message);
    process.exitCode = 1; // 위 §종료코드와 같은 까닭 — 호출 뒤 exit() 는 노드를 죽인다(09-08)
  });
}
