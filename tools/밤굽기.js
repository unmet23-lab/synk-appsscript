 ...
 *   ⓪몽글 친구 5 → ①화면 14(옛 AgX 자)+덧씌움 → ②NPC 몸색 재료 8 → ③NPC 20 → ④시트
 *     → ⑤라이브러리 52 → ⑥검사
 *   ⓪ 친구 5장은 **맨 앞**이다(요소 세션 합의 08-25 `c4c56d91`) — 중간에 끼우면 ⑥검사가
 ...
 *   ⏱ 친구 5 ≈3.5시간(GPU OOM 되물림 전제) · 화면 ≈1.5시간(미실측) · NPC 28 = 6.2시간(실측
...
const 친구밖 = path.join(루트, 'docs', '캐릭터', '친구공방_0825');
...
/* ⓪ 몽글 친구 5장 — 표정 세트(§2-H · 유호 확정 08-25 「명품화 + 몽글이처럼 많은 표정」).
 *   **맨 앞**에 두는 까닭(요소 세션 합의 08-25 · `c4c56d91`): 중간에 끼우면 뒤의 ⑥검사가
 *   기준 시각으로 셀 때 갈래를 오독한다(08-24 거짓 빨간불 170장이 그 자리였다).
 *   🔴 **본체도 다시 굽는다** — `몽글친구_v2.png` 는 옛 털 자로 구운 판이다. 표정 넷만 새로 구우면
 *      세트 안에서 결이 갈린다(bake-from-a-moving-canon).
 *   ⏱ 조율판(자식 35·700px·40샘플)은 GPU(ONEAPI)로 1.6~1.8분에 난다 — 실측 08-25 17:0x.
 *      정본(자식 70·1024px·96샘플)은 470만 가닥이라 OOM 될 공산이 크고, 되물리면 장당 ~41분이다.
 *      그래서 첫 장만 GPU-먼저로 재고, 되물렸으면 **남은 넷은 처음부터 CPU** 로 간다 —
 *      되물림 준비를 네 번 더 치르지 않게. 판정 표지는 인코딩 안 타는 ASCII(`OUT_OF_RESOURCES`)와
 *      «걸린 시간» 둘을 같이 본다(로그 한글은 CP949 로 깨져 표지로 못 쓴다 — 트랙 §0).
 */
단계('⓪ 몽글 친구 5장 — 1024px · 샘플 96 · 새 털 정본', () => {
  fs.mkdirSync(친구밖, { recursive: true });
  let 실패 = 0;
  let 장치 = 'GPU';                       // 첫 장이 답한다
  for (const 표정 of ['본체', '눈감음', '눈웃음', '우34', '좌34']) {
    const 파일 = path.join(친구밖, `친구_${표정}.png`);
    const t0 = Date.now();
    const r = spawnSync(BL, ['-b', '-P', path.join(루트, 'tools', '몽글친구굽기.py'), '--',
      `저장소=${루트}`, `출력=${파일}`, `표정=${표정}`,
      '너비=1024', '비율=1.0', '샘플=96', '조리개=2.8', `장치=${장치}`],
      { cwd: 루트, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 });
    /* 성공 판정은 «이 굽기가 새로 썼나»(mtime) — ②와 같은 자. */
    let 새로 = false;
