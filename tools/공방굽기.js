...
/* 🔴 09-07 — `--help` 를 붙였더니 **그대로 굽기 시작했다**(336원이 나갔다). 모르는 손잡이는
 *   위 파서가 조용히 무시하고, 손잡이가 하나도 안 걸리면 계획 «전체»의 「아직」이 과녁이 된다.
 *   묻는 말을 굽는 말로 읽는 도구는 위험하다 — 도움말은 여기서 끝낸다(0원). */
if (인자.help || 인자.h || 인자.도움 || 인자['?']) {
  console.log([
    '공방굽기 — docs/공방/계획.json 의 「아직」을 굽는다. ⚠ 돈이 든다(4K 한 장 ≈ 336원).',
    '',
    '  node tools/공방굽기.js --목록                      # 무엇이 남았나만 본다(0원)',
    '  node tools/공방굽기.js --묶음 "손 도구"            # 그 묶음의 「아직」을 전부',
    '  node tools/공방굽기.js --묶음 "손 도구" --것 가위   # 그중 하나만',
    '  node tools/공방굽기.js --묶음 "손 도구" --크기 4K',
    '',
    '🔴 --묶음 을 안 주면 계획 «전체»가 과녁이다 — 남의 트랙 몫까지 함께 나간다.',
  ].join('\n'));
  process.exit(0);
}
...
(async () => {
  ...
  for (let i = 0; i < 굽을것.length; i++) {
    ...
    /* 🔑 계획 항목에 `참조` 가 있으면 그 그림을 같이 보낸다(옷입히기처럼 «누구에게 맞는지»가
     *   있어야 크기·모양이 서는 것들). 없으면 여태처럼 문장만 보낸다 — 옛 항목은 그대로 돈다.
     *   경로가 하나라도 없으면 `한컷` 이 굽기 «전»에 던진다(빈 참조로 구우면 색이 갈린다). */
