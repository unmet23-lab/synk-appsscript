...
(async () => {
   ...
   *   🔑 09-09 배경 정리 후 기본 = `docs/라디오/무대/dream_field.png`(남긴 13번 반딧불 노을 들판).
   *     삭제한 노을 해변을 다시 읽지 않는다. 따뜻한 노을의 마무리 역할은 남긴 들판이 잇는다.
   *   ⚠ 배경도 우리가 구운 «펠트 실물»이라 재질이 캐릭터와 한 몸이다(CSS 흉내가 아니다). */
  const 무대 = path.join(뿌리, 'docs', '라디오', '무대', (process.env.SYNK_무대 || 'dream_field') + '.png');
