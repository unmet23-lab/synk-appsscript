...
def 케이크조각(위치, 바닥z, 돌림=0.0):
    ...
    for 이름, 반경, 두께, 천 in (("시트1", 1.00, 0.20, 시트천), ("크림1", 1.02, 0.07, 크림천),
        ...
        층.append(o)
        z += 두께
    # 위의 열매 — 케이크의 «한 점». 시선이 여기서 멈춘다(4계 ②).
    윗열매 = 구("케이크열매", 0.145, (x + 0.42 * math.cos(math.radians(돌림)),
                                  y + 0.42 * math.sin(math.radians(돌림)), z + 0.115), 세그=56)
...
def 로고찡긋플레이팅세우기():
    ...
    bpy.ops.mesh.primitive_cylinder_add(radius=3.0, depth=0.20, vertices=192, location=(0, 0, -0.25))
    ...
    bpy.ops.mesh.primitive_torus_add(major_radius=2.96, minor_radius=0.13, location=(0, 0, -0.15),
    ...
    알판들 = []
    for 갈래이름, 위치, 롤, 기울 in (
            ("버터",  (-1.30, 0.75,  0.018), -14.0, ((0.62, 0.78, 0.0), 2.2)),
            ("라피스", (-0.30, -0.72, 0.155),   7.0, ((0.8268, 0.5625, 0.0), 8.53)),
            ("팝",    ( 1.35, 0.85,  0.014),   6.0, ((-0.85, 0.53, 0.0), 1.6))):
    ...
    for i, (x, y, r, 회전) in enumerate((
            (1.90, -1.10, 0.34, (9, -16, -28)),      # 오른쪽 앞 — 열매 무리가 이 위에 앉는다
            (-1.88, -1.02, 0.30, (7, 13, 41)))):     # 왼쪽 앞 — 아몬드 무리의 받침
    ...
    열매천 = 펠트재질("PopDeep", 스케일=3.6, 돌림=11.0, 염색보정=True, 채도=1.15, 밝기=0.82,
                   결흩기=0.5, 거칠기=0.72)
    for i, (x, y, r) in enumerate(((1.76, -1.27, 0.180), (2.06, -1.03, 0.142), (1.58, -1.62, 0.126))):
    ...
    for i, (x, y, r, 눕힘) in enumerate((
            (-2.02, -0.55, 0.155, (0.18, 0.10, -0.55)),
            (-1.74, -1.34, 0.140, (0.12, -0.22, 1.05)),
            (-2.20, -1.10, 0.132, (0.22, 0.14, 0.35)))):
    ...
    # ④-d 한 상 (유호 08-21 「밑에 과일이나 케이크나 음료수같은 플레이팅을 제대로 해야할것같은데」)
    #     쿠키 접시 하나로는 «접시 사진»이지 «플레이팅»이 아니다. 잔과 케이크가 놓이는 순간
    #     화면은 «누군가의 자리»가 된다 — 그게 요리 사진이 파는 것이다.
    #     ⚠전부 상 위(z -0.35)에 직접 놓는다 — 냅킨·쿠키 접시는 가운데 것이고 이들은 그 밖이다.
    상면 = -0.35
    컵세트((-3.95, 1.75), 상면)
    # 케이크 받침 — 작은 접시(쿠키 접시와 같은 천이라 «한 벌»로 읽힌다)
    # 🔴 11판 실측: 접시 반경 1.42 를 (3.62,-2.32) 에 두니 쿠키 접시(r3.0)와 **0.12 겹쳤다** —
    #    거리 4.30 < 3.0+1.42. 두 접시가 겹치면 «잘린 원»으로 보여 그릇이 아니라 얼룩이 된다.
    bpy.ops.mesh.primitive_cylinder_add(radius=1.18, depth=0.09, vertices=128,
                                        location=(3.55, -2.55, 상면 + 0.045))
    작은접시 = bpy.context.object; 작은접시.name = "케이크접시"
    모따기(작은접시, 폭=0.035, 단=3)
    작은접시.data.materials.append(펠트재질("Stone", 스케일=2.4, 돌림=11.0, 염색보정=True,
                                       채도=1.0, 밝기=0.70, 결흩기=0.7))
    케이크조각((3.55, -2.55), 상면 + 0.09, 돌림=-118.0)
    # 🔴 12판: 왼쪽 한 알이 외따로 떨어져 «흘린 것»으로 읽혔다 — 곁들임은 무리다(4판 교훈 재확인).
    for i, (x, y, 크기, 돌 ) in enumerate(((2.60, -3.60, 0.30, 24.0),
                                         (3.22, -4.02, 0.26, -52.0),
                                         (2.12, -4.08, 0.235, 71.0))):
        딸기("딸기%d" % i, (x, y, 상면 + 크기 * 0.62), 크기=크기, 돌림=돌)
    ...
    for 판 in 알판들:
        for _ in range(3):                           # 🔴 알 «위»가 헷갈림의 진원지다 — 11 → 3
            a = 손.uniform(0, math.tau)
