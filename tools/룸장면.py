...
if 부품 == "합주":
    ...
    _눈 = _mu0.Vector((1.1, -15.9, 10.1)) * 당김
    cam.location = _눈
...
def 로고찡긋플레이팅세우기():
    ...
    냅킨.scale = (3.95, 2.98, 0.042)
    냅킨.rotation_euler = (0, 0, math.radians(-15.0))
    ...
    for 갈래이름, 위치, 롤, 기울 in (
        ...
        찡긋배지하나(쌍꺾쇠변주[갈래이름], 이름앞=갈래이름, 위치=위치, 롤=롤, 기울=기울)
    알자리 = [(-1.30, 0.75, 0.17), (-0.30, -0.72, 0.30), (1.35, 0.85, 0.165)]
    ...
    잎 = 구("곁잎", 0.34, (1.90, -1.10, -0.115), 세그=64)
    잎.scale = (1.0, 0.42, 0.085)
    잎.rotation_euler = (math.radians(9), math.radians(-16), math.radians(-28))
    잎.data.materials.append(펠트재질("MeadowDeep", 스케일=4.0, 돌림=5.0, 염색보정=True,
                                  채도=1.1, 밝기=0.86, 결흩기=0.5))
    ...
    열매천 = 펠트재질("PopDeep", 스케일=3.6, 돌림=11.0, 염색보정=True, 채도=1.15, 밝기=0.82,
                   결흩기=0.5, 거칠기=0.72)
    for i, (x, y, r) in enumerate(((1.76, -1.27, 0.180), (2.06, -1.03, 0.142))):
    ...
    for i in range(9):
        ...
        r = 손.uniform(2.30, 3.00)                   # 림 안쪽 — 그릇이 생겼으니 부스러기도 담긴다
    ...
    for cx, cy, 윗면 in 알자리:
        for _ in range(9):
            ...
            땀하나("가루%d" % 번, None, (cx + d * math.cos(a), cy + d * math.sin(a)),
                  (math.cos(a + 0.8), math.sin(a + 0.8)), 윗면 + 0.006,
                  길이=손.uniform(0.034, 0.058), 굵기=0.0125, 솟음=0.006, 잠김=0.0, 재질=가루천)
            번 += 1
    for _ in range(10):                              # 접시에도 조금 — 뿌리다 흘린 것
        a = 손.uniform(0, math.tau); r = 손.uniform(1.1, 2.9)
