...
def 로고배지세우기():
    ...
    실 = 실재질()
    # 🔴 10판 개편(유호 08-21 「닭같이 느껴지네 — 눈 배치를 바꾼다던지 많이 귀엽게」):
    #   옆모습 외눈(새 문법) + 왼쪽 향한 큰 꺾쇠(부리) + 정수리 뭉치(볏) 셋이 겹쳐 «닭»이 됐다.
    #   처방 = 정면 얼굴 — 큰 구슬눈 «둘» + 꺾쇠는 작게 중앙 아래(장난스러운 < 입 · 로고 방향 유지)
    #   + 뿔은 11시로 비켜 구름처럼 + 볼 홍조 «둘»(마스코트 문법: 구슬눈·입 아낌).
    입 = 꺾쇠몸("입", 크기=0.20, 두께=0.085, 모폭=0.030, 평면="XY")
    수제눌림(입, 세기=0.010, 크기=0.3)
    입.data.materials.append(펠트재질("Coral", 스케일=2.6, 돌림=9.0, 염색보정=True, 결흩기=0.45))
    붙이기(입, (0.0, -0.26, 0.20))     # 눈-입 간격이 좁을수록 아기 비율 — 귀여움의 자
    # 🔴 5판 실측: 입에 실땀을 두르니 과밀해 신호(코랄 1점)를 잡아먹었다 — 입은 민 코랄이 정답.

    for 이름, ex in (("눈왼", -0.34), ("눈오", 0.34)):
        눈 = 구(이름, 0.135, (0, 0, 0), 세그=96)
        눈.data.materials.append(구슬재질())
        붙이기(눈, (ex, 0.14, 0.20))

    for 이름, bx in (("홍조왼", -0.54), ("홍조오", 0.54)):
        bpy.ops.mesh.primitive_cylinder_add(radius=0.095, depth=0.024, vertices=96, location=(0, 0, 0))
        홍조 = bpy.context.object; 홍조.name = 이름
        홍조.scale = (1.0, 0.62, 1.0)
        홍조.data.materials.append(펠트재질("CoralSoft", 스케일=3.0, 염색보정=True, 결흩기=0.45))
        붙이기(홍조, (bx * 0.963, -0.08, 0.163))   # 눈 바로 아래 바깥 — 홍조의 정석 자리
    ...
    for i, (x, y, r) in enumerate([(-0.24, 0.96, 0.075), (-0.13, 1.0, 0.088), (-0.02, 0.972, 0.066)]):
        뿔 = 구("뿔%d" % i, r, (0, 0, 0), 세그=64)                     # 양모 뿔 — 원판 윗단에 걸친다
        뿔.scale = 뿔모양[i]
        뿔.rotation_euler = (0.3, 0.2 * i, 0.4 + 0.5 * i)
        수제눌림(뿔, 세기=0.02, 크기=0.3)
