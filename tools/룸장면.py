...
if 부품 == "합주":
    ...
    cam_data.dof.aperture_fstop = 3.2                 # 앞 알은 선명, 뒤 알은 살짝 풀린다
...
if 부품 == "로고찡긋플레이팅":
    ...
    area("창빛", 창위치, _dw.to_track_quat('-Z', 'Y').to_euler(), 5.0, 1500 * 램프배, shape='SQUARE')
    반사위치 = (4.0, -3.2, 1.8)
    _dr = _mu2.Vector((0, 0, 0)) - _mu2.Vector(반사위치)
    area("반사판", 반사위치, _dr.to_track_quat('-Z', 'Y').to_euler(), 6.0, 70 * 램프배)
...
def 찡긋배지하나(갈래, 이름앞="찡긋", 위치=(0, 0, 0), 롤=0.0):
...
def 로고찡긋플레이팅세우기():
    ...
    식탁보 → 접시 → 세 알의 놓임새 → 흩어진 실 부스러기 → 창빛 → 얕은 심도.
    ...
    # ② 접시 — 양모 회색. 음식 사진의 정석(어두운 상 · 밝은 접시 · 색 있는 디저트).
    ...
    모따기(접시, 폭=0.07, 단=4)
    접시.data.materials.append(펠트재질("Stone", 스케일=2.2, 돌림=7.0, 염색보정=True,
                                    채도=1.0, 밝기=0.70, 결흩기=0.7))

    # ③ 세 알 — 손으로 놓은 자리(정삼각 배치 금지: 계산한 티가 나면 «차림»이 아니라 «도표»다).
    #    접시 윗면 -0.15 = 배지 밑면(두께 0.30 의 절반) — 눈이 아니라 계산으로 앉힌다(4계 ④).
    for 갈래이름, 위치, 롤 in (("버터",  (-1.28, 0.62, 0.0),  -9.0),
                            ("라피스", ( 1.32, 0.78, 0.0),   7.0),
                            ("팝",    ( 0.06, -1.42, 0.0), -3.0)):
        찡긋배지하나(쌍꺾쇠변주[갈래이름], 이름앞=갈래이름, 위치=위치, 롤=롤)
    ...
    for i in range(9):
        ...
        r = 손.uniform(2.30, 3.45)                   # 접시 여백 중심 — 두어 알만 테두리를 넘는다
        z = -0.15 if r < 3.2 else -0.35              # 놓인 면의 높이가 다르다
        땀하나("부스러기%d" % i, None, (r * math.cos(a), r * math.sin(a)),
              (math.cos(a + 1.2), math.sin(a + 1.2)), z + 0.010,
