...
const 이름부름_표식 = /⚠?삭제됨|삭제됐다|지금 없다|⚰|걷혔다|철거됐다|은퇴|옛 회귀|없앴다/;
const 이름부름_과녁 =
...
function 훑기(뿌리, 모음 = []) {
  let 목록;
  try { 목록 = fs.readdirSync(뿌리, { withFileTypes: true }); } catch (_) { return 모음; }
  for (const e of 목록) {
  ...
  for (const f of 판파일들) {
    ...
    t.split('\n').forEach((줄, i) => {
      for (const m of 줄.matchAll(이름부름_과녁)) {
        ...
        if (이름부름_표식.test(줄)) continue;                  // 알고 남긴 기록 = 면제
 ...
 * ㉡ `docs/_ops/밤굽기도장.json` — `{ 시각, 무엇, 완주: true|false, 사유 }`.
...
function 밤굽기Section(뿌리 = ROOT) {
  ...
  return { 측정: true, 시각: o.시각 || null, 무엇: o.무엇 || null, 완주: o.완주, 사유: o.사유 || null, 나이시간 };
}
  ...
  if (밤굽기.ok) {
    ...
    if (!v.측정 && !v.아직) {
      ...
      notes.push({
        ...
        text: `${v.시각 || '(시각 미상)'}(${나이}) · ${v.무엇 || '(무엇 미상)'} — 사유: ${v.사유 || '(안 적혔다)'}` +
