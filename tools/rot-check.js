...
function 이름부름Section(뿌리 = ROOT) {
  ...
  for (const f of 판파일들) {
    ...
    /* 면제 ㉰ (2026-09-01 · 위임 3단계 ⓒ) — **감사 산출물은 «부름»이 아니라 «제안»이다.**
     *   `docs/_ops/감사조스펙/` 의 JSON 은 감사가 그날 낸 결과 기록이고, 그 안의
     *   `"tests": "…신규 …"` 는 **「이런 검사를 지어라」는 미래형 일감**이다. 지금 그 파일이
     *   없는 것이 정상이고, 그걸 「없는 파일을 부른다」로 세면 그 스펙은 **원리상 영영 적색**이다
     *   (고치려면 과거 기록을 조작해야 한다). 실측 09-01: 없는것 20종 중 4자리가 여기였다.
     * 🔴 2026-09-03 넓힘 — **같은 모양이 «인계» 폴더에도 있었다.** 트랙을 넘기며 남기는
     *   `docs/_ops/…인계/재료/` 의 JSON 도 `"measure"`·`"fix"`·`"what"` 칸에 「이 자로 재라 ·
     *   이 도구를 지어라」를 적는다 — 부름이 아니라 **미래형 일감**이고, 그 폴더는 스스로
     *   「작업 중 폴더 · 산출물이 서면 그 커밋에서 통째로 지운다」고 선언한다.
     *   실측 09-03: 없는것 38종 중 **35종**이 `엔진7종_상향_인계/재료/` 한 폴더였다.
     *   🔑 이게 왜 «수리»인가: 그 35종이 **진짜 결함 1건**을 소음으로 덮고 있었다 — 사진 편집
     *   굽기 도구가 «지어진 적 없는» 검증 도구를 화면에 「이걸 돌려라」로 띄우던 자리다(경로는
     *   위 ⚠ 대로 여기 안 적는다 — 적으면 이 주석이 제 꼬리를 문다). 안 걸러지는 경고는 안 읽힌다.
     *   ⚠ 폴더 이름을 박지 않고 «인계/재료» 무늬로 잡는다 — 인계 폴더는 트랙마다 새로 나고
     *     사라진다. 박으면 다음 인계에서 같은 병이 그대로 되돌아온다. */
    if (rel.startsWith('docs/_ops/감사조스펙/')) continue;
    if (/^docs\/_ops\/[^/]*인계\/재료\//.test(rel)) continue;
    ...
    줄들.forEach((줄, i) => {
      for (const m of 줄.matchAll(이름부름_과녁)) {
        const 이름 = m[0];
        const 앞 = 줄[m.index - 1];
        const 뒤 = 줄[m.index + 이름.length];
        // 백틱은 «산문 강조»라 부름이다 — 홑·겹따옴표만 픽스처 «값»으로 본다(위 머리말 08-30 좁힘).
        const 인용 = /['"]/.test(앞 || '') && /['"]/.test(뒤 || '');
        if (시험파일 && 인용) continue;                       // 픽스처 값이지 부름이 아니다
