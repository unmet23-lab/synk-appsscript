...
def 눈들(im: Image.Image, 결='검정'):
    ...
    r, g, b = a[:, :, 0], a[:, :, 1], a[:, :, 2]
    if 결 == '버터':
        골들 = [(r > 190) & (g > 150) & (b < 150) & (r - b > 70)]
    elif 결 == '초록':
        # 🔴 까몽은 몸이 짙은 숯빛이라 «어두운 곳»으로 눈을 찾으면 몸 전체가 걸린다(09-06 실측:
        #   모자를 씌운 컷에서 눈을 한 번도 못 찾았다). 까몽의 표식은 «사과빛 초록 홍채»이고
        #   그 색은 몸에도 옷에도 없다 — 그것으로 찾는다.
        # 🔴 홍채가 «옅게» 그려지는 장이 있다(09-06 저녁 · 까몽 안경 · 사과빛이 아니라 연한 쑥빛).
        #   그렇다고 문턱을 그냥 넓히면 «다른 장»이 대신 깨진다(SYNK 후드가 그랬다) — 넓은 자는
        #   초록기 도는 크림까지 물어 눈이 옷에 붙어 버린다.
        #   ⇒ **좁은 자를 먼저 대고, 짝을 못 찾을 때만 넓힌다.** 넓히는 쪽은 되돌릴 수 없는 손해가
        #     없다(못 찾으면 그 장은 어차피 멎는다). 자홍 몸 위에서는 초록이 몸에도 바탕에도 없다.
        골들 = [(g > 90) & (g - r > 15) & (g - b > 20),
                (g > 70) & (g - r > 6) & (g - b > 12)]
    else:
        골들 = [(밝기 < 70) & (채도 < 60)]
...
def _짝찾기(골, 알파, h, w):
    """마스크 하나에서 «눈 한 쌍»을 고른다. 못 고르면 None."""
    골 = 골.copy()
    if 알파 is not None:
        골 &= 알파 > 128
