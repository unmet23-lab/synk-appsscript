 ...
 *
 * ■ 이 도구가 «안» 하는 것
 *   채택 판정을 안 한다. 산출은 시안이다.
 *   🔴 「몸이 안 변했나」를 기계로 재는 `tools/편집검증.js` 는 **지어진 적이 없다**(09-03 실측 —
 *      이 저장소에도 형제 SYNK-talk 에도, 두 저장소의 git 이력에도 0건. 트랙.md·결정.md 에도
 *      일감으로 안 적혀 있다). 앞판은 이 줄을 「…가 잰다」로 적어 **없는 검사를 있다고** 말했다.
 *      지금 그 판정은 **사람 눈**이다 — 원본과 시안을 나란히 놓고 본다.
 *      ⚠ 그 도구를 실제로 짓기 «전»에는 이 줄을 「있다」로 되돌리지 않는다.
 *
 * 쓰기:
...
(async () => {
  ...
  console.log('  ⚠ 시안이다 — 「몸이 안 변했나」를 재는 기계는 아직 없다(원본과 나란히 놓고 눈으로 본다).');
