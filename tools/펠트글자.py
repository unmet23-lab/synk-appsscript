...
def 펠트글자(글, 천경로, 크기=200, 여백=44, 천배율=0.12, 땀=False, 폰트=폰트기본,
    ...
    # 🔑 자간은 «오려 붙이기» 때문에 필요하다(09-07 유호 지적으로 드러났다).
    #    보풀이 획 밖으로 번지므로, 폰트가 알맞다고 보는 간격이 펠트에서는 붙어 보인다.
    #    「안녕하세요」의 「녕」 받침이 「하」에 닿아 한 덩어리로 읽혔다.
    #    자간 0 이면 옛 동작 그대로다 — 낱자로 그리되 폰트 진행폭을 그대로 쓴다.
    벌림 = 크기 * 자간
    bb = ImageDraw.Draw(Image.new('L', (8, 8))).textbbox((0, 0), 글, font=f)
    W = bb[2] - bb[0] + 여백 * 2 + int(round(벌림 * max(len(글) - 1, 0)))
    H = bb[3] - bb[1] + 여백 * 2
    깔끔 = Image.new('L', (W, H), 0)
    d0 = ImageDraw.Draw(깔끔)
    if 벌림 <= 0:
        d0.text((여백 - bb[0], 여백 - bb[1]), 글, font=f, fill=255)
    else:
        x = float(여백 - bb[0])
        for 자 in 글:
            d0.text((x, 여백 - bb[1]), 자, font=f, fill=255)
            x += f.getlength(자) + (0 if 자 == ' ' else 벌림)
...
def main():
    ...
    ap.add_argument('--자간', type=float, default=0.0,
                    help='낱자 사이를 글자 크기의 몇 배만큼 벌릴까(0.04 쯤이면 보풀이 안 닿는다)')
    ...
    im = 펠트글자(a.글, 천경로, 크기=a.크기, 천배율=a.천배율, 땀=a.땀, 폰트=a.폰트, 자간=a.자간,
                늘임=a.늘임)
