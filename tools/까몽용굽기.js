...
async function main() {
  ...
  /* 🔴 09-06 검수가 짚은 자리 — 그전에는 여덟 판이 «전부» 실패해도 종료 코드가 0(성공)이었다.
     돈 벽에서 돌아설 때도, 세 판이 잇달아 막혀 멈출 때도 `return 마무리()` 하나로 끝나서
     부르는 쪽(배치·셸)이 「됐다」로 읽었다. 기억 zero-is-a-success-face-taxonomy 의 무늬다.
     ⇒ 「과녁만큼 났나」를 세어 못 난 것이 있으면 실패로 끝낸다. 갈래마다 세지 않고 «결과»로 센다 —
       돈 벽이든 몫 벽이든 파일이 안 났으면 실패다. */
  function 마무리() {
    const 못함 = 대상.length - 구움 - 건너뜀;
    console.log(`\n[까몽용굽기] 구움 ${구움} · 건너뜀 ${건너뜀} · 못함 ${못함} / 과녁 ${대상.length}`
      + ` · 든 돈 ≈ ${구움 * 336}원 · ${낼곳}`);
    if (못함 > 0) {
      console.error(`🔴 ${못함}판이 안 났다 — 종료 상태를 실패로 낸다(부르는 쪽이 「됐다」로 읽지 않게).`);
      process.exitCode = 1;
    }
  }
  마무리();
