...
function 점유(항목, 행들) {
  ...
  if (Number.isInteger(항목.id)) {
    const 키 = `#Q${항목.id}`;
    for (const 행 of 행들) {
      if (행.큐 && 행.큐.has(키)) return { 근거: 키, 트랙: 행.트랙 };
    }
  }
  ...
  const 후보 = [];
  for (const 행 of 행들) {
    if (씨 && 행.선언.includes(씨)) { 후보.push({ 순위: 1, 근거: '씨앗', 트랙: 행.트랙 }); continue; }
    줄참조RE.lastIndex = 0;
    let 걸림 = null;
