 ...
 * ⚠ 판정을 `require.main` 블록 **안**에 둔다 — 이 파일은 몸통이 최상위에서 도는 꼴이라, 게이트만
 *   밖에 두면 `require` 하는 쪽의 argv 를 읽고 남의 실행을 남의 이름으로 끊는다(회귀 ③-3).
 * ⚠ 검색 문구(옮길 줄)는 위치 인자라 `--` 로 안 시작한다 — 애초에 안 걸린다. */
const 아는플래그 = ['--dry', '--미완확인'];
if (require.main === module) {
  const 플래그오류 = 인자게이트('board-move', args, 아는플래그);
