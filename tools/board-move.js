...
/* `--미완확인` = 원칙 ⑦(아래)의 유일한 탈출구. 사유를 **말로** 받는 이유는 F103 이다 —
 * 따를 수 없는 처방은 우회를 정상 통로로 만드니, 거절이 내미는 처방은 실행 가능해야 한다. */
const 미완확인 = args.includes('--미완확인');
const needle = args.filter((a) => a !== '--dry' && a !== '--미완확인').join(' ').trim();
...
if (!미완확인) {
  ...
  if (남의줄 && 보드lib.활성행(row)) {
    ...
    die('이 줄은 **남의 죽은 세션 줄인데 상태 칸이 「끝났다」고 말하지 않는다**(원칙 ⑦ · F397).\n' +
      ...
      `       node tools/board-move.js "${needle}" --미완확인\n` +
