...
/* 줄을 뺀 뒤 **남는 게 상투구뿐인가** — 그러면 파일째 지운다 (2026-08-12).
 *
 * 왜: 이관은 지금까지 줄만 빼고 **파일을 남겼다.** 실측 08-12 — 보드 폴더 262개 중 데이터행이
 * 있는 건 18개고, 0바이트 189 · 표 머리글만 56 이 죽은 껍데기다(93%). 그 껍데기는 조용하지 않다:
 * `board-guard` 는 PreToolUse 라 **파일을 쓸 때마다** 폴더를 통째로 훑고, 그 훑기가 244개의 빈
 * 파일을 매번 연다. 세션이 늘수록 곱해지는 값이라 「그냥 남은 파일」이 아니라 조율값이다.
 *
 * ⚠ 새는 방향을 「지운다」로 두지 않는다 — 판정은 **좁게**다:
 *   · `표줄` 이 하나라도 남으면 안 지운다. `표줄` 은 넓은 쪽이라 **유령까지 센다**(F322·F330·F331) —
 *     규격 밖 줄을 「데이터행이 아니니 빈 파일」로 읽어 조용히 지우면 그게 유령보다 나쁘다.
 *   · 상투구(제목·표 머리글·구분선·빈 줄) 말고 **사람이 쓴 글이 한 줄이라도 있으면 안 지운다.**
 *     보드 파일은 규격이 좁지만 메모를 적어 둔 세션이 있을 수 있고, 그건 내 것이 아니다.
 *   판정을 여기 한 곳에만 두는 이유는 `표줄` 과 같다 — 두 곳에 적으면 갈라지고, 갈라진 쪽의
 *   증상은 「지워도 되는 줄 알았다」다. */
function 껍데기인가(text) {
  for (const raw of String(text).split(/\r?\n/)) {
    const l = raw.trim();
    if (!l) continue;                          // 빈 줄
    if (isRow(l)) return false;                // 데이터행이든 유령이든 **줄이 남아 있다**
...
const c = commit(보드추적중 ? [ARCHIVE, BOARD] : [ARCHIVE],
  ...
  ['board-move 자동 커밋(F102) — 아카이브 삽입과 보드 제거를 **한 커밋**에 담는다.',
    '원자라 「보드 삭제만 착지」가 원리상 없다. 실패하면 보드를 원문으로 되돌린다.'].join('\n'));
