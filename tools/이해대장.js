 ...
 *      node tools/이해대장.js --바로가기  → 생성 + 바탕화면 「SYNK 방향」에 .lnk
 */
'use strict';
...
const 정본경로 = process.env.SYNK_대장_정본 || path.join(ROOT, 'docs', '철학_실물현황.md');
 ...
 * 왜 여기냐: 바탕화면 「SYNK 방향」에 화면마다 바로가기를 늘리면 그때마다 유호님 손이 든다
...
const 함께볼것 = [
  { 파일: 'SYNK_철학_요약.html', 제목: '철학 정본의 핵심', 한줄: '모든 작업의 판단 기준' },
  { 파일: '엔진/SYNK_엔진_지도.html', 제목: '엔진 7종', 한줄: '일곱 엔진의 역할과 현재 설계' },
  { 파일: '브랜드킷.html', 제목: '브랜드 킷', 한줄: '확정 색·서체·시각 요소' },
  { 파일: '홍보물/브랜드소개_20260909/synk.html', 제목: '현재 회사 소개서', 한줄: 'SYNK와 세 사업의 역할' },
];
...
const 킷 = {
  ...
  coral: '#F96859', coral3: '#AE322A', coralWash: '#FEF0E9',
...
function 표뽑기(md, 제목정규식) {
  ...
  for (let j = i + 1; j < lines.length; j++) {
     ...
     * 밀린 뒤에도 오류는 안 나 「비었나()」가 엉뚱한 칸을 읽는다(tools/lib/표.js 머리 · 회귀 tests/표칸.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)). */
...
function 비었나(칸) {
  ...
  /* 「없다 — 설명」(A-2 수업·현장 v1.4 의 의도된 없음)은 빈 칸이 아니다 — 설명이 본문으로
   * 그려져야 한다. 빈 칸은 «없다» 뿐일 때만(08-22 — 옛 `/^없다/` 는 의도 값까지 「비었다」로
   * 뭉개 원문 설명이 화면에서 사라졌다). */
...
/** 층 이름을 깊이 0 의 첫 「—」에서 가른다 — 「㉠ 실력 — 무엇을 틀리는가」 → 짧은 「㉠ 실력」·설명 「무엇을…」.
 *  🔴 괄호 «안»의 「—」에서 자르지 않는다(08-22 실측 — 「수업·현장 (v1.4 신설 — …)」이
 *  닫는 괄호 없이 잘렸다). 짧은이름과 설명이 «같은 경계 하나»를 쓰게 분해를 한 함수로 둔다. */
function 이름분해(이름) {
  const s = String(이름);
  const 여는 = { '(': ')', '「': '」', '«': '»', '『': '』', '[': ']', '⟨': '⟩' };
  const 닫는 = new Set(Object.values(여는));
  let 깊이 = 0;
  for (let i = 0; i < s.length; i++) {
    const ch = s[i];
    if (여는[ch]) 깊이 += 1;
    else if (닫는.has(ch)) 깊이 = Math.max(0, 깊이 - 1);
    else if (ch === '—' && 깊이 === 0) {
      return { 짧은: s.slice(0, i).trim(), 설명: s.slice(i + 1).trim() };
    }
  }
  return { 짧은: s.trim(), 설명: '' };
}
...
function 엔진점수(root) {
  ...
  /* 🔴 «답이 안 온 회차»는 점수가 아니다(08-22 신설). `무응답`(벤더가 거절해 교정행이 안 선 건수)이
   *   0 이 아니면 그 회차의 전체 점수는 **품질이 아니라 도달률**을 잰다 — v7 r2 가 벤더 400 61건으로
   *   36/102 를 냈는데 화면은 그것을 머리 숫자로 세우고 «▼52 폭락»까지 그릴 참이었다. 그래서
   *   ①머리는 마지막 «답이 온» 회차가 들고 ②그 뒤에 답이 안 온 회차가 있으면 그 «수»를 경고로 낸다
   *   (숨기지 않는다 — 미실행을 통과로도, 실패로도 안 읽게). 답이 온 회차가 하나도 없으면 마지막
   *   회차를 그대로 세우되 미측정 표식을 단다(F207 — 「점수 0」과 「안 재봤다」는 다른 상태다). */
  const 답온것 = 회차.filter((r) => !Number(r.무응답));
  const 현재 = 답온것.length ? 답온것[답온것.length - 1] : 회차[회차.length - 1];
  const 미측정 = !Number(현재.무응답) ? 0 : 1;
  const 뒤에안온것 = 답온것.length ? 회차.length - 회차.lastIndexOf(현재) - 1 : 회차.length;
  const 앞 = 답온것.length ? 답온것.slice(0, -1) : 회차.slice(0, -1);
  const 직전 = [...앞].reverse().find((r) => r.판 === 현재.판) || null;
  return {
    현재, 직전, 미측정, 뒤에안온것,
    변화: 직전 ? 현재.전체 - 직전.전체 : null,
    회차수: 회차.length, 어디: p,
  };
}
 ...
 *  🔴 괄호·인용부 «안»의 「·」는 자르지 않는다(08-22 실측 — A-2 「오늘의 도전 · 오늘의 성장」이
 *  괄호째 세 조각으로 깨지고, 조각 머리가 닫는 괄호 없이 잘렸다). 정규식 split 은 깊이를
 *  못 세므로 문자 순회로 깊이 0 에서만 자른다. 붙은 「·」(수업·현장)는 낱말의 일부라 안 자른다. */
function 조각들(내용) {
  const 여는 = { '(': ')', '「': '」', '«': '»', '『': '』', '[': ']', '⟨': '⟩' };
  const 닫는 = new Set(Object.values(여는));
  const s = String(내용);
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (여는[ch]) 깊이 += 1;
      ...
      조각.push(s.slice(시작, i));
      시작 = i + 1;
    }
  }
  조각.push(s.slice(시작));
  return 조각.map((x) => x.trim()).filter(Boolean);
}
...
function 카드들(행들) {
  ...
  return 몸.map((행) => {
    const 층 = 짧은이름(행[0]);
    const 기호 = 층기호(행[0]) || '?';
    const 설명 = 이름분해(행[0]).설명;   // 짧은이름과 «같은 경계 하나»(괄호 안 — 에서 안 가른다)
...
function main() {
  ...
  const ver = (md.match(/<!--\s*현황:\s*(.*?)\s*-->/) || md.match(/<!--\s*정본:\s*(v[\d.]+)\s*-->/) || [, '(판 미상)'])[1];
  /* [발전안 20260911 갈래 ⑤] 시점 둘을 가른다 — 이 화면에는 «손 판정»(A-1·A-2 칸 · 「엔진에 닿는가」 결론 = 실물현황의 관찰 기록)과
   *   «기계 값»(아래 실측 블록 = 생성 시점의 코드에서 잰 래칫·연결표·점수)이 함께 산다. 09-09 이관 뒤 손 판정은 갱신 통로가 없고
   *   코드는 계속 움직여, 둘의 시점이 다르다는 것을 화면이 말하지 않으면 옛 관찰이 오늘의 사실처럼 읽힌다(GPT 검토 09-11 이 짚은 약점).
   *   관찰 시점은 실물현황 머리의 「YYYY-MM-DD 분리」에서, 기계 값의 기준은 엔진 판(Code.js SYNK_VERSION)에서 읽는다 — 날짜를 생성 시각으로
   *   찍으면 `--검사` 가 매일 «안 따라왔다»를 내므로 판 번호로 못박는다(코드가 바뀌면 판도 바뀐다). 못 읽으면 «미상»이지 빈칸이 아니다(F207). */
  const 관찰시점 = (md.match(/(\d{4}-\d{2}-\d{2})\s*분리/) || [, '(시점 미상)'])[1];
  const 엔진판 = (() => {
    try { return (fs.readFileSync(path.join(ROOT, 'Code.js'), 'utf8').match(/const SYNK_VERSION = '([^']+)'/) || [, '(판 미상)'])[1]; }
    catch { return '(판 미상)'; }
  })();
  ...
  if (!이해 || !실물) {
    console.error('[이해대장] 부록 A 의 표를 못 읽었다 — 절 구조가 바뀌었다.'
      ...
      + '\n  → 빈 화면을 내지 않는다(「비었다」와 「못 읽었다」가 같은 모양이면 안 된다). 정본을 열어라: docs/철학_실물현황.md');
    process.exit(1);
  }
   ...
   *   `tests/이해대장.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)` 의 「실측 블록이 1개가 아니다」가 이 자리를 지킨다.
    ...
    return `수집 <b>${s.전체}종</b> = 제품층 ${s.제품} + 자산층 ${s.자산} + <b>도달 0 이 ${s.도달0}종</b>`
      ...
      + (s.연결표 && s.연결표.length
        ? `<details class="연결표"><summary>연결표 ${s.연결표.length}줄 — 원천 → 읽는 부품 → 언제 → 대장 칸`
          + ` <span class="잔글">(대장 ${['㉠', '㉡', '㉢'].map((c) => `${c} ${s.대장칸별[c] || 0}`).join(' · ')}`
          /* [발전안 20260911 갈래 ④] 누구에게 — 철학 §2 네 단계 ④. 값 정본 = 수집도달_() 의 수신자 칸 · 자 = tools/lib/시트도달.js 수신자들.
           *   이 수는 «선언»의 분포다 — 실제 전달 증거는 관통왕복 시험이 낸다(시험 통과 ≠ 전달 증명). */
          + ` · 누구에게 ${['학생', '강사', '학부모', '운영자', '내부'].map((c) => `${c} ${(s.수신자별 || {})[c] || 0}`).join(' · ')})</span></summary>`
          + '<table class="연결표"><thead><tr><th>원천(탭)</th><th>읽는 부품</th><th>층</th><th>언제</th><th>대장 칸</th><th>누구에게</th></tr></thead><tbody>'
          + s.연결표.map((e) => `<tr><td><code>${esc(e.탭)}</code></td><td><code>${esc(e.소비자)}</code>${e.손 ? ' <span class="잔글">(사람이 눌러야 돈다)</span>' : ''}</td>`
            + `<td>${esc(e.층)}</td><td>${esc(e.회수)}</td><td>${esc(e.대장칸)}</td><td>${esc(e.수신자 || '')}</td></tr>`).join('')
          + '</tbody></table></details>'
        : '')
...
<div class="판 룸">
<h1>이해 대장</h1>
<p class="부제">관찰 기록 ${ver} · 관찰 시점 <b>${esc(관찰시점)}</b> · 철학 실물현황에서 자동으로 그린다. A-1·A-2 의 칸과 「엔진에 닿는가」 결론은 그 시점의 <b>손 판정</b>이고, 아래 실측 블록은 생성 시점의 코드(엔진 ${esc(엔진판)})가 잰 <b>기계 값</b>이다 — 둘의 시점이 다르면 기계 값을 먼저 보고 손 판정이 낡았는지 코드로 가른다. <b>이 화면은 손으로 고치지 않는다.</b> 고칠 것이 있으면 아래 코드를 눌러 복사해 말하면 된다.</p>
...
<div class="요약">
  ...
  ${점수
    ? `<div class="수치${점수.미측정 || 점수.뒤에안온것 ? ' 경고' : ''}"><b>${점수.현재.전체} / ${점수.현재.분모}</b>엔진 점수 · ${esc(점수.현재.판)} (${esc(점수.현재.날짜)})${점수.변화 === null ? ' · 같은 판 첫 회차' : ` · 같은 판 대비 ${점수.변화 > 0 ? '▲' : 점수.변화 < 0 ? '▼' : '='}${Math.abs(점수.변화)}`}`
      + `${점수.미측정 ? `<span class="잔글"> — ⚠ <b>답이 온 회차가 없다</b>(무응답 ${esc(점수.현재.무응답)}건): 이 숫자는 품질이 아니라 도달률이다</span>` : ''}`
      + `${점수.뒤에안온것 ? `<span class="잔글"> — ⚠ 이 뒤 <b>${점수.뒤에안온것}회차</b>는 벤더가 답을 안 줘 점수가 없다(품질 하락 아님 · 다시 돌려야 한다)</span>` : ''}</div>`
...
<h2 class="절">A-1. 이해 대장 — 그 학생을 얼마나 알게 됐나 <span style="font-weight:400;color:${킷.slate2};font-size:14px">(손 판정 · 관찰 ${esc(관찰시점)})</span></h2>
...
<div class="실측">📏 엔진 도달 실측 <span class="잔글">(앱 사건 층 · 기계 값 · 엔진 ${esc(엔진판)} 시점의 코드)</span> — ${도달줄}
<p>🔴 ${빨강줄}</p>
${빨강사유}</div>
<div class="${시트층클래스}">🗂 시트층 도달 실측 <span class="잔글">(오늘 실제로 쌓이는 곳 — 위 래칫이 원리상 못 보는 층 · 기계 값 · 엔진 ${esc(엔진판)})</span> — ${시트줄}</div>
...
function 폴백(말, 끝) {
   ...
   *   회귀 `tests/이해대장.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)` 가 이미 같은 축을 못박고 있다(열화판을 커밋에 싣지 않는다). */
  if (process.argv.includes('--검사')) {
     ...
     *   요구하는 소비자가 없다. 내용 차이는 그대로 잡힌다(회귀 `tests/대장동봉.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)` 가 양쪽을 못박는다). */
    ...
    const 방아쇠 = (process.env.SYNK_대장_방아쇠 || 'docs/철학_실물현황.md').split('\n').filter(Boolean);
    console.error(`[이해대장] 차단 — 이 커밋이 화면의 부모(${방아쇠.join(' · ')})를 고치는데 화면(\`docs/이해대장.html\`)이 ${지금 === null ? '**없다**' : '**안 따라온다**'}.`);
    console.error('');
    console.error('  이대로 커밋하면 동봉이 HEAD 에서 갈린다(옛 회귀 `tests/이해대장.test.js` 는 08-19 대청소 e75fc7fc 로 걷혔다 — 지금 그 자리는 `tools/대장동봉검사.js` 다). 그 적색은 고친 사람이 아니라');
  ...
  console.log(`[이해대장] ${path.relative(ROOT, 산출경로)} — 정본 ${ver} · 이해 ${칸전체.length}칸 중 ${빈칸}칸이 비었다`
    ...
    + (점수
      ? ` · 엔진 점수 ${점수.현재.전체}/${점수.현재.분모}(${점수.현재.판} · 회차 ${점수.회차수}`
        + `${점수.미측정 ? ` · ⚠답이 온 회차 0 — 무응답 ${점수.현재.무응답}` : ''}`
        + `${점수.뒤에안온것 ? ` · ⚠뒤 ${점수.뒤에안온것}회차 무응답(점수 없음)` : ''})`
      : ' · 엔진 점수 아직 안 쟀다'));

  if (바로가기) {
    const { execFileSync } = require('node:child_process');
    /* 운영자료 폴더 통로는 하나뿐이다(유호 상시 08-09) — 손 경로 금지.
     * 갈래 = **방향**(08-20 재편 — 이해 대장은 판단 층이다. 유호 지시 08-17 재편에서 이 화면을 예시로 직접 들었다).
