...
const MANIFEST = [
  ...
  /* 강사가 교실에서 «펴 놓고 보는» 종이다 — 읽는 문서가 아니라 그 자리에서 낱말을 눈으로 집는 용도라
   * 축마다 표 하나로 끊어 뒀다(넘겨 가며 찾지 않게). */
  { src: 'docs/발음_수업활동.md',         title: '발음 1분 활동 — 강사용',      sub: '축 여섯 · 듣기부터 · 한 차시에 하나', group: '전달용' },
  { src: 'docs/발음_미션_작성지.md',       title: '발음 낱말 — 이렇게 정했습니다',           sub: 'AI 가 정한 낱말 열한 개 — 보시고 걸리는 것만 말씀하시면 됩니다', group: '전달용' },
 ...
 *      걸린다(회귀 `tests/문서문자.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)`). 필요하면 코드포인트로 쓴다.
...
const 인쇄_치환 = [
  ...
  [/ⓐ/g, '(a)'], [/ⓑ/g, '(b)'], [/ⓒ/g, '(c)'], [/ⓓ/g, '(d)'],
  /* 원 문자 한글 — 철학 정본이 층 이름으로 쓴다(이해 세 층 ㉠㉡㉢ · 갈래 ㉮㉯㉰㉱).
   ...
   * 🔑 낱글자로 적지 않고 **계열을 통째로** 받는다 — 이 두 계열은 갈래가 «늘어나는» 자리다.
   *    ㉰ 다음 ㉱(유호 확정 08-24 「라로 진행해」)이 원고에 들어온 날 강사 매뉴얼 1벌이 깨졌다.
   *    실측은 그대로다: 계열 끝(㉭㉻)까지 낱자 28종 + 괄호 2종이 브랜드 3종 cmap 에 전부 있다
   *    (2026-08-24 · 결손 0/30). 그러니 다음 갈래가 와도 종이에서 뜻을 잃지 않는다. */
  [/[㉠-㉭]/g, (c) => '(' + 'ㄱㄴㄷㄹㅁㅂㅅㅇㅈㅊㅋㅌㅍㅎ'[c.codePointAt(0) - 0x3260] + ')'],
  [/[㉮-㉻]/g, (c) => '(' + '가나다라마바사아자차카타파하'[c.codePointAt(0) - 0x326E] + ')'],
