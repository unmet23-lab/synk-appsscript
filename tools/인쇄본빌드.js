 ...
 * ⛔ **한자는 치환하지 않는다 — 원문에서 없앤다.** 2026-08-07 유호님 확정: 문서에 쓰는 문자는
 *    **한글·몽골어·영어 셋뿐**이다. 종이에서만 바꾸면 화면(md)엔 남아 정책이 반쪽이 되므로,
 *    한자는 아래 게이트가 잡아 **빌드를 깬다**(예외 등재 없음). 어원 묶음 카드의 生活社會部 도
 *    「생 = 살다 → 생활·학생」 식 뿌리 음절 표기로 원문을 고쳤다(커리큘럼 정본 §6-2).
