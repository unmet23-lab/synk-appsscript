...
${우주복들.length ? `<h2 id="s0d" class="듦"><span class="번호">04</span><span>우주복판 — 갑옷을 전부 벗다</span></h2>
...
베개몸 우주복 · 실땀 솔기 · 가슴 생명유지 패널(단추 셋) · 목 링 · 등의 학생 책가방 · 장갑이 쥔 거대 연필.
...
<div class="짝">${우주복들.map((r) => 칸(r, r.이름.includes('몽글') ? '우주복 몽글 · 오트 슈트 + 버터 속빛' : r.이름.includes('까몽') ? '우주복 까몽 · 먹 헬멧(민얼굴)' : '우주복 뒷태 · 책가방 셋')).join('')}</div>` : ''}
