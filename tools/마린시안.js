...
<p class="듦"><b>까몽 명품화판</b>(유호 08-28 「흰 배경 하나만 · 감각적이고 트렌디하게」) — 디자이너 토이
문법 셋. ①<b>단호함 = 눈빛</b>: 바이저 «속» 가로 발광 막대의 빛이 브라우 틈 둘로만 새 나와 «치켜뜬
단호한 눈매»가 된다(칠이 아니라 번짐이 원리상 없다 · STL 엔 안 나간다) ②<b>귀여움 = 볼터치</b>: 검은
마스크 뺨에 코랄 매트 두 점 ③<b>절제 = 모노 팔레트</b>: 검은 연필·검은 방패·먹 결 260 — 색은 코랄
점 몇 개만 튄다. 단호한 눈매 «위»에 볼터치가 앉는 그 대비가 이 판의 얼굴이다.</p>
<div class="짝">${우주복들.map((r) => 칸(r, r.이름.includes('몽글') ? '우주복 몽글 · 스파르타 선생님(방패·연필 창)' : '우주복 까몽 · 명품화판(눈빛·볼터치·모노)')).join('')}</div>` : ''}
