...
function main() {
  ...
  const 해석들 = 집기((f) => /^primey_해석_(몽글|까몽|뒷태)\.png$/.test(f));
  const 우주복들 = 집기((f) => /^primey_해석_우주복(몽글|까몽|뒷태)\.png$/.test(f));
...
<div class="짝">${해석들.map((r) => 칸(r, r.이름.includes('몽글') ? '몽글 해석 · 코랄 펠트 + 버터 눈' : r.이름.includes('까몽') ? '까몽 해석 · 먹 펠트(민얼굴)' : '뒷태 · 학생 책가방')).join('')}</div>` : ''}
...
<p class="듦"><b>스파르타 선생님</b>(유호 08-28) — 얼굴은 <b>정면</b>(z32 실측각), 투구 위 <b>가로 볏</b>
(켄투리온·스파르타 «지휘관»의 표식 — 마디 아홉을 투구 실표면에 스냅), 왼팔에 <b>방패</b>(라피스 면 +
코랄 보스 + 테두리 실땀 — 오브 배지의 문법), 오른손의 거대 연필이 창을 진다. 어깨엔 서전트식
<b>어깨 방어구</b>(장갑색 돔 + 버터 트림 · 머리보다 넓게). 버터 목 링은 유호 반려로 뺐다(「입쪽 노란 줄
부자연」) — 슈트색 깃이 조용히 목을 잇는다.</p>
<p class="듦"><b>얼굴</b> — 서전트의 헬멧이 그대로 위엄을 진다(바이저 격자 · 볼 통풍구 — 이마 «해골»은
귀여운 오브 코인이 덮고, 입 밑 «목 칼라 띠»는 머리를 낮춰 몸속에 묻었다 · 유호 08-28 반려 둘).
눈은 칠하지 않았다(돌아간 격자 위에서 번진다 — 실측) — <b>헬멧 «속»에 발광 구</b>를 넣어 바이저 틈으로만
빛이 새게 했다. 몽글판은 버터 속빛, 까몽판은 민얼굴(빈 헬멧의 어둠)이다. 속빛 구는 프린팅 STL 엔 안 나간다.</p>
<div class="짝">${우주복들.map((r) => 칸(r, r.이름.includes('몽글') ? '우주복 몽글 · 스파르타 선생님(볏·방패)' : r.이름.includes('까몽') ? '우주복 까몽 · 먹 헬멧 + 코랄 볏' : '우주복 뒷태 · 책가방 셋')).join('')}</div>` : ''}
