...
def _치마지연(몸, 램프, sdx, sdy):
    """치마 팔로우스루 — 자락을 «조금 전의 몸» 쪽으로 **연속 워프**한다(행마다 시프트가
    램프로 매끄럽게 자라는 후방 바이리니어 샘플링). v1 은 몸을 두 층으로 잘라 아래층만
    시프트하는 파티션이었는데, 시프트가 서면 비워진 행의 알파가 반쯤 꺼져 무대색이
    «몸통 분리» 띠로 비쳤다(유호 신고 08-15 · 6.05s 정지컷 실측 — 웃음 들썩임 구간이
    시프트 최대라 거기서 보였다). 연속 변위장은 층이 하나라 구멍이 원리적으로 없다.
    램프 기울기·시프트 상한(0.035H)에서 1-w'·sdy > 0.5 라 접힘도 없다. 시프트 0 = 항등."""
    if abs(sdx) < 0.5 and abs(sdy) < 0.5:
        return 몸
    A = np.asarray(몸, dtype=np.float32)
    H, W = A.shape[:2]
    a = A[:, :, 3:4] / 255.0
    pm = np.concatenate([A[:, :, :3] * a, a * 255.0], axis=2)
    y = np.arange(H, dtype=np.float32)
    ys = y - 램프 * sdy  # 후방 근사 역맵 — 램프가 완만해 시각 오차는 램프 모양 미세 변형뿐
    y0 = np.clip(np.floor(ys).astype(np.int32), 0, H - 1)
    y1 = np.clip(y0 + 1, 0, H - 1)
    fy = (ys - np.floor(ys))[:, None, None]
    유효 = ((ys >= 0) & (ys <= H - 1)).astype(np.float32)[:, None, None]
    행 = pm[y0] * (1 - fy) + pm[y1] * fy
    xs = np.arange(W, dtype=np.float32)[None, :] - (램프 * sdx)[:, None]
    x0i = np.clip(np.floor(xs).astype(np.int32), 0, W - 1)
    x1i = np.clip(x0i + 1, 0, W - 1)
    fx = (xs - np.floor(xs))[:, :, None]
    유효 = 유효 * (((xs >= 0) & (xs <= W - 1)).astype(np.float32)[:, :, None])
    행 = np.take_along_axis(행, np.repeat(x0i[:, :, None], 4, axis=2), axis=1) * (1 - fx) \
        + np.take_along_axis(행, np.repeat(x1i[:, :, None], 4, axis=2), axis=1) * fx
    행 *= 유효
    ao = 행[:, :, 3:4] / 255.0
    rgb = 행[:, :, :3] / np.maximum(ao, 1e-4)
    return Image.fromarray(
        np.dstack([np.clip(rgb, 0, 255), np.clip(행[:, :, 3], 0, 255)]).astype(np.uint8), "RGBA")
...
def 라이브(args):
    ...
    # 패치 상자(기본 기준 · 전 컷 공용 크기) — 여백은 최대 시선 이동(±0.076·눈거리)보다 훨씬 크고,
    # 컷 간 얼굴 배치 오차(실측 ~5px)+검출 강등 한계(0.07·눈거리)가 다 들어와도 페더 밖이어야 한다.
    padx, pady = 눈거리 * 0.46, 눈거리 * 0.38
    ...
    for k in ("감음", "웃음"):
        찾음 = _눈찾기(컷들[k])
        상대 = (중점[0], 중점[1] * 컷들[k].height / H0)  # 실루엣 정렬 좌표 — 같은 참조 렌더라 오차 ~5px
        if 찾음 is not None:
            Le, Re = 찾음
            중심e = ((Le[0] + Re[0]) / 2.0, (Le[1] + Re[1]) / 2.0)
            if abs(중심e[0] - 상대[0]) > 눈거리 * 0.07 or abs(중심e[1] - 상대[1]) > 눈거리 * 0.07:
                # 검출이 실루엣 정렬에서 멀리 이탈 = 특징 일부만 잡힌 것(실측 08-15: 웃음 왼눈
                # ∧의 획 하나만 잡혀 +25px — 크롭이 밀려 왼눈이 상자 밖으로 잘렸다 · 유호 신고).
                중심e = 상대
                검출로그.append(f"{k}=검출이탈→상대")
            else:
                검출로그.append(f"{k}=검출")
    ...
    M = {"기본": _소프트팽창((기L패 < 본색밝기 - 40).astype(np.float32),
                          눈거리 * 0.09, 눈거리 * 0.03)}
    for k in ("감음", "웃음"):
        # v1 은 기본과의 «차이»를 마스크로 썼는데, 렌더 간 양모 결이 달라 차이가 상자 전체에서
        # 임계를 넘었다(08-15 실측: 마스크가 전면 1 = 사각 창문과 같음 · 특징 선택이 죽어 있었다).
        # 특징의 정의로 돌아간다: 지울 것(기본의 뜬 눈)과 그릴 것(표정의 스티치) 어두운 픽셀의
        # 합집합만 — 표정 쪽은 임계를 느슨히(-25) 잡는다(감은 눈 곡선은 어둠 신호가 약하다).
        pL패 = (P[k][:, :, 0] * 0.299 + P[k][:, :, 1] * 0.587 + P[k][:, :, 2] * 0.114)
        특징 = ((기L패 < 본색밝기 - 40) | ((pL패 < 본색밝기 - 25) & (P[k][:, :, 3] > 200)))
        M[k] = _소프트팽창(특징.astype(np.float32), 눈거리 * 0.07, 눈거리 * 0.03)

    # 자가 게이트(08-15 신설): 표정의 «스티치 심»이 페더 띠에 걸리면 눈이 잘린 채 조용히 구워진다
    # — 그날 실사고의 모양 그대로라 기계로 막는다. 임계 -85 = 스티치 심(lum 5~30)만 보고
    # 그늘 양모(-40~-70층)는 무시한다(08-15 4컷 실측: 정렬 정상이면 띠 안 ≤8px · 사고면 수백px).
    # 대가: 스티치가 유난히 옅은 미래 컷에선 이가 빠질 수 있다 — 그땐 아래 분모 출력부터 본다.
    페더띠 = 페더판 < 0.98
    for k in ("감음", "웃음"):
        pL패 = (P[k][:, :, 0] * 0.299 + P[k][:, :, 1] * 0.587 + P[k][:, :, 2] * 0.114)
        심 = (pL패 < 본색밝기 - 85) & (P[k][:, :, 3] > 200)
        잘림 = int((심 & 페더띠).sum())
        if 잘림 > 60:
            print(f"🔴 {k} 스티치 심이 패치 가장자리 페더에 걸렸다({잘림}px/심 {int(심.sum())}px)"
                  " — 눈이 잘려 나간다(통과 아님)")
            return 2
