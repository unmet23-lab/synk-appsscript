 ...
 *
 * 🔑 **너비 1800 이 선명도의 진짜 손잡이다** (유호 확정 08-22 「1800px 원본으로 저장」).
 *   털 한 가닥은 900px 에서 1픽셀보다 가늘어, 몇 샘플을 쏘든 픽셀 안에서 평균나 «얼룩»이 된다.
 *   실측(Coral · 오브 몸통 라플라시안 분산): 900·96샘플 31.47 → 900·384샘플 36.24(+15%, 5분)
 *   → **1800·96샘플 57.70(+83%, 3분55초)**. 샘플을 4배 쓰는 것보다 싸고 5배 낫다.
 *   ⚠ 디노이저 설정은 손잡이가 아니다 — denoising_input_passes·prefilter 는 블렌더 5.2 에서
 *     이미 RGB_ALBEDO_NORMAL·ACCURATE 가 기본값이라, 명시해도 픽셀이 안 바뀐다(실측 08-22).
 *
 * 사용:  node tools/오브굽기.js [--밖 폴더] [--샘플 96] [--너비 1800] [--색 "Coral,Oat"]
 */
'use strict';
...
const 견본 = 인자['샘플'] || '96';
const 너비 = 인자['너비'] || '1800';   // 900 은 털이 서브픽셀이라 얼룩이 된다(위 실측)
