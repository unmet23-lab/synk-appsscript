#!/usr/bin/env node
/* 운영자료 내보내기 — 유호님이 눈으로 보는 산출물을 바탕화면의 SYNK · SYNK LAB · SYNK SHIFT · SYNK PULSE로 보낸다.
 *
 ...
 *   위 전부에 `--갈래 방향|브랜드|사업|홍보|shift|pulse` 를 붙일 수 있다(기본 = 사업 · `--지금상태` 는 모든 갈래를 본다).
 *
...
/** 바탕화면 분류 정본 — 유호 확정 2026-09-09.
 * 회사 공통 기준·엔진은 SYNK, 학원은 LAB, 제작 방법 사업은 SHIFT, 창작 자산은 PULSE.
 * 번호와 옛 분류 별칭을 추가하지 않는다. 새 문서는 기존 안내·정본과 합칠 수 있는지 먼저 본다.
 * 키는 명령줄용 짧은 이름, 값은 바탕화면 기준 상대 경로다. */
const 갈래들 = Object.freeze({
  방향: 'SYNK',
  브랜드: path.join('SYNK', '엔진·브랜드'),
  사업: path.join('SYNK LAB', '운영'),
  홍보: path.join('SYNK LAB', '홍보'),
  shift: 'SYNK SHIFT',
  pulse: 'SYNK PULSE',
});
const 기본갈래 = '사업';
...
const 상태파일 = path.join(갈래들.방향, '_지금상태.html');
...
/** PowerShell **작은따옴표 리터럴**로 감싼다 — 안쪽 `'` 는 둘로 늘려 막는다.
 *
 * 🔴 2026-09-04 실측으로 갈아탄 자리다. 예전에는 `JSON.stringify` 로 감쌌는데, 그건 **JS 문법**이라
 *   역슬래시를 `\\` 로 두 배 늘린다. PowerShell 의 큰따옴표 안에서 역슬래시는 탈출문자가 아니므로
 *   `C:\\Users\\...` 가 **글자 그대로** 저장된다.
 *   · `TargetPath` 는 멀쩡해 보였다 — 셸 링크가 과녁 경로만 스스로 폈기 때문이다(우연히 가려졌다).
 *   · `WorkingDirectory` 에 같은 값을 넣자 `C:\\Users\\q1212\\Documents\\SYNK-appsscript` 로 굳어
 *     드러났다. **증상이 한 칸에서만 보이는 종류라, 안 보이던 쪽도 원래 틀렸던 것이다.**
 *   ⇒ 작은따옴표 리터럴은 역슬래시를 건드리지 않는다. 값 넷 전부 이 통로로 보낸다. */
function PS리터럴(값) {
 ...
 *
 * 🆕 2026-09-04 — `인자`·`작업폴더`·`설명` 을 받는다(전부 선택 · 안 주면 예전과 **글자 하나까지 같다**).
 *   왜: 문서 바로가기 말고 **프로그램 런처**가 필요해졌다(`claude.exe /design-login`).
 *   그 자리를 새 파일로 세우면 위 ㉠~㉢ 코드페이지 우회(ASCII 임시이름 + fs.rename)가 **둘로 갈린다** —
 *   갈린 쪽 증상은 언제나 「조용히 딴 이름」이라 안 보인다. 그래서 통로를 하나로 둔다.
 *   🔴 `설명` 에는 코드페이지 밖 글자(긴 줄표·이모지)를 쓰지 않는다 — 이름과 달리 **우회가 없다.**
 *     실측 2026-09-04: 기본 설명의 긴 줄표가 이미 `?` 로 깎인 채 서 있었다
 *     (`SYNK 로드맵.lnk` → `SYNK 세 이름의 역할과 시간표 ? 상시 로드맵`).
 *     ⚠ 깎는 층은 셸이 아니다 — `ps` 는 이미 `-EncodedCommand`(UTF-16LE)라 글자가 온전히 닿는다.
 *     **COM 이 .lnk 에 «쓸 때»** ANSI 로 깎는다. 그래서 이름처럼 ASCII 우회로 피할 수가 없다
 *     (이름은 파일명이라 rename 으로 확정할 수 있지만 설명은 파일 «안»이다).
 *     한글·가운뎃점(·)은 CP949 안이라 멀쩡하다 — 죽지 않고 조용히 한 글자만 바뀐다. */
function 바로가기만들기(링크경로, 대상, { 셸 = ps, 인자 = null, 작업폴더 = null, 설명 = null } = {}) {
  ...
  try {
    셸(
      '$s=New-Object -ComObject WScript.Shell;' +
        `$l=$s.CreateShortcut(${PS리터럴(임시)});` +
        `$l.TargetPath=${PS리터럴(대상절대)};` +
        (인자 ? `$l.Arguments=${PS리터럴(String(인자))};` : '') +
        (작업폴더 ? `$l.WorkingDirectory=${PS리터럴(path.resolve(작업폴더))};` : '') +
        `$l.Description=${PS리터럴(설명 || 'SYNK 정본 바로가기 — 원본이 바뀌면 자동으로 최신이 열립니다')};` +
        '$l.Save()'
    );
  } catch (e) {
    치우기();
...
function 상태판정({ 바로가기 = false, 대상없음 = false, 외부 = false, 무덤 = false, 자기 = false,
   ...
   *   ⚠ 그 둘은 **09-02 에 유호 확정으로 정식 삭제됐다**(「읽은 적 없어, 브리핑 아예 꺼줘」 ·
   *     결정.md) — 08-20 은 사고였지만 09-02 는 판정이다. 이 문단은 «사고의 기록»으로 남긴다.
   *     되살리지 말 것. 아래 규칙 자체는 뉴스 전용이 아니라 「자동 생성 … N시간에 1번」을 제 입으로
   *     밝히는 **모든** 산출물에 걸리므로, 지금 대상이 0이어도 코드는 산다.
...
function main(argv) {
  ...
  if (인자.includes('--목록') || 인자.length === 0) {
    ...
    console.log(`\n${폴더}  — 전체 ${합}건 (SYNK · SYNK LAB · SYNK SHIFT · SYNK PULSE)`);
    if (인자.length === 0) {
      console.log('\n쓰기: node tools/운영자료.js <파일...> [--갈래 방향|브랜드|사업|홍보|shift|pulse] [--이름 "표시 이름"]');
    }
    return 0;
  }
