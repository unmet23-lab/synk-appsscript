 ...
 *    `tests/운영자료.test.js` 의 「넣은 파일 이름에 번호가 안 붙는다」가 부활을 막는다. */
 ...
 *   그 자리는 회귀(`tests/반출도달.test.js`)가 「캐시를 껐을 때와 켰을 때가 같은가」로 문다. */
const 캐시경로 = (() => {
  try {
...
function 캐시키(절대) {
  try {
    ...
    // (`tests/운영자료.test.js` 「소스에 날 제어문자가 없다」가 잡았다 · 같은 자리 세 번째),
