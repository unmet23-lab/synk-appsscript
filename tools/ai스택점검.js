...
/* ── ③실행자 (GPT) — 「발주를 던졌나」와 「실행자가 탔나」는 다른 물음이다 (2026-09-06) ──
 *
 * ■ 왜 붙였나
 *   09-04 에 실행자 역할이 배치됐는데(유호 확정 · 결정 원장) 이 자는 그 레인을 **아예 안 봤다**.
 *   그래서 「배치대로 도는가」의 답이 넷 중 셋만으로 나고 있었다 — 넷째 벤더 자리가 조용하면
 *   그것도 「걸린 것 없음」으로 읽힌다(0건이 성공 얼굴).
 *
 * ■ 두 물음을 갈라 센다 — 하나로 세면 «발주만 던지고 멈춘 것»이 완주로 보인다
 *   ㉮ 던졌나  = 장부 줄 수. 0 이면 레인이 한 번도 안 돌았다.
 *   ㉯ 탔나    = `라운드들` 이 빈 런. 발주 검토가 돌려보내 **실행자를 못 태운** 자리다.
 *   ⚠ 시험 런(`probe-`)은 제품 런이 아니다 — 섞어 세면 「돌고 있다」는 거짓 초록이 난다. */
const 실행장부 = () => process.env.SYNK_EXEC_LEDGER || path.join(ROOT, 'docs', '_ops', '실행기록.jsonl');

function 실행자축() {
  const 적색 = [], 알림 = [];
  const 행들 = jsonl(실행장부());
  if (!행들 || !행들.length) {
    알림.push('🟠 **실행자 장부가 비어 있다** — 09-04 에 배치된 레인이 한 번도 안 돌았다.');
    알림.push('   던지기: node tools/codex-build.js <발주서>');
    return { 적색, 알림, 셈: { 합: 0, 제품: 0, 완주: 0, 못탐: 0 } };
  }
  const 시험런 = (r) => /(^|[\\/])probe-/.test(String((r && r.저장소) || ''));
  const 제품 = 행들.filter((r) => !시험런(r));
  const 완주 = 제품.filter((r) => r && r.상태 === '완주');
  const 못탐 = 제품.filter((r) => r && (!Array.isArray(r.라운드들) || !r.라운드들.length));
  if (!제품.length) {
    알림.push(`🟠 **시험 런만 ${행들.length}건 — 제품 저장소 런이 0이다.** 배선은 살았지만 값을 낸 적이 없다.`);
  } else if (못탐.length) {
    알림.push(`🟠 발주는 던졌는데 **실행자를 못 태운 런 ${못탐.length}건** — 발주 검토가 돌려보낸 자리다:`);
    for (const r of 못탐) {
      const 제목 = (r.발주 && r.발주.제목) || (r.발주 && r.발주.경로) || '(제목 없음)';
      const 구멍 = (r.검토 && Array.isArray(r.검토.구멍들)) ? r.검토.구멍들.length : 0;
      알림.push(`   · ${제목} — ${r.상태 || '상태 없음'}${구멍 ? ` · 구멍 ${구멍}건` : ''}`);
    }
    알림.push('   메운 판을 다시 던지면 그 자리에서 이어진다.');
  }
  return { 적색, 알림, 셈: { 합: 행들.length, 제품: 제품.length, 완주: 완주.length, 못탐: 못탐.length } };
}
 ...
 * 09-10부터 개발 검수의 최상 Gemini는 Google AI Pro 구독으로 분리했고, API 기본은 무료 「글」이다.
 * 「돈」은 이미지·음악·음성·영상처럼 API가 필요한 수동 작업만, 명시 승인 때 열린다. */
const 매달린것 = {
  글: ['몽골어 검문·시험과 GitHub 검수의 무료 API 경로(무료 몫 상한은 그대로 있다)',
      '로컬 최상 검수는 이 키가 아니라 Google AI Pro 구독 통로를 쓴다'],
  돈: ['명시한 그림 굽기·음악(Lyria)·목소리·음성 실측·Veo 영상만 여기로 온다',
      '`--유료-api` 또는 관리된 `SYNK_ALLOW_PAID_API=1` 없이는 토큰·모델 호출 전에 차단된다'],
};
...
function 한열쇠축(용도, r) {
  ...
  if (g.종류 === '유료차단') {
    알림.push('🟢 제미나이 「돈」 문은 **의도대로 잠겨 있다** — 자동 프로브·자동 생성으로 크레딧을 쓰지 않는다.');
    알림.push('   · 실제 종량제 작업에서만 `--유료-api`를 붙인다.');
  } else if (g.살았나 === false && 서버쪽) {
    ...
    알림.push('   처방: 잠시 뒤 다시 잰다 — node tools/모델정책.js --제미나이확인' + (용도 === '돈' ? ' 돈 --유료-api' : ' 글'));
  } else if (g.살았나 === false) {
    /* 상시 무료 글 문은 적색, 명시할 때만 여는 유료 돈 문은 알림이다. */
    ...
    담을곳.push(g.종류 === '자격'
      ? (용도 === '돈'
        ? '   처방: Vertex 전용 자격을 tools/구글계정붙이기.js로 다시 연결하고 프로젝트 일치를 확인한다. clasp 계정으로 대체하지 않는다. 다시 재기: node tools/모델정책.js --제미나이확인 돈 --유료-api'
        : '   처방: 키 교체(9월 Standard 키 거부 예고) — AI Studio 「Key Type」 확인 뒤 새 키를 같은 파일에.')
      : 용도 === '글'
        ? '   무료 몫이 찼으면 재설정을 기다린다. 로컬 최상 검수는 Google AI Pro 구독 통로라 계속 쓸 수 있다.'
        : '   유료 문은 기본 차단 상태로 둔다. 필요할 때만 결제 상태를 확인하고 `--유료-api`로 한 번 연다.');
  } else if (g.살았나 !== true) {
    /* 「키없음」은 «아직 안 만든 것»이라 다음 수가 분명하다 — 그건 적색으로 부르고 만드는 법까지 준다.
     * 네트워크·타임아웃은 «못 물어본 것»이라 알림이다(우는 검사는 꺼진다). */
    if (g.종류 === '키없음') {
      const 담을곳 = 용도 === '돈' ? 알림 : 적색;
      담을곳.push(`${용도 === '돈' ? '🟠' : '🔴'} **제미나이 「${용도}」 열쇠 파일이 없다** — 여기 매달린 것이 통째로 안 돈다:`);
      for (const 줄 of 매달린것[용도]) 담을곳.push('   · ' + 줄);
      for (const 줄 of String(g.안내 || g.사유 || '').split('\n')) if (줄.trim()) 담을곳.push('   ' + 줄.trim());
    } else {
...
module.exports = { 심문커버리지, 몽골어검문, 밖의사실축, 실행자축, 키생존축, 키생존기록, 표식꼴, 도장경로, 생존캐시경로, 실행장부 };
