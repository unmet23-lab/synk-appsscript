 ...
 * 🔄 09-05 밤 — 기본 문이 「돈」(Vertex · 크레딧)으로 바뀌어(유호 확정 · 정책 `기본용도()`) 매달린 것이 뒤집혔다:
 *   상시 도는 것(몽골어 검문 · 검수 둘째 눈 · 설계 심문)이 전부 돈 열쇠에 매달리고, 글 열쇠는 `--글` 을 손으로 준 예비다.
 * (09-03 판은 반대였다: 돈을 태우는 것은 그림·음악이고 글은 20% 안이었다.) */
const 매달린것 = {
  글: ['예비 문이다(09-05 밤부터) — `--글`·`--용도 글` 을 손으로 준 실행만 여기로 온다 · 자동으로 도는 것은 없다',
      '크레딧이 끝나는 날 기본을 다시 여쭙는 자리(그때 이 문이 다시 상시가 될 수 있다)'],
  돈: ['몽골어 검문 ①역번역 ②문법 — 대외로 나갈 몽골어가 **지금 아무 자도 안 지난다**',
      '설계 심문·검수 둘째 눈의 gemini 회차 · 그림 굽기(나노바나나 프로) · 음악(Lyria) · 목소리·음성 실측',
      '크레딧(₩435,523 · 시한 2026-11-13)이 마르면 카드로 넘어간다 — 「죽었다」가 잔액 소진이면 그 자리가 유호님 손이다'],
};
...
function 한열쇠축(용도, r) {
  ...
  if (g.살았나 === false && 서버쪽) {
    ...
    /* 🔑 **용도마다 «우는 크기»가 다르다** — 상시 도는 자리가 죽으면 적색, 예비가 죽으면 알림.
     *   🔄 09-05 밤 유호 확정(「앞으로 제미나이 열쇠는 vertex로」)으로 상시 자리가 「돈」이 됐다 → 돈 = 적색 · 글 = 알림.
     *   (09-03 「항상 무료먼저」 때는 반대였다.) 원리는 같다 — 안 쓰는 자리가 매 세션 빨갛게 울면 그게 모래주머니고,
     *   그러면 진짜 적색까지 안 읽힌다. */
    const 담을곳 = 용도 === '글' ? 알림 : 적색;
    담을곳.push(`${용도 === '글' ? '🟠' : '🔴'} **제미나이 「${용도}」 열쇠가 죽어 있다** (${언제} · ${g.상태 || ''} ${g.종류 || ''}) — 여기 매달린 것:`);
    for (const 줄 of 매달린것[용도]) 담을곳.push('   · ' + 줄);
    if (g.사유) 담을곳.push(`   · 구글이 한 말: ${String(g.사유).slice(0, 160)}`);
    담을곳.push(g.종류 === '자격'
      ? (용도 === '돈'
        ? '   처방: Vertex 는 OAuth 토큰이다 — 토큰이 죽었으면 npx clasp login(유호님 손 · 브라우저 동의 한 번). 다시 재기: node tools/모델정책.js --제미나이확인'
        : '   처방: 키 교체(9월 Standard 키 거부 예고) — AI Studio 「Key Type」 확인 뒤 새 키를 같은 파일에.')
      : 용도 === '글'
        ? '   **지금 멈추는 것은 없다** — 예비 문이다(09-05 밤부터 기본은 돈). 공짜 몫이 찼으면 오후 4~5시 재설정을 기다린다.'
        : '   처방: 잔액 소진이면 크레딧 충전(유호님 손 · console.cloud.google.com/billing) · 벽(429)이면 잠시 뒤 다시. 다시 재기: node tools/모델정책.js --제미나이확인');
  } else if (g.살았나 !== true) {
