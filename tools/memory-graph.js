 ...
 *
 * ■ 「머리줄 대기」 갈래 — 왜 «본문 ⏳» 가 아니라 «description» 인가 (v2 · 08-31 · 유호 지시)
 *   위 넷 중 셋째가 「본문 `⏳` 91벌 = 전부 정상」이다. **그건 지금도 참이고 여기서 안 뒤집는다** —
 *   MEMORY.md 머리말이 「토픽 파일 «안»의 ⏳ 는 대기가 아니라 역사다」로 이미 처분했다.
 *   🔑 **다른 축은 «머리줄»이다.** `description` 은 세션 첫머리에 **색인으로 올라가는 유일한 줄**이라
 *   읽는 사람이 그것을 «지금 상태»로 읽는다. 그래서 본문이 갱신돼도 머리줄이 안 따라오면
 *   **다음 세션이 이미 답하신 것을 또 여쭙는다.** 08-31 에 접기로 하루에 **여섯 번** 그 무늬를 잡았고,
 *   `season-review-vessel` 안에는 「유호님이 이 답을 **두 번** 주셨다」가 실제로 적혀 있었다.
 *   📏 **정밀도를 재고 좁혔다**(08-31): 낱말만으로 잡으면 19벌 중 진짜는 6 = **32%**(우는 검사).
 *     ⓐ「유호님이 아직 안 정하셨다」를 «말하는» 어법으로 좁히고 ⓑ정정 표지(옛·낡았·닫혔다)가 든
 *     머리줄은 뺐다 — 안 빼면 **오늘 고친 파일이 내일 운다**(고치는 것을 벌하는 검사는 꺼진다).
 *     결과 9벌 · 진짜 8 ≈ 89%. 첫 실물 = `repos-are-public` 이 「유호님 판정 대기」인데
 *     정본 `yuho-open-decisions` §⑪ 는 **「닫혔다 08-31」**이었다.
 *   🔑 그래서 «정본 대조»를 한 칸 같이 낸다 — 그 파일이 `yuho-open-decisions` 에 얼굴을 걸고 있나.
 *     둘 중 하나가 반드시 참이다: **정본에 없으면** 진짜 대기인데 정본이 모르는 것이거나 머리줄이
 *     낡은 것이고, **정본에 있으면** 정본이 그 자리를 이미 닫았는지 눈으로 봐야 한다.
 *   ⚠ **사람 몫이다** — 기계는 「지금도 대기인가」를 원리상 못 안다. 삼키면 유호님 앞에 걸린
 *     «진짜» 자리가 조용히 사라진다(이 자가 지키는 경계 그대로).
 */

/** 자는 «한 곳»에만 산다 — 자가 흔들리면 「줄었다/늘었다」가 거짓이 된다. */
const 위생자판 = Object.freeze({
  판: 3,                    // 자를 고치면 이 수를 올린다 — 래칫이 «폴더가 변한 것»과 «자가 변한 것»을 가른다
  시각지문절: 4,             // 이만큼이면 「한 파일 = 한 사실」이 아니라 «세션 일지»다
  description최대: 200,
});

/* 머리줄이 「유호님이 아직 안 정하셨다」고 «말하는» 어법. 낱말 하나로 잡으면 32% 로 운다 —
 * 대기어 홀로가 아니라 **누가 무엇을 기다리는지**가 문장에 있어야 센다(머리말 §머리줄 대기). */
const 머리줄대기 = /⏳|(유호|판정|결정|승인|픽)[^,.·)]{0,8}대기(?![실열표])|안 정한|안 정하셨|픽 하나|골라주세요/;
/* 이미 «고친» 머리줄은 뺀다 — 정정문은 옛 표기를 인용으로 들고 다니므로 안 빼면 고친 파일이 운다. */
const 머리줄정정 = /옇|낡았|무효|닫혔다|종결|역사»?다|묶었다|묶여 있/;
/* [2026-08-31] 「«역사»다」·「되돌아오는 자리에 묶었다」도 정정이다.
 *   그 둘은 「이건 지금 유호님 앞에 안 걸려 있다」를 정확히 말하는 문장인데,
 *   안 넣으면 **이미 고친 파일이 영원히 빨간카로 남고** 다음 사람은 규칙이 아니라 검사를 끔는다
 *   (tests/기억위생.test.js 「고치는 것을 벌하면 검사가 꺼진다」가 지키는 자리). */
/** 유호님 앞에 걸린 것의 정본 — 머리줄 대기를 여기와 대조한다(색인에 옮겨 적으면 갈린다). */
const 대기정본slug = 'yuho-open-decisions';
...
function 위생(nodes, dir) {
  const 틀 = {
    ...
    분모: 0, 확인불가: null,
    고칠것: { 역링크뒤처짐: [] },
    판정거리: { name불일치: [], node_type없음: [], type없음: [], description과길기: [], 일지형: [], 표식깨짐: [], 머리줄대기: [] },
  };
  ...
  /* 「유호님 앞에 걸린 것」 정본이 이 파일을 쥐고 있나 — 🔴 정본이 «없으면» false 가 아니라 null 이다.
   *   「안 쥔다」와 「못 쟀다」가 같은 얼굴이면 그건 자가 아니다(이 파일 :382 규약 그대로). */
  const 정본본문 = (토픽.find((t) => t.slug === 대기정본slug) || {}).본문;
  const 정본이쥔다 = (slug) => (정본본문 == null ? null : 정본본문.includes(`[[${slug}]]`));
  ...
  for (const n of 토픽) {
    ...
    /* 머리줄이 「유호님 대기」를 주장하나 — 정본 대조 한 칸을 달아 낸다(머리말 §머리줄 대기). */
    if (fm.description && 머리줄대기.test(fm.description) && !머리줄정정.test(fm.description) && n.slug !== 대기정본slug) {
      틀.판정거리.머리줄대기.push({ 파일: n.slug, 머리: fm.description.replace(/^"|"$/g, '').slice(0, 90), 정본: 정본이쥔다(n.slug) });
    }
...
function 래칫(현재, 폴더, 파일 = 래칫경로()) {
  const p = path.resolve(파일);
...
function 위생훅줄(옵션 = {}) {
  ...
  try {
    ...
    const 판정 = [
      ...
      /* 정본O 를 앞세운다 — 정본이 «쥐고 있는» 것은 그 정본에서 닫혔는지 바로 대조되므로 답이 제일 싸다. */
      ['머리줄 대기', w.판정거리.머리줄대기, (x) => {
        const 것 = x.slice().sort((a, b) => (b.정본 === true) - (a.정본 === true))[0];
        const 곳 = 것.정본 === null ? '정본을 못 읽었다' : 것.정본 ? `정본(${대기정본slug})이 쥔다 — 거기서 닫혔나 본다` : '정본에 없다';
        return `${x.length}벌 — ${것.파일} 「${것.머리.slice(0, 44)}…」 · ${곳} · ⓐ아직 대기 ⓑ닫혔다(머리줄 고침) ⓒ그대로`;
      }],
    ].filter(([, x]) => x.length);
    ...
    /* 시험은 임시 기억 폴더의 수를 운영 래칫에 쌓지 않도록 별도 파일을 주입한다.
     * 평시 훅·CLI는 인자를 주지 않으므로 기존 운영 경로 하나를 그대로 쓴다. */
...
function main() {
  ...
  if (args.includes('--위생') || args.includes('--위생고침')) {
    ...
    표('머리줄이 «유호님 대기»를 말한다', w.판정거리.머리줄대기,
      (x) => `${x.정본 === null ? '정본?' : x.정본 ? '정본O' : '정본✗'} ${x.파일}\n            ${x.머리}`);
