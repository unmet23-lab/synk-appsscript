 ...
 *   · 발표물 조판의 아름다움 — 못 잰다.
 *   · ①의 «글자 수»는 병이 아니라 참고다. 옛 판은 머리 24줄을 통째로 세서 «좋아진 문서의 리드»를
 *     병으로 읽었다(1,562자). 지금은 날짜·판 표기 꼴만 세지만, 그것도 「개원은 2027-02-25」처럼
 *     리드가 마땅히 지는 사실을 잡는다 — 그래서 판정은 «판 표기가 머리에 있나» 한 줄이 진다.
...
const 죽은것 = [
  ...
  /* 🔴 08-26 유호 확정 「7단계 캐릭터는 이제 없어. 마스코트로 대체될거야」로 이 셋이
   *   «지키는것»에서 «죽은것»으로 넘어왔다. 넘기기 전 이 자는 「7단계가 줄면 잘못이다」를
   *   재고 있었다 — 확정과 정반대 방향이라, 스윕을 도우라고 만든 자가 스윕을 말렸다. */
  { 말: '7단계', 왜: '2026-08-26 은퇴 — 가이드 세 축(곁에 온다·또렷해진다·밝아진다)이 대신한다' },
  { 말: '뉴로', 왜: '2026-08-26 은퇴 — 구 7단계의 첫 칸' },
  { 말: '싱크마스터', 왜: '2026-08-26 은퇴 — 구 7단계의 끝 칸' },
];

/** 🔑 «살아 있으니 지우면 안 되는» 것 — 이 자의 존재 이유 절반이 여기다.
 *  이걸 안 적어 두면 다음 세션이 낡음 정리 중에 멀쩡한 기능을 문서에서 지운다.
 *
 *  🔴 08-26 에 이 칸의 내용이 통째로 바뀌었다. 여기 있던 「뉴로·싱크마스터·7단계」는
 *  유호 확정으로 은퇴해 위 죽은것 으로 넘어갔다. 남은 위험은 **그 밑에 깔린 기계**다 —
 *  7단계를 걷어낼 때 그것이 서 있던 «문법 숙달 게이트»(GRAMMAR_BANK 72종 · Code.js:798
 *  `grammar: 72`)까지 같이 지우기 쉽다. 그 게이트는 라이브에서 검증됐고 뒤를 잇는
 *  세 축이 그대로 물려받는다 — 「공부로만 자란다」를 코드로 강제하는 것이 그것 하나다. */
const 지키는것 = ['문법 게이트', '문법 숙달 게이트'];

/** ⑦ 만든 사람만 아는 낱말 — 풀이 없이 서면 독자가 해독을 한 겹 더 해야 한다 */
const 조어 = ['콕핏', '리텐션 레이더', '케어 사각', '격파 찬스', '기회 밸런스', '알림 다이어트', '위성 클래스'];

/** 🔑 «은퇴 선언» 줄 — 죽은 낱말을 «죽었다고 적으려고» 쓴 자리. 병에서 뺀다. */
const 은퇴선언 = /은퇴|폐지|소각|걷어|~~/;
/* 그리고 「구 7단계」처럼 **낱말 앞에 「구」를 붙여** 옛것이라고 밝힌 자리도 병이 아니다. */
...
function 재기(경로) {
  ...
  /* 🔴 «머리 두께»가 아니라 «개정이력 두께»를 잰다(08-26 수리 셋째). 리라이팅 뒤 머리 블록은
   *   개정이력이 아니라 «한 줄 리드»라, 첫 판은 좋아진 문서의 리드를 병으로 셌다(1,562자). */
  const 이력꼴 = (l) => /\bv\d|20\d\d[-.]\d\d|개정|이력|작성일|버전/.test(l);
  let 이력글자 = 0;
  for (let i = 2; !html && i < 줄들.length; i++) {
    if (!줄들[i].trim()) { if (이력글자) break; continue; }
    if (이력꼴(줄들[i])) 이력글자 += 줄들[i].length;
    if (i > 24) break;              // 머리 블록만 — 본문까지 세지 않는다
  }

  /* ② 메모장 조판 — 코드 스팬(``) 안은 조판이 아니라 조판의 «인용»이라 벗기고 잰다 */
  /* 코드 스팬과 «낫표 인용»을 벗긴다 — 정본이 「미확정 칸은 ____ 로 비운다」고 «규칙을 적을 때»
   *   그 줄을 병으로 세면, 규칙을 적은 문서가 제일 아파 보인다(집필 규범 15줄이 그랬다). */
  const 벗긴줄들 = 줄들.map((l) => l.replace(/`[^`]*`/g, '').replace(/「[^」]*」/g, ''));
  ...
  /* ③ 문서가 자기 이야기를 한다 */
  const 기계표기 = 벗긴줄들.filter((l) => /<!--|-->|doc-graph|파생:\s*docs\//.test(l)).length;

  /* ⑦ 조어 — «풀이 없는» 것만. 같은 줄에 → 가 있으면 그 줄이 곧 풀이다(집필 규범 500줄이 그렇다). */
  let 조어수 = 0;
  for (const l of 줄들) {
    if (l.includes('→')) continue;
    for (const w of 조어) 조어수 += 세기(l, w);
  }
  ...
  const 죽음 = 죽은것.map((d) => {
    ...
    for (const l of 줄들) {
      const n = 세기(l, d.말);
      if (!n) continue;
      /* 🔑 «은퇴 선언»은 병이 아니라 약이다 — 「구 7단계는 은퇴했다」를 병으로 세면
       *   그 줄을 지우게 되고, 그러면 소각이 안 끝난 것을 아무도 모르게 된다(08-26). */
      const 옛것표기 = l.includes('구 ' + d.말) || l.includes('구 「' + d.말);
      if (은퇴선언.test(l) || 옛것표기 || l.includes('|') || l.includes('「' + d.말 + '」')) 반례 += n; else 병 += n;
    }
    return { ...d, 수: 병, 반례 };
...
줄('머리 24줄의 날짜·판 표기 글자 (참고)', 요약.병1_이력글자.toLocaleString());
console.log('   ⚠ 이 수는 «병»이 아니라 참고다 — 리라이팅 뒤로는 「개원은 2027-02-25」 같은');
console.log('     리드의 사실이 여기 잡힌다. 병은 위 한 줄(판 표기가 머리에 있나)이 잰다.');
...
줄('금칙표·낫표 안·은퇴 선언 (반례 — 병 아님)', 요약.병8_반례총수);
console.log('   ⚠ 반례를 지우면 규범이 되레 무너진다 — 「쓰지 마세요」도 「은퇴했다」도 그 낱말을 적어야 선다.');

console.log('\n🔑 지우면 안 되는 것 (살아 있다 — Code.js 에서 지금도 돈다)');
줄('문법 숙달 게이트가 남아 있는 문서', 요약.지킴_살아있는벌, 요약.문서수);
console.log('   ⚠ 이 수가 «줄면» 낡음 정리 중에 멀쩡한 기능을 지운 것이다.');
console.log('   🔑 7단계를 걷을 때 그 밑의 게이트까지 같이 지우기 쉽다 — 게이트는 세 축이 물려받는다.');
