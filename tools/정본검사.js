...
function 재기(경로) {
  ...
  /* ② 메모장 조판 */
  const 구분선 = 줄들.filter((l) => /[─═]{10,}/.test(l)).length;
  const 머리기호 = 세기(본문, '◆');
  const 밑줄칸 = 줄들.filter((l) => /_{4,}/.test(l)).length;
  const 괘선 = 줄들.filter((l) => /[┌┬┐├┼┤└┴┘│]/.test(l)).length;
  ...
  const 죽음 = 죽은것.map((d) => ({ ...d, 수: 세기(본문, d.말) })).filter((d) => d.수 > 0);
...
줄('죽은 것이 남은 문서', 요약.병8_죽은것남은벌, 요약.문서수);
줄('죽은 것 총 등장', 요약.병8_죽은것총수);
