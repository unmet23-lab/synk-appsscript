...
function 역형상읽기() {
  ...
  const 표 = {};
  /* 🔴 08-27: 이 정규식이 **5칸 튜플**을 찾고 있었는데 표는 그 전에 이미 6칸이 돼 있었다
   *   (`몸색` 이 붙었다). 그래서 **0건을 잡고 조용히 빈 표를 돌려주고**, 지면은 네 역 모두
   *   「가로 undefined · 세로 undefined」를 싣고 있었다 — 「손으로 옮겨 적으면 낡는다」를 막으려
   *   지은 함수가 «다른 방식으로» 거짓을 말하고 있었다. 파싱은 옮겨 적기보다 낫지만 공짜가 아니다.
   *   ⇒ 6칸으로 맞추고, **0건이면 던진다**(아래) — 이 갈래에서 0 은 언제나 «성공 얼굴»이다. */
  const 재 = /'(boss|prof|lead|insp)':\s*\('([^']+)',\s*([\d.]+),\s*([\d.]+),\s*([\d.]+),\s*([\d.]+),\s*'([^']+)'\)/g;
  let m;
  while ((m = 재.exec(원문)) !== null) {
    표[m[1]] = { 형태: m[2], 가로: +m[3], 세로: +m[4], 크기: +m[5], 각짐: +m[6], 몸색: m[7] };
  }
  const 센것 = Object.keys(표).length;
  if (센것 !== 4) {
    throw new Error(
      `🔴 역형상 표를 ${센것}/4 만 읽었다 — NPC세트굽기.py 의 «역형상» 튜플 모양이 바뀌었다.\n`
      + '   지면이 「undefined」를 싣고도 종료코드 0 으로 나가는 자리라 여기서 멈춘다.\n'
      + '   → tools/NPC시트.js 의 정규식을 그 튜플 칸 수에 맞춘다.');
  }
  return 표;
}
...
function main() {
  ...
  const 배지줄들 = 역들.map((r) => {
    ...
    return `<h3 class="듦">${esc(역뜻[r][0])} <code>${r}</code>
        <span class="메타">— ${esc(f.형태 || '?')} · 가로 ${f.가로} · 세로 ${f.세로} · 크기 ${f.크기}${f.각짐 ? ` · 각짐 ${f.각짐}` : ''}${f.몸색 ? ` · 몸색 ${esc(f.몸색)}` : ''}</span></h3>
