...
#   2026-09-09 선별: 한옥 마당 · 네온 물가 · 거울 수면과 문 · 반딧불 노을 들판만 허용한다.
#   house는 기존 곡의 네온 편성 키다(층_house.mp4). 삭제한 house.png 밤도시를 다시 읽지 않는다.
...
#   2) 덮개 굽기(0원): node tools/라디오배경굽기.js --장르 neon_water --무대덮개  (chuseok · dream_water · dream_field도)
...
판만들기() {  # $1 장르 · $2 scale · $3 crop
  local g="$1" cover="$R/docs/라디오/무대덮개/$1.png" out="$OUT/_판/${1}_선별20260909.png"
  local source="$R/docs/라디오/무대/$1.png"
  case "$g" in house|neon_water) source="$R/docs/라디오/무대/보관/전자_네온물가.png";; esac
  [ -f "$out" ] && return 0
  ffmpeg -y -loglevel error -i "$source" -i "$cover" \
...
for a in "$IN"/*.aac; do
  ...
  case "$genre" in
    chuseok|neon_water|house|dream_water|dream_field) ;;
    *) echo "🔴 $n — 삭제되었거나 모르는 배경 키: $genre (다른 그림으로 치환하지 않음)"; bad=$((bad+1)); continue ;;
  esac
  out="$OUT/$n.ts"
  ...
  # 드림 두 키의 옛 층 영상은 실내 방이다. 보존한 야외 원본과 섞지 않는다.
  case "$genre" in
    house|neon_water) layer="$R/docs/라디오/무대영상/층_house.mp4" ;;
    dream_water) layer="$R/docs/라디오/무대영상/층_거울수면과문.mp4" ;;
    dream_field) layer="$R/docs/라디오/무대영상/층_반딧불노을들판.mp4" ;;
    chuseok) layer="$R/docs/라디오/무대영상/층_chuseok.mp4" ;;
  esac
  if [ -f "$layer" ]; then
    ...
    rc=$?
      ...
      # 승인 장면의 영상이 아직 없으면 그 장면의 원본 정지 그림만 사용한다.
      판만들기 "$genre" 1357:-2 1280:720:34:17 && \
      ffmpeg -y -loglevel error -loop 1 -framerate 30 -i "$OUT/_판/${genre}_선별20260909.png" -i "$a" -map 0:v -map 1:a -shortest -pix_fmt yuv420p \
  ...
  t1=$(date +%s)
  src_note="정지 그림"; [ -f "$layer" ] && src_note="층 영상"
