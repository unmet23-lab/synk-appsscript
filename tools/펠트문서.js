...
const 면들 = {
  ...
  펠트오브: { 천: 기본색이름 },   /* 낮(인쇄) 표지 명패의 «자» — v0.5(09-01)부터 화면 표지는 합주 장면(글자 투명)이라
                                     밤 짝은 걷었고, 이 면은 인쇄의 평면 Chalk 명패 하나만 진다(천 = 근백색 근사). */
...
const 짝들 = [
  ...
  { 글자: 'Ink', 면: '펠트오브', 지면: '낮', 큰가: true, 자리: '낮의 오브만 명패다(화면은 합주 장면·글자 투명 · v0.5) — 평면 Chalk 위 이니셜 2.3rem·800, 대형 기준' },
];
...
function 스킨(쓸천, 림 = '무채', 히어로 = null, 쓴클래스 = null) {
  // 🔑 `규격`(= `_규격` 통짜)은 여기서 안 받는다 — 지면이 적는 규격은 «쓰는 천»에서 온다(아래 규격주).
  const { 천, 색, 서체, 부품들 } = 정본읽기();
  ...
  /* 🔑 중복을 걷는다 — `기본천` 은 [기본색이름, 'Chalk', 'Paper'] 인데 기본색이 Chalk 면 같은 천이 둘이 되고,
   *   지면 주석이 「Chalk 결 95.1% · Chalk 결 95.1% · …」로 같은 말을 두 번 한다(09-04 실측). CSS 변수도 둘로 난다. */
  const 목록 = [...new Set(쓸천 && 쓸천.length ? 쓸천 : 기본천)];
  ...
  /* 🔴 2026-09-04 — 규격 줄이 22벌 지면에서 «undefinedpx undefined» 로 나가고 있었다(codex `2544cb310b16`).
   *   `_규격` 이 한 겹 깊어졌는데(지금 = `{출발:{한변,형식}, 사다리, 결하한}`) 읽는 쪽은 옛 모양(`규격.한변`)을 봤다.
   *   🔑 그런데 「출발」로 갈아 끼우는 것도 틀린다 — **천마다 규격이 다르다**(09-04 실측: 240/q80 20벌 ·
   *     240/q92 5벌 · 288/q92 3벌). 사다리가 생기면서 «하나의 규격»이라는 전제 자체가 옛 판이 됐다.
   *   ⇒ 그 지면이 «실제로 쓰는» 천의 규격만 모아 적는다. 하나면 하나, 갈리면 갈린 대로 — 지면이 참을 말한다. */
  const 규격주 = [...new Set(목록.map((n) => `${천[n].한변}px WEBP q${천[n].품질}`))].join(' · ');
...
${천줄}
  ...
  /* ── 표지 = 무대 위 «합주» 히어로 + 편집 타이포 ─────────────────────────── */
  /* v0.5(09-01 · 유호 「요소가 부족해 — 훨씬 더 명품」): 표지 히어로를 명패에서 «장면»으로.
     룸굽기 부품표가 정확히 이 쓰임(「표지·히어로 장면 — 재질 셋이 서로를 안다」)으로 구워 둔
     합주(유리·레진·펠트 한 무대 · 두 층 몸+접지)를 표지가 처음 입는다 — 여섯 자 ⑥(합주)의 집행.
     v0.4 의 단품 펠트 명패(표지오브굽기 통로)는 이 판이 서면서 걷었다(낡은 것은 남기지 않는다). */
  .표지{padding:76px 0 58px;display:grid;gap:30px;grid-template-columns:1fr;}
  ...
  .표지 .오브::before{
    content:'';position:absolute;inset:-20% -10%;border-radius:0;pointer-events:none;box-shadow:none;
  ...
  /* (v0.5 에서 옛 데스크톱 2열(112px 명패 + 글)을 걷었다 — 히어로가 «장면»이 되면서
     표지는 전 폭에서 한 기둥이다: 장면 위 · 타이포 아래.) */
  ...
  @media print{
    ...
    /* 표지 — 합주 렌더는 «검은 무대의 장면»이라 흰 종이에 안 옮긴다(비네트가 얼룩이 된다).
       종이는 평면 Chalk 명패(재질 흉내 0 — 정직한 플랫) + 엔진 이니셜을 도로 보인다.
       「배경 그래픽」이 꺼진 인쇄에선 배경류가 전부 빠져 글자만 남는다 — 잃는 것 없음. */
    .표지{display:block;}
    .표지 .오브{width:112px;height:112px;aspect-ratio:auto;margin-bottom:14px;
      background-image:none;background-color:${색['Chalk']};border-radius:31px;
      display:grid;place-items:center;color:${색['Ink']};font-weight:800;font-size:2.3rem;}
    .표지 .오브::before{display:none;}
...
/* 🔑 `스크립트` 를 내보내는 이유 = `tools/룸조판.js` 가 **디스크를 거치지 않고** 같은 스킨을 얹는다.
 *   조판의 원고는 마크다운이라 `굽기(입력파일, 출력파일)` 의 «HTML 원고 파일»이 없다 —
 *   중간 HTML 을 파일로 떨구면 원고가 둘(.md 와 .html)이 되고, 둘이면 갈린다.
 *   그래서 주입 재료(`스킨`+`스크립트`)를 열어 준다. **주입 순서·내용은 여기 하나가 정본이다.** */
module.exports = { 스킨, 스크립트, 굽기, 검사, 원고, 대비판정, 표식, 기본천, 림레시피, 클래스지도,
