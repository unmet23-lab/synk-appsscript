...
if (require.main === module) {
   ...
   * ⚠ 이 도구는 하위 명령을 **`--` 낱말**로 받는다(`--굽기`·`--원고` …) — 그 여덟이 자기
   *   낱말이고, 추출기가 「argv 한 칸(`모드 = 인자[0]`)을 낱말과 견주는 꼴」을 보게 넓힌 뒤에야
   *   기계가 그 목록을 확인해 준다(그 전엔 `--림` 하나만 보였다). */
  const 아는플래그 = ['--검사', '--굽기', '--대비', '--되메우기', '--림', '--스킨', '--원고', '--재현', '--전량'];
  const 플래그오류 = 인자게이트('펠트문서', 인자, 아는플래그);
