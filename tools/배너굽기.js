...
const 안들 = {
  ...
  synklab: {
     ...
     * 여백 없이 잘려 있어 같은 폭이면 작아 보이고, 실땀이 보여야 구운 값이 산다. */
    로고크기: 360,
    천: { 보풀: '#FFFFFF', 그늘: 색['Oat'], 어둡나: false },
    워터마크: true,
    워터실: 실.여린코랄,
  ...
  링크드인코랄: {
    규격: '링크드인',
    이름: '⑨ 링크드인 · 구운 펠트 워드마크 + 한 줄 — ✅ 확정 (유호 09-06 · 로고 세대 09-08) · @yuhobuilds',
    설명: '⑧과 한 글자만 다르다(k 가 코랄). 🆕 09-08 에 로고를 «그린 것»에서 «구운 펠트 실물»로 갈았다 · 업로드본 = yuhobuilds_링크드인.png',
    바탕: 색['Ink Deep'],
    글자: 색['Paper'],
    보조: 색['Stone'],
    로고: () => 구운워드마크(),
     ...
     *   ② 실땀과 보풀이 «보여야» 구운 값이 산다 — 190px 에서는 결이 뭉개져 그냥 흐린 글자가 됐다.
...
/** 구운 워드마크 — «그린 것»이 아니라 제미나이가 구운 펠트 실물의 누끼다.
 *
 * 왜 있나 (유호 지시 2026-09-08 「요소들이랑 로고랑 둘 다 최신화 됬으니까」):
 *   `로고정본.js` 의 `워드마크()` 는 SVG 로 «펠트처럼 보이게 그린다». 그 판이 09-08 까지
 *   링크드인 배경과 유튜브 배너에 서 있었는데, 브랜드 규율은 「펠트 실물만 · 재질 흉내 0」이다
 *   (명품마케팅 §③ 눈 · 기억 loom-baked-assets-only-for-ui). 같은 날 synkbrief 프로필을
 *   그린 도장에서 구운 도장으로 갈면서 배너만 옛 세대로 남는 것이 어긋나 여기를 열었다.
 *
 * 🔑 **왜 4K 원본을 안 자르고 누끼를 쓰나** — `밤워드마크_어두운_4K.png` 는 4096² 안에 글자가
 *   높이의 31% 를 차지한다. 링크드인 배경은 4:1 이라 그 판을 그대로 자르면 글자가 띠 밖으로
 *   넘친다. 그래서 «글자만 오려 낸» 누끼를 얹고 바탕은 도구가 굽던 천을 그대로 쓴다.
 *
 * ⚠ 이 누끼는 «단색 k» 판이 없다 — 구울 때 k 가 코랄로 굳었다. 그래서 이 함수를 쓰는 안은
 *   자동으로 «코랄 k» 갈래가 된다(⑨ 링크드인코랄이 이미 그 확정이라 부딪히지 않는다).
 */
function 구운워드마크(판 = '밤') {
  /* 🆕 09-08 저녁 — 밝은 판이 생겼다(유호 지시 「로고 지금 구워줘」 · 336원).
   *   그전까지 구운 워드마크는 «크림 글자» 하나뿐이라 양모 바탕 배너(⑤ synklab)에서는 사라졌고,
   *   그 자리만 «그린» 벡터 로고로 남아 있었다 — 유호님이 그것을 잡으셨다.
   *   양모 판 = 잉크 글자(#2B2320) + 코랄 k + 크림 실땀, 계획 「양모 워드마크 · 밝은 판」. */
  const 이름 = 판 === '양모' ? '양모워드마크_누끼.png' : '밤워드마크_누끼.png';
  const 경로 = path.join(ROOT, 'docs/Loom_자산/구움/', 이름);
