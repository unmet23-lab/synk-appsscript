...
const 안들 = {
  ...
  밤기호: {
    ...
    글자: 색['Paper'],
    보조: 색['Stone'],
    /* 현행 로고 = 승인 Paper 사진형. SYNK 네 글자의 실땀만 없애고 펠트 결을 유지한다. */
    로고: () => 구운워드마크(),
    천: { 보풀: 색['Deep Wool'], 그늘: '#000000', 어둡나: true },
    워터마크: true,
    이음: true,
  ...
  synkbrief: {
    이름: '④ 밤 바탕 · Paper SYNK — @synkbrief (SYNK 브리핑)',
    설명: '③과 한 벌이다(같은 유호님 축·같은 한국어권). 같은 중립색 펠트 로고를 사용하고 문안으로 구분 · 업로드본 = synkbrief_배너.png',
    바탕: 색['Ink Deep'],
    글자: 색['Paper'],
    보조: 색['Stone'],
     ...
     *   현재 로고는 09-10 승인에 따라 Paper 네 글자를 사용한다. 이 기록을 코랄 k 재도입의 근거로 쓰지 않는다.
  ...
  synklab: {
    ...
    보조: 색['Ash Wool'],
    로고: () => 구운워드마크('양모'),
    /* 누끼는 그린 판보다 커야 한다(링크드인에서 배운 값 그대로 · 09-08).
     * 여백 없이 잘려 있어 같은 폭이면 작아 보이고, 펠트 결이 읽히는 표시 크기를 유지한다. */
    로고크기: 360,
    천: { 보풀: '#FFFFFF', 그늘: 색['Oat'], 어둡나: false },
    워터마크: true,
    워터실: 실.여린코랄,
    ...
    /* 🆕 09-08 이름 교체 (유호 지시) — 「24시간 한국어 라디오」 → 「K-LOFI 24」.
     * 까닭 = 한국어를 아직 못 읽는 몽골 학생이 옛 제목을 못 읽고, 사람들이 실제로 찾는
     * 낱말(lofi)이 없었다. 같은 이름이 썸네일(라디오배경굽기.js)·채널 설명·트랙에도 선다. */
    큰줄: 'K-LOFI 24',
    작은줄: '',
    확정: 'synklab_배너',
  },
  ...
  /* 확정 링크드인 배너. 현재 이름은 네 글자 모두 Paper 펠트이며 SYNK 실땀은 없다. */
  링크드인코랄: {
    규격: '링크드인',
    이름: '⑨ 링크드인 · Paper 펠트 + 한 줄 — 현행 로고 (09-10) · @yuhobuilds',
    설명: '09-10 승인 Paper 펠트 워드마크를 사용한다 · 업로드본 = yuhobuilds_링크드인.png',
    바탕: 색['Ink Deep'],
    글자: 색['Paper'],
    보조: 색['Stone'],
    로고: () => 구운워드마크(),
    /* 🔑 구운 누끼는 «그린» 워드마크보다 커야 한다(09-08 눈검증). 까닭 둘:
     *   ① 누끼는 글자에 꽉 맞게 잘려 여백이 없어서, 같은 폭이면 그린 판보다 작아 보인다.
     *   ② 펠트 결이 실제 크기에서 읽혀야 한다 — 190px 에서는 결이 뭉개져 그냥 흐린 글자가 됐다.
     * 그리고 문안을 68 → 52 로 줄인다. 첫 판에서 문안이 워드마크보다 커져 히어로가 뒤집혔다
     * (명품 배너 실측 = 히어로 하나). */
    로고크기: 360,
    천: { 보풀: 색['Deep Wool'], 그늘: '#000000', 어둡나: true },
    워터마크: false,
...
/** 구운 워드마크 — 현행 승인 킷의 중립색 SYNK 사진형 누끼를 소비한다.
 *  양모 바탕에는 Ink, 어두운 바탕에는 Paper 네 글자를 쓴다.
 *  2026-09-10 승인: SYNK의 실땀만 제거하고 펠트 몸체·그늘·y/k 형태는 유지한다.
 *  사업명 조합과 단독 도장은 각자의 승인 원본을 그대로 사용한다.
 */
function 구운워드마크(판 = '밤') {
  const 이름 = 판 === '양모' ? 'SYNK-Ink.png' : 'SYNK-Paper.png';
  const 경로 = path.join(ROOT, 'docs/홍보물/마케팅실행_20260909/브랜드킷', 이름);
  if (!fs.existsSync(경로)) throw new Error('구운 워드마크 누끼가 없다: ' + 경로);
...
<style>
  ...
  /* 구운 워드마크(누끼 PNG)도 같은 자리에 선다 — 09-08. svg 와 폭 규칙을 나눠 쓴다. */
  .로고 img{width:100%;height:auto;display:block;}
