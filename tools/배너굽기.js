...
const 안들 = {
  ...
  synklab커버: {
    ...
    워터마크: true,
    워터실: 실.여린코랄,
    /* ⑤에서 눈으로 맞춘 0.22 를 그대로 쓴다 — 같은 밝기의 천이라 같은 알파가 같게 읽힌다. */
    워터투명: 0.22,
    이음: true,
    몽글: true,
...
function 지면(안, 안내선) {
  ...
  const 워터 = a.워터마크
    ? `<div class="워터" style="opacity:${투명}">${잉크펠트꺾쇠(H, a.워터실 || 실.잉크)}</div>` : '';
...
<style>
  ...
  .까몽{left:${몽글우}px;bottom:${Math.round(H - (H * 이음비 + 30) - (몽글폭 * 0.573 / 0.912) * 0.057)}px;width:${Math.round(몽글폭 * 0.573 / 0.912)}px;}
...
<div class="판">
  ...
  ${천}<div class="결"></div>${이음}${워터}${몽글}${까몽}${선}
  <div class="속">
