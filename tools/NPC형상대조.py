...
def 실루엣(im, 문턱=140, 성김=2):
    """주인공(배지) 화소만 — **뒤판(밤천)은 뺀다.** 좌표 집합으로 둬야 겹침(IoU)을 잴 수 있다.

    🔴 08-24 실사고: 문턱이 60 이라 «뒤판까지» 실루엣에 들어와 있었다. 뒤판은 네 역이 똑같이
      깔고 있는 큰 판이라, 배지가 아무리 갈려도 겹침은 높게·크기비는 1 에 붙게 나온다.
      그 값으로 문턱을 걸어 **3판 본굽기를 잘못 막았다** — lead/insp 가 1.13배로 읽혔는데
      뒤판을 빼고 재니 **1.31배**였다(문턱은 1.30).
      방아쇠는 새 색관리다: PBR 중립 + 노출 +0.25 가 뒤판 Ink 를 문턱 위로 들어올렸다.
      🔑 **색을 바꾸면 잣대의 문턱도 다시 재야 한다.** 자를 고쳐 놓고 자의 눈금을 안 본 것이다.
    ⇒ 140. 문턱대를 훑어 고른 값이다 — L>100·140·170·200 에서 lead/insp 가 1.31·1.31·1.29·1.27 로
      평평하고, 60 만 홀로 1.13 으로 튄다. 그 튐이 뒤판이 섞인 자리다."""
    g = im.convert('L')
    px = g.load()
    w, h = g.size
