 ...
 * 🔑 2026-08-20 **이사** — 원래 `tools/lib/보드낡음.js(⚠삭제됨 39019553 2026-08-20 — 지금 없다)` 에 살았다. 보드 층을 걷을 때 이 함수만
 ...
 *   🔴 실측 2026-08-17(F527 분모 세기 중 발각) — **두 과녁 중 «자동으로 도는» 쪽만 멀어 있었다**:
 *     · 아래 `작업본문서들` = 기본 과녁 + `--스테이징`(**pre-commit 이 부르는 길**) … 한글 경로 .md 를
 *       8벌 건드린 작업본에서 **0/8** 을 봤다(`quotepath=false` 로 재면 8/8).
 *     · `전량문서들` = `--전량` 보고용 … `readdirSync` 로 디스크를 훑어 **원래 멀쩡했다**(A/B 확인).
 *   🔑 그래서 아무도 못 봤다 — 손으로 부르는 모드는 늘 맞는 수를 냈고, 조용히 도는 강제 모드만
 *     틀렸다. 화면에는 늘 「문서 N벌 · 새 인용 0건」이 찍혔다(맹점 ④: 맞는 얼굴로 틀린 값).
 *   📏 크기 — 면제를 뺀 과녁 168벌 중 **143벌이 비ASCII 경로**다(`docs/SYNK_철학.md` 포함).
 *   회귀 = `tests/인용검사_한글경로.test.js(⚠삭제됨 e75fc7fc 2026-08-19 — 지금 없다)`(픽스처가 한글 경로 문서를 실제로 잡는지 못박는다). */
function git(args) {
  try {
...
function 작업본문서들(스테이징) {
  ...
  /* 🔴 `--스테이징` 과녁에서는 미추적 파일을 **보지 않는다**(고침 2026-08-26).
   *
   * 옛 주석은 「`git add` 로 갓 올린 새 문서가 정확히 그 꼴」이라고 했는데 그게 틀렸다 —
   * `git add` 한 파일은 위 `--cached … HEAD` 에 `A` 로 이미 잡히고, `ls-files --others` 는
   * 정확히 **add 하지 않은** 파일만 낸다. 그래서 이 줄이 잡던 것은 «커밋에 실릴 새 문서»가
   * 아니라 **아무도 add 하지 않은 남의 미완 파일**이었다.
   *
   * 실물로 드러난 자리(08-26): 다른 세션이 쓰던 미추적 `docs/가져가는것_설계_v1.md` 의 거짓 인용
   * 1건이 **그 파일과 아무 상관 없는 세션의 커밋을 막았다**(영상 통로 커밋 · 80파일).
   * 이건 바로 위 pre-commit 훅이 스스로 금지한 상황이다 —
   *   「**이번 커밋이 새로 쓴 줄만** 본다 … 전량을 막으면 **남의 낡은 인용으로 내 커밋이 막힌다**.
   *    따를 수 없는 처방은 우회를 정상 통로로 만든다(F103)」.
   *
   * 기본 과녁(`--스테이징` 없이 사람이 부르는 길)에서는 그대로 본다 — 거기선 「작업본이 지금
   * 건드린 문서」가 과녁이라 미추적도 내 것이고, 막는 게 아니라 보여주는 자리다. */
  const b = 스테이징 ? true : 담기(git(['ls-files', '--others', '--exclude-standard']));
  return { 목록: [...모음], 잰것: a && b };
}
