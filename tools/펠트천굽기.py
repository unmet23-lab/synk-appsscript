...
def 굽는다(이름, 역할표):
    ...
    작은 = src.resize((한변, 한변), Image.LANCZOS)
    통 = io.BytesIO()
    작은.save(통, 'WEBP', quality=품질, method=6)
    통.seek(0)
    구운것 = Image.open(통)
    남은결 = 100.0 * L별(구운것).std() / 기준결 if 기준결 else 0.0

    if 남은결 < 결하한:
        return None, '결 %.1f%% < 하한 %.1f%% — 압축이 섬유를 먹었다' % (남은결, 결하한)
...
def main():
    ...
    산출 = {
        ...
        '_규격': {'한변': 한변, '형식': 'WEBP q%d' % 품질, '결하한': 결하한},
        '천': 천,
    }
