...
function 상담AI_점검() {
  ...
  시험.forEach(({ q, 기대인계 }) => {
    ...
    try {
       ...
       *   한 사건이 두 번 세어진다). */
      /* 🔑 **있나/없나가 아니라 «얼마나»로 잰다**(codex P2 655ee49e — 이 자리의 세 번째 수리).
       *   한글 한 자만 보면 몽골어 답에 「한국어」·「SYNK」 같은 낱말이 하나 섞이는 순간 초록이 된다.
       *   글자다운 글자(한글·키릴·라틴)만 남기고 그중 한글이 절반을 넘는지 본다 — 숫자·기호·공백은
       *   어느 언어에도 속하지 않으므로 분모에서 뺀다(₮·42만 이 비율을 흔들면 안 된다).
       *   0.5 인 이유: 한국어 답에도 TOPIK·SYNK 같은 라틴이 섞이지만 실측에서 늘 한글이 압도한다.
       *   ⚠ 이 자를 또 고쳐야 하면 손보지 말고 통째로 다시 본다 — 한 판정을 세 번 넘게 기우면
       *     그건 자가 잘못 서 있다는 뜻이다. */
      const 답글9 = String(r.data.reply || '');
      const 글자9 = 답글9.replace(/[^가-힣Ѐ-ӿA-Za-z]/g, '');
      const 한글수9 = (글자9.match(/[가-힣]/g) || []).length;
      const 언어어긋9 = 글자9.length > 0 && 한글수9 / 글자9.length < 0.5;
      const 키릴9 = /[Ѐ-ӿ]/.test(답글9);
      /* 🔑 **한 질문 = 한 건.** 인계와 언어에서 각각 더하면 둘 다 틀린 질문이 2건으로 세어져
      ...
      Logger.log('\n❓ ' + q + '\n💬 ' + (답글9 || '(빈 답변)') +
        ...
        (언어어긋9 ? '\n   🔴 한국어로 물었는데 한글이 한 자도 없다' + (키릴9 ? '(' + 상담_설정.기본언어 + '로 답했다)' : '') : '') +
      ...
      요약9.push((어긋9 ? '🔴 ' : '✅ ') + q
        + '\n   인계=' + 인계9 + (맞나 ? '' : ' (기대 ' + 기대인계 + ')')
        + (언어어긋9 ? '\n   🔴 한국어로 물었는데 한글이 한 자도 없다' + (키릴9 ? ' — ' + 상담_설정.기본언어 + '로 답했다(【말투】 규칙 위반)' : '') : '')
   ...
   *   합계가 아니라 갈래를 읽는다(잰 것 = 시험 − 막힘). */
  const 잰것9 = 시험.length - 막힘9;
  const 판정9 = 어긋남9
    ? '🔴 기대와 어긋난 자리 ' + 어긋남9 + '건 — 지식·금칙·프롬프트 중 하나가 어긋났다'
    : (막힘9
      ? '⛔ ' + 잰것9 + '/' + 시험.length + '건만 쟀다 — 나머지 ' + 막힘9 + '건은 진단 상한에 걸려 «안 물어봤다»(통과가 아니다)'
