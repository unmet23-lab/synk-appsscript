...
function 상담AI_점검() {
  ...
  시험.forEach(({ q, 기대인계 }) => {
    ...
    try {
       ...
       * 이 표시가 없던 08-28 실측에서 몽골어 답이 ✅ 로 지나갔다: 인계 여부만 보는 자는
       * 「엉뚱한 언어로 답한 것」을 원리상 못 본다 — 자가 안 보는 자리는 언제나 통과로 보인다. */
      const 키릴9 = /[Ѐ-ӿ]/.test(String(r.data.reply || ''));
      if (키릴9) 어긋남9 += 1;
      요약9.push((맞나 && !키릴9 ? '✅ ' : '🔴 ') + q
        + '\n   인계=' + 인계9 + (맞나 ? '' : ' (기대 ' + 기대인계 + ')')
        + (키릴9 ? '\n   🔴 한국어로 물었는데 ' + 상담_설정.기본언어 + '로 답했다 — 【말투】 규칙 위반' : '')
        + '\n   답: ' + (r.data.reply ? String(r.data.reply).slice(0, 220) + (String(r.data.reply).length > 220 ? '…' : '') : '(빈 답변)'));
    } catch (e) {
      어긋남9 += 1;
      Logger.log('\n❓ ' + q + '\n❌ 오류: ' + e.message);
