...
/* ══════════════════════════════════════════════════════════
 * 3층 — 벤더 중립 AI 어댑터
 * [2026-09-11 재대조] 이 저장소에서 api.anthropic.com을 직접 부르는 곳은 **9군데**다 — 상담_호출_·
 * 상담_인계초안_·상담_역번역_·callClaudeFeedback_·aiCall_·aiText_·callClaudeTalk_·두뇌_AI_anthropic_·aiConnectionCheck.
 * 이 호출 자리에서 옛글자 게이트도 함께 확인한다. ⚠개수를 못박던 `tests/옛글자런타임.test.js` 는 08-19
 * 대철거(e75fc7fc)로 걷혔다 — 이 줄이 다섯 달 동안 없는 검사를 가리키고 있었다(심문 G8 · 08-24 정정).
 * 새 호출 자리를 추가하면 그 커밋에서 이 목록과 게이트 유무를 대조한다.
 * 두뇌는 그 길을 늘리지 않고 여기 하나만 쓴다 — 나중에 GPT·음성 API를 붙일 때 고칠 곳이 한 곳이 되도록.
 *
 * ─── [T4 · 2026-08-29] «프롬프트 캐싱» 추정 기록 — 아래 글자 수·토큰 환산은 당시 값이다 ───
 * 09-11 코드상 세 자리(상담_호출_·상담_인계초안_·이 함수)에 `cache_control` 이 있고 나머지 여섯에는 없다.
 * 아래는 그중 **배치 넷에 붙이지 않았던 당시 근거**다(상담_역번역_의 토큰 수는 여기서 재지 않았다):
 *   Opus 5 의 최소 캐시 프리픽스 = **512 토큰**(정본 = `claude-api` 스킬 shared/prompt-caching.md 표 ·
 *   모델별로 다르다 · 코드가 읽는 자리 = 엔진_콘텐츠AI.js `AI_캐시최소토큰_`). 그 선을 못 넘으면
 *   `cache_control` 을 붙여도 캐시가 **조용히 안 만들어진다** — 오류도 증상도 없고 청구서만 그대로다.
 *   그래서 붙여 놓고 「캐싱 적용 완료」라 적는 것이 이 저장소가 제일 싫어하는 무늬가 된다.
 *
 * 실측(글자 수 → 토큰 «추정». 환산비 0.649토큰/자 = 형제 저장소 실측 6,761자→4,385토큰에서 왔다):
 *   · `callClaudeTalk_`      TALK_SYSTEM_PROMPT   378자 ≈ 245토큰 (선의 48%)
 *   · `callClaudeFeedback_`  FB_SYSTEM_PROMPT     405자 ≈ 263토큰 (51%)
 *   · `aiCall_`              가장 긴 소비자(개인화 튜터) 484자 ≈ 314토큰 (61%) · 나머지는 48~176토큰
 *   · `aiText_`              시스템 41자 ≈ 27토큰 (5%)
 *   ⇒ 넷 다 선의 «절반 안팎». 프롬프트를 늘리지 않는 한 캐싱은 켜지지 않는다.
 *
 * 🔑 당시 판단은 프롬프트 크기를 근거로 했다. 이 기록만으로 현재 캐시의 동작·효과를 확정하지 않는다.
 *   당시 대조군을 같은 자로 재면: `상담_시스템_()` = 6,169자 ≈ **4,004토큰(선의 782%)**
 *   — 당시 지식 33항목·금칙 10조를 반복해 싣는 자리였다. 실제 캐시 적중은 응답의 usage로 확인해야 한다.
 *   (`두뇌_AI_` 의 `req.system` 은 **시트에서 읽어 오는 지식**이라 정적으로 못 쟀다 = 「안 재봤다」.
 *    상한이 60,000자니 선은 넘을 것으로 «보이나» 지식이 비면 안 넘는다.)
 *
 * 🔴 이 수는 **토크나이저로 잰 것이 아니라 글자 수 환산 추정**이다 — 「안 재봤다」에 가깝다.
 *   그래서 진짜 수를 응답이 말하게 했다: `AI사용_기록_`(엔진_콘텐츠AI.js)이 네 자리의 `usage` 를
 *   모으고, 평균 입력이 선을 넘는 날 **저절로 메일이 온다**(이 판정이 뒤집히는 자리를 사람 암기에
 *   안 맡긴다). 학생이 0명인 지금은 그 배치가 안 돌아 장부도 비어 있다 — 그 0은 「안 샌다」가 아니라
 *   「아직 안 불렀다」다.
 * ⚠ 모델을 바꾸면 이 판정 전체가 바뀐다 — Opus 4.6 은 같은 선이 **4096** 이라, 4,004토큰인 상담AI 조차
 *   그날 아슬아슬하게 선 아래로 떨어져 «걸려 있는데 안 도는» 캐시가 된다(증상 0).
 *   `AI_FEEDBACK_MODEL`(Code.js)·`상담AI_모델_()` 을 만지는 손이 이 문단을 같이 읽는다.
 * ══════════════════════════════════════════════════════════ */
