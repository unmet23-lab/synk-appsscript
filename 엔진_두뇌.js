 ...
 *
 * ─── [T4 · 2026-08-29] 그 일곱의 «프롬프트 캐싱» 실측 — 여기가 이 판정의 유일한 정본이다 ───
 * 세 자리(상담_호출_·상담_인계보조·이 함수)에만 `cache_control` 이 걸려 있고 넷은 안 걸려 있는데,
 * **넷에 붙이지 않았다.** 붙일 수 없어서다:
 ...
 *
 * 🔑 그래서 **지금 상태는 우연이 아니라 맞는 상태다** — 걸린 셋은 프롬프트가 큰 자리고, 안 걸린 넷은
 *   작은 자리다. 대조군으로 같은 자로 재 보면: `상담_시스템_()` = 6,169자 ≈ **4,004토큰(선의 782%)**
 *   — 지식 33항목·금칙 10조를 매번 통째로 싣고 세션마다 같으니, 그 자리의 `cache_control` 은 실제로 일한다.
