...
function 킷추출() {
  ...
  // ⏏ 단 `선택` 앵커는 예외다: 유호님이 「나중에 별로면 쉽게 빼게」 하신 조항이라, 그 줄을 지우는 것이
  //    **CI 를 빨갛게 만들면 안 뺀다**. 없으면 그냥 안 싣는다(빼기 = DESIGN.md 그 한 줄 삭제).
  const 선택 = new Set(['정적4줄']);
  조각.죽은앵커 = Object.entries(조각)
    .filter(([k, v]) => k !== '죽은앵커' && !v && !선택.has(k)).map(([k]) => k);
  return 조각;
}
...
function 메시지(filePath) {
  ...
  const 킷 = [k.삼규칙, k.이모지, '', k.철칙, '', k.감각, k.정적4줄]
