#!/usr/bin/env node
// SYNK 상태줄 — 명령창(터미널)에서 claude 를 띄웠을 때 화면 아래에 붙는 두 줄.
//
// 🔴 **유호님 화면에는 이게 안 뜬다**(09-03 · 유호님 화면 사진으로 확정).
//    데스크탑 앱 Code 탭은 상태줄을 그리는 자리가 아예 없고, 대신 제 바를 그린다:
//      입력칸 위  = `synk-appsscript master` + `+78,316 -278`
//      입력칸 아래 = `권한 무시` · `Opus 5` · `최대`
//    그래서 모델·생각깊이·저장소·가지·변경량은 **앱이 이미 보여준다** — 여기 것은 덤이다.
//    앱 바에 «없는» 것(맥락·재활용)은 .claude/hooks/context-nudge.js 가 훅 알림으로 낸다.
//    이 파일은 명령창을 쓸 때를 위해 남긴다. 유호님이 보시는 자리를 여기로 «착각하지 말 것».
//
