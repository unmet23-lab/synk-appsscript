...
// 블록 지시는 **공용 통로 하나**에서 온다 — context-budget 도 같은 자리에서 300k 에 울어,
// 서로를 모른 채 각자 지시하니 한 세션에 블록이 두 번 나갔다(F122).
const 지시 = report.blockOrder(cwd, sid, 인계문);
...
const 본문 =
  ...
  + `    새 창엔 자동 입력되고, 안 닿는 자리(다른 계정·폰)에선 유호님이 그 블록을 붙여넣는다.\n`
