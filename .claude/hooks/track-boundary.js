...
const 파일 = report.writeHandoffFile(cwd, 인계문, { sessionId: report.hostSessionId(sid), reason: '트랙 경계' });
...
const 본문 =
  ...
  + `    이때 **아래 인계문 전문을 코드 블록으로 화면에 그대로 옮긴다** — 파일 경로나 「자동\n`
  + `    입력된다」 안내로 **대체 금지**(복사할 실물이 화면에 있어야 한다 — 08-04 실측: 실물 없는\n`
  + `    안내는 유호님에게 「안 된다」로 보였다). 새 창엔 자동 입력되고, 안 닿는 자리(다른 계정·폰)\n`
  + `    에선 유호님이 그 블록을 붙여넣는다. 이 발화 뒤 커밋·보드가 더 진행됐으면\n`
  + `    \`node tools/인계문.js\` 로 새로 뽑아 그걸 붙인다.\n`
