...
if (wake) {
  out.hookSpecificOutput = {
    ...
    additionalContext: [
      ...
      '④ `node tools/인계문.js` 를 돌려 stdout **전문을 코드 블록으로 보고 맨 끝에 붙인다** — 그게 유호님이 새 세션에 붙여넣을 지시문구다. 파일 경로나 「자동 입력된다」 안내로 **대체하지 않는다**(복사할 실물이 화면에 있어야 한다 — 08-04 실측: 실물 없는 안내는 유호님에게 「안 된다」로 보였다).',
      '보고는 짧게 하되 마지막은 반드시 ④의 인계문 블록 + 「이 창을 닫고 새로 여세요 — 새 창에 자동 입력되고, 안 되면(다른 계정·폰) 위 블록을 복사해 붙이면 됩니다」로 끝낸다. 새 작업을 시작하지 않는다 — 끊을지 말지는 유호님이 정한다.',
    ].join('\n'),
  };
}
