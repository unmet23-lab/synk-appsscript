...
if (wake) {
  ...
  out.hookSpecificOutput = {
    ...
    additionalContext: [
      ...
      `④ ${지시.본문}`,
      /* ⚠ 「첫 메시지로 자동 입력된다」로 되돌리지 않는다 (F661 ㈐) — 인계문은 이제 유호님의 첫 발화
       *   자리를 안 쓰고 **배경 맥락**으로 들어간다. 그래서 자동 출발이 없어졌고, 옛 동작을 얻는
       *   한 마디(「이어서」)를 같이 줘야 한다. 안 주면 유호님은 창을 열고 아무 일도 안 일어나는
       *   것을 보게 된다(따를 수 없는 처방 · F103). */
      '보고는 짧게 하되 마지막은 **인계문 블록 하나 + 「이 창을 닫고 새로 여세요 — 새 창은 이 인계문을 배경으로 알고 시작합니다. 용건이 있으시면 그냥 말씀하시고, 그대로 이어가시려면 「이어서」 한 마디면 됩니다(다른 계정·폰이면 위 블록을 복사해 붙이세요)」**로 끝낸다(블록은 세션당 하나 · ④ 참조). 새 작업을 시작하지 않는다 — 끊을지 말지는 유호님이 정한다.',
    ].join('\n'),
  };
}
