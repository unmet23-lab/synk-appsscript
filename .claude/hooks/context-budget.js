...
/* ⚠ 「이 **아래** 블록」이라고 못 박지 않는다 — `blockOrder` 표식(F122)을 track-boundary 가 먼저
 *   태웠으면 이 발화의 additionalContext 엔 전문이 안 실리고, AI 는 **앞선 답변에서 이미 낸 블록**을
 *   재사용한다(그게 「세션당 한 블록」의 뜻이다). 그때 「아래」는 가리킬 것이 없는 거짓 안내가 된다. */
const 인계칸 = wake
  ? '📋 인계문 전문은 **이 세션 답변의 코드 블록**으로 나간다 — 여기 겹쳐 싣지 않는다'
    + '(같은 글이 두 벌이면 어느 쪽을 복사할지가 사라진다 · 이미 위에 나왔으면 그것이다).\n'
    + '   보조 사본 `docs/_ops/인계문.md` · 새 창·`/clear` 는 **자동 입력**이라 복붙 자체가 불필요하다.'
  : report.frame(handoffMsg)
    + '\n💡 창을 새로 열거나 `/clear` 하면 위 문구가 **자동으로 입력**된다(복붙 불필요 · 수동 재출력 `node tools/인계문.js --no-save`).';
