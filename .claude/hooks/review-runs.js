...
function main() {
  ...
  if (s.멈춤.length) {
    ...
    for (const x of s.멈춤) {
      ...
      /* 🔴 F511 — 여기가 **맞는 얼굴로 틀린 값**을 내던 자리다. 옛 판은 `codex-review.js` 를
       *   박아 놓고 `(x.런.인자 || [])` 로 관대하게 접어, 인자 칸이 없는 사실심문 런에서
       *   `이어받기: node tools/codex-review.js ` (빈 인자)를 냈다. 그 줄을 붙여 넣으면
       *   이어받는 대신 **기본 검수가 새로 돈다.** 명령은 종류가 정한다 — 통로 하나에서 받는다. */
      줄.push(런.이어받기줄(x.런, '     '));
    }
  }
