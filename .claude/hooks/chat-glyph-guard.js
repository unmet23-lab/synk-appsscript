...
//
// 옛글자 정의는 tests/문서문자.test.js 와 **같은 값**이어야 한다. 훅이 테스트 파일을 require 하면
//   안 되므로 정의는 `.claude/hooks/lib/옛글자.js` 에 두고 **훅들은 거기서 가져온다**
//   (2026-08-09 F298: 쓰는 순간을 막는 board-guard 가 셋째 소비자로 붙어 사본을 접었다).
//   [단일 출처] 회귀(tests/채팅글자가드.test.js)가 lib 와 문서문자.test.js 의 문자 클래스를 대조한다.
//   ⚠ 범위 경계를 글자로 적지 않는다 — 적으면 문서문자 스캔이 이 파일을 잡는다(그쪽 머리말과 같은 규율).
//
...
const { 옛글자 } = require(path.join(__dirname, 'lib', '옛글자.js'));
