 ...
 *     ⚠ **세션은 이 변수를 못 쓴다**(실측 08-15 · F103). `Bash` 한 호출에서 `export` 한 값은 다음
 *     호출에서 빈값이고(셸 상태 비영속), 이 훅은 세션의 셸이 아니라 Claude Code 가 띄우므로 그 env 가
 *     닿는 경로 자체가 없다. `settings.json` 의 `env` 는 **저장소 전체 공유**라 켜면 남의 세션 자동커밋도
 *     함께 꺼진다. 즉 이건 사람(유호님)의 환경 스위치지 「트랙 여는 동안 세션이 잠그는 자물쇠」가 아니다 —
 *     F342 가 처방으로 적었던 그 형태는 **실행 불가능한 처방**이었고, 그래서 F248·F444 가 반복됐다.
 *     지금 그 자리를 대신 지키는 것은 아래 「그 초록은 무엇을 잰 초록인가」 게이트다(자동·선언 불필요).
 *
...
function 한저장소(뿌리, 이름, 내touched, 남touched, 지문) {
  ...
  /* ── 🔴 **그 초록은 무엇을 잰 초록인가** (마찰 F248+F444 한 트랙 처분 · 2026-08-15) ──────
   * 바로 위 게이트는 실을 테스트를 **작업본** 상태로 돌린다. 그런데 커밋에 실리는 것은 `후보`
   * 뿐이다 — 작업본에는 있는데 이 커밋에서 빠지는 내용(아래 네 버킷)이 있으면, 그 초록은
   * 「지금 디스크에 다 있다」를 잰 것이지 **커밋 뒤 HEAD 가 서는가**를 잰 것이 아니다.
   *
   * 실측 재현 08-15(픽스처): 짝 `구현.js` 를 무기록으로 빼고 그 짝에 기대는 새 테스트를 두면,
   *   작업본 초록 → 테스트만 착지 → **커밋 뒤 HEAD 적색**. 훅은 그때도 「구현.js 는 안 실었다」고
   *   말하고 있었다 — 뺀 사실은 알면서 그래서 자기 초록이 증거가 아니라는 것만 연결이 없었다.
   *   이것이 F248(08-08 master 60분 적색)·F444 2번째(08-15 `bea5da2f` — 남의 /deploy 게이트 차단)의
   *   실제 통로다. 새는 방향은 침묵이 아니라 **남에게 전가된 적색**이라 이 훅의 다른 누수보다 나쁘다.
   *
   * 🔑 새 원칙이 아니다 — 이 훅이 이미 적어 둔 **「모름은 커밋하지 않는다」** 를 빠뜨린 자리에
   *   적용할 뿐이다. 빠지는 내용이 있으면 커밋 뒤 판이 서는지는 **모름**이다.
   * 🔑 대상은 **테스트 파일뿐**이다 — 나머지는 그대로 싣는다(적색 게이트와 같은 「떨어뜨리기」).
   *   통째로 보류하면 F025(미커밋 무보호) 쪽으로 사고가 옮겨갈 뿐이다.
   * ⚠ 적색·미측정은 방아쇠에 **안 넣는다** — 그건 「내용이 빠진다」가 아니라 이미 떨어뜨린 테스트다.
   *   🔑 넣었을 때의 값을 **실측했다**(변이 ⑤ · 08-15): 지금 판에서는 **아무 것도 안 바뀐다** —
   *   위 게이트가 실을 테스트를 «한 배치»로 떨어뜨리므로, 적색·미측정이 차 있으면 `후보` 에 테스트
   *   파일이 애초에 남지 않아 이 가지가 도달 불가다(동치 변이 · 회귀가 못 잡는 게 정상이다).
   *   그래도 안 넣는 이유는 **그 배치 성질이 파일 단위로 쪼개지는 날** 곧바로 거짓양성이 되기
   *   때문이다 — 적색 하나가 옆의 초록 테스트를 영영 안 실리게 만든다(거짓양성이 곧 무시다).
   * ⚠ 대가: 테스트와 **무관한** 파일 하나가 빠졌을 뿐인 판에서도 그 턴엔 테스트가 안 실린다 —
   *   F025 노출이 한 턴 이어진다. 대신 조용하지 않다(아래 처방 문구가 손 커밋 명령을 그대로 준다). */
  const 증거없음 = [];
  const 안실리는내용 = 함께남김.length + 깨짐.length + 무기록.length + 밖에서바뀜.length;
  if (안실리는내용) {
    for (const f of 후보.filter((f) => /(^|\/)tests\/[^/]*\.test\.js$/.test(f))) {
      증거없음.push(f);
      const i = 후보.indexOf(f); if (i >= 0) 후보.splice(i, 1);
    }
  }

  const 뺀것있나 = 함께남김.length || 깨짐.length || 무기록.length || 밖에서바뀜.length || 적색.length || 미측정.length || 증거없음.length;
  if (!후보.length) return 뺀것있나 ? { 이름, 후보: [], 함께남김, 깨짐, 무기록, 밖에서바뀜, 적색, 미측정, 증거없음 } : null;
  ...
  return { 이름, 후보, 함께남김, 깨짐, 무기록, 밖에서바뀜, 적색, 미측정, 증거없음, 해시: (gitOk(['rev-parse', '--short', 'HEAD'], 뿌리) || '').trim(), 미완, 머리 };
}
...
for (const r of 결과들) {
  ...
  /* F248+F444: 초록은 분모와 함께만 읽는다 — 안 실리는 내용이 남았으면 그 초록은 HEAD 의 증거가 아니다. */
  if (r.증거없음 && r.증거없음.length) 줄.push(`   🔴 ${딱지}테스트 ${r.증거없음.length}건은 **초록이지만 안 실었다** — 이 커밋에 안 실리는 내용이 작업본에 남아(위 ⚠들) 그 초록은 «커밋 뒤 HEAD 가 서는가»의 증거가 아니다(F248·F444 — 반쪽이 착지하면 master 가 빨개지고 남의 /deploy 가 막힌다): ${r.증거없음.join(', ')} — 짝까지 한 벌로 모아 손 커밋: git commit -m "..." -- ${r.증거없음.join(' ')} <짝 경로>`);
}
