...
if (resulting !== null && 이전 !== null) {
  ...
  if (남의선언) {
    ...
    const 가지판 = (() => {
      ...
      try {
        return 보드lib.가지줄들(보드ROOT, typeof 주입 === 'string' ? { 출력: 주입 } : undefined);
      } catch (_) { return { 줄들: [], 모름: true }; }
    })();
    const 가지출처 = new Map();          // 줄 → 가지 이름 (처방이 «어디를 보라»고 말하려면 필요하다)
    const 가지선언 = [];
    {
      const 본것 = new Set(옛줄.map((l) => String(l).trim()));
      for (const r of 가지판.줄들) {
        if (r.파일 === base) continue;                    // 내 파일의 가지판은 내 줄이다
        if (!isActiveRow(r.줄)) continue;                 // 완료·대기 줄은 자리를 안 잡는다
        const 키 = String(r.줄).trim();
        if (본것.has(키)) continue;                        // master 에 이미 든 줄을 두 번 세지 않는다
        본것.add(키);
        가지출처.set(키, (r.출처 && r.출처.가지) || '');
        가지선언.push(r.줄);
      }
    }
    if (가지판.모름) {
