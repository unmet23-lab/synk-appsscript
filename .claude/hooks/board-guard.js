...
if (resulting !== null && 이전 !== null) {
  ...
  if (남의선언) {
    ...
    const 가지판 = (() => {
      ...
      /* F628 — 원격 층에도 **자기 주입구**가 있어야 한다. 가지 주입구 하나만 두면 회귀는 늘
       *   「원격 모름」 상태로만 돌아서, 이 훅의 원격 갈래가 **한 번도 안 도는 채로 초록**이 된다
       *   (장치와 그 발동 조건은 같은 커밋에서 정한다 · CLAUDE.md 신뢰성). */
      const 원격주입 = process.env.SYNK_BOARD_REMOTE_LOG;
      try {
        return 보드lib.활성줄전량(보드ROOT, {
          메인: 옛줄.map((l) => ({ 줄: l, 파일: base })),
          가지제외파일: base,                              // 내 파일의 가지판은 내 줄이다(접기 «전»에 뺀다)
          ...(typeof 주입 === 'string' ? { 출력: 주입 } : {}),
          ...(typeof 원격주입 === 'string' ? { 원격출력: 원격주입 } : {}),
        });
      } catch (_) { return { 줄들: [], 모름: true, 못본층: ['미머지 가지', '원격 master'] }; }
    })();
    const 가지출처 = new Map();          // 줄 → 가지 이름 (처방이 «어디를 보라»고 말하려면 필요하다)
    const 가지선언 = [];
    /* 🔴 F628 — **세 번째 층**: 이미 `origin/master` 에 있는데 이 트리가 아직 안 받은 선언.
     *   옛 판은 `if (!r.가지) continue` 로 그 줄을 통째로 버렸다(원격 줄엔 가지 이름이 없다).
     *   즉 정본이 그 층을 실어 보내도 **이 훅에서 다시 사각이 됐다** — 새는 방향은 「통과」다.
     * 🔑 가지와 **갈라 둔다**: 처방이 정반대다. 가지 줄은 「아직 안 머지됐다 · `gh pr list`」,
     *   이 줄은 「이미 머지됐는데 내가 안 받았다 · `git merge --ff-only origin/master`」다(F542). */
    const 원격출처 = new Map();          // 줄 → ref 이름
    const 원격선언 = [];
    for (const r of 가지판.줄들) {
      if (r.원격 && !r.가지) { 원격출처.set(String(r.줄).trim(), r.원격); 원격선언.push(r.줄); continue; }
    ...
    if ((가지판.못본층 || []).includes('원격 master')) {
      셈설명.push('`origin/master` 쪽 선언을 **못 훑었다** — 「거기 선언 0」이 아니라 **모름**이다(F207·F628).'
        + ' 이 트리가 뒤처져 있으면 남이 master 에 직접 올린 선언은 그만큼 안 보인다.');
    }
    ...
    let 원격겹침 = false;                                 // F628 — 처방이 또 갈린다(`git merge --ff-only`)
    for (const 새 of 새줄) {
      const 내자리 = 자리들(새);
      if (!내자리.size) continue;
      for (const 남 of 남의선언.concat(가지선언, 원격선언)) { // F528 가지 · F628 origin/master
        ...
        /* F628 — 원격 줄도 이 파일 밖에서 왔다. F291 면제를 주면 「내 파일을 쓸 때」마다 이 층이
         *   통째로 조용해지고, 그게 실사용의 기본 모양이라 사실상 «안 도는 장치»가 된다(맹점 ④). */
        const 원격줄인가 = 원격출처.has(String(남).trim());
        const 밖에서온줄 = 가지줄인가 || 원격줄인가;
        ...
        if (!밖에서온줄 && 내파일인가 && 내파일줄) continue; // F291 — 내 파일에 있으면 내 줄이다
        ...
        const 통 = 놓은줄(남) ? 놓은자리겹침 : (죽은주인인가(남, 밖에서온줄) ? 죽은자리겹침 : 겹침);
        if (!밖에서온줄 && 내파일줄) 자기파일겹침 = true;
        if (가지줄인가) 가지겹침 = true;
        if (원격줄인가) 원격겹침 = true;
        const 출처표 = 가지줄인가 ? ` · 가지 \`${가지출처.get(String(남).trim()) || '?'}\``
          : (원격줄인가 ? ` · \`${원격출처.get(String(남).trim()) || 'origin/master'}\` (내 트리가 아직 안 받았다)` : '');
      ...
      deny(
        '[board-guard] 이 자리는 **남이 이미 선언했다** — 보드의 활성 줄이다(커밋 여부는 안 가른다 · F161·F203):\n- ' +
          ...
          (가지겹침
            ...
            : '') +
          /* F628 — 원격 줄의 처방은 가지와 **정반대**다. 그 트랙은 이미 머지됐고, 안 보이던 이유는
           * 내 트리가 뒤처져서다. 「PR 을 닫아라」를 내면 없는 PR 을 뒤지게 된다(따를 수 없는 처방 · F103). */
          (원격겹침
            ? '\n   · **`origin/master` 에 이미 있는 선언이다** — 아직 안 머지된 가지가 아니라 **내 트리가 뒤처진 것**이다.' +
              '\n     받아서 직접 읽는다: `git fetch origin master && git merge --ff-only origin/master`' +
              '\n     (받고 나면 그 줄은 메인 보드에 선다 — 그때 주인의 생사·🎫 를 그 줄에서 본다).'
            : '') +
