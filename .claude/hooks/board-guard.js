...
/* ── 🕰 이 겹침 판정이 «낡은 보드»로 나왔나 (F631 정정 · 2026-08-18) ────────────────
 *
 * 🔴 실측이 내 첫 진단을 뒤집었다. #Q115 를 두 세션이 19분 차로 각자 선언해 둘 다 끝까지 갔을 때
 *   (F631) 나는 「생사를 못 재서 가드가 조용히 통과했다」고 신고문에 적었다. **틀렸다** —
 *   그 자리에서 재 보니 `생사쟀나` 는 **true** 였다. 진짜 구멍은 한 칸 앞이다:
 *   **`남의줄` 은 «작업본»의 보드 파일을 읽는다.** 즉 19분 전에 origin 으로 올라간 남의 선언은,
 *   내 체크아웃이 그것을 안 받았으면 **원리상 안 보인다.** 겹침 0 이 나온 게 아니라 **볼 것이 없었다.**
 *
 * 🔑 그리고 그 둘은 화면에서 **같은 모양**이다 — 「겹치는 줄 없음(조용함)」. 이 저장소가 매번
 *   당하는 그 형태다(통과와 미실행이 같은 모양이면 안 된다). 그래서 판정을 못 고치더라도
 *   **판정이 무엇을 못 봤는지는 말한다.**
 *
 * 🔑 재는 것은 `repo-staleness.js` 의 `재다()` 하나다 — 여기서 다시 조립하면 그 순간 갈라진다
 *   (그 훅이 이미 `docs/_ops/보드` 만 겨눈 `보드변경` 을 낸다 · F192 가 세운 자리).
 * ⚠ `fetch:false` 다 — 매 편집에 네트워크를 물리면 그게 곧 꺼지는 이유가 된다. 그래서 이 판정은
 *   **마지막으로 fetch 한 origin ref 기준**이고, 그 갱신은 같은 매처의 `repo-staleness` 가
 *   스로틀을 걸고 따로 한다. 즉 여기 숫자는 «작게» 나올 수 있다 — 안전한 방향은 아니지만
 *   0 을 「최신」이라 **단정하지 않는 것**으로 갚는다(아래 세 갈래).
 * ⚠ 대가: **표의 데이터 줄이 바뀌는 편집에서만** 잰다(실측 23ms). 늘 재면 편집마다 git 을 부른다.
 *   ⚠ 「새 선언」만이 아니다 — 상태 칸만 고쳐도 줄 «텍스트»가 달라져 새 줄로 세어진다.
 *   좁히려다 진짜 선언을 놓치는 쪽보다 낫다고 봤다(머리글·주석만 고치는 편집은 안 잰다).
 *   틀릴 때의 모습 = **거짓 안심**(ref 가 낡아 「보드 0번 바뀜」이 나온다) — 그래서 «0» 도
 *   말없이 넘기지 않고 「이 판정은 로컬 ref 기준」임을 세 갈래 중 하나로 항상 남긴다.
 * 👉 닫는 것 = **없다.** 새 경보이고, 무의미해지는 옛 검사도 없다. */
let _낡음쟀나 = false;
let _낡음 = null;
function 보드낡음() {
  if (_낡음쟀나) return _낡음;
  _낡음쟀나 = true;
  try { _낡음 = require(path.join(__dirname, 'repo-staleness.js')).재다(보드ROOT, { fetch: false }); }
  catch (_) { _낡음 = null; }   // 못 쟀다 — 「최신」으로 접지 않는다
  return _낡음;
}

/** 겹침 판정이 무엇을 못 봤나 → 사람이 읽을 한 줄(없으면 null). 세 갈래를 **다르게** 낸다. */
function 낡은판정줄() {
  const r = 보드낡음();
  if (r === null) {
    return 'origin/master 를 **못 읽었다** — 이 겹침 판정이 낡은 보드로 났는지조차 «모른다»(같다는 뜻이 아니다).';
  }
  if (!r.뒤) return null;                       // 로컬 ref 기준 최신 — 판정을 믿을 수 있다
  if (r.보드변경 === null || r.보드변경 === undefined) {
    return `origin 보다 **${r.뒤} 커밋 뒤**인데, 그 사이 보드가 바뀌었는지는 **못 쟀다**.`;
  }
  if (!r.보드변경) return null;                  // 뒤처졌지만 보드는 안 바뀜 — 이 판정엔 영향 없다
  return `origin 보다 **${r.뒤} 커밋 뒤**이고 그 사이 **보드가 ${r.보드변경}번 바뀌었다**`
    + `${r.나이분 ? ` (${r.나이분}분 전 판)` : ''} — 그 선언들은 이 판정에 **안 들어왔다**.`;
}

/* ⚠ 판정 본체는 `물려받은` 선언 **뒤**에 산다(아래 「⑧ 판정」) — 여기서 부르면 TDZ 로 훅이
 *   통째로 죽고, **죽은 훅은 조용히 통과로 읽힌다**(F239 가 같은 자리에서 실제로 그랬다). */
function 트랙충돌(물려받은, 미룬deny, 낡은판정) {
  ...
  /* 🕰 **겹침이 0 이어도** 낸다 — 「겹치는 줄 없음」과 「볼 보드가 낡았다」가 화면에서 같은
   *   모양이면 안 된다. 그게 F631 이 난 자리다(위 블록). 새 줄을 선언할 때만 잰다. */
  if (새줄.length && 낡은판정) { const 말 = 낡은판정줄(); if (말) 낡은판정.push(말); }
  for (const 내줄 of 새줄) {
...
function 자리들(line) {
  ...
  /* 대기열 영구 ID(`#Q49`) — 유호 채택 08-14. 규약이 「집으면 트랙 칸에 그 #Q 를 적는다」이므로
   * 표식 `머리` 와 달리 **선언 칸 어디서든** 선점이다(괄호 속이 관습 표기다 — 「대기열 P1 줄36」 처럼).
   * 🚫 스팬·비켜남 구절은 `금지스팬빼기` 가 먼저 떼니 「🚫#Q49 안 집는다」는 자리가 아니다. */
  for (const q of 보드lib.큐표식(`${금지스팬빼기(트랙칸(line))}\n${파일}`)) 자리.add(q);
...
/* ⚠ **이 조건이 곧 발화 스위치다** — 새 알림 배열을 여기 안 실으면 그 장치는 있어도 안 돈다
 *   (CLAUDE.md: 장치와 그 발동 조건은 같은 커밋에서 정한다). 🕰 를 넣을 때 실제로 빠뜨렸고
 *   회귀가 그 자리에서 잡았다 — 그래서 이 주석을 여기 남긴다. */
if (남의긴칸.length || 물려받은.length || 놓은자리겹침.length || 죽은자리겹침.length || 셈설명.length || 낡은판정.length) {
  const 조각 = [];
  ...
  /* 🕰 이 판정이 «낡은 보드»로 났다 — **막지 않지만 말은 한다** (F631 정정 · 2026-08-18).
   * 「겹치는 줄 없음(조용함)」과 「볼 보드가 낡아서 안 보였음」이 화면에서 같은 모양이면 안 된다.
   * 실측: 두 세션이 19분 차로 같은 대기열 줄을 각자 선언해 둘 다 끝까지 갔고, 뒤에 선언한 쪽의
   * 체크아웃에는 앞 선언이 **아직 안 와 있었다**. 가드는 조용했고, 그 침묵은 「안 겹친다」로 읽혔다. */
  if (낡은판정.length) {
    조각.push('🕰 [board-guard] 위 겹침 판정은 **낡은 보드**로 냈다 — 「안 겹친다」가 아니라 「못 봤다」일 수 있다(F631):\n- '
      + [...new Set(낡은판정)].join('\n- ')
      + '\n→ 선언을 굳히기 전에 한 번 당긴다:  git fetch origin master && git merge origin/master'
      + '\n   그 뒤 `node tools/board.js` 로 남의 활성 줄을 다시 본다 — 겹치면 **비켜나 다른 것을 고른다**.'
      + '\n   · 이 숫자는 **마지막으로 fetch 한 origin ref** 기준이라 «작게» 나올 수 있다(매 편집에 네트워크를 안 문다).'
      + '\n   · 막지 않는다 — 뒤처짐은 차단 사유가 아니고, 막으면 클라우드 세션이 아무것도 못 한다(F192 와 같은 규율).');
  }
