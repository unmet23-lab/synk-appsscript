...
function 동승본문() {
  ...
  return `[track-collision] 내가 만지려는 \`${동승경고.파일}\` 을 **같은 트리의 다른 세션이 지금 들고 있다.**\n`
    + `${줄.join('\n')}\n\n`
    + '왜 알리나: 커밋이 아니라 **살아있는 편집**이라 ①②③(남의 커밋 축)에 원리상 안 걸리고,\n'
    ...
    + '   2) 이미 편집했으면 **내 것만 걷어낸다** — 그대로 두면 내 바이트가 남의 커밋에 실려 나간다(F073).\n'
    + '   3) 다른 트랙이면 그대로 진행하되, 커밋은 경로를 못 박아 한다: git commit -m "..." -- 경로\n'
    + '   누가 무엇을 들었는지 전량: node tools/작업본소유자.js';
}
function 동승요약() {
  if (!동승경고) return '';
  return `⚠ ${동승경고.파일} 을 다른 세션이 지금 편집 중이다 — `
    + 동승경고.세션.map((s) => 동승경고.짧게(s.sid)).join(', ');
}
