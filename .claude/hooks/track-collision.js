...
function 내보드줄() {
  ...
  for (const 줄 of 본문.split(/\r?\n/)) {
    ...
    /* 동점이면 **뒤에 있는 줄**이 이긴다. 세션은 자기 줄을 보드 끝에 붙이므로 뒤쪽이 더 최근이고,
     * 더 최근이 내 줄일 확률이 높다.
     * 🔴 신설 당일 실측 오탐: 내 줄과 남의 줄이 1:1 동점이었는데 앞줄(남의 것)이 이겨,
     *   내가 선언한 적 없는 `엔진_수집.js` 를 「내가 선언한」이라고 보고했다.
     *   `>` 였으면 먼저 만난 줄이 계속 이긴다 — 보드는 위쪽이 오래된 트랙이라 방향이 정반대였다. */
    if (점수 > 0 && (!최고 || 점수 >= 최고.점수)) 최고 = { 점수, 트랙: 칸[2], 파일칸: 칸[3] };
  }
  return 최고;
}
const 내줄 = 내보드줄();
...
const 내표식 = new Set();
if (내줄) {
  for (const m of `${내줄.트랙} ${내줄.파일칸}`.matchAll(/\bF0\d{2}\b|\[v\d+\.\d+\]/g)) 내표식.add(m[0]);
}
