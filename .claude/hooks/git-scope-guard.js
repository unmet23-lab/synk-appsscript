 ...
 *   검사와 커밋 사이의 경합 창은 원리상 못 없앤다 — 무한 차단이 지키는 건 그 창이 아니라 인내심뿐이다. */
{
  ...
  if (커밋세그.length && 저장소인가) {
    ...
    /* ⑨-a **「지웠다」와 「제자리에서 고쳤다」를 가른다** (F189 · 2026-08-07).
     *
     * 🔴 왜 필요했나 — ⑨의 판정이 처음엔 「그 줄이 작업본에 **문자열로** 남아 있나」 하나뿐이었다.
     *   그러면 공유 문서의 그 줄을 **수정**하는 순간 원문이 사라지므로 되돌림으로 잡힌다. 실측:
     *   절단문서 ①-8 종결 커밋(`02cd32d`)이 앞 커밋의 진행표 한 줄에 8번을 끼워 넣는 편집인데 차단됐고,
     *   작업본은 HEAD 기준이라 스테일도 아니었다. **새는 방향이 나빴다**: 처방(그 줄 복원)을 따르면
     *   내 수정이 사라지므로 따를 수 없고, 남는 문이 `GIT_SCOPE_BYPASS=1` 뿐인데 그건 ⑨만이 아니라
     *   **전 규칙**을 끈다 — 따를 수 없는 처방이 우회를 정상 통로로 만든다(F103 형태).
     *
     * 🔑 그런데 「+ 줄이 하나라도 있으면 수정」은 **F187 을 그대로 통과시킨다.** 도구가 옛 판으로
     *   덮어쓴 자리도 diff 에선 `-남의줄 / +내줄` 한 블록으로 붙어 나오기 때문이다(회귀가 이걸 잡는다).
     *   그래서 통과에 **적극적 증거**를 요구한다 — 세 관문을 다 통과해야 「제자리 수정」이다:
     *     ① 그 `-` 줄이 속한 변경 블록에 잴 만한 길이의 `+` 줄이 **있다**(없으면 순수 삭제 = 되돌림).
     *     ② 그 `+` 줄이 남의 커밋이 **지운** 줄과 같지 않다 — 같으면 그 커밋의 **이전 판이 돌아온 것**이고
     *        그게 되돌림의 정의 그 자체다(F187 실사고가 정확히 이 모양이었다).
     *     ③ 그 `+` 줄이 사라진 줄과 **닮았다** — 앞뒤로 겹치는 부분이 60% 이상. 끼워 넣기·부분 수정은
     *        앞이나 뒤가 통째로 남지만, 다른 줄로 갈아치운 자리는 안 남는다.
     *   못 쟀거나 블록을 못 찾으면 **되돌림 쪽**으로 센다 — 판정 불가를 통과로 번역하지 않는다.
     *
     * 판정 자체는 `lib/되돌림판정.js` 가 진다 — git 도 파일시스템도 안 보는 순수 함수라, 훅 안에 두면
     *   프로세스를 띄워야만 만질 수 있어 관문 하나하나의 탐지력을 못 박을 수가 없다(변이 3건이 실제로
     *   그 자리로 샜다). 여기는 **읽기만** 한다 — 두 곳에 적으면 갈라진다. */
    const { 변경블록, 제자리수정인가 } = require(p.join(__dirname, 'lib', '되돌림판정.js'));

    /* ⑨ **되돌림** — 범위 안의 파일이 남이 **이미 커밋한 줄**을 지우고 있나 (F187 · 2026-08-07).
     *
    ...
    if (범위.length) {
      ...
      try {
        ...
        for (const f of g(['diff', 'HEAD', '--name-only', '--', ...범위]).split('\n').filter(Boolean).slice(0, 20)) {
          ...
          try {
            ...
            더한줄 = g(['diff', `${해시}^`, 해시, '--', f]).split('\n')
              .filter((l) => l.startsWith('+') && !l.startsWith('+++'))
              /* 짧은 줄(`}` · `});`)은 어디에나 있어 「있다」가 우연히 참이 된다 —
               * 새는 방향이 통과라 길이 하한을 둔다. 놓치면 ⑧이 받는다. */
              .map((l) => l.slice(1).replace(/\r$/, '').trim()).filter((l) => l.length >= 8);
          } catch { continue; }
          if (!더한줄.length) continue;
          let 지금 = '';
          try { 지금 = fs.readFileSync(p.join(gitCwd, f), 'utf8'); } catch { continue; }
          const 후보 = [...new Set(더한줄)].filter((l) => !지금.includes(l));
          if (!후보.length) continue;
          /* F189 — 여기서 「지웠다」와 「제자리에서 고쳤다」를 가른다. 위 `지금.includes` 는 둘을
           * 같은 모양으로 만든다: 그 줄을 수정해도 원문 문자열은 사라지기 때문이다. */
          let 내블록 = null;
          try { 내블록 = 변경블록(g(['diff', 'HEAD', '--', f])); } catch { 내블록 = null; }
          let 지운줄 = new Set();
          try {
            지운줄 = new Set(g(['diff', `${해시}^`, 해시, '--', f]).split('\n')
              .filter((l) => l.startsWith('-') && !l.startsWith('---'))
              .map((l) => l.slice(1).replace(/\r$/, '').trim()).filter((l) => l.length >= 8));
          } catch { /* 최초 커밋 — 되돌아갈 이전 판이 없다(관문②는 그냥 통과) */ }
          const 사라진 = 후보.filter((l) => !제자리수정인가(l, 내블록, 지운줄));
          if (사라진.length) 되돌림.push({ 파일: f, 해시: 해시.slice(0, 7), sid, 사라진 });
        }
      } catch { 되돌림 = []; }
      ...
      if (되돌림.length) {
        ...
        deny(`[git-scope-guard] 이 커밋은 **남이 방금 커밋한 ${총}줄을 지운다**(되돌림 · F187).`
          ...
          + '\n**제자리 수정은 아니다** — 그 줄을 고친 자리는 이미 걸러냈다(F189). 여기 남은 건 대체한 것이'
          + ' 없거나(순수 삭제), 그 커밋 **이전 판**이 돌아왔거나, 아주 다른 줄로 갈아치운 자리다.'
