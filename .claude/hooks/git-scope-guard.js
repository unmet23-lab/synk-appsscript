...
/* ⑧⑨ 은퇴 — 2026-08-19 유호님 지시.
 *
 * 둘 다 "이 파일을 다른 세션도 만졌는가"를 판정하려고 `tools/작업본소유자.js` 를 거쳐
 * 원격에 `git fetch` 를 시도했다. 그 판정은 **여러 세션이 같은 작업 트리를 공유하며
 * 보드 파일로 조율하던 체계**를 전제로 만들어졌는데, 그 체계(docs/_ops/보드 등)를
 * 오늘 이미 지웠다. 남은 것은 실측 5.4초짜리 타임아웃뿐이었다 — 커밋마다 없는 것을
 * 확인하려 드는 세금이었다.
 *
 ...
 * 아래 원문은 지우지 않고 남긴다 — 다시 여러 세션이 한 트리를 공유하게 되면 참고할 수 있게. */
process.exit(0);
 ...
 *   검사와 커밋 사이의 경합 창은 원리상 못 없앤다 — 무한 차단이 지키는 건 그 창이 아니라 인내심뿐이다. */
{
  ...
  if (커밋세그.length && 저장소인가) {
    ...
    if (범위.length) {
      let 소유 = null;
      let 되돌림 = [];
      /* ⚰ 이 규칙(⑦ 되돌림)의 정본 `tools/작업본소유자.js` 는 08-20 「보드 층을 걷는다」(39019553 ·
       *   유호 확정 — 조율은 하네스로)에서 삭제됐다. 아래 require 는 그날부터 늘 throw → catch 로
       *   ⑦ 전체가 **조용히 비활성**이었다(08-23 전수조사 실측). 이 표식이 그 침묵을 문서화한다 —
       *   ⑦·⑧을 하네스 시대에 맞게 걷을지·재정본화할지는 트랙.md 등재분이 판정한다. */
      try {
      ...
      let 항목 = null;
      /* ⚰ ⑦과 같은 소멸 정본(08-20 · 위 표식 참조) — 이 require 도 늘 실패해 ⑧은 「전부 모름」
       *   경로로만 돈다. 소유 판정(내것/남의것/함께)은 그날부터 실질 0 이다. */
      try {
