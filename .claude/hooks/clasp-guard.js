...
/* 🔓 배포 게이트를 «막기»에서 «알리기»로 내렸다 (유호 지시 2026-09-09 「이것도 풀어」)
 *
 * 유호님이 「gpt에 걸려있는 모든 권한을 줘」에 이어, 내가 「라이브 배포만 안 열었다」고 낸 뒤
 * 그 셋을 그대로 인용하며 「이것도 풀어」라 하셨다. 재확인이므로 그대로 따른다.
 *
 * ■ 무엇이 «알림»으로 내려갔나 — 절차를 재는 둘
 *     · 배포 게이트 종합(시험 미실행 · 미커밋 · 이종검수 미확인 …)
 *     · 워크트리에서의 push
 *   막지 않되 «무엇을 건너뛰는지»는 화면에 그대로 낸다. 사람이 보고 판단한다.
 *
 * ■ 그대로 «막는» 둘 — 절차가 아니라 «잃음·샘»을 재는 자리
 *     · clasp pull  = 읽기처럼 생겼는데 작업본 덮어쓰기다(F040). 미커밋에 닿으면 git 어디에도
 *                     사본이 없어 복구가 불가능하다. 유호님이 푸신 것은 「학생 앱으로 «나가는» 통로」이고
 *                     이건 방향이 반대다.
 *     · 보안 검사   = 토큰·자격증명이 코드에 박힌 채 라이브로 나가는 것. CLAUDE.md 안전 넷의
 *                     「자격증명」 자리라 배포 권한과 층이 다르다.
 *   🔴 이 둘을 푸는 것은 이 지시에 안 들어 있다고 읽었다. 다르면 유호님이 한 마디로 뒤집으신다.
 *
 * 되돌리려면 이 값을 false 로 두면 게이트가 다시 «막는다».
 */
const 게이트풀림 = true;
...
/** 게이트성 차단은 풀림 스위치를 따른다. 잃음·샘을 재는 자리는 이 함수를 안 쓴다. */
function 게이트(reason) {
  return 게이트풀림 ? 알린다(reason) : deny(reason);
}
...
if (callerCwd) {
  try {
    ...
    if (gitDir !== commonDir) {
      ...
      게이트(
        '[clasp-guard] 워크트리에서의 배포:\n' +
          `- 워크트리에서 clasp push/deploy 를 한다 (${path.basename(callerCwd)}${br ? ' · ' + br : ''})\n` +
