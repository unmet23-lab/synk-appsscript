...
/* 보드 정본 조각(`docs/_ops/보드/<지문>.md`) — 확장자는 `.md` 지만 **코드와 같은 이유로** 막는다.
 *
 * 🔴 실측 2026-08-18 (`local_9a6c0547`): 보드 선언을 `cat > docs/_ops/보드/<지문>.md <<EOF` 로 썼다.
 *   `board-guard` 는 등록 매처가 `Edit|Write|MultiEdit` 라 **한 번도 안 돌았다** — 트랙 충돌·칸
 *   상한·지문 검사가 통째로 건너뛰어졌다. 그날 남과 **82초 차**로 같은 트랙(F616)을 선언했고,
 *   둘 다 끝까지 지었다(F340 이 막으려던 그것 · 오늘 네 번째 경합).
 *   CLAUDE.md 「가드는 **등록층**에서 샌다 — 매처·필터가 훅이 막는 것을 다 잡는지 회귀로 검사한다」.
 *   그 조항이 이미 있었는데 이 짝(Bash ↔ board-guard)만 비어 있었다.
 *
 * ⚠ 막는 것은 **셸 쓰기**뿐이다 — `node tools/board-move.js`·`보드수거.js` 처럼 도구가 노드에서
 *   쓰는 것은 이 훅의 눈에 안 보이고, 그게 옳다(그 도구들은 자기 판정을 스스로 진다).
 * ⚠ 처방이 다르다 — 코드는 「`Edit` 로 고쳐라」지만 보드는 **「`Write` 로 써라」**다.
 *   board-guard 가 도는 통로가 그것뿐이라, 처방을 코드와 같이 주면 따라도 가드가 여전히 안 돈다. */
const 보드꼴 = 'docs[\\\\/]_ops[\\\\/]보드[\\\\/][^\\\\/]+\\.md';
const 보드RE = new RegExp(`(?:^|[\\\\/])${보드꼴}$`, 'i');
const 보드경로RE = new RegExp(`[^\\s'"|;&<>()]*${보드꼴}\\b`, 'gi');
/** 인용 안을 살릴지 정하는 판정 — 코드 경로 **또는** 보드 조각(`stripNonExecutedText` 의 keepQuoted). */
const 보호RE = new RegExp(`(?:\\.(${코드확장.join('|')})|${보드꼴})$`, 'i');
...
/* 보드 조각 전용 처방 — 위 `처방`(「Edit 로 고쳐라」)만 주면 **따라도 가드가 여전히 안 돈다.**
 * board-guard 가 도는 통로는 `Edit|Write|MultiEdit` 뿐이라, 여기서는 도구 이름을 콕 집어 준다. */
const 보드처방 =
  '\n\n→ 보드 줄은 **`Write` 도구로 쓴다** — 그래야 `board-guard` 가 돈다:'
  + '\n   · 남의 활성 줄과 **트랙이 겹치는지**(F340 — 겹친 채 나란히 지으면 둘 다 끝까지 간다)'
  + '\n   · 칸 200자·활성 12줄·전체 18줄 상한, 그리고 줄에 **내 지문**이 적혔는지'
  + '\n   조립은 `node tools/대기열.js --집기 #Qnn --파일 "…"` 이 해 준다 — 그 출력을 그대로 `Write`.'
  + '\n   완료 줄 이관은 `node tools/board-move.js "문구"`(그 도구는 노드에서 쓰므로 이 훅이 안 막는다).'
  + '\n   🔴 2026-08-18 실측: 셸로 선언해 이 가드가 통째로 건너뛰어졌고, **82초 차로 남과 같은 트랙**을'
  + '\n      선언해 둘 다 지었다(오늘 네 번째 경합).';
...
/** 문자열 안에서 보호 대상 경로만 뽑는다 — 코드 파일 **+ 보드 정본 조각**(위 `보드RE` 머리말). */
function 코드경로들(s) {
  const hits = [...(String(s).match(경로RE) || []), ...(String(s).match(보드경로RE) || [])];
  return [...new Set(hits)].filter((p) => !임시경로(p));
}

/** 이미 토큰으로 갈라 놓은 **한 인자**가 보호 대상인가 (경로RE 를 다시 돌리지 않는다 · F063).
 *  cp·mv 는 인자 자리로 뜻이 갈려서 문자열 스캔이 아니라 토큰 판정이 필요하다. */
const 코드대상인가 = (p) => (확장RE.test(String(p)) || 보드RE.test(String(p))) && !임시경로(p);

/** 걸린 것이 보드 조각인가 — 차단문의 **이름과 처방**이 여기서 갈린다(둘을 같이 주면 처방이 헛돈다). */
const 보드인가 = (경로들) => (경로들 || []).some((p) => 보드RE.test(되돌림(String(p))));
const 대상이름 = (경로들) => (보드인가(경로들) ? '보드 정본 조각' : '코드 파일');
...
const 실행부 = stripNonExecutedText(cmd, {
  keepQuoted: 보호RE,   // 코드 경로 + 보드 조각 — `> "docs/_ops/보드/x.md"` 를 놓치면 규칙②가 헛돈다
...
if (ip && ip대상.length) {
  out('deny', `[code-edit-guard] \`${ip.이름} ${ip.플래그}\` 가 ${대상이름(ip대상)}을 **제자리에서** 바꾼다 — \`${ip대상[0]}\``
...
for (const 규칙 of 쓰기규칙) {
  ...
  while ((m = 규칙.re.exec(실행부))) {
    for (const 조각 of 규칙.대상(m)) {
      ...
      out('deny', `[code-edit-guard] ${규칙.왜}로 ${대상이름(맞은것)}을 덮어쓴다 — \`${되돌림(맞은것[0])}\``
  ...
  for (const 후보 of 실제대상들(위치, 대상)) {
    ...
    out('deny', 상태 === 'unknown'
      ? `[code-edit-guard] \`${이름}\` 의 대상을 특정할 수 없다 — \`${되돌림(후보)}\``
        ...
        + 처방
      : `[code-edit-guard] \`${이름}\` 가 **이미 있는** ${대상이름([후보])}을 덮어쓴다 — \`${되돌림(후보)}\``
...
for (const { 형태, 본문 } of 본문들) {
  ...
  const 맞은것 = 코드경로들(본문);
  if (!맞은것.length) continue;
  out('deny', `[code-edit-guard] ${형태} 스크립트가 ${대상이름(맞은것)}에 쓴다 — \`${맞은것[0]}\``
