...
/* ── 세션보드 파일은 셸로 쓰지 않는다 (마찰 F635 · 2026-08-18) ─────────────────────
 *
 * 왜 여기인가 — `board-guard` 는 `Edit|Write|MultiEdit` 에만 등록돼 있다. 즉 **셸로 쓰면
 *   그 가드를 원리상 안 지난다.** 실측 2026-08-18: `printf … > docs/_ops/보드/<지문>.md` 가
 *   종료코드 0 · 143바이트 · **가드 발동 0** 으로 지나갔고, 같은 위반을 Write 로 내면 deny 였다.
 *   CLAUDE.md 세션 규약이 「그대로 Write — 써야 board-guard 가 겹침을 본다(F415)」로 사람 규율을
 *   깔아 두었지만, 하네스 기본 지시는 「가능하면 Bash 로 편집하라」라서 규율과 기본값이
 *   정면으로 반대를 가리킨다. 프로즈보다 기계로 막을 수 있으면 기계로 옮긴다(CLAUDE.md 신뢰성).
 *
 * 🔑 새 훅을 안 세우고 여기 얹는 이유는 이 파일 위쪽 「심장박동」 절과 같다 —
 *   **이 훅이 모든 셸 호출에 실제로 도는 유일한 훅**이고, 셸 문자열에서 대상을 뽑는 어려운 일은
 *   이미 여기서 치르고 있다. 판정을 두 곳에 적으면 갈라진다(F063).
 *
 * 🔑 `실행부` 가 아니라 **원본 `cmd`** 를 본다: 아래 `stripNonExecutedText` 는 `keepQuoted: 확장RE`
 *   라 «코드 확장자»만 살린다 — 인용된 `.md` 경로는 걷혀서 여기까지 안 온다.
 *
 * ⚠ 틀릴 때의 모습 (대가): 읽기만 하는 명령에 경로가 적혀 있는데 그 줄에 우연히 쓰기 낱말이
 *   있으면 막는다. 그래서 **쓰기 낱말이 있을 때만** 잡고, 읽기(`cat`·`grep`·`sed -n`)와
 *   git 명령은 통과시킨다. git 은 `git-scope-guard` 가 따로 지고, 보드 파일을 커밋하는 것은
 *   막을 일이 아니다.
 * ⚠ 닫은 것 1개: 「board-guard 를 지나지 않는 보드 쓰기」 통로. 남는 구멍은 PowerShell 전용
 *   낱말인데, 아래 목록에 함께 적었다(노트북이 없어 실측은 못 했다 — F348). */
{
  const 보드경로 = /(?:^|[\s'"=(])((?:[^\s'"|;&<>()]*[/\\])?docs[/\\]_ops[/\\]보드[/\\][^\s'"|;&<>()]*\.md)/;
  const m = 보드경로.exec(cmd);
  if (m && !/^\s*git\s/.test(cmd)) {
    const 대상 = m[1];
    const 쓰기 = [
      new RegExp(`>>?\\s*['"]?${대상.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`),  // 리다이렉트가 그 경로를 향한다
      /\bsed\s+(-[^\s]*\s+)*-[^\s]*i/,                    // sed -i (제자리 편집)
      /\btee\b/, /\bcp\b/, /\bmv\b/, /\brm\b/, /\btruncate\b/, /\bdd\b/,
      /\bSet-Content\b|\bAdd-Content\b|\bOut-File\b/i,    // PowerShell (못 쟀다)
      /\bopen\s*\([^)]*['"][wax]/,                        // python open(p,'w')
      /\bwriteFileSync\b|\bappendFileSync\b/,             // node
    ].some((re) => re.test(cmd));
    if (쓰기) {
      out('deny', `[code-edit-guard] 세션보드를 **셸로** 쓰려 한다 — \`${대상}\`\n`
        + 'board-guard 는 `Edit|Write|MultiEdit` 에만 등록돼 있어서, 이 통로로 쓰면 그 가드를 **원리상 안 지난다**.\n'
        + '실측 2026-08-18(F635): 같은 쓰기가 셸에선 종료코드 0 으로 지나갔고 Write 로는 deny 였다.\n'
        + '그 가드가 보는 것은 길이만이 아니다 — **트랙 충돌·유령 줄·남의 줄 편집**(F073)까지 본다.\n'
        + '→ `Write` 도구로 그 파일 전체를 쓴다(CLAUDE.md 세션 규약 · F415).\n'
        + '   완료 줄을 옮기는 것이면 손으로 쓰지 말고 `node tools/board-move.js "문구"` 를 쓴다(F046).\n'
        + '   정말 셸이어야 하는 자리라면 CODE_EDIT_BYPASS=1 을 붙이고, 왜 그런지를 커밋 메시지에 남긴다.');
    }
  }
}
