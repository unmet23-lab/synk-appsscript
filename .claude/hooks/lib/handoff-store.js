...
const 공용장부 = Object.freeze([
  ...
  'docs/버전_이력.md',
  'docs/지침_이력.md',
  /* 🔴 F293 — 대기열이 빠져 있었다. 이 파일은 보드와 **같은 성질**이다: 세션마다 자기 줄을 넣고
   *   층을 갱신하는 선언판이고, 남의 줄이 섞인 채 함께 커밋되는 것이 관행이다(실측 2026-08-12:
   *   살아있는 두 세션이 동시에 들고 있었다). 빠져 있으니 겹침이 «충돌»로 세어졌고, 대기열 자신이
   *   내미는 처방(「고르면 그 층을 갱신해 담는다」)이 그 겹침에 막혔다. */
  'docs/_ops/작업대기열.md',
]);

/** 폴더째 공용인 자리 — 보드 정본은 세션별 파일(F250) · 장부 행은 번호별 조각 파일(F250 후반)이라
 *  이름을 다 적을 수 없다. */
const 공용장부폴더 = Object.freeze(['docs/_ops/보드/', 'docs/_ops/장부/']);
