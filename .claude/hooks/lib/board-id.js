/* board-id — 보드 줄에서 **세션 지문**을 뽑는 한 곳 (마찰 F165).
 *
 * 왜 있나 (2026-08-07 실사고):
 *   보드 줄의 주인을 찾는 도구가 둘인데(인계문 생성기 `session-report.boardTrack`,
 *   이관 도구 `board-move` 원칙 ⑥) **둘 다 줄에 적힌 커밋 해시로만** 찾았다.
 *   해시는 이 저장소의 *관행*이지 규칙이 아니다 — 「양보 종결」처럼 코드 커밋이 하나도
 *   안 나간 트랙은 줄에 해시가 없다. 그래서 세션 `a0b27c60` 의 인계문이 「내 줄을 못 찾았다,
 *   정말 없으면 멈춰라」로 나갔고, 다음 세션은 **보드에 멀쩡히 있는 자기 줄**을 두고
 *   멈출 뻔했다. board-move 쪽 증상은 반대다 — 재료가 없으니 원칙 ⑥이 그냥 통과한다
 *   (살아있는 세션의 줄을 아무 경고 없이 아카이브로 옮긴다 · F146 이 그 사고였다).
 *
 * 🔑 재료를 늘리는 게 아니라 **재료를 한 곳에서 만든다.** 두 도구가 각자 「어느 줄이
 *   누구 것인가」를 적으면 갈라지고, 갈라진 쪽의 증상은 언제나 「통과」다(CLAUDE.md 신뢰성 ④).
 *   정책은 각자다 — 인계문은 **모르면 모름으로 두고**(남의 줄을 이어받는 게 더 큰 사고),
 *   이관은 **의심되면 멈춘다**(안 옮겨도 잃는 게 없다). 뽑는 자리만 공유한다.
 *
 * ⚠ 접두를 **요구한다**. 접두 없는 8자리 hex 는 커밋 해시와 생김새가 같아서, 그냥 8자리를
 *   지문으로 읽으면 남의 커밋 해시가 적힌 줄이 내 줄로 판정된다(새는 방향 = 이어받기).
 */
'use strict';

/** 세션 id → 보드에 적는 지문(8자리). `작업본소유자.짧게` 와 같은 규칙 —
 *  둘이 갈라지면 도구마다 다른 줄을 내 줄이라 하므로 회귀가 동일성을 못박는다. */
function 지문(sid) {
  return String(sid || '').replace(/^[a-z]+_/, '').slice(0, 8).toLowerCase();
}

/* ── 환경에서 세션 id 를 «뽑는» 통로 (마찰 F634 · 2026-08-18) ────────────────
 *
 * 왜 갈랐나 — **축들이 서로 다른 규격을 요구하는데 뽑는 함수가 하나였다.**
 *   ㉠ 연락 축(커밋 트레일러): 「이 세션에 **메시지를 보낼 수 있는** id」여야 한다.
 *      F628 이 이 축을 정하고 `CLAUDE_CODE_SESSION_ID` 로 내려가는 것을 회귀로 금지했다 —
 *      그건 내부 에이전트 id 라, 거기까지 폴백하면 트레일러의 뜻이 「연락 가능한 id」에서
 *      「그냥 구분자」로 내려앉는다. 그 판단은 이 축에서 **옳다.**
 *   ㉡ 보드 축(파일명·주인 판정): `docs/_ops/보드/<지문>.md` 가 되어야 하므로 **hex 8자리**여야 한다.
 *      `파일지문()` 이 `/^([0-9a-f]{8})\.md$/` 로 못박는 그 규격이다.
 *   ㉢ 「아무 id 면 되는」 축: 락 소유자·상태 파일 이름. 겹치지만 않으면 된다.
 *
 * 🔴 실측 2026-08-18 (클라우드 컨테이너) — 세 후보가 이렇게 갈린다:
 *      HOST   `CLAUDE_CODE_HOST_SESSION_ID`   = **없음**            → 지문 `''`
 *      REMOTE `CLAUDE_CODE_REMOTE_SESSION_ID` = `cse_01Scr7GJ3A…`  → `01scr7gj` = **hex 아님**
 *      SESSION`CLAUDE_CODE_SESSION_ID`        = `79415d5c-36d0-…`  → `79415d5c` = **유일하게 hex**
 *    그래서 F628 의 폴백 사슬(HOST→REMOTE)만으로는 보드 축이 원리상 안 선다. 폴백을 아무리
 *    늘려도 **한 함수로는 못 푼다** — 축을 갈라야 한다. 그전까지 클라우드 세션은 여덟 자리에서
 *    빈 지문으로 돌았고(`board.js`·`board-move.js`·`대기열.js --집기` 등), 새는 방향은 「통과」였다.
 *
 * 🔑 **HOST 가 있으면 세 통로 다 지금까지와 바이트 단위로 같다.** 폴백은 HOST 가 없을 때만 탄다 —
 *   즉 노트북 세션의 판정은 하나도 안 바뀌고, 옛 커밋·옛 보드 줄의 주인도 그대로다. 이 성질이
 *   없으면 「과거 판정이 갈린다」는 대가를 치러야 했다(F634 신고문 ㉠의 ⚠ 그 자리).
 *
 * ⚠ 틀릴 때의 모습 — 이 셋이 **같은 값을 줄 것이라고 부르는 쪽이 기대하면** 조용히 갈린다.
 *   클라우드에서 `연락지문()`≠`보드지문()` 이다(`01scr7gj` vs `79415d5c`). 그러니 커밋 트레일러로
 *   보드 파일을 찾거나, 보드 지문으로 세션에 메시지를 보내려 하면 안 된다. 그 둘을 잇는 것은
 *   **사람이 적은 보드 줄**뿐이다. (⚰tests/지문축.test.js 가 이걸 못박았었는데 08-20 보드 층
 *   철거와 함께 삭제됐다 — 08-23 실측. 지금 이 갈라짐·직독 금지를 기계로 잡는 층은 없다.)
 * ⚠ 닫은 것 1개: 호출부가 `process.env` 를 각자 읽던 통로(실측 26파일·40곳).
 */

/** 이 환경에서 «메시지를 보낼 수 있는» 세션 id 원문 — 트레일러·연락 축. HOST → REMOTE. */
function 연락id(env) {
  const e = env || process.env;
  return String(e.CLAUDE_CODE_HOST_SESSION_ID || e.CLAUDE_CODE_REMOTE_SESSION_ID || '');
}

/** 연락 축의 8자리 지문. 값이 없으면 `''` — 「주인 없음」이지 「내 것」이 아니다. */
function 연락지문(env) {
  return 지문(연락id(env));
}

/** 이 환경에서 «보드 파일명이 될 수 있는» 세션 id 원문 — 주인 판정 축. HOST → SESSION(hex8 일 때만).
 *
 *  REMOTE 로는 **안 내려간다**: 그 값은 base62 라 `파일지문()` 이 원리상 못 읽는다(위 실측).
 *  SESSION 도 **hex8 로 시작할 때만** 받는다 — 모양이 안 맞는 값을 통과시키면 그 세션은 자기 파일을
 *  만들 수는 있는데 아무도 그 파일의 주인을 못 읽는 상태가 된다(가장 나쁜 쪽: 조용히 고아가 된다). */
function 보드id(env) {
  const e = env || process.env;
  const host = String(e.CLAUDE_CODE_HOST_SESSION_ID || '');
  if (host) return host;
  const sess = String(e.CLAUDE_CODE_SESSION_ID || '');
  return /^[0-9a-f]{8}(?:[-0-9a-f]*)?$/i.test(sess) ? sess : '';
}

/** 보드 축의 8자리 지문.
 *
 * 🔴 **HOST 가 준 값은 모양을 심사하지 않는다** — 심사하면 「HOST 가 있으면 옛 판정과 바이트로
 *   같다」는 이 통로의 근본 약속이 깨진다. 처음 짤 때 여기서 틀렸고 CI 가 잡았다(실측 2026-08-18:
 *   `인계문수거`·`이종검수`·`컨텍스트예산` 등이 `sid-MINE`·`local_TESTID-1234` 같은 **hex 아닌**
 *   id 로 도는데, 그것을 규격 미달로 떨어뜨리니 「내 파일이 커밋에 없다」가 됐다).
 *   운영에서 HOST 는 언제나 `local_<uuid>` 라 hex 지만, **그건 관행이지 이 함수가 걸 조건이 아니다.**
 *
 * 🔑 규격(hex8)을 요구하는 것은 **폴백뿐이다.** 폴백은 내가 새로 연 갈래라 옛 약속이 없고,
 *   모양이 안 맞는 값을 통과시키면 그 세션은 자기 보드 파일을 만들 수는 있는데 아무도 그
 *   파일의 주인을 못 읽는 고아가 된다(`파일지문()` 이 hex8 만 읽는다). */
function 보드지문(env) {
  const e = env || process.env;
  const host = String(e.CLAUDE_CODE_HOST_SESSION_ID || '');
  if (host) return 지문(host);
  const f = 지문(보드id(e));
  return /^[0-9a-f]{8}$/.test(f) ? f : '';
}

/** 이 세션을 가리키는 **아무 id** — 「겹치지만 않으면 되는」 자리 전용(락 소유자·상태 파일 이름·기록).
 *
 *  위 두 축과 **쓰임이 다르다.** 여기서 나온 값으로 주인을 판정하거나 메시지를 보내면 안 된다 —
 *  그래서 세 함수를 갈라 두었다. 이 자리는 「값이 있기만 하면」 되므로 폴백이 가장 넓다.
 *  ⚠ 옛 통로들은 여기서 `'unknown'`·`'nosid'` 로 접었다 — 그러면 **모든 세션이 같은 이름**을 써서
 *    락과 상태 파일이 서로를 덮는다. 이제 클라우드에서도 실제 값이 잡히므로 그 겹침이 사라진다. */
function 세션id(env) {
  const e = env || process.env;
  return String(e.CLAUDE_CODE_HOST_SESSION_ID || e.CLAUDE_CODE_REMOTE_SESSION_ID || e.CLAUDE_CODE_SESSION_ID || '');
}

/** 줄에 **직접 적힌** 세션 지문들. `local_302d8acd` 처럼 접두가 붙은 것만 센다. */
function 줄의지문(line) {
  const 표 = String(line || '').match(/\b[a-z]+_[0-9a-f]{8}\b/gi) || [];
  return [...new Set(표.map((t) => t.slice(t.indexOf('_') + 1).toLowerCase()))];
}

/** 이 줄이 그 세션 것이라고 **줄 스스로 말하는가.** 추측하지 않는다 — 적혀 있어야 참이다. */
function 줄이말하나(line, sid) {
  const 나 = 지문(sid);
  return !!나 && 줄의지문(line).includes(나);
}

/** 보드 **파일 이름**이 밝히는 주인 — `docs/_ops/보드/<지문>.md` 의 그 지문 (마찰 F338).
 *
 * 🔑 줄 **안**에 무엇이 인용돼 있든, 파일이 주인을 밝히면 그 말이 최종이다.
 *   F250 이후 보드 정본은 세션별 파일이고 「내 줄은 내 파일에만」은 board-guard 가 기계로
 *   강제한다 — 즉 파일 경로는 관행이 아니라 **규칙**이고, 줄 내용(지문 인용·해시 인용)보다
 *   무겁다. 2026-08-12 실측이 그 차이였다: 죽은 `ffa75dcb` 의 인계문이 산 `e556253e` 의
 *   활성 줄을 「▶ 내 보드 줄」로 실어 내보냈다. 그 줄은 `e556253e.md` 안에 있었는데
 *   ㉠제목에 `ffa75dcb` 를 인용하고 ㉡상태 칸에 ffa75dcb 의 커밋 `f5fcfa30` 을 인용해서,
 *   해시 폴백이 그것을 먼저 물었다(파일명 사전순 `e…` < `f…` 라 내 줄보다 위였다).
 *   결과물은 **죽은 세션의 인계문이 남의 활성 트랙을 「이어라」로 지시하는** 모양 —
 *   F073·F165·F170·F202 가 금지한 바로 그것을 장치가 시킨다.
 *
 * ⚠ 파일 이름이 지문 꼴이 아니면(`유물-*.md` · 픽스처 `sess.md`) `''` 다 — 「주인 없음」이지
 *   「내 것」이 아니다. 부르는 쪽은 그 둘을 절대 같게 다루면 안 된다(빈 문자열끼리 `===` 로
 *   비교하면 주인 없는 파일이 통째로 내 것이 된다 · 새는 방향은 여기서도 「통과」다). */
function 파일지문(파일명) {
  const base = String(파일명 || '').split(/[\\/]/).pop();
  const m = base.match(/^([0-9a-f]{8})\.md$/i);
  return m ? m[1].toLowerCase() : '';
}

module.exports = { 지문, 줄의지문, 줄이말하나, 파일지문, 연락id, 연락지문, 보드id, 보드지문, 세션id };
