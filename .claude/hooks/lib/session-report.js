...
function boardTrack(cwd, commits, sid) {
  ...
  if (나) {
    const 내줄들 = rows.filter((r) => 보드id.파일지문(r.파일) === 나).map((r) => r.줄);
    if (내줄들.length) {
      const 고름 = 내줄들.find((l) => 보드id.줄이말하나(l, sid))
        || 내줄들.find((l) => hashes.some((h) => l.indexOf(h) !== -1))
        || 내줄들[내줄들.length - 1];
      const r = 뽑기(고름, '파일');
      if (r) return r;
    }
  ...
  const 무주 = rows.filter((r) => !보드id.파일지문(r.파일)).map((r) => r.줄);
  for (const line of 무주) {
...
function 이음매(지문) {
  return 'SYNK 이어서 작업한다 — 이 글은 ' + (지문 ? `**끝난 세션 \`${지문}\`** 이` : '**끝난 세션**이')
    + ' 남긴 것이라 아래의 「나·내」는 전부 그 세션이다. **너는 다른 세션이다** — 네 지문은 '
...
function buildHandoff(cwd, fallbackId, opts) {
  ...
  return [
    ...
    track ? `▶ 그 세션의 보드 줄: ${track.track}` : null,
    ...
    track
      ? (종결줄(track.state)
        ...
        : '위 「상태/다음」에 적힌 다음 할 일부터 이어라. 열기 직전 `git log --oneline -10` 으로 ' +
          ...
          '⚠ 그 줄은 **끝난 세션의 보드 파일**에 있다 — 고치지 말고 `docs/_ops/보드/<네 지문>.md` 에 ' +
...
module.exports = { dirtyCount, myCommits, hostSessionId, boardTrack, 무주줄, 종결줄, buildHandoff, frame, blockOrder, writeHandoffFile };
