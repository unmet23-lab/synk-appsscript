...
function buildHandoff(cwd, fallbackId, opts) {
  ...
  return [
    ...
    track
      ? (종결줄(track.state)
        ? '▶ **이 트랙은 종결로 적혀 있다**(상태 칸이 ✅ 로 열린다) — 위 「상태/다음」의 ⚠·⏳ 잔여는 ' +
          '**배경이지 지시가 아니다.** 「남이 배포하면 따라 나가는 것」과 「내가 더 할 일」이 거기선 같은 모양이다. ' +
          새트랙
...
module.exports = { dirtyCount, myCommits, hostSessionId, boardTrack, 무주줄, 종결줄, buildHandoff, frame, blockOrder, writeHandoffFile, 낡음, 낡음경고 };
