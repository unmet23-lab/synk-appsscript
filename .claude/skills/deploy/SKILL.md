...
4-B. **이종 검수 (GPT/Codex) — 3단 파이프라인(유호님 설계 2026-08-05).** **커밋 전에.** 대상이 미커밋 변경이라 커밋한 뒤엔 볼 것이 달라진다.
   ```bash
   ...
   픽 전환 `--검수 luna`(기본 sol · 둘 다 max · 근거=tools/모델정책.js) · 유호님 「빠르게」 = ① 생략+`--버그만`.
...
### 이종 검수 게이트 (4-B의 기계 층 — 2026-08-05 도입)
