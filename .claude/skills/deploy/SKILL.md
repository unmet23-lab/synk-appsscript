...
4-B. **①배포 검수 (GPT/Codex) — 3단 파이프라인(유호님 설계 2026-08-05).** **커밋 전에.** 대상이 미커밋 변경이라 커밋한 뒤엔 볼 것이 달라진다.
   > 🔑 GPT 역할은 셋이고 이게 **①번**이다(08-06 이름 분리) — ②설계 심문(✅켜짐 · `--심문 <문서>` · sol/max)·③물리 작성과 **다른 일**이다. 지도 = `docs/AI_스택_가이드.md` §1-2.
   ```bash
   ...
   픽 전환 `--검수 sol`(**기본 luna** — 유호님 확정 08-05 "sol 쓰니까 너무 비싸다" · 둘 다 max · 정본=`tools/모델정책.js:78`) · 유호님 「빠르게」 = ① 생략+`--버그만`.
