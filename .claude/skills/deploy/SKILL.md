...
4-B. **이종 검수 (GPT/Codex)** — **커밋 전에.** 대상이 미커밋 변경이라 커밋한 뒤엔 볼 것이 달라진다.
   ```bash
   node tools/codex-review.js            # 미커밋이 있으면 그것, 없으면 HEAD
   node tools/codex-review.js --검수 luna # 빠른 쪽 픽 — 선택지는 sol·luna 둘뿐, 둘 다 max 추론(유호 확정 2026-08-05 · 근거=tools/모델정책.js)
   ```
   ChatGPT 구독 로그인으로 돌아 **추가 비용 0**, 읽기 전용 샌드박스(쓰기 차단은 2026-08-05 실측), xhigh 실측 5~8분(max 는 더 걸릴 수 있다).
