...
function 회고_알게된것_(ss, sid, 시즌) {
  ...
  if (dg && dg.getLastRow() >= 2) {
    ...
    if (c('학생번호') > -1 && c('시작점') > -1) dg.getRange(2, 1, dg.getLastRow() - 1, dg.getLastColumn()).getValues().forEach(function (r) {
       ...
       *   적용 기준 §2 「관측과 성향」(빼되 원관측과 정정 이력은 구분해 남긴다) · 철학 §5. 현재 효력이 있는 부정이 없으면 옛 동작 그대로. */
      let 고침 = null;
      if (c('학생고침') > -1) { try { 고침 = JSON.parse(r[c('학생고침')] || 'null'); } catch (x) { 고침 = null; } }
      /* 문형별 마지막 명시 판정을 읽는다. 옛 단일 고침과 새 누적 이력을 모두 읽으며, 다른 문형의 정정이 앞선 부정을 지우지 않는다.
       *   현재 열을 통째로 비우지 않으므로 재채점 뒤 새로 생긴 자리는 남는다. 같은 문형에 나중에 «맞아요»라고 답하면 다시 포함한다. */
      const 부정한자리 = Object.create(null);
      진단부정문형_(고침).forEach(function (g) { 부정한자리[g] = 1; });
      const 현재다음 = Array.isArray(다음) ? 다음.map(String) : [];
      const 남은다음 = 현재다음.filter(function (g) { return !부정한자리[g]; });
